...
TL;DR: 本文研究了多模态视觉语言模型中的成员推理攻击，并提出了神经科学启发的拓扑正则化框架来增强模型对隐私攻击的抵抗能力。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛部署，隐私泄露风险增加。现有研究主要关注单模态系统的隐私攻击，而多模态模型对隐私攻击的脆弱性尚未充分探索。

Method: 引入神经科学启发的拓扑正则化框架，在BLIP、PaliGemma 2和ViT-GPT2三个视觉语言模型上测试，使用COCO、CC3M和NoCaps三个基准数据集。

Result: 实验显示，神经启发的VLM在BLIP模型上使MIA攻击成功率下降24%平均ROC-AUC，同时保持相似的模型效用（MPNet和ROUGE-2指标）。

Conclusion: 神经启发的多模态视觉语言模型对隐私攻击具有更强的抵抗能力，且不会显著影响模型性能，为多模态模型的隐私保护提供了有效方法。
...
Result: Inferix专门设计用于世界模拟，与高并发系统（如vLLM或SGLang）和经典视频扩散模型（如xDiTs）区分开来。支持高效、可变长度和高质量的视频生成，具备交互式视频流和性能分析功能。

Conclusion: Inferix作为一个专注于世界模拟的专用推理引擎，通过优化的半自回归解码和集成评估基准，为世界模型的探索和发展提供了重要工具，希望社区共同推进其发展。
...
TL;DR: 本文提出了一种基于深度强化学习的自适应视频对象识别方法LTED-Ada，在资源受限设备上通过混合边缘检测和本地跟踪策略来平衡识别精度和延迟要求。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）上的快速准确视频对象识别面临挑战，需要平衡计算密集的边缘检测和轻量级本地跟踪之间的权衡。
...
Conclusion: LTED-Ada方法在单设备和多设备场景下都能有效平衡帧率、识别精度和延迟要求，提供了一种高效的视频对象识别解决方案。
...
TL;DR: DeeAD是一个无需训练的、基于动作引导的早期退出框架，通过评估中间轨迹的物理可行性来加速视觉语言动作（VLA）模型的推理过程，在保持规划质量和安全性的同时显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作（VLA）模型在自动驾驶中统一了感知、推理和轨迹生成，但由于深度transformer堆栈导致推理延迟显著，需要一种高效的加速方法。

Method: 提出DeeAD框架，不依赖置信度分数，而是在预测轨迹与轻量级规划先验（如导航或低精度规划）在可容忍偏差（<2m）内对齐时终止推理；引入多跳控制器根据分数变化率自适应跳过冗余层。

Result: 在Bench2Drive基准测试中，实现了高达28%的transformer层稀疏度和29%的延迟降低，同时保持了规划质量和安全性。

Conclusion: DeeAD能够无缝集成到现有VLA模型（如ORION）中，无需重新训练，为自动驾驶规划提供了高效的推理加速解决方案。
...
TL;DR: DinoLizer是基于DINOv2的模型，用于定位生成式修复图像中的篡改区域。通过在DINOv2上添加线性分类头，在14×14补丁分辨率下预测篡改，使用滑动窗口策略处理大图像，在多个修复数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在定位生成式修复图像中的篡改区域方面存在局限，需要开发更准确和鲁棒的检测方法。

Method: 基于DINOv2预训练模型，添加线性分类头处理ViT的补丁嵌入，使用滑动窗口策略处理大图像，并通过后处理优化二值篡改掩码。

Result: DinoLizer在多个修复数据集上超越最先进的局部篡改检测器，平均IoU提高12%，对常见后处理操作保持鲁棒性。

Conclusion: DinoLizer展示了Vision Transformers在篡改定位任务中的强大表示能力，通过广泛的消融研究证实了其优越性。
...
TL;DR: CANVAS是一个用于评估视觉语言模型在工具调用界面设计任务中性能的基准，包含598个基于工具的设计任务，涵盖设计复制和设计修改两种任务类型。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估基于工具的设计性能，而理解视觉语言模型在设计软件中迭代编辑UI设计的能力对于其与设计师协作的潜力至关重要。

Method: 从3.3K个移动UI设计中采样，创建包含598个工具设计任务的基准，支持通过上下文工具调用来逐步更新设计，包括设计复制和设计修改两种任务类型。

Result: 领先模型展现出更策略性的工具调用，提高了设计质量，同时识别出模型常见的错误模式。

Conclusion: CANVAS基准揭示了视觉语言模型在工具调用界面设计中的能力，为未来增强基于工具的设计能力提供了指导。
...
TL;DR: 提出文本引导语义图像编码器（TIE），通过文本查询条件化生成图像表示，提升视觉语言模型性能并显著改善推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统图像编码器独立预训练，对下游任务和文本查询不敏感，限制了视觉语言模型的性能。

Method: 开发文本引导语义图像编码器（TIE），使图像表示能够根据输入文本查询进行条件化生成。

Result: 在1B和3B规模上，TIE模型在九个图像到文本基准测试中平均提升1.5和1.3分，DocVQA和InfoVQA任务提升达6分，同时仅使用一半图像块，推理效率显著提高。

Conclusion: TIE通过文本条件化训练有效优化编码器捕捉关键视觉特征，增强模型可解释性和查询特定定位能力。
...
TL;DR: Δ-NeRF是一个用于增量式神经辐射场优化的模块化残差框架，通过残差控制器、不确定性感知门控机制和视图选择策略，实现在不访问历史数据的情况下优化NeRF模型，显著减少训练时间和数据需求。
...
Method: 提出Δ-NeRF框架，包含：1）残差控制器向冻结的基础NeRF注入逐层修正；2）不确定性感知门控机制自适应结合基础和优化预测；3）视图选择策略减少训练数据；4）知识蒸馏压缩模型。

Result: 在卫星图像上实验显示，Δ-NeRF性能与联合训练相当，训练时间减少30-42%，数据需求减少47%。PSNR比简单微调提高43.5%，在某些指标上超越联合训练。

Conclusion: Δ-NeRF提供了一种有效的增量式NeRF优化方法，解决了灾难性遗忘问题，显著提升了训练效率，在卫星地形分析等应用中具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖标注数据集或手工规则，存在数据稀缺问题。StM旨在增强生成视频合成的控制能力，通过无监督方式学习复杂组合动态。

Method: 将未标注视频分割为动态前景和背景层，通过自组合学习主体与场景交互；采用变换感知训练流程，包含多层融合和增强实现可负担感知合成，以及保持前景保真度的身份保持损失。

Result: 实验表明StM在定量基准测试和人类/VLLM定性评估中均优于最先进方法。

Conclusion: StM框架通过分割-合并策略有效解决了生成视频合成的控制增强和数据稀缺问题，实现了更真实的视频生成效果。
...
TL;DR: Sphinx是一个针对视觉感知和推理核心认知原语的合成环境，通过程序化生成包含图案、图块、图表、图标和几何原语的谜题，并提供可验证的真实解。该基准涵盖25种任务类型，评估显示最先进的GPT-5模型准确率仅为51.1%，远低于人类表现。强化学习与可验证奖励(RLVR)能显著提升模型在这些任务上的准确性。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够精确评估和构建大规模数据集的视觉感知与推理基准，针对核心认知原语进行系统化测试。

Method: 使用程序化生成方法创建包含多种视觉元素的谜题，每个谜题都配有可验证的真实解。涵盖对称检测、几何变换、空间推理、图表解释和序列预测等25种任务类型。

Result: 评估显示当前最先进的大型视觉语言模型(GPT-5)在Sphinx基准上的准确率仅为51.1%，远低于人类表现。使用强化学习与可验证奖励(RLVR)方法能显著提升模型性能。

Conclusion: Sphinx基准揭示了当前视觉语言模型在核心认知任务上的局限性，而强化学习与可验证奖励方法为提升多模态推理能力提供了有前景的途径。
...
TL;DR: RefTr是一种用于血管树中心线生成的3D图像到图模型，通过汇合轨迹的循环精炼来生成中心线图。该模型采用生产者-精炼器架构，基于Transformer解码器，能够保持高召回率并减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 在临床任务中，准确检测血管树中心线并保持正确树状拓扑至关重要。保持高召回率尤为关键，因为遗漏小分支可能导致致命错误。

Method: 使用生产者-精炼器架构：生产者提出初始汇合轨迹，精炼器循环精炼这些轨迹形成最终中心线图。引入高效的非极大值抑制算法来合并重复分支。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前SOTA的召回率和相当的精密度，同时提供更快的推理速度和显著更少的参数。

Conclusion: RefTr展示了作为3D医学成像中血管树分析的新最先进框架的潜力，在保持高召回率的同时减少了参数数量并提高了效率。
...
Method: 使用两个相同相机组件在9个不同场景中捕获图像，涵盖10个焦距（28-70mm）和5个光圈（f/2.8-f/22），共50种光学配置，每个场景2000张图像。

Result: 创建了包含18000张高分辨率图像的数据集，支持几何和光学效应的受控分析，涵盖深度估计、景深渲染、去模糊、3D场景重建和新视角合成等任务。

Conclusion: 该数据集填补了真实世界光学泛化研究的空白，展示了当前最先进方法在真实光学条件下的挑战，并发布了数据集、校准文件和评估代码以支持可重复研究。
...
TL;DR: 该论文提出了首个大规模无监督视觉记忆性数据集，包含82,000多个视频及其描述性回忆数据，用于建模视觉内容的记忆性信号。
...
TL;DR: 提出一种从立体雾天图像序列中动态估计雾模型参数的方法，通过同时优化所有参数避免误差传播，可集成到现有SLAM系统中，并创建了首个包含真实雾天道路场景的立体数据集SDIRF。


<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾参数容易导致误差传播，且假设雾全局均匀无法处理真实世界中的不均匀雾况，需要开发能动态适应真实雾况的参数估计方法。

Method: 通过求解新型优化问题同时估计所有雾参数，假设雾仅局部均匀以处理全局不均匀的真实雾，可作为附加模块集成到现有视觉SLAM或里程计系统中。

Result: 在合成雾数据和真实SDIRF数据集上的实验表明，该方法在合成数据上获得最准确估计，在真实雾中适应性更好，优于现有方法。

Conclusion: 提出的雾参数估计算法能有效处理真实不均匀雾况，SDIRF数据集为雾天视觉感知研究提供了重要资源，代码和数据集已开源以推动相关研究。
...
Method: 提出V^2-SAM框架，包含：1）基于DINOv3特征的跨视角锚点提示生成器（V^2-Anchor），建立几何感知对应；2）跨视角视觉提示生成器（V^2-Visual），通过新颖的视觉提示匹配器从特征和结构角度对齐ego-exo表示；3）多专家设计和后验循环一致性选择器（PCCS）自适应选择最可靠的专家。

Result: 在Ego-Exo4D（ego-exo对象对应）、DAVIS-2017（视频对象跟踪）和HANDAL-X（机器人就绪跨视角对应）上进行了广泛实验，验证了V^2-SAM的有效性。

Conclusion: V^2-SAM在多个跨视角对应任务中实现了新的最先进性能，成功将SAM2从单视角分割扩展到跨视角对应场景。
...
TL;DR: 提出了Null-Text Test-Time Alignment (Null-TTA)方法，通过在推理时优化无分类器引导中的无条件嵌入来对齐扩散模型，避免奖励黑客问题，并在语义相干流形上实现对齐。


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法倾向于欠优化或过优化（奖励黑客）目标奖励函数，需要一种更稳健的方法来防止奖励黑客并保持跨奖励泛化能力。

Method: 通过优化无分类器引导中的无条件嵌入而非操纵潜在变量或噪声变量，使对齐在语义相干流形上进行，直接引导模型的生成分布朝向目标奖励。
...
TL;DR: 本研究评估了一种深度学习模型，用于减少桥小脑角池对比增强T1加权MRI的造影剂剂量。通过模拟低剂量T1ce图像，训练DL模型从低剂量图像恢复标准剂量图像质量，结果显示在10%-30%标准剂量下即可获得良好的图像质量和诊断性能。


<details>
  <summary>Details</summary>
Motivation: 减少MRI检查中造影剂的使用剂量，降低患者风险和治疗成本，同时保持足够的图像质量用于临床诊断。

Method: 多中心回顾性研究，使用前庭神经鞘瘤患者的T1和T1ce图像模拟低剂量T1ce，训练深度学习模型从低剂量图像恢复标准剂量图像质量，评估图像质量和分割性能。

Result: 随着输入剂量增加，恢复图像的SSIM从0.639增加到0.993，PSNR从21.6dB增加到41.4dB。在10%输入剂量下，使用DL恢复的T1ce进行分割将Dice系数从0.673提升到0.734，HD95从2.38mm改善到2.07mm。10%和30%输入剂量的DL恢复图像均表现优秀。

Conclusion: 深度学习模型显著改善了桥小脑角池低剂量MRI的图像质量，使得在仅使用10%-30%标准剂量的情况下即可实现病变检测和诊断特征化。
...
<details>
  <summary>Details</summary>
Motivation: 视频具有天然的时间连贯性，但现有轻量级模型难以有效捕捉复杂的时间动态。通过强制中间层嵌入的平滑性，可以更好地对齐视频的时间特性。

Method: 使用高斯随机游走建模连续帧中间层嵌入的变化，惩罚表示上的突变，促进低加速度的解决方案。

Result: 在Kinetics-600上提升准确率3.8%-6.4%；MoViNets模型家族在各自FLOP约束下提升SOTA 3.8%-6.1%；MobileNetV3和MoViNets-Stream在相近内存占用下提升4.9%-6.4%。

Conclusion: 平滑正则化技术能有效提升轻量级视频识别模型的性能，通过时间连贯性约束使模型更好地捕捉视频动态。
...
TL;DR: 本文提出了一个开放词汇组合解释框架，利用开放词汇语义分割模型生成掩码来计算神经元激活与任意概念之间的组合解释，解决了传统方法依赖人工标注数据的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统组合解释方法依赖人工标注数据集，限制了其应用范围和概念灵活性。本文旨在开发一个能够探索神经元对任意概念和数据集编码信息的框架。
...
Conclusion: 该框架显著扩展了组合解释的适用范围，使其能够处理任意概念和数据集，为理解神经元如何编码信息提供了更强大的工具。
...
TL;DR: 本文介绍了UruDendro4数据集，包含102个火炬松横截面图像样本，用于树木年轮检测和体积建模研究。该数据集在多个树干高度采集样本，支持基于横截面图像的年木材估算体积模型开发。


<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，提供包含多个树干高度样本的数据集，以支持树木年轮检测和体积建模研究，改进传统手动测量方法的不精确和耗时问题。

Method: 创建UruDendro4数据集，包含102个火炬松横截面图像样本，每个样本都手动标注了年生长轮。使用最先进的方法（特别是DeepCS-TRD方法）在该数据集上建立自动年轮检测的性能基线。

Result: DeepCS-TRD方法在UruDendro4数据集上取得了最佳性能：平均精度0.838，平均召回率0.782，适应兰德误差得分0.084。消融实验验证了最终参数配置，并证明包含该数据集的训练能提高模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据稀缺的空白，支持树木年轮检测和体积建模研究。DeepCS-TRD方法在该数据集上表现出色，为自动年轮检测提供了可靠的性能基准。
...
TL;DR: BUSTR是一个无需配对图像-报告监督的多任务视觉语言框架，用于生成乳腺超声报告，通过结构化描述符和影像组学特征构建报告，在多个数据集上提升了报告生成质量和临床效果。


<details>
  <summary>Details</summary>
Motivation: 乳腺超声报告生成面临缺乏配对图像-报告数据集和大语言模型产生幻觉的风险，需要开发不依赖配对监督的方法。
...
TL;DR: 本文提出了表达性图像合成任务，开发了StickerNet两阶段框架，通过从真实在线编辑平台收集的180万编辑行为数据学习，在用户研究中优于基线方法并接近人类编辑行为。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究专注于视觉真实性和语义合理性，但在现代内容创作中，用户更倾向于创作艺术性、趣味性或社交参与性的非写实内容，需要新的任务定义来反映真实编辑平台上的用户行为。

Method: 提出StickerNet两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和尺度等放置参数。数据集来自匿名在线视觉创作平台的180万真实编辑行为。

Result: 用户研究和定量评估显示，StickerNet优于常见基线方法，并紧密匹配人类放置行为，证明了从真实世界编辑模式学习的有效性。

Conclusion: 这项工作引入了视觉理解的新方向，强调表达性和用户意图而非写实性，为图像编辑研究开辟了新路径。
...
TL;DR: TrafficLens是一个针对多摄像头交通路口的定制算法，通过利用摄像头的重叠覆盖区域和迭代应用视觉语言模型，显著减少了视频到文本的转换时间，同时保持信息准确性。


<details>
  <summary>Details</summary>
Motivation: 多摄像头交通路口产生大量视频数据，传统方法使用视觉语言模型将视频转换为文本耗时较长，无法及时利用交通视频生成洞察和调查事件。

Method: 采用顺序方法利用摄像头重叠覆盖区域，迭代应用不同令牌限制的视觉语言模型，将前一个摄像头的输出作为后续摄像头的提示，并通过对象级相似性检测器智能绕过冗余的视觉语言模型调用。

Result: 实验结果表明，TrafficLens将视频到文本转换时间减少了高达4倍，同时保持了信息准确性。

Conclusion: TrafficLens通过优化多摄像头交通视频分析流程，实现了高效且准确的视频内容理解，为智能交通系统提供了实用的解决方案。
...
TL;DR: 本文提出了一种结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于安全的多机构病理图像分类，通过加密CLS标记实现30倍通信减少，同时防止模型反转攻击。


<details>
  <summary>Details</summary>
Motivation: 医疗机构的协作机器学习面临隐私法规限制，传统联邦学习的模型梯度易受重建攻击泄露敏感医疗信息，需要更安全的隐私保护方案。

Method: 使用Vision Transformers的CLS标记作为紧凑特征表示，采用CKKS同态加密在传输前加密这些标记，实现安全的特征聚合。

Result: 在三客户端联邦设置中，传统梯度方法易受模型反转攻击（PSNR: 52.26 dB），而提出的CLS保护HE方法能防止此类攻击，每轮仅需326 KB加密数据传输，在未加密域达到96.12%分类准确率，加密域达90.02%。

Conclusion: 该方法通过加密CLS标记实现了高效的隐私保护联邦学习，在保持强隐私保证的同时显著降低通信开销，为医疗数据协作提供了可行的安全解决方案。
...
Method: 使用双整流流框架，并行预测内容和风格轨迹，通过动态中点插值融合两个路径的速度，结合注意力注入引导风格整合。

Result: 实验证明该方法在多样化风格和内容上具有良好泛化能力，实现了高效且视觉保真的风格迁移。

Conclusion: 提出的免反演风格迁移框架通过双整流流和动态融合策略，有效解决了传统方法的效率和失真问题，为风格迁移提供了高效可靠的解决方案。
...
Method: 在训练期间将参考图像蒸馏到类别原型内存中，维护每个类别的EMA更新原型，从查询预测混合权重来生成指导向量，无需测试时参考图像。提出了双向注意力对齐模块来弥合参考统计和伪装查询特征之间的表示差距。
...
Method: 提出了GuardTrace-VL安全审计器，构建了GuardTrace数据集，采用多样化的提示策略生成，并通过MLRM和人工投票验证流程进行精炼。还提出了三阶段渐进训练方案与数据精炼过程相结合。

Result: 在提出的测试集上，GuardTrace-VL模型在不安全推理检测任务中达到93.1%的F1分数，相比之前最强的多模态安全防御方法提升了13.5%的F1分数。

Conclusion: GuardTrace-VL能够有效检测多模态大推理模型在推理过程中出现的不安全内容，显著提升了多模态安全防护能力。
...
TL;DR: 该论文提出了一种基于扩散模型的图像分层分解方法，通过轻量级微调将修复模型适应于分层分解任务，并引入多模态上下文融合模块来保留细节。


<details>
  <summary>Details</summary>
Motivation: 图像可以看作前景对象在背景上的分层组合，这种分层表示能够实现元素的独立编辑，为内容创作提供更大灵活性。尽管大型生成模型取得了进展，但由于方法和数据有限，将单张图像分解为图层仍然具有挑战性。

Method: 观察到图层分解与修复任务之间的强关联，提出通过轻量级微调将基于扩散的修复模型适应于图层分解。为了在潜在空间中保留细节，引入了具有线性注意力复杂度的多模态上下文融合模块。模型完全在开源资产构建的合成数据集上训练。

Result: 该方法在物体移除和遮挡恢复方面实现了优越性能，为下游编辑和创意应用开辟了新的可能性。

Conclusion: 提出的方法成功地将修复模型适应于图层分解任务，通过多模态上下文融合模块有效保留了图像细节，在合成数据集上训练取得了良好效果，为图像编辑应用提供了新的解决方案。
...
TL;DR: MERGE是首个用于新闻图片描述的多模态实体感知检索增强生成框架，通过构建实体中心的多模态知识库、多阶段假设-描述策略和动态检索机制，解决了现有方法在信息覆盖、跨模态对齐和视觉实体定位方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有新闻图片描述方法面临三个关键挑战：(1)信息覆盖不完整，(2)跨模态对齐弱，(3)视觉实体定位不优。为解决这些问题，需要开发能够整合文本、视觉和结构化知识的新框架。

Method: 提出MERGE框架：1)构建实体中心的多模态知识库(EMKB)整合文本、视觉和结构化知识；2)采用多阶段假设-描述策略改进跨模态对齐；3)通过图像内容引导的动态检索增强视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上显著优于现有最优方法：CIDEr得分分别提升+6.84和+1.16，命名实体识别F1得分分别提升+4.14和+2.64。在未见过的Visual News数据集上表现优异，CIDEr提升+20.17，F1得分提升+6.22。

Conclusion: MERGE框架通过实体感知的检索增强生成方法，有效解决了新闻图片描述中的关键挑战，在多个数据集上展现了卓越的性能和强大的领域适应性，证明了其方法的有效性和鲁棒性。
...
TL;DR: MetaRank是一个元学习框架，用于自动选择最适合特定目标数据集的模型可迁移性评估指标，解决了传统方法依赖平均性能而忽略任务特定性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的模型可迁移性评估方法选择往往随意或仅基于历史平均性能，但实际效果高度依赖具体任务，缺乏通用的最优指标。

Method: 将指标选择建模为学习排序问题，使用预训练语言模型编码数据集和指标的文本描述，在共享语义空间中训练元预测器，采用列表式目标函数优化顶级指标的排序准确性。

Result: 在11个预训练模型和11个目标数据集上的广泛实验证明了该方法的强有效性。

Conclusion: MetaRank通过元学习实现了任务感知的自动指标选择，显著提升了模型可迁移性评估的准确性和效率。
...
TL;DR: 提出了一种新的可信多视图分类框架，通过引入原型表示每个视图的邻居结构，简化了视图内邻居关系学习并实现视图间结构动态对齐，从而更高效一致地发现跨视图共识。
...
TL;DR: CameraMaster是一个统一的相机感知框架，通过解耦相机指令和参数嵌入来实现精确的图像修饰控制，解决了现有方法在参数控制和可扩展性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的文本引导扩散模型在图像编辑中存在两个主要问题：一是仅依赖模糊的文本提示难以实现精确的相机参数控制；二是为参数调整训练独立头/权重会损害可扩展性、多参数组合和对细微变化的敏感性。
...
Conclusion: CameraMaster通过统一的相机感知框架成功解决了图像修饰中精确参数控制的挑战，实现了物理一致的图像处理效果。
...
TL;DR: 提出了CaptionQA基准，通过下游任务效用评估图像描述质量，涵盖4个领域，包含33,027个密集标注的多选题，揭示了当前MLLM在描述效用方面与图像本身存在显著差距。
...
Method: 构建CaptionQA基准，包含4个领域（自然、文档、电商、具身AI）的细粒度分类体系，创建33,027个需要视觉信息回答的多选题，通过LLM仅使用描述回答问题来直接测量描述效用。

Result: 评估发现最先进MLLM在描述效用方面与图像本身存在显著差距，在传统图像QA基准上表现相近的模型在描述效用上最多下降32%。

Conclusion: CaptionQA基准揭示了当前图像描述在支持下游任务方面的局限性，为评估描述质量提供了基于效用的新视角，并提供了可扩展到新领域的开源管道。
...
TL;DR: LungNoduleAgent是一个创新的协作多智能体系统，专门用于分析肺部CT扫描，通过将诊断过程分解为顺序组件来提高结节描述和恶性分级的精确度。


<details>
  <summary>Details</summary>
Motivation: 当前基于多模态大语言模型的肺部CT分析在准确描述结节形态和融入医学专业知识方面存在挑战，影响了模型在临床环境中的可靠性和有效性。协作多智能体系统在医学应用中平衡通用性和精确性方面具有潜力，但在病理学领域尚未充分探索。

Method: LungNoduleAgent包含三个主要模块：1) Nodule Spotter协调临床检测模型准确识别结节；2) Radiologist整合局部图像描述技术生成全面的CT报告；3) Doctor Agent System使用图像和CT报告进行恶性推理，由病理知识库和多智能体系统框架支持。

Result: 在两个私有数据集和公共LIDC-IDRI数据集上的广泛测试表明，LungNoduleAgent超越了主流视觉语言模型、智能体系统和先进专家模型。

Conclusion: 这些结果强调了区域级语义对齐和多智能体协作在结节诊断中的重要性。LungNoduleAgent是支持肺部结节临床分析的有前景的基础工具。
...
TL;DR: 提出了一种新颖的图像去模糊框架，通过将强大的生成先验与显式密集物理约束相结合，解决了空间变化图像去模糊问题。该方法在严重模糊场景下超越了现有最先进的模型方法和生成基线。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的方法存在两个主要问题：基于模型的深度展开方法虽然能强制执行物理约束，但会产生过度平滑、带有伪影的纹理；而生成模型虽然能实现卓越的感知质量，但由于物理约束较弱会产生幻觉细节。需要一种能调和这两种范式的方法。

Method: 将退化场建模为高维压缩核的密集连续体，捕捉运动和退化模式的微小变化。利用这个丰富的描述符场来调节ControlNet架构，强烈引导扩散采样过程。

Result: 在挑战性严重模糊场景中，该方法在物理准确性和感知真实性之间有效架起了桥梁，超越了最先进的基于模型方法和生成基线。

Conclusion: 所提出的框架通过将生成先验与密集物理约束相结合，成功解决了空间变化图像去模糊的根本不适定问题，在保持物理准确性的同时实现了感知真实性。
...
TL;DR: MUSE是一个统一的图像情感合成框架，能够同时进行情感生成和编辑，避免了传统方法将这两个任务分离的低效问题。
...
Method: 采用概念上对齐测试时间缩放(TTS)的策略，利用现成的情感分类器进行基于梯度的情感令牌优化，通过语义相似度确定最佳引导时机，并使用多情感损失减少固有和相似情感的干扰。
...
TL;DR: 提出T-NIG模型，通过将时间参数融入正态逆伽马分布来处理不规则时间间隔的脑图像序列，实现长期阿尔茨海默病预测和图像生成。
...
Conclusion: T-NIG模型能够有效处理不规则时间分布的脑图像数据，在长期阿尔茨海默病预测中保持疾病特征，具有很好的应用前景。
...
Method: 将虹膜图像沿一个维度分割成子图像序列，输入3D-CNN捕捉时空特征，采用课程学习方式训练模型，结合三元组损失和ArcFace损失进行端到端训练。

Result: 该方法能够在深度度量域中嵌入时间依赖性，提高特征的可区分性，从而在面临旋转、缩放、反射和模糊等挑战时实现高度可区分的嵌入。

Conclusion: 该框架为虹膜认证提供了一个鲁棒且可泛化的解决方案，能够有效处理各种现实世界中的变化挑战。
...
TL;DR: AnchorOPT是一个动态锚点提示学习框架，通过动态学习锚点值和位置矩阵，解决了现有CLIP提示学习方法中锚点静态固定的局限性。
...
Method: 提出AnchorOPT框架：1）动态学习任务特定的锚点值而非手工设计；2）通过可学习位置矩阵自适应优化锚点与软标记的位置关系。采用两阶段训练：先学习锚点标记，然后冻结并转移到第二阶段优化软标记和位置矩阵。

Result: 仅使用简单的可学习锚点和位置矩阵，就能达到或超过某些包含额外可学习模块或正则化技术的方法的性能。作为即插即用模块，在不同数据集上都能带来一致的性能提升。

Conclusion: AnchorOPT通过引入动态锚点值和位置优化，有效提升了CLIP提示学习的灵活性和性能，是一个高效且通用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 先前关于语言模型的研究报告了逆扩展效应，即文本干扰项导致更长但效果更差的推理。本研究旨在探究多模态环境中是否会出现类似现象。
...
Conclusion: RadarFM通过统一表示学习解决了雷达感知中的任务碎片化问题，为雷达基础模型的发展奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs压缩视觉令牌以减少资源消耗，但视觉信息丢失会降低理解能力。虽然一些方法引入知识蒸馏，但忽略了高效学生模型与原始教师模型之间因视觉令牌不平衡导致的细粒度视觉理解差异。

Method: 提出EM-KD范式：1）使用曼哈顿距离和匈牙利匹配算法对齐师生视觉令牌；2）引入两种蒸馏策略：视觉语言亲和力蒸馏（VLAD）和视觉语义蒸馏（VSD）。VLAD计算文本令牌与对齐视觉令牌的亲和矩阵，最小化平滑L1距离；VSD使用反向KL散度测量词汇空间中对齐视觉逻辑的离散概率分布。

Result: 在多样化基准测试中，EM-KD训练模型在准确性和效率上都显著优于先前的高效MLLMs，验证了其有效性。与配备相同视觉令牌匹配策略的先前蒸馏方法相比，EM-KD也实现了更好的性能。

Conclusion: EM-KD通过有效的视觉令牌对齐和双重蒸馏策略，成功解决了高效MLLMs中视觉信息丢失的问题，在保持效率的同时显著提升了模型的理解能力。
...
### [43] [G$^2$VLM: Geometry Grounded Vision Language Model with Unified 3D Reconstruction and Spatial Reasoning](https://arxiv.org/abs/2511.21688)
*Wenbo Hu,Jingli Lin,Yilin Long,Yunlong Ran,Lihan Jiang,Yifan Wang,Chenming Zhu,Runsen Xu,Tai Wang,Jiangmiao Pang*

Main category: cs.CV

TL;DR: G²VLM是一个几何基础视觉语言模型，通过整合3D视觉几何学习来提升空间智能，在3D重建和空间理解任务上表现优异。
...
Method: 提出G²VLM模型，利用学习到的3D视觉几何特征直接预测3D属性，并通过上下文学习和交错推理增强空间推理任务，采用统一设计在大量多视角图像和视频数据上训练。

Result: 实验结果显示G²VLM在3D重建任务上达到与最先进前馈模型相当的结果，在空间理解和推理任务上表现更好或具有竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，需要在保持几何保真度的同时合成视觉上合理的外观，特别是在大视角变化下。然而，基于几何的3DGS和外观驱动的扩散模型的有效融合面临挑战，缺乏像素级、3D一致的编辑标准往往导致过度修复和几何漂移。
...
Conclusion: FaithFusion通过EIG驱动的3DGS-扩散融合框架，成功解决了可控驾驶场景重建中的几何保真度和外观合成问题，实现了SOTA性能。
...
<details>
  <summary>Details</summary>
Motivation: 预训练的大规模视觉生成模型在迁移到下游任务时存在显著的参数冗余问题，需要一种专门针对生成模型特性的剪枝方法。

Method: 提出熵引导剪枝策略，使用数据依赖的条件熵偏差作为指导指标，评估模块重要性；采用零样本自适应剪枝框架，动态决定剪枝时机和程度。
...
### [46] [CtrlVDiff: Controllable Video Generation via Unified Multimodal Video Diffusion](https://arxiv.org/abs/2511.21129)
*Dianbing Xi,Jiepeng Wang,Yuanzhi Liang,Xi Qiu,Jialun Liu,Hao Pan,Yuchi Huo,Rui Wang,Haibin Huang,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: CtrlVDiff是一个统一的扩散框架，同时解决视频理解和可控视频生成的双重挑战，通过融合几何和图形模态实现精确控制和时域一致性。


<details>
  <summary>Details</summary>
Motivation: 传统几何线索（如深度、边缘）不足以约束外观、材质和光照，限制了物理意义的编辑（如重光照、材质替换）并导致时域漂移。需要引入图形模态来提供互补约束。

Method: 提出CtrlVDiff模型，采用混合模态控制策略（HMCS），融合深度、法线、分割、边缘和图形本征（反照率、粗糙度、金属度）等特征，并构建MMVideo混合数据集进行训练。

Result: 在理解和生成基准测试中，CtrlVDiff实现了优越的可控性和保真度，支持分层编辑（重光照、材质调整、对象插入），并在部分模态缺失时保持鲁棒性。

Conclusion: 通过融合多种模态，CtrlVDiff在视频理解和可控生成方面超越了现有方法，实现了精确的物理编辑和时域一致性。
...
TL;DR: 本文提出Ent-Prog框架，通过条件熵膨胀评估模型组件重要性进行优先级训练，结合自适应渐进调度减少计算复杂度，在保持生成性能的同时显著降低训练时间和GPU内存消耗。
...
Conclusion: Ent-Prog是一个高效的人类视频生成扩散模型训练框架，能显著降低资源消耗同时保持模型性能。
...
TL;DR: TEAR是一个针对文本到视频模型安全性的自动化红队框架，专注于利用时间动态性来发现政策违规的视频生成风险。


<details>
  <summary>Details</summary>
Motivation: 现有的安全评估方法主要针对静态图像和文本生成，无法捕捉视频生成中的复杂时间动态特性，因此需要专门针对T2V模型时间序列安全风险的评估框架。

Method: 采用时间感知的测试生成器，通过两阶段优化方法（初始生成器训练和时间感知在线偏好学习）来生成文本无害但能利用时间动态性引发政策违规视频的提示，并使用精炼模型循环改进提示的隐蔽性和对抗效果。

Result: 在开源和商业T2V系统上的广泛实验评估显示，TEAR实现了超过80%的攻击成功率，相比之前最佳结果57%有显著提升。

Conclusion: TEAR框架有效揭示了T2V模型在时间动态性方面的安全风险，证明了专门针对视频生成时间特性的安全评估方法的必要性。
...
TL;DR: LLaVA-UHD v3提出渐进式视觉压缩(PVC)方法，可在标准ViT中实现高效原生分辨率编码，在保持竞争力的同时显著降低计算开销。
...
Method: 提出渐进式视觉压缩(PVC)方法，包含精细化补丁嵌入和窗口化token压缩两个模块，可在ViT中实现灵活补丁大小缩放和分层局部token聚合。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展策略（如Best-of-N）在视觉自回归模型中应用效果不佳，因为它们在错误生成轨迹上消耗完整计算，且光栅扫描解码缺乏整个画布的蓝图，限制了扩展效益。

Method: GridAR采用网格划分的渐进生成方案，在同一画布位置生成多个部分候选，早期修剪不可行候选，并将可行候选固定为锚点指导后续解码。同时使用布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR比Best-of-N（N=8）在T2I-CompBench++上性能提升14.4%，同时成本降低25.6%。在图像编辑任务中，PIE-Bench上语义保持度比更大N的基线提高13.9%。
...
TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过新颖的三阶段场景标记化流程，实现了多种3D场景理解任务，并在3D参考分割、3D视觉问答和3D密集描述等任务上取得了显著改进。
...
Method: 采用基于多尺度正态分布变换表示的三阶段场景标记化流程，包括构建多尺度NDT表示、通过多尺度NDT解码器融合跨尺度特征生成整体场景标记，并将解码器重新用作人机交互提示和分割掩码解码的通用接口。

Result: 在3D参考分割、3D视觉问答和3D密集描述等任务上实现了显著改进。

Conclusion: NDTokenizer3D提供了一个细粒度的通用3D视觉语言模型，统一了多样化的3D场景理解任务于单一架构中。
...
TL;DR: UPA-RFAS是一个针对视觉-语言-动作模型的通用对抗补丁攻击框架，能够在未知架构、微调变体和仿真到现实迁移场景下实现跨模型传输。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗补丁攻击方法通常过拟合到单一模型，在黑盒设置下失效，缺乏通用性和可迁移性。

Method: UPA-RFAS结合了特征空间目标、鲁棒增强的双阶段min-max优化、补丁注意力主导和补丁语义错配等VLA特定损失函数。

Result: 实验表明UPA-RFAS在不同VLA模型、操作套件和物理执行中都能实现跨模型、跨任务和跨视角的一致传输。

Conclusion: 该方法暴露了基于补丁的实用攻击面，为未来防御建立了强基准。
...
TL;DR: BotaCLIP是一个轻量级多模态对比学习框架，通过将高分辨率航空影像与植物样方数据对齐，来适应预训练的地球观测基础模型DOFA，以注入领域特定知识。
...
Method: 采用对比学习框架，通过正则化策略缓解灾难性遗忘，将高分辨率航空影像与植物样方数据进行多模态对齐。
...
TL;DR: 本文提出了一种名为Action-Region Tracking (ART)的新框架，通过查询-响应机制来发现和跟踪局部细节的动态变化，以解决细粒度动作识别中难以捕捉细微差异的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的动作识别方法往往只能捕捉粗粒度的运动模式，而难以识别随时间演变的局部区域的细微差异。细粒度动作识别需要区分具有细微差别的动作类别。

Method: 提出区域特定语义激活模块，使用判别性和文本约束的语义作为查询来捕获每帧中最相关的区域响应；将捕获的区域响应组织成动作轨迹片段，通过连接相关响应来表征基于区域的动作动态；设计多级轨迹对比约束来优化动作轨迹片段。

Result: 在广泛使用的动作识别基准测试上的综合实验表明，该方法优于之前的最先进基线方法。

Conclusion: ART框架通过跟踪局部细节的动态变化，能够有效区分相似动作，在细粒度动作识别任务上表现出优越性能。
...
TL;DR: ConFu是一个多模态对比学习框架，通过联合嵌入单个模态和融合模态到统一表示空间，同时保持成对关系并捕获高阶依赖关系。
...
Method: ConFu扩展了传统的成对对比目标，增加了融合模态对比项，鼓励模态对与第三个模态的联合嵌入，从而捕获高阶依赖关系。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和一对多检索。

Conclusion: ConFu能够有效捕获高阶依赖关系（如XOR类关系），同时保持强成对对应关系，为多模态表示学习提供了统一框架。
...
TL;DR: 本文对三种生成建模方法（DDPM、CFM、MeanFlow）进行对比研究，发现CFM在CIFAR-10上以50步采样达到FID 24.15，显著优于DDPM的402.98。MeanFlow通过单步采样实现FID 29.15，推理时间减少50倍。还将CFM扩展到图像修复，在中心掩码上PSNR从4.95提升至8.57 dB（+73%）。


<details>
  <summary>Details</summary>
Motivation: 比较不同生成建模范式的性能差异，探索更高效的采样方法，并扩展条件流匹配到图像修复任务。

Method: 使用统一的TinyUNet架构（<150万参数）在CIFAR-10上实现DDPM、CFM和MeanFlow三种方法，并将CFM扩展到图像修复，采用四种掩码类型和掩码引导采样。

Result: CFM在50步采样时FID为24.15，DDPM为402.98；MeanFlow单步采样FID为29.15。图像修复模型在中心掩码上PSNR从4.95提升至8.57 dB（+73%），SSIM从0.289提升至0.418（+45%）。

Conclusion: CFM在生成质量上显著优于DDPM，MeanFlow实现了高效的单步生成，CFM在图像修复任务中表现出色，修复感知训练能大幅提升性能。
...
TL;DR: SurgMLLMBench是一个统一的多模态基准，专门用于开发和评估用于手术场景理解的交互式多模态大语言模型，包含新收集的MAVIS数据集，整合了像素级器械分割掩码和结构化VQA标注。
...
Method: 提出SurgMLLMBench基准，整合像素级器械分割掩码和结构化VQA标注，涵盖腹腔镜、机器人辅助和显微手术领域，采用统一分类法。

Result: 基线实验显示，在SurgMLLMBench上训练的单一模型在不同领域实现一致性能，并能有效泛化到未见数据集。
...
TL;DR: FIELDS是一种3D人脸重建方法，通过结合3D表情参数监督和情感识别分支，从单张图像生成保留微妙情感细节的高保真3D面部模型。


<details>
  <summary>Details</summary>
Motivation: 现有3D人脸重建方法依赖2D监督且缺乏3D真实数据，难以捕捉细微的情感信息。

Method: 扩展自监督2D图像一致性线索，加入直接3D表情参数监督和辅助情感识别分支，使用自发4D面部扫描的真实表情参数指导编码器，并采用强度感知情感损失。

Result: 生成的3D重建模型保留了微妙情感线索，显著提升了野外面部表情识别性能，同时保持自然度。

Conclusion: FIELDS通过双重监督策略弥合了2D/3D领域差距，减轻了表情强度偏差，能够从单张图像生成情感丰富的逼真面部表情模型。
...
### [59] [Monet: Reasoning in Latent Visual Space Beyond Images and Language](https://arxiv.org/abs/2511.21395)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在抽象视觉思维方面存在局限性，其灵活性受限于外部工具，无法实现类人的抽象视觉推理。

Method: 采用三阶段蒸馏式监督微调流水线，包括潜在视觉对齐、监督微调和VLPO强化学习。构建了包含12.5万条真实世界、图表、OCR和几何推理链的高质量数据集。

Result: Monet-7B模型在真实世界感知和推理基准上表现一致提升，在具有挑战性的抽象视觉推理任务上展现出强大的分布外泛化能力。

Conclusion: 该工作为视觉潜在推理的未来发展提供了见解，证明了在潜在视觉空间中进行直接推理的可行性。
...
### [60] [AVFakeBench: A Comprehensive Audio-Video Forgery Detection Benchmark for AV-LMMs](https://arxiv.org/abs/2511.21251)
...
Conclusion: AVFakeBench为音视频伪造检测提供了全面基准，揭示了当前方法在复杂伪造检测中的局限性。
...
### [61] [LaGen: Towards Autoregressive LiDAR Scene Generation](https://arxiv.org/abs/2511.21256)
...
Method: 提出LaGen框架，采用单帧LiDAR输入作为起点，利用边界框信息作为条件生成高保真4D场景点云；引入场景解耦估计模块增强对象级内容的交互生成能力，以及噪声调制模块减轻长时程生成中的误差累积。

Result: 在基于nuScenes构建的评估协议上，LaGen全面优于最先进的LiDAR生成和预测模型，特别是在后续帧上表现更佳。

Conclusion: LaGen是首个能够实现长时程LiDAR场景逐帧自回归生成的框架，在生成质量和交互能力方面显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化字幕生成方法通常使用CNNs/Transformers提取视觉表示或结合辅助任务来增强结果，但存在区域感知弱和时间对齐有限的问题。
...
Conclusion: 提出的方法通过SAM基础模型有效提升了遥感变化字幕生成的性能，源代码将在GitHub上发布。
...
TL;DR: MatchGS是一个利用3D高斯溅射生成精确图像匹配训练数据的框架，通过几何校正和2D-3D表示对齐，显著提升零样本图像匹配性能。


<details>
  <summary>Details</summary>
Motivation: 基于学习的图像匹配需要大规模、多样化且几何准确的训练数据。虽然3DGS能够实现逼真的新视角合成，但其几何不准确性和深度渲染偏差限制了其在对应关系标注中的应用。

Method: MatchGS采用双管齐下的方法：(1) 几何保真的数据生成流程，通过优化3DGS几何结构生成高精度对应关系标签；(2) 2D-3D表示对齐策略，将3DGS的显式3D知识注入2D匹配器中。

Result: 生成的对应关系将极线误差降低了40倍，在公共基准测试中零样本性能提升高达17.7%。

Conclusion: 通过适当的几何优化，3DGS可以成为可扩展、高保真且结构丰富的数据源，为新一代鲁棒的零样本图像匹配器铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 利用大量未标记的工业演示数据来支持视觉-语言-动作模型的预训练，解决制造业中具身AI集成缺乏结构化训练数据的问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上表现出色，研究人员希望开发一个统一模型，通过多任务学习在多个任务上表现优异。视觉语言模型在遥感图像理解、定位和超高分辨率图像推理方面显示出潜力，其统一的基于文本的接口为多任务学习提供了重要可能性。

Method: 1. 创建数据管理引擎，包括数据获取、离线处理和集成，以及在线加载和加权；2. 提出统一的动态分辨率策略处理遥感图像的不同尺度；3. 针对超高分辨率图像引入Zoom-in Chain机制及相应数据集LRS-VQA-Zoom；4. 显著增强模型的目标检测能力并提出新的评估协议。

Result: 广泛的实验表明，RSCoVLM在多样化任务上实现了最先进的性能，超越了现有的遥感视觉语言模型，甚至可与专门的专家模型相媲美。所有训练和评估工具、模型权重和数据集都已完全开源以支持可重复性。

Conclusion: RSCoVLM为通用遥感模型的进一步发展提供了强有力的基线，有望推动该领域的进一步进展。
...
TL;DR: 提出一种频率感知的token缩减策略，通过区分高频和低频token来缓解自注意力中的秩崩溃问题，在降低计算复杂度的同时保持性能。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在处理长序列时存在二次计算复杂度问题，现有token缩减方法忽视了自注意力的频率特性（如秩崩溃和过度平滑现象）。

Method: 将token划分为高频token和低频token，选择性保留高频token，同时将低频token聚合为紧凑的直流token以保留必要的低频成分。

Result: 通过广泛实验和分析，该方法在减少计算开销的同时显著提高了准确性，并缓解了秩崩溃和过度平滑问题。

Conclusion: 该频率感知token缩减策略有效平衡了计算效率和性能，同时揭示了先前方法的隐含频率特性和局限性。
...
TL;DR: 提出HTTM方法加速VGGT模型，通过头级时间合并技术解决现有合并方法导致的特征同质化问题，实现7倍加速且性能损失可忽略。
...
Method: 提出头级时间合并(HTTM)，在多头粒度上合并token，保持特征token的独特性，并利用空间局部性和时间对应性实现高合并比。

Result: 在GPU推理中实现高达7倍加速，性能下降可忽略不计。

Conclusion: HTTM是一种无需训练的高效3D token合并方法，能显著加速VGGT模型，同时保持模型表示能力。
...
Method: 使用两种权重合并：任务间权重合并（平均所有先前阶段模型的权重）和任务内权重合并（组合当前阶段内的模型参数），并结合有界更新技术限制累积更新。

Result: 在标准CIL基准测试中表现出色，性能优于现有最先进方法。

Conclusion: M&B方法能有效减少灾难性遗忘，无需修改模型架构即可无缝集成到现有CIL方法中。
...
### [70] [Multimodal Robust Prompt Distillation for 3D Point Cloud Models](https://arxiv.org/abs/2511.21574)
...
Method: 采用教师-学生框架，通过多模态知识蒸馏学习轻量级提示，使用置信门控机制动态平衡来自深度投影视觉模型、高性能3D模型和文本编码器的三个教师模型的贡献。

Result: MRPD在广泛的白盒和黑盒攻击下显著优于最先进的防御方法，甚至在干净数据上也能获得更好性能。
...
### [71] [Qwen3-VL Technical Report](https://arxiv.org/abs/2511.21631)
...
TL;DR: Qwen3-VL是Qwen系列中最先进的视觉语言模型，在多个多模态基准测试中表现优异，支持256K令牌的交错上下文，包含密集和MoE变体，在纯文本理解、长上下文理解和多模态推理方面均有显著提升。
...
Result: 在可比令牌预算和延迟约束下，Qwen3-VL在密集和MoE架构中均实现了卓越性能，在MMMU和视觉数学基准测试中表现领先。
...
### [72] [Endo-G$^{2}$T: Geometry-Guided & Temporally Aware Time-Embedded 4DGS For Endoscopic Scenes](https://arxiv.org/abs/2511.21367)
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视角依赖效应（如镜面反射、湿润反射和遮挡），纯光度监督与几何不一致，导致早期几何漂移，错误形状在密集化过程中被强化且难以纠正。需要为4D高斯泼溅早期锚定几何，同时保持动态内窥镜场景的时间一致性和效率。

Method: 1. 几何引导先验蒸馏：将置信度门控的单目深度转换为监督，使用尺度不变深度和深度梯度损失，采用预热到上限的调度来软注入先验避免早期过拟合
2. 时间嵌入高斯场：在XYZT中表示动态，采用转子式旋转参数化，实现时间一致几何和轻量正则化
3. 关键帧约束流式处理：在最大点数预算下进行关键帧聚焦优化，非关键帧进行轻量更新，提高效率和长时稳定性

Result: 在EndoNeRF和StereoMIS-P1数据集上，Endo-G²T在单目重建基线中达到了最先进的结果。

Conclusion: Endo-G²T通过几何引导和时间感知的训练方案，成功解决了动态内窥镜场景重建中的几何漂移和时间一致性问题，实现了高质量的4D重建效果。
...
### [73] [Continual Error Correction on Low-Resource Devices](https://arxiv.org/abs/2511.21652)
...
TL;DR: 提出了一种新颖的AI错误校正系统，通过少样本学习在资源受限设备上高效修正AI误分类，结合服务器端基础模型训练和设备端原型分类，实现原型更新而非模型重训练的错误修正。


<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中的普及凸显了预测错误降低用户体验的问题，现有解决方案主要关注错误检测，但缺乏高效的修正机制，特别是在资源受限设备上。

Method: 系统包含两个关键组件：(1) 服务器端管道利用知识蒸馏将基础模型的鲁棒特征表示转移到设备兼容架构；(2) 设备端机制通过原型适应实现超高效错误修正。

Result: 在Food-101和Flowers-102数据集上，一次样本场景中实现超过50%的错误修正，同时保持极低的遗忘率（小于0.02%）和可忽略的计算开销。

Conclusion: 通过Android演示应用验证了系统在实际场景中的实用性，证明了在资源受限设备上实现高效AI错误修正的可行性。
...
### [74] [Thinking With Bounding Boxes: Enhancing Spatio-Temporal Video Grounding via Reinforcement Fine-Tuning](https://arxiv.org/abs/2511.21375)
...
Result: 在HCSTVG-v1/v2和VidSTG上评估，STVG-o1在HCSTVG上创下新记录，在HCSTVG-v1上m_tIoU比最佳任务特定方法高出7.3%，在VidSTG上与专用模型相当，大幅超越所有现有MLLM方法。

Conclusion: STVG-o1证明了多模态大语言模型作为精确时空定位任务骨干的可行性和强大能力，并展示了强大的跨数据集开放词汇泛化能力。
...
### [75] [Attention-Guided Patch-Wise Sparse Adversarial Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.21663)
*Naifu Zhang,Wei Tao,Xi Xiao,Qianpu Sun,Yuxin Zheng,Wentao Mo,Peiqiang Wang,Nan Zhang*

Main category: cs.CV

TL;DR: ADVLA是一个针对视觉-语言-动作(VLA)模型的对抗攻击框架，通过在视觉编码器到文本特征空间的投影特征上直接应用对抗扰动，在低幅度约束下有效破坏下游动作预测，同时保持扰动稀疏且聚焦。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法需要昂贵的端到端训练，且往往生成明显的扰动补丁，因此需要开发更高效、更隐蔽的攻击方法。

Method: ADVLA框架直接在视觉编码器投影到文本特征空间的特征上应用对抗扰动，采用三种策略增强敏感性、强制稀疏性和集中扰动，结合Top-K掩码实现局部稀疏攻击。
...
### [76] [DiverseVAR: Balancing Diversity and Quality of Next-Scale Visual Autoregressive Models](https://arxiv.org/abs/2511.21415)
...
<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中虽然表现出色，但存在多样性不足的问题，即使对于简单提示也会生成几乎相同的图像。这个问题在现有研究中被忽视，主要关注点集中在图像质量上。

Method: 采用两阶段方法：1）在文本嵌入中注入噪声以增强多样性；2）提出scale-travel潜在精炼技术，利用多尺度自编码器提取粗尺度标记，在中间阶段恢复生成以保持图像质量。

Result: 实验表明，文本嵌入噪声注入与scale-travel精炼相结合，显著提升了多样性，同时最小化了图像质量下降，在多样性-质量权衡中达到了新的帕累托前沿。

Conclusion: DiverseVAR框架有效解决了VAR模型的多样性限制问题，无需重新训练或大量计算开销，为文本条件视觉生成提供了更好的多样性-质量平衡。
...
### [77] [E-M3RF: An Equivariant Multimodal 3D Re-assembly Framework](https://arxiv.org/abs/2511.21422)
*Adeela Islam,Stefano Fiorini,Manuel Lecha,Theodore Tsesmelis,Stuart James,Pietro Morerio,Alessio Del Bue*

Main category: cs.CV

TL;DR: E-M3RF是一个等变多模态3D重组框架，通过结合几何特征和颜色特征来解决传统方法在几何信息不足时的重组问题，使用SE(3)流匹配预测碎片变换。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法主要依赖几何特征，当几何信息不足或模糊时（如小碎片、侵蚀碎片或对称碎片）表现不佳，且缺乏防止重叠组装的物理约束。

Method: 使用旋转等变编码器处理3D点位置生成旋转一致的几何特征，使用transformer编码颜色特征，将两种特征结合形成多模态表示，通过SE(3)流匹配预测变换。

Result: 在四个数据集上的实验表明，在RePAIR数据集上相比竞争方法，旋转误差减少23.1%，平移误差减少13.2%，Chamfer距离降低18.4%。

Conclusion: E-M3RF通过结合几何和颜色多模态特征，显著提升了3D碎片重组的准确性，特别是在几何信息不足的情况下。
...
Method: 1. 提出线性混合架构去噪器，平衡生成效率和质量；2. 设计时间步蒸馏策略，将采样步骤从20+压缩到仅2步；3. 应用移动端特定的注意力优化，实现注意力操作2倍加速。

Result: 首次在移动设备上实现快速720p图像到视频生成，质量与现有模型相当，在单步条件下每帧720p视频生成时间小于100毫秒。
...
<details>
  <summary>Details</summary>
Motivation: 传统自注意力蒸馏方法独立对齐师生特征图，无法充分捕获像素间的复杂关系，需要更有效的非局部知识传递机制来提升蒸馏效果。

Method: 引入交叉注意力机制，让学生特征图的每个像素都能动态关注教师特征图的所有像素，仅通过额外的损失函数实现非局部知识传递。
...
TL;DR: 本文系统研究了深度伪造检测方法中各种设计选择对性能的影响，发现检测效果更多依赖于数据预处理、增强策略等实现细节而非核心设计，通过分离各因素影响建立了架构无关的最佳实践。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测方法的有效性往往更依赖于实现细节而非核心设计，这使得公平比较检测器变得困难，难以理解真正影响性能的因素。

Method: 系统研究不同设计选择对深度伪造检测模型准确性和泛化能力的影响，重点关注训练、推理和增量更新相关方面，通过分离各因素影响来建立最佳实践。

Result: 实验识别出一组能够持续改进深度伪造检测的设计选择，并在AI-GenBench基准测试中实现了最先进的性能。

Conclusion: 通过系统分析设计选择的影响，建立了适用于未来深度伪造检测系统的鲁棒、架构无关的最佳实践。
...
TL;DR: 提出了一种用于抗核抗体检测的新框架，处理多实例多标签学习任务，使用未经预处理的显微镜图像，通过实例采样器、概率伪标签分发器和自定步调学习率系数实现端到端优化。


<details>
  <summary>Details</summary>
Motivation: 手动ANA检测缓慢、劳动密集且需要多年训练，现有自动化方法在真实临床环境中面临多实例多标签学习的挑战。

Method: 受人类标记逻辑启发，识别一致的ANA子区域并分配聚合标签，使用实例采样器抑制低置信度实例，概率伪标签分发器基于实例可区分性自适应分配标签，自定步调学习根据经验标签观察调整训练。

Result: 在ANA数据集上，模型相比先前最佳方法实现了+7.0% F1-Macro和+12.6% mAP的提升，在公共医学MIML基准测试中关键指标排名前2，汉明损失和单错误分别减少达18.2%和26.9%。

Conclusion: 该框架克服了传统MIML方法的限制，支持端到端优化，在ANA检测和医学MIML任务中取得了最先进的结果。
...
TL;DR: 本文提出了一种用于构建遥感基础模型的高效集成专家框架，通过分解训练过程为轻量级任务特定专家，解决了当前大模型方法在计算资源和可持续性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型主要依赖扩大模型规模和数据集容量，这需要巨大的计算和数据资源，限制了可访问性，且与可持续AI原则相悖。本文旨在提供一种更高效、可持续的替代方案。

Method: 采用集成专家框架，将训练过程分解为轻量级的任务特定ConvNeXtV2专家，这些专家可以被冻结和重用。该方法支持联邦训练、剪枝和持续专家集成。

Result: 该框架在效率、可解释性和可扩展性方面具有显著优势，特别适合协作和资源受限的环境。

Conclusion: 该工作为构建可扩展且高效的遥感基础模型开辟了新的方向，提供了一种可持续的替代方案来应对当前大模型方法的局限性。
...
TL;DR: 本研究提出了一种基于定量指标的序列图像生成方法，结合年龄缩放因子来生成特定年龄的MRI图像，用于预测阿尔茨海默病的进展阶段。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的及时识别对于制定个性化治疗策略至关重要，但现有方法在输入序列时间间隔不规则时难以准确表征疾病特征。

Method: 开发了基于定量指标的序列图像生成方法，整合年龄缩放因子来生成年龄特定的MRI图像，并应用年龄缩放像素损失来增强MRI图像的迭代生成。
...
TL;DR: 本文提出了PRFL框架，在潜在空间中进行偏好优化，解决了视频生成中奖励反馈学习的内存和效率问题。
...
Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架，在完整去噪链中进行潜在空间偏好优化，无需VAE解码。

Result: PRFL显著提高了与人类偏好的对齐度，同时大幅减少了内存消耗和训练时间。

Conclusion: 在潜在空间进行奖励反馈学习是视频生成对齐的更有效方法，PRFL框架为视频生成优化提供了高效解决方案。
...
TL;DR: 该论文提出了UAVLight基准数据集，用于解决多视角3D重建中的光照不一致问题，特别是在无人机户外重建场景下，通过在不同固定时间点沿重复飞行路径采集数据，提供自然光照变化下的标准化评估。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建中的光照不一致是一个根本性挑战，特别是在无人机户外重建中，光照变化会导致几何漂移、颜色不一致和阴影印记。现有数据集要么缺乏有意义的光照多样性，要么时间跨度太长导致几何和语义变化混淆了光照鲁棒性研究。

Method: 引入UAVLight基准，通过在多个固定时间点沿可重复的地理参考飞行路径采集场景数据，在保持几何、校准和视点一致性的前提下产生自然光照变化。

Result: UAVLight提供了在真实户外环境中开发光照鲁棒重建方法的可靠基础，支持跨光照条件的标准化评估协议。
...
TL;DR: 提出了一种结合2D/3D地标配准的U-Net训练框架，用于骨盆透视图像中的地标检测，解决传统方法假设固定前后位视图的局限性。
...
Method: 将2D/3D地标配准集成到U-Net地标预测模型的训练中，比较基线U-Net、带姿态估计损失的U-Net以及用姿态估计损失微调的U-Net的性能差异。

Result: 在患者姿态变化的真实手术条件下评估地标检测精度，分析不同方法的性能表现。

Conclusion: 提出的框架能够更好地处理骨盆透视图像中姿态变化的问题，提高地标检测的准确性和鲁棒性。
...
Method: 提出Harmony框架：1）跨任务协同训练利用音频驱动视频和视频驱动音频生成任务的强监督信号；2）全局-局部解耦交互模块实现高效精确的时序-风格对齐；3）同步增强CFG在推理时显式分离和放大对齐信号。

Result: 广泛实验表明Harmony在生成保真度和细粒度音频-视频同步方面显著优于现有方法，建立了新的最先进水平。
...
<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球非常普遍，但由于口腔内良性、癌前和恶性病变在视觉上相似，通常较晚才被诊断。早期实施计算机辅助诊断系统可以显著改善临床结果。
...
Conclusion: 该框架展示了过采样和增强策略在少数类分类中的有效性，是迈向可信赖的口腔癌早期计算机辅助诊断系统的第一步，在临床环境中具有应用前景。
...
TL;DR: MoGAN是一个专注于提升视频生成运动质量的训练后框架，通过光学流判别器和分布匹配正则化器，在保持视觉保真度的同时显著改善运动真实感。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在帧级保真度上表现良好，但在运动连贯性、动态性和真实感方面存在不足，容易出现抖动、重影或不合理的动态效果。标准去噪MSE目标缺乏对时间一致性的直接监督，导致模型即使损失较低仍可能生成质量较差的运动。
...
Conclusion: MoGAN在不牺牲视觉保真度或效率的前提下，提供了显著更真实的运动，为快速高质量视频生成提供了一条实用路径。
...
TL;DR: 提出一种自提示、点监督的框架，仅使用稀疏点标注将SAM适配到遥感图像，通过Refine-Requery-Reinforce循环逐步提升分割质量。
...
Result: 在WHU、HRSID和NWPU VHR-10三个遥感图像基准数据集上，方法持续超越预训练SAM和近期点监督分割方法。
...
TL;DR: CaFlow是一个用于长时动作质量评估的统一框架，结合反事实去混淆和双向时间条件流，通过因果反事实正则化和双向流模块提升长期时间建模的鲁棒性和连贯性。
...
Method: 提出CaFlow框架：1）因果反事实正则化（CCR）模块以自监督方式解耦因果和混淆特征，通过反事实干预增强因果鲁棒性；2）BiT-Flow模块通过循环一致性约束建模前向和后向动态，产生更平滑连贯的表示。

Result: 在多个长时AQA基准测试上的广泛实验表明，CaFlow实现了最先进的性能。

Conclusion: CaFlow通过整合反事实去混淆和双向时间条件流，有效解决了长时动作质量评估中的挑战，为建模长时间动态提供了鲁棒且连贯的解决方案。
...
TL;DR: Multi-Crit是一个评估多模态模型作为评判者的基准，专注于测试模型遵循多样化细粒度标准的能力，涵盖开放式生成和可验证推理任务。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型作为评判者时，在遵循多样化细粒度评估标准方面的能力尚未充分探索，需要系统评估其多元标准遵循能力。

Method: 通过严格的数据筛选流程构建Multi-Crit基准，收集具有多标准人工标注的挑战性响应对，并引入三个新指标评估多元标准遵循、标准切换灵活性和识别标准级偏好冲突的能力。

Result: 对25个LMM的分析显示：专有模型在多元标准一致性方面仍有困难（尤其在开放式评估中）；开源模型在灵活遵循多样化标准方面更落后；基于整体判断信号的批评微调能增强视觉基础但无法泛化到多元标准级判断。

Conclusion: Multi-Crit为构建可靠和可引导的多模态AI评估奠定了基础，揭示了当前多模态评判者在多元标准遵循方面的局限性。
...
TL;DR: 本文首次系统研究仅通过相机轨迹（而非像素）来感知视频内容的可行性，发现相机轨迹是揭示视频内容的有效信号，并提出了CamFormer对比学习框架来对齐相机轨迹与自然语言。


<details>
  <summary>Details</summary>
Motivation: 探索仅通过相机运动轨迹（而非实际像素内容）来理解视频内容的可能性，验证"如何移动"能否揭示"在做什么"或"在观察什么"。

Method: 提出对比学习框架训练CamFormer编码器，将相机姿态轨迹投影到联合嵌入空间，与自然语言对齐，支持多种相机姿态估计方法。

Result: 相机轨迹是揭示视频内容的有效信号，CamFormer嵌入在跨模态对齐、分类和时间分析等下游任务中表现优异，且对不同的相机姿态估计方法具有鲁棒性。

Conclusion: 相机轨迹作为一种轻量级、鲁棒且多功能的模态，能够有效感知视频内容，证实了"如何移动"确实可以揭示"在做什么"或"在观察什么"。
...
<details>
  <summary>Details</summary>
Motivation: 现代扩散模型在生成高质量多样化图像方面表现出色，但在组合性和多模态控制方面仍存在困难，特别是当用户需要同时指定多种控制信号时。

Method: 将多样控制信号编码为单一复合画布图像，提出多任务画布训练策略，在统一学习范式中优化扩散模型以联合理解和整合异构控制。

Result: 在多人组合、姿态控制组合、布局约束生成和多控制生成等挑战性基准测试中，Canvas-to-Image在身份保持和控制遵循方面显著优于最先进方法。

Conclusion: Canvas-to-Image通过统一框架实现了对多种控制信号的集成视觉-空间推理，在多控制场景下表现出良好的泛化能力。
...
TL;DR: 论文主张LLM效率研究应从追求大规模复杂技术转向注重资源有限环境下的稳健简化，提出了新的研究方向：无需重新训练改造预训练模型架构、轻量级微调、经济推理、轻量知识管理，以及采用开销感知效率作为新基准。


<details>
  <summary>Details</summary>
Motivation: 当前主流LLM效率方法（MoE、推测解码、复杂RAG）仅适用于拥有庞大基础设施的科技巨头，导致大多数组织无法受益，加剧了技术不平等和碳浪费问题。

Method: 提出新的研究议程：1）无需重新训练的预训练模型架构改造；2）保持对齐的轻量级微调；3）长思维链的经济推理；4）无需重型RAG管道的动态知识管理；5）采用开销感知效率作为标准基准。

Result: 通过重新定义效率概念，将采用成本、可持续性和公平性纳入考量，可以促进LLM部署的民主化，减少不平等和碳浪费。

Conclusion: LLM效率研究的未来方向应该是稳健简化，使效率技术能够在有限资源和专业知识下发挥作用，从而实现技术民主化和可持续发展。
...
TL;DR: 本文提出了Harmonic Token Projection (HTP)，一种无需训练、词汇表或随机参数的可逆确定性文本嵌入框架，通过Unicode整数生成谐波轨迹来编码令牌，实现符号与向量空间的双射映射。


<details>
  <summary>Details</summary>
Motivation: 传统神经嵌入依赖统计共现或优化方法，缺乏可解释性和可逆性。HTP旨在通过确定性几何方法建立透明高效的语义表示替代方案。

Method: HTP将每个令牌的Unicode整数表示解析为谐波轨迹，生成相位一致的投影，保持结构和可逆性，通过几何对齐进行语义相似度估计。

Result: 在STS-B及其多语言扩展上的实验显示，HTP在英语中达到Spearman相关系数0.68，在十种语言中保持稳定性能，计算成本可忽略，每句对延迟亚毫秒级。
...
<details>
  <summary>Details</summary>
Motivation: 传统判别式奖励模型存在数据效率低和容易奖励过优化的问题，需要新的训练范式来改进奖励模型的性能。

Method: 采用三种策略：1) 将问答对重构为基于偏好的指令；2) 聚合多样化偏好任务的奖励；3) 在不同dropout率下平均价值头输出以稳定奖励。

Result: 大量实验证明了PIRA方法的有效性。

Conclusion: PIRA训练范式能够有效解决奖励模型面临的数据效率低和奖励过优化挑战。
...
TL;DR: 本文研究了大型语言模型在法律领域的应用，通过重组文档结构、定义修辞角色和模拟法院推理步骤来提升模型在法律判决预测任务中的表现。
...
Conclusion: 通过结构化信息处理和引入领域特定知识，可以有效提升大型语言模型在法律专业任务中的表现。
...
Method: 从Reddit收集自报告诊断数据，应用严格预处理（语言过滤、NSFW内容移除、去重），使用LIWC进行语言分析，并进行二元分类实验比较微调语言模型和词袋特征。
...
TL;DR: FlexCode是一个基于流行度感知的生成式推荐框架，通过动态分配协作过滤和语义编码本的固定token预算，解决了传统方法使用单一编码本处理热门和长尾项目的不足。
...
Method: FlexCode框架包含：1）协作过滤编码本和语义编码本的双编码本结构；2）轻量级混合专家模型动态平衡CF精度和语义泛化；3）对齐和平滑性目标确保在流行度谱系中的连贯性。

Result: 在公共和工业级数据集上的实验表明，FlexCode始终优于强基线方法，在准确性和尾部鲁棒性方面表现更优。
...
TL;DR: 本文介绍了首个阿拉伯语跨领域上下文相关文本到SQL数据集Ar-SParC，包含3,450个相关问题序列共10,225个问题。使用GPT模型进行40个实验，开发了GAT纠正器方法，在零样本和上下文学习设置下分别平均提升执行准确率1.9%和1.72%。


<details>
  <summary>Details</summary>
Motivation: 目前大多数文本到SQL研究和数据集都集中在英语和中文，阿拉伯语领域尚属空白，需要填补这一研究空缺。

Method: 构建Ar-SParC数据集，使用GPT-3.5-turbo和GPT-4.5-turbo进行40个实验，应用10种提示工程技术，开发GAT纠正器方法。

Result: GAT纠正器在所有40个实验中均提升了性能，零样本设置下执行准确率和交互准确率平均提升1.9%，上下文学习设置下分别提升1.72%和0.92%。

Conclusion: 成功创建了首个阿拉伯语文本到SQL数据集，提出的GAT纠正器方法有效提升了模型性能，为阿拉伯语自然语言数据库交互提供了重要基础。
...
TL;DR: 本研究开发了一个层次化推理错误分类法，用于评估大型语言模型在肿瘤学决策支持中的推理缺陷，发现23%的解读存在推理错误，这些错误与指南不符且可能有害的建议相关。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在临床基准测试中表现良好，但可能通过错误推理得出正确结论，这种失败模式在肿瘤学决策支持中具有安全影响，且无法通过基于准确性的评估捕获。

Method: 使用CORAL数据集的乳腺癌和胰腺癌笔记，标注600个推理轨迹来定义三层分类法，将计算失败映射到认知偏差框架。在822个前列腺癌咨询笔记的响应中验证分类法，模拟提取、分析和临床推荐任务。

Result: 23%的解读存在推理错误，确认偏差和锚定偏差最常见。推理失败与指南不符和潜在有害建议相关，特别是在晚期疾病管理中。自动化评估器能检测错误存在但无法可靠分类子类型。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型部署采用统一的提示策略，对复杂分析任务和简单事实问题都使用冗长的响应模式，导致令牌效率低下，而输出令牌成本比输入令牌高4-8倍。

Method: 比较两种路由方法：使用预计算嵌入的简单MLP和微调的RoBERTa变换器。在1000个MMLU问题上评估，MLP路由器的准确率达到90.5%。

Result: MLP路由器在测试数据上达到90.5%的路由准确率，略高于RoBERTa(89.5%)，但参数少1.25亿。该方法在三个主要LLM提供商(OpenAI GPT-4、Google Gemini、Anthropic Claude)上表现一致，令牌减少32.6%-33.9%。

Conclusion: 动态模板选择能有效降低令牌成本，MLP路由器性能优于更复杂的变换器模型，且路由决策在不同LLM提供商间具有通用性。
...
TL;DR: 二维材料在能源存储和转换领域具有广泛应用，但相关信息分散在众多研究论文中，需要系统整理。


<details>
  <summary>Details</summary>
Motivation: 由于二维材料的独特物理化学和电子特性在能源领域应用广泛，但相关信息和制备方法分散在不同论文中，需要集中整理和分析。

Method: 通过分析已发表的研究论文，收集和整理二维材料的性质、制备方法等信息。

Result: 发现二维材料的重要信息确实分散在不同论文中，需要系统化的整理方法。

Conclusion: 需要开发系统方法来整合和分析二维材料研究中的分散信息，以促进该领域的进一步发展。
...
TL;DR: 本文提出了多前缀记忆框架，通过评估序列可通过不同前缀检索的多样性来定义记忆化，解决了现有记忆化定义在对齐模型中的不足。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易逐字记忆训练数据，带来隐私和版权风险，但现有记忆化定义存在缺陷，特别是在对齐模型中无法全面捕捉这一现象。

Method: 引入多前缀记忆框架，核心观点是记忆序列被深度编码，可通过比非记忆内容多得多的不同前缀检索。通过外部对抗性搜索识别能引出目标序列的不同前缀数量来形式化定义记忆化。

Result: 在开源和对齐聊天模型上的实验表明，多前缀定义能可靠区分记忆化和非记忆化数据，为审计LLM数据泄露提供了稳健实用的工具。

Conclusion: 多前缀记忆框架通过量化记忆稳健性（检索路径多样性）来重新定义记忆化，为评估语言模型数据泄露提供了更全面的方法。
...
### [106] [SAGE: An Agentic Explainer Framework for Interpreting SAE Features in Language Models](https://arxiv.org/abs/2511.20820)
*Jiaojiao Han,Wujiang Xu,Mingyu Jin,Mengnan Du*

Main category: cs.CL

TL;DR: SAGE是一个基于智能体的框架，将稀疏自编码器特征解释从被动的单次生成任务转变为主动的、基于解释驱动的迭代过程，显著提高了特征解释的生成和预测准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的内部机制仍然不透明，这对其安全可靠部署构成挑战。稀疏自编码器是分解LLM表示的有前景工具，但解释SAE捕获的特征仍然是一个具有挑战性的任务。

Method: SAGE框架通过系统地为每个特征制定多个解释，设计有针对性的实验来测试这些解释，并根据经验激活反馈迭代优化解释，实现严格的方法论。

Result: 在不同语言模型的SAE特征上的实验表明，与最先进的基线方法相比，SAGE产生的解释在生成和预测准确性方面显著更高。

Conclusion: SAGE通过将特征解释重构为主动的、解释驱动的过程，为理解LLM内部表示提供了更有效的工具，显著提升了特征解释的质量和可靠性。
...
### [107] [Structured Prompting Enables More Robust, Holistic Evaluation of Language Models](https://arxiv.org/abs/2511.20836)
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型基准测试框架（如HELM）使用固定提示，无法跨模型泛化，导致性能估计不准确，可能低估模型真实能力。需要估计每个模型通过提示优化的性能上限。

Method: 开发了可复现的DSPy+HELM框架，引入结构化提示方法（包括推理提示），使用四种提示方法评估四个前沿语言模型在七个基准测试上的表现。

Result: 研究发现：HELM低估语言模型性能（平均4%）；性能估计在基准测试间差异更大（标准差增加2%）；性能差距被错误表示（3/7基准测试的排名翻转）；引入推理（思维链）可减少模型对提示设计的敏感性。

Conclusion: 这是首个大规模实证研究语言模型在不同基准测试和提示方法下行为的研究，表明可扩展的性能上限估计能够创建更有决策价值的基准测试。
...
### [108] [Length-MAX Tokenizer for Language Models](https://arxiv.org/abs/2511.20849)
*Dong Dong,Weijie Su*
...
<details>
  <summary>Details</summary>
Motivation: 优化语言模型的分词器，减少表示文本所需的token数量，从而提高训练和推理效率。

Method: 将长度加权目标最大化建模为图划分问题，并开发贪心近似算法来获取词汇表。

Result: 在GPT-2模型上训练显示，达到固定验证损失所需的步骤减少17.2-18.5%，推理延迟降低12.7-13.7%，吞吐量提升16%，同时在下游任务上表现更好。

Conclusion: 优化平均token长度而非仅频率，是实现更高效语言建模的有效方法，且不会牺牲下游性能。
...
### [109] [Evo-Memory: Benchmarking LLM Agent Test-time Learning with Self-Evolving Memory](https://arxiv.org/abs/2511.20857)
...
TL;DR: Evo-Memory是一个用于评估LLM智能体自进化记忆的流式基准框架，通过序列化任务流测试LLM的记忆搜索、适应和进化能力，并提出了ReMem方法来整合推理、任务行动和记忆更新。
...
Method: 构建了Evo-Memory基准框架，将数据集结构化到序列任务流中；统一实现了十多个代表性记忆模块；提出了ExpRAG基线方法和ReMem行动-思考-记忆精炼流水线。

Result: 在10个多样化多轮目标导向和单轮推理问答数据集上评估了记忆模块，展示了持续改进的能力。

Conclusion: Evo-Memory填补了LLM智能体自进化记忆评估的空白，为测试时记忆演化提供了标准化框架，有助于提升LLM在连续任务环境中的表现。
...
### [110] [Winning with Less for Low Resource Languages: Advantage of Cross-Lingual English_Persian Argument Mining Model over LLM Augmentation](https://arxiv.org/abs/2511.20872)
*Ali Jahan,Masood Ghayoomi,Annette Hautli-Janisz*

Main category: cs.CL

TL;DR: 本文研究跨语言论证挖掘方法，针对低资源语言波斯语，通过三种训练场景（零样本迁移、LLM增强、跨语言模型）进行实验，发现轻量级跨语言混合方法优于资源密集的增强方法。
...
Method: 构建三种训练场景：零样本迁移（仅用英语数据）、LLM增强（英语数据+合成样本）、跨语言模型（英语数据+人工翻译波斯语句子），在英语和波斯语语料上进行评估。

Result: 零样本迁移模型在英语和波斯语测试集上的F1分数分别为50.2%和50.7%；LLM增强模型提升至59.2%（英语）和69.3%（波斯语）；跨语言模型在波斯语测试集上达到74.8%的F1分数。

Conclusion: 轻量级跨语言混合方法能够显著优于资源密集的增强方法，为低资源语言的论证挖掘任务提供了实用的解决方案。
...
### [111] [Emergence and Localisation of Semantic Role Circuits in LLMs](https://arxiv.org/abs/2511.20910)
*Nura Aljaafari,Danilo S. Carvalho,André Freitas*

Main category: cs.CL

TL;DR: 提出了一种结合角色交叉最小对、时间涌现分析和跨模型比较的方法，研究LLMs如何实现语义角色，发现LLMs形成紧凑、因果隔离的抽象语义结构机制，这些机制在尺度和架构间存在部分转移。
...
Method: 整合角色交叉最小对、时间涌现分析和跨模型比较的方法，研究LLMs如何实现语义角色。

Result: 发现：(i)高度集中的电路（89-94%归因在28个节点内）；(ii)渐进结构精炼而非相变，较大模型有时绕过局部化电路；(iii)中等跨尺度保守性（24-59%组件重叠）同时具有高光谱相似性。

Conclusion: LLMs形成紧凑、因果隔离的抽象语义结构机制，这些机制在尺度和架构间表现出部分转移特性。
...
### [112] [Chatty-KG: A Multi-Agent AI System for On-Demand Conversational Question Answering over Knowledge Graphs](https://arxiv.org/abs/2511.20940)
*Reham Omar,Abdelghny Orogat,Ibrahim Abdelaziz,Omij Mangukiya,Panos Kalnis,Essam Mansour*

Main category: cs.CL

TL;DR: Chatty-KG是一个用于知识图谱对话问答的模块化多智能体系统，结合了RAG式检索和结构化查询执行，在单轮和多轮问答中显著优于现有方法。
...
Method: 通过任务专用LLM智能体协作生成SPARQL查询，包括上下文解释、对话跟踪、实体关系链接和高效查询规划，实现自然问题到可执行查询的准确低延迟转换。

Result: 在大型多样化知识图谱上的实验显示，Chatty-KG在单轮和多轮设置中显著优于最先进基线，获得更高的F1和P@1分数，模块化设计保持对话连贯性并支持演化知识图谱。

Conclusion: Chatty-KG统一了对话灵活性和结构化知识图谱基础，为可靠的多轮KGQA提供了可扩展和可扩展的方法，与商业和开源LLM广泛兼容且性能稳定。
...
### [113] [TrackList: Tracing Back Query Linguistic Diversity for Head and Tail Knowledge in Open Large Language Models](https://arxiv.org/abs/2511.21006)
*Ioana Buhnila,Aman Sinha,Mathieu Constant*

Main category: cs.CL

TL;DR: 本文评估了大型语言模型在不同类型语言查询中的表现差异，发现模型在定义类问题上表现最佳，而在举例说明类问题上表现最差。同时，模型更倾向于对流行知识进行释义，对专业术语知识则较少处理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在回答定义类查询时表现高效，但在处理其他类型回答（如举例、释义等）时表现不佳。本研究旨在评估这种性能下降，并研究预训练数据对模型回答多样化语言查询的影响。

Method: 使用TrackList细粒度语言和统计分析流程，引入RefoMed-EN数据集（包含6170个人工标注的医学术语及其定义、命名、举例、解释或释义），通过句法和语义相似度指标、统计相关性和嵌入向量评估模型输出质量。

Result: 结果显示，LLM在定义类问题上的任务性能最高，而在举例说明类问题上最低。对于定义类问题，大型语言模型更倾向于对流行和频繁的知识进行释义，而对尾部和技术性知识（特别是在专业文本中）较少处理。

Conclusion: 大型语言模型在处理不同类型语言查询时存在显著性能差异，定义类问题表现最佳，举例说明类问题表现最差。模型对流行知识的处理优于技术性专业知识，这反映了预训练数据分布的影响。
...
### [114] [Semantic Anchors in In-Context Learning: Why Small LLMs Cannot Flip Their Labels](https://arxiv.org/abs/2511.21038)
...
<details>
  <summary>Details</summary>
Motivation: 探究ICL是覆盖预训练标签语义还是仅细化现有语义主干，理解ICL在标签语义重映射方面的基本限制。

Method: 将LLMs视为提示诱导分类器，对比自然演示（正确标签）和反转演示（系统翻转标签含义）下的行为，分解ICL行为为三个对齐指标（真实、先验和提示对齐），引入语义覆盖率。

Result: 在8个分类任务和8个开源LLM上发现语义锚定观点的一致性证据：自然演示下ICL提高准确性但保持强先验对齐；反转演示下模型无法学习连贯的反语义分类器，语义覆盖率在少样本设置中保持为零。

Conclusion: ICL主要调整输入如何投影到预训练期间学习的稳定语义方向，而非灵活重映射标签含义，表明在这些规模上覆盖标签语义需要ICL之外的干预措施。
...
### [115] [Enhancing Burmese News Classification with Kolmogorov-Arnold Network Head Fine-tuning](https://arxiv.org/abs/2511.21081)
*Thura Aung,Eaint Kay Khaing Kyaw,Ye Kyaw Thu,Thazin Myint Oo,Thepchai Supnithi*

Main category: cs.CL

TL;DR: 该研究探索在缅甸语等低资源语言分类任务中使用Kolmogorov-Arnold Networks (KANs) 替代传统的多层感知机(MLPs)作为分类头，评估了三种KAN变体在不同嵌入表示上的性能。


<details>
  <summary>Details</summary>
Motivation: 在低资源语言分类任务中，通常只微调最后的分类层而保持预训练编码器权重冻结。MLPs虽然常用，但其固定的非线性可能限制表达能力并增加计算成本，因此需要探索更优的替代方案。
...
Result: 实验结果显示KAN分类头与MLPs竞争或更优。EfficientKAN与fastText组合获得最高F1分数（0.928），FasterKAN在速度和准确性之间提供最佳平衡。在变换器嵌入上，EfficientKAN与mBERT组合达到0.917 F1，匹配或略优于MLPs。

Conclusion: KANs是MLPs在低资源语言分类任务中表达能力强、效率高的替代方案，特别适合资源受限的环境。
...
### [116] [Orthographic Constraint Satisfaction and Human Difficulty Alignment in Large Language Models](https://arxiv.org/abs/2511.21086)
*Bryan E. Tuck,Rakesh M. Verma*

Main category: cs.CL

TL;DR: 该论文评估了三种大语言模型家族在字符级约束满足任务上的表现，发现架构差异比参数规模扩展对性能影响更大，模型在拼写不常见但约束有效的词汇上存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 评估不同架构的大语言模型在满足硬性拼写约束任务上的表现，探索模型规模扩展与架构设计对约束满足能力的影响。

Method: 评估28种配置的三种模型家族（Qwen3、Claude Haiku-4.5、GPT-5-mini）在58个需要字符级约束满足的单词谜题上的表现，使用10,000名人类解谜者的难度评级进行分析。

Result: 架构差异产生显著性能差距（2.0-2.2倍），而家族内参数扩展仅带来83%增益；高容量模型在思考预算增加时表现提升，但中等规模模型会饱和或退化；模型在拼写不常见词汇上存在系统性失败（89-96%错误率）。

Conclusion: 约束满足可能需要专门的架构特征或训练目标，而不仅仅是参数规模扩展或计算预算增加；模型过度依赖分布合理性，惩罚了拼写不典型但约束有效的模式。
...
### [117] [MortgageLLM: Domain-Adaptive Pretraining with Residual Instruction Transfer, Alignment Tuning, and Task-Specific Routing](https://arxiv.org/abs/2511.21101)
*Manish Jain,Satheesh Kumar Ponnambalam,Salman Faroz,Chandrakanth Lns,Vinay Sharma*

Main category: cs.CL

TL;DR: 提出了MortgageLLM，一个针对抵押贷款金融领域的专业大语言模型，采用双专家架构解决单一模型在多任务优化中的性能权衡问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用领域表现出色，但在专业领域如抵押贷款金融中需要增强领域知识，同时保持指令跟随能力。单一模型在处理结构化任务和对话任务时存在性能权衡。

Method: 采用双轨专业化框架，从单一基础模型(LLaMA-3.1-8B)开发两个专家模型：对话问答模型和结构化任务模型。应用指令残差技术恢复领域适应后的指令跟随能力，并使用智能任务路由机制。

Result: 在领域特定基准测试中，最终模型(MLM v2)显著优于基础模型：LLM-as-a-Judge评分在总结任务中为4.58(对比3.99)，问答任务中为4.09(对比4.0)，分类任务中为2.6(对比1.2)；BERTScore在总结任务中为0.77(对比0.74)，问答任务中为0.68(对比0.58)，分类任务中为0.75(对比0.73)。

Conclusion: 双专家架构结合指令残差技术能有效解决专业领域大语言模型的性能权衡问题，在抵押贷款金融领域实现了显著性能提升。
...
### [118] [Self-Guided Defense: Adaptive Safety Alignment for Reasoning Models via Synthesized Guidelines](https://arxiv.org/abs/2511.21214)
*Yuhang Wang,Yanxu Zhu,Dongyuan Lu,Jitao Sang*

Main category: cs.CL

TL;DR: SGASA框架通过模型生成的安全指南和自适应对齐方法，增强推理模型对抗有害对抗性提示的能力，同时减少对良性请求的不必要拒绝。


<details>
  <summary>Details</summary>
Motivation: 由于对抗性越狱提示具有隐蔽性和欺骗性，往往能绕过内置安全机制导致生成有害内容，因此需要自适应安全对齐方法来增强模型防御能力。
...
### [119] [Can Finetuing LLMs on Small Human Samples Increase Heterogeneity, Alignment, and Belief-Action Coherence?](https://arxiv.org/abs/2511.21218)
*Steven Wang,Kyle Hunt,Shaojie Tang,Kenneth Joseph*

Main category: cs.CL

TL;DR: 本研究探讨了在少量人类调查数据上微调大语言模型是否能改善其模拟人类行为的真实性。通过信息泄露行为实验，发现微调能显著提升模型的异质性、对齐度和信念-行为一致性，但仍无法复现原始研究的回归系数，表明LLM生成的数据仍不适合替代人类参与者进行正式推断分析。


<details>
  <summary>Details</summary>
Motivation: 当前关于大语言模型是否能替代人类参与者的争论中，存在LLM与真实人类行为不一致的问题。本研究旨在探索通过小样本人类数据微调是否能缓解这些问题，使LLM模拟结果更接近真实人类行为。

Method: 使用信息泄露行为实验，比较人类和LLM生成的反应，评估分布差异、子群对齐、信念-行为一致性和回归系数恢复等多个维度。在少量人类调查数据上对基础模型进行微调。

Result: 微调显著改善了模型的异质性、对齐度和信念-行为一致性，但即使表现最好的微调模型也无法复现原始研究的回归系数。

Conclusion: 虽然微调能在一定程度上改善LLM模拟人类行为的表现，但LLM生成的数据仍不适合替代人类参与者进行正式的推断分析。
...
### [120] [Developing an Open Conversational Speech Corpus for the Isan Language](https://arxiv.org/abs/2511.21229)
*Adisai Na-Thalang,Chanakan Wittayasakpan,Kritsadha Phatcharoen,Supakit Buakaw*

Main category: cs.CL

TL;DR: 开发了首个伊桑语开放式对话语音数据集，包含自然语音而非朗读语音，捕捉真实语言现象如口语化表达、自发韵律、不流利和泰语代码转换。


<details>
  <summary>Details</summary>
Motivation: 解决伊桑语缺乏标准化正字法的问题，现有语音语料库主要基于朗读或脚本语音，需要构建包含自然对话的资源来支持包容性AI发展。

Method: 建立实用的转录协议，在表征准确性和计算处理需求之间取得平衡，处理伊桑语与泰语之间不同词汇声调导致的书写实践差异。

Result: 成功创建首个伊桑语开放式对话语音数据集，包含自然语音特征，解决了转录指南设计中的一致性和语言真实性挑战。

Conclusion: 该数据集作为开放资源发布，旨在支持对代表性不足语言的研究，为建模对话语音的语言和技术挑战提供基础，促进包容性AI发展。
...
### [121] [PEFT-Bench: A Parameter-Efficient Fine-Tuning Methods Benchmark](https://arxiv.org/abs/2511.21285)
...
Method: 开发了PEFT-Bench基准测试，在27个NLP数据集和6种PEFT方法上进行评估，并引入了PSCP指标来综合考虑训练参数、推理速度和训练内存使用。

Result: 建立了一个统一的评估框架，能够系统地比较不同PEFT方法的性能表现，并提供了综合考虑多种因素的评估指标。

Conclusion: PEFT-Bench填补了当前PEFT方法评估的空白，为研究人员提供了一个可复现、全面的评估平台，有助于推动参数高效微调技术的发展。
...
### [122] [Training Introspective Behavior: Fine-Tuning Induces Reliable Internal State Detection in a 7B Model](https://arxiv.org/abs/2511.21399)
*Joshua Fonseca Rivera*

Main category: cs.CL

TL;DR: 通过微调训练，将7B参数语言模型从几乎完全失败（0.4%准确率）转变为可靠检测注入的激活模式（85%准确率），满足准确性、基础性和内部性标准，展示了内省能力可以通过训练直接诱导。
...
Method: 通过对瞬时单令牌注入进行微调训练，使模型能够检测、保留并报告在单个令牌位置注入的短暂"想法"的语义内容。

Result: 模型从0.4%准确率提升到85%准确率，假阳性率为0%，能够泛化到未见过的概念向量（7.5个百分点差距），满足准确性、基础性和内部性三个标准。

Conclusion: 至少一种内省行为组件可以通过训练直接诱导，为构建内置AI透明度提供了一条途径，但并未建立Lindsey意义上的元认知表征。
...
### [123] [Can LLMs extract human-like fine-grained evidence for evidence-based fact-checking?](https://arxiv.org/abs/2511.21401)
*Antonín Jarolím,Martin Fajčík,Lucia Makaiová*

Main category: cs.CL

TL;DR: 本文针对捷克语和斯洛伐克语虚假信息检测，创建了细粒度证据提取数据集，评估了多种大语言模型在证据提取任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 在线新闻评论中经常传播虚假信息，需要有效方法来检测事实错误信息。为了支持或反驳这些评论中的主张，需要识别相关文档并准确定位证明或反驳每个主张的具体文本片段。

Method: 创建了由付费标注者标注的细粒度证据数据集，评估了多种大语言模型（包括llama3.1:8b、gpt-oss-120b、qwen3:14b、deepseek-r1:32b和gpt-oss:20b）在该数据集上的表现。

Result: LLMs经常无法从源文本中逐字复制证据，导致无效输出。llama3.1:8b模型尽管规模较小但正确输出比例较高，而gpt-oss-120b模型尽管参数更多但表现不佳。qwen3:14b、deepseek-r1:32b和gpt-oss:20b在模型大小与人类标注对齐之间达到了有效平衡。

Conclusion: 大语言模型在细粒度证据提取任务中表现存在显著差异，模型大小与性能并非线性相关，某些中等规模模型在准确性和效率方面表现更优。
...
### [124] [Text-to-SQL as Dual-State Reasoning: Integrating Adaptive Context and Progressive Generation](https://arxiv.org/abs/2511.21402)
...
Method: 引入双状态推理框架，包括自适应上下文状态（通过精炼大型模式和选择相关结构构建紧凑的语义忠实环境）和渐进生成状态（将SQL合成形式化为反馈引导的状态转换，使模型能够自我纠正并与用户意图对齐）。

Result: 在Spider 2.0-Snow上达到35.28%的执行准确率，在BIRD开发集上达到68.32%的执行准确率。

Conclusion: DSR-SQL框架有效解决了复杂企业数据库中的Text-to-SQL挑战，实现了竞争性性能，并将开源实现。
...
### [125] [Odin: Oriented Dual-module Integration for Text-rich Network Representation Learning](https://arxiv.org/abs/2511.21416)
*Kaifeng Hong,Yinglong Zhang,Xiaoying Hong,Xuewen Xia,Xing Xu*

Main category: cs.CL

TL;DR: Odin是一个新颖的文本属性图模型架构，通过定向双模块机制在特定Transformer层注入图结构，避免了传统GNN的过平滑问题，并严格包含纯Transformer和GNN的表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖GNN但受限于过平滑和跳数依赖扩散，要么使用Transformer但忽略图拓扑结构。需要一种能有效结合文本理解和结构推理的新方法。

Method: 提出Odin架构，在选定Transformer深度通过定向双模块机制注入图结构，多跳结构在特定层集成，产生与语义层次对齐的结构抽象。还引入了轻量级变体Light Odin。

Result: 在多个文本丰富图基准测试中，Odin实现了最先进的准确率，Light Odin以显著降低的计算成本提供竞争性性能。

Conclusion: Odin和Light Odin形成了一个统一的、无跳数的结构-文本集成框架，为文本属性图建模提供了新的解决方案。
...
### [126] [A Systematic Study of Model Merging Techniques in Large Language Models](https://arxiv.org/abs/2511.21437)
...
<details>
  <summary>Details</summary>
Motivation: 模型融合提供了一种无需额外训练即可复用模型并高效提升性能的方法，但尚不清楚这些在小模型和分类器上报告的优势是否能推广到LLMs。

Method: 对六种最先进的融合方法（包括最近提出的子空间方法）进行大规模系统评估，涵盖四个开源LLM、每个基础模型十二个微调检查点以及十六个标准LLM基准测试。

Result: Task Arithmetic是唯一能可靠在LLMs上产生性能增益的方法，其他干扰感知和子空间融合方法通常导致显著性能下降。

Conclusion: 当前融合技术不能直接迁移到现代LLMs，这促使需要设计LLM特定的融合算法和融合感知的微调方法。
...
### [127] [Hierarchical Ranking Neural Network for Long Document Readability Assessment](https://arxiv.org/abs/2511.21473)
*Yurui Zheng,Yijun Chen,Shaohong Zhang*

Main category: cs.CL

TL;DR: 提出了一种双向可读性评估机制，通过捕捉上下文信息识别文本中语义丰富区域来预测句子级可读性，并使用成对排序算法建模可读性级别间的序数关系。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在可读性评估中未能充分考虑文本长度和可读性标签的序数关系，需要更有效的评估机制。

Method: 使用双向可读性评估机制捕捉上下文信息预测句子级可读性，结合成对排序算法通过标签减法建模可读性级别的序数关系。
...
### [128] [Voice, Bias, and Coreference: An Interpretability Study of Gender in Speech Translation](https://arxiv.org/abs/2511.21517)
...
<details>
  <summary>Details</summary>
Motivation: 语音通过声学线索（如音高）传递说话者性别信息，这在语音翻译中可能导致性别误判风险。目前对语音翻译模型如何做出性别分配决策的理解仍很有限。

Method: 研究三个语言对（英-西/法/意）的语音翻译模型，分析训练数据模式、内部语言模型偏见和声学信息的交互作用，使用对比性特征归因分析频谱图。

Result: 模型不简单复制训练数据中的术语特定性别关联，而是学习更广泛的男性主导模式。内部语言模型表现出强烈男性偏见，但模型能基于声学输入覆盖这些偏好。更高性别准确率的模型使用第一人称代词将性别化术语与说话者关联，利用频谱分布而非集中音高的性别信息。

Conclusion: 语音翻译模型采用复杂机制进行性别分配，能够整合声学信息和语言上下文，通过第一人称代词链接实现更准确的性别识别，揭示了模型处理性别信息的新机制。
...
### [129] [Bangla Sign Language Translation: Dataset Creation Challenges, Benchmarking and Prospects](https://arxiv.org/abs/2511.21533)
*Husne Ara Rubaiyeat,Hasan Mahmud,Md Kamrul Hasan*

Main category: cs.CL

TL;DR: 本文介绍了用于孟加拉手语翻译的低资源数据集IsharaKhobor及其两个子集，解决了该领域数据稀缺问题，并提供了基准测试结果。


<details>
  <summary>Details</summary>
Motivation: 孟加拉手语翻译研究因资源匮乏而受限，需要标准句子级数据集来开发面向孟加拉语听障社区的AI辅助工具。

Method: 创建了IsharaKhobor数据集及其两个子集，使用基于地标的原始嵌入和RQE嵌入进行基准测试，并对词汇限制和规范化进行了消融研究。

Result: 开发了公开可用的数据集IsharaKhobor、IsharaKhobor_small和IsharaKhobor_canonical_small，并提供了基准性能。

Conclusion: 该数据集为孟加拉手语翻译研究提供了重要资源，通过词汇限制和规范化处理改善了数据质量，推动了该领域的发展。
...
### [130] [RoParQ: Paraphrase-Aware Alignment of Large Language Models Towards Robustness to Paraphrased Questions](https://arxiv.org/abs/2511.21568)
*Minjoon Choi*
...
Result: 目标对齐显著增强鲁棒性，轻量级微调模型达到与更大预训练模型相当的一致性水平。

Conclusion: 该方法有效缓解表面记忆问题，培养更鲁棒可靠的LLMs。
...
### [131] [Auxiliary Metrics Help Decoding Skill Neurons in the Wild](https://arxiv.org/abs/2511.21610)
*Yixiu Zhao,Xiaozhi Wang,Zijun Yao,Lei Hou,Juanzi Li*

Main category: cs.CL

TL;DR: 本文提出了一种简单轻量的方法来识别大语言模型中编码特定技能的神经元，通过将神经元激活与外部标签和模型置信度等辅助指标相关联，在复杂多技能场景下发现可解释的任务特定行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在各种任务上表现出卓越能力，但其内部机制仍然不透明。本文旨在开发一种方法来隔离编码特定技能的神经元，以增强模型的可解释性。
...
Conclusion: 该方法能够有效识别大语言模型中编码特定技能的神经元，增强模型可解释性，并发现模型推理过程中的潜在捷径。
...
### [132] [Beyond URLs: Metadata Diversity and Position for Efficient LLM Pretraining](https://arxiv.org/abs/2511.21613)
*Dongyang Fan,Diba Hashemi,Sai Praneeth Karimireddy,Martin Jaggi*

Main category: cs.CL

TL;DR: 本文研究了在大型语言模型预训练中利用多种元数据来加速训练的方法，发现细粒度元数据（如文档质量指标）比仅使用URL更有效，并提出了元数据追加和可学习元标记等技术来提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 先前的研究只关注URL这一种元数据信号，但其他类型的元数据可能带来更大的训练加速效果，因此需要探索更广泛的元数据类型及其在LLM预训练中的应用价值。

Method: 研究了多种元数据类型，引入元数据追加作为辅助任务来提升训练效率，使用可学习元标记配合掩码损失来诱导质量感知的潜在结构，并通过探测分析潜在表示来理解元数据如何影响学习过程。

Result: 发现细粒度元数据（如文档质量指标）能有效加速预训练，元数据追加和可学习元标记技术都能部分恢复训练加速效果，通过潜在表示分析揭示了元数据塑造学习过程的具体机制。

Conclusion: 研究结果为整合元数据以提升LLM预训练的效率和效果提供了实用指导，强调了细粒度元数据在加速训练中的重要作用。
...
### [133] [The author is dead, but what if they never lived? A reception experiment on Czech AI- and human-authored poetry](https://arxiv.org/abs/2511.21629)
*Anna Marklová,Ondřej Vinš,Martina Vokáčová,Jiří Milička*

Main category: cs.CL

TL;DR: 捷克语母语者难以区分AI与人类创作的诗歌，正确识别率仅为45.8%。研究发现存在作者偏见：当参与者认为诗歌是AI创作时，评价会更低，尽管实际上AI诗歌的平均评价与人类诗歌相当或更高。


<details>
  <summary>Details</summary>
Motivation: 研究捷克语AI生成诗歌的感知，探索在形态复杂、资源较少的斯拉夫语言中，AI能否创作出令人信服的诗歌，以及读者如何判断和评价这些作品。

Method: 让捷克语母语者识别AI和人类创作的诗歌，并对其进行美学评价。使用逻辑回归模型分析识别准确性与诗歌喜好之间的关系。

Result: 参与者对作者身份的猜测准确率接近随机水平（45.8%），表明捷克AI生成诗歌与人类作品难以区分。美学评价显示强烈的作者偏见：当认为诗歌是AI创作时评分更低，尽管AI诗歌实际评价相当或更高。诗歌喜好与识别准确性呈负相关。

Conclusion: AI能够在捷克语这种形态复杂、资源较少的斯拉夫语言中创作出令人信服的诗歌。读者的作者信念与诗歌美学评价密切相关，存在明显的作者偏见效应。
...
### [134] [ToolOrchestra: Elevating Intelligence via Efficient Model and Tool Orchestration](https://arxiv.org/abs/2511.21689)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然强大，但在解决深度复杂问题时仍然面临概念挑战和计算成本高的问题。

Method: 使用强化学习训练小型编排器，结合结果、效率和用户偏好感知的奖励机制来协调各种工具。

Result: Orchestrator（8B模型）在Humanity's Last Exam上获得37.1%的分数，超越GPT-5（35.1%）且效率提高2.5倍；在tau2-Bench和FRAMES上大幅超越GPT-5，仅使用约30%的成本。
...
### [135] [Revisiting Generalization Across Difficulty Levels: It's Not So Easy](https://arxiv.org/abs/2511.21692)
*Yeganeh Kordi,Nihal V. Nayak,Max Zuo,Ilana Nguyen,Stephen H. Bach*

Main category: cs.CL

TL;DR: 本文通过系统评估大型语言模型在不同任务难度上的泛化能力，发现跨难度泛化通常有限，训练数据中需要包含多种难度级别的样本。


<details>
  <summary>Details</summary>
Motivation: 现有研究对于在简单或困难数据上训练是否能带来更好结果存在分歧，且不清楚这些收益是在简单还是困难测试数据上体现。本文旨在系统评估LLMs在不同难度任务上的泛化能力。

Method: 使用数千种不同LLMs的输出和项目反应理论(IRT)对六个数据集中的示例进行难度排序，完全基于LLMs的能力确定难度评级，排除人类主观判断。

Result: 跨难度泛化通常有限；在简单或困难数据上训练都无法在完整难度范围内实现一致的改进。

Conclusion: LLMs的训练和评估数据中需要包含多种难度级别，在难度方面走捷径是有风险的。
...
<details>
  <summary>Details</summary>
Motivation: 双曲嵌入的质量与输入层次结构密切相关，而最优双曲嵌入需要高分支因子和单继承特性。为了帮助知识工程师重新组织层次知识，研究LLM是否具备自动重构层次结构以满足这些标准的能力。

Method: 提出基于提示的方法，利用大型语言模型在已知双曲嵌入期望标准的指导下转换现有层次结构。

Result: 在16个不同层次结构上的实验表明，LLM重构的层次结构在多个标准嵌入质量指标上始终产生更高质量的双曲嵌入。

Conclusion: LLM引导的层次结构重构能够实现可解释的重组，为知识工程师提供合理性解释，并有效提升双曲嵌入质量。
...
TL;DR: AssurAI是一个针对韩语多模态生成AI安全评估的新数据集，包含11,480个文本、图像、视频和音频实例，涵盖35种AI风险因素，专门针对韩国社会文化背景设计。
...
Result: 构建并发布了AssurAI数据集，试点研究验证了其在评估最新LLMs安全性方面的有效性。

Conclusion: AssurAI数据集有助于为韩国社区开发更安全可靠的生成AI系统，已向公众发布。
...
TL;DR: A²Flow是一个基于自适应抽象算子的全自动智能体工作流生成框架，通过三阶段算子提取过程自动生成可重用的执行算子，无需手动预定义，在性能和资源使用方面优于现有方法。
...
TL;DR: 该论文提出了一个名为"Reasoning With a Star"的太阳物理学推理数据集和基准测试方法，通过多智能体模式在需要演绎推理的问题上优于直接提示方法。


<details>
  <summary>Details</summary>
Motivation: 解决太阳物理学中科学推理的挑战，包括整合物理假设、保持单位一致性以及提供清晰的科学格式，而不仅仅是事实回忆。

Method: 构建了基于NASA和UCAR"Living With a Star"暑期学校问题集的数据集，包含问题上下文、推理步骤、预期答案类型等；开发了程序化评分器，使用单位感知数值容差、符号等价性和模式验证；比较了单次提示基线和四种多智能体模式。

Result: 发现通过系统工程原则分解工作流程的多智能体模式在需要演绎推理的问题上表现优于直接提示方法。

Conclusion: 多智能体方法在太阳物理学推理任务中，特别是对于演绎推理问题，比直接提示更有效。
...
<details>
  <summary>Details</summary>
Motivation: 将数字孪生技术从工业领域扩展到医疗健康领域，旨在通过创建患者特异性虚拟模型，实现从被动治疗向预测性、预防性和个性化医疗的转变。

Method: 整合医学影像、生物传感器和计算模型，构建动态数据驱动的患者虚拟对应体，通过实时数据流持续更新，支持双向交互。

Result: 代表性应用包括心脏数字孪生预测心律失常治疗结果、肿瘤学数字孪生跟踪肿瘤进展和优化放疗、药理学数字孪生加速药物发现。

Conclusion: 尽管面临挑战，但可解释AI、联邦学习和统一监管框架等新兴解决方案为未来发展提供了有前景的路径，多器官数字孪生、基因组学整合和伦理治理将是关键发展方向。
...
### [141] [Cross Domain Evaluation of Multimodal Chain-of-Thought Reasoning of different datasets into the Amazon CoT Framework](https://arxiv.org/abs/2511.20701)
*Nitya Tiwari,Parv Maheshwari,Vidisha Agarwal*

Main category: cs.AI

TL;DR: 本文对多模态思维链推理进行了综合分析，评估其在需要常识和世界知识的A-OKVQA、OKVQA和ChartQA数据集上的有效性，发现视觉集成能减少幻觉但效果因问题类型而异。


<details>
  <summary>Details</summary>
Motivation: 虽然CoT已在多模态设置中扩展到科学问答基准，但其在不同领域的泛化能力仍未充分探索，特别是需要广泛常识和世界知识的领域。

Method: 采用Zhang等人提出的两阶段框架，将理由生成与答案推理分离，通过门控融合机制将视觉特征与基于T5的语言模型集成。

Result: 视觉集成显著减少了理由生成中的幻觉，但CoT推理效果在不同问题类型间差异很大，常识推理尤其具有挑战性。

Conclusion: 本研究为实施多模态推理系统提供了实用见解，并确定了跨领域泛化的关键改进领域。
...
### [142] [Representation Interventions Enable Lifelong Unstructured Knowledge Control](https://arxiv.org/abs/2511.20892)
*Xuyuan Liu,Zhengzhang Chen,Xinshuai Dong,Yanchi Liu,Xujiang Zhao,Shengyu Chen,Haoyu Wang,Yujun Yan,Haifeng Chen*

Main category: cs.AI

TL;DR: RILKE是一种在表示空间进行干预的知识控制方法，能够在保持基础权重冻结的情况下，对LLMs的复杂非结构化知识进行细粒度控制，支持大规模终身学习场景。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生错误或过时的内容，如何在无需昂贵重新训练的情况下高效准确地更新其知识是一个主要挑战，特别是在终身学习场景中需要处理多个编辑且避免相互干扰。

Method: RILKE通过在模型的表示空间进行干预来实现知识控制，训练时学习抗释义和编辑局部化的模块，将每个更新限制在低维子空间以减少跨编辑干扰；推理时通过查询自适应路由器选择适当的模块来指导生成。
...
### [143] [ENACT: Evaluating Embodied Cognition with World Modeling of Egocentric Interaction](https://arxiv.org/abs/2511.20937)
...
TL;DR: ENACT是一个评估具身认知的基准测试，通过视觉问答形式评估模型从第一人称交互中进行世界建模的能力，包含前向世界建模和逆向世界建模两个任务。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型是否表现出具身认知的特征，即智能是否源于感觉运动交互而非被动观察。

Method: 构建ENACT基准测试，将具身认知评估转化为从第一人称交互中进行世界建模的视觉问答任务，包含前向世界建模（给定动作重排观察序列）和逆向世界建模（给定观察重排动作序列）。

Result: 实验显示前沿视觉语言模型与人类之间存在性能差距，且随着交互时间跨度增加差距扩大。模型在逆向任务上表现更好，并表现出人类中心偏见，如偏好右手动作、当相机参数或视角偏离人类视觉时性能下降。

Conclusion: 现代视觉语言模型在具身认知能力上仍存在不足，特别是在长时程交互和适应非人类视觉条件方面，需要进一步研究和发展具身认知能力。
...
### [144] [Improving Procedural Skill Explanations via Constrained Generation: A Symbolic-LLM Hybrid Architecture](https://arxiv.org/abs/2511.20942)
*Rahul Dass,Thomas Bowlin,Zebing Li,Xiao Jin,Ashok Goel*

Main category: cs.AI

TL;DR: Ivy是一个AI教练系统，通过结合符号化的任务-方法-知识(TMK)模型和生成式解释层，提供结构化的多步骤解释，以改进程序技能学习中的教学解释质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)通常产生流畅但浅层的响应，缺乏因果、目标导向和组合逻辑结构，无法有效传达程序技能学习中的深层理解。

Method: Ivy系统结合符号TMK模型和生成式解释层，TMK编码因果转换、目标层次和问题分解，约束LLM在明确结构边界内构建解释。

Result: 与GPT和检索增强GPT基线相比，Ivy在三个推理维度上通过专家和独立注释评估，显示符号约束持续提高了解释的结构质量。

Conclusion: 该研究展示了一种可扩展的AI教育方法，通过符号约束增强了AI生成解释在智能教练系统中的教学价值。
...
### [145] [ICPO: Intrinsic Confidence-Driven Group Relative Preference Optimization for Efficient Reinforcement Learning](https://arxiv.org/abs/2511.21005)
*Jinpeng Wang,Chao Li,Ting Ye,Mengyuan Zhang,Wei Liu,Jian Luan*

Main category: cs.AI

TL;DR: 本文提出了一种名为ICPO的新方法，通过结合语言模型生成概率的内在置信度和可验证奖励，解决了现有RLVR方法中存在的粗粒度奖励、奖励噪声和低效探索等问题。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习与可验证奖励方法存在粗粒度奖励、奖励噪声和低效探索等问题，导致训练不稳定和熵崩溃，需要改进以提升大语言模型的推理能力。

Method: 提出ICPO方法，通过计算多个响应在同一输入提示下的相对生成概率来获得偏好优势分数，并将该分数与可验证奖励结合指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的实验表明，ICPO相比GRPO能稳定提升推理能力，有效缓解奖励噪声问题并防止模型对特定策略的过拟合。

Conclusion: ICPO方法通过利用语言模型生成概率的内在置信度，成功解决了RLVR方法的局限性，为提升大语言模型推理能力提供了有效途径。
...
### [146] [Causality Without Causal Models](https://arxiv.org/abs/2511.21260)
*Joseph Y. Halpern,Rafael Pass*

Main category: cs.AI

TL;DR: 本文对Halpern和Pearl的因果定义进行了抽象化，提取其关键特征，使其能够应用于任何定义了反事实的模型。通过抽象化，该定义可以处理更广泛的模型类型和更复杂的逻辑公式。


<details>
  <summary>Details</summary>
Motivation: Halpern-Pearl的因果定义局限于因果模型，无法处理包含析取、否定、信念和嵌套反事实的复杂公式。本文旨在扩展这一定义的适用范围。

Method: 通过抽象化Halpern-Pearl因果定义的关键特征，将其从因果模型框架中解耦，使其适用于任何定义了反事实的模型系统。

Result: 成功开发了一个抽象的因果定义框架，能够处理更广泛的模型类型和更复杂的逻辑公式，包括回溯推理和嵌套反事实等情况。

Conclusion: 抽象化的因果定义不仅扩展了应用范围，还深化了对因果定义核心特征的理解，同时可以进一步扩展到解释的定义框架。
...
### [147] [New Hybrid Heuristics for Pseudo-Boolean Propagation](https://arxiv.org/abs/2511.21417)
...
<details>
  <summary>Details</summary>
Motivation: 当前伪布尔求解中最成功的单元传播策略是结合观察文字方案和计数方法的混合模式，但现有方法仍有改进空间

Method: 提出了用于混合决策的新启发式方法，在RoundingSAT求解器中实现

Result: 新启发式方法能够显著超越当前方法

Conclusion: 新提出的启发式方法在伪布尔求解的混合单元传播策略中表现出色
...
### [148] [SpatialBench: Benchmarking Multimodal Large Language Models for Spatial Cognition](https://arxiv.org/abs/2511.21471)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准往往将空间认知过度简化为单一维度指标，无法捕捉空间能力的层次结构和相互依赖性，需要更系统化的评估框架。

Method: 构建了层次化空间认知框架（五个认知层次），开发了SpatialBench大规模细粒度基准（涵盖15个任务），并引入了高层能力导向的统一评估指标。

Result: 实验发现模型在不同认知层次上表现分层：感知基础能力强，但在符号推理、因果推断和规划方面仍有局限；人类进行选择性目标导向抽象，而MLLM倾向于过度关注表面细节。
...
### [149] [Pessimistic Verification for Open Ended Math Questions](https://arxiv.org/abs/2511.21522)
...
### [150] [Self-Transparency Failures in Expert-Persona LLMs: A Large-Scale Behavioral Audit](https://arxiv.org/abs/2511.21569)
*Alex Diep*

Main category: cs.AI

TL;DR: 本研究审计了16个开源模型在专业角色下的AI身份披露行为，发现模型在专业领域存在显著的不一致性披露模式，某些模型在推理优化后反而降低了透明度。


<details>
  <summary>Details</summary>
Motivation: 在专业高风险领域中，如果语言模型不能可靠地披露其AI身份，用户就无法信任其能力边界，可能导致用户受到伤害。

Method: 采用共同花园设计，对16个开源模型（4B-671B参数）进行了19,200次试验，使用贝叶斯验证和Rogan-Gladen校正确保结果稳健性。

Result: 模型在专业领域表现出显著不一致的披露行为：金融顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%。披露率范围从2.8%到73.6%，模型身份比参数数量更能预测行为。推理优化在某些模型中主动抑制了自我透明度。
...
### [151] [From Prediction to Foresight: The Role of AI in Designing Responsible Futures](https://arxiv.org/abs/2511.21570)
*Maria Perez-Ortiz*

Main category: cs.AI

TL;DR: 本文提出了"负责任计算前瞻"概念，探讨了人工智能在负责任前瞻中的作用，强调AI应作为支持工具而非替代人类决策，以帮助政策制定者应对未来不确定性。


<details>
  <summary>Details</summary>
Motivation: 在技术快速发展和全球挑战日益复杂的时代，政策制定者需要负责任的前瞻框架来应对未来不确定性并塑造未来。

Method: 建立负责任计算前瞻的基础原则，开发AI驱动的前瞻工具，结合模拟和情景分析来增强政策制定能力。

Result: AI与模拟和情景分析结合，能增强政策制定者应对不确定性、评估风险和制定可持续战略的能力。

Conclusion: AI应作为负责任、以人为本的前瞻中的支持工具，补充而非替代政策制定者的判断，以主动塑造有韧性和道德健全的未来。
...
### [152] [On the Limits of Innate Planning in Large Language Models](https://arxiv.org/abs/2511.21591)
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在规划和状态推理方面的实际能力，特别是在不需要代码执行或其他工具的情况下，通过经典的8拼图任务来评估这些能力。

Method: 使用8拼图任务测试四个模型，采用常见的提示条件（零样本、思维链、算法思维）和分层纠正反馈，并引入外部移动验证器来提供有效移动。

Result: 反馈机制对某些模型-提示组合提高了成功率，但许多成功运行时间长、计算成本高且间接。即使有外部移动验证器，所有模型都无法解决任何拼图。定性分析显示模型存在内部状态表示脆弱和启发式规划能力弱的问题。

Conclusion: 在没有外部工具的情况下，当前大型语言模型在规划方面存在显著限制，进一步进展可能需要维护显式状态和执行结构化搜索的机制。
...
### [153] [Bridging the Unavoidable A Priori: A Framework for Comparative Causal Modeling](https://arxiv.org/abs/2511.21636)
...
<details>
  <summary>Details</summary>
Motivation: AI/ML模型在解决未解决问题时存在放大人类偏见的意外后果，需要利用更丰富的系统动力学因果模型来指导负责任AI/ML的开发。

Method: 将系统动力学和结构方程建模结合到一个共同的数学框架中，用于从分布生成系统、开发方法并比较结果。

Result: 建立了一个能够统一系统动力学和结构方程建模的数学框架，为数据科学和AI/ML应用提供系统动力学认识论支持。

Conclusion: 该框架有助于克服不同方法基于不同基本假设的障碍，为负责任AI/ML的发展提供更系统的理论基础。
