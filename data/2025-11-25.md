...
TL;DR: 使用持久同调（PH）方法从拓扑数据分析角度客观区分艺术家风格，包括不同艺术流派、同一流派内艺术家，以及区分艺术家作品与AI生成风格模仿作品。
...
Method: 采用拓扑数据分析中的持久同调（PH）方法，该方法能够捕捉数据的拓扑特征并用于风格分析。

Result: PH方法能够以统计确定性区分不同艺术家风格，包括不同艺术流派的艺术家和同一流派内的艺术家，并能有效区分艺术家原作与AI生成的风格模仿作品。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的数学工具，能够有效量化并区分艺术家的个人风格特征。
...
<details>
  <summary>Details</summary>
Motivation: 由于训练图像在数量和变化上都很有限，特别是不同个体的面部表情，使用生成AI技术生成动物面部具有挑战性。本研究聚焦于在系统神经科学和进化研究中广泛研究的猕猴。

Method: 1) 使用运动转移技术通过计算机图形学为静态图像添加动画来进行数据增强；2) 基于初始训练的StyleGAN2模型的潜在表示进行样本选择，确保训练数据集的多样性和均匀采样；3) 损失函数优化以确保细微运动（如眼球运动）的准确再现。

Result: 该方法能够为多个猕猴个体生成多样化的面部表情，优于仅使用原始静态图像训练的模型。模型在基于风格的图像编辑方面也很有效，特定风格参数对应不同的面部运动。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专门为高质量双人肖像生成设计的大规模基准数据集，包含超过10万张图像，并开发了DHumanDiff基线方法用于双人肖像生成。
...
Method: 构建PairHuman数据集（包含10万+图像及丰富元数据），并开发DHumanDiff基线方法，增强面部一致性并平衡个性化人物生成与语义驱动场景创建。

Result: 实验结果表明，所提出的数据集和方法能够生成具有卓越视觉质量且符合人类偏好的高度定制化肖像。
...
TL;DR: 本文提出了一种用于中子成像中混合高斯-泊松噪声去除的无监督自动编码器方法，结合CDF 97小波变换，在保持图像保真度和边缘信息方面优于传统滤波方法。


<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变分析中至关重要，但图像常被高斯和泊松噪声共同污染，传统滤波方法难以有效处理这种混合噪声，且缺乏真实数据限制了机器学习方法的应用。

Method: 采用无监督自动编码器，在潜在空间中集成Cohen-Daubechies-Feauveau (CDF 97)小波变换，专门针对混合高斯-泊松噪声进行去噪处理。

Result: 该方法成功去除了中子成像数据中的噪声，相比非机器学习方法如BM3D，具有更低的重建误差和更好的边缘保持性能。
...
TL;DR: 本文提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代品，在保持安全性的同时显著提升零样本准确率，解决了视觉语言模型安全微调与泛化性能之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型安全微调方法采用刚性对齐策略，强制将不安全概念映射到单一预定义安全目标，破坏了模型学习的语义结构，导致泛化性能显著下降。

Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代品，最小化表示变化。开发SaFeR-CLIP微调框架，应用最小干预原则。

Result: SaFeR-CLIP成功调和了安全性与性能，相比先前方法恢复了高达8.0%的零样本准确率，同时保持强大的安全性。还贡献了NSFW-Caps基准数据集，包含1000个高度对齐的配对用于分布偏移下的安全性测试。
...
TL;DR: 提出了一个三阶段框架，从单个SVG输入生成多视角一致的SVG，具有几何和颜色一致性，无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 单对象SVG的多视角一致性生成研究不足，需要解决从单视角输入生成几何和颜色一致的多视角SVG的问题。

Method: 三阶段框架：1）将栅格化输入提升为3D表示并渲染多视角图像；2）扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立跨视角部件级对应关系；3）在栅格到矢量转换中执行路径整合和结构优化。
...
<details>
  <summary>Details</summary>
Motivation: 传统手动创建3D网格耗时且难以扩展，现有自回归模型存在质量与速度的权衡问题，且顺序依赖限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册来检索、生成和集成网格组件，解耦生成过程的顺序依赖。

Result: Mesh RAG在各种基础自回归网格生成模型中显著提升网格质量，加速生成速度，并实现增量编辑功能。

Conclusion: Mesh RAG框架有效解决了自回归网格生成模型的局限性，无需重新训练即可实现质量提升、速度优化和编辑能力增强。
...
<details>
  <summary>Details</summary>
Motivation: 弥合创意意图与功能性虚拟空间之间的差距，让创作者无需专业3D技能就能设计连贯、可导航的世界，推动可访问的大规模生成式世界构建。

Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解，采用模块化架构支持对布局、规模和样式的细粒度控制。

Result: 系统能够生成几何一致、视觉丰富且实时渲染效率高的世界，可直接在标准游戏引擎中探索或编辑，创建完全纹理化的可遍历环境。

Conclusion: 这项工作代表了向可访问的大规模生成式世界构建迈出的一步，推动了3D生成AI在游戏、模拟和沉浸式社交环境应用的前沿发展。
...
TL;DR: REO-Instruct是首个针对地球观测的统一基准，结合描述性和回归任务，建立从人类活动到生物量回归的认知逻辑链，填补了多模态感知与生物物理变量测量之间的空白。
...
Method: 通过混合人机协作流程生成和验证结构化文本标注，整合配准的Sentinel-2和ALOS-2影像，建立森林生态场景中的认知逻辑链。
...
TL;DR: BOP-ASK是一个用于对象交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖六个任务，旨在解决现有视觉语言模型在细粒度空间理解方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现良好，但缺乏对物体交互的细粒度空间理解能力，如精确3D定位、物体物理兼容性、物体可供性和多步空间规划等。
...
Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，显著提升了模型在对象交互推理方面的能力，并发布了数据集和生成管道。
...
TL;DR: 提出了Parts-Mamba混合模型，结合GCN和Mamba架构，通过部分特定扫描特征和部分-身体融合模块来增强骨架动作识别在遮挡情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中由于遮挡或通信质量问题，骨架数据往往不完整，现有GCN模型在缺少局部上下文时表现不佳。

Method: 设计Parts-Mamba混合GCN-Mamba模型，包含部分特定扫描特征和部分-身体融合模块，用于捕捉和保持远距离关节的上下文信息。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效处理不完整骨架数据，在遮挡情况下显著提升骨架动作识别性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein（JGW）目标，扩展了传统GW距离框架，支持多对多对象匹配，在形状匹配和生物分子复合物匹配中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统Gromov-Wasserstein距离仅限于单对象间的成对匹配，无法满足需要多对一或多对多对象匹配的应用场景需求。

Method: 通过引入JGW目标，将GW框架扩展到支持对象集合的同时匹配，采用最优传输算法（包括熵正则化）来求解点云对象表示。

Result: 与GW的其他变体相比，该方法在准确性和计算效率上表现优越，在合成和真实数据集上的实验验证了其在多形状匹配中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 在逆问题中，使用预训练生成模型作为先验时，通过强制对齐模型内部表示与自监督编码器的表示，可以提供强大的归纳偏置，提高收敛性和样本质量。

Method: 提出REPA方法，在推理时对齐扩散或流式模型与预训练自监督视觉编码器（如DINOv2）的表示，通过正则化引导重建过程，即使没有真实信号也能使用近似目标特征。

Result: 实验证明REPA在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提高了重建质量，同时通过减少离散化步骤数量提供了显著的效率提升。

Conclusion: REPA方法通过表示对齐有效提升了逆问题重建的感知保真度，具有通用性和效率优势，可集成到多种最先进的逆问题求解器中。
...
TL;DR: 提出NFGlassNet方法，利用闪光/无闪光图像中的反射动态特性进行玻璃表面检测，通过反射对比挖掘模块和反射引导注意力模块实现更准确的玻璃定位。


<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，未能充分利用玻璃本身的固有特性。观察到在真实场景中，玻璃前后光照强度差异导致反射变化，利用这一现象可改进检测精度。
...
TL;DR: 该论文提出了R-AVST数据集用于音频-视觉时空推理，包含5K个未修剪视频和27K个对象，覆盖100种音频-视觉事件。同时提出了AVST-Zero模型，使用强化学习直接优化行为，无需中间监督。
...
Method: 构建R-AVST数据集：使用基于LLM的关键对象提取、自动空间标注和人工质量检查的流水线。提出AVST-Zero模型：基于强化学习，通过精心设计的多维度奖励直接优化行为，避免中间监督。

Result: R-AVST数据集包含5K个未修剪视频、27K个对象和8K+高质量问答对。AVST-Zero在基准测试中表现出与现有模型竞争的性能，验证了R-AVST数据集在推进音频-视觉时空推理方面的有效性。
...
TL;DR: 本文提出Warm Diffusion模型，通过联合控制模糊和噪声，将热扩散（纯噪声）和冷扩散（纯模糊）两种范式统一起来，利用图像频谱依赖性简化得分模型估计。


<details>
  <summary>Details</summary>
Motivation: 热扩散未能利用高频细节与低频结构之间的强相关性，导致生成早期步骤出现随机行为；冷扩散虽然利用图像相关性进行预测，但忽略了噪声在塑造数据流形中的作用，导致流形外问题。

Method: 提出Warm Diffusion（模糊-噪声混合扩散模型BNMD），采用分治策略利用图像频谱依赖性，通过解耦去噪和去模糊过程简化得分模型估计，并分析模糊-噪声比（BNR）来研究模型学习动态与数据流形变化之间的权衡。
...
Method: 提出Delta-Denoising方法，使用最小提示对驱动两个共享调度的扩散分支进行对比，通过累积去噪差异生成定位图，指导潜在修复过程，同时进行标记级提示优化和空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了出色的生成效果、真实性和下游检测性能的持续提升。

Conclusion: DeltaDeno提供了一种无需训练和真实异常样本的零样本异常生成解决方案，在生成质量和下游任务性能方面表现优异。
...
TL;DR: 本文提出DGAF-VSR方法，通过特征域对齐和密集引导解决基于扩散模型的视频超分辨率中的误差累积、空间伪影和感知质量-保真度权衡问题。
...
Result: 在合成和真实数据集上的实验表明，DGAF-VSR在感知质量（DISTS降低35.82%）、保真度（PSNR提升0.20dB）和时间一致性（tLPIPS降低30.37%）方面优于现有方法。

Conclusion: 特征域对齐和密集引导能有效提升基于扩散模型的视频超分辨率性能，解决了现有方法的局限性。
...
Conclusion: 该方法有效减轻了形状扭曲，提供了解剖学上忠实的牙齿边界。
...
TL;DR: 该论文提出了OmniGround基准测试，用于评估视频时空定位任务，并开发了PG-TAF训练免费框架来提升复杂真实场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频时空定位模型在多样化对象和复杂查询的真实世界需求中存在显著差距，主要由于基准测试范围有限导致模型存在类别偏见、推理过于简化和语言鲁棒性差的问题。
...
Conclusion: OmniGround基准测试揭示了现有模型的局限性，PG-TAF框架通过分解时空定位任务显著提升了复杂场景下的性能表现。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型中个体级隐私推理能力的基准，揭示了现有模型在隐私推理方面的系统性漏洞。
...
Method: 提出隐私感知与推理(PPR)框架，构建包含合成个体档案的双语多模态数据集，设计9个挑战性任务评估从属性检测到跨图像重识别和链式推理的完整PPR谱系。

Result: 对50多个基础模型和商业VLM的大规模评估显示：(1)许多VLM存在显著的未测量推理型隐私风险；(2)感知级指标无法有效预测推理风险；(3)现有安全对齐对此类推理攻击不一致且无效。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光流方程作为物理正则化，无需预先估计运动场即可同时恢复时间一致的图像和运动场，在动态心脏MRI上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预先估计的光流，但在欠采样情况下不准确且会降低重建质量，需要一种能够同时恢复高质量图像和准确运动场的方法。

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光流场，通过光流方程耦合作为物理正则化，并结合数据一致性损失进行联合优化。

Result: 在动态心脏MRI数据集上的实验表明，该方法在重建质量、运动估计准确性和时间保真度方面优于最先进的运动补偿和深度学习方法。

Conclusion: 基于流正则化约束的隐式联合建模在动态MRI重建中具有重要潜力，能够同时获得高质量重建图像和准确运动估计。
...
TL;DR: 本文提出FingerCap任务，旨在生成捕捉手部动作细节的手指级语义文本描述，并构建了包含4万对手部动作视频和文本的FingerCap-40K数据集。为解决视频MLLMs的时间稀疏性问题，提出了FiGOP方法，通过结合RGB关键帧和后续手部关键点来恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解精细的人类手部动作对视觉感知、具身智能和多模态通信至关重要，但现有方法难以捕捉手指级的细微动态。

Method: 提出FiGOP方法，将每个RGB关键帧与后续手部关键点配对，通过轻量级时间编码器将关键点转换为运动嵌入并与RGB特征集成，在不增加RGB密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，强力的开源和闭源Video-MLLMs在手指级推理方面仍有困难，而FiGOP增强模型在HandJudge和人类研究中均获得一致提升。

Conclusion: FiGOP方法有效解决了视频MLLMs的时间稀疏性问题，显著提升了手指级手部动作理解的性能。
...
TL;DR: 本文提出了一种基于点监督的面部表情定位方法，通过双分支框架解决仅使用单时间戳标注的训练问题。核心创新包括高斯分布强度建模模块和对比学习损失函数，在多个数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖耗时的时间边界标注，本文旨在探索仅需单时间戳标注的点监督面部表情定位，以降低标注成本。

Method: 提出双分支框架：1）基于高斯分布的自适应强度建模模块进行软伪标签生成；2）类别感知的顶点分类分支区分宏表情和微表情；3）引入强度感知对比损失增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了所提框架的有效性。

Conclusion: 该点监督面部表情定位框架能够有效利用单时间戳标注，通过软伪标签和对比学习实现了良好的性能，为降低标注成本提供了可行方案。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为两个相互依赖的潜在空间：RGB外观和四个PBR贴图。将它们表述为5帧序列，采用视频扩散架构来捕捉相关性并转移RGB生成模型的视觉先验。

Result: 在MatHybrid-410K混合语料库上训练，MatPedia实现了原生1024×1024合成，在质量和多样性上都显著超越现有方法。

Conclusion: MatPedia提供了一个统一的框架，能够在单一架构内处理文本到材料生成、图像到材料生成和本征分解等多种材料任务。
...
TL;DR: 本文提出了一种边缘设备上基于原始食物图像合成逼真熟食图像的方法，通过引入首个带厨师标注熟度级别的烤箱烹饪进程数据集，以及边缘高效的食谱和烹饪状态引导生成器，显著提升了合成图像质量。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上从原始输入合成逼真熟食图像是一个具有挑战性的生成任务，需要模型捕捉烹饪过程中纹理、颜色和结构的复杂变化。现有图像生成方法要么产生不真实的结果，要么资源消耗过大不适合边缘部署。

Method: 引入首个带厨师标注熟度级别的烤箱烹饪进程数据集；提出边缘高效的食谱和烹饪状态引导生成器，根据原始食物图像合成逼真食物图像；引入领域特定的烹饪图像相似度(CIS)指标，既作为训练损失又作为进度监控信号。

Result: 模型在现有基准上表现优异，FID分数显著降低（在自有数据集上提升30%；在公共数据集上提升60%），实现了用户偏好视觉目标而非固定预设。

Conclusion: 该方法成功解决了边缘设备上烹饪图像合成的挑战，通过领域特定的相似度指标确保了时间一致性和烹饪合理性，为个性化烹饪视觉目标提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和新类别时容易陷入过自信，特别是在区分与已知类具有细粒度视觉相似性的'困难未知类'时表现不佳。

Method: 提出语义感知提示增强模块分解图像为判别性语义标记；采用双工对比学习保持与已知类的分离性和语义邻近性；使用语义引导扩散模块合成伪未知样本。

Result: 在五个基准测试中，相比最先进方法实现了3%准确率和5% H分数的持续提升。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集域泛化中的核心挑战，显著提升了模型性能。
...
Result: 相比3DGS，在15%预算下获得超过0.6 dB PSNR增益，在紧凑3DGS方面达到最先进性能。

Conclusion: 该方法完全可学习且无需人工干预，实现了高效的3D高斯分布剪枝，显著提升了紧凑3DGS的性能。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中进行身份替换来实现人脸去识别。它使用新颖的试剂损失来指导优化，发现身份编辑方向，并提出线性和测地线编辑方案来有效导航潜在流形。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间难以取得良好平衡。FLUID受到化学中替换机制的启发，旨在直接在预训练扩散模型的潜在空间中进行身份替换，实现更好的去识别效果。

Method: 将身份编辑重新解释为预训练无条件扩散模型潜在h空间中的语义位移。通过由新颖试剂损失指导的优化来发现身份编辑方向，这些损失监督属性保留和身份抑制。提出了线性和测地线（基于切线）编辑方案来导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验结果表明，FLUID在身份抑制和属性保留之间实现了优越的平衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过直接在预训练扩散模型的潜在空间中进行身份替换，实现了高效的人脸去识别，在身份抑制和属性保留方面表现出色，为隐私保护应用提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射性材料会产生层叠的深度模糊，而现有模型只能被动估计最近表面的静态深度图，无法像人类那样主动切换焦点来感知所需深度。

Method: 提出DepthFocus，一个基于标量深度偏好进行条件控制的视觉Transformer模型，主要使用新构建的50万张多层合成数据集进行训练，该数据集专门设计用于捕捉多样的透射效果。

Result: DepthFocus不仅在BOOSTER等传统单深度基准测试中达到最先进性能，还在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计能力，同时在未见过的透射场景中表现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动和类人3D感知迈出的重要一步，展示了在复杂场景中进行选择性深度感知的能力。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态标注和传统球位置标注，支持球追踪、球拍姿态估计和轨迹预测三个任务。
...
Conclusion: RacketVision为动态物体追踪、条件运动预测和体育多模态分析提供了多功能资源和强有力起点。
...
TL;DR: ConfusedTourist是一个评估视觉语言模型文化对抗鲁棒性的新套件，发现模型在面对混合文化线索时准确率大幅下降，注意力会系统性地转向干扰线索。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型评估通常只关注单一文化概念，忽视了多文化线索共存场景，而模型在多样化文化输入中的稳定性对于支持多元文化社会至关重要。

Method: 引入ConfusedTourist文化对抗鲁棒性套件，通过图像堆叠和基于图像生成的扰动来测试模型对地理线索扰动的稳定性。
...
TL;DR: 提出OmniPT框架，将行人跟踪任务建模为基础模型可执行的任务，通过RL-Mid Training-SFT-RL训练阶段实现跟踪、基于参考的跟踪和语义理解功能。


<details>
  <summary>Details</summary>
Motivation: LVLMs在图像级任务表现优异，但在实例级任务如行人跟踪中与专家模型存在性能差距，而结合对象跟踪和自然语言的新任务需要高级语义理解能力。

Method: 基于预训练LVLM权重，采用RL-Mid Training-SFT-RL训练流程：先通过RL使模型输出可监督的边界框格式，再用大量行人相关数据集进行中间训练，然后在多个行人跟踪数据集上进行监督微调，最后通过RL提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试中的实验结果表明，该方法性能优于先前方法。

Conclusion: OmniPT框架成功将行人跟踪任务建模为基础模型可执行的任务，实现了统一的跟踪、参考跟踪和语义理解功能，在基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI检查。现有方法从CT合成MRI，但低剂量协议导致CT体积稀疏且分辨率差，难以准确重建完整脑部MRI体积。

Method: 提出ReBrain框架：1）使用布朗桥扩散模型在2D维度合成MRI切片；2）通过微调检索模型从先验数据库中检索结构和病理相似的CT切片；3）通过ControlNet分支将检索切片作为参考，确保结构连续性；4）对罕见检索失败情况应用球面线性插值提供补充指导。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain能够有效解决稀疏CT条件下脑部MRI重建的挑战，为无法进行MRI检查的患者提供了可行的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中暴露随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择灵活性受限。需要一种能结合图像先验知识（如中心偏差和局部性）的方法。

Method: STAR利用图像补丁位置网格中均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的部分观察作为序列前缀出现。

Result: STAR在保持采样性能的同时提供了灵活的序列顺序，无需对语言AR建模中广泛采用的模型架构进行重大更改。

Conclusion: STAR通过定制但结构化的随机化策略，相比随机排列，保持了后缀完成能力，同时维持了采样性能。
...
TL;DR: REArtGS++是一种基于平面高斯抛光和时序几何约束的通用化铰接物体重建方法，通过解耦螺旋运动建模和部件运动混合，在部件级表面重建和关节参数估计方面优于现有方法。
...
Method: 提出解耦螺旋运动建模方法，无需关节类型先验，通过部件运动混合联合优化部件感知高斯和关节参数；引入时间连续几何约束，鼓励高斯平面化，并通过泰勒一阶展开实现平面法向和深度的时序一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在通用化部件级表面重建和关节参数估计方面具有优越性。

Conclusion: REArtGS++通过时序几何约束和平面高斯抛光，实现了更通用的铰接物体重建，在复杂关节类型和多部件场景中表现出色。
...
### [37] [Planning with Sketch-Guided Verification for Physics-Aware Video Generation](https://arxiv.org/abs/2511.17450)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要使用单次规划或迭代细化，前者限于简单运动，后者计算成本高。需要一种既能生成高质量动态一致轨迹又高效的方法。

Method: 提出草图验证规划框架：预测多个候选运动计划，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代优化直到找到满意方案。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。扩展轨迹候选数量能持续提升性能。

Conclusion: SketchVerify通过草图验证规划有效解决了视频生成中运动规划的质量和效率问题，实现了更好的动态一致性和物理合理性。
...
TL;DR: 本文提出了DiverseVAR方法，通过抑制特征图中的关键组件在模型输入中的影响并在输出中放大它，有效解决了VAR模型的多样性崩溃问题，无需额外训练即可显著提升生成多样性。


<details>
  <summary>Details</summary>
Motivation: 尽管VAR模型在推理效率和图像质量方面具有优势，但它们经常遭受多样性崩溃问题，类似于在少步蒸馏扩散模型中观察到的输出变异性减少现象。

Method: 提出DiverseVAR方法，分析发现特征图中的关键组件是早期尺度多样性形成的关键因素。通过在模型输入中抑制关键组件并在模型输出中放大它，有效解锁VAR模型的固有生成潜力。

Result: 实证结果表明，该方法显著增强了生成多样性，仅对性能产生可忽略的影响。

Conclusion: DiverseVAR是一种简单而有效的方法，能够在不需额外训练的情况下恢复VAR模型的生成多样性，同时保持高保真合成质量。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在多种任务上表现出色，但它们仍然容易产生幻觉。本研究旨在深入理解幻觉产生的机制并提出有效的干预方法。

Method: 提出了与Transformer因果架构对齐的综合干预框架，分析不同干预路径对幻觉的影响，并针对判别式和生成式格式设计了识别和干预关键幻觉头的方法。

Result: 实验表明，该方法在多个基准测试中能够持续减少不同对齐类型的幻觉现象。

Conclusion: LVLMs的幻觉并非来自单一因果路径，而是多种路径相互作用的结果，且模型依赖的路径因问答对齐格式而异，针对性的干预方法能有效减少幻觉。
...
TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像，利用凸包语义掩码指导扩散模型生成新的合成LiDAR点云，以解决自动驾驶训练数据中边缘案例不足的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样化的点云数据，特别是复杂的边缘案例场景。从真实世界获取这些数据具有挑战性，而现有的3D虚拟环境模拟方法耗时且难以完全捕捉真实场景的复杂性。

Method: 将LiDAR点云转换为2D距离图像作为中间表示，使用基于凸包的语义掩码进行条件化，通过扩散模型实现语义引导的点云生成，确保几何一致性和真实感。
...
Result: 在UCF-Crime和XD-Violence数据集上，仅处理28.55%和16.04%的帧就实现了最先进的训练免费性能，证明了稀疏推理对于基于大模型的视频异常检测是足够的。

Conclusion: 研究表明稀疏推理足以实现有效的基于大模型的视频异常检测，显著减少了计算负担。
...
### [43] [Where Culture Fades: Revealing the Cultural Gap in Text-to-Image Generation](https://arxiv.org/abs/2511.17282)
...
TL;DR: 本文分析了多语言文本到图像模型在跨语言文化一致性方面的问题，发现现有模型往往产生文化中性或英语偏见的结果。通过定位文化敏感神经元，提出了两种对齐策略来提升文化一致性。


<details>
  <summary>Details</summary>
Motivation: 多语言文本到图像模型虽然发展迅速，但输出在不同文化背景下存在差异。由于语言承载文化内涵，多语言提示生成的图像应保持跨语言文化一致性，而现有模型在这方面存在不足。

Method: 提出了一种探测方法，将文化敏感信号定位到少数固定层中的一小部分神经元。基于此发现，引入了两种互补的对齐策略：推理时文化激活（无需主干微调）和层定向文化增强（仅更新文化相关层）。

Result: 在CultureBench上的实验表明，相比强基线方法，所提方法在文化一致性方面取得了持续改进，同时保持了保真度和多样性。

Conclusion: 通过定位和激活文化敏感神经元，可以有效提升多语言文本到图像模型的文化一致性，而无需对整个模型进行大规模微调。
...
TL;DR: 本文提出了一种名为分区自适应对比学习（PACL）的方法，通过利用预训练文本模型的知识来增强预训练视觉模型的情感感知能力，以解决视觉情感识别中的'情感鸿沟'问题。
...
Method: 提出分区自适应对比学习方法（PACL），关注社交媒体数据中图像和文本之间的事实和情感联系，通过分离不同类型的样本并为每种类型设计不同的对比学习策略，动态构建正负样本对，充分利用噪声样本的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要优化语义理解而非几何推理，CroCo虽然针对3D理解但仍有改进空间，本文旨在学习更适合3D视觉的特征表示。

Method: 扩展MAE到任意多视图，对所有视图统一掩码，使用轻量级解码器和帧间注意力机制，比CroCo更简单且可扩展。

Result: 在多种下游任务评估中，包括前向重建、密集图像匹配和相对姿态估计，MuM模型均优于当前最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过多视图掩码自编码方法，MuM成功学习了更适合3D视觉任务的特征表示，在多个几何推理任务上表现出色。
...
Conclusion: PEGS框架成功解决了大时空尺度刚性运动重建的挑战，通过物理先验和事件流增强实现了优越的运动恢复性能。
...
TL;DR: 本文提出了一种使用生成模型创建多样化合成人脸图像的方法，并发布了包含27,780张图像的DIF-V数据集，旨在解决现有面部验证系统中的偏见问题。
...
Method: 整合先进的生成模型创建多样化高质量合成人脸图像，强调代表各种面部特征，确保符合身份证照片的特征要求。

Result: 分析显示现有验证模型对某些性别和种族存在偏见，身份风格修改会对模型性能产生负面影响。

Conclusion: 这项工作通过解决现有数据集中的固有不平等问题，不仅丰富了人工智能多样性和伦理的讨论，还为开发更具包容性和可靠性的面部验证技术奠定了基础。
...
TL;DR: 本文提出了一种从1D相关色温(CCT)到2D色度空间的转变，使用轻量级多层感知器(MLP)进行比色映射，显著提高了非普朗克光源(如LED)下的色彩精度。
...
Method: 采用2D色度空间替代1D CCT，使用轻量级MLP利用2D色度特征进行稳健的比色映射，通过包含代表性LED光源的光箱校准程序训练MLP。

Result: 在多种LED照明下验证，该方法平均减少LED照明场景中角度再现误差22%，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT转向2D色度空间结合MLP的方法显著提高了非普朗克光源下的比色精度，同时保持计算效率和兼容性。
...
TL;DR: 提出一个多阶段优化框架，将高性能浮点图像压缩模型转换为高效的8位整数FPGA实现，通过动态范围感知量化、混合精度搜索和通道剪枝等技术，在保持性能的同时显著降低计算复杂度。
...
Method: 1. 动态范围感知量化(DRAQ)：统计校准的激活裁剪和权重正则化；2. 渐进混合精度搜索算法；3. 针对GDN层的通道剪枝方法。

Result: DRAQ将BD-rate开销从30%降至6.3%，硬件感知优化进一步降低20%以上计算复杂度，最终模型在效率和性能上均优于现有FPGA实现。
...
TL;DR: CC-DiceCE损失函数基于CC-Metrics框架，旨在解决医学图像分割中传统Dice损失对小病灶分割不足的问题，通过实例级评估提高检测召回率。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）由于小病灶相对体积小，对整体损失贡献可忽略，导致小病灶分割不足。需要实例级损失函数来改善小病灶检测性能。

Method: 基于CC-Metrics框架开发CC-DiceCE损失函数，并与现有的blob损失在nnU-Net框架中进行比较，使用标准化设置进行基准测试。

Result: CC-DiceCE损失提高了检测召回率，分割性能仅有轻微下降，但会产生稍多的假阳性。在多数据集研究中，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效提高小病灶检测性能，在保持分割质量的同时显著改善召回率，是优于现有blob损失的解决方案。
...
TL;DR: 该论文介绍了RSFM数据库和REMSA智能体，用于解决遥感领域基础模型选择困难的问题。RS-FMD包含150多个遥感基础模型，REMSA是基于LLM的智能体，能够通过自然语言查询自动选择合适模型。
...
Method: 构建RSFM数据库覆盖150多个模型，开发REMSA智能体，使用自然语言查询、上下文学习和透明化推理来选择和推荐模型。

Result: 在75个专家验证的查询场景和900个配置下，REMSA优于多个基线方法，包括朴素智能体、密集检索和非结构化RAG-based LLM。
...
TL;DR: DiffRefiner是一个两阶段轨迹预测框架，通过结合判别式轨迹提议和生成式细化，在自动驾驶轨迹预测中实现了最先进的性能。
...
Method: 提出两阶段框架：第一阶段使用基于Transformer的提议解码器从传感器输入回归生成粗略轨迹预测；第二阶段应用扩散细化器迭代去噪和优化这些初始预测。
...
TL;DR: UI-Styler是一种针对超声图像的类感知风格迁移框架，通过模式匹配机制将目标域纹理模式转移到源图像，同时保持源结构内容，并使用基于目标域伪标签的类感知提示策略实现语义对齐。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响固定黑盒下游推理模型的性能。现有无配对图像翻译方法在域适应过程中往往忽视类特定语义对齐，导致内容-类别映射错位，损害诊断准确性。

Method: 提出UI-Styler框架：1）使用模式匹配机制将目标图像中的纹理模式转移到源图像；2）引入基于目标域伪标签的类感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）中持续优于现有无配对图像翻译方法，达到最先进性能。

Conclusion: UI-Styler通过类感知风格迁移有效解决了超声图像跨设备域适应问题，在保持结构内容的同时实现语义对齐，显著提升了诊断准确性。
...
TL;DR: FireScope是一个基于视觉语言模型(VLM)的推理生成框架，通过结合Sentinel-2影像和气候数据预测野火风险图，并在跨大陆测试中表现出优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有野火风险预测方法缺乏因果推理和多模态理解能力，导致泛化性能不足。需要开发能够整合视觉、气候和地理因素的综合推理框架。

Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格；开发FireScope框架，通过强化学习和视觉监督学习预测风险栅格并生成推理轨迹。

Result: 在美国训练、欧洲测试的跨大陆评估中，FireScope取得显著性能提升；专家反馈和自动化分析确认其推理轨迹具有忠实性和语义意义。

Conclusion: 基于语言的推理能够提升视觉生成模型的泛化能力和可解释性，FireScope-Bench有望成为推进推理驱动、可解释和可泛化空间建模的基础。
...
Conclusion: 自监督表征在深度伪造检测中具有前景，能学习有意义的模式，但实现稳健的跨域性能仍然困难。
...
TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹，通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，实现更精确和灵活的运动操控。
...
Conclusion: PostCam通过统一表示和控制信号融合，显著提升了动态场景中相机轨迹编辑的精度和生成视频的视觉质量。
...
TL;DR: 本文提出VLA-4D模型，通过4D感知的视觉表示和时空动作表示，解决了视觉-语言-动作模型在时空一致性操作中的挑战。
...
Method: 1) 4D感知视觉表示：提取视觉特征，将1D时间嵌入3D位置形成4D嵌入，通过跨注意力机制融合为统一视觉表示；2) 时空动作表示：扩展传统空间动作表示加入时间信息，实现时空规划，并将多模态表示对齐到LLM中进行时空动作预测。

Result: 在机器人操作的不同任务中进行了广泛实验，验证了该方法在时空一致性操作方面的优越性。

Conclusion: VLA-4D模型通过统一的4D感知框架，实现了空间平滑和时间一致的机器人操作，解决了现有方法在时空一致性控制方面的局限性。
...
TL;DR: 本文提出CA-SAM方法，通过轻量级的对齐层模块和持续学习策略，在医学图像分割中平衡SAM模型的计算效率和性能，解决异构隐私政策下的持续学习问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的异构隐私政策使得联合训练不可行，需要从数据流中持续学习而不发生灾难性遗忘。虽然SAM模型具有强大的零样本先验，但其参数量大、计算开销高，限制了实际部署。
...
Result: 在九个医学分割数据集的持续学习场景下进行实验，CA-SAM实现了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM通过轻量级对齐层和持续学习策略有效解决了医学图像分割中的持续学习挑战。
...
Method: 首先通过轻量级跟踪和重建模块构建局部一致的子图，然后逐步对齐和融合到全局高斯地图中，强制跨视角几何一致性，全局地图反馈纠正局部漂移。
...
Method: 联合优化局部Transformer进行高分辨率体素特征提取和全局Transformer进行全扫描上下文建模，结合DINO风格自蒸馏和基于SigLIP的视觉-语言对齐使用配对放射学报告。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下始终优于先前的CT基础模型。

Conclusion: SPECTRE是一个可扩展、开放且完全基于Transformer的3D医学成像基础模型，证明无需依赖私有数据即可获得高性能、可泛化的表示。
...
Conclusion: 该框架能够生成连续、不确定性感知的语义地图，有效处理鱼眼图像的几何挑战。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接从图像学习连续3D语义占据，在保持恒定内存的同时实现长距离监督和推理，在Occ3D-nuScenes基准上比之前方法提升26%语义RayIoU。


<details>
  <summary>Details</summary>
Motivation: 解决从图像学习3D场景几何和语义的挑战，避免昂贵的大规模3D标注，通过自监督学习直接从传感器数据获取3D结构信息。

Method: 使用基于查询的自监督框架，通过独立4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入压缩场景表示保持近场细节同时平滑压缩远场区域。
...
TL;DR: 提出了一种基于四元数保真度项的彩色图像盲反卷积方法，通过四元数卷积核建模颜色通道间的相互关系，有效去除伪影并显著改善去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转为灰度或单独处理每个颜色通道，忽略了颜色通道间的关系，导致去模糊效果不佳。

Method: 设计了一个新颖的四元数保真度项，利用四元数卷积核的特性，包含四个核：一个捕获整体模糊的非负卷积核和三个无约束的对应RGB通道的卷积核来建模通道间未知的相互依赖关系，并使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法能有效去除伪影并显著改善去模糊效果。

Conclusion: 该方法作为彩色图像反卷积的强大工具具有潜力，通过建模颜色通道间关系显著提升了盲反卷积性能。
...
TL;DR: 本文提出了一种基于传统文本搜索的相关性反馈机制，用于改进视觉语言模型在推理时的检索性能，包括伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈四种策略。


<details>
  <summary>Details</summary>
Motivation: 提高视觉语言模型的检索性能通常需要微调和扩展到更大的模型变体，本文旨在通过模型无关的相关性反馈机制来改进检索效果，既可以作为微调的替代方案，也可以与微调后的模型配合使用。

Method: 提出了四种反馈策略：1）伪相关性反馈（PRF），基于排名靠前的结果优化查询嵌入；2）生成式相关性反馈（GRF），使用合成字幕进行查询优化；3）注意力反馈总结器（AFS），基于Transformer的定制模型，整合相关项目的多模态细粒度特征；4）使用真实字幕的显式反馈作为上限基线。

Result: 在Flickr30k和COCO数据集上的实验表明，与无反馈检索相比，GRF、AFS和显式反馈使较小VLM的MRR@5提高了3-5%，较大VLM提高了1-3%。AFS与显式反馈类似，能够减轻查询漂移，在迭代多轮检索设置中比GRF更稳健。

Conclusion: 相关性反馈可以持续增强不同VLM的检索性能，为交互式和自适应视觉搜索开辟了新的机会。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，通过预训练、多粒度微调和强化学习优化，专门解决立体化学信息识别难题，在立体化学结构识别方面达到最先进性能。
...
Conclusion: MolSight框架通过创新的三阶段训练方法有效解决了OCSR中的立体化学识别难题，为化学信息学、药物发现和相关领域的LLM应用提供了强大的工具。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，显著提升了姿态参数预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要依赖电容图像，但只能估计俯仰角和偏航角，且在大角度输入时精度下降。需要一种能够同时准确预测全面指尖姿态信息的方法。

Method: 提出双模态输入方法，包括从带有屏下指纹传感器的触摸屏获取的电容图像和指纹补丁。这种方法能够可靠估计仅使用单一模态无法实现的滚转角。
...
TL;DR: 本文提出SpatialGeo，一种基于几何和语义特征层次融合的新型视觉编码器，旨在提升多模态大语言模型的空间推理能力，解决现有模型在三维空间空间关系理解方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 大多数多模态大语言模型在解释和推理三维空间中的空间排列方面存在空间推理能力有限的问题，这主要源于现有视觉编码器（如CLIP）的损失性嵌入，仅限于实例级语义特征。

Method: 提出基于几何和语义特征层次融合的视觉编码器，通过层次适配器将视觉自监督学习中的几何特征与CLIP语义特征互补，增强空间感知能力。使用预训练的LLaVA模型进行高效训练，并通过随机特征丢弃优化以避免仅依赖CLIP编码器的平凡解。

Result: 实验结果显示，SpatialGeo在空间推理任务中提高了准确性，在SpatialRGPT-Bench上将最先进模型的准确率提升了至少8.0%，同时推理期间内存成本减少了约50%。

Conclusion: SpatialGeo通过层次融合几何和语义特征，有效增强了多模态大语言模型的空间感知能力，在空间推理任务中取得了显著改进，同时降低了计算成本。
...
Method: 采用相对姿态初始化与显式特征匹配，然后进行局部联合优化，最后通过包含特征轨迹的束调整进行全局优化。

Result: 在基准数据集上的广泛评估表明，该方法在姿态估计精度和新视图合成质量方面均优于现有最先进方法，在挑战性场景中表现出鲁棒性。

Conclusion: NoPe-NeRF++是首个将局部和全局线索与NeRF无缝结合的方法，验证了设计选择的合理性，在复杂场景中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 生成图像模型在处理透明物体时表现不佳，特别是折射效果，因为它们未能充分学习光学定律。透明物体的折射会与图像其他部分的表面相交，对颜色产生约束，而现有模型无法准确渲染这种效果。

Method: 在生成过程的每一步中，使用斯涅尔折射定律对物体边界内外的像素进行扭曲和合并。对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图同步，使用相同的扭曲和合并过程来恢复其外观。

Result: 该方法生成的图像在光学上更加合理，能够更好地遵守物理约束，相比现有方法产生了更符合光学原理的折射效果。

Conclusion: 通过同步像素和应用光学物理定律，可以显著改善生成图像中透明物体的折射渲染质量，生成更真实的光学效果。
...
TL;DR: 提出一个统一框架用于多媒介绘画过程生成，通过语义驱动的风格控制机制将多种媒介嵌入扩散模型的条件空间，并使用跨媒介风格增强，实现一致的纹理演化和过程转移。


<details>
  <summary>Details</summary>
Motivation: 现有绘画教程视频缺乏交互性和个性化，而生成模型在跨媒介泛化和时间一致性方面存在困难，无法忠实再现人类创作流程。

Method: 采用语义驱动风格控制机制，将多种媒介嵌入扩散模型条件空间，使用跨媒介风格增强和反向绘画训练策略，确保平滑且符合人类创作习惯的生成过程。

Result: 构建了大规模真实绘画过程数据集，在跨媒介一致性、时间连贯性和最终图像保真度方面取得优异结果，在LPIPS、DINO和CLIP指标上表现强劲。

Conclusion: 提出的感知距离轮廓(PDP)曲线能够定量建模创作序列（构图、色块划分、细节精炼），准确反映人类艺术创作进程。
...
TL;DR: DSeq-JEPA提出了一种基于人类视觉感知的序列化联合嵌入预测架构，通过识别主要判别区域并按重要性顺序预测，将JEPA的潜在预测与GPT风格的序列推理相结合。


<details>
  <summary>Details</summary>
Motivation: 现有的I-JEPA方法对所有区域进行统一独立处理，缺乏对预测顺序和位置的明确指导。受人类视觉选择性注意和序列处理机制的启发，需要开发能够按重要性顺序进行预测的自监督学习方法。

Method: 首先基于Transformer衍生的显著性图识别主要判别区域，然后按判别性顺序预测后续区域，形成从主要到次要线索的课程式语义进展，整合了JEPA风格的潜在预测和GPT风格的序列推理。

Result: 在图像分类、细粒度视觉分类、检测分割以及低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA通过引入判别性序列预测机制，有效结合了预测性和自回归自监督学习的优势，在多个视觉任务上取得了优于现有方法的性能。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占用估计方法，通过多层级球面高斯近似实现训练监督，相比高斯方法减少75%内存占用、提升124%推理速度，并在mIoU上提升5.9%。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占用估计中广泛应用，但大量高斯基元显著增加内存需求，不适合实时推理。超二次曲面允许减少基元数量和内存需求，但缺乏超二次曲面光栅化器来实现模型监督。

Method: 采用超二次曲面场景表示，通过多层级球面细分高斯近似超二次曲面，实现高斯光栅化用于训练监督。

Result: 在Occ3D数据集上，相比之前的高斯方法，内存占用减少75%，推理速度提升124%，mIoU提升5.9%，基元数量减少84%。

Conclusion: 这是首个实现实时推理同时保持竞争力的占用模型，超二次曲面的使用显著减少了场景建模所需的基元数量。
...
TL;DR: 提出ATAC方法，通过在CLIP嵌入空间计算增强诱导的漂移向量来推断语义恢复方向，有效提升对抗性攻击下的鲁棒性，性能超越现有方法近50%。


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配中表现出色，但对图像对抗性扰动高度脆弱。由于对抗性微调成本过高，现有测试时防御策略的鲁棒性仍然有限。

Method: ATAC直接在CLIP嵌入空间中操作，计算增强诱导的漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来校正嵌入。

Result: 在广泛基准测试中，ATAC始终实现极高的鲁棒性，平均超越先前最先进方法近50%，同时计算开销极小。在非常规和极端设置下保持最先进鲁棒性，甚至对自适应攻击也实现非平凡鲁棒性。
...
TL;DR: SVRecon将稀疏体素光栅化与符号距离函数(SDF)结合用于高保真表面重建，通过几何初始化和空间平滑损失解决稀疏体素优化中的局部极小值问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素在空间上与其邻居解耦且边界锐利，这使得它们在优化过程中容易陷入局部极小值。虽然SDF值提供了自然平滑连续的几何场，但在独立参数化的稀疏体素之间保持这种平滑性具有挑战性。

Method: 1) 使用视觉几何模型进行鲁棒几何初始化；2) 通过空间平滑损失强制父-子体素组和兄弟体素组之间的连贯关系，促进连贯平滑的体素结构。

Result: 在各种基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon通过整合SDF和稀疏体素光栅化，成功解决了稀疏体素优化中的局部极小值问题，实现了高保真表面重建。
...
TL;DR: 本文提出了非参数概率鲁棒性(NPPR)作为对抗鲁棒性的补充指标，解决了现有概率鲁棒性方法依赖预定义扰动分布的局限性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对微小输入扰动敏感，现有概率鲁棒性方法假设固定已知的扰动分布，这在实践中不现实。

Method: 采用非参数统计建模范式，直接从数据中学习优化的扰动分布，基于高斯混合模型和多层感知机头以及双三次上采样开发NPPR估计器。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验表明，NPPR比现有方法保守40%，提供更实用的鲁棒性评估。

Conclusion: NPPR是一个更实用的鲁棒性度量标准，能在分布不确定性下进行保守的概率鲁棒性评估。
...
TL;DR: MorphSeek是一个用于可变形图像配准的细粒度表示级策略优化框架，通过将配准问题重新表述为潜在特征空间中的空间连续优化过程，实现了高效探索和从粗到细的优化。
...
Method: 在编码器顶部引入随机高斯策略头来建模潜在特征的分布，结合无监督预热和弱监督微调，通过多轨迹采样稳定训练并提高标签效率。

Result: 在三个3D配准基准测试（OASIS脑MRI、LiTS肝脏CT和腹部MR-CT）中，相比竞争基线实现了持续的Dice改进，同时保持高标签效率和低参数成本。

Conclusion: MorphSeek推进了表示级策略学习范式，实现了空间一致且数据高效的变形优化，为高维设置中的可扩展视觉对齐提供了原理性、主干无关和优化器无关的解决方案。
...
TL;DR: 本文提出了一种名为MCMoE的多模态动作质量评估框架，通过专家混合模型处理推理阶段可能缺失的模态，在完整和不完整多模态学习场景下都取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现实应用中多模态动作质量评估经常面临部分模态缺失的问题，现有方法在模态缺失时无法正常工作且性能急剧下降，需要解决这一挑战。

Method: 提出MCMoE框架，包含自适应门控模态生成器重构缺失模态，设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示，通过专家混合进一步细化和补充缺失模态。

Result: 在三个公开AQA基准测试中，MCMoE在完整和不完整多模态学习场景下都实现了最先进的性能。

Conclusion: MCMoE框架有效解决了多模态动作质量评估中的模态缺失问题，统一了单模态和联合表示学习，在现实应用中具有重要价值。
...
TL;DR: 提出了MMT-ARD框架，通过多教师知识蒸馏提升视觉语言模型的对抗鲁棒性，在ImageNet上实现+4.32%的鲁棒准确率和+3.5%的零样本准确率提升，训练效率提高2.3倍。


<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗知识蒸馏方法存在知识多样性有限、收敛慢、难以平衡鲁棒性和准确性等问题，需要更有效的解决方案来提升视觉语言模型在安全关键应用中的对抗鲁棒性。

Method: 提出多模态多教师对抗鲁棒蒸馏框架，采用双教师知识融合架构，结合动态权重分配策略和自适应sigmoid加权函数，协同优化清洁特征保持和鲁棒特征增强。

Result: 在ImageNet和零样本基准测试中，ViT-B-32模型的鲁棒准确率提升+4.32%，零样本准确率提升+3.5%，训练效率比传统单教师方法提高2.3倍。

Conclusion: MMT-ARD框架有效提升了多模态大模型的对抗鲁棒性，证明了其有效性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要关注基于事实观察的前向模拟，但许多新兴应用需要回答反事实查询，如"如果移除这个物体会发生什么？"，因此需要开发能够处理干预输入的反事实世界模型。

Method: CWMDT框架包含三个步骤：1) 构建场景的数字孪生，以结构化文本形式编码对象及其关系；2) 使用大语言模型推理反事实干预如何随时间传播并改变观察场景；3) 使用修改后的表示条件化视频扩散模型来生成反事实视觉序列。

Result: 在两个基准测试上的评估显示，CWMDT方法实现了最先进的性能，表明视频的替代表示（如数字孪生）为基于视频前向模拟的世界模型提供了强大的控制信号。

Conclusion: CWMDT成功地将标准视频扩散模型转化为有效的反事实世界模型，通过数字孪生表示实现了对特定场景属性的针对性干预，为物理AI行为评估等应用提供了重要工具。
...
TL;DR: Radar2Shape是一个去噪扩散模型，通过将雷达信号频率与多分辨率形状特征关联，从部分可观测的雷达信号进行3D重建。该方法采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后以从粗到细的方式基于雷达信号频率扩散到该空间。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定3D物体形状在商业和航空航天应用中至关重要，但现有深度学习方法难以表示任意形状或处理有限视角收集的真实雷达信号。光学3D重建方法虽然能从有限视角生成任意形状，但将雷达信号简单视为相机视图时会遇到困难。

Method: 提出Radar2Shape两阶段方法：1）学习具有层次分辨率形状特征的规范化潜在空间；2）以从粗到细的方式基于雷达信号频率扩散到该潜在空间。该方法将雷达信号频率与多分辨率形状特征相关联。

Result: Radar2Shape能够成功从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实世界数据上展现出鲁棒的泛化能力。同时发布了两个合成基准数据集以促进未来研究。

Conclusion: Radar2Shape通过将雷达信号频率与多分辨率形状特征关联，有效解决了从部分可观测雷达信号进行3D重建的挑战，为将此类模型安全适应到真实世界雷达系统奠定了基础。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并发现脊柱年龄差与退行性病变和生活方式因素相关。


<details>
  <summary>Details</summary>
Motivation: 脊柱是复杂的结构，容易发生与年龄相关的退行性病变，通过MRI可以识别这些病变。需要开发一种方法来量化脊柱健康状况。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性病变聚类，建立深度学习模型，通过消融研究选择最佳模型配置。

Result: 模型预测的脊柱年龄与实际年龄的差值（脊柱年龄差）与椎间盘突出、骨赘、椎管狭窄、骨折等病变相关，也与吸烟和体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差可能作为衡量整体脊柱健康的有用生物标志物，与退行性病变和生活方式因素密切相关。
...
TL;DR: 本文分析了多模态模型中LLM规模缩减对视觉能力的影响，发现视觉能力比语言能力下降更严重，并提出了Extract+Think方法来解决这一瓶颈。
...
Method: 首先分析LLM规模缩减对视觉能力的影响，然后提出视觉提取调优方法，训练模型提取任务相关的视觉细节，最后采用逐步推理生成答案，形成Extract+Think方法。

Result: 研究发现LLM规模缩减对视觉能力的影响比对语言能力的影响更严重，视觉提取调优方法有效缓解了这一瓶颈，在效率和性能方面设立了新标准。

Conclusion: Extract+Think方法通过分离视觉提取和推理过程，有效解决了LLM规模缩减带来的视觉能力下降问题，为高效多模态系统提供了新思路。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大信息区域、重新编码像素并更新推理状态，在文本丰富视频理解任务上取得SOTA效果。
...
Method: 提出视觉反刍：迭代选择帧、放大信息区域、重新编码像素、更新推理状态。构建两个数据集，采用多阶段反刍学习框架，通过SFT和GRPO-based RL逐步微调7B LMM。
...
TL;DR: 提出了一种新的原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法在速度和一致性方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法存在严重限制：基于优化的方法速度过慢，而基于多视图2D编辑的前馈方法常常出现几何不一致和视觉质量下降的问题。

Method: 创建大规模多模态指令引导3D编辑数据集，探索两种条件策略：传统交叉注意力机制和新型3D令牌拼接方法。

Result: 令牌拼接方法在参数效率和性能方面表现更优，在生成质量、3D一致性和指令忠实度方面超越了现有的2D提升方法。

Conclusion: 该方法为指令引导的3D编辑设立了新的基准，在保持效率的同时显著提升了编辑质量。
...
TL;DR: 本文提出了语义金字塔索引（SPI），一种用于向量数据库的多分辨率索引框架，通过查询自适应分辨率控制来解决现有RAG系统中检索速度与语义相关性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的向量数据库检索管道依赖于扁平或单分辨率索引结构，无法适应不同用户查询所需的变化语义粒度，导致检索速度和上下文相关性之间的次优权衡。

Method: SPI在文档嵌入上构建语义金字塔，通过轻量级分类器为每个查询动态选择最佳分辨率级别，实现从粗到细的渐进式检索。

Result: 在多个RAG任务上的评估显示，SPI实现了高达5.7倍的检索加速和1.8倍的内存效率提升，同时将端到端QA F1分数提高了最多2.5个点。

Conclusion: SPI框架与现有向量数据库基础设施兼容，使其易于在生产RAG系统中部署，为检索增强生成系统提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试工具目标单一，无法整合系统指标和任务特定指标，缺乏用户友好的统一基准测试框架来支持多种推理引擎、使用场景和量化级别。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动对选定的LLM、推理引擎和量化级别进行基准测试，涵盖单流、批处理和服务器等使用场景。

Result: 在四个常见LLM任务、三个硬件平台和四个推理引擎上的测试结果显示，任务性能与系统级效率之间存在多种权衡，不同推理引擎和模型存在显著差异。

Conclusion: 本地推理没有单一最佳设置，这强烈需要像Bench360这样的框架来帮助用户找到最佳配置。
...
TL;DR: 本研究评估了大型语言模型对低资源语言突尼斯阿拉伯语的理解能力，创建了包含突尼斯方言、标准阿拉伯语和英语的平行数据集，并在转写、翻译和情感分析任务上测试了多个模型。
...
Method: 创建包含突尼斯方言、标准阿拉伯语和英语的平行数据集及情感标签，在转写、翻译和情感分析三个任务上对多个流行大语言模型进行基准测试。
...
TL;DR: 本文提出EliDecide方法，通过椭圆体决策边界解决文本开放意图分类问题，相比传统球形边界能更好地适应不同特征方向的分布差异。


<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类别呈各向同性分布，将边界限制为球形，忽视了不同方向的分布差异，限制了开放意图检测的性能。

Method: 首先使用监督对比学习获得判别性特征空间，然后通过可学习矩阵参数化椭圆体作为已知类别边界，最后通过双损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准数据集和问题分类数据集上达到最先进性能，椭圆体边界的灵活性展现出优越的开放意图检测能力和泛化潜力。
...
TL;DR: 本文提出了一种评估提示词是否能有效引导生成文本朝向特定人类价值观的方法，通过量化目标价值观在生成响应中的存在和增益来评分，并在Wizard-Vicuna模型上验证了价值观引导的可能性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的对齐方法主要是静态微调，无法适应涉及动态价值观和偏好的日常情境，需要开发能够有效引导模型响应朝向特定人类价值观的实用方法。

Method: 提出了一种实用、可重复且模型无关的程序，使用Schwartz基本人类价值观理论和对话数据集进行结构化评估，比较基线提示与明确基于价值观的提示的效果。

Result: 实验表明，即使不改变模型或动态优化提示，价值观引导也是可能的，明确基于价值观的提示能够有效引导生成文本朝向目标价值观。

Conclusion: 本文的方法为评估和实现语言模型的价值观对齐提供了一种有效的框架，证明了在不修改模型参数的情况下通过提示工程实现价值观引导的可行性。
...
TL;DR: 本文提出了一种基于概念梯度的可解释性技术，用于毒性检测中的概念归因分析，通过构建目标词典集和计算词-概念对齐分数来识别导致分类错误的过度归因问题。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中毒性内容传播问题日益严重，虽然文本毒性检测取得进展，但基于概念的解释在毒性检测中仍有限。现有方法往往存在概念对目标类的不均衡归因导致分类错误。

Method: 利用概念梯度方法提供更因果的解释，测量概念变化如何直接影响模型输出；构建目标词典集捕捉导致误分类的有毒词汇；计算词-概念对齐分数量化过度归因程度；提出无词典增强策略生成排除预定义有毒词典的毒性样本。

Result: 开发了概念梯度解释技术，能够识别毒性检测模型中的过度归因问题，通过词-概念对齐分数量化词汇对误分类的贡献程度。

Conclusion: 该方法为毒性检测提供了更深入的可解释性分析，揭示了模型在缺乏明确词汇重叠时仍可能存在的过度归因模式，有助于理解模型对更广泛毒性语言模式的归因机制。
...
TL;DR: 本文研究了AI检测模型对经过AI轻微润色的人类撰写阿拉伯语文章的误判问题，构建了两个数据集评估14个LLM和商业AI检测器的性能。
...
Method: 生成两个阿拉伯语数据集：第一个包含800篇文章（半AI生成半人类撰写）用于评估14个LLM和商业AI检测器；第二个Ar-APT数据集包含400篇人类撰写文章，由10个LLM使用4种润色设置处理，共16400个样本。

Result: 所有AI检测器都错误地将大量文章归因于AI。表现最佳的LLM Claude-4 Sonnet准确率为83.51%，但对LLaMA-3轻微润色的文章准确率降至57.63%；最佳商业模型originality.AI准确率92%，但对Mistral或Gemma-3轻微润色的文章准确率降至12%。

Conclusion: AI检测模型对经过AI轻微润色的人类撰写文章存在显著误判问题，性能大幅下降，需要改进检测算法以应对这种边界情况。
...
TL;DR: 本文复现了基于最近邻的测试时训练方法，通过在推理时使用检索到的最近邻序列微调语言模型，显著降低了困惑度和比特率指标，特别是在结构化数据集上效果最明显。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在语言模型中的有效性，特别是对于未在类似数据上预训练的模型，这种方法能让小模型接近大模型的性能。

Method: 使用预训练的RoBERTa嵌入和Faiss索引检索每个测试输入的20个最近邻，对GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型应用每个邻居一次梯度更新。

Result: 测试时训练显著降低了困惑度和比特率指标，在GitHub和EuroParl等结构化数据集上改进最大。未在The Pile上预训练的模型受益更多，小模型能接近大模型性能。

Conclusion: 最近邻测试时训练具有鲁棒性和通用性，即使对于现代推理优化架构也能带来一致增益，同时提出了内存高效的检索实现方法。
...
TL;DR: 本文系统分析了多语言大模型中语言信息在内部表示空间的结构和层间演化，发现语言信息在第一个transformer块就实现高度分离，且在整个模型深度中保持线性可分。语言方向与词汇嵌入的对齐程度与训练数据的语言组成密切相关。


<details>
  <summary>Details</summary>
Motivation: 虽然多语言大模型在多种语言上表现出色，但对其内部表示空间中语言信息的结构方式以及在不同层间的演化过程缺乏系统分析。

Method: 在六个多语言大模型上进行全面探测研究，覆盖所有268个transformer层，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。
...
Conclusion: 多语言大模型不是通过表面字符特征来区分语言，而是通过由训练语料塑造的潜在表示结构。研究结果为多语言表示学习中的数据组合策略和公平性提供了实用见解。
...
TL;DR: 本文提出了一种基于语言模型的SNOMED CT本体嵌入方法，用于解决词汇外查询的分层概念检索问题，在OOV查询场景下优于基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT作为生物医学本体，其知识检索面临语言歧义、同义词、多义词等挑战，特别是当查询是词汇外(OOV)时，即在本体中无等效匹配时问题更为严重。

Method: 采用基于语言模型的本体嵌入方法，构建了针对SNOMED CT的OOV查询标注数据集，测试最直接子类和较少相关祖先的检索能力。

Result: 该方法在SNOMED CT概念检索中优于SBERT和两种词汇匹配基线方法，证明了其在OOV查询场景下的有效性。
...
TL;DR: 本文研究了LLM激活空间中的同理心行动方向，通过对比提示在EIA基准上测试了三个模型的检测和操控能力，发现所有模型都能高精度检测同理心，但操控效果因模型而异。


<details>
  <summary>Details</summary>
Motivation: 探索LLM中同理心行动作为线性方向的存在性，研究安全训练对同理心编码和操控性的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上进行检测和操控实验。

Result: 检测方面：所有模型AUROC达0.996-1.00，同理心编码独立于安全训练；操控方面：Qwen和Phi-3双向操控成功率高且连贯，Dolphin仅支持增强同理心的单向操控。

Conclusion: 检测和操控之间存在模型依赖性差距，安全训练影响操控鲁棒性而非防止操控，需要更多模型验证。
...
TL;DR: 本文介绍了JGU Mainz在BLP-2025代码生成共享任务中的获胜系统，采用多智能体管道方法，通过代码生成和调试两个智能体的协作，实现了95.4%的Pass@1分数。
...
Method: 提出多智能体管道方法：首先由代码生成智能体根据输入指令生成初始解决方案，然后执行单元测试，仅将失败案例转发给调试智能体进行错误分析和解决方案修订。

Result: 在共享任务中获得第一名，Pass@1分数达到95.4%。

Conclusion: 多智能体管道方法在代码生成任务中表现优异，通过迭代调试显著提高了代码质量。
...
TL;DR: 本文重新探讨了非宾格动词的句法分类与语义特征（施事性和终结性）之间的关系，使用可解释维度验证了传统理论观点。


<details>
  <summary>Details</summary>
Motivation: 尽管传统理论认为施事性动作动词更倾向非作格句法，终结性事件动词更倾向非宾格句法，但Kim等人的研究发现人类评分对预测句法行为效果不佳。本文旨在重新验证这一关系。

Method: 使用基于对立极种子词计算的可解释维度，结合人类判断来评估语义属性。

Result: 研究结果支持了非作格/非宾格与施事性/终结性之间的联系，证明可解释维度与人类判断结合能为难以通过评分任务评估的语义属性提供有价值证据。

Conclusion: 可解释维度方法能够有效揭示非宾格动词句法分类与语义特征之间的关联，为语义属性评估提供了新途径。
...
Method: PEPPER通过将输入提示重写为语义上不同但视觉上相似的提示，并添加不显眼的元素，来破坏触发器并稀释触发词的影响。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成冗长、冗余的响应，这降低了清晰度和用户满意度，并增加了模型开发者的成本，特别是那些按输出token收费的专有模型。

Method: 通过计算三种压缩比的平均值来量化非必要内容：1）原始响应与LLM抽象摘要的压缩比；2）原始响应与LLM抽取摘要的压缩比；3）LLM在保持意义的前提下尽可能移除非必要词语的压缩比。

Result: 实验结果表明，所提出的指标能够识别LLM输出中的冗余，为对话AI系统提供了一种无需人工标注的自动化响应简洁性评估工具。

Conclusion: 该方法为评估LLM响应的简洁性提供了一种实用且无需参考标准的自动化解决方案。
...
TL;DR: 提出Soft Concept Mixing (SCM)训练方案，通过在训练中直接暴露软概念表示来弥合推理中的软概念与训练中离散token之间的差距，提升大语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常通过生成离散token进行推理，这限制了其表达能力。虽然软概念推理是很有前景的方向，但模型是在离散token上训练的，存在训练与推理方式的不匹配。

Method: 提出SCM训练方案：构建软概念向量（嵌入的概率加权平均），将其混合到模型隐藏状态中，并使用强化学习优化整个潜在推理过程。

Result: 在五个推理基准测试上的实验表明，SCM提高了大语言模型的推理性能，同时保持了稳定的训练动态。

Conclusion: SCM通过软概念感知训练有效提升了LLMs的推理能力，证明了在训练中引入软概念表示的重要性。
...
TL;DR: 本文提出了一种改进Tiny Recursive Models (TRMs) 训练效率的新方法，通过将潜在推理框架化为分类器自由引导和隐式策略改进算法，显著减少了前向传播次数并消除了停止机制，在保持质量的同时大幅提升了训练效率。


<details>
  <summary>Details</summary>
Motivation: 研究如何以最小改动进一步提高小型循环架构（如TRMs）在复杂推理任务上的效率，这些模型已被证明在ARC等任务上能超越大型语言模型。

Method: 将TRMs的潜在推理框架化为分类器自由引导和隐式策略改进算法，提出为每个训练循环提供目标的新型训练方案。

Result: 方法显著提升了训练效率：总前向传播次数减少18倍，消除了停止机制，在仅0.8M参数下达到ARC-1 24%的准确率，优于大多数LLMs。

Conclusion: 提出的训练方案能够在不牺牲模型质量的前提下，大幅提升TRMs的训练效率，为小型高效推理模型的发展提供了有前景的方向。
...
TL;DR: 本研究探讨了训练数据的统计特性与诱导头形成的关系，发现批次大小和上下文长度的组合能预测诱导头形成点，表面二元组重复频率和可靠性强烈影响诱导头形成，并确定了这些值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 诱导头被认为是现代语言模型具有出色上下文学习能力的基础，但其形成的精确机制尚不清楚。本研究旨在揭示训练数据统计特性与诱导头形成之间的确切关系。

Method: 通过分析自然和合成数据的统计特性，研究批次大小、上下文长度、表面二元组重复频率和可靠性等因素对诱导头形成的影响。

Result: 发现批次大小和上下文长度的简单方程能预测诱导头形成点；表面二元组重复频率和可靠性强烈影响诱导头形成，并找到精确的帕累托边界；当频率和可靠性高时，局部依赖足以形成诱导头，但当这些值低时，类别性和边际分布形状变得重要。

Conclusion: 训练数据的统计特性，特别是表面二元组重复频率和可靠性，是诱导头形成的关键因素，这些发现为理解语言模型上下文学习能力的形成机制提供了重要见解。
...
TL;DR: 提出了一种新颖的基于论证感知的量化摘要任务，用于揭示对话中论点的声明-理由结构，并通过ARQUSUMM框架实现该任务，在论证结构表示和量化准确性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题需要不仅总结多样化论点，还要揭示其理由和论证结构。现有对话摘要方法虽然考虑了句子间的论证关系，但未能深入挖掘句子内部的论证结构。

Method: 提出ARQUSUMM框架：1）利用LLM少样本学习识别句子中的命题及其声明-理由关系；2）采用论证结构感知的聚类算法聚合论点并量化其支持度。

Result: 实验表明ARQUSUMM在论证结构表示、文本质量和量化准确性方面优于现有对话和量化摘要模型，生成的摘要对用户更有帮助。

Conclusion: 该研究成功实现了论证感知的量化摘要任务，能够有效揭示对话中的深层论证结构，为在线对话分析提供了新的解决方案。
...
TL;DR: 本研究提出了一种监督微调方法来增强领域特定信息抽取能力，针对湖南历史名人领域设计了细粒度的模式引导指令模板，并对四个大型语言模型进行参数高效的指令微调，显著提升了在低资源环境下文化传承知识抽取和知识图谱构建的性能。


<details>
  <summary>Details</summary>
Motivation: 解决湖南历史名人系统数据资源有限、通用模型在领域知识抽取和结构化输出生成方面表现不佳的问题，推动历史文化研究中的知识抽取和知识图谱构建。

Method: 设计针对湖南历史名人领域的细粒度模式引导指令模板，构建指令调优数据集，对Qwen2.5-7B、Qwen3-8B、DeepSeek-R1-Distill-Qwen-7B和Llama-3.1-8B-Instruct四个公开大模型进行参数高效的指令微调。
...
TL;DR: 本文构建了一个视觉说服力数据集，提出了视觉说服因素分类法，并探索了认知引导和知识注入策略。研究发现VLMs存在召回偏向，高层次的语义对齐是最强预测因子，而基于对象的简洁理由能显著提升模型性能。
...
Result: VLMs存在召回偏向（过度预测高说服力），对低/中层次特征的判别能力较弱，高层次语义对齐是最强预测因子。简单指令或无引导推理支架效果有限，而基于对象的简洁理由能显著提升精确度和F1分数。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模真实评估中对透明可解释自动评分系统的需求，现有系统缺乏广泛接受的可解释性解决方案。

Method: 开发AnalyticScore框架：1)提取可识别响应元素，2)使用LLM将响应转化为人类可解释特征值，3)应用直观的有序逻辑回归模型进行评分。

Result: 在ASAP-SAS数据集的10个项目上，AnalyticScore的评分准确性优于许多不可解释方法，平均仅比不可解释SOTA方法低0.06 QWK，且特征化行为与人类标注者高度一致。

Conclusion: 提出的FGTI原则和AnalyticScore框架为可解释自动评分提供了可行方案，在保持高准确性的同时实现了透明可解释的评分过程。
...
TL;DR: MUCH是首个用于公平评估声明级不确定性量化方法的基准，包含4,873个样本，覆盖4种欧洲语言和4个开源LLM，提供完整的生成对数概率数据，并提出高效的确定性分割算法。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型缺乏可靠性的问题，为声明级不确定性量化方法提供公平、可复现的评估基准，满足实际部署需求。

Method: 构建多语言基准数据集，提供24个生成对数概率数据，开发确定性分割算法（仅需0.2%的LLM生成时间），支持实时监控。

Result: 创建了包含4,873个样本的基准，覆盖英语、法语、西班牙语和德语，评估显示现有方法在性能和效率方面仍有很大改进空间。

Conclusion: MUCH基准为声明级不确定性量化研究提供了重要基础设施，当前方法仍需在准确性和效率方面进一步优化。
...
TL;DR: 本文探索使用上下文压缩作为预训练任务来改进LLM的文本表示能力，提出的LLM2Comp模型在多种任务上优于现有基于LLM的文本编码器，且具有更高的样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要是因果模型，专注于下一个token预测，不适合生成整体文本表示。虽然已有研究引入预训练任务来适应LLM用于文本表示，但大多依赖token级预测目标，存在局限性。

Method: 提出使用上下文压缩作为无监督预训练任务，模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测。通过精心设计的压缩目标和对比学习来增强LLM的文本表示能力。

Result: 实验表明，精心设计的压缩目标能显著提升基于LLM的文本表示，优于使用token级预训练任务的模型。通过对比学习进一步改进的LLM2Comp模型在广泛任务上优于当代基于LLM的文本编码器，且需要更少的训练数据。

Conclusion: 上下文压缩是一种有效的预训练任务，能够显著提升LLM的文本表示能力，LLM2Comp模型在多个任务上表现出色且具有更高的样本效率。
...
TL;DR: 本文介绍了LangMark——一个包含20.7万个三元组的多语言自动后编辑数据集，涵盖英语到7种语言的翻译，用于提升机器翻译质量并减少人工干预。
...
Method: 构建并发布LangMark数据集，包含源文本、NMT输出和人工后编辑翻译的三元组，由专业语言学家标注。利用该数据集评估LLM在少样本提示下执行APE的效果。

Result: 实验表明，使用少样本提示的大型语言模型能够有效执行APE，改进领先的商业甚至专有机器翻译系统。

Conclusion: LangMark这一新资源将促进APE系统未来的开发和评估。
...
TL;DR: 本文介绍了PLLuM项目中用于微调波兰大语言模型的指令数据集，提出了有机、转换和合成指令的功能分类，并比较了人工编写与合成指令数据集在语言适应中的影响，同时发布了PLLuMIC数据集子集。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目中的波兰大语言模型开发提供指令数据集，研究不同类型指令在语言适应中的作用，并分享数据集开发经验以指导其他类似项目。

Method: 构建了包含有机、转换和合成指令的功能分类体系，通过人工编写和合成方法创建指令数据集，并分析不同类型指令对模型性能的影响。

Result: 开发了PLLuM指令语料库，提出了指令的功能分类框架，发布了PLLuMIC代表性子集，为其他LLM开发提供了数据集构建指导。

Conclusion: 不同类型指令在语言适应中具有不同作用，人工编写与合成指令各有优势，PLLuMIC数据集可为类似项目提供有价值的参考和指导。
...
TL;DR: 本文提出了基于方面的因果弃权(ABCA)框架，通过因果推理分析LLM内部知识多样性来实现早期弃权，防止不可靠回答的产生。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前防止不可靠回答。LLM从不同来源获取的知识具有多面性，这种多样性可用于评估知识可靠性。

Method: ABCA框架通过因果推理分析LLM内部知识多样性，基于方面条件因果效应评估知识可靠性，支持两种弃权类型：方面效应不一致(知识冲突)和方面效应一致支持弃权(知识不足)。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到了最先进的性能，并增强了弃权决策的可解释性。

Conclusion: ABCA框架通过利用LLM内部知识多样性进行因果分析，实现了更有效的早期弃权，为减少LLM幻觉提供了新方法。
...
TL;DR: 本文提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合在一个统一框架中，通过注意力机制动态确定每种特征类型的相对重要性，在新闻分类任务中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法虽然能捕捉词汇重要性但缺乏上下文语义理解，而现代深度学习方法关注语义但可能忽略简单有效的统计指标。需要一种能结合两者优势的方法来提升新闻分类效果。

Method: 提出AGFF模型，使用基于注意力的机制动态融合统计特征（如TF-IDF）和语义特征，让模型能够自适应地权衡两种特征类型的重要性。

Result: 在基准新闻数据集上的评估显示，AGFF模型在分类性能上优于传统统计模型和纯语义深度学习模型，消融研究验证了融合过程中各组件的重要性。

Conclusion: 策略性地整合不同类型特征能显著提升分类准确性，该模型能够平衡并利用统计和语义表示的互补优势，为实际新闻分类任务提供实用有效的解决方案。
...
TL;DR: 提出了一种名为E3的层剪枝框架，通过可微分掩码优化和熵感知知识蒸馏，解决了性能下降、训练成本高和加速有限等部署挑战。
...
Result: 在Qwen3-32B模型上剪枝25%层时，在MATH-500上达到96%准确率，仅比原始模型(96.8%)下降0.8%，优于现有SOTA方法(95%)，推理速度提升1.33倍，仅消耗0.5B tokens(占后训练数据量的0.5%)。

Conclusion: E3框架在多种模型架构和基准测试中表现出优越性，能够有效平衡任务效果、训练经济性和推理效率。
...
TL;DR: PARROT框架用于评估大语言模型在权威和说服压力下的准确性退化现象，通过双盲评估、置信度追踪和行为分类来量化模型的顺从性表现。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在权威压力下过度顺从的现象，评估模型在真实世界部署中对社会压力的抵抗能力。

Method: 使用PARROT框架，包括双盲评估比较中性问题和权威错误版本，基于对数似然的置信度校准追踪，以及八状态行为分类法。
...
TL;DR: MirageTVQA是一个新的多语言表格问答基准，包含近6万个QA对，覆盖24种语言，专门测试视觉语言模型在视觉噪声和多语言环境下的表现。
...
Method: 创建包含视觉噪声的多语言表格数据集，评估领先视觉语言模型在噪声处理和多语言推理方面的表现。

Result: 主要视觉语言模型在视觉噪声下性能下降超过35%，且存在明显的英语优先偏见，多语言推理能力不足。

Conclusion: MirageTVQA为评估和推动更鲁棒的表格推理视觉语言模型提供了基准，揭示了模型在现实场景中的关键缺陷。
...
TL;DR: 本文介绍了爱沙尼亚语版本的WinoGrande常识推理测试集的本地化翻译和评估，比较了人工翻译和机器翻译的效果，发现人工翻译质量更好但模型性能略低于原英文测试集，而机器翻译效果明显更差。


<details>
  <summary>Details</summary>
Motivation: 为评估大型语言模型在爱沙尼亚语上的常识推理能力，需要创建本地化和文化适应的测试数据集，并探索高质量机器翻译的可行性。

Method: 由翻译专家进行人工翻译和本地化适应，同时设计针对爱沙尼亚语特点和WinoGrande数据集翻译挑战的详细提示，评估专有和开源模型在人工翻译和机器翻译测试集上的表现。

Result: 模型在人工翻译的爱沙尼亚语数据集上表现略低于原英文测试集，在机器翻译数据上表现明显更差；提示工程对翻译质量或模型准确性的提升有限。
...
TL;DR: 该研究基于语料库分析普通话单音节词在自然对话中的音高轮廓实现，发现词义对音高有显著影响，即使控制多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索普通话单音节词在自然对话中的音高轮廓实现，特别关注词义对音高轮廓的影响，挑战传统的声调理论。

Method: 使用广义加性模型分解观察到的音高轮廓，将音高轮廓分解为与不同控制变量和语义预测因子相关的组成部分。

Result: 研究发现词义是音高实现的重要预测因素，词义比词本身更能预测音高轮廓，异形同音词具有不同的音高轮廓，且通过上下文嵌入可以准确预测单个词标记的音高轮廓。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示将语言在视觉上下文中进行基础化，以构建更鲁棒的自然语言理解系统，避免文本偏见和表面启发式方法的影响。
...
TL;DR: 本文提出了一种名为Selective RoPE的输入依赖旋转位置嵌入机制，它通过任意角度旋转来增强语言建模能力，适用于线性和softmax变换器，并在语言建模和序列任务中表现出改进性能。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控机制能改善语言相关任务的启发，作者希望将输入依赖的位置编码引入到旋转位置嵌入中，以增强位置信息的表达能力。

Method: 提出了Selective RoPE机制，这是一种输入依赖的旋转嵌入方法，可以执行任意角度的旋转，适用于线性和softmax变换器。该方法揭示了softmax注意力中隐藏的旋转结构，并在状态空间模型和门控线性变换器中分析了实部和虚部的作用。

Result: 通过在门控变换器中装备Selective RoPE，验证了该方法在语言建模以及复制、状态跟踪和检索等困难序列任务上的性能提升。

Conclusion: Selective RoPE通过输入依赖的旋转机制有效增强了位置编码能力，为变换器模型提供了更灵活和强大的位置信息处理方式。
...
### [122] [PUCP-Metrix: A Comprehensive Open-Source Repository of Linguistic Metrics for Spanish](https://arxiv.org/abs/2511.17402)
...
Method: 开发了一个包含182个语言指标的开放源码存储库，支持细粒度和可解释的文本分析。
...
TL;DR: ReVeL框架将多项选择题改写为开放式问题，解决选项泄露信号问题，提升模型训练和评估的可靠性。
...
Method: 提出ReVeL框架，将多项选择题分类并重写为开放式问题，保持答案可验证性，使用GRPO方法微调Qwen2.5-VL模型。

Result: 在多项选择基准测试中准确率相当，开放式问题准确率提升约6个百分点，评估中揭示多项选择题有高达20个百分点的分数膨胀。

Conclusion: ReVeL框架比基于多项选择题的训练具有更好的数据效率和更稳健的奖励信号，同时降低评估成本和延迟。
...
<details>
  <summary>Details</summary>
Motivation: 传统RLVR在数学定理证明等任务中扩展性有限，因为中间推理过程至关重要但最终答案难以直接可靠验证，而token级监督微调容易退化为机械记忆。

Method: 提出两阶段训练流程：首先在数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调，通过掩码填充和步骤重排序构建过程级自监督奖励。
...
<details>
  <summary>Details</summary>
Motivation: 受到复古BASIC简洁性的启发，重新利用编号行和简单命令作为可解释的认知控制层，使现代LLM能够可靠地模拟这类简短程序，实现模型内部透明的多步推理。

Method: 使用自然语言解释器文件指定命令语义、内存更新和日志行为，通过心理模型解释器提取声明性和程序性知识，检测矛盾并在必要时产生解决方案。

Result: 在三个LLM上进行知识提取、冲突检测和推理任务的基准测试比较显示，所有模型都能执行Cognitive BASIC程序，整体表现强劲但不均匀。

Conclusion: Cognitive BASIC为LLM推理提供了一种结构化的、可解释的方法，使模型能够可靠地执行多步推理任务。
...
TL;DR: 提出了一个系统化基准测试修订框架，通过统计分析响应模式来标记潜在无效问题，结合LLM初步评审减少人工工作量，在9个基准测试中达到84%的精确度。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效的基准问题会削弱其可靠性。手动识别和修正数千个基准问题不可行，成为可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设——平均分足以概括模型性能，构建统计分析方法。通过分析响应模式，当项目统计值超出预期范围时标记为潜在问题，并引入LLM评审作为初步筛选。

Result: 在9个广泛使用的基准测试中，该方法指导专家评审识别问题问题的精确度高达84%，显著提高了基准测试的可靠性。

Conclusion: 该框架为系统化基准测试修订提供了高效且可扩展的解决方案，通过统计分析和LLM辅助评审有效减少了人工工作量，提升了基准测试的质量和可靠性。
...
### [128] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 论文提出DAPS++方法，将扩散模型在逆问题求解中重新解释为EM框架中的初始化阶段，通过解耦扩散阶段和数据驱动优化来提高计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下的基于分数的扩散方法在实践中存在局限性：先验提供的指导有限，重建主要由测量一致性项驱动，导致推理过程与扩散动力学解耦。需要澄清这种结构并改进方法。

Method: 重新解释扩散在逆问题求解中的作用，将其作为EM框架中的初始化阶段。提出DAPS++方法，使似然项更直接地指导推理，同时保持数值稳定性。

Result: DAPS++在保持统一扩散轨迹有效性的同时，显著减少了函数评估次数和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: DAPS++通过解耦扩散阶段和数据驱动优化，为扩散模型在逆问题求解中的实际有效性提供了新的理论解释，并实现了更高效的性能。
...
### [129] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的情节奖励面临两个主要挑战：(1) 随着探索空间扩大，导致奖励的联合动作轨迹呈指数级稀疏；(2) 现有方法往往忽略能够影响团队状态的联合动作。

Method: 提出了相互内在奖励(MIR)方法，激励个体智能体探索能够影响队友的行为，并与原始策略结合使用。同时创建了MiniGrid-MA环境用于多智能体强化学习的实验验证。

Result: 在MiniGrid-MA环境中与最先进方法进行比较，实验结果表明所提方法具有优越性能。

Conclusion: MIR是一种简单而有效的增强策略，能够有效解决多智能体强化学习中极端稀疏奖励问题，显著提升团队探索能力和算法性能。
...
### [130] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了一个基于分层任务抽象机制（HTAM）的新型多智能体框架EarthAgent，专门用于解决复杂地理空间分析任务，相比通用智能体框架在专业领域表现更优。
...
Result: 构建了EarthAgent系统并在GeoPlan-bench基准测试中评估，实验表明EarthAgent显著优于一系列现有的单智能体和多智能体系统。
...
### [131] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为实现智能AI系统的基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具备能力、灵活性、透明度、合作性和问责性的智能系统。
...
Method: 将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具与自适应数据驱动方法相结合。

Result: 提出了一个连接形式理论和实践自主性的智能系统视角，为构建具备透明度、合作性和问责性的智能系统指明了路径。

Conclusion: 通过整合AAMAS社区的概念工具与数据驱动方法，可以实现既具备能力和灵活性，又具有透明度、合作性和问责性的智能AI系统，从而在形式理论和实践自主性之间建立桥梁。
...
### [132] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了在LLM监控中使用合成数据和离策略数据对探针泛化性能的影响，发现响应生成策略显著影响探针性能，且训练数据域的偏移会导致更大的性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多行为的自然示例稀少，研究人员不得不依赖合成或离策略的LLM响应来训练探针，但这种方法对探针泛化能力的影响尚不明确。

Method: 在八个不同的LLM行为上测试线性和注意力探针，评估不同响应生成策略对探针性能的影响，并分析从离策略数据到在策略数据的泛化能力。

Result: 发现从离策略数据成功泛化到在策略测试集的能力可以预测在策略泛化的成功；欺骗和沙袋行为探针可能在真实监控场景中无法从离策略泛化到在策略数据；不同域测试分数始终低于同域测试分数。

Conclusion: 在缺乏在策略数据的情况下，使用同域离策略数据比使用不同域的在策略数据产生更可靠的探针，强调了需要更好处理LLM监控中分布偏移的方法。
...
### [133] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
*Jiaxi Liu,Chengyuan Ma,Hang Zhou,Weizhe Tang,Shixiao Liang,Haoyang Ding,Xiaopeng Li,Bin Ran*

Main category: cs.AI

TL;DR: 提出SRA-CP框架，通过风险感知的选择性协作感知，在保持安全关键物体检测精度的同时大幅减少通信带宽使用。
...
Result: 相比通用协作感知方法，安全关键物体平均精度损失小于1%，通信带宽使用仅需20%。相比现有选择性方法，感知性能提升15%。

Conclusion: SRA-CP框架能有效解决协作感知中的通信带宽和动态环境适应性问题，在保持安全性的同时显著提高通信效率。
...
### [134] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
*Thorsten Fröhlich,Tim Schlippe*

Main category: cs.AI

TL;DR: RubiSCoT是一个AI支持的论文评估框架，使用自然语言处理技术（包括大语言模型、检索增强生成和结构化思维链提示）来提升从开题到最终提交的论文评估过程。
