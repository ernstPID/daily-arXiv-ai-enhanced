...
Method: 部署自主智能体动态调优训练参数，使用非平稳多臂老虎机算法平衡探索与利用，采用双速率指数移动平均平滑和混合精度训练确保鲁棒性和效率。

Result: 在Pascal VOC上达到mAP 97.4、OF1 92.3、CF1 91.4；在COCO上达到mAP 92.8、OF1 88.2、CF1 87.1；在VG-256上达到mAP 60.9、OF1 70.8、CF1 61.1，均优于基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在指令调整时面临大规模数据集训练的高计算成本问题，现有数据选择方法存在计算开销大和重要性-多样性分离处理的缺点。

Method: 提出CoIDO双目标框架，使用轻量级插件评分器在少量随机样本上学习候选集分布，通过同方差不确定性公式平衡重要性和多样性，实现高效可扩展的数据选择。

Result: 仅使用20%随机样本训练CoIDO评分器，然后选择20%数据子集进行指令调整，在LLaVA-1.5-7B模型的10个下游任务上平均达到全数据微调98.2%的性能。

Conclusion: CoIDO框架能显著降低多模态大语言模型指令调整的计算成本，同时保持接近全数据微调的性能，为高效数据选择提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤患者对标准治疗（Stupp方案）的反应高度异质性，传统方法需要至少两个月才能通过MRI观察到治疗效果。早期预测治疗反应对于推进个性化医疗至关重要。

Method: 提出一种潜在扩散模型，通过治疗前MRI和肿瘤定位的串联条件化，以及使用生存信息的无分类器引导来增强生成质量，实现切片到切片的翻译以生成治疗后MRI。

Result: 模型在包含140名胶质母细胞瘤患者的本地数据集上进行了训练和测试，数据集包括治疗前后T1-Gd MRI、专家手动勾画的肿瘤定位和生存信息。

Conclusion: 该方法能够早期预测胶质母细胞瘤患者的治疗反应，通过生成治疗后MRI来反映肿瘤演变，为个性化治疗决策提供支持。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI和大型语言模型的快速发展使得生成的数字内容高度逼真且上下文相关，这对数字内容认证提出了挑战，需要验证数字数据的完整性和来源。

Method: 开发了一个系统，通过五个基准嵌入模型处理多样化的AI和人类生成图像数据集，基于假设AI生成图像在嵌入空间中更接近其他AI生成内容，而人类创建图像在其领域内聚类。
...
<details>
  <summary>Details</summary>
Motivation: 现有的流匹配模型需要专门的步长嵌入来实现轨迹跳过，但这与现有模型不兼容，需要从头重新训练，成本几乎与预训练相当。

Method: 利用速度场自蒸馏原理，在速度场而非样本空间工作，通过在线自引导蒸馏快速学习，无需步长嵌入即可实现更激进的捷径机制。

Result: 该方法训练效率极高，例如在不到一个A100天的时间内生成3步Flux模型，并能整合到预训练阶段本身，产生无需质量妥协的高效少步流模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统的人类活动识别方法在水下环境中效果不佳，特别是潜水员佩戴的传感器与自主水下航行器通信时存在无线信号衰减问题。需要一种新方法来监测潜水员安全，特别是识别医疗紧急情况。

Method: 利用计算机视觉从3D人体关节关键点流生成高保真运动数据，创建"伪IMU"。将分类器集成到自主水下航行器上，通过模拟遇险场景进行实验验证。

Result: 该方法成功绕过了水下无线信号衰减问题，能够有效识别异常潜水员行为，表明医疗紧急情况的发生。

Conclusion: 该方法在推进机器人监测和潜水员安全方面具有实用性和有效性，为水下环境中的潜水员安全监测提供了一种创新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决RGB和X射线成像在数据表示差异和配对数据集有限方面的挑战，扩展3D高斯泼溅技术的应用范围，提升可视化、模拟和无损检测能力。

Method: 收集新的配对RGB和X射线数据，执行分层拟合来对齐RGB和X射线辐射高斯泼溅，并提出X射线参考损失以确保一致的内部结构。

Result: InsideOut有效解决了两种模态间数据表示差异和有限配对数据集带来的挑战，显著扩展了3D高斯泼溅技术的适用性。

Conclusion: 该方法成功融合了RGB表面细节和X射线内部结构，为多个领域的可视化、模拟和无损检测提供了增强能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决零样本2D目标检测和分割问题，特别是在未见过的3D物体上的识别，需要一种无需额外训练就能泛化的强大框架。

Method: 利用3D未见物体的2D多视图模板和查询图像的2D目标提议，结合类别和补丁嵌入（使用GeM归一化），采用联合相似度度量（绝对和相对相似度），并通过不确定性感知目标先验优化相似度得分。

Result: 在BOP Challenge 2025中，MUSE在Classic Core、H3和Industrial三个赛道均排名第一，实现了最先进的性能。

Conclusion: MUSE提供了一个强大且可泛化的零样本2D目标检测和分割框架，无需额外训练或微调即可达到最优效果。
...
<details>
  <summary>Details</summary>
Motivation: 性别分类系统经常继承并放大训练数据中的人口统计不平衡问题，现有数据集存在显著的交叉性代表性不足，导致模型产生偏见。

Method: 首先审计五个广泛使用的性别分类数据集，然后在两个最平衡的数据集上训练分类器，最后构建BalancedFace新数据集，通过混合FairFace和UTKFace的图像并补充其他集合的图像来填补人口统计空白。

Result: 在BalancedFace上训练的标准分类器将种族亚组间的最大真阳性率差距减少了50%以上，将平均差异影响分数向理想值1.0拉近了63%，同时整体准确性损失最小。
...
Method: 设计了类感知标签精炼模块和几何感知标签精炼组件，结合自训练策略进行标签更新，通过迭代过程持续提升伪标签质量并扩展标签覆盖范围。

Result: 在ScanNet和S3DIS基准测试中实现了最先进的性能，并在无监督设置下表现出卓越的泛化能力。

Conclusion: 该方法通过整合3D几何先验和类感知机制，有效提升了3D弱监督语义分割的性能，证明了其鲁棒设计和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中的深度学习分割算法存在内在数据偏见问题，特别是在基于种族和性别等敏感属性的性能差异方面，需要评估分割模型的公平性。

Method: 使用四种方法（UNesT、nnU-Net、CoTr和ANTs）在包含四个人口统计学亚组的数据集上分割伏隔核，采用手动标记的金标准分割进行训练和测试，通过公平性度量指标和线性混合模型分析人口统计学变量对分割准确性和体积的影响。

Result: 在种族匹配训练和测试时，ANTs和UNesT的分割准确性显著提高，而nnU-Net表现出与人口统计学匹配无关的稳健性能。使用有偏模型观察到的性别效应与手动分割一致，但种族效应在除一个模型外的所有模型中消失。

Conclusion: 分割模型的性能存在人口统计学偏见，种族匹配训练能改善某些模型的准确性，而nnU-Net表现出更好的公平性。有偏模型能捕捉性别效应但可能掩盖种族效应，需要开发更公平的分割算法。
...
Result: 识别出三种观众类型（72.8%稳定欣赏者、13.2%低变异性下降者、14.0%变异性改善者），发现积极观看顺序效应，但观众类型间无显著差异。

Conclusion: 该数据集支持文化感知情感AI开发，挑战了疲劳假说，为个性化娱乐系统提供非西方背景的研究基础。
...
<details>
  <summary>Details</summary>
Motivation: 在线学习环境中学生参与度检测对于改善学习成果和个性化教学至关重要，需要开发高效的视频分析方法。

Method: 使用双流架构处理面部裁剪和完整视频帧，通过EfficientNetV2提取空间特征，采用LSTM和Transformer进行时序建模，并应用针对性数据增强技术。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统对罕见、分布外场景中的语义异常存在严重脆弱性，现有视觉语言模型方法性能不可靠且依赖昂贵的专有模型，限制了实际部署。

Method: 采用结构化推理框架，包含两阶段流程：结构化场景描述提取和多模态评估，通过四个语义层（街道、基础设施、可移动物体、环境）进行系统分析。

Result: 在真实世界驾驶场景中达到89.6%召回率和88.0%准确率，显著优于非结构化基线。微调后的7B参数开源模型达到90.8%召回率和93.8%准确率，超越所有评估模型。

Conclusion: SAVANT解决了异常检测中的数据稀缺问题，为自动驾驶系统提供了可靠、可访问的语义监控实用路径，支持低成本本地部署。
...
<details>
  <summary>Details</summary>
Motivation: 红棕榈螨侵染已成为严重问题，特别是在棕榈种植广泛地区，导致生产力下降和经济损失。准确及早识别受螨虫侵染的植物对于有效管理至关重要。

Method: 应用TriggerNet可解释AI框架，集成Grad-CAM、RISE、FullGrad和TCAV生成可视化解释；使用CNN、EfficientNet、MobileNet、ViT、ResNet50、InceptionV3等深度学习模型，以及Random Forest、SVM、KNN等机器学习分类器；利用Snorkel通过启发式规则和模式高效标注疾病类别。

Result: 研究在11种植物物种（槟榔、枣椰树、天堂鸟、椰子树、姜、柑橘树、油棕、兰花、香蕉树、鳄梨树、铁树）上进行训练和评估，将植物分为健康、黄斑、红褐色青铜化和丝网四个疾病类别。

Conclusion: 该研究为红棕榈螨侵染的早期检测提供了一种有效的可解释AI解决方案，通过自动化标注和多种模型比较，提高了植物疾病分类的准确性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在几何推理方面存在困难，无法有效处理3D空间信息。需要开发能够同时处理相机轨迹生成和自然语言描述的方法，以支持房地产和旅游领域的自动化视频创作。

Method: 通过扩散过程生成受已知相机位姿约束的平滑视频轨迹，将3D信息整合到视觉语言模型中用于生成基于3D的描述，使用3D高斯泼溅技术沿轨迹渲染新视角。

Result: 实验表明，将3D相机轨迹整合到文本生成过程中比独立处理每个任务的方法性能更好。提出了新的联合评估指标，并在包含1200多个房屋导览视频的数据集上进行了验证。

Conclusion: 该方法能够实现无需专业知识和设备的自动化专业质量视频创作，适用于房地产和旅游应用，通过3D信息的整合显著提升了描述生成的质量。
...
Method: 构建基于464个独特(部分,主体)对的语义原子数据集，生成37k提示并使用高保真文本到图像模型合成图像；训练具有部分条件引导的自定义扩散先验模型，引导图像条件特征以强制语义身份和空间布局。

Result: 人类评估和提出的PartEval指标显示，Chimera在部分对齐和组合准确性方面优于其他基线14%，在视觉质量方面优于21%。

Conclusion: Chimera能够有效生成由多个源图像部分组合的新对象，在组合准确性和视觉质量方面显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 微塑料作为普遍污染物对生态系统和人类健康构成威胁，但其微小尺寸使得检测、分类和去除变得困难。传统检测方法需要人工分析，无法高效用于大规模筛选研究。

Method: 采用扫描电子显微镜（SEM）成像与基于机器学习的物体检测相结合的方法，特别关注具有对称重复背景图案的过滤场景，使用YOLO模型进行检测。

Result: 研究发现不同YOLO模型在检测任务中的质量存在差异，优化预处理对检测效果有重要影响。同时识别出专家标注数据有限等挑战。

Conclusion: 机器学习在微塑料检测方面具有潜力，但仍面临训练数据不足等挑战，需要进一步优化预处理方法和解决数据标注问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统ViT将输入图像划分为统一大小的补丁，无论内容如何，导致高分辨率图像输入序列过长。APT旨在通过自适应补丁大小来解决这个问题。

Method: 使用多种不同大小的补丁在同一图像中，在更均匀的区域分配更大的补丁大小，在更复杂的区域分配更小的补丁，以减少输入token总数。

Result: 在ViT-L上实现40%的吞吐量提升，在ViT-H上实现50%的吞吐量提升，同时保持下游性能；在视觉QA、目标检测和语义分割中实现高达30%的训练和推理加速。

Conclusion: APT能够显著加速ViT的推理和训练，同时保持性能，并且可以应用于预训练的ViT模型，仅需少量训练即可收敛。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS技术正在从根本上改变基于姿态图像的3D重建领域，它通过显式建模3D高斯集合实现了高效的体素化渲染，但与图形管线的无缝集成也带来了高内存占用、光照效果固化等挑战。

Method: 教程从3DGS的泼溅公式化开始，系统性地探索了解决其局限性的主要方法，包括内存优化、光照分离和二次光线效果支持等方面的技术改进。

Result: 3DGS展现出高效的实时渲染能力，特别适用于前馈管道，在表面重建、虚拟化身建模、动画和内容生成等应用中表现出色。

Conclusion: 3D高斯泼溅技术为3D重建领域带来了革命性变革，虽然存在内存和光照方面的限制，但其高效的渲染能力和广泛的应用前景使其成为该领域的重要发展方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在低资源、预算受限环境下的部署问题，避免小模型需要昂贵微调才能达到大模型性能的困境。

Method: 基于上下文学习框架，采用跨模态演示选择策略、教师测试时缩放减少噪声、学生不确定性条件化等技术，动态构建演示池并最小化教师查询。

Result: 小模型性能显著提升（最高33%），仅需少量教师标注（低至4%），并能与教师模型的零样本性能竞争。

Conclusion: 提出的在线上下文蒸馏方法为低资源环境下的视觉语言模型部署提供了高效可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统V2X系统面临高带宽需求、语义丢失和互操作性挑战，自然语言作为通信媒介具有语义丰富、决策级推理和低带宽优势，但引入了新的安全漏洞。

Method: 开发了攻击策略分类（连接中断、中继/重放干扰、内容欺骗、多连接伪造），提出了SafeCoop防御框架，包含语义防火墙、语言-感知一致性检查和多源共识机制。

Result: 在CARLA仿真中32个关键场景下评估，恶意攻击下驾驶分数提升69.15%，恶意检测F1分数最高达67.32%。
...
TL;DR: World-in-World平台首次在闭环环境中评估生成世界模型在具身智能体决策中的效用，发现视觉质量不等于任务成功，可控性更重要，且推理时计算分配能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要采用开环协议强调视觉质量，未能解决世界模型是否真正帮助具身智能体在任务中成功的核心问题。

Method: 开发World-in-World开放平台，提供统一在线规划策略和标准化动作API，在四个闭环环境中评估多样化世界模型，以任务成功为主要指标。

Result: 发现三个意外结果：视觉质量不保证任务成功，可控性更重要；动作-观察数据后训练比升级预训练视频生成器更有效；增加推理时计算能显著提升闭环性能。

Conclusion: 世界模型在具身决策中的效用评估需要闭环环境，视觉质量不是决定性因素，可控性和推理时优化更为关键。
...
Method: 使用基于高分辨率地形和反射率模型的射线追踪模拟生成LunarStereo数据集，涵盖月球南极区域的不同高度、光照条件和视角。基于该数据集对MASt3R模型进行领域自适应微调。
...
<details>
  <summary>Details</summary>
Motivation: 拥挤场景中的异常检测面临严重的人际遮挡和高度动态、上下文相关的运动模式的挑战。现有方法难以适应变化的拥挤密度，且缺乏可解释的异常指标。
...
Conclusion: VelocityNet框架成功解决了拥挤场景中异常检测的挑战，提供了可解释的异常指标，并在密集拥挤环境中实现了有效的实时检测。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数医学视觉语言模型无法同时生成诊断文本和像素级分割掩码，这限制了临床应用的潜力。辅助系统如果无法同时提供这两种模态，对医疗从业者的价值有限。

Method: 首先引入RadDiagSeg-D数据集，将异常检测、诊断和多目标分割整合为统一的分层任务；然后基于该数据集提出RadDiagSeg-M模型，能够进行联合异常检测、诊断和灵活分割。

Result: RadDiagSeg-M在异常检测、诊断和多目标文本-掩码生成任务的所有组件上都表现出色，建立了稳健且有竞争力的基准。

Conclusion: 该方法有效丰富了辅助诊断的上下文信息，提供了高度信息化和临床有用的输出，解决了医学视觉语言模型在联合生成文本和分割掩码方面的关键限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全评估方法将视觉和语言输入分开处理，忽略了联合解释的风险，且无法区分明确不安全内容和边界情况，导致过度阻止或拒绝不足的问题。

Method: 使用多阶段流水线结合真实图像和人工标注，构建包含8,187个样本的大规模基准，涵盖15种危害类别，评估11个最先进模型。

Result: 模型在清晰单模态安全信号上准确率达90%以上，但在需要联合图像-文本推理时性能大幅下降至20-55%。34%的联合分类错误发生在单独模态分类正确的情况下，表明缺乏组合推理能力。

Conclusion: 该框架揭示了当前模型在联合图像-文本理解和对齐方面的弱点，为稳健视觉语言安全研究的下一步里程碑提供了关键测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有去偏方法受限于样本表示多样性，朴素生成增强往往保留原有偏见，且单纯为罕见类生成更多数据存在两个核心问题：实例频率不能完全反映模型真实数据需求，当前布局到图像合成缺乏保真度和控制能力。

Method: 引入表示分数诊断频率之外的表示差距，指导创建无偏布局；用精确视觉蓝图替代模糊文本提示，采用生成对齐策略促进检测器和生成器之间的通信。

Result: 显著缩小了代表性不足物体组的性能差距，大/罕见实例比基线提升4.4/3.6 mAP，在生成图像的布局准确性上比先前L2I合成模型提升15.9 mAP。

Conclusion: 该生成去偏框架通过表示分数和生成对齐策略，有效解决了物体检测中的表示偏见问题，显著提升了罕见物体的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究长上下文压缩的可行性，探索历史长上下文压缩和LLM内存遗忘机制等研究领域，同时提供高实用价值的大规模文档处理能力。

Method: 使用DeepEncoder作为核心引擎，在高分辨率输入下保持低激活度，同时实现高压缩比，确保视觉令牌数量可控；结合DeepSeek3B-MoE-A570M解码器进行OCR识别。

Result: 当文本令牌数量在视觉令牌10倍以内时（压缩比<10x），OCR精度达97%；压缩比20x时精度仍保持约60%。在OmniDocBench上超越GOT-OCR2.0和MinerU2.0，单A100-40G每天可处理20万+页面。

Conclusion: DeepSeek-OCR在长上下文压缩方面展现出巨大潜力，不仅学术价值显著，而且具有很高的实用价值，能够高效处理大规模文档数据。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D物体分类中合成数据训练与真实稀疏、噪声LiDAR扫描之间的领域差距问题，当前方法要么无法泛化到室外场景，要么缺乏语义多样性来识别罕见或未见过的物体。

Method: 提出一个从真实驾驶数据中挖掘对象级三元组（点云、图像、文本描述）的流水线，并采用基于课程的数据混合策略，先在语义丰富的合成CAD数据上训练模型，然后逐步适应真实世界扫描的特定特征。

Result: 仅需在每批次训练中引入1.5%的真实世界样本，就能在nuScenes基准测试上提升零样本准确率27%。最终模型在nuScenes和TruckScenes等挑战性室外数据集上达到最先进性能，比之前最佳方法提升19.3%。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语义高斯溅射方法在利用大规模2D视觉模型将2D语义特征投影到3D场景时存在两个主要问题：预处理时单个掩码的上下文线索不足，以及融合多视图特征时的不一致性和细节缺失。

Method: 该方法包含两个模块：上下文感知特征提取（为每个掩码增强丰富的语义上下文）和注意力驱动特征聚合（选择性融合多视图特征以减少对齐错误和不完整性）。

Result: 在基准数据集上的广泛实验表明，OpenInsGaussian在开放词汇3D高斯分割方面达到了最先进的性能，大幅优于现有基线方法。

Conclusion: 这些发现证明了所提方法的鲁棒性和通用性，标志着在3D场景理解及其在实际部署方面迈出了重要一步。
...
TL;DR: 提出一种基于双曲空间学习的视频3D人体网格恢复方法，利用时序运动先验和双曲空间优化策略来准确捕捉人体层次结构。
...
Method: 设计时序运动先验提取模块，分别从3D姿态序列和图像特征序列提取时序运动特征；提出双曲空间优化学习策略，利用时序运动先验在双曲空间中分别优化3D姿态和姿态运动信息；提出双曲网格优化损失确保学习过程稳定有效。

Result: 在大型公开数据集上的广泛实验结果表明，该方法相比现有最先进方法具有优越性。

Conclusion: 双曲空间学习结合时序运动先验能够有效捕捉人体层次结构，实现准确平滑的3D人体网格恢复。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在自然场景理解方面取得了显著成功，但在水下环境中的应用仍未被充分探索。水下图像存在严重的光衰减、颜色失真和悬浮粒子散射等独特挑战，需要专门的海洋生态系统和生物分类学知识。

Method: 构建了UWBench基准，包含15,003张高分辨率水下图像，涵盖海洋、珊瑚礁和深海栖息地等多种水生环境。每张图像都配备了人工验证的标注，包括15,281个对象指代表达和124,983个问答对。基于此建立了三个综合基准：详细图像描述、视觉定位和视觉问答。
...
Method: 1. 基于树形拓扑的分层参数聚合方法；2. 基于参数差异的风格混合方法(FedStyle)；3. 渐进式个性化融合策略；4. 推理阶段使用特征相似度指导模型链检索进行集成决策。
...
<details>
  <summary>Details</summary>
Motivation: 流式视频视觉语言模型面临两个基本约束：因果性（无法访问未来帧）和积累性（令牌无限增长导致效率瓶颈）。现有方法仅调节后LLM的kv缓存，而昂贵的预LLM预填充成本未被解决。

Method: 提出两阶段框架：1）因果时间缩减：基于相邻帧变化和令牌显著性选择令牌，对每帧施加固定预算，仅处理紧凑的视觉令牌子集；2）在线量化内存：以4位格式存储令牌，按需检索相关组并反量化，保持活动kv缓存有界。

Result: 实验显示：15.7倍kv缓存压缩，峰值内存降低1.2倍，TTFT速度提升2倍。在离线基准测试中平均准确率达63.8%，在RVS上达55.8%/3.7，在无需训练方法中保持最先进精度。
...
Method: 将深度估计重新定义为逆问题，利用基于RGB图像条件化的预训练潜在扩散模型，结合立体几何约束来学习尺度和偏移，实现准确的深度恢复。

Result: 广泛的实验表明，该方法在室内、室外和复杂环境中都能匹配或超越最先进的方法，特别是在涉及半透明和镜面表面的挑战性场景中表现优异。

Conclusion: 该训练免费的解决方案可以无缝集成到现有的基于扩散的单目深度估计框架中，无需重新训练即可在各种环境中实现准确的度量深度估计。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学MLLM仅依赖内部知识进行推理，当遇到超出训练范围的病例时会产生幻觉推理和事实错误。虽然最近出现了Agentic RAG方法，但它们局限于单模态LLM，忽略了推理和检索过程中的关键视觉信息。

Method: 设计了两阶段强化学习策略，通过定制奖励刺激模型利用视觉诊断发现和文本临床信息进行有效检索。进一步提出了置信度驱动的图像重新检索方法，在检测到低预测置信度时进行测试时扩展。

Result: 在各种公共医学基准测试中，Med-RwR相比基线模型有显著改进，证明了通过外部知识集成增强推理能力的有效性。在不熟悉领域表现出卓越的泛化能力，在提出的ECBench上性能提升8.8%。

Conclusion: Med-RwR通过主动检索外部知识有效增强了医学多模态大语言模型的推理能力，特别是在处理超出训练范围的病例时表现出更好的可靠性和泛化性。
...
<details>
  <summary>Details</summary>
Motivation: 成像技术对生物医学研究和现代医学至关重要，但大多数多模态大语言模型是为通用数据集的低分辨率图像设计的，可能导致关键信息丢失，因此需要研究图像分辨率对模型性能的影响。

Method: 通过研究图像分辨率对MLLM性能的影响，比较原生分辨率训练和推理、训练与推理分辨率不匹配的情况，以及混合分辨率训练的效果。

Result: 原生分辨率训练和推理显著提高多个任务的性能；训练与推理分辨率不匹配会严重降低性能；混合分辨率训练能有效缓解不匹配问题，并在计算约束和性能需求之间取得平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶世界模型在状态模态、视频序列长度、动作控制精度和奖励意识方面存在限制，需要开发一个更全面的统一框架。

Method: 采用全景视频联合生成（RGB、语义、深度、3D占用），使用归一化全景Plucker射线图编码输入轨迹，并利用生成的3D占用直接定义基于规则的密集奖励。

Result: 在视频生成、控制精度和长时程稳定性方面达到最先进性能，通过占用基础奖励提供可靠的闭环评估框架。

Conclusion: OmniNWM成功解决了自动驾驶世界模型在状态、动作和奖励三个核心维度的限制，提供了一个统一且高性能的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法将深度视觉特征构成的视觉空间作为嵌入空间，但视觉空间中实例的离散分布使得数据结构不明显。作者认为优化视觉空间对于更有效地将语义向量嵌入到视觉空间至关重要。

Method: 提出了两种策略：1）视觉原型方法，为每个视觉类别学习一个视觉原型，用原型特征代替离散的视觉特征；2）在中间嵌入空间中优化视觉特征结构，设计了一个基于多层感知机框架的算法来学习共同的中间嵌入空间并使视觉数据结构更具区分性。

Result: 在四个基准数据集上的广泛实验评估表明，优化视觉空间对零样本学习有益，提出的基于原型的方法实现了新的最先进性能。

Conclusion: 优化视觉空间对零样本学习至关重要，视觉原型方法能够有效改进零样本学习的性能，达到了新的最先进水平。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现出色，但容易产生对象幻觉——描述不存在或识别错误的物体。现有方法在可扩展性、适应性和模型独立性方面仍存在挑战。
...
Conclusion: 自适应集成方法展示了改善大型视觉语言模型在高风险应用中鲁棒性的有前景方向。
...
Method: 1) 时间维度：引入动态自适应焦点采样机制，逐步聚焦与问题最相关的音视频片段；2) 模态维度：提出偏好感知策略，独立建模各模态贡献；3) 引入双路径对比损失来增强跨维度的连贯性和互补性。
...
TL;DR: 本文提出了Diffusion-DRO方法，这是一种基于逆强化学习的偏好学习框架，通过将偏好学习转化为排序问题来改进文本到图像扩散模型的对齐效果，避免了奖励模型的依赖，并整合了离线专家演示和在线策略生成的负样本。


<details>
  <summary>Details</summary>
Motivation: 现有的直接偏好优化方法虽然通过配对比较训练提升了训练稳定性，但仍面临sigmoid函数的非线性特性导致的图像概率估计不准确，以及离线数据集多样性有限的问题。

Method: Diffusion-DRO框架将偏好学习转化为排序问题，消除了对奖励模型的依赖，将训练目标简化为去噪公式。该方法独特地整合了离线专家演示和在线策略生成的负样本。

Result: 综合实验表明，Diffusion-DRO在一系列具有挑战性和未见过的提示上提供了改进的生成质量，在定量指标和用户研究中均优于最先进的基线方法。

Conclusion: Diffusion-DRO通过基于逆强化学习的偏好学习框架，有效解决了现有方法在图像概率估计和数据集多样性方面的限制，显著提升了文本到图像扩散模型的对齐性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人类-物体交互检测方法主要关注成对关系，忽略了现实场景中交互往往来自集体行为（多个人和物体参与联合活动）。

Method: 提出GroupHOI框架，使用基于边界框空间特征的可学习邻近性估计器将人和物体分组，在每个组内通过自注意力计算软对应关系来聚合和分发上下文线索，并在基于transformer的交互解码器中增强语义相似性。

Result: 在HICO-DET和V-COCO基准测试上表现出优于现有方法的性能，在更具挑战性的非语言交互检测任务上也表现出领先性能。

Conclusion: GroupHOI通过从群体视角重新审视关系建模，有效处理了集体行为中的交互检测问题，在多个基准测试中取得了最先进的性能。
...
TL;DR: FeatureFool是一种针对视频分类器和Video-LLMs的零查询黑盒攻击方法，通过利用DNN提取的特征信息直接改变干净视频的特征空间，无需多次查询交互。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒对抗攻击需要多轮模型交互和大量查询，在真实场景中不实用且难以扩展到Video-LLMs。视频领域缺乏直接利用特征图来改变干净视频特征空间的攻击方法。

Method: 提出FeatureFool方法，利用从DNN提取的信息直接改变干净视频的特征空间，实现零查询攻击。该方法通过特征图的迁移性来绕过Video-LLM识别。
...
<details>
  <summary>Details</summary>
Motivation: 现有ICA方法主要依赖单一视觉模态的手工或浅层CNN特征，无法充分捕捉与图像复杂度相关的感知表示。跨模态场景语义信息在其他计算机视觉任务中表现出色，但在ICA领域尚未探索。
...
TL;DR: 本文提出了一种新的对抗性剪枝方法S2AP，通过在分数空间中最小化锐度来稳定掩码选择，从而提高对抗性剪枝方法的鲁棒性。
...
Method: 提出S2AP方法，引入分数空间锐度最小化概念，在掩码搜索过程中通过扰动重要性分数并最小化相应的鲁棒损失来优化。

Result: 在多个数据集、模型和稀疏度水平上的广泛实验表明，S2AP能有效最小化分数空间中的锐度，稳定掩码选择，并最终提高对抗性剪枝方法的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病与显著的皮层萎缩相关，几何表面模型对于分析解剖结构的3D形状至关重要，需要开发有效的形状分析诊断工具。

Method: 使用ADNI的160名参与者T1加权MRI数据，通过Freesurfer重建皮层表面模型。利用Ricci流计算共形参数化得到面积扭曲和共形因子，从网格几何直接计算高斯曲率，应用香农熵创建紧凑特征向量，训练多种分类器。

Result: MLP和逻辑回归分类器表现最佳，准确率和F1分数均达到98.62%，在区分AD患者和健康对照方面非常有效。

Conclusion: 共形几何特征熵为皮层形态测量提供了强大而稳健的度量，高分类准确性表明该方法在阿尔茨海默病研究和诊断中具有重要应用潜力。
...
TL;DR: 本文提出了一种基于深度学习的板球视频自动分析系统，能够提取三柱门得分球、检测板球并建模球轨迹，使用YOLOv8进行场地和球检测，结合OCR提取记分卡，通过图像预处理实现稳健的文本提取。
...
Method: 采用YOLOv8架构进行场地和球检测，结合光学字符识别提取记分卡，使用灰度变换、幂变换和形态学操作进行图像预处理，应用迁移学习优化球检测模型。
...
TL;DR: ScaleNet是一种高效的视觉Transformer扩展方法，通过在预训练模型中插入额外层并使用权重共享来减少参数增长，同时通过适配器模块保持性能，显著降低了模型扩展的计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前视觉Transformer模型规模越大性能越好，但从头训练大模型计算成本高昂，需要一种更高效的模型扩展方法。

Method: 在预训练ViT中插入额外层，采用层间权重共享机制减少参数增长，并通过并行适配器模块引入少量调整参数来缓解共享权重可能导致的性能下降。

Result: 在ImageNet-1K数据集上，2倍深度扩展的DeiT-Base模型相比从头训练准确率提升7.42%，且仅需三分之一的训练周期。在目标检测等下游任务中也表现出良好潜力。

Conclusion: ScaleNet提供了一种计算高效的ViT模型扩展方案，能够在显著减少训练成本的同时提升模型性能，具有广泛的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 阻碍开发理解个人偏好的生成模型的关键挑战是缺乏真实世界和细粒度的用户偏好标注数据。

Method: 构建包含真实用户交互数据的ImageGem数据集，利用用户偏好标注训练偏好对齐模型，研究检索模型和视觉语言模型在个性化任务中的性能，并提出在潜在权重空间中编辑定制扩散模型的端到端框架。

Result: 基于ImageGem数据集训练的偏好对齐模型表现更好，该数据集首次实现了生成模型个性化的新范式。

Conclusion: ImageGem数据集为生成模型个性化研究提供了重要支持，开启了理解细粒度个人偏好的生成模型发展的新途径。
...
TL;DR: RISE是一个用于伪装目标检测的检索自增强范式，通过利用整个训练数据集为单张图像生成伪标签，无需人工标注即可训练COD模型。该方法包含原型库构建和KNN检索两个阶段，采用聚类-检索策略和Multi-View KNN检索来提升伪掩码质量。


<details>
  <summary>Details</summary>
Motivation: 现有的伪装目标检测方法主要依赖图像级建模或基于标注的优化，难以利用数据集级别的上下文信息，且需要大量人工标注。RISE旨在通过无监督方式挖掘训练数据集的整体信息来生成高质量的伪标签，减少对人工标注的依赖。

Method: 1. 构建环境和伪装对象的原型库（无标注训练图像）；2. 采用聚类-检索策略：先通过聚类生成粗掩码，然后基于直方图的图像过滤和跨类别检索生成高置信度原型；3. 多视图KNN检索：整合不同视图的检索结果生成更鲁棒和精确的伪掩码。

Result: 大量实验表明，RISE在无监督和基于提示的方法中表现优于当前最先进的方法。

Conclusion: RISE通过检索自增强范式成功利用数据集级上下文信息生成高质量伪标签，为伪装目标检测提供了一种有效的无监督解决方案，显著减少了对人工标注的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种能够在单个中端GPU上生成高质量3D胸部CT扫描的方法，降低现有方法的计算成本，同时提供对输出解剖特征的精确控制。

Method: 使用潜在扩散模型，以3D解剖掩码为条件生成256x256x256尺寸的体素图像，分辨率为1mm各向同性。掩码描绘肺部和结节区域，实现精确的解剖特征控制。

Result: 实验结果表明，仅以结节掩码为条件会导致解剖学上不正确的输出，强调了纳入全局肺结构对于准确条件合成的重要性。该方法支持生成具有不同属性肺结节和无肺结节的多样化CT体积。

Conclusion: 该方法为训练AI模型或医疗专业人员提供了一个有价值的工具，能够高效生成高质量的3D胸部CT扫描，同时保持对解剖特征的精确控制。
...
TL;DR: 提出VFM-VAE直接集成视觉基础模型到潜在扩散模型中，避免蒸馏方法导致的语义偏差问题，通过多尺度潜在融合和渐进分辨率重建实现高质量重建，并开发SE-CKNNA指标和联合对齐策略，大幅加速收敛。


<details>
  <summary>Details</summary>
Motivation: 现有基于蒸馏的方法会削弱与原始视觉基础模型的鲁棒对齐，导致语义偏差。需要更直接的方法来集成视觉基础模型。

Method: 设计VFM-VAE变分自编码器，采用多尺度潜在融合和渐进分辨率重建模块，从空间粗糙的VFM特征实现高质量重建。提出SE-CKNNA指标分析表示动态，开发联合tokenizer-diffusion对齐策略。

Result: 仅80轮训练达到gFID 2.20（无CFG），相比先前tokenizer加速10倍；640轮训练后达到gFID 1.62，确立了直接VFM集成的优越性。

Conclusion: 直接视觉基础模型集成是潜在扩散模型的优越范式，在性能和效率方面均优于蒸馏方法。
...
Method: 两阶段优化框架：第一阶段在正交相机坐标系学习视频HDR高斯表示；第二阶段将视频高斯变换到世界空间并与相机位姿联合优化，并提出时间亮度正则化策略增强HDR外观的时间一致性。

Result: 在构建的新评估基准上，Mono4DGS-HDR在渲染质量和速度上显著优于现有方法的适配方案。

Conclusion: 提出的方法成功实现了从无位姿单目LDR视频重建4D HDR场景，在质量和效率方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有车辆品牌型号识别方法难以适应新发布车型，CLIP模型虽然具有强大的视觉-文本对齐能力，但其固定预训练权重限制了性能，且图像特定微调成本高昂。

Method: 使用视觉语言模型将车辆图像转换为描述性属性，与文本特征数据库进行比对，检索相关条目并与描述组合形成提示，最后由语言模型推断品牌和型号。

Result: 实验表明，该方法相比CLIP基线提高了近20%的识别准确率。
...
TL;DR: 开发了DWaste平台，在资源受限设备上实现实时垃圾分类，比较了多种模型在准确性、延迟和碳排放方面的表现，发现轻量级目标检测模型更适合实时低功耗应用。
...
Method: 开发DWaste计算机视觉平台，在智能手机和边缘设备上实现实时垃圾分类，包括离线功能。测试了多种图像分类和目标检测模型，并使用自定义工具Annotated Lab进行数据标注。

Result: EfficientNetV2S分类器准确率最高（约96%），但延迟高（0.22s）且碳排放高；轻量级目标检测模型性能良好（最高77% mAP），推理速度快（0.03s），模型尺寸小（<7MB）。模型量化可减少75%的模型大小和VRAM使用。

Conclusion: 成功实现了"更环保AI"模型，支持在边缘设备上进行实时可持续垃圾分类，轻量级目标检测模型在资源受限环境中表现更优。
...
TL;DR: 本文提出了一种基于射线对齐的模板物体姿态估计方法，将姿态估计重新表述为射线对齐问题，使用扩散变换器架构来对齐查询图像与姿态模板，在多个基准数据集上取得了具有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 传统的基于模板的物体姿态估计管道通过检索最匹配的模板并与观测图像对齐来估计姿态，但检索错误模板会导致姿态预测不准确。为了解决这个问题，需要一种更稳健的姿态估计方法。

Method: 将模板物体姿态估计重新表述为射线对齐问题，使用扩散变换器架构学习多个姿态模板图像的视向与无姿态查询图像的对齐。重新参数化物体旋转为物体中心相机射线，并通过扩展尺度不变平移估计来建模物体平移。采用基于缩小模板采样的粗到精训练策略。

Result: 在多个基准数据集上的广泛实验表明，该方法在未见物体姿态估计方面与最先进方法相比具有竞争力。

Conclusion: 提出的射线对齐公式和扩散变换器架构为模板物体姿态估计提供了一种有效的方法，能够利用模板的几何先验指导准确的查询姿态推断。
...
TL;DR: 提出了Occluded nuScenes数据集，这是nuScenes基准的扩展，提供可控、参数化的多传感器遮挡数据，用于系统评估自动驾驶感知模型在恶劣条件下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集虽然包含传感器噪声和环境变化，但缺乏可控、参数化和可复现的多传感器退化数据，限制了系统评估感知和融合架构在明确恶劣条件下的能力。

Method: 为相机模态提供完整版和迷你版，包含四种遮挡类型；为雷达和LiDAR提供参数化遮挡脚本，每种实现三种退化类型，支持灵活可重复的遮挡数据生成。

Result: 发布了首个具有可控和可复现退化的多传感器遮挡数据集，支持在部分传感器故障和环境干扰下对感知模型进行一致、可复现的评估。
...
TL;DR: Kaleido是一个主题到视频生成框架，通过专门的数据构建流程和参考旋转位置编码，解决了多主题一致性和背景分离问题，显著提升了参考保真度和语义一致性。
...
TL;DR: CovMatch是一种可扩展的多模态数据集蒸馏框架，通过对齐真实和合成特征的跨协方差并正则化各模态内的特征分布，实现了更强的跨模态对齐和性能提升。


<details>
  <summary>Details</summary>
Motivation: 多模态数据集蒸馏在扩展到多模态对比学习时面临关键挑战：学习跨模态对齐和管理大型编码器的高计算成本。现有方法通过冻结文本编码器来应对可扩展性问题，但这严重限制了语义对齐并成为性能扩展的瓶颈。

Method: 提出CovMatch框架，对齐真实和合成特征的跨协方差，同时正则化各模态内的特征分布。与先前方法不同，CovMatch允许联合优化两个编码器。

Result: 在Flickr30K和COCO数据集上的评估显示，CovMatch优于最先进的多模态蒸馏方法，仅使用500个合成对就在检索准确率上实现了高达6.8%的绝对增益。

Conclusion: CovMatch通过联合优化两个编码器和跨协方差对齐，显著提升了多模态数据集蒸馏的性能，解决了现有方法在跨模态对齐方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在理解3D空间关系方面存在局限，主要依赖纯文本或2D视觉线索，其有限的表示能力阻碍了需要3D空间想象的任务表现。

Method: 采用两阶段训练：首先通过监督训练将VLM生成的3D潜在表示与3D基础模型对齐，然后仅基于结果信号优化整个推理轨迹以改进3D心理建模。
...
TL;DR: ε-Seg是一种基于分层变分自编码器的语义分割方法，用于电子显微镜生物图像，通过中心区域掩码、稀疏标签对比学习、高斯混合模型先验和无聚类标签预测，在仅有0.05%或更少标注数据的情况下实现竞争性分割性能。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜生物图像语义分割在生命科学中仍具挑战性，数据复杂度高，即使人类观察者也难以处理。需要开发能够在极稀疏标注下有效学习的方法。

Method: 使用分层变分自编码器，结合中心区域掩码和修复损失学习鲁棒嵌入；采用稀疏标签对比学习和高斯混合模型先验塑造潜在空间；使用MLP分割头直接预测类别标签而非聚类。

Result: 在2个密集电子显微镜生物组织数据集上展示竞争性稀疏监督分割结果，并在荧光显微镜数据上验证方法适用性。

Conclusion: ε-Seg能够在有限训练标注下对复杂生物图像数据实现有效的稀疏监督语义分割。
...
TL;DR: 本文研究了影响端到端深度学习在历史笔迹识别中性能的各种因素，发现大多数模型在零样本场景下泛化能力差，但识别出一种简单设计能达到顶尖性能的配置。


<details>
  <summary>Details</summary>
Motivation: 历史笔迹识别面临笔迹多样性、文档退化和标记样本有限等挑战，传统方法依赖手工特征在小数据集表现良好，但端到端方法在真实文档级场景中泛化困难。

Method: 探索了不同预处理方法、骨干架构和后处理策略的组合，包括文本分割、补丁采样和特征聚合等技术。

Result: 大多数配置表现不佳，主要由于低层视觉特征捕获弱、补丁表示不一致和对内容噪声敏感，但发现一种端到端设置能达到与顶尖系统相当的结果。

Conclusion: 研究揭示了构建鲁棒端到端系统的关键挑战，并为历史文档笔迹识别中的设计选择提供了重要见解。
...
TL;DR: 本文提出了Mixture-of-Groups Attention (MoGA)，一种高效的稀疏注意力机制，通过轻量级可学习的token路由器精确匹配token，解决了DiT在长视频生成中全注意力二次方复杂度的问题。


<details>
  <summary>Details</summary>
Motivation: 长视频生成中的Diffusion Transformers受限于全注意力机制随序列长度增长的二次方复杂度，而注意力存在高度冗余性，输出主要由少量查询-键对主导。现有稀疏方法依赖分块粗估计，其精度-效率权衡受限于块大小。

Method: 提出MoGA稀疏注意力机制，使用轻量级可学习的token路由器进行精确token匹配，无需分块估计。通过语义感知路由实现有效的长程交互，作为无核方法可与现代注意力栈（如FlashAttention和序列并行）无缝集成。

Result: 基于MoGA开发了高效长视频生成模型，能够端到端生成分钟级、多镜头、480p、24fps的视频，上下文长度约580k。在各种视频生成任务上的综合实验验证了方法的有效性。

Conclusion: MoGA通过精确的token路由解决了长视频生成中的注意力效率问题，实现了高效的长序列处理能力，为长视频生成提供了有效的解决方案。
...
TL;DR: SeeTok提出了一种视觉中心的文本处理方法，将文本渲染为图像，利用预训练的多模态大语言模型进行理解，相比传统的子词分词方法，在保持性能的同时大幅减少了计算量。


<details>
  <summary>Details</summary>
Motivation: 人类通过视觉识别文字形状、布局和模式来阅读，而现代大语言模型依赖子词分词方法，对低资源语言过度分割，产生无意义的序列并增加计算开销。

Method: SeeTok方法将文本渲染为图像（视觉文本），利用预训练的多模态LLM来解释这些图像，复用从大规模多模态训练中学到的OCR和文本-视觉对齐能力。

Result: 在三个不同语言任务中，SeeTok匹配或超越了子词分词器的性能，同时减少了4.43倍的token使用量和70.5%的FLOPs，在跨语言泛化、抗噪性和语言层次结构方面也有额外优势。
...
TL;DR: UniGenBench++是一个用于文本到图像生成的统一语义评估基准，包含600个分层组织的提示，涵盖5个主要主题和20个子主题，支持中英文短长格式，通过多模态大语言模型进行可靠评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏多样化的提示场景和多语言支持，且仅提供粗粒度评估，无法进行细粒度子维度评估，限制了真实世界应用的适用性。

Method: 构建包含600个提示的分层基准，涵盖5个主要主题和20个子主题，支持中英文短长格式；利用Gemini-2.5-Pro多模态大语言模型开发评估流程；训练离线评估模型。
...
TL;DR: GAR是一种区域级多模态大语言模型，通过RoI对齐特征重放技术，能够利用全局上下文进行精确感知，建模多个提示间的交互，并实现高级组合推理。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在处理复杂场景时难以进行细粒度分析，而区域级模型通常孤立理解给定区域，忽略了关键的全局上下文。

Method: 引入Grasp Any Region (GAR)模型，采用有效的RoI对齐特征重放技术，支持利用全局上下文进行精确感知，并建模多个提示间的交互。

Result: GAR-1B在DLC-Bench上超越DAM-3B +4.5，在GAR-Bench-VQA上甚至超过InternVL3-78B。零样本GAR-8B在VideoRefer-BenchQ上优于领域内VideoRefer-7B。

Conclusion: GAR模型实现了从被动描述到主动对话的范式转变，展示了强大的区域级视觉理解能力，并能轻松迁移到视频领域。
...
TL;DR: VC2L提出了一种统一的视觉中心对比学习框架，通过将文本、图像及其组合都渲染为图像，在像素空间中处理多模态数据，无需OCR、文本标记化或模态融合策略。


<details>
  <summary>Details</summary>
Motivation: 解决CLIP等对比视觉语言模型在处理复杂网页文档时的局限性，特别是当文本和图像交错、松散对齐或嵌入视觉形式时。

Method: 使用单一视觉变换器在像素空间中建模所有输入，采用片段级对比学习目标对齐连续多模态片段，利用文档的内在连贯性。

Result: 在提出的三个检索基准测试和M-BEIR、MTEB等数据集上，VC2L相比CLIP风格模型实现了竞争性或更优的性能。
...
TL;DR: 本文提出EMIM模块，将传统动作识别中的cost volume思想与transformer结合，通过构建类似cost volume的亲和矩阵来增强运动建模能力，在运动敏感数据集上表现优异。
...
Method: 提出显式运动信息挖掘模块（EMIM），以cost volume风格构建理想的亲和矩阵，其中关键候选标记集以滑动窗口方式从下一帧中基于查询的相邻区域采样。构建的亲和矩阵既用于聚合上下文信息进行外观建模，也转换为运动特征进行运动建模。

Result: 在四个广泛使用的数据集上验证了方法的运动建模能力，特别是在运动敏感数据集Something-Something V1和V2上，性能优于现有的最先进方法。

Conclusion: 通过将cost volume的有效运动建模特性整合到transformer中，提出的EMIM模块显著提升了在运动敏感数据集上的性能，证明了该方法在动作识别中的有效性。
...
TL;DR: PLANA3R是一个无需相机位姿的室内场景度量3D重建框架，通过平面3D基元表示和视觉变换器，从无位姿的双视图图像中重建度量3D结构，无需平面标注监督。


<details>
  <summary>Details</summary>
Motivation: 利用室内场景固有的几何规律性，通过紧凑的平面3D基元表示来解决室内场景的度量3D重建问题，避免对3D平面标注的依赖。

Method: 使用视觉变换器提取稀疏平面基元，估计相对相机位姿，通过平面溅射进行几何学习监督，梯度通过高分辨率渲染的深度和法线图传播。
...
Method: 识别空间局部性和语义汇聚现象，将注意力头分为空间局部性头和语义汇聚头，分别采用短近期令牌窗口和紧凑高关注令牌集的策略进行KV缓存压缩。

Result: 实验证明该方法能实现5倍内存使用减少和6.6倍整体吞吐量加速，仅带来最小视觉质量损失。

Conclusion: 该方法使资源受限硬件上能够高效进行原生自回归图像生成，为图像生成领域的KV缓存压缩提供了有效解决方案。
...
TL;DR: IF-VidCap是一个新的可控视频字幕基准，包含1,400个高质量样本，评估模型在格式正确性和内容正确性两个维度的表现。评估发现专有模型仍占主导但开源模型差距正在缩小，密集字幕专用模型在复杂指令下表现不如通用MLLMs。


<details>
  <summary>Details</summary>
Motivation: 当前基准主要评估描述全面性而忽视了指令跟随能力，实际应用需要遵循特定用户指令而非生成无约束的详尽描述。

Method: 引入IF-VidCap基准，采用系统性框架评估字幕的格式正确性和内容正确性两个维度，对超过20个知名模型进行全面评估。

Result: 专有模型仍占主导但性能差距正在缩小，顶级开源解决方案已接近同等水平；密集字幕专用模型在复杂指令下表现不如通用MLLMs。
...
TL;DR: 本文提出了ColIAGS，一种针对结肠镜检查优化的3D高斯泼溅框架，通过改进的外观建模和几何建模来解决动态光照下的视图合成问题。


<details>
  <summary>Details</summary>
Motivation: 传统的3D高斯泼溅方法假设静态光照，无法处理结肠镜检查中由动态光源/相机引起的光度变化，导致重建质量下降。

Method: 提出改进的外观建模，包含两种光照衰减因子；改进的几何建模使用高维视图嵌入来增强高斯几何属性预测；利用余弦嵌入隐式生成光照衰减解决方案。

Result: 在标准基准测试中，ColIAGS实现了新颖视图合成和精确几何重建的双重能力，在渲染保真度方面显著优于现有方法，同时大幅降低深度MSE。

Conclusion: ColIAGS框架有效解决了结肠镜检查中的动态光照问题，在视图合成和几何重建方面都取得了显著改进。
...
TL;DR: 本文提出SEAL框架，通过自然层次结构指导的语义感知分层学习，解决广义类别发现(GCD)问题，在细粒度和粗粒度数据集上均取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法依赖单层语义或手动设计的抽象层次结构，限制了泛化能力和可扩展性。需要利用自然存在的层次结构来改进类别发现。
...
Conclusion: SEAL框架通过利用自然层次结构和改进的对比学习策略，有效解决了GCD问题，在多个数据集上表现出色。
...
TL;DR: 本研究利用地理空间基础模型评估城市热岛效应缓解策略，通过微调模型预测未来气候情景下的地表温度，并展示其在数据稀缺区域的应用价值。


<details>
  <summary>Details</summary>
Motivation: 随着城市化和气候变化的加剧，城市热岛效应日益严重。传统机器学习模型在数据有限时预测不准确，特别是在服务不足地区。地理空间基础模型提供了更好的泛化能力。

Method: 建立城市热模式的实证地面真实数据，量化绿地的冷却效应，并以此为基础微调基础模型来预测未来气候情景下的地表温度。

Result: 基础模型能够有效评估城市热岛缓解策略，在数据稀缺区域表现出强大的预测能力。

Conclusion: 地理空间基础模型为评估城市热岛缓解策略提供了有力工具，有助于支持更具气候韧性的城市建设。
...
Conclusion: UltraGen框架成功解决了高分辨率视频生成的计算瓶颈问题，为原生高分辨率视频合成提供了高效可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: CLIP文本编码器存在77个token的最大输入长度限制，无法有效处理长文本和进行细粒度语义理解，同时缺乏多语言输入支持，限制了其更广泛的应用。

Method: 首先从CLIP文本编码器向LLM嵌入器蒸馏知识，利用CLIP的预训练知识建立初始对齐；然后通过图像-文本对比调优进一步对齐CLIP图像编码器和LLM嵌入器，使用自蒸馏正则化防止过拟合；在表示继承和对比调优过程中采用实例语义对齐损失和嵌入结构对齐损失。

Result: 该方法有效对齐了CLIP图像编码器与LLM嵌入器，克服了直接对比学习可能破坏CLIP图像编码器内在视觉-语言对齐的问题。

Conclusion: ProCLIP通过渐进式对齐策略成功解决了CLIP文本编码器的局限性，同时保持了CLIP预训练知识的有效利用。
...
TL;DR: 提出一种教师-学生框架，通过集成两个监督头和一个自监督头来增强胸部X光片的疾病和症状检测，在COVID-19、结核病和正常病例分类中达到98.85%准确率，症状检测获得90.09%的宏F1分数。


<details>
  <summary>Details</summary>
Motivation: 结核病是全球健康问题，特别是在资源有限和偏远地区。早期检测对治疗至关重要，但缺乏熟练的放射科医生凸显了对AI驱动筛查工具的需求。

Method: 采用教师-学生框架，集成两个监督头和一个自监督头来增强胸部X光片的疾病和症状检测。

Result: 模型在区分COVID-19、结核病和正常病例方面达到98.85%准确率，在多标签症状检测方面获得90.09%的宏F1分数，显著优于基线方法。
...
TL;DR: SAM 2++是一个统一的视频跟踪模型，能够处理任意粒度的跟踪任务（掩码、边界框、点），通过任务特定提示、统一解码器和任务自适应内存机制实现跨粒度跟踪的统一。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪器通常针对单一任务设计，依赖定制化模块，导致泛化能力受限且模型设计冗余。需要开发一个能够统一处理不同粒度跟踪任务的模型。
...
TL;DR: 提出统一的层次化掩码框架，揭示模型架构与注意力掩码构建的等价性，并基于此开发M3Dphormer图变换器，通过多级掩码和双注意力计算实现自适应交互建模。
...
Method: 提出层次化掩码框架，将架构设计转化为掩码构造问题。开发M3Dphormer模型，包含三个理论基础的层次掩码、双级专家路由机制和基于局部掩码稀疏度的双注意力计算方案。

Result: 在多个基准测试上的广泛实验表明，M3Dphormer实现了最先进的性能，验证了统一框架和模型设计的有效性。

Conclusion: 层次化掩码框架为图变换器提供了统一建模范式，M3Dphormer通过自适应集成多级交互信息，在保持可扩展性的同时实现了优异的性能。
...
TL;DR: 提出动态空间智能基准DSI-Bench，包含近1000个动态视频和1700多个手动标注问题，用于评估模型在动态3D场景中的空间推理能力。
...
TL;DR: 提出基于精神分析理论的多智能体框架，通过智能体交互模拟自我意识、前意识和无意识，使用个性化模块结合固定特质和动态需求，在情感丰富对话上进行参数高效微调。


<details>
  <summary>Details</summary>
Motivation: 基于精神分析理论，为大型语言模型建立人工意识模型，通过模拟人类心理结构来增强模型的情感深度和个性化认知能力。

Method: 使用多智能体框架模拟心理结构，包含个性化模块（固定特质+动态需求），在情感丰富对话数据集上进行参数高效微调。

Result: 在8个个性化条件下评估，微调模型获得71.2%的偏好率，情感深度提升且输出方差降低。

Conclusion: 该框架展示了在个性化认知方面的潜力，能够实现自适应、个性化的意识建模。
...
TL;DR: POPI是一个个性化LLM框架，通过偏好推断模型将用户信号提炼为自然语言摘要，作为透明、紧凑、可转移的个性化表示来调节共享生成模型，实现高效个性化响应。


<details>
  <summary>Details</summary>
Motivation: 现有对齐技术（如RLHF、DPO）主要优化群体平均水平，忽视个体差异；而传统个性化方法要么计算成本过高，要么效率低下且易受噪声影响。

Method: 提出POPI框架：使用偏好推断模型将异质用户信号提炼为简洁自然语言摘要，作为个性化表示；通过强化学习联合优化偏好推断和个性化生成，确保摘要最大程度编码有用偏好信息。

Result: 在四个个性化基准测试中，POPI持续提升个性化准确性，同时大幅减少上下文开销；优化后的摘要可无缝迁移到冻结的现成LLM，实现即插即用个性化而无需权重更新。

Conclusion: POPI提供了一种高效、可转移的个性化解决方案，通过自然语言摘要作为中间表示，在保持性能的同时显著降低计算和上下文成本。
...
TL;DR: 本文提出通过增强预训练数据的世界上下文（DWC）来改善大语言模型的幻觉和可信度问题，通过在真实世界场景中锚定数据来减少模型训练的不确定性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的幻觉和可信度问题本质上源于预训练阶段，包括预训练数据和下一个token预测学习机制。现有方法主要关注后训练和推理技术，但预训练数据的质量是关键。

Method: 提出DWC方法，为预训练数据添加世界上下文信息，并增加大量反映工业场景的数据。继续预训练JT-35B-Base模型，使用1.5万亿DWC token，并引入后训练程序激活DWC潜力。

Result: 与相似规模的Qwen模型相比，JT-Safe-35B在安全和可信度评估基准上的平均性能提升了1.79%，且仅使用了6.2万亿token进行预训练。

Conclusion: 通过增强预训练数据的世界上下文，可以有效提升大语言模型的安全性和可信度，同时减少训练数据需求。
...
Result: 在多个基准测试上的综合评估表明，Select-Then-Decompose策略始终位于帕累托前沿，在性能和成本之间实现了最佳平衡。
...
TL;DR: 本文对在线游戏聊天中的自动毒性检测进行了NLP方法比较分析，评估了传统机器学习、大语言模型、微调Transformer和RAG方法，提出了混合审核系统架构，发现微调DistilBERT在准确性和成本之间达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 在线游戏聊天中的毒性内容检测需要高效、成本可控的解决方案，传统方法在动态环境中存在局限性，需要系统评估不同NLP方法的性能表现。

Method: 采用综合评估框架，比较传统机器学习模型（带嵌入）、大语言模型（零样本和少样本提示）、微调Transformer模型和检索增强生成方法，评估分类准确性、处理速度和计算成本三个维度。

Result: 实验结果显示不同方法性能差异显著，微调的DistilBERT在准确性和成本之间达到最优平衡，为在线游戏环境提供了有效的解决方案。

Conclusion: 研究为动态在线游戏环境中部署成本效益高、高效的内容审核系统提供了实证依据，混合审核系统架构能够优化人工审核工作量并实现持续学习。
...
TL;DR: 该研究探讨了在噪声口语数据中扩展命名实体识别（NER）模型到新的PII实体时，发现联合微调BERT模型在标准语义实体和新模式PII上可以实现和平共存，但LOC实体由于与PII共享模式特征而特别脆弱，同时发现了反向O标签表示漂移现象。


<details>
  <summary>Details</summary>
Motivation: 扩展NER模型以适应新的PII实体在噪声口语数据中是常见需求，研究旨在理解模型如何同时处理标准语义实体和模式化PII实体而不产生显著性能下降。

Method: 使用增量学习设置作为诊断工具，测量语义漂移，分析不同实体类型的表示重叠，并研究O标签分类器的冻结与解冻对学习的影响。

Result: 发现LOC实体因与PII共享模式特征（如邮政编码）而特别脆弱；识别出反向O标签表示漂移现象，即模型初始训练将PII模式映射到'O'标签会阻碍新学习，只有解冻O标签分类器才能解决此问题。

Conclusion: 该工作提供了NER模型适应的机制诊断，强调了特征独立性、表示重叠和O标签可塑性的重要性，为模型扩展提供了重要见解。
...
TL;DR: AtlasKV是一种参数化知识集成方法，通过将十亿级知识图谱集成到大型语言模型中，避免了传统检索增强生成方法的外部检索模块依赖和高延迟问题。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成方法依赖外部检索模块和检索到的文本上下文，在大规模知识增强时会导致显著的推理延迟。

Method: 提出KG2KV和HiKVP方法，将知识图谱三元组以次线性时间和内存复杂度集成到LLMs中，利用LLMs固有的注意力机制。

Result: 能够以极低的GPU内存成本（小于20GB显存）增强具有十亿级知识图谱的LLMs，同时保持强大的知识基础和泛化性能。

Conclusion: AtlasKV提供了一种可扩展、有效且通用的参数化知识集成方式，无需外部检索器、长上下文先验或重新训练即可适应新知识。
...
TL;DR: 该研究开发了一个衡量知识编辑技术植入信念深度的框架，发现简单提示和机制编辑技术无法深度植入知识，而合成文档微调(SDF)方法能成功植入行为类似真实知识的信念，但成功率并非普遍。


<details>
  <summary>Details</summary>
Motivation: 评估知识编辑技术是否真正让大语言模型相信植入的事实知识，而不仅仅是表面接受。

Method: 开发了信念深度测量框架，通过三个维度评估：1)在相关上下文中的泛化能力；2)对自我审查和直接挑战的鲁棒性；3)与真实知识的表示相似性(使用线性探针测量)。

Result: 简单提示和机制编辑技术无法深度植入知识，而SDF方法通常能成功植入行为类似真实知识的信念，但植入与基本世界知识相矛盾的信念时效果脆弱且表示与真实知识不同。

Conclusion: 该工作为信念深度引入了可测量的标准，为知识编辑技术在实际应用中的严格评估提供了基础。
...
TL;DR: SimBA是一个三阶段框架，用于简化语言模型基准测试分析。通过数据集和模型比较、发现代表性子集、以及使用该子集预测模型性能，能够显著减少评估所需的数据集数量，同时保持模型排名和性能预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型在大规模基准测试上进行评估，但这些基准测试难以理解，特别是在模型选择方面。需要一种方法来简化基准测试分析，提高模型训练效率和数据集验证效果。

Method: SimBA框架包含三个阶段：stalk（进行数据集和模型比较）、prowl（发现代表性子集）、pounce（使用代表性子集预测模型性能）。开发了一种基于原始评估分数的代表性集合发现算法。

Result: 在HELM、MMLU和BigBenchLite三个基准测试上，仅使用6.25%、1.7%和28.4%的数据集就能达到至少95%的覆盖率。使用这些代表性子集可以保持模型排名，并在保留模型集上实现接近零均方误差的性能预测。

Conclusion: SimBA能够帮助模型开发者提高训练效率，并帮助数据集创建者验证新数据集是否与基准测试中的现有数据集不同。该框架代码已开源。
...
TL;DR: 本文提出STEAM方法，通过回译检测增强多语言水印的鲁棒性，解决现有方法在中低资源语言中翻译攻击下的失效问题。


<details>
  <summary>Details</summary>
Motivation: 当前多语言水印方法声称具有跨语言鲁棒性，但实际上仅在高资源语言上评估，在中低资源语言中无法抵御翻译攻击，存在不公平性。

Method: 提出STEAM方法，基于回译的检测机制，能够恢复因翻译而丢失的水印强度，兼容任何水印方法，对分词器和语言具有鲁棒性。

Result: 在17种语言上平均提升0.19 AUC和40% TPR@1%，显著增强了多语言水印的公平性和鲁棒性。

Conclusion: STEAM为跨不同语言实现更公平的水印提供了一条简单而鲁棒的路径，解决了语义聚类在词汇量不足语言中的失效问题。
...
<details>
  <summary>Details</summary>
Motivation: 在序列交互数据建模中，当语义上下文有限或缺失时，LLM的性能往往会受到影响。需要一种方法来自动丰富这些序列的语义内容。

Method: 使用LLM在少样本设置下生成辅助上下文信号，通过推断用户意图的潜在语义方面和物品关系来增强原始序列。生成信号包括推断的使用场景、物品意图或主题摘要等。

Result: 在基准序列建模任务中集成生成的资源，性能得到持续提升。分析显示LLM生成的信号具有高语义新颖性和多样性，增强了下游模型的表示能力。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs处理文本中句间依赖关系（如代词回指和词汇连贯性）的翻译能力，探索推理提示对翻译质量的影响。

Method: 使用英语-法语DiscEvalMT基准测试，评估12个LLM在两个任务上的表现：区分正确与错误翻译、生成正确翻译，比较有无链式思维推理提示的效果。

Result: 最佳模型通过推理达到约90%的区分准确率和92%的COMET分数，GPT-4、GPT-4o和Phi表现突出，观察到"强者更强"效应。

Conclusion: LLMs能够有效处理句间依赖关系的翻译任务，推理提示显著提升性能，且性能越好的模型从推理中获益越多。
...
TL;DR: 本研究提出了一个结合法律理论和实证评估的实验协议，测试了四个AI系统在法律任务中的表现。专门化的JusIA模型在模拟律师日常工作的任务中持续优于通用AI系统。


<details>
  <summary>Details</summary>
Motivation: 评估通用AI在法律领域的应用效果，探索领域专业化对法律AI输出的重要性，以及建立理论基础与实证评估相结合的评价方法。

Method: 结合法律理论（如材料正确性、系统一致性、论证完整性）与48名法律专业人员的实证评估，测试四个AI系统（JusIA、ChatGPT Free、ChatGPT Pro、Gemini）在模拟律师日常工作任务中的表现。

Result: 专门化模型JusIA在所有测试任务中持续优于通用AI系统，证明了领域专业化和理论基础评估对可靠法律AI输出的重要性。

Conclusion: 领域专业化和理论基础的评估协议对于确保法律AI输出的可靠性至关重要，专门化模型在特定法律任务中表现优于通用模型。
...
TL;DR: 该研究探索使用大型语言模型预测《龙与地下城》玩家行为并生成Avrae Discord机器人命令，发现指令模型相比推理模型更适合此任务，且提示词微小变化会显著影响输出。


<details>
  <summary>Details</summary>
Motivation: 研究旨在利用LLM技术自动预测DnD玩家行为并生成对应的机器人命令，以提升游戏体验和自动化程度。

Method: 使用FIREBALL数据集评估推理模型DeepSeek-R1-Distill-LLaMA-8B和指令模型LLaMA-3.1-8B-Instruct的命令生成能力。

Result: 研究发现指令模型足以完成此任务，且提示词的微小变化会显著影响模型输出质量。

Conclusion: 指令模型比推理模型更适合DnD命令生成任务，提示词设计对模型性能有重要影响。
...
TL;DR: 研究发现大语言模型内部编码的问题难度与人类判断一致，人类标注的难度信息在强化学习训练中能够稳定追踪模型泛化性能，而基于模型性能自动估计的难度指标会随着模型改进而失准。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型是否内部编码与人类判断一致的问题难度表示，以及这种表示在强化学习后训练中如何追踪泛化性能。

Method: 在60个模型上训练线性探针，评估Easy2HardBench的数学和编程子集，分析人类标注难度和LLM衍生难度的可解码性，并在GRPO训练中追踪难度探针的变化。

Result: 人类标注难度具有强线性可解码性（ρ≈0.88）且随模型规模扩展，而LLM衍生难度较弱且扩展性差；沿难度方向引导可减少幻觉并提高准确性；在训练过程中，人类难度探针与测试准确率正相关，而LLM难度探针与性能负相关。
...
TL;DR: 该论文提出了一种基于规则的机制可解释性方法，通过从注意力层提取特征描述规则，包括skip-gram规则、缺失规则和计数规则，为特征提供更客观的解释。


<details>
  <summary>Details</summary>
Motivation: 当前机制可解释性方法主要依赖主观检查特征激活的文本序列，缺乏客观的规则描述。本文旨在开发一种基于规则的描述方法，能够自动提取特征的行为模式。

Method: 从注意力层训练的特征中提取三种类型的规则：(1) skip-gram规则，描述输入模式与输出token的关联；(2) 缺失规则，描述特定输入模式不激活输出token；(3) 计数规则，基于词频阈值触发行为。

Result: 在GPT-2小模型上应用该方法，发现大多数特征可以用约100个skip-gram规则很好描述，第一层中超过四分之一的特征包含缺失规则，并识别出一些计数规则的例子。

Conclusion: 本文为基于规则的特征描述研究奠定了基础，定义了规则类型、提取方法，并初步分类了它们代表的行为模式，为未来研究提供了框架。
...
TL;DR: 提出了一种基于任务描述自动选择提示技术并生成高质量提示的新方法，该方法构建了任务集群与提示技术关联的知识库，在23个任务上表现优于标准提示和现有自动提示生成工具。
...
Method: 构建任务集群与提示技术关联的知识库，根据用户任务描述分配到最相关任务集群，动态整合知识库中的技术生成提示。
...
TL;DR: 提出了CMT-Bench基准测试，用于评估LLM在动态文本转表格任务中的鲁棒性，发现当前模型在提取线索缺失、长上下文和实体形式变化时表现脆弱。


<details>
  <summary>Details</summary>
Motivation: 现有的文本转表格系统依赖大量提示工程或迭代事件提取，计算成本高且难以理解模型如何推理时序演变叙事。需要开发能评估模型鲁棒性的诊断基准。

Method: 构建基于板球评论的CMT-Bench基准，包含三个语义保持维度：提取线索消融、时间前缀测试、实体形式扰动，以分离提取捷径与状态跟踪能力。

Result: 测试发现：无提取摘要时性能大幅下降；输入长度增加时性能单调下降；实体形式变化时准确率一致下降；数值错误模式显著变化表明推理漂移。
...
TL;DR: LLM作为评估器时存在评分范围偏差问题，本文通过对比解码方法缓解该偏差，显著提升了与人类判断的相关性。


<details>
  <summary>Details</summary>
Motivation: LLM作为无参考直接评分评估器时，其评分结果可靠性存在挑战，主要问题在于评分范围偏差——LLM评分高度依赖预设评分范围，且同系列模型存在类似偏差。

Method: 采用对比解码方法来缓解评分范围偏差问题。

Result: 通过对比解码方法，在不同评分范围内与人类判断的Spearman相关性平均相对提升了11.3%。

Conclusion: 对比解码方法能有效缓解LLM评估器中的评分范围偏差问题，显著提高评估结果与人类判断的一致性。
...
TL;DR: 本文提出MARCUS系统，用于从叙事文本中计算生成基于事件和关系的人物弧线，通过提取事件、角色、情感等要素来量化人物关系发展。


<details>
  <summary>Details</summary>
Motivation: 人物弧线是文学研究中的重要理论工具，但缺乏计算量化方法。本研究旨在通过计算建模使这一理论概念具体化，为后续应用铺平道路。

Method: 开发MARCUS NLP流水线，提取事件、参与角色、隐含情感和情绪，建模角色间关系，并在整个叙事过程中跟踪聚合这些关系以生成图形化的人物弧线。

Result: 成功应用于《哈利·波特》和《指环王》两个长篇奇幻系列，生成了人物弧线图，并对方法进行了评估。

Conclusion: 该方法为人物弧线提供了量化表示，讨论了现有挑战，提出了应用前景和未来工作方向。
...
TL;DR: DelvePO是一个任务无关的提示优化框架，通过方向引导的自进化方式优化提示，将提示解耦为不同组件并引入工作记忆机制，在各种任务中表现优于现有方法。
...
Method: 提出DelvePO框架，将提示解耦为不同组件以探索各因素对任务的影响，引入工作记忆机制帮助LLMs缓解自身不确定性并获得关键洞察来指导新提示生成。

Result: 在涵盖多个领域的任务上对开源和闭源LLMs进行广泛实验，结果显示DelvePO在相同实验设置下始终优于先前的最先进方法。

Conclusion: DelvePO框架通过自进化方式有效优化提示，在不同任务中展现出良好的有效性和可迁移性。
...
TL;DR: 将长文本输入渲染为单个图像提供给LLM，实现输入压缩，显著减少解码器token使用量而不降低任务性能


<details>
  <summary>Details</summary>
Motivation: 利用多模态LLM处理视觉输入的能力，探索通过将文本输入转换为图像形式来减少token使用量，同时保持模型性能的可能性

Method: 将长文本输入渲染为单个图像，直接提供给解码器LLM，在RULER（长上下文检索）和CNN/DailyMail（文档摘要）两个基准上进行实验

Result: 文本转图像方法实现了显著的token节省（通常接近一半），且没有降低任务性能
...
TL;DR: 本文构建了英汉盲文混合数据集，提出基于语法树的盲文数据增强方法和盲文知识微调(BKFT)方法，显著提升了盲文翻译性能。


<details>
  <summary>Details</summary>
Motivation: 盲文在视障人士教育和信息获取中至关重要，但面临数据稀缺和混合文本歧义等挑战，需要改进盲文信息处理方法。

Method: 构建英汉盲文混合数据集，提出基于语法树的数据增强方法，并设计盲文知识微调(BKFT)来降低盲文上下文特征学习难度。
...
<details>
  <summary>Details</summary>
Motivation: 解决美国食物不安全问题，现有系统存在检索不完整、地理相关性差、无法适应现实约束等问题，导致最弱势群体无法获取急需资源。

Method: 1) 整合官方数据库、社区平台和社交媒体的异构数据；2) 使用轻量级强化学习算法优化地理可及性和营养正确性；3) 建立在线反馈循环动态调整检索策略。

Result: Food4All框架实现了营养标注和在需求点提供指导，为面临食物不安全及其复合健康风险的人群提供直接支持。

Conclusion: 该框架为构建可扩展、公平和智能的系统迈出了重要一步，直接支持面临食物不安全问题的人群。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学问答方法存在局限性：RAG方法容易受到噪声或不完整检索的影响，而GAG方法容易产生幻觉或不准确信息。两者都可能误导推理并削弱答案可靠性。

Method: MedRGAG包含两个关键模块：知识引导上下文补全（KGCC）指导生成器产生补充检索缺失知识的背景文档；知识感知文档选择（KADS）自适应选择检索和生成文档的最优组合形成简洁而全面的证据。

Result: 在五个医学问答基准测试上的广泛实验表明，MedRGAG相比MedRAG提升了12.5%，相比MedGENIE提升了4.5%。

Conclusion: MedRGAG通过统一检索和生成，有效提升了知识密集型推理的效果，证明了整合外部和参数知识在医学问答中的有效性。
...
TL;DR: 本研究探讨了在医疗领域（特别是心电图学）中，通过微调开源大语言模型和使用检索增强生成技术，与通用模型Claude Sonnet 3.7进行比较。结果显示微调模型在多项选择题和自动文本指标上表现优异，而人类专家更偏好Claude 3.7和RAG处理复杂查询。
...
Method: 研究方法包括在特定领域文献上微调开源模型，实施多层评估框架，比较微调模型、检索增强生成和通用模型Claude Sonnet 3.7，涵盖多项选择题评估、自动文本指标、LLM作为评判者评估和人类专家评估。

Result: 微调的Llama 3.1 70B在多项选择题评估和自动文本指标上表现最优，在LLM作为评判者评估中排名第二（仅次于Claude 3.7）。人类专家评估更偏好Claude 3.7和RAG方法处理复杂查询。微调模型在所有评估模式中显著优于其基础版本。

Conclusion: 研究结论显示不同评估方法之间存在显著的性能异质性，突显了评估复杂性。尽管如此，通过微调和RAG的领域特定适应能够与专有模型竞争，支持了保护隐私、可本地部署的临床解决方案的可行性。
...
TL;DR: HYDRE是一个混合远程监督关系抽取框架，结合了训练好的DSRE模型和大型语言模型的上下文学习能力，通过动态示例检索策略提升关系抽取性能，并在跨语言设置中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的DSRE模型依赖任务特定训练，与大型语言模型的上下文学习结合不足。主要挑战是LLM可能因噪声标注而无法正确学习关系语义。
...
Conclusion: HYDRE框架有效结合了DSRE模型和LLM的优势，通过动态示例检索解决了噪声标注问题，在单语言和跨语言关系抽取任务中都表现出色。
...
TL;DR: KrishokBondhu是一个面向孟加拉语农民的语音驱动、呼叫中心集成的农业咨询平台，基于检索增强生成(RAG)框架，通过电话接口提供实时、情境感知的农业建议。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国许多农民难以获得及时、专业的农业指导，需要开发适合当地语言和通信条件的解决方案。

Method: 系统整合权威农业手册和出版物，通过OCR和文档解析数字化内容，使用向量数据库进行语义检索，结合语音转文本、RAG模块、Gemma 3-4B大语言模型和文本转语音技术。

Result: 在试点评估中，系统对72.7%的多样化农业查询产生高质量回答，相比KisanQRS基准，综合得分提高44.7%，在情境丰富度和完整性方面有显著提升。
...
TL;DR: 提出了韩文简单问答数据集(KoSimpleQA)，用于评估大语言模型在韩语文化知识方面的真实性。该数据集包含1000个简短的事实性问题，具有明确的答案。评估结果显示，即使是表现最好的模型正确率也只有33.7%，表明该数据集的挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估主要关注英语知识，缺乏对韩语文化知识的专门评估。需要构建一个具有挑战性且易于评分的韩语事实问答基准。

Method: 构建了包含1000个简短事实性问题的韩文简单问答数据集(KoSimpleQA)，这些问题具有明确的答案。对支持韩语的各种规模的开源大语言模型进行了全面评估，并分析了推理模型在事实问答任务中的表现。

Result: 评估结果显示，即使是表现最好的模型正确率也只有33.7%。在KoSimpleQA上的性能排名与英文SimpleQA存在显著差异。推理模型能够更好地激发其潜在知识，并在不确定时更好地选择放弃回答。

Conclusion: KoSimpleQA是一个具有挑战性的韩语文化知识评估基准，揭示了现有模型在韩语事实知识方面的局限性。推理能力有助于模型更好地利用其潜在知识并提高不确定性处理能力。
...
TL;DR: 该论文针对英语ASR系统对第二语言使用者的公平性问题，提出了一种结合传统经验风险最小化与公平性驱动的优化方法，通过轻量级适配器微调显著提升了不同口音群体间的公平性。


<details>
  <summary>Details</summary>
Motivation: 现有广泛使用的ASR模型（如Whisper和Seamless-M4T）在26个不同口音群体间存在显著的词错误率波动，表明存在严重的公平性差距，需要开发更公平的ASR系统。

Method: 提出公平性提示微调方法，使用轻量级适配器，结合谱解耦、组分布鲁棒优化和不变风险最小化等公平性驱动目标，与传统经验风险最小化交叉熵损失融合。

Result: 在宏观平均词错误率方面，相比预训练的Whisper和SeamlessM4T模型分别实现了58.7%和58.5%的相对改进，相比标准经验风险最小化微调分别提升了9.7%和7.8%。

Conclusion: 所提出的方法有效提升了ASR系统在不同口音群体间的公平性，同时保持了整体识别准确性，为解决第二语言使用者ASR公平性问题提供了有效解决方案。
...
TL;DR: MENTOR框架通过结合强化学习和教师引导蒸馏，解决了将大语言模型的工具使用能力蒸馏到小语言模型时的泛化问题和奖励稀疏性问题。


<details>
  <summary>Details</summary>
Motivation: 当前主流的监督微调方法泛化能力差，而标准强化学习方法由于奖励稀疏导致探索效率低和策略次优，需要一种更好的蒸馏方法。

Method: 提出MENTOR框架，结合强化学习和教师引导蒸馏，使用教师参考轨迹构建密集的复合教师引导奖励，提供细粒度指导。

Result: 大量实验表明，MENTOR相比监督微调和标准稀疏奖励强化学习基线，显著提高了小语言模型的跨域泛化能力和策略能力。

Conclusion: MENTOR框架通过协同结合强化学习和教师引导蒸馏，有效解决了工具使用能力蒸馏中的关键挑战，为实际应用提供了更优的解决方案。
...
TL;DR: Adamas是一种轻量级但高精度的稀疏注意力机制，用于长上下文推理。它通过Hadamard变换、分桶和2位压缩产生紧凑表示，利用曼哈顿距离估计进行高效top-k选择，在仅使用64个token预算时就能匹配全注意力的准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型支持数十万到数百万token的长上下文窗口，但扩展的上下文加剧了自注意力的二次方成本，导致自回归解码延迟严重。现有的稀疏注意力方法虽然缓解了这些成本，但依赖于启发式模式，难以召回每个查询的关键键值对，导致准确性下降。

Method: Adamas应用Hadamard变换、分桶和2位压缩来产生紧凑表示，并利用曼哈顿距离估计进行高效的top-k选择。该方法设计用于长上下文推理，支持比先前最先进方法高8倍的稀疏度。

Result: 实验显示，Adamas在仅使用64个token预算时就能匹配全注意力的准确性，在128个token时达到近乎无损的性能。在32K长度序列上，实现了高达4.4倍的自注意力和1.5倍的端到端加速。值得注意的是，Adamas达到了与全注意力相当甚至更低的困惑度。

Conclusion: Adamas是一种有效的稀疏注意力机制，能够在积极稀疏条件下保持准确性，支持更高的稀疏度并提供显著的加速效果。
...
TL;DR: 提出了一种新的提示范式CoCT（概念思维链），通过先标记概念再生成详细内容的方式，提升LLM在开放域任务中的表现，特别是在日常和情感支持对话中取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: CoT在数学、编程和推理任务中表现良好，但在开放域任务中效果有限，因为这类任务缺乏明确定义的推理步骤或逻辑转换。

Method: 提出CoCT范式，让LLM先标记概念（如情感、策略、主题），然后生成详细内容，允许在话语中进行概念链，鼓励LLM进行深度和战略性思考。

Result: 在日常和情感支持对话任务中，自动、人工和模型评估均显示CoCT超越了Self-Refine、ECoT、ToT、SoT和RAG等基线方法。
...
TL;DR: 本文提出了一种token级别的可靠性度量方法，用于量化手语翻译中模型对视觉信息的依赖程度，从而检测幻觉问题。该方法结合特征敏感性和反事实信号，能够预测幻觉率并区分基于视觉的翻译和猜测。


<details>
  <summary>Details</summary>
Motivation: 手语翻译中，模型生成与视觉证据不符的流畅文本（幻觉）是一个严重问题，特别是无gloss模型更容易出现这种问题，因为它们直接将连续的手语动作映射到自然语言，缺乏中间gloss监督作为对齐。

Method: 提出token级可靠性度量方法，结合特征敏感性（测量视频被屏蔽时内部特征的变化）和反事实信号（捕获干净和改变视频输入之间的概率差异），聚合为句子级可靠性评分。

Result: 在两个SLT基准测试（PHOENIX-2014T和CSL-Daily）上评估，结果显示可靠性能够预测幻觉率，跨数据集和架构泛化，在视觉退化时降低。结合文本信号（置信度、困惑度或熵）可进一步改进幻觉风险估计。

Conclusion: 可靠性是诊断SLT中幻觉的实用可重用工具，为多模态生成中更稳健的幻觉检测奠定了基础，并解释了为什么无gloss模型更容易出现幻觉。
...
TL;DR: 该研究评估了大型语言模型在幽默生成过程中与有害内容的耦合关系，发现有害输出会获得更高的幽默评分，特别是在基于角色的提示下，表明生成器和评估器之间存在偏见放大循环。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地用于创意写作和参与内容，其输出安全性引发担忧。本研究以幽默生成为测试平台，评估现代LLM流程中的趣味性优化如何与有害内容耦合。

Method: 通过联合测量幽默度、刻板印象性和毒性来评估六个模型，并辅以通过信息论指标分析不一致信号。在额外的讽刺生成任务中进行外部验证，使用人类感知的趣味性判断。

Result: 有害输出获得更高的幽默评分，在基于角色的提示下进一步增加。刻板印象/有毒笑话的平均幽默评分提高10-21%，被LLM指标标记为有趣的笑话中刻板印象笑话出现频率增加11-28%，在人类认为有趣的笑话中出现频率增加高达10%。

Conclusion: LLM幽默生成存在与有害内容的系统性耦合，有害线索会扩大预测不确定性，甚至可能使有害笑点对某些模型更加预期，表明在学习的幽默分布中存在结构性嵌入。
...
TL;DR: ChronoPlay是一个用于自动生成游戏RAG基准测试的新框架，通过双动态更新机制跟踪游戏内容更新和玩家社区变化，确保测试的真实性和时效性。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对动态领域（如在线游戏）的RAG系统基准测试，难以进行标准化评估，主要挑战在于游戏内容更新和玩家社区关注点的双重动态变化。

Method: 采用双动态更新机制跟踪游戏内容和玩家社区变化，使用双源合成引擎从官方来源和玩家社区获取信息，确保事实准确性和查询模式真实性。

Result: 在三个不同游戏上实例化了该框架，创建了游戏领域首个动态RAG基准测试，为模型在复杂现实条件下的性能评估提供了新见解。

Conclusion: ChronoPlay成功解决了游戏RAG系统评估的标准化问题，为动态领域的RAG性能评估提供了有效的自动化基准测试解决方案。
...
TL;DR: DePass是一个基于分解前向传播的统一特征归因框架，通过将隐藏状态分解为自定义加性分量并在固定注意力分数和MLP激活的情况下传播，实现忠实、细粒度的归因。
...
Method: DePass框架在单次分解前向传播中，将隐藏状态分解为定制化加性分量，并在固定注意力分数和MLP激活的情况下传播这些分量，无需辅助训练。

Result: 在token级、模型组件级和子空间级归因任务上的验证表明，DePass具有有效性和保真度，能够归因Transformer模型中任意组件间的信息流。

Conclusion: DePass有望成为可解释性领域更广泛应用的基础工具，为理解Transformer模型内部机制提供有力支持。
...
TL;DR: 本文提出了两个意大利语医学基准数据集IMB-QA和IMB-MCQA，研究了LLM在医学问答任务中的表现，发现领域专业知识和高效信息检索比模型规模更重要。
...
Method: 构建了两个意大利语医学基准数据集：IMB-QA（782,644个医患对话）和IMB-MCQA（25,862个多选题）。使用LLM改进医疗论坛数据的清晰度和一致性，比较了多种LLM架构在开放性和多选题任务中的表现，并实验了RAG和领域特定微调策略。

Result: 实验表明，专门的适应策略在医学问答任务中可以超越更大的通用模型。RAG和领域特定微调显示出优越性能。

Conclusion: 有效的医学AI系统可能更多受益于领域专业知识和高效信息检索，而非模型规模的增加。作者发布了数据集和评估框架以支持多语言医学问答研究。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案具有并行解码潜力，但实际应用中速度表现不佳，限制了其实际效用，需要系统研究其效率问题。

Method: 通过经验基准测试和基于roofline的理论分析，比较DLM与AR模型的吞吐量表现，并研究各种加速策略在不同批处理大小下的效果。

Result: 自回归模型通常获得更高的吞吐量，而DLM持续落后；双缓存和并行解码等加速技术主要在小批量时有效，随着规模扩大其优势减弱。

Conclusion: 需要改进评估方法和开发更有效的加速策略来推动DLM研究的进展。
...
TL;DR: 论文指出当前大语言模型存在文化推理能力不足的问题，默认采用西方文化视角，无法适应全球用户的文化多样性需求，需要将文化推理作为基础能力来发展。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在日常工具中广泛应用，但其回复往往反映单一文化视角，忽视了全球用户的文化多样性。这种文化推理能力的缺失可能导致刻板印象、忽视少数群体观点、削弱信任和传播仇恨。

Method: 通过定义文化推理能力（识别文化特定知识价值观和社会规范，并调整输出以符合用户期望），分析当前模型在道德判断、习语解释和提供建议时默认采用西方规范的问题，并指出现有评估方法的局限性。

Result: 研究表明当前模型在文化推理方面存在明显缺陷，即使基于调查数据进行微调也只能部分改善。现有评估方法主要报告静态准确率，无法捕捉上下文中的适应性推理。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域具有变革潜力，但需要深入了解训练数据特征如何影响模型行为（包括潜在偏见），以确保负责任和公平的发展。当前数据集管理和偏见评估实践缺乏透明度。

Method: 引入HC4医疗综合语料库（超过890亿标记），使用既有的通用基准和新颖的医疗特定方法进行深入评估。

Result: 提供了对临床AI应用中公平性和安全性的关键见解，重点关注阿片类药物处方在不同人口群体中的差异倾向。

Conclusion: 该研究为促进临床AI应用的公平性和安全性提供了重要支持，强调了全面评估框架的必要性。
...
TL;DR: 使用机器学习和自然语言处理技术自动检测灰姑娘变体中的主题模式，并通过聚类和降维分析其异同，展示了大型语言模型在民间故事分析中的应用。


<details>
  <summary>Details</summary>
Motivation: 将人工智能方法应用于数字人文学科，特别是民间故事研究，以支持大规模文本分析。

Method: 采用机器学习和自然语言处理技术自动检测主题模式，使用聚类和降维方法分析故事变体的相似性和差异性。

Result: 大型语言模型能够检测故事中的复杂交互模式，支持大规模文本集合的计算分析和跨语言比较。

Conclusion: 该方法为民间故事研究提供了有效的计算分析工具，促进了跨语言比较和大规模文本分析。
...
TL;DR: 该论文关注数字非人化问题，提出当前研究主要关注明显的负面言论，而忽视了更隐蔽的非人化形式。作者构建了一个理论指导的双语数据集，用于训练机器学习模型检测各种维度的非人化内容。


<details>
  <summary>Details</summary>
Motivation: 当前计算语言学和自然语言处理领域对数字非人化问题关注不足，现有研究主要集中于明显的负面言论，而忽略了更隐蔽但同样有害的非人化形式，这些形式会强化对边缘群体的偏见。

Method: 使用不同的采样方法从Twitter和Reddit收集理论指导的双语数据集，通过众包工作者和专家对16,000个实例进行文档级和跨度级标注，涵盖非人化的不同维度。

Result: 构建的数据集覆盖了非人化的不同维度，作为机器学习模型的训练资源和未来非人化检测技术的基准。微调的机器学习模型在零样本和少样本上下文设置中超越了最先进模型的性能。

Conclusion: 该研究填补了数字非人化检测领域的空白，提供了一个全面的数据集和有效的检测方法，能够识别更隐蔽的非人化形式，有助于减少在线互动中对边缘群体的有害偏见。
...
TL;DR: 本研究评估了从超声舌部运动学数据中可靠估计线性谐振振荡器参数的可能性，并与EMA数据进行比较，发现两者参数相当，支持使用超声运动学评估发音动力学模型。


<details>
  <summary>Details</summary>
Motivation: 传统上使用电磁关节造影术(EMA)等体表点数据来建模语音控制的动力学系统，但超声成像技术的最新进展使其成为有前景的替代方案。

Method: 比较从超声舌部运动学数据估计的线性谐振振荡器参数与同时记录的EMA数据估计的参数，并评估下颌短肌腱追踪对下颌运动的捕捉效果。

Result: 超声和EMA产生的动力学参数具有可比性，下颌短肌腱追踪也能充分捕捉下颌运动。

Conclusion: 研究结果支持使用超声运动学数据来评估发音动力学模型。
...
TL;DR: 本研究首次调查了LLM在长上下文医疗问答中的理解能力，通过多维度评估揭示了模型规模效应、局限性、记忆问题以及推理模型的优势，特别研究了RAG对医疗长上下文理解的影响。


<details>
  <summary>Details</summary>
Motivation: 首次系统研究LLM在长上下文医疗QA中的理解能力，填补了该领域的研究空白，旨在评估LLM在临床相关医疗问答中的实际表现。

Method: 采用综合评估方法，涵盖不同相关性设置、不同能力的LLM模型、多种任务形式的数据集，并特别研究了RAG在单文档与多文档推理数据集中的效果。

Result: 揭示了模型规模效应、局限性、底层记忆问题，展示了推理模型的优势，发现了RAG在医疗长上下文理解中的最佳设置和改进策略。

Conclusion: 通过多角度评估和错误分析，阐明了RAG相对于长上下文的优势场景，揭示了常见的失败案例，为医疗长上下文理解提供了重要见解。
...
TL;DR: 本文提出贝叶斯因子化适配器用于语音基础模型，通过在零附近设置先验实现稀疏适配矩阵，在适应特定领域的同时保留基础模型的通用能力。


<details>
  <summary>Details</summary>
Motivation: 大型语音基础模型在多个领域表现优异，但需要适应本地需求如语码转换（混合语言）。直接微调存在过拟合目标领域和覆盖基础模型广泛能力的风险。

Method: 探索贝叶斯因子化适配器，将先验置于零附近以获得稀疏适配矩阵，应用于Whisper模型并在多语言语码转换场景中评估。

Result: 结果显示仅产生最小适应损失，同时显著减少基础模型的灾难性遗忘。与LoRA相比，该方法在仅4%新领域性能下降的情况下实现54%的向后增益。

Conclusion: 贝叶斯适配方法在微调语音基础模型时能有效保持泛化能力，不牺牲通用性能。
...
TL;DR: 该论文提出了SemiAdapt和SemiLoRA两种半监督推理高效方法，用于改进低资源语言（如爱尔兰语）的神经机器翻译领域适应性能，能够匹配甚至超越全模型微调的效果。


<details>
  <summary>Details</summary>
Motivation: 针对大规模多语言模型微调计算成本高的问题，特别是对低资源语言领域（如爱尔兰语翻译）的研究者构成障碍，需要更高效的参数优化方法。

Method: 采用参数高效微调（PEFT）方法，特别是低秩适应（LoRA）技术，引入SemiAdapt和SemiLoRA半监督推理高效方法，通过训练少量参数来增强领域适应能力。

Result: SemiAdapt能够超越全领域微调，而SemiLoRA能够使PEFT方法达到或超越全模型微调的性能，特别是在更大和更嘈杂的语料库上表现优异。
...
TL;DR: 提出RLAAR框架，通过课程强化学习和可验证奖励机制，解决大语言模型在多轮对话中的性能衰减问题，提升模型判断问题可解性和平衡回答与放弃的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在单轮指令跟随中表现良好，但在多轮对话中随着信息逐步揭示会出现性能衰减（LiC问题），需要提升模型在多轮对话中的可靠性和可信度。

Method: 采用基于能力的课程学习策略，逐步增加对话难度，使用多轮在线策略rollouts和混合奖励系统，训练模型平衡问题解决与明智放弃。

Result: 在LiC基准测试中，RLAAR显著缓解了性能衰减（从62.6%提升到75.1%），并提高了校准放弃率（从33.5%提升到73.4%）。

Conclusion: RLAAR为构建可靠和可信赖的多轮对话大语言模型提供了实用方法，有效解决了Lost-in-Conversation问题。
...
TL;DR: 对186K篇美国报纸文章和45K篇专栏文章的审计发现，约9%的新发表文章部分或全部由AI生成，AI使用在小型地方媒体、特定主题和某些所有权集团中更常见，且AI使用很少被披露。
...
Method: 使用Pangram这一最先进的AI检测器，审计了来自1500家美国报纸在线版的186K篇文章和来自三大报纸的45K篇专栏文章，并对100篇AI标记文章进行了手动审计。

Result: 约9%的新发表文章部分或全部由AI生成；AI使用在小型地方媒体、天气和技术等特定主题以及某些所有权集团中更常见；专栏文章比新闻文章AI生成内容可能性高6.4倍；AI使用很少被披露（仅5%）。

Conclusion: 审计结果强调了迫切需要提高透明度并更新关于AI在新闻业使用的编辑标准，以维护公众信任。
...
TL;DR: 提出了一种用于工业资产健康的知识蒸馏框架，通过思维链蒸馏将大型语言模型的推理能力转移到更高效的小型模型上，显著提升了小型模型在工业领域的推理性能。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型在工业4.0等专业领域进行复杂推理仍然具有挑战性，需要提升其推理能力以支持准确的领域特定任务。

Method: 使用思维链蒸馏方法，通过多项选择题提示从大型语言模型向小型模型转移推理能力，并进行上下文学习来验证生成知识的质量。

Result: 经过思维链推理微调的小型模型性能显著优于基础模型，缩小了与大型语言模型对应版本的差距。

Conclusion: 知识蒸馏框架有效提升了小型语言模型在工业资产健康领域的推理能力，为专业领域提供了更高效、成本效益更好的解决方案。
...
TL;DR: MTraining是一种利用动态稀疏注意力实现超长上下文LLM高效训练的分布式方法，通过动态稀疏训练模式、平衡稀疏环形注意力和分层稀疏环形注意力三个组件，在32个A100 GPU集群上将Qwen2.5-3B的上下文窗口从32K扩展到512K，训练吞吐量提升6倍且保持模型精度。
...
Method: 提出MTraining方法，包含动态稀疏训练模式、平衡稀疏环形注意力和分层稀疏环形注意力三个关键组件，协同解决动态稀疏注意力机制中的计算不平衡和通信开销问题。

Result: 在32个A100 GPU集群上成功将Qwen2.5-3B的上下文窗口从32K扩展到512K，在RULER、PG-19、InfiniteBench和Needle In A Haystack等下游任务评估中，训练吞吐量提升6倍且模型精度保持不变。

Conclusion: MTraining为超长上下文LLM训练提供了一种高效的分布式解决方案，显著提升了训练效率同时保持了模型性能。
...
TL;DR: 提出Critique-Post-Edit框架，通过个性化生成奖励模型和基于批评的后编辑机制，解决LLM个性化中的奖励攻击问题，实现更忠实可控的个性化。


<details>
  <summary>Details</summary>
Motivation: 现有方法如SFT和标准RLHF在个性化任务中表现不佳，标量奖励模型容易受到奖励攻击，导致冗长和表面个性化的响应。

Method: 结合个性化生成奖励模型（提供多维评分和文本批评）和Critique-Post-Edit机制（策略模型根据批评修订输出），构建强化学习框架。

Result: 在长度控制评估中显著优于标准PPO，个性化Qwen2.5-7B平均胜率提升11%，Qwen2.5-14B超越GPT-4性能。

Conclusion: 该方法为忠实、高效和可控的个性化提供了实用路径。
...
TL;DR: LightMem是一个新的记忆系统，通过受人类记忆模型启发的三层架构（感觉记忆、短时记忆、长时记忆），在保持高性能的同时显著提升了效率，减少了token使用量、API调用次数和运行时间。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型记忆系统存在显著的时间和计算开销问题，需要一种能够在性能和效率之间取得平衡的记忆系统。

Method: 受Atkinson-Shiffrin人类记忆模型启发，LightMem将记忆组织为三个互补阶段：认知启发的感官记忆快速过滤无关信息，主题感知的短时记忆组织总结内容，以及具有睡眠时间更新的长时记忆通过离线程序解耦整合过程。

Result: 在LongMemEval上的实验显示，LightMem在准确性上比强基线提升高达10.9%，同时将token使用量减少高达117倍，API调用减少高达159倍，运行时间减少超过12倍。

Conclusion: LightMem在记忆系统性能和效率之间取得了良好平衡，为LLMs在动态复杂环境中有效利用历史交互信息提供了高效解决方案。
...
TL;DR: 本文提出了一个"猜测-精炼"框架来解释大语言模型如何内部结构化计算以进行预测，揭示了模型在不同层级的预测动态：早期层主要预测高频词作为统计猜测，随着上下文信息的发展，这些猜测在深层被精炼为上下文合适的词。


<details>
  <summary>Details</summary>
Motivation: 尽管有证据表明大语言模型不均匀地使用其深度，但我们仍然缺乏对其逐层预测动态的细粒度理解。本文旨在追踪几个开放权重模型在推理过程中的中间表示，揭示其深度使用的结构和细微差别。

Method: 提出"猜测-精炼"框架，通过追踪多个开放权重模型的中间表示来分析逐层预测动态。进行三个案例研究：词性分析、事实回忆任务分析和多项选择任务分析。

Result: 发现早期层的顶级预测主要由高频词组成，这些词由于缺乏适当上下文信息而作为统计猜测被提出。随着上下文信息在深层发展，这些初始猜测有超过70%的时间被精炼为上下文合适的词。词性分析显示功能词平均最早被正确预测；事实回忆任务中多词答案的第一个词需要更多计算深度；多项选择任务中模型在前半层识别响应格式，但在最后才最终确定响应。

Conclusion: 研究结果提供了LLM深度使用的详细视图，揭示了成功预测背后的逐层计算，为未来改进基于transformer模型的计算效率提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM微调后行为被锁定在源模型架构中的问题，现有权重空间迁移方法脆弱且间接，需要更直接的迁移范式。

Method: 学习源模型和目标模型激活流形间的非线性映射，将预训练LoRA作为冻结行为核，通过轻量级双向投影头在激活空间进行转换。

Result: 在Llama-2和Mistral等异构模型家族间迁移，CAST转换的适配器性能达到目标模型上重新训练LoRA的85-95%，优于现有权重空间迁移技术。
...
TL;DR: 提出了一种多样性感知检索框架，通过平衡意图覆盖和语言多样性来选择上下文示例，在固定token预算下显著提升了多轮意图理解的准确性。


<details>
  <summary>Details</summary>
Motivation: 现实部署中面临严格的token预算和噪声上下文，大多数检索管道强调相关性而忽略了集合级别的多样性以及更多上下文或示例顺序等混淆因素。

Method: 开发了多样性感知检索框架，选择上下文示例以平衡意图覆盖和语言多样性，并将此选择与标准LLM解码器集成；评估强制执行预算匹配的提示和随机化位置。
...
TL;DR: 本文提出了一种多智能体口头强化学习算法，通过动态构建和优化多智能体协作结构来提升复杂推理能力，在数学推理、创意写作等任务上显著优于单智能体和现有多智能体方法。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统要么采用预定义结构，要么依赖多数投票或圆桌辩论，这会抑制正确但非主导智能体的贡献。作者假设有效的智能体通信对多智能体推理至关重要，辩论质量起着重要作用。

Method: 提出多智能体口头强化学习算法，定义动作空间和反馈机制，评估辩论过程中的通信鲁棒性和连贯性，最终通过所有智能体的多数投票做出决策。

Result: 在数学推理、创意写作、科学推理和数值排序等任务上的评估表明，该方法显著优于单智能体提示方法和最先进的多智能体框架。

Conclusion: 动态构建和优化多智能体协作结构能够有效提升复杂推理能力，验证了智能体通信质量和辩论过程对多智能体系统性能的重要性。
...
Method: 基于九个公理（A1-A9）的形式化方法，包括历史单调性、因果无环性和可追溯性。采用模型驱动方法进行事件验证、参与者授权和因果链自动构建。

Result: 开发了boldsea系统作为可执行本体论的工作流引擎，理论构造在BSL语言中实现，证明了方法的实际适用性。

Conclusion: 该形式化方法为分布式系统、微服务架构和DLT平台提供了有效的建模框架，特别适用于处理多视角场景中的冲突事实。
...
TL;DR: 提出了一种名为planned diffusion的混合方法，结合了自回归模型和扩散模型的优势，通过两阶段生成（先创建自回归计划，再用扩散模型并行生成）来优化文本生成的速度-质量权衡。
...
Method: 两阶段方法：第一阶段使用自回归模型创建短计划，将输出分解为更小、独立的片段；第二阶段使用扩散模型同时生成这些片段。

Result: 在AlpacaEval测试中，实现了速度-质量的Pareto最优权衡，相比自回归生成获得1.27x到1.81x的加速，仅损失0.87%到5.4%的胜率。

Conclusion: planned diffusion方法扩展了速度-质量的Pareto前沿，为更快、高质量的文本生成提供了实用路径，其规划机制简洁可靠，且可通过简单运行时参数灵活控制质量-延迟权衡。
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型在部署成本和延迟方面具有优势，但在复杂领域特定任务上的准确性往往落后于大型模型。虽然有监督微调可以弥补这一差距，但需要大量手动数据准备和迭代优化工作。
...
Conclusion: PaDA-Agent提供了一种有效的方法来简化小语言模型的数据增强过程，通过协调操作显著提升模型性能，解决了传统方法需要大量手动工作的问题。
...
TL;DR: 提出了一个句子级标注数据集，用于在LLM推理过程中基于激活的安全行为监测，填补了现有数据集仅整体标注推理的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本推理步骤的安全监测方法可能遗漏细微的有害模式，且可能被隐藏不安全推理的模型规避，需要更精细的激活层面监测技术。

Method: 构建包含推理序列和句子级安全行为标注的数据集，提取用于检测和影响这些行为的转向向量，在模型激活层面进行安全行为监测。

Result: 成功提取了能够检测和引导安全行为的表示，展示了激活层面技术在改进推理安全监督方面的潜力。

Conclusion: 激活层面的监测技术能够更精确地识别推理链中特定安全行为的发生时机，为AI安全监督提供了更有效的方法。
...
TL;DR: 提出基于大语言模型的多智能体模拟框架，用于模拟消费者决策和社会动态，在价格折扣营销场景中提供可操作的策略测试结果。


<details>
  <summary>Details</summary>
Motivation: 传统的事后分析和基于规则的智能体模型难以捕捉人类行为和社会互动的复杂性，需要一种更准确的方法来在真实部署前设计和评估营销策略。

Method: 构建基于大语言模型的多智能体模拟框架，让生成智能体能够互动、表达内部推理、形成习惯并做出购买决策，无需预定义规则。

Result: 在价格折扣营销场景中，该系统提供了可操作的策略测试结果，并揭示了传统方法无法发现的新兴社会模式。
...
TL;DR: 提出Saber算法，一种无需训练的新型采样方法，用于提升扩散语言模型在代码生成任务中的推理速度和输出质量，通过自适应加速和回溯机制解决性能与速度的权衡问题。
...
Method: 提出Saber算法，包含两个关键机制：1）随着代码上下文建立而自适应加速采样过程；2）引入回溯机制来反转生成的token。
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏对人工通用智能（AGI）的具体定义，当前专业AI与人类水平认知之间的差距难以衡量。本文旨在通过量化框架来明确这一差距。

Method: 基于Cattell-Horn-Carroll理论（最经验验证的人类认知模型），将通用智能分解为10个核心认知领域（包括推理、记忆、感知等），并采用成熟的人类心理测量工具来评估AI系统。
...
TL;DR: 提出ssToken方法，通过自调制和语义感知的token选择来解决现有token级数据选择方法的局限性，无需额外参考模型，结合损失差异和语义重要性进行更有效的token筛选。


<details>
  <summary>Details</summary>
Motivation: 现有token级选择方法需要训练或访问额外参考模型，且仅依赖损失信息无法很好保留语义重要的token，限制了数据选择的效果。

Method: ssToken利用历史模型计算当前模型的token损失差异作为自调制信号，并引入基于注意力的语义感知token重要性估计指标，将两种正交的选择策略结合。

Result: 实验表明自调制选择和语义感知选择单独使用均优于全数据微调，两者结合的ssToken实现协同增益，超越现有token级选择方法，在保持训练效率的同时提升性能。

Conclusion: ssToken通过自调制和语义感知的token选择策略，有效解决了现有方法的局限性，为LLM的监督微调提供了更优的数据质量提升方案。
...
TL;DR: 本文测试了8个前沿大语言模型在开放式但规则约束的任务中的自我反思能力，发现模型反思后性能提升有限，且经常重复相同的约束违反，表明当前LLM的'反思'缺乏人类式的主动目标驱动监控机制。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大语言模型的'反思'功能是否与人类的反思推理功能等价。现有研究多在封闭式任务中测试，可能掩盖了自我纠正的局限性，因此需要在开放式但规则约束的真实任务中评估。

Method: 在开放式但规则约束的科学测试项目生成任务中测试8个前沿模型，要求模型首先生成有效项目，然后基于自身批评进行修订，评估反思前后的性能变化。

Result: 首轮性能很差（平均约1个有效项目，常为0），反思后仅带来适度提升（也约1个）。关键发现是第二轮尝试经常重复相同的约束违反，表明'纠正收益'主要来自偶然生成有效项目而非错误检测和原则性修复。

Conclusion: 当前LLM的'反思'缺乏功能性证据证明存在主动、目标驱动的监控机制，这种机制能帮助人类在首轮就尊重约束。在模型本身实例化此类机制前，可靠性能需要外部结构来强制执行约束。
...
<details>
  <summary>Details</summary>
Motivation: 多类无监督异常检测需要统一模型检测多个类别的异常，但现有Transformer架构存在身份捷径问题，即直接复制输入到输出，导致正常与异常样本的重建误差差异减小，难以区分。

Method: 提出ShortcutBreaker框架：1) 基于矩阵秩不等式设计低秩噪声瓶颈，将高维特征投影到低秩潜在空间防止平凡身份复制；2) 利用ViT全局建模能力引入全局扰动注意力防止解码器中的信息捷径。

Result: 在四个基准数据集（MVTec-AD、ViSA、Real-IAD和Universal Medical）上分别达到99.8%、98.9%、90.6%和87.8%的图像级AUROC，优于现有MUAD方法。

Conclusion: ShortcutBreaker通过解决身份捷径问题，在多类无监督异常检测任务中实现了显著性能提升，在不同场景下均优于现有方法。
...
### [150] [Automated urban waterlogging assessment and early warning through a mixture of foundation models](https://arxiv.org/abs/2510.18425)
...
Method: 采用半监督微调策略和思维链提示策略，释放基础模型在数据稀缺下游任务中的潜力。
...
### [151] [AlphaOPT: Formulating Optimization Programs with Self-Improving LLM Experience Library](https://arxiv.org/abs/2510.18428)
...
TL;DR: AlphaOPT是一个自改进的经验库系统，使LLM能够从有限的演示和求解器反馈中学习优化建模，无需标注推理轨迹或参数更新。它通过两阶段循环持续改进：库学习和库演化。


<details>
  <summary>Details</summary>
Motivation: 优化建模在各行业决策中至关重要，但难以自动化。现有LLM方法要么依赖脆弱的提示，要么需要昂贵的重新训练且泛化能力有限。

Method: 采用持续两阶段循环：1) 库学习阶段从失败尝试中反思，提取求解器验证的结构化见解；2) 库演化阶段诊断检索偏差，改进存储见解的适用条件。

Result: 实验显示AlphaOPT随数据增加稳步提升（从100到300个训练项，准确率从65%升至72%），在仅使用答案训练时，在分布外OptiBench数据集上比最强基线高出7.7%。

Conclusion: AlphaOPT能够从有限演示中高效学习，通过更新库而非模型权重实现持续扩展，使知识显式化且可解释，支持人工检查和干预。
...
### [152] [PlanU: Large Language Model Decision Making through Planning under Uncertainty](https://arxiv.org/abs/2510.18442)
*Ziwei Deng,Mian Deng,Chenjing Liang,Zeming Gao,Chennan Ma,Chenxing Lin,Haipeng Zhang,Songzhu Mei,Cheng Wang,Siqi Shen*

Main category: cs.AI

TL;DR: 本文提出PlanU方法，通过将蒙特卡洛树搜索中的节点回报建模为分位数分布，并引入带有好奇心的上置信界限分数来平衡探索与利用，以解决LLM在不确定性环境下的决策问题。


<details>
  <summary>Details</summary>
Motivation: LLM在不确定性环境中的决策能力不足，特别是面对环境随机性时表现不佳。现有方法主要关注LLM自身不确定性，而忽略了环境不确定性，导致在随机状态转移环境中性能较差。

Method: 提出PlanU方法，在蒙特卡洛树搜索中捕获不确定性，将每个节点的回报建模为分位数分布，并引入UCC分数来估计MCTS节点的不确定性，从而平衡探索与利用。

Result: 通过大量实验验证了PlanU在LLM不确定性决策任务中的有效性。

Conclusion: PlanU方法能够有效处理LLM决策中的不确定性，特别是在随机环境中的多步决策任务中表现出色。
...
### [153] [CircuitSeer: Mining High-Quality Data by Probing Mathematical Reasoning Circuits in LLMs](https://arxiv.org/abs/2510.18470)
*Shaobo Wang,Yongliang Miao,Yuancheng Liu,and Qianli Ma,Ning Liao,Linfeng Zhang*

Main category: cs.AI

TL;DR: CircuitSeer是一种基于模型内部注意力机制的数据选择方法，通过识别推理任务中激活的稀疏注意力头来量化数据复杂性，仅使用10%的数据就能在数学推理任务上获得比全数据集训练更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法依赖昂贵的外部模型或不透明的启发式方法，而作者希望利用模型内部机制来更有效地选择高质量训练数据。

Method: 发现复杂推理任务会激活稀疏的专用注意力头，形成核心推理电路，基于此提出CircuitSeer方法，通过测量数据对这些关键电路的影响来量化推理复杂性。

Result: 在4个模型和9个数据集上的实验表明，CircuitSeer方法仅使用10%的数据就能让Qwen2.5-Math-7B模型在平均Pass@1指标上比全数据集训练提升1.4个百分点。

Conclusion: 基于模型内部电路的数据选择方法CircuitSeer在效率和效果上都优于传统方法，为大规模语言模型的高效训练提供了新思路。
...
### [154] [Counterfactual Reasoning for Steerable Pluralistic Value Alignment of Large Language Models](https://arxiv.org/abs/2510.18526)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在不同文化背景中的应用增多，需要超越平均原则（如HHH）与多元人类价值观对齐。现有方法面临价值观复杂性和可操控性两大挑战。

Method: 提出COUPLE框架，使用结构因果模型(SCM)建模价值观间的复杂依赖关系和优先级，并应用反事实推理生成符合特定价值目标的输出。

Result: 在两个具有不同价值体系的数据集上评估，COUPLE在多种价值目标类型上都优于其他基线方法。
...
### [155] [SOCIA-Nabla: Textual Gradient Meets Multi-Agent Orchestration for Automated Simulator Generation](https://arxiv.org/abs/2510.18551)
*Yuncheng Hua,Sion Weatherhead,Mehdi Jafari,Hao Xue,Flora D. Salim*

Main category: cs.AI

TL;DR: SOCIA-Nabla是一个端到端的智能体框架，将模拟器构建视为代码实例优化，通过文本计算图中的LLM驱动智能体和文本梯度下降方法实现自动化的代码合成、执行、评估和修复循环。


<details>
  <summary>Details</summary>
Motivation: 将脆弱的提示管道转换为可复现、约束感知的模拟器代码生成，实现跨领域和模拟粒度的扩展，同时最小化专家工作量。

Method: 在文本计算图中嵌入专门的LLM驱动智能体作为节点，工作流管理器执行损失驱动的循环：代码合成→执行→评估→代码修复，采用文本梯度下降优化，保留人机交互用于任务规范确认。

Result: 在三个CPS任务（用户建模、口罩采用和个人移动性）中实现了最先进的整体准确率。

Conclusion: 通过将多智能体编排与损失对齐的优化视图统一，SOCIA-Nabla能够生成可复现、约束感知的模拟器代码，并跨领域和模拟粒度扩展。
...
### [156] [Extracting alignment data in open models](https://arxiv.org/abs/2510.18554)
...
TL;DR: 研究表明可以从经过后训练的模型中提取大量对齐训练数据，这些数据可用于提升模型的长上下文推理、安全性、指令遵循和数学能力。通过高质量的嵌入模型来衡量语义相似性比传统的字符串匹配方法更有效，能够发现更多可提取的数据。


<details>
  <summary>Details</summary>
Motivation: 揭示从后训练模型中提取对齐训练数据的可能性，以及这种数据提取可能带来的风险，特别是关于模型蒸馏实践的下游影响。

Method: 使用高质量的嵌入模型来衡量字符串之间的语义相似性，而不是依赖传统的近似字符串匹配方法。通过这种方法从经过SFT或RL等后训练阶段的模型中提取训练数据。

Result: 研究发现模型容易重现后训练阶段使用的训练数据，这些提取的数据可以用来训练基础模型，恢复相当程度的原始性能。嵌入模型方法比字符串匹配方法多发现了约10倍的可提取数据。

Conclusion: 这项工作暴露了提取对齐数据的潜在风险，并引发了关于蒸馏实践下游影响的讨论：由于模型似乎会重现其训练集的各个方面，蒸馏可以被视为间接地在模型的原始数据集上进行训练。
...
### [157] [QuantEvolve: Automating Quantitative Strategy Discovery through Multi-Agent Evolutionary Framework](https://arxiv.org/abs/2510.18569)
*Junhyeog Yun,Hyoun Jun Lee,Insu Jeon*

Main category: cs.AI

TL;DR: QuantEvolve是一个结合质量多样性优化和假设驱动策略生成的进化框架，用于自动化开发量化交易策略，能够在动态市场中生成多样化且适应性强的策略。
...
Method: 采用进化框架，结合质量多样性优化和假设驱动的多智能体系统，通过特征映射（策略类型、风险配置、换手率、收益特征）维护多样化策略集，并迭代生成和评估策略。

Result: 实证结果显示QuantEvolve优于传统基线方法，能够生成适应市场状态变化和个体投资需求的多样化复杂策略。
...
### [158] [VAR: Visual Attention Reasoning via Structured Search and Backtracking](https://arxiv.org/abs/2510.18619)
*Wei Cai,Jian Zhao,Yuchen Yuan,Tianle Zhang,Ming Zhu,Haichuan Tang,Chi Zhang,Xuelong Li*

Main category: cs.AI

TL;DR: VAR框架通过结构化搜索解决MLLMs的幻觉问题，将推理分解为可追踪证据定位和基于搜索的思维链生成，引入回溯机制进行自我修正。
...
Method: 将基础推理重构为在推理轨迹空间上的结构化搜索，包含可追踪证据定位和基于搜索的思维链生成两个阶段，采用多面奖励函数进行语义和几何自验证。

Result: VAR-7B模型在幻觉和安全基准测试中创下新记录，显著优于现有开源模型，与领先专有系统表现相当。

Conclusion: VAR框架有效解决了MLLMs的幻觉问题，通过结构化搜索和回溯机制提升了推理的可靠性和准确性。
...
### [159] [Leveraging Association Rules for Better Predictions and Better Explanations](https://arxiv.org/abs/2510.18628)
*Gilles Audemard,Sylvie Coste-Marquis,Pierre Marquis,Mehdi Sabiri,Nicolas Szczepanski*

Main category: cs.AI

TL;DR: 提出了一种结合数据和知识的分类方法，利用数据挖掘从数据中推导关联规则，提升树基模型的预测性能并改进解释任务。


<details>
  <summary>Details</summary>
Motivation: 结合数据和知识来提高分类任务的预测性能和解释能力，通过关联规则增强树基模型的表现。

Method: 使用数据挖掘从数据中推导关联规则（可能包含否定），将这些规则应用于决策树和随机森林等树基模型，用于提升分类预测性能和生成更通用的溯因解释。

Result: 实验表明，该方法在预测性能和解释大小方面都能为树基模型带来益处。

Conclusion: 提出的结合数据和知识的方法能有效提升树基分类模型的预测性能和解释质量。
...
### [160] [Comparative Expressivity for Structured Argumentation Frameworks with Uncertain Rules and Premises](https://arxiv.org/abs/2510.18631)
*Carlo Proietti,Antonio Yuste-Ginel*

Main category: cs.AI

TL;DR: 本文研究了形式论证中定性不确定性的建模，比较了抽象模型和结构化模型在表达能力上的差异，提出了新的表达能力概念，并给出了正负两方面的表达能力结果。


<details>
  <summary>Details</summary>
Motivation: 形式论证中的定性不确定性建模对于实际应用和理论理解都至关重要，但现有工作大多关注抽象模型。本文旨在研究这些抽象模型的合理实例化，将论证的不确定性基于其组成部分（规则和前提）进行结构化处理。

Method: 通过将论证的不确定性基于其组成部分（规则和前提）进行结构化处理，引入了一个能够处理抽象和结构化形式体系的表达能力概念，比较了抽象和结构化论证模型在不确定性方面的表达能力。

Result: 提出了负向和正向的表达能力结果，比较了抽象模型（包括不完整抽象论证框架及其依赖扩展）和结构化模型（ASPIC+）在表达能力上的差异。

Conclusion: 研究结果表明抽象模型和结构化模型在表达能力上存在差异，这些结果对不完整抽象论证框架及其依赖扩展以及ASPIC+等模型具有重要意义。
...
### [161] [Query Decomposition for RAG: Balancing Exploration-Exploitation](https://arxiv.org/abs/2510.18633)
*Roxana Petcu,Kenton Murray,Daniel Khashabi,Evangelos Kanoulas,Maarten de Rijke,Dawn Lawrie,Kevin Duh*

Main category: cs.AI

TL;DR: 该论文提出将检索增强生成系统中的查询分解和文档检索建模为探索-利用权衡问题，使用多臂老虎机方法动态选择信息量最大的子查询，通过估计文档相关性实现了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中在检索文档时需要平衡的两个关键问题：(i) 检索足够广泛以捕获所有相关材料，(ii) 限制检索以避免过度噪声和计算成本。

Method: 将查询分解和文档检索建模为探索-利用设置，使用多臂老虎机学习方法动态选择最信息丰富的子查询，利用排序信息和人工判断来估计文档相关性。

Result: 使用排序信息和人工判断估计文档相关性带来了35%的文档级精度提升，15%的α-nDCG增长，并在长文本生成的下游任务中表现更好。

Conclusion: 将RAG系统中的查询分解和文档检索建模为探索-利用问题，并使用多臂老虎机方法能够有效平衡检索的广度和精度，显著提升系统性能。
...
### [162] [Seg the HAB: Language-Guided Geospatial Algae Bloom Reasoning and Segmentation](https://arxiv.org/abs/2510.18751)
*Patterson Hsieh,Jerry Yeh,Mao-Chi He,Wen-Han Hsieh,Elvis Hsieh*

Main category: cs.AI

TL;DR: ALGOS是一个用于有害藻华监测的分割和推理系统，结合遥感图像理解和严重程度估计，通过GeoSAM辅助人工评估和微调视觉语言模型，在分割和严重程度估计方面表现稳健。
...
### [163] [Decoding Funded Research: Comparative Analysis of Topic Models and Uncovering the Effect of Gender and Geographic Location](https://arxiv.org/abs/2510.18803)
*Shirin Tavakoli Kafiabad,Andrea Schiffauerova,Ashkan Ebadi*

Main category: cs.AI

TL;DR: 本研究分析了加拿大自然科学与工程研究委员会18年的资助提案，比较了LDA、STM和BERTopic三种主题建模方法，并开发了COFFEE算法来增强BERTopic的协变量分析能力。


<details>
  <summary>Details</summary>
Motivation: 优化国家科学投资需要了解研究趋势变化及人口地理因素的影响，特别是在公平、多样性和包容性承诺背景下。

Method: 使用LDA、STM和BERTopic三种主题建模方法分析NSERC 2005-2022年的资助提案，并开发COFFEE算法为BERTopic提供协变量效应估计功能。
