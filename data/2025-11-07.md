...
TL;DR: 评估地理空间嵌入在Togo农田制图中的效用，使用Presto和AlphaEarth嵌入生成农田地图，证明该方法能简化工作流程并实现高精度分类。


<details>
  <summary>Details</summary>
Motivation: 准确及时的土地覆盖地图对于理解土地利用变化（气候变化的关键驱动因素）至关重要。地理空间嵌入提供了更高效和可访问的景观特征制图方式，但在实际制图应用中的使用仍待探索。

Method: 使用Presto和AlphaEarth的地理空间嵌入生成Togo的农田地图，评估这些嵌入在农田分类中的效用。

Result: 地理空间嵌入能够简化工作流程，实现高精度的农田分类，支持更好地评估土地利用变化及其气候影响。

Conclusion: 地理空间嵌入为农田制图提供了高效且准确的解决方案，有助于更好地理解土地利用变化对气候的影响。
...
TL;DR: 提出了一种名为“生成提示”的训练方法，通过生成模型创建虚拟样本来直接在整个输入空间中强制执行已知的不变性，相比传统数据增强方法在多个数据集上取得了更好的性能。
...
Method: 使用在训练集上训练的生成模型来近似输入分布并生成未标记的虚拟图像，然后在半监督方式下同时训练分类和提示目标，利用未标记的虚拟样本来引导模型学习所需的提示（不变性）。

Result: 在多个数据集、架构和损失函数上，生成提示方法在学习相同属性时始终优于标准数据增强。在细粒度视觉分类基准上，相比使用数据增强的微调模型，获得了最高1.78%的top-1准确率提升（平均0.63%），在CheXpert X-ray数据集上平均性能提升1.286%。

Conclusion: 生成提示方法能够有效学习不变性属性，在多个视觉任务中显著优于传统数据增强方法，为模型训练提供了更有效的不变性学习框架。
...
TL;DR: ProM3E是一个概率掩码多模态嵌入模型，用于生态学领域的任意到任意多模态表示生成。它通过嵌入空间的掩码模态重建来学习从少量上下文模态推断缺失模态，支持嵌入空间的模态反转，并能分析不同模态融合的可行性。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理生态学中多种模态数据（如图像、声音、文本等）的统一模型，解决多模态表示学习和跨模态检索的挑战。

Method: 基于掩码模态重建的嵌入空间学习，通过概率模型推断缺失模态，支持模态反转，并提出混合跨模态和同模态相似度的新型跨模态检索方法。

Result: 模型在所有检索任务中表现出优越性能，通过线性探测任务展示了其出色的表示学习能力。

Conclusion: ProM3E为生态学多模态数据提供了一个有效的统一表示学习框架，支持任意模态间的生成和检索，代码和数据集将公开发布。
...
TL;DR: EvtSlowTV是一个从YouTube视频构建的大规模事件相机数据集，包含超过130亿个事件，用于解决事件相机深度估计中数据标注不足的问题。该数据集支持自监督学习框架，能够提升模型在复杂场景和运动中的泛化能力。
...
Method: 从YouTube公开视频构建EvtSlowTV大规模事件相机数据集，包含各种环境条件和运动类型。采用自监督学习框架，利用原始事件流的高动态范围潜力，无需基于帧的标注。

Result: EvtSlowTV比现有事件数据集大一个数量级，提供了无约束的自然场景设置。训练使用该数据集能增强模型在复杂场景和运动中的泛化能力。

Conclusion: EvtSlowTV数据集解决了事件相机深度估计中的数据稀缺问题，支持自监督学习，保持了事件数据的异步特性，为事件相机的深度估计提供了更好的训练基础。
...
TL;DR: 本文提出了一种新的混合CNN-ViT神经架构搜索空间，用于在模型大小限制下寻找高效的图像分类架构，在CIFAR10数据集上表现出优于ResNet-based tinyML模型的准确性和推理速度。
...
Method: 构建了一个新的混合CNN-ViT神经架构搜索空间，包含混合CNN和ViT块以学习局部和全局信息，以及新颖的可搜索池化层池化块用于高效特征图缩减。

Result: 在CIFAR10数据集上的实验结果表明，所提出的搜索空间能够产生在严格模型大小约束下具有优越准确性和推理速度的混合CNN-ViT架构，优于基于ResNet的tinyML模型。

Conclusion: 提出的混合CNN-ViT搜索空间能够有效找到适合tinyML部署的高效架构，在保持高性能的同时满足严格的模型大小限制。
...
TL;DR: SCALE-VLP是一个软加权对比视觉语言预训练框架，专门针对CT等体积医学数据设计，整合体积空间语义和领域知识语义，在有限监督下实现结构一致和语义基础的表示。
...
Result: 相比先前最优方法，SCALE-VLP实现高达4.3倍的top-1 CT报告检索提升，异常分类提高10个百分点，报告生成达到ROUGE-L 0.44和BERT-F1 0.89。在零样本外部数据集评估中观察到一致增益。
...
TL;DR: 提出了一种基于自监督学习的标签高效方法，仅使用1000个标注图像块实现全州范围的1米分辨率土地覆盖分类，通过BYOL预训练策略在大量未标记图像上训练ResNet-101编码器，然后微调多个语义分割架构，在密西西比州实现了87.14%的总体精度。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率土地覆盖分类中大规模代表性训练数据收集困难的问题，降低对大量人工标注数据的依赖，推动米级土地覆盖制图在大范围应用中的普及。

Method: 使用BYOL自监督预训练策略在377,921个未标记彩色红外航空图像块上预训练ResNet-101编码器，然后将学习到的编码器权重迁移到多个深度语义分割架构中，使用非常小的训练数据集进行微调。

Result: 使用表现最佳的U-Net模型集成实现了87.14%的总体精度和75.58%的宏F1分数，完成了密西西比州超过1230亿像素的8类土地覆盖制图，在水体和森林区域表现出准确映射，但在农田、草本和裸地类型区分方面存在挑战。

Conclusion: 自监督学习是减少对大量人工标注数据需求的有效策略，直接解决了高空间分辨率土地覆盖制图规模化应用的主要限制。
...
<details>
  <summary>Details</summary>
Motivation: 解决脑部MRI中不同成像序列组合的挑战，避免为每种模态组合训练单独模型，提高模型在模态缺失或未见情况下的适应性。

Method: 使用单一编码器架构，结合可学习模态嵌入、条件层归一化、掩码自编码目标和方差-协方差正则化器，在约60,000个多中心MRI上进行自监督重建和模态插补训练。

Result: 初步结果表明该方法可行，计划在脑肿瘤和多发性硬化症分割以及病变分类任务上进行更详细的评估。

Conclusion: 该模型能够灵活处理不同MRI序列组合，无需为每种模态组合训练单独模型，为脑部MRI分析提供了统一的基础模型解决方案。
...
TL;DR: SLIP提出了一种结构感知的语言-图像预训练方法，通过引入结构对比损失来建模结构化图中的实体关系，在跨模态检索和分类任务中优于CLIP。


<details>
  <summary>Details</summary>
Motivation: 现有VLP方法将图像-文本对视为孤立训练样本，忽略了现实领域中丰富的图结构关系，如电商产品共购图。受人类知识编码为关系认知图的神经科学证据启发，需要建模结构化关系。

Method: 引入结构感知语言-图像预训练(SLIP)，集成结构对比损失来对齐模态并建模结构化图中相邻实体间的关系，构建了大规模亚马逊产品共购多模态图数据集。

Result: 实验结果显示SLIP在零样本和少样本设置下，在跨模态检索和分类任务中持续优于CLIP，证明了关系监督对跨模态对齐的价值。

Conclusion: 关系监督对于跨模态对齐具有重要价值，SLIP通过建模结构化关系提升了VLP性能。
...
Result: 生成10,000张图像数据集的人工时间仅为30.5小时，相比手动标注的166.7小时减少了81.7%。在ISC组件检测任务上，使用ISC-Perception训练的检测器达到了0.756的mAP@0.50，显著优于仅使用合成数据或照片级真实数据的模型。

Conclusion: ISC-Perception通过弥合建筑机器人感知的数据缺口，促进了定制物体检测器的快速开发，可供研究和工业使用。
...
TL;DR: DentalSplat是一个用于从稀疏正畸图像进行3D重建的有效框架，通过先验引导的密集立体重建模型初始化点云，结合尺度自适应剪枝策略和光学流几何约束，在仅有三个稀疏输入视图的情况下实现高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 在正畸治疗中，从多个视角观察患者的牙齿咬合有助于及时临床决策。传统3D高斯泼溅方法需要密集多视图输入和精确相机位姿，而正畸病例通常只有三个稀疏图像，导致重建质量严重下降。

Method: 使用先验引导的密集立体重建模型初始化点云，采用尺度自适应剪枝策略提高训练效率和重建质量，在极稀疏视角下加入光学流作为几何约束和梯度正则化来增强渲染保真度。
...
<details>
  <summary>Details</summary>
Motivation: 集成电路制造过程复杂，缺陷会导致良率损失和可靠性下降。监督方法需要大量人工标注且难以处理新类别和罕见缺陷，基于聚类的方法因缺乏先验而性能不稳定。

Method: 1) 自归一化信息引导的缺陷检测：通过可学习的正常信息提取器聚合代表性正常特征，使用重建残差粗略定位缺陷区域；2) 自适应二值化策略处理缺陷显著性变化；3) 自缺陷信息引导的缺陷分类：结合软掩码引导注意力机制将空间缺陷先验注入师生模型。

Result: 在涵盖三个关键制造阶段和15种缺陷类型的真实数据集上验证，实验证明在缺陷检测和未见缺陷分类方面均表现出稳健性能。
...
TL;DR: \sysname系统通过算法和系统优化加速视觉引导的物理属性推理，将端到端延迟从10-20分钟降至6秒以内，在ABO数据集上实现62.9-287.2倍加速，同时保持或提升物理属性估计精度。


<details>
  <summary>Details</summary>
Motivation: 解决视觉引导物理属性推理中运行延迟过高的问题，实现增强视觉认知能力，特别是在智能眼镜等实时应用场景中。

Method: 结合快速几何3D重建、高效语义特征融合和并行视图编码等算法与系统优化，最小化推理管道的运行延迟。

Result: 端到端延迟从10-20分钟降至6秒以内，在ABO数据集上实现62.9-287.2倍加速，物理属性估计精度与SOTA基线相当或略优，材料分割和体素级推理性能更优。

Conclusion: \sysname系统显著加速了物理属性推理，在真实世界场景中表现稳健，结合眼动追踪可在杂乱环境中定位感兴趣对象，为智能眼镜应用提供高效解决方案。
...
TL;DR: 本文开发并部署了首个用于无人机系统图像中建筑物损坏评估的AI/ML系统，在飓风Debby和Helene期间实际运行，解决了灾害响应中图像数据过载的问题。


<details>
  <summary>Details</summary>
Motivation: 灾害响应中无人机团队每天收集大量图像（47GB-369GB），远超专家处理能力，导致响应延迟，需要计算机视觉技术来缓解数据过载问题。

Method: 使用包含21,716个建筑物损坏标签的最大已知灾后无人机图像数据集进行模型训练，并对91名灾害从业人员进行操作培训，开发建筑物损坏评估模型。

Result: 最佳模型在飓风Debby和Helene响应期间部署，在约18分钟内评估了415栋建筑物，实现了实际灾害中的AI/ML应用。

Conclusion: 这项工作建立了无人机图像建筑物损坏评估的实践标准，为AI/ML研究和用户社区提供了实际灾害中AI/ML应用的文档和经验教训。
...
TL;DR: 提出了一种深度引导的肝脏标志点分割框架，通过结合RGB和深度特征，在腹腔镜肝脏手术中实现精确的解剖结构分割。


<details>
  <summary>Details</summary>
Motivation: 在腹腔镜肝脏手术中，2D视频流限制了深度感知，使标志点定位变得复杂。现有方法在融合RGB和深度特征以及高效适配大规模视觉模型方面仍面临挑战。

Method: 使用SAM2编码器提取RGB特征，DA2编码器提取深度感知特征。提出SRFT-GaLore低秩梯度投影方法，用子采样随机傅里叶变换替代计算昂贵的SVD，实现高效微调。通过交叉注意力融合模块整合RGB和深度线索。

Result: 在L3D数据集上，Dice相似系数提升4.85%，平均对称表面距离降低11.78点。在新建的LLSD数据集上保持竞争力，显著优于基于SAM的基线方法。

Conclusion: SRFT-GaLore增强的双编码器框架能够在实时、深度受限的手术环境下实现可扩展且精确的分割。
...
TL;DR: 该论文提出了PitVQA-Anticipation数据集和SurgAnt-ViVQA模型，专注于内窥镜经鼻蝶垂体手术中的前瞻性视觉问答任务，通过时间建模实现手术步骤预测。


<details>
  <summary>Details</summary>
Motivation: 现有手术VQA系统主要关注当前场景的静态分析，缺乏对未来手术事件和器械需求的预测能力，这在视野受限、工作流快速变化的内窥镜手术中至关重要。

Method: 提出SurgAnt-ViVQA视频语言模型，使用GRU门控时间交叉注意力模块编码帧间动态，通过参数高效微调将大语言模型适配到手术领域，支持四个预测任务。

Result: 在PitVQA-Anticipation和EndoVis数据集上测试，SurgAnt-ViVQA超越了基于图像和视频的基线方法，消融实验显示时间循环和门控融合是性能提升的关键因素。

Conclusion: 通过时间感知编码器和细粒度门控交叉注意力的结合，SurgAnt-ViVQA将手术VQA从回顾性描述推进到前瞻性预测，为可靠的未来感知手术辅助系统奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 公开的大规模医学影像数据集对于开发和验证人工智能模型以及进行回顾性临床研究至关重要，但结合功能和解剖成像与详细临床报告的多癌症类型数据集仍然稀缺。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在多图像环境中存在两个关键问题：缺乏跨不同图像的细粒度感知能力，以及在多视觉输入上进行有效推理和信息合成的能力下降。现有研究主要关注单图像设置或特定受限场景，缺乏对复杂多图像推理任务的深入理解。

Method: 提出QG-CoC（问题引导的标题链）方法，这是一种通用的零样本提示方法，能够有效处理任意数量图像的问题。该方法通过问题引导的方式生成图像标题链，实现感知与推理的无缝集成。

Result: 在各种开源和闭源多模态大语言模型上对多图像和单图像基准进行评估，实验结果表明QG-CoC在不同任务中表现出竞争力，并在现有提示方法失败的挑战性场景中展现出稳健的改进。

Conclusion: QG-CoC方法能够有效提升多模态大语言模型在多图像场景下的感知和推理能力，为处理复杂多图像任务提供了一种有效的解决方案。
...
TL;DR: 提出了一种结合样本混合和扩散合成的数据增强方法MCPMix，通过掩码一致性配对混合和真实锚定学习退火机制，在保持像素级语义的同时增加多样性，在多个医学图像分割数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法存在局限性：样本混合会导致软标签歧义，而扩散合成会引入合成-真实域偏移。需要一种能结合两者优势的方法，在增加多样性的同时保持语义一致性。

Method: 提出MCPMix方法：为每个真实图像生成相同掩码下的合成对应物，进行掩码一致性配对混合，只混合图像外观而保持原始硬掩码监督；同时使用RLA机制自适应调整混合强度和损失权重。

Result: 在Kvasir-SEG、PICCOLO、CVC-ClinicDB、NPC-LES和ISIC 2017等多个数据集上实现了最先进的分割性能，相比基线方法获得一致提升。
...
TL;DR: T-PMambaSR是一个轻量级超分辨率框架，结合窗口自注意力和渐进式Mamba，通过多尺度感受野交互建立细粒度建模范式，并引入自适应高频细化模块恢复丢失的高频细节。
...
Method: 提出T-PMambaSR框架，集成窗口自注意力和渐进式Mamba，实现不同尺度感受野的交互，并引入自适应高频细化模块。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割模型难以在实用计算预算下同时实现高效的全局上下文建模和长距离依赖推理。
...
### [23] [IEC3D-AD: A 3D Dataset of Industrial Equipment Components for Unsupervised Point Cloud Anomaly Detection](https://arxiv.org/abs/2511.03267)
*Bingyang Guo,Hongjie Li,Ruiyun Yu,Hanzhe Liang,Jinbao Wang*

Main category: cs.CV

TL;DR: 本文提出了一个针对工业设备组件的3D异常检测数据集IEC3D-AD，并基于此开发了GMANet方法，通过几何形态分析生成合成点云样本，优化空间差异来提高异常检测性能。
...
### [24] [Unified Long Video Inpainting and Outpainting via Overlapping High-Order Co-Denoising](https://arxiv.org/abs/2511.03272)
*Shuangquan Lyu,Steven Mao,Yue Ma*

Main category: cs.CV

TL;DR: 提出了一种基于LoRA微调和重叠-融合时序协同去噪策略的统一方法，用于实现任意长度视频的高质量修复和外延，解决了长视频生成和编辑中的可控性挑战。


<details>
  <summary>Details</summary>
Motivation: 解决长视频生成的根本挑战，特别是视频修复和外延中的高可控性需求，同时处理这两个问题以实现对长视频片段的可控编辑。

Method: 利用LoRA高效微调大型预训练视频扩散模型（如Wan 2.1），采用重叠-融合时序协同去噪策略和高阶求解器来保持长序列的一致性。

Result: 在包含数百帧的对象编辑和添加等挑战性任务上验证，在质量指标（PSNR/SSIM）和感知真实性（LPIPS）方面优于基线方法Wan 2.1和VACE。

Conclusion: 该方法实现了参数效率和性能优越的平衡，能够以最小开销进行实用的长距离视频编辑，无可见接缝或漂移。
...
### [25] [Diffusion-SDPO: Safeguarded Direct Preference Optimization for Diffusion Models](https://arxiv.org/abs/2511.03317)
*Minghao Fu,Guo-Hua Wang,Tianyu Cui,Qing-Guo Chen,Zhao Xu,Weihua Luo,Kaifu Zhang*

Main category: cs.CV

TL;DR: 本文提出了Diffusion-SDPO方法，解决了扩散模型直接偏好优化中的关键问题：偏好边际增大并不总能提升生成质量，反而可能增加重构误差。新方法通过自适应缩放失败者梯度来保护获胜者，确保偏好输出的误差不会增加。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型的直接偏好优化方法存在病理问题，偏好边际扩大不一定改善生成质量，反而可能同时增加获胜和失败分支的重构误差，导致偏好分支也受到不利影响。

Method: 提出Diffusion-SDPO方法，采用受保护的更新规则，根据失败者梯度与获胜者梯度的对齐情况自适应缩放失败者梯度，通过一阶分析得到闭式缩放系数，保证每个优化步骤中偏好输出的误差非递增。
...
### [26] [SurgViVQA: Temporally-Grounded Video Question Answering for Surgical Scene Understanding](https://arxiv.org/abs/2511.03325)
...
<details>
  <summary>Details</summary>
Motivation: 当前外科视频问答方法局限于静态图像特征，缺乏对时间动态性的建模，而可用数据集往往缺少时间标注，忽略了准确程序解释所需的关键动态信息。

Method: 提出SurgViVQA模型，使用掩码视频-文本编码器融合视频和问题特征，捕捉运动和工具-组织交互等时间线索，然后通过微调的大型语言模型解码为连贯答案。

Result: 在REAL-Colon-VQA和EndoVis18-VQA数据集上的实验验证显示，SurgViVQA在关键词准确性方面优于PitVQA模型，分别提升11%和9%。扰动研究进一步证实了模型对问题表述变化的鲁棒性。

Conclusion: SurgViVQA和REAL-Colon-VQA数据集为外科视频问答提供了时间感知理解的框架，使AI模型能够更有效地解释动态程序上下文。
...
### [27] [Multi-Object Tracking Retrieval with LLaVA-Video: A Training-Free Solution to MOT25-StAG Challenge](https://arxiv.org/abs/2511.03332)
*Yi Yang,Yiming Xu,Timo Kaiser,Hao Cheng,Bodo Rosenhahn,Michael Ying Yang*

Main category: cs.CV

TL;DR: 本文提出了一种解决MOT25-时空动作定位挑战的两阶段零样本方法，结合FastTracker跟踪模型和LLaVA-Video多模态大语言模型，在测试集上获得m-HIoU 20.68和HOTA 10.73的成绩，赢得第二名。


<details>
  <summary>Details</summary>
Motivation: 准确地在复杂真实场景视频中定位和跟踪符合特定和自由形式语言查询的多个对象，解决MOT25-时空动作定位挑战。

Method: 将任务建模为视频检索问题，采用两阶段零样本方法，结合SOTA跟踪模型FastTracker和多模态大语言模型LLaVA-Video。

Result: 在MOT25-StAG测试集上，方法获得m-HIoU 20.68和HOTA 10.73的分数，赢得挑战赛第二名。

Conclusion: 提出的两阶段零样本方法在复杂视频场景的语言查询对象定位和跟踪任务中表现优异，证明了多模态模型在时空动作定位任务中的有效性。
...
### [28] [UniAVGen: Unified Audio and Video Generation with Asymmetric Cross-Modal Interactions](https://arxiv.org/abs/2511.03334)
...
<details>
  <summary>Details</summary>
Motivation: 现有开源音频-视频生成方法因缺乏有效的跨模态建模，导致唇部同步不足和语义一致性差。

Method: 采用双分支联合合成架构，包含两个并行扩散变换器；核心是非对称跨模态交互机制，实现双向时间对齐的跨注意力；增强的面部感知调制模块动态优先处理显著区域；提出模态感知无分类器引导策略。
...
### [29] [Decoupling Augmentation Bias in Prompt Learning for Vision-Language Models](https://arxiv.org/abs/2511.03367)
...
Method: 提出AAPL方法，引入对抗性token嵌入来分离图像增强产生的视觉变化与语义表示，使学习到的提示能够专注于与目标类别对齐的视觉判别特征。

Result: 在11个基准数据集上的实验表明，AAPL在少样本、零样本、跨数据集和领域泛化设置中均优于现有方法。

Conclusion: 图像级增强特别是属性特定变化可以有效支持提示学习，AAPL通过解耦视觉变化与语义表示显著提升了模型的泛化性能。
...
### [30] [Robust Alignment of the Human Embryo in 3D Ultrasound using PCA and an Ensemble of Heuristic, Atlas-based and Learning-based Classifiers Evaluated on the Rotterdam Periconceptional Cohort](https://arxiv.org/abs/2511.03416)
...
TL;DR: 提出了一种自动化方法，通过主成分分析(PCA)从胚胎分割掩模中提取主轴，并使用三种策略选择标准方向，实现三维超声图像中胚胎的标准化对齐。
...
Method: 基于胚胎分割掩模应用PCA提取主轴，生成四个候选方向，然后使用三种策略选择标准方向：基于皮尔逊相关的形状启发式方法、通过归一化互相关与图谱进行图像匹配、以及随机森林分类器。

Result: 在2166个图像上测试，PCA在99.0%的图像中正确提取胚胎主轴。皮尔逊启发式、基于图谱和随机森林方法分别正确选择了97.4%、95.8%和98.4%的图像。多数投票方法达到98.5%的准确率。
...
Result: 实验证明该方法能够鲁棒地捕捉包括撕裂和切割在内的各种拓扑事件，在合成和真实数据上都优于基线方法。
...
TL;DR: Anny是一个基于人体测量学知识的开源、可微分人体模型，无需3D扫描即可生成从婴儿到老年人的各种体型，通过WHO人口统计数据校准，支持毫米级扫描拟合、可控合成数据生成和人体网格恢复。


<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型依赖昂贵的3D扫描和专有学习形状空间，存在人口统计范围狭窄的问题，需要开发一个开放、基于人体测量学知识、覆盖广泛人群的简单人体模型。

Method: 基于MakeHuman社区的人体测量学知识构建完全可微分模型，使用表型参数（性别、年龄、身高、体重）控制混合形状，通过WHO人口统计数据校准，创建连续可解释的形状空间。

Result: Anny模型能够生成从婴儿到老年人的各种体型，支持毫米级扫描拟合，生成的Anny-One数据集包含80万张照片级真实感人体图像，HMR模型训练性能可与基于扫描的模型相媲美。

Conclusion: Anny提供了一个开放、语义可控、人口统计基础扎实的人体建模基础框架，其简单性不影响性能，同时保持可解释性和广泛代表性，为以人为中心的3D建模提供了可访问的基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交互动检测方法忽视了面部表情、注视和手势等细微线索，主要依赖个体的整体表示，且直接检测社交群体而未显式建模个体间的底层互动，这限制了捕捉局部社交信号的能力。

Method: 首先检测个体并使用部位感知线索增强其特征，然后通过基于相似性的推理关联个体来推断群体配置，该推理不仅考虑空间关系，还考虑暗示互动的细微社交线索。

Result: 在NVI数据集上的实验表明，该方法优于先前方法，达到了新的最先进水平。

Conclusion: 提出的部位感知自底向上群体推理框架能够更准确地捕捉细粒度社交互动，通过显式建模个体间的底层互动和利用细微社交线索，实现了更精确的群体推断。
...
TL;DR: DANCE是一个视频动作识别框架，通过解耦运动动态、物体和场景概念来提供可解释的预测，在保持竞争力的同时显著提升了解释清晰度。


<details>
  <summary>Details</summary>
Motivation: 现有视频动作识别模型的解释方法存在两个问题：基于显著性的方法产生纠缠的解释，无法区分运动与空间上下文；基于语言的方法难以解释直觉理解但难以言喻的运动。

Method: 提出DANCE框架，采用前概念瓶颈设计，通过三种解耦概念类型预测动作：运动动态概念（人体姿态序列）、物体概念和场景概念（由大语言模型自动提取）。

Result: 在KTH、Penn Action、HAA500和UCF-101四个数据集上的实验表明，DANCE在保持竞争力的同时显著改善了解释清晰度，用户研究验证了其优越的可解释性。

Conclusion: DANCE不仅提供更清晰的解释，还有利于模型调试、编辑和失败分析，为视频动作识别提供了有效的可解释性解决方案。
...
TL;DR: ARC是一种新颖的、无需标注的缓存框架，通过动态管理小型高价值语料库，将存储需求降低到原始语料的0.015%，同时提供高达79.8%的答案命中率，并将平均检索延迟降低80%。
...
Result: 在三个检索数据集上的综合实验表明，ARC将存储需求降至原始语料的0.015%，提供高达79.8%的答案命中率，平均检索延迟降低80%。
...
TL;DR: 提出LEGO-Eval评估框架和LEGO-Bench基准，用于评估3D场景生成与细粒度指令的对齐程度，解决现有方法在评估真实世界环境场景生成质量方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的3D场景缺乏真实世界的空间布局和物体属性，这源于指令过于粗糙。需要细粒度指令来生成更真实的场景，否则训练出的具身智能体会学习与现实世界物理和语义不符的先验知识。

Method: 引入LEGO-Eval评估框架，配备多样化工具来显式地锚定场景组件，实现更准确的对齐评估。同时提出LEGO-Bench基准，包含详细指令以指定复杂布局和真实世界环境属性。

Result: LEGO-Eval在评估场景-指令对齐方面比VLM-as-a-judge高出0.41 F1分数。使用LEGO-Bench基准测试显示，当前生成方法存在显著局限性，所有评估方法的成功率最多只有10%能完全对齐细粒度指令。

Conclusion: 当前3D场景生成方法在满足细粒度指令要求方面表现不佳，LEGO-Eval提供了更可靠的评估框架，LEGO-Bench为改进3D场景生成提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: ABSA研究主要集中在商业领域，而在教育和医疗等高需求低资源领域存在研究空白；现有评估方法过于严格，无法准确反映生成模型的性能；现有方法依赖资源密集的知识注入。

Method: 1) 提出FTS-OBP评估方法，处理边界变化；2) 研究小型解码器生成语言模型(<7B参数)，探索数据无关和数据轻量微调方法；3) 提出多任务微调策略；4) 发布教育评论ABSA资源集。

Result: FTS-OBP与传统指标强相关并提供细粒度诊断；1.5-3.8B模型通过多任务微调可超越专有大模型，仅需200-1,000样本在单GPU上接近基准结果。

Conclusion: 该方法成功解决了低资源领域ABSA的挑战，为教育和医疗等领域提供了可行的解决方案，同时改进了评估方法以更好地反映模型性能。
...
TL;DR: 本文研究了单边对话问题（1SC），即在只能记录对话一方的情况下推断和学习对话内容。通过评估提示和微调模型在多个数据集上的表现，发现访问未来一轮对话和话语长度信息能改善重建效果，占位符提示有助于减少幻觉，大模型通过提示能生成有前景的重建结果，而小模型需要微调。高质量摘要可以在不重建缺失对话的情况下生成。


<details>
  <summary>Details</summary>
Motivation: 现实世界中许多场景（如远程医疗、呼叫中心、智能眼镜）只能记录对话的一方，这限制了对话式AI的应用。本文旨在解决这一单边对话问题，开发能够从单边对话中推断和学习的方法。

Method: 使用MultiWOZ、DailyDialog和Candor数据集，评估提示和微调模型在两种任务上的表现：实时重建缺失说话者的对话轮次，以及从单边转录生成摘要。采用人工A/B测试和LLM-as-a-judge指标进行评估。

Result: 研究发现：访问未来一轮对话和话语长度信息能改善重建效果；占位符提示有助于减少幻觉；大模型通过提示能生成有前景的重建结果，而小模型需要微调；高质量摘要可以在不重建缺失对话的情况下生成。

Conclusion: 本文提出了单边对话问题作为一个新的挑战，并报告了有前景的结果，标志着向隐私感知对话式AI迈出了一步。
...
TL;DR: PolyNorm是一种基于提示的文本归一化方法，使用大语言模型减少对手工规则的依赖，在八种语言上相比生产级系统持续降低词错误率。
...
Conclusion: PolyNorm方法能够减少对手工规则的依赖，实现更广泛的语言适用性，为多语言文本归一化研究提供了新途径。
...
TL;DR: 提出基于控制屏障函数的LLM对齐框架，通过安全过滤器干预文本生成，无需微调基线模型即可实现对齐目标


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型对齐问题，确保生成用户期望的文本内容，提供无需微调的对齐方法

Method: 使用控制屏障函数作为安全过滤器，对基线LLM生成的预测token进行干预，可直接应用评估模型进行过滤器设计

Result: 基于开源语言模型实现了文本生成系统，能够生成积极文本

Conclusion: 控制屏障函数框架为LLM对齐提供了一种有效的附加式解决方案，无需修改基线模型即可实现期望的对齐效果
...
Result: 闭源模型整体领先（Gemini-2.5-Pro 42.66 vs GLM-4.5V 30.45），空间和几何推理普遍薄弱（≤30%），发现常见错误模式如方向错误、跨视图身份保持脆弱、反事实指令遵循差等。
...
TL;DR: 本文提出了一个基于LLM的利益相关者风险评估框架，通过让LLM充当法官来预测和解释AI系统中的风险，生成可解释的利益相关者特定政策，并展示不同利益相关者对相同风险的分歧。
...
Conclusion: 强调利益相关者感知解释的重要性，这对于使LLM评估更加透明、可解释并与人类中心AI治理目标一致至关重要。
...
TL;DR: 本文对12种不确定性估计方法在LLM问答任务中的表现进行了实证研究，分析了信息基、密度基和语义一致性等方法在分布内和分布外数据集上的表现差异。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各行业的广泛应用，确保其输出的可信度至关重要，不确定性估计在其中扮演关键角色。需要系统评估不同不确定性估计方法在LLM中的鲁棒性和有效性。

Method: 使用12种不同的不确定性估计方法和4种生成质量指标（包括LLMScore），在问答任务的分布内和分布外数据集上评估LLM生成答案的不确定性。

Result: 信息基方法在分布内设置中表现优异，因其与模型对数据的理解一致；密度基方法和P(True)指标在分布外场景中表现更好，能有效捕捉模型的认知不确定性；语义一致性方法在不同数据集和生成指标上表现可靠。
...
TL;DR: BengaliMoralBench是首个针对孟加拉语和社会文化背景的大规模伦理基准，涵盖5个道德领域和50个文化相关子主题，用于评估多语言大语言模型在孟加拉文化环境中的伦理对齐表现。


<details>
  <summary>Details</summary>
Motivation: 随着多语言大语言模型在南亚地区的应用增加，现有伦理基准主要基于英语和西方框架，缺乏对孟加拉语（全球第六大语言，2.85亿使用者）文化细微差别的考虑，需要开发符合当地伦理规范的评估工具。

Method: 通过母语者共识标注构建包含5个道德领域（日常活动、习惯、育儿、家庭关系、宗教活动）和50个子主题的基准，使用三种伦理视角（美德伦理、常识伦理、正义伦理）进行系统评估。

Result: 对Llama、Gemma、Qwen、DeepSeek等多语言大语言模型的零样本评估显示性能差异显著（准确率50-91%），定性分析揭示在文化基础、常识推理和道德公平性方面存在一致弱点。

Conclusion: BengaliMoralBench为负责任的本土化提供了基础，支持在孟加拉国等多语言低资源环境中部署符合文化伦理的AI系统。
...
TL;DR: 提出语言图模型(LGM)来解决大语言模型在理解模糊或概念不对齐术语时的困难，通过提取元关系(继承、别名、组合)并使用反思机制验证，结合概念迭代检索算法动态提供相关信息，无需截断即可处理任意长度文本，在标准基准测试中优于现有RAG方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然语义理解能力强，但在处理用户指令中的模糊或概念不对齐术语时表现不佳，需要一种方法来增强概念清晰度。

Method: 提出语言图模型(LGM)，从自然语言中提取继承、别名和组合三种元关系，使用反思机制验证这些关系，并采用概念迭代检索算法动态向LLM提供相关关系和描述。

Result: 在标准基准测试中，LGM始终优于现有的RAG基线方法，能够处理任意长度文本而无需截断。

Conclusion: LGM通过提取和验证元关系，结合动态概念检索，有效提升了大语言模型的概念理解能力和响应准确性，优于传统RAG方法。
...
TL;DR: ISI的SARAL项目在MATERIAL计划中开发了跨语言信息检索方法，专注于检索查询相关文档集而非仅排序列表，在第三阶段评估中在5/6条件下表现优于其他团队。


<details>
  <summary>Details</summary>
Motivation: 推进跨语言信息检索技术，特别是在检索相关文档集合而非仅排序文档列表方面取得突破。

Method: 开发新颖的跨语言信息检索方法，专注于检索查询相关文档集合。

Result: 在MATERIAL第三阶段评估中，SARAL在涉及波斯语、哈萨克语和格鲁吉亚语的六个评估条件中的五个条件下表现优于其他团队。

Conclusion: SARAL的方法在跨语言信息检索方面表现优异，特别是在检索相关文档集合方面具有优势。
...
TL;DR: IndicSuperTokenizer是一个针对印度多语言大语言模型的tokenizer，结合了子词和多词分词方法，通过语言特定的预分词实现了更好的语言对齐，在生育率得分上达到新SOTA，推理吞吐量比LLaMA4提升44%。


<details>
  <summary>Details</summary>
Motivation: 为多语言LLM设计有效的tokenizer具有挑战性，因为存在不同的文字系统和丰富的形态变化。虽然BPE等子词方法被广泛采用，但它们在多语言环境中的有效性仍未充分探索。

Method: 结合子词和多词分词方法，采用语言特定的预分词策略，使token更符合语言结构。

Result: 在英语、22种印度语言和代码数据上的评估显示，平均生育率得分比LLaMA4提高39.5%，比当前最佳Sutra提高18%，推理吞吐量比LLaMA4提高44%，同时在英语和印度语言基准测试中保持可比性能。

Conclusion: IndicSuperTokenizer通过创新的分词策略在多语言环境中取得了显著改进，证明了设计选择的稳健性，为多语言LLM的tokenizer设计提供了有价值的见解。
...
TL;DR: 本研究比较了四种开源LLM（Mistral-7b-instruct、LLaMa2-7b-chat、Falcon-7b-instruct、Orca-mini-v3-7b）和OpenAI的GPT-3.5在计算机科学文献问答任务中的性能，使用RAG技术增强模型能力。GPT-3.5表现最佳，Mistral-7b-instruct在开源模型中表现最好，Orca-mini-v3-7b响应速度最快。


<details>
  <summary>Details</summary>
Motivation: 随着RAG技术与大型语言模型的结合日益重要，需要比较不同LLM在多样化领域问答任务中的性能表现，特别是在计算机科学文献领域。

Method: 使用RAG技术支持，评估四种开源LLM和GPT-3.5在计算机科学文献问答任务中的表现。评估指标包括二元问题的准确率和精确率，以及长答案问题的专家排名、Gemini模型排名和余弦相似度。

Result: GPT-3.5结合RAG在二元和长答案问题上表现最佳。在开源模型中，Mistral-7b-instruct表现最好，Orca-mini-v3-7b响应延迟最短，LLaMa2-7b-chat响应延迟最长。

Conclusion: 开源LLM在适当的基础设施支持下，可以与GPT-3.5等专有模型相媲美，Mistral-7b-instruct是表现最好的开源模型。
...
TL;DR: SCALE是一种宽度扩展架构，通过在冻结预训练参数的同时在线性模块中插入轻量级扩展来增加模型容量，同时保持基础模型的功能不变。


<details>
  <summary>Details</summary>
Motivation: 当前持续预训练的进展更多地依赖于扩展正确的结构而非仅仅扩展参数数量，需要一种既能增加容量又不会干扰基础模型功能的方法。

Method: 提出SCALE架构，采用持久保持原则（冻结预训练权重）和协作适应原则（选择性训练扩展组件），包括SCALE-Preserve、SCALE-Adapt和带路由扩展的SCALE-Route三个变体。

Result: 在合成传记基准测试中，SCALE缓解了深度扩展中的严重遗忘问题；在韩语语料持续预训练中，SCALE变体在英语评估中遗忘较少，在韩语基准上获得有竞争力的提升，提供了最佳的稳定性-可塑性权衡。

Conclusion: SCALE架构通过保持和适应的协同作用，在持续预训练中实现了更好的稳定性-可塑性平衡，相比标准持续学习方法优化更稳定。
...
TL;DR: 本文评估了两种领先的多模态大语言模型（Seed1.5-VL和Gemini-2.5-Flash）在医学应用中的"思考模式"能力，发现激活思考模式相比标准非思考模式在大多数任务中改进有限，复杂医学任务表现仍不理想。


<details>
  <summary>Details</summary>
Motivation: 随着"双状态"多模态大语言模型的快速发展，需要严格评估这些模型增强的推理过程如何影响临床任务中的模型性能和可靠性。

Method: 使用VQA-RAD和ROCOv2数据集，在四个视觉医学任务上评估两种领先MLLM的主动"思考模式"能力。

Result: 研究发现，对于大多数任务，激活思考模式相比标准非思考模式的改进仍然有限。在复杂医学任务（如开放式VQA和医学图像解释）上的表现仍然不理想。

Conclusion: 医学领域需要特定领域的医学数据和更先进的医学知识整合方法，当前MLLM在复杂医学任务中的能力仍有待提升。
...
<details>
  <summary>Details</summary>
Motivation: 系统识别和评估生成式人工智能在生物信息学中日益增长的发展，评估有影响力的GenAI策略在方法论进步、预测性能和专业化方面的表现，并确定用于高级建模、数据密集型发现和综合生物学分析的有前景方法。

Method: 采用系统综述和荟萃分析方法，提出六个研究问题(RQ1-RQ6)，评估GenAI在生物信息学各子领域的应用、模型架构、领域效益、改进验证、约束因素和数据集支持。

Result: RQ1显示GenAI在序列分析、分子设计和综合数据建模中表现优于传统方法；RQ2表明专业化模型架构优于通用模型；RQ3发现生物信息学领域在分子分析和数据整合方面获益显著；RQ4验证了结构建模、功能预测和合成数据生成的改进；RQ5识别了可扩展性不足和数据偏差等约束；RQ6确认分子、细胞和文本数据集广泛支持GenAI训练。

Conclusion: 生成式人工智能已成为生物信息学的变革性方法，在多个子领域表现出色，专业化模型架构具有优势，但面临可扩展性和数据偏差等挑战，未来需要关注稳健评估和生物学基础建模。
...
TL;DR: 本文提出了"沉默偏见"概念，指安全对齐大语言模型中潜在存在但被掩盖的不公平偏好，并开发了Silenced Bias Benchmark (SBB)基准，通过激活引导减少模型拒绝来揭示这些偏见。


<details>
  <summary>Details</summary>
Motivation: 现有公平性评估方法通常基于问答模式，将模型的拒绝响应解读为公平性指标，这造成了虚假的公平感，忽略了模型潜在空间中的不公平偏好。

Method: 提出SBB基准，采用激活引导技术减少模型在问答过程中的拒绝行为，从而揭示被安全对齐掩盖的沉默偏见，支持扩展到新的人口群体和主题。

Result: 在多个大语言模型上的实验表明，模型的直接响应与底层公平性问题之间存在显著差异，暴露了安全对齐可能掩盖的偏见问题。
...
<details>
  <summary>Details</summary>
Motivation: 针对复杂逻辑数据增强中过度依赖人工标注成本高，以及直接使用大语言模型生成导致示例不可解释且逻辑同质化的问题，提出LFC-DA方法。

Method: LFC-DA采用符号逻辑控制流程：首先将逻辑文本映射为命题表达式，编译紧凑规则库，然后通过有界状态空间搜索系统性地发现有效公式，最后将这些公式转化为自然语言问题。

Result: 在ReClor和LogiQA数据集上的实验表明，该方法显著提高了预训练模型的逻辑推理准确性。

Conclusion: LFC-DA在大语言模型引导的逻辑数据增强方面具有有效性，能够确保逻辑多样性和严谨性。
...
TL;DR: 本文研究发现，在机器翻译中，使用非对称BPE（源语言和目标语言采用不同合并操作次数）比传统的对称BPE能显著提升性能，特别是在低资源场景下。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译研究通常为源语言和目标语言使用相同的BPE超参数设置，但这种对称方法可能无法在不同语言对和数据规模下获得最优性能。

Method: 研究在不同数据量和语言对下评估BPE分词策略对机器翻译系统性能的影响，重点比较对称BPE和非对称BPE方法。

Result: 非对称BPE在低资源设置（50K、100K和500K句对）下显著优于对称方法，在英语-印地语翻译中获得平均5.32、4.46和0.7 CHRF++的提升。在6个额外语言对的12个系统中，10个系统观察到统计显著改进。

Conclusion: 源语言使用高NMO（4K-32K）和目标语言使用低NMO（0.5K-2K）的非对称BPE配置能提供最佳结果，特别有利于低资源机器翻译。
...
TL;DR: 该论文研究了小语言模型在关系抽取中处理数据类型和对象属性的能力，发现稀有属性的长尾分布是主要瓶颈，并评估了多种策略来解决这个问题。


<details>
  <summary>Details</summary>
Motivation: 研究小语言模型在完整RDF图抽取中如何处理数据类型和对象属性，重点关注稀有属性带来的挑战。

Method: 评估了分层抽样、加权损失、数据集扩展和基于模板的合成数据增强等多种策略。

Result: 发现最佳策略是构建训练集，使每个属性的出现次数超过给定阈值，从而在不平衡目标属性上获得均衡性能。
...
TL;DR: 3TF框架通过思想训练和思想自由推理，让模型在推理时无需生成显式思维链，同时保持高质量推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要压缩冗长的推理输出，但仍依赖显式推理。3TF旨在让模型在内部进行丰富推理，同时保持外部输出简洁。

Method: 训练混合模型支持推理和非推理模式，在CoT标注数据上进一步训练以内部化结构化推理，推理时使用非推理模式强制简洁输出。

Result: 3TF训练模型在思想自由推理下，在推理基准测试中获得显著提升。

Conclusion: 高质量推理可以通过隐式方式学习和执行，无需显式逐步生成思维链。
...
TL;DR: QuestionRAG是一个解决QA系统中输入错误问题的框架，通过外部知识增强和强化学习对齐来改进大语言模型的问题纠正能力。


<details>
  <summary>Details</summary>
Motivation: QA系统中的输入错误经常导致错误回答，大语言模型在处理这类任务时存在误解用户意图或过度修正问题结构的问题。

Method: QuestionRAG采用两种策略：1）通过外部知识（如搜索结果、相关实体）增强输入以解决误解问题；2）使用强化学习将模型目标与精确修正对齐，避免过度修正。

Result: 结果表明知识增强对于理解错误问题至关重要，基于强化学习的对齐方法比传统监督微调更有效，显著提升了模型遵循指令和泛化的能力。

Conclusion: 通过整合这两种策略，QuestionRAG释放了大语言模型在问题纠正任务中的全部潜力。
...
TL;DR: CareMedEval是一个用于评估大语言模型在生物医学批判性评估和推理任务表现的数据集，包含534个基于37篇科学文章的问题，源自法国医学生真实考试。


<details>
  <summary>Details</summary>
Motivation: 生物医学领域的科学文献批判性评估是一项关键技能，虽然大语言模型能提供支持，但其在专业领域的批判性推理可靠性仍然有限，需要专门的评估工具。

Method: 创建CareMedEval数据集，基于真实医学考试，包含534个问题，评估大语言模型在多种上下文条件下的表现，包括生成中间推理标记的效果。

Result: 最先进的通用和专业生物医学大语言模型表现不佳，即使生成中间推理标记能显著改善结果，精确匹配率仍不超过0.5，特别是在研究局限性和统计分析问题上表现最差。

Conclusion: CareMedEval为基于科学文献的推理提供了一个具有挑战性的基准，揭示了当前大语言模型的局限性，为未来开发自动化批判性评估支持工具铺平了道路。
...
TL;DR: Kastor框架通过RDF模式提取方法改进小语言模型的关系抽取能力，将SHACL形状验证任务重构为评估所有可能属性组合并选择最优组合，显著提升模型泛化性能，并通过迭代学习优化噪声知识库。


<details>
  <summary>Details</summary>
Motivation: 满足专业领域知识库补全和精化的需求，通过RDF模式提取方法在有限文本和RDF数据上训练高效的小语言模型。

Method: 将传统SHACL形状验证任务重构为评估所有可能属性组合，为每个训练示例选择最优组合，并采用迭代学习过程来精化噪声知识库。

Result: 显著增强了模型的泛化能力和性能，能够创建能够发现新相关事实的鲁棒模型。

Conclusion: Kastor框架通过改进的RDF模式提取方法和迭代学习过程，有效提升了小语言模型在关系抽取任务中的表现，特别适用于专业领域知识库的完善工作。
...
Conclusion: BanglaSTEM数据集和训练的翻译模型公开可用，显著改善了孟加拉语技术内容的翻译质量，解决了现有翻译系统在技术术语处理上的局限性。
...
TL;DR: 提出了HaluMem基准，这是首个针对记忆系统的操作级幻觉评估基准，包含三个评估任务（记忆提取、记忆更新和记忆问答），通过构建大规模多轮人机交互数据集来全面揭示不同操作阶段的幻觉行为。
...
Method: 定义三个评估任务（记忆提取、记忆更新和记忆问答），构建用户中心的多轮人机交互数据集HaluMem-Medium和HaluMem-Long，包含约15k记忆点和3.5k多类型问题，对话轮次达1.5k和2.6k，上下文长度超过1M tokens。
...
TL;DR: 提出了一个可扩展的多轮指令跟随能力评估框架EvolIF，通过三层机制解耦语言表面形式和用户意图模拟，动态构建包含状态变化和回溯的基准测试，在模型耗尽模拟用户耐心时终止对话。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常局限于固定轮次，容易饱和且无法考虑用户的交互体验，需要评估大语言模型在多主题对话中跟随用户指令的能力。

Method: 使用三层机制跟踪约束、指令和主题，模拟用户-LLM交互，动态构建包含状态变化和回溯的基准测试，定义了一套捕捉交互过程质量的指标。

Result: 构建了EvolIF基准，包含九种不同的约束类型。GPT-5表现出色，平均维持18.54轮对话，稳健性达70.31%，显著优于Gemini-2.5-Pro 11.41%。
...
TL;DR: 该论文创建了MultiZebraLogic数据集，包含多种语言、主题和难度的斑马谜题，用于评估大型语言模型的逻辑推理能力。研究发现4x5大小的谜题对推理模型o3-mini具有挑战性，而干扰线索会显著降低模型性能。


<details>
  <summary>Details</summary>
Motivation: 为了全面评估大型语言模型在不同语言和难度下的逻辑推理能力，需要创建大规模、高质量的基准数据集。

Method: 生成了多种语言、主题、大小和线索类型的斑马谜题，包括14种线索类型和8种干扰线索类型，并在9种日耳曼语言中发布了128+1024个谜题的数据集。

Result: 研究发现2x3大小的谜题对非推理模型GPT-4o mini具有挑战性，4x5大小的谜题对推理模型o3-mini具有挑战性。5个干扰线索使o3-mini在4x5谜题上的准确率下降15±7%。语言和主题对性能影响不显著。

Conclusion: 成功创建了可适应更多语言和主题的斑马谜题生成代码和数据集，为评估LLMs的逻辑推理能力提供了有效工具。
...
TL;DR: 本文首次实证展示了Transformer语言模型中可控局部性的实现，通过可调局部性参数实现表示局部化程度的连续控制，在可解释的局部编码和高效的分布式表示之间动态插值。
...
Method: 使用两层Transformer架构在WikiText语料库上进行实验，系统变化局部性参数λ从1.0（完全局部）到0.0（完全分布式），通过显式惩罚阈值和信息论设计原则实现数学控制。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法主要关注表示层面的解耦，而该研究旨在通过仅使用大边距合成数据来实现迭代控制和跨语音的高表达性，避免嵌入先验或辅助模块的需求。

Method: 核心创新在于仅利用大边距合成数据，这种方法绕过了基于嵌入的先验或辅助模块的需求，实现了迭代控制和高表达性。

Result: 评估结果显示，Step-Audio-EditX在情感编辑和其他细粒度控制任务上超越了MiniMax-2.6-hd和Doubao-Seed-TTS-2.0。

Conclusion: 该模型代表了从传统表示层面解耦关注的根本性转变，通过大边距学习方法实现了表达性音频编辑和强大的零样本TTS能力。
...
TL;DR: 本文提出了中文多文档问答数据集ChiMDQA，包含6个领域的6,068个高质量问答对，涵盖10个细粒度类别，为中文NLP任务提供重要数据基础。
...
Result: 成功构建了包含6,068个高质量问答对的ChiMDQA数据集，涵盖10个细粒度类别，确保数据集多样性和高质量。

Conclusion: ChiMDQA数据集为中文文档理解、知识抽取和智能问答系统等NLP任务提供了重要数据基础，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 旨在帮助政策制定者更好地理解、分析和制定法律规范，提升法律研究和法规制定的效率。

Method: 构建了针对法律领域的监督数据集进行模型微调，并集成检索增强生成技术以获取最新法律知识。

Result: 该方法显著提升了法律研究和法规制定的有效性，能够主动协助政策制定者解释法规并起草符合当前需求的新法规。

Conclusion: 结合微调和检索增强生成的方法为法律领域提供了一个有价值的工具，能够适应不断变化的法律环境。
...
TL;DR: 本文提出了一个视角主义标注方案，用于分析协作对话中的指称误解，通过分别捕捉说话者和听者的理解来追踪理解的形成、分歧和修复过程。


<details>
  <summary>Details</summary>
Motivation: 在非对称协作对话中，参与者可能认为他们达成了共识，但实际上指的是不同的实体。本文旨在研究这种指称误解现象。

Method: 使用方案约束的LLM标注流程对HCRC MapTask语料库进行标注，获得了13,000个带可靠性估计的标注指称表达式，分析理解状态。

Result: 结果显示，一旦词汇变体统一后，完全误解很少见，但多重性差异会系统性地导致分歧，揭示了表面共识可能掩盖指称错位。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer模型的兴起，关系抽取领域出现了大量新研究，需要对这些工作进行系统整理，为研究者和实践者提供全面的参考。

Method: 使用自动化框架收集和标注出版物，对34篇综述、64个数据集和104个模型进行分析，从多个维度整合结果。

Result: 识别了当前关系抽取研究的方法论进展、基准资源发展以及语义网技术的整合情况。

Conclusion: 研究揭示了关系抽取领域的发展趋势、现有局限性和开放挑战，为理解该领域的演变和未来方向提供了全面参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本立场检测方法存在泛化性问题、文本与目标间缺乏连贯性，以及过度依赖显式推理、缺乏细致解释、难以解释模型预测过程等问题。

Method: IRIS框架将立场检测视为信息检索排序任务，基于文本中的隐式推理序列和显式语言特征来理解立场，提供内在可解释性，无需真实推理依据。

Result: 在VAST、EZ-STANCE、P-Stance和RFD基准数据集上的广泛实验证明，即使使用50%、30%甚至10%的训练数据，模型仍具有良好的泛化性能。

Conclusion: IRIS框架通过其提出的架构和可解释设计，在零样本立场检测任务中表现出优越的泛化能力和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 开放数据仓库对非专家用户存在访问障碍，需要数据集发现、模式映射和统计分析的专业知识。虽然大语言模型在单个任务中表现良好，但在端到端分析工作流中面临注意力稀释、推理模式干扰和错误传播的根本限制。

Method: 提出多智能体框架，将分析工作流分解为四个专门智能体：意图澄清、数据集发现、分析和报告。这种架构在每个阶段保持专注的注意力并实现验证。

Result: 评估了5个模型和50个查询，得出多智能体LLM系统的五个设计原则：1)专业化独立于模型强度提供价值；2)智能体分为通用型和条件型；3)智能体缓解不同的失败模式；4)架构优势在不同任务复杂度下持续存在；5)智能体有效性在不同模型间差异显著。
...
TL;DR: ScalingEval是一个大规模基准测试研究，系统比较了36个大型语言模型在多个产品类别中的表现，使用共识驱动的评估协议，无需人工标注即可实现可复现的LLM评估器比较。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型作为评判者的能力对于构建可扩展和可信赖的评估流程日益重要，需要系统性的基准测试来比较不同模型的性能。

Method: 采用多智能体框架，通过可扩展的多数投票将模式审计和问题代码聚合成真实标签，使用共识驱动的评估协议进行系统比较。

Result: 主要发现：Claude 3.5 Sonnet决策置信度最高；Gemini 1.5 Pro在各类别中整体表现最佳；GPT-4o提供最佳延迟-准确率-成本权衡；GPT-OSS 20B在开源模型中领先。结构化领域共识强，生活方式类别存在持续分歧。

Conclusion: ScalingEval为LLM作为评判者建立了可复现的基准和评估协议，提供了关于扩展性、可靠性和模型系列权衡的可操作指导。
...
TL;DR: 本文构建了首个基准测试来评估LLMs是否内化了描述现实世界概率分布的知识，结果显示LLMs在理解现实世界统计分布方面表现不佳，且缺乏Pearl因果层次中观测分布的知识。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在科学应用中展现出巨大潜力，但需要更通用的智能能力。关键区别在于事实性知识和概率性知识，LLMs被宣传为现实世界分布的通用近似器，但统计学中的维度灾难理论对此提出了根本性挑战。

Method: 开发首个基准测试，直接测试LLMs是否能够获取描述现实世界人口的经验分布，涵盖经济、健康、教育和社会行为等领域。

Result: LLMs整体表现不佳，似乎没有自然地内化现实世界统计信息。在Pearl因果层次框架下，语言模型缺乏观测分布知识。

Conclusion: LLMs在理解现实世界概率分布方面能力有限，根据因果层次定理，这意味着它们在干预性和反事实性知识方面也存在局限性。
...
TL;DR: SnapStream是一种KV缓存压缩方法，可在保持模型精度的同时实现4倍片上内存使用改进，在真实生产环境中支持128k上下文长度和1832 tokens/秒的推理速度。


<details>
  <summary>Details</summary>
Motivation: 随着100B+参数大语言模型和100k+上下文长度的普及，对片上内存支持大KV缓存的需求增加，但现有技术如StreamingLLM和SnapKV难以在工业部署框架中应用，且对现代指令跟随和推理模型的精度影响不明确。

Method: 开发SnapStream KV缓存压缩方法，可在静态图和连续批处理的生产推理系统中部署，探索其在Llama-3.1-8B-Instruct和DeepSeek-R1上的精度影响。

Result: 在16路张量并行部署的DeepSeek-671B上，SnapStream实现了4倍片上内存使用改进，在LongBench-v2、AIME24和LiveCodeBench上引入最小精度损失，支持128k上下文长度和1832 tokens/秒的推理速度。

Conclusion: SnapStream是首个在具有静态图和连续批处理的生产推理系统中部署的稀疏KV注意力技术实现，为大规模部署提供了可行的KV缓存压缩解决方案。
...
Method: 提出基于能力的监控方法，围绕共享模型能力（如摘要、推理、翻译、安全防护等）组织监控，实现跨任务检测。
...
TL;DR: 对miniF2F基准测试中正式和非正式数学陈述进行深入分析，发现原始基准存在大量形式化与自然语言表述不一致的问题，修正后创建miniF2F-v2版本，使定理证明管道的准确率从40%提升至70%。


<details>
  <summary>Details</summary>
Motivation: 评估AI系统在数学奥林匹克竞赛场景下的表现，包括自然语言理解、形式化表达和定理证明的完整流程，揭示现有基准测试中形式化与非正式表述不一致的问题。

Method: 对miniF2F基准进行全面分析，识别形式化与自然语言表述之间的差异，修正所有错误和不一致之处，创建经过验证的miniF2F-v2数据集，并重新评估定理证明管道的性能。

Result: 原始miniF2F中超过一半问题存在表述不一致，导致最佳准确率仅为36%。修正后的miniF2F-v2使准确率提升至70%，但仍显示自动形式化模型与定理证明器之间存在显著不对齐。
...
TL;DR: 本文提出了一种利用多模态大语言模型辅助设计烟花算法的新方法，通过引入关键部分概念扩展烟花算法处理复杂高维任务的能力，并在旅行商问题和电子设计自动化问题上取得了优于或达到SOTA的结果。
...
Conclusion: 结合多模态大语言模型的烟花算法设计框架能够有效处理复杂优化问题，在特定任务上展现出优越性能，为优化算法设计提供了新思路。
...
TL;DR: 本文提出了一种新型的LLM安全响应框架，在输入和输出两个层面系统保护大语言模型。输入层使用四层级分类模型实现精准风险识别，输出层结合RAG和解释模型确保回答基于可信知识库，显著提升了模型安全性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，其安全问题日益突出，严重制约了在关键领域的可信部署。需要系统性的安全防护框架来保障LLM的安全使用。

Method: 在输入层面采用基于监督微调的四层级安全分类模型（安全、不安全、条件安全、专注注意）；在输出层面集成检索增强生成（RAG）和专门微调的解释模型，确保回答基于实时可信知识库。

Result: 风险召回率达到99.3%；在公开安全评估基准上安全得分显著高于基线模型TinyR1-Safety-8B；在专有高风险测试集上组件获得100%安全得分。
...
TL;DR: 本文提出了一个多智能体AI框架，将工程设计过程形式化，通过专业化的知识驱动智能体协作生成和优化设计候选方案，并以NACA翼型气动优化为例验证了该框架的有效性。
...
Method: 构建了包含三个关键AI智能体的框架：图本体学家使用LLM从翼型设计文献构建领域知识图谱；系统工程师根据人类管理者制定技术需求；设计工程师利用设计知识图谱和计算工具提出候选翼型。系统工程师通过自身知识图谱进行定性和定量反馈，形成迭代反馈循环。

Result: 该框架成功应用于4位数NACA翼型的气动优化，最终设计通过优化提升了升阻比等性能指标，验证了协作AI智能体在工程设计中的有效性。
...
TL;DR: 本文介绍了为Adobe Summit开发的领域特定AI助手Summit Concierge，它能够处理各种活动相关查询，并在数据稀疏、质量保证和快速部署等现实约束下运行。


<details>
  <summary>Details</summary>
Motivation: 生成式AI助手在企业环境中具有提升生产力、简化信息访问和改善用户体验的巨大潜力，特别是在活动管理领域。

Method: 采用人在环路的开发工作流程，结合提示工程、检索基础化和轻量级人工验证，以应对数据稀疏和质量保证等挑战。

Result: 成功开发并部署了Summit Concierge系统，展示了敏捷、反馈驱动的开发方法能够在冷启动场景下实现可扩展且可靠的AI助手。

Conclusion: 通过人类参与的工作流程和迭代开发方法，即使在数据有限的条件下也能构建有效的领域特定AI助手，为类似企业应用提供了可行路径。
...
TL;DR: 大型语言模型（LLMs）能够仅基于大五人格量表的最小定量输入，准确预测人类心理特质的相关结构，其表现接近直接在数据集上训练的机器学习算法。


<details>
  <summary>Details</summary>
Motivation: 研究探索LLMs是否能够从最小定量输入中建模人类心理特质之间的相关结构，这对于理解LLMs的心理模拟能力和推理机制具有重要意义。

Method: 使用816名人类个体的大五人格量表响应提示各种LLMs，让它们扮演这些个体在其他九个心理量表上的响应，并分析其推理过程。

Result: LLMs在捕捉人类心理结构方面表现出色，量表间相关模式与人类数据高度一致（R² > 0.89），零样本表现显著优于基于语义相似性的预测。分析显示LLMs采用两阶段推理过程：信息选择压缩和基于摘要的响应生成。

Conclusion: LLMs能够通过抽象和推理过程从最小数据中精确预测个体心理特质，这既为心理模拟提供了强大工具，也为理解其涌现推理能力提供了宝贵见解。
...
TL;DR: 本文提出了一个名为AAA的网页可访问性审计框架，通过人机协作模式实现WCAG-EM标准的规模化执行，包含GRASP图采样方法和MaC多模态辅助系统。
...
Method: AAA框架包含两个关键创新：GRASP基于图的多模态采样方法，通过学习视觉、文本和关系线索的嵌入表示确保代表性页面覆盖；MaC多模态大语言模型辅助系统，通过跨模态推理和智能协助支持审计员完成高难度任务。

Result: 广泛的实验证明了方法的有效性，提供了小型语言模型经过微调后可以作为专家系统的见解。作者还贡献了四个用于审计流程核心阶段基准测试的新数据集。
...
TL;DR: 本文对多种机器学习模型中的解释问题进行了参数化复杂度理论分析，重点关注具有透明内部机制的模型，包括决策树、决策集、决策列表、布尔电路及其集成模型。


<details>
  <summary>Details</summary>
Motivation: 填补可解释AI领域的重要空白，为理解生成这些模型解释的复杂性提供理论基础，促进AI系统的透明度和问责制。

Method: 对两类主要解释问题（溯因和对比）及其局部和全局变体进行理论分析，涵盖多种透明ML模型。

Result: 提供了对解释问题参数化复杂度的全面理解，揭示了不同模型在解释生成方面的计算特性。
...
TL;DR: Solly是第一个在简化版Liar's Poker中达到精英人类水平的AI智能体，该游戏以广泛的多玩家互动为特点。


<details>
  <summary>Details</summary>
Motivation: AI研究者长期关注扑克类游戏作为多玩家动态、不完全信息和不确定性推理的测试平台，但现有突破主要针对双人对局，多玩家动态被抑制。

Method: 使用无模型、演员-评论家深度强化学习算法通过自我对弈训练Solly。

Result: Solly在单挑和多玩家Liar's Poker中达到精英人类水平（胜率超过50%且盈利），优于具备推理能力的大型语言模型，开发了新颖叫牌策略并有效随机化游戏。

Conclusion: Solly展示了在复杂多玩家不完全信息游戏中实现精英水平的能力，为AI在多玩家互动环境中的发展提供了重要进展。
