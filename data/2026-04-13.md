...
TL;DR: 提出了一种用于法证分析的多模态仇恨和威胁检测框架，根据证据配置选择性地应用文本分析、多模态融合或仅图像语义推理
...
Method: 提出案例驱动的多模态方法，明确确定文本证据的存在和来源（嵌入式文本、关联上下文文本、仅图像证据），根据证据配置选择性地应用文本分析、多模态融合或基于ViT的视觉语言模型的图像语义推理
...
TL;DR: 提出了一个半自动化框架，将中世纪手稿中的二维微型画转换为三维数字模型，支持XR、3D打印和Web可视化。评估了7种图像转3D方法，发现Hi3DGen在拓扑质量和表面细节间取得最佳平衡。通过案例研究展示了该框架在哥特式和文艺复兴时期手稿中的应用。


<details>
  <summary>Details</summary>
Motivation: 将中世纪手稿中的二维微型画转换为三维数字模型，以支持扩展现实(XR)应用、触觉3D打印和基于Web的可视化，特别是为视障用户提供触觉体验。

Method: 开发了一个半自动化框架，结合SAM分割、Hi3DGen网格生成、ZBrush专家精修和AI辅助纹理处理。评估了7种图像到3D方法(TripoSR, SF3D, SPAR3D, TRELLIS, Wonder3D, SAM 3D, Hi3DGen)，使用渲染指标和体积度量进行分析。

Result: 在69个手稿图像上评估发现，Hi3DGen通过其法线桥接方法在拓扑质量和丰富表面细节之间取得最佳平衡。框架成功应用于梵蒂冈图书馆的Decretum Gratiani哥特式插图和Giulio Clovio的文艺复兴微型画案例研究。

Conclusion: 提出的半自动化框架能够有效将中世纪手稿中的二维微型画转换为三维数字模型，支持WebXR可视化、AR叠加和触觉3D打印，为文化遗产数字化和视障用户可访问性提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了利用互补的归纳偏置来更好地预测视频显著性，需要一种能够整合不同专家知识的框架来捕捉复杂的时空显著性线索

Method: 提出ViSAGE多专家集成框架，每个专门的解码器执行自适应门控和调制来精炼时空特征，不同专家的互补预测在推理时进行融合
...
TL;DR: MARINER是一个专门针对海洋环境的多模态基准测试，包含16,629张多源海事图像，涵盖63个细粒度船舶类别、多样化恶劣环境和5种典型动态海事事件，用于评估细粒度分类、目标检测和视觉问答任务。
...
Method: 基于新颖的实体-环境-事件（3E）范式构建MARINER基准，包含多源海事图像、细粒度船舶分类、多样化环境和典型海事事件，涵盖多种视觉任务。

Result: 对主流多模态大语言模型的广泛评估显示，即使是先进模型在复杂海洋场景中的细粒度辨别和因果推理方面也存在困难。
...
TL;DR: WildDet3D提出了一种统一的几何感知架构，支持文本、点和框提示，并能融合深度信号，同时构建了最大的开放3D检测数据集，在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测需要泛化到闭集类别之外，支持多样提示模态，并利用可用的几何线索。现有方法存在两个瓶颈：仅支持单一提示类型且缺乏融合额外几何线索的机制，以及当前3D数据集覆盖类别有限且环境受控，限制了开放世界迁移。

Method: 提出了WildDet3D统一几何感知架构，原生支持文本、点和框提示，并能在推理时融合辅助深度信号。同时构建了WildDet3D-Data数据集，通过从现有2D标注生成候选3D框并保留人工验证的结果，包含超过100万张图像和13.5K个类别。

Result: 在开放世界设置中，WildDet3D在新引入的WildDet3D-Bench上达到22.6/24.8 AP3D（文本/框提示）。在Omni3D上达到34.2/36.4 AP3D。在零样本评估中，在Argoverse 2和ScanNet上达到40.3/48.9 ODS。融合深度线索在推理时带来显著提升（平均+20.7 AP）。

Conclusion: WildDet3D解决了现有方法的两个关键瓶颈，通过统一的几何感知架构和大型开放数据集，在多个基准测试中建立了新的最先进水平，显著提升了开放世界3D物体检测的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成艺术评估器主要关注表面图像质量或字面提示匹配，无法评估创作者意图的深层象征或抽象意义，这限制了生成艺术表达复杂人类经验的能力。

Method: 基于皮尔士计算符号学理论，将人-生成艺术交互建模为级联符号过程，提出SemJudge评估器，使用分层符号图重建从提示到生成作品的意义构建过程。

Result: 大量定量实验显示SemJudge在解释密集型美术基准上比现有评估器更接近人类判断；用户研究表明SemJudge能产生更深层、更有洞察力的艺术解释。

Conclusion: SemJudge通过评估象征和索引意义，推动生成艺术超越"漂亮"图像生成，成为能够表达复杂人类经验的媒介，为生成艺术评估提供了新的理论框架。
...
TL;DR: 3D-VCD：首个用于3D具身智能体幻觉缓解的推理时视觉对比解码框架，通过对比原始和扭曲3D场景图的预测来抑制依赖语言先验而非场景证据的token


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型作为3D环境中具身智能体的推理核心，容易产生幻觉导致不安全和不接地的决策。现有推理时幻觉缓解方法主要针对2D视觉语言设置，无法迁移到具身3D推理，因为3D环境中的失败源于物体存在、空间布局和几何接地问题，而非像素级不一致

Method: 3D-VCD框架：通过对以物体为中心的表示应用语义和几何扰动（如类别替换、坐标或范围破坏）来构建扭曲的3D场景图。通过对比原始和扭曲3D上下文下的预测，抑制对场景证据不敏感、可能由语言先验驱动的token

Result: 在3D-POPE和HEAL基准测试中评估，3D-VCD无需重新训练即可持续改进接地推理，证明基于结构化3D表示的推理时对比解码是提高具身智能可靠性的有效实用途径

Conclusion: 3D-VCD是首个针对3D具身智能体幻觉缓解的推理时视觉对比解码框架，通过结构化3D表示的对比解码有效提高具身智能的可靠性
...
TL;DR: InsEdit是一个基于指令的视频编辑模型，通过视觉编辑架构和互相关注机制，仅需少量视频编辑数据即可实现高质量视频编辑，同时支持图像编辑。


<details>
  <summary>Details</summary>
Motivation: 指令式视频编辑是一种自然的视频内容控制方式，但现有方法通常需要大量视频编辑数据，而高质量视频编辑数据稀缺。本文旨在探索如何在不依赖大规模视频编辑数据的情况下，将视频生成模型转化为强大的视频编辑器。

Method: 基于HunyuanVideo-1.5构建InsEdit模型，结合视觉编辑架构和基于互相关注机制的视频数据管道，创建对齐的视频对，使编辑可以从视频中间帧开始而非仅从第一帧开始。

Result: 仅使用约10万视频编辑数据，InsEdit在视频指令编辑基准测试中取得了开源方法中最先进的结果。同时，由于训练方案包含图像编辑数据，最终模型无需修改即可支持图像编辑。

Conclusion: 视频生成骨干模型可以在不需要大规模视频编辑数据的情况下成为强大的视频编辑器，InsEdit通过创新的架构和数据管道设计，在少量数据下实现了高质量的指令式视频编辑，并具备图像编辑能力。
...
Method: 基于EfficientNet-B0构建EfficientSign模型，加入两个注意力模块（通道注意力Squeeze-and-Excitation和空间注意力层聚焦手势），并与传统分类器（SVM、逻辑回归、KNN）在深度特征上对比。

Result: EfficientSign在12,637张印度手语字母图像（26类）上达到99.94%准确率，与ResNet18的99.97%相当但参数减少62%（4.2M vs 11.2M）。SVM、逻辑回归、KNN分别达到99.63%、99.03%、96.33%，均远超2015年SURF方法的92%。

Conclusion: 注意力增强的轻量学习模型为印度手语识别提供了高效可部署的解决方案，无需大规模模型或手工特征工程。
...
TL;DR: 提出了统一多模态不确定推理（UMUI）任务和CLUE方法，在文本、音频、视频等多模态中实现校准的概率估计，3B参数模型性能优于32B基线模型


<details>
  <summary>Details</summary>
Motivation: 现有不确定推理研究主要局限于文本模态，其他模态仅支持单模态二元蕴含判断，缺乏跨模态的细粒度概率推理框架

Method: 提出UMUI任务框架，创建人工标注的多模态概率评估集，开发CLUE方法结合自洽教师校准和基于分布的置信度探测

Result: 3B参数模型在所有模态上达到或优于32B参数基线模型，在音频、视觉和视听设置中均表现出色

Conclusion: UMUI任务框架和CLUE方法成功扩展了不确定推理到多模态领域，实现了跨模态的校准概率估计能力
...
TL;DR: 提出了RS-OVC，首个用于遥感图像的开放词汇计数模型，能够基于文本和/或视觉条件对训练期间未见过的物体类别进行准确计数。
...
Method: 提出了RS-OVC模型，首个用于遥感图像的开放词汇计数模型，能够基于文本和/或视觉条件对未见过的物体类别进行计数。

Result: 模型能够准确计数训练期间未见过的物体类别，仅基于文本和/或视觉条件即可实现。

Conclusion: RS-OVC解决了遥感物体计数中的开放词汇问题，为动态实时监测场景提供了更灵活的解决方案。
...
TL;DR: 该研究为虚拟试脱（VTOFF）任务建立了稳健的扩散模型架构基础，通过系统分析生成主干、条件机制和训练策略三个设计维度，在VITON-HD和DressCode数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 虚拟试穿（VTON）领域已有快速发展，但其逆问题——从穿着图像重建标准服装的虚拟试脱（VTOFF）——仍是一个研究不足的领域。本研究旨在为VTOFF建立稳健的架构基础。

Method: 基于Dual-UNet扩散模型架构，系统研究三个设计维度：1) 生成主干（比较不同Stable Diffusion变体）；2) 条件机制（消融不同掩码设计、图像条件输入方式、高层语义特征）；3) 损失函数和训练策略（评估注意力辅助损失、感知目标、多阶段课程学习）。

Result: 在VITON-HD和DressCode数据集上取得最先进性能，主要指标DISTS下降9.5%，在LPIPS、FID、KID和SSIM指标上表现优异，为未来VTOFF研究提供了强基线基准和设计洞见。

Conclusion: 该研究为虚拟试脱任务建立了稳健的扩散模型架构基础，通过系统实验揭示了不同配置选项的权衡关系，为未来VTOFF研究提供了有价值的指导。
...
TL;DR: LMGenDrive：首个结合LLM多模态理解和生成世界模型的端到端闭环驾驶框架，通过生成未来驾驶视频和控制信号，在指令跟随、时空理解和罕见场景鲁棒性方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶在长尾和开放世界场景中的泛化能力仍是主要瓶颈。现有方法要么使用LLM/VLM进行视觉语言理解和推理，要么研究生成世界模型捕捉驾驶场景的时空演化。受人类智能统一理解和想象的启发，本文探索将两者结合的自动驾驶统一模型。

Method: 提出LMGenDrive框架，结合基于LLM的多模态理解和生成世界模型，给定多视角相机输入和自然语言指令，同时生成未来驾驶视频和控制信号。采用渐进式三阶段训练策略：从视觉预训练到多步长时域驾驶，支持低延迟在线规划和自回归离线视频生成。

Result: 在具有挑战性的闭环基准测试中显著优于先前方法，在指令跟随、时空理解和罕见场景鲁棒性方面有明显提升，表明统一多模态理解和生成是更通用、鲁棒的具身决策系统的有前景方向。

Conclusion: 统一多模态理解和生成是提高自动驾驶系统泛化性和鲁棒性的有效途径，LMGenDrive框架通过结合LLM的语义先验和生成世界模型的时空建模能力，为更通用的具身决策系统提供了新方向。
...
TL;DR: 本文扩展了车牌识别基准数据集，引入新标注和改进训练方法，显著提升性能
...
Method: 1. 将原始基准数据集扩展至三倍规模，增加两个采集日；2. 修订标注并引入新标签（车牌级、车辆级、图像级）；3. 提出基于指数移动平均的损失函数和精炼学习率调度器的训练方法；4. 引入新的协议处理训练与评估分割中的相机污染问题。
...
TL;DR: SIC3D是一个可控的图像条件文本到3D生成管道，使用3D高斯泼溅技术，通过两阶段方法实现：首先生成3D对象内容，然后从参考图像转移风格。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D对象生成方法存在可控性有限和纹理模糊的问题，主要受限于文本模态的表达能力。需要更精确的控制机制来提升生成质量。

Method: 采用两阶段管道：1) 使用文本到3DGS生成模型从文本生成3D对象内容；2) 通过变分风格化分数蒸馏损失从参考图像转移风格到3DGS，并应用缩放正则化防止伪影。

Result: 实验表明SIC3D在几何保真度和风格一致性方面优于现有方法，在定性和定量评估中都表现出色。

Conclusion: SIC3D通过结合文本到3D生成和图像条件风格转移，解决了现有方法的可控性和纹理模糊问题，实现了高质量的3D对象生成。
...
TL;DR: 提出上下文对齐推理框架，通过强制异质临床证据间的一致性来改善医学视觉语言模型，减少幻觉并提高可靠性


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在放射学任务中表现良好，但常因过度依赖主导模态而产生流畅但弱基础性的结论，需要提高推理的可靠性和可信度

Method: 引入上下文对齐推理框架，在冻结的VLM基础上增强结构化上下文信号（放射组学统计、可解释性激活、词汇基础语义线索），通过上下文验证整合信号，生成包含支持证据、不确定性估计、局限性和安全说明的结构化输出

Result: 在胸部X光数据集上，上下文对齐将AUC从0.918提升到0.925，减少幻觉关键词（1.14到0.25），产生更简洁的推理解释（19.4到15.3词），同时保持校准的不确定性；跨数据集评估显示模态信息性显著影响推理行为

Conclusion: 强制多证据一致性提高了医学多模态推理的可靠性和可信度，同时保持了底层模型架构；辅助信号单独作用有限，只有通过上下文验证整合才能获得性能提升
...
TL;DR: 本文提出了首个大规模敏感内容场景图基准SenBen，并开发了一个紧凑的241M学生模型，在敏感内容检测、场景图生成和推理效率方面均优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有内容审核系统只能将图像分类为安全或不安全，但缺乏空间定位和可解释性：无法解释检测到了什么敏感行为、涉及哪些对象、发生在哪里。需要一种能够提供细粒度解释的敏感内容检测方法。

Method: 1) 创建SenBen基准：包含13,999帧电影图像，标注了Visual Genome风格的场景图（25个对象类、28个属性、14个谓词）和16个敏感标签；2) 通过多任务蒸馏方法将前沿VLM压缩为241M学生模型，采用后缀对象身份、词汇感知召回损失和解耦的Query2Label标签头等技术解决词汇不平衡问题。

Result: 学生模型在SenBen召回率上比标准交叉熵训练提升6.4个百分点；在场景图指标上，除了Gemini模型外优于所有评估的VLM和商业安全API；在所有模型中实现了最高的对象检测和图像描述分数，推理速度快7.6倍，GPU内存使用减少16倍。

Conclusion: SenBen基准填补了敏感内容场景图数据的空白，提出的紧凑学生模型在保持高性能的同时显著提升了推理效率，为可解释的内容审核系统提供了有效解决方案。
...
TL;DR: DefakeQ是首个专门为深度伪造检测器设计的量化框架，通过自适应双向压缩策略在保持检测性能的同时实现模型轻量化，支持边缘设备实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法计算密集、参数庞大，难以在资源受限的边缘设备上实时部署；同时，现有量化技术会破坏检测所需的细微伪造痕迹，导致性能显著下降。

Method: 提出DefakeQ量化框架，采用新颖的自适应双向压缩策略，同时利用特征相关性并消除冗余，在模型紧凑性和检测性能之间取得有效平衡。

Result: 在5个基准数据集和11个最先进的骨干检测器上进行广泛实验，DefakeQ始终优于现有量化和模型压缩基线方法，并在移动设备上实现实时深度伪造检测。

Conclusion: DefakeQ成功解决了深度伪造检测器在边缘设备部署的挑战，为资源受限环境下的实时媒体取证提供了实用解决方案。
...
TL;DR: BIAS是一个快速、受生物启发的动态视觉显著性检测模型，结合静态和运动信息，在连续视频流中实现毫秒级延迟的显著性检测，在交通事故分析中表现出色。


<details>
  <summary>Details</summary>
Motivation: 开发一个既具有生物合理性又计算高效的动态视觉显著性检测模型，能够实时处理连续视频流，并应用于实际场景如交通事故分析。
...
Conclusion: BIAS成功地将生物合理性与计算效率相结合，实现了可解释、高速的动态显著性检测，在真实世界应用中表现出强大的实用性。
...
TL;DR: 本文提出了一种不确定性感知方法用于基于文本的人物搜索，通过显式估计图像-文本对的不确定性，并自适应调整损失权重，充分利用弱正样本对提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的人物搜索方法通常专注于视觉和文本模态之间的严格一对一对应匹配（如对比学习），但无意中忽视了弱正样本对——即同一人物但文本描述来自不同视角（摄像头）的图像-文本对。为了充分利用这些弱正样本，需要一种方法来处理这种不确定性。

Method: 提出不确定性感知方法，包含两个模块：1）不确定性估计：获取给定正样本对的相对置信度；2）不确定性正则化：基于预测的不确定性自适应调整损失权重。此外，还引入了分组图像-文本匹配损失来进一步促进弱正样本对之间的表示空间。

Result: 在三个广泛使用的数据集（CUHK-PEDES、RSTPReid和ICFG-PEDES）上进行实验，与现有竞争方法相比，mAP分别提升了3.06%、3.55%和6.94%。

Conclusion: 提出的不确定性感知方法能够显式防止模型将潜在的弱正样本推远，有效利用了弱正样本对，显著提升了基于文本的人物搜索性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实部署中，视觉-语言大模型面临多语言多模态复合攻击的挑战：有害图像与低资源语言文本组合容易绕过为高资源语言设计的防御，暴露了当前跨语言跨模态安全方法的结构性盲点。

Method: 提出两阶段框架：1) 通过对比有害和良性输入的激活模式识别安全神经元；2) 通过梯度掩码将参数更新严格限制在该子空间内，影响少于0.03%的参数。

Result: 显著提升安全性的同时保持多语言多模态泛化能力；分析发现安全神经元在语言和模态间存在适度重叠，支持零样本跨语言跨模态安全能力迁移。

Conclusion: Precise Shield为基于神经元级、迁移的安全增强提供了新方向，通过精确识别和约束安全神经元，有效应对多语言多模态复合攻击。
...
TL;DR: HM-Bench是首个专门评估多模态大语言模型在遥感高光谱图像理解能力的基准，包含19,337个问答对，涵盖13个任务类别，采用PCA合成图像和结构化文本报告双模态评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在自然图像理解方面取得显著进展，但在高光谱图像感知和推理方面仍未被充分探索，而高光谱图像是遥感领域的重要模态。高光谱图像的高维度和复杂的光谱-空间特性给主要基于RGB数据训练的模型带来了独特挑战。

Method: 提出HM-Bench基准，包含大规模数据集（19,337个问答对，13个任务类别）。由于现有MLLM无法原生处理原始高光谱立方体数据，提出双模态评估框架：将HSI数据转换为两种互补表示形式：1）基于PCA的合成图像；2）结构化文本报告。这种框架便于系统比较不同表示对模型性能的影响。

Result: 对18个代表性MLLM的广泛评估显示，模型在处理复杂空间-光谱推理任务方面存在显著困难。结果还表明，视觉输入通常优于文本输入，强调了基于光谱-空间证据进行接地的重要性。

Conclusion: HM-Bench填补了MLLM在高光谱图像理解评估方面的空白，揭示了现有模型在复杂遥感任务中的局限性，强调了视觉表示相对于文本表示的优势，为未来研究提供了重要基准。
...
TL;DR: GeoMMBench是一个全面的遥感多模态问答基准，用于评估大语言模型在遥感领域的表现，发现现有模型存在系统缺陷，并提出了GeoMMAgent多智能体框架来提升性能。


<details>
  <summary>Details</summary>
Motivation: 遥感领域多模态大语言模型发展受限，面临领域知识广泛、传感器模态异构、任务碎片化等独特挑战，需要建立更全面的评估基准和解决方案。

Method: 1) 构建GeoMMBench多模态问答基准，覆盖遥感多个学科、传感器和任务；2) 评估36个开源和专有大语言模型；3) 提出GeoMMAgent多智能体框架，集成检索、感知和推理能力，利用领域特定的遥感模型和工具。

Result: 评估发现现有大语言模型在领域知识、感知基础和推理能力方面存在系统缺陷；GeoMMAgent显著优于独立的大语言模型，证明了工具增强智能体在解决复杂遥感挑战中的重要性。

Conclusion: 需要更全面的基准来评估遥感多模态大语言模型，而工具增强的多智能体框架是提升遥感领域AI性能的有效途径，能够动态应对复杂的地球科学和遥感挑战。
...
TL;DR: UniDG是一个通用的缺陷生成基础模型，通过大规模数据集和创新的缺陷上下文编辑方法，解决了现有缺陷生成方法泛化性差、真实感不足的问题，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有缺陷生成方法依赖少样本学习，由于缺乏大规模配对的缺陷编辑数据，容易过拟合到特定缺陷类别。缺陷尺度和形态的巨大变化进一步限制了泛化能力、降低了真实感和类别一致性。

Method: 1) 构建UDG数据集：包含30万个正常-异常-掩码-描述四元组；2) UniDG模型：支持基于参考的缺陷生成和基于文本指令的缺陷编辑，无需逐类别微调；3) 缺陷上下文编辑：自适应缺陷裁剪和结构化双联画输入格式；4) MM-DiT多模态注意力融合参考和目标条件；5) 两阶段训练策略：多样性SFT后接一致性RFT。

Result: 在MVTec-AD和VisA数据集上的实验表明，UniDG在合成质量和下游单类/多类异常检测/定位任务上优于现有的少样本异常生成和图像插入/编辑基线方法。

Conclusion: UniDG通过大规模数据集和创新的缺陷上下文编辑方法，解决了现有缺陷生成方法的局限性，在缺陷生成质量和下游任务性能上取得了显著提升，为通用缺陷生成提供了有效的解决方案。
...
TL;DR: MV3DIS是一个从粗到精的零样本3D实例分割框架，通过引入3D先验和多视图一致性匹配，解决了现有方法因独立处理单帧和仅依赖2D指标导致的3D分割碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D实例分割方法依赖人工标注的3D监督训练，限制了可扩展性和对新物体的泛化能力。现有基于SAM的方法独立处理每帧且仅依赖2D指标，忽略了多视图关联和3D先验，导致跨视图2D掩码不一致和3D分割碎片化。

Method: 提出MV3DIS框架：1）3D引导掩码匹配策略，使用粗3D分割作为公共参考来匹配跨视图2D掩码，通过3D覆盖分布整合多视图一致性；2）在一致2D掩码指导下进一步细化粗3D分割为精确3D实例；3）深度一致性加权方案，量化投影可靠性以抑制物体间遮挡带来的模糊性。
...
TL;DR: TAIHRI是一个专为近距离人机交互设计的视觉语言模型，通过量化3D关键点到有限交互空间，实现任务相关身体部位的精确度量级空间定位。


<details>
  <summary>Details</summary>
Motivation: 传统3D人体关键点估计方法主要关注相对于根关节的整体重建质量，但在实际人机交互场景中，机器人更关心在自我中心相机3D坐标系下任务相关身体部位的精确度量级空间定位。
...
Conclusion: TAIHRI为具身人机交互领域开辟了新的研究方向，能够理解用户的运动指令并将机器人的注意力引导到最任务相关的关键点上。
...
Method: 提出M-IDoL模型，采用信息分解学习框架：1）最大化模态间熵，通过将多模态表示分散到可分离的专家混合子空间，实现跨模态表示特异性；2）最小化模态内不确定性，在每个MoE子空间内进行细粒度语义区分，丰富每个模态的表示多样性。

Result: 在115万张医学图像上预训练后，M-IDoL在21个下游临床任务上表现出卓越的泛化能力，在5种成像模态（X光、眼底、OCT、皮肤镜和病理）上优于20个基础模型；学习到的表示具有模态特异性且多样性丰富，特征簇在模态间分离更清晰，模态内特征区分更细粒度。

Conclusion: M-IDoL通过信息分解学习框架有效解决了医学基础模型中的信息模糊问题，实现了更好的模态特异性和多样性，在多种临床任务上展现出优越的泛化性能。
...
TL;DR: MASS方法通过可变形2D高斯Surfel表示，从单目第一人称视频重建高保真3D手部，解决了现有方法在几何分辨率、手物交互和计算成本方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目第一人称视频重建高保真3D手部时面临三个主要挑战：1) 高分辨率几何捕获能力有限；2) 难以处理手物交互和复杂手持物体；3) 计算成本高，不适合实时应用。

Method: 提出Mesh-inellipse Aligned deformable Surfel Splatting (MASS)方法：1) 使用网格对齐的Steiner内切椭圆和分形致密化进行网格到Surfel转换，从粗糙参数化手部网格初始化高分辨率2D高斯Surfel；2) 提出高斯Surfel变形，通过预测Surfel属性残差更新和引入不透明度掩码来建模手部变形和个性化特征；3) 采用两阶段训练策略和新型绑定损失来提高优化鲁棒性和重建质量。

Result: 在ARCTIC数据集、Hand Appearance数据集和Interhand2.6M数据集上的广泛实验表明，该方法相比最先进方法实现了更优的重建性能。

Conclusion: MASS方法通过创新的可变形2D高斯Surfel表示，有效解决了单目第一人称视频中高保真3D手部重建的挑战，在保持高质量重建的同时降低了计算成本。
...
TL;DR: TouchAnything框架利用预训练的大规模2D视觉扩散模型作为语义和几何先验，从稀疏触觉测量中实现3D重建，仅需少量触摸即可重建准确几何形状。


<details>
  <summary>Details</summary>
Motivation: 视觉在遮挡或挑战性光照条件下变得不可靠，而触觉传感通过物理接触提供直接几何信息。然而，仅从稀疏局部触摸重建全局3D几何形状本质上是欠约束的。

Method: 利用预训练的大规模2D视觉扩散模型作为语义和几何先验，将几何知识从视觉域转移到触觉域。给定稀疏接触约束和对象的粗略类别描述，将重建表述为优化问题，强制触觉一致性同时引导解决方案朝向与扩散先验一致的形状。

Result: 该方法仅需少量触摸即可重建准确几何形状，优于现有基线方法，并能够实现先前未见对象实例的开放世界3D重建。

Conclusion: TouchAnything框架成功地将预训练视觉扩散模型的几何知识转移到触觉域，实现了从稀疏触觉测量中进行准确3D重建，为在视觉受限场景下的物体几何感知提供了有效解决方案。
...
TL;DR: 视觉语言模型在遥感影像上的直接提示方法效果不佳，少量监督微调即可显著超越零样本性能


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在遥感影像上的适应性问题，特别是测试"通过领域特定语言提示可以引导冻结模型完成专业任务"这一假设的有效性

Method: 在CloudSEN12+云分割基准上使用CLIPSeg模型，评估60种提示变体（包括简单标签、领域术语、外观描述符和上下文线索），并与监督微调（包括全微调和低秩适应）进行对比

Result: 所有提示变体都低于零样本基线（0.255 mIoU），工程化提示最低只有0.07 mIoU；仅需0.1%标记数据（约8张图像）的监督微调即可超越零样本性能，5-10%数据可达到最大mIoU的85%；全微调始终优于低秩适应

Conclusion: 对于将视觉语言模型适应到专业影像领域，标记数据不是昂贵的替代方案，而是值得投入的路径；语言提示无法弥补自然图像表示与卫星光谱影像之间的分布差异
...
TL;DR: 提出了一种用于卫星图像语义分割的动态类感知不确定性主动学习方法（DCAU-AL），通过实时跟踪各类别性能差距并动态调整采样权重，有效解决类别不平衡问题，显著减少标注成本同时保持高模型性能。


<details>
  <summary>Details</summary>
Motivation: 卫星图像语义分割对土地覆盖制图和环境监测至关重要，但大规模高分辨率卫星数据标注成本高昂。传统主动学习方法依赖全局不确定性或多样性度量，缺乏针对训练过程中表现不佳或稀有类别的适应性，导致系统偏差。

Method: 提出DCAU-AL方法，这是一种自适应获取函数，基于实时类别性能差距优先选择样本。该方法持续跟踪每个类别的分割性能，并在整个主动学习过程中动态调整采样权重，重点关注表现不佳或代表性不足的类别。

Result: 在OpenEarth土地覆盖数据集上的大量实验表明，DCAU-AL显著优于现有主动学习方法，特别是在严重类别不平衡情况下，提供了优越的每类IoU和改善的标注效率。

Conclusion: DCAU-AL通过动态调整采样策略有效解决了主动学习中的类别不平衡问题，为大规模卫星图像分割应用提供了一种高效且适应性强的解决方案，显著减少了标注成本同时保持了高分割精度。
...
TL;DR: ActFER：一个基于多模态大语言模型的主动式面部表情识别框架，通过动态调用工具进行面部检测、对齐和局部区域放大，结合视觉思维链进行推理，使用UC-GRPO强化学习算法优化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于MLLM的面部表情识别方法采用被动范式，依赖外部准备的面部输入并进行单次推理，缺乏主动感知能力。需要开发能够主动获取视觉证据并进行多模态推理的框架。

Method: 提出ActFER框架，将FER重新定义为主动视觉证据获取+多模态推理。动态调用面部检测和对齐工具，选择性放大信息丰富的局部区域，通过视觉思维链对面部动作单元和情绪进行推理。开发UC-GRPO强化学习算法，使用基于AU的多层次可验证奖励、查询条件对比效用估计和情绪感知EMA校准。

Result: ActFER在UC-GRPO训练下持续优于被动式MLLM-based FER基线方法，显著提高了AU预测准确率。

Conclusion: ActFER通过主动视觉证据获取和强化学习优化，实现了更准确的面部表情识别，将FER从被动标签预测推向主动感知推理的新范式。
...
TL;DR: PinpointQA是首个针对室内视频中小物体空间理解的数据集和基准测试，包含1,024个场景和10,094个问答对，旨在评估模型能否精确定位视频中的目标物体。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在室内视频的小物体空间理解方面存在显著挑战，尽管这对物体搜索和辅助应用具有实际价值。现有基准测试虽然推动了视频空间智能、具身推理和诊断感知，但没有直接评估模型能否精确定位视频中的目标物体并为下游应用提供足够精确的位置信息。

Method: 基于ScanNet++和ScanNet200构建PinpointQA数据集，包含1,024个场景和10,094个问答对。数据集组织为四个渐进式挑战任务：目标存在验证、最近参考识别、细粒度空间描述和结构化空间预测。使用中间空间表示自动生成问答对，并通过质量控制进一步优化。

Result: 在代表性MLLMs上的实验显示，在渐进任务链中存在一致的能力差距，其中结构化空间预测任务尤其困难。在PinpointQA上进行监督微调能带来显著提升，特别是在更难的任务上，表明PinpointQA既可作为诊断基准，也可作为有效的训练数据集。

Conclusion: PinpointQA是首个针对室内视频中小物体空间理解的数据集和基准测试，填补了现有评估体系的空白。它不仅能有效诊断模型在空间理解方面的能力差距，还能通过监督微调显著提升模型性能，为物体搜索和辅助应用等下游任务提供支持。
...
TL;DR: Matrix-Game 3.0是一个内存增强的交互式世界模型，实现了720p实时长视频生成，通过数据、模型和推理的系统性改进，在5B参数下达到40 FPS，并在分钟级序列中保持稳定的内存一致性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在交互式视频生成中难以同时实现内存支持的长时态一致性和高分辨率实时生成，限制了其在现实场景中的应用。

Method: 1. 升级工业级无限数据引擎，整合虚幻引擎合成数据、AAA游戏自动采集和真实视频增强，生成高质量四元组数据；2. 提出长时态一致性训练框架，通过建模预测残差和重新注入不完美生成帧实现自校正，结合相机感知内存检索和注入实现长时时空一致性；3. 设计基于分布匹配蒸馏的多段自回归蒸馏策略，结合模型量化和VAE解码器剪枝实现高效实时推理。

Result: Matrix-Game 3.0在5B模型下实现720p分辨率40 FPS实时生成，在分钟级序列中保持稳定内存一致性。扩展到2x14B模型进一步提升了生成质量、动态性和泛化能力。

Conclusion: 该方法为工业级可部署世界模型提供了实用路径，通过系统性改进在数据、模型和推理三个层面解决了长时态一致性与实时高分辨率生成的平衡问题。
...
TL;DR: 提出自适应医疗AI框架，通过多指标特征分析和不确定性门控机制，在数据漂移时自动决定何时重新训练，保持肾小球病理图像分类性能稳定。


<details>
  <summary>Details</summary>
Motivation: 自适应医疗AI模型在动态临床环境中常因数据漂移导致性能下降，需要解决数据偏移和灾难性遗忘问题，实现持续学习。

Method: 三阶段方法：1）使用欧几里得、余弦、马氏距离等多指标特征分析评估新图像与训练分布的统计相似性；2）基于蒙特卡洛dropout的不确定性门控筛选低预测熵的图像；3）在严格性能保障下（指标下降不超过5%）对筛选后的图像进行增量式重新训练。

Result: 在ResNet18集成模型和多中心数据集实验中，该框架防止了性能退化：添加新图像后AUC保持约0.92，准确率约89%，无明显变化。

Conclusion: 该方法有效解决了医疗影像AI中的数据偏移问题，避免了灾难性遗忘，实现了持续学习能力。
...
TL;DR: BlendFusion是一个从3D场景生成高质量合成图像-文本数据集的框架，通过路径追踪和对象中心相机放置策略解决扩散模型生成数据的视觉不一致性和模型崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的合成数据存在视觉不一致问题，使用这些数据训练模型会导致自噬反馈循环和模型崩溃（MAD）。需要一种更可靠的合成数据生成方法。

Method: 提出BlendFusion框架，使用路径追踪从3D场景生成图像，采用对象中心相机放置策略、鲁棒过滤机制和自动标注技术，创建高质量图像-文本对数据集FineBLEND。

Result: 创建了FineBLEND数据集，实证分析显示其质量优于现有图像-文本数据集，对象中心相机放置策略比对象无关采样方法更有效。

Conclusion: BlendFusion提供了一个可扩展、可配置的开源框架，使社区能够从3D场景创建自己的高质量数据集，避免了扩散模型生成数据的缺陷。
...
<details>
  <summary>Details</summary>
Motivation: 当前汽车制造质量评估中的视觉异常检测方法都是任务特定的，缺乏统一的多任务评估基准，阻碍了多任务视觉异常检测的发展。
...
Conclusion: CAD数据集填补了汽车相关多任务视觉异常检测领域的基准空白，为未来研究提供了标准化评估平台，同时揭示了多任务学习在促进知识转移和任务交互方面的潜力及挑战。
...
TL;DR: ImageProtector是一种用户端图像保护方法，通过在图像中嵌入几乎不可见的扰动，诱导多模态大语言模型生成拒绝响应，从而防止敏感信息被提取。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在分析互联网图像数据时存在安全隐患，特别是开放权重的MLLMs可能被滥用来大规模提取个人图像中的敏感信息（如身份、位置等私人细节）。

Method: 提出ImageProtector方法，在用户分享图像前嵌入精心设计的、几乎不可见的扰动，这种扰动作为视觉提示注入攻击作用于MLLMs，诱导模型生成拒绝响应。

Result: 在6个MLLMs和4个数据集上验证了ImageProtector的有效性。评估了三种潜在对抗措施（高斯噪声、DiffPure和对抗训练），发现它们虽然能部分缓解ImageProtector的影响，但同时会降低模型准确性和/或效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决实际HDR成像中的关键挑战：在动态场景、光照变化和手持相机抖动条件下，如何有效融合多曝光图像并消除错位和重影伪影。

Method: 建立包含100个训练序列（7个曝光级别）和100个测试序列（5个曝光级别）的基准数据集，通过PSNR、SSIM和LPIPS指标评估参赛方法，同时考虑感知质量、效率和可复现性。

Result: 吸引了114个团队参与，收到987份提交，获胜方法显著提升了去除多曝光融合伪影和恢复细节的能力。

Conclusion: 该挑战赛为动态场景多曝光图像融合提供了重要的基准测试平台，推动了HDR成像技术的发展，相关数据集和代码已开源。
...
TL;DR: SiMing-Bench是首个评估多模态大语言模型在完整临床技能视频中进行过程级判断能力的基准，专注于跟踪交互驱动的状态更新如何影响整个工作流程的程序正确性。
...
Result: 测试多种开源和闭源MLLMs发现，模型与医生判断的一致性普遍较弱；即使在整体程序级相关性看似可接受时，对评分标准定义的中间步骤的判断能力仍然薄弱，表明粗粒度全局评估严重高估了当前模型的程序判断能力。

Conclusion: 当前MLLMs在程序判断方面的瓶颈不仅在于细粒度评分或时间定位，而在于建模连续交互如何随时间更新程序状态的能力不足。
...
TL;DR: 该论文提出了一种数据集级别的物体中心监督方案，用于在3D高斯泼溅中学习物体表示，通过预训练的槽注意力模块学习场景无关的物体码本，实现跨视角和跨场景的一致物体识别，无需额外的掩码处理或多视角对齐。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基础模型的3D场景理解方法存在以下问题：1）监督信号不是真正的物体中心；2）需要额外的掩码预处理/后处理；3）需要专门的训练和损失设计来解决跨视角的掩码身份冲突；4）学习的3D场景身份是场景依赖的，限制了跨场景的泛化能力。

Method: 基于预训练的槽注意力全局物体中心学习模块，学习一个场景无关的物体码本，提供跨视角和跨场景的一致、身份锚定的表示。将码本与模块的无监督物体掩码结合，直接监督3D高斯的身份特征，无需额外的掩码处理或显式的多视角对齐。

Result: 该方法能够学习场景无关的码本，实现物体监督和识别，无需每个场景的微调或重新训练。将无监督物体中心学习引入3D高斯泼溅，产生更结构化的表示，并在下游任务（如机器人交互、场景理解和跨场景泛化）中表现更好。

Conclusion: 提出的数据集级别物体中心监督方案解决了现有方法在物体身份一致性和跨场景泛化方面的局限性，通过场景无关的物体码本实现了更有效的3D物体表示学习，为下游应用提供了更好的基础。
...
TL;DR: 该研究提出了SIA-RAPN基准，用于评估机器人辅助肾部分切除术中肾缝合阶段的细粒度动作分割，比较了四种时序模型在50个临床视频上的性能。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助肾部分切除术中肾缝合阶段的细粒度动作分割面临视觉相似手势、可变持续时间和严重类别不平衡的挑战，需要建立标准化基准来评估时序模型性能。

Method: 使用50个da Vinci Xi系统采集的临床视频，标注12个帧级标签，构建SIA-RAPN基准。比较四种基于I3D特征的时序模型：MS-TCN++、AsFormer、TUT和DiffAct。评估指标包括平衡准确率、编辑分数、分段F1分数（10%、25%、50%重叠阈值）、帧级准确率和帧级平均精度。

Result: 在五个不同数据划分配置下，DiffAct在F1分数、帧级准确率、编辑分数和帧级mAP方面表现最佳，而MS-TCN++在平衡准确率方面表现最好。基准还报告了在单端口RAPN数据集上的跨域结果。

Conclusion: SIA-RAPN基准为机器人辅助肾部分切除术中肾缝合阶段的细粒度动作分割提供了标准化评估框架，DiffAct模型在多个关键指标上表现最优，但不同模型在不同评估指标上各有优势。
...
TL;DR: Tora3是一个基于轨迹引导的音视频生成框架，通过使用物体轨迹作为共享的运动学先验，显著提升了音视频生成中的物理一致性和运动-声音同步质量。


<details>
  <summary>Details</summary>
Motivation: 现有的音视频生成方法在感知质量和多模态一致性方面取得了进展，但在生成具有合理运动-声音关系的音视频内容方面仍然面临挑战。现有方法通常产生视觉上不稳定的物体运动，以及仅与显著运动或接触事件松散对齐的声音，主要原因是缺乏视频和音频生成共享的显式运动感知结构。

Method: Tora3采用轨迹引导的音视频生成框架，使用物体轨迹作为共享的运动学先验。具体包括：1) 设计轨迹对齐的运动表示用于视频生成；2) 开发基于轨迹导出的二阶运动学状态驱动的运动-音频对齐模块；3) 提出混合流匹配方案，在轨迹条件区域保持轨迹保真度，同时在其它区域保持局部一致性。此外，还构建了PAV大规模音视频数据集，强调运动相关模式并包含自动提取的运动标注。

Result: 大量实验表明，Tora3在运动真实性、运动-声音同步性和整体音视频生成质量方面均优于强大的开源基线方法。

Conclusion: Tora3通过将物体轨迹作为共享的运动学先验来联合引导视觉运动和声学事件，显著提升了音视频生成中的物理一致性和运动-声音关系质量，为解决现有方法在运动-声音对齐方面的挑战提供了有效解决方案。
...
Method: 1) 使用动作导出的可行轨迹指导生成下一帧图像，捕捉丰富的时空线索；2) 对自生成的未来想象帧进行推理以优化预测轨迹；3) 创建nuScenes-GR-20K数据集；4) 采用三阶段训练策略（预训练、监督微调、强化学习）

Result: 在规划和未来生成基准测试中，VLA-World持续超越最先进的VLA和世界模型基线

Conclusion: VLA-World通过统一预测想象与反思推理，实现了更好的驾驶预见性、更高性能和更好的可解释性
...
TL;DR: 利用配对的空间转录组和H&E数据，通过跨模态蒸馏将转录组衍生的组织结构转移到仅需组织学的模型，实现仅用H&E图像就能预测组织微环境


<details>
  <summary>Details</summary>
Motivation: 空间转录组学能提供丰富的组织分子信息，但成本高且稀缺；而H&E组织学图像虽然丰富但信号不够精细。需要一种方法将转录组学衍生的组织结构转移到更易获取的组织学数据上

Method: 提出跨模态蒸馏框架，利用配对的空间转录组和H&E数据训练模型，将转录组学衍生的组织结构知识转移到仅基于组织学的模型中

Result: 蒸馏模型在多种组织类型和疾病背景下，比基于形态学的无监督基线方法更接近转录组学衍生的组织结构，并能恢复有生物学意义的细胞类型组成

Conclusion: 该框架在训练时利用配对数据，推理时仅需H&E组织学图像，无需转录组输入，为大规模组织分析提供了实用解决方案
...
TL;DR: 提出一种多模态物理基础方法，用于在严重手部遮挡下进行度量尺度的无模态物体重建和姿态估计，通过结合视觉、本体感觉和触觉信号来减少遮挡区域的模糊性。
...
Method: 使用姿态感知、相机对齐的有符号距离场(SDF)表示物体结构，通过Structure-VAE学习紧凑潜在空间。在潜在空间中训练条件流匹配扩散模型，先在纯视觉图像上预训练，然后在遮挡操作场景上微调，条件包括可见RGB证据、遮挡/可见性掩码、手部潜在表示和触觉信息。微调和推理过程中加入基于物理的目标和可微分解码器引导，减少手-物体穿透并对齐重建表面与接触观测。

Result: 模拟实验表明，添加本体感觉和触觉显著改善了遮挡下的完成度，相比纯视觉基线产生了物理上合理的重建结果，并保持正确的真实世界尺度。模型成功部署到真实人形机器人上，即使末端执行器与训练时不同也能有效迁移。

Conclusion: 多模态物理基础方法通过结合视觉、本体感觉和触觉信号，能够在严重手部遮挡下实现度量尺度的无模态物体重建和姿态估计，产生物理一致的结构估计，自然集成到现有两阶段重建流程中。
...
TL;DR: FIRE-CIR是一个用于时尚图像检索的模型，通过问题驱动的视觉推理来提高检索准确性和可解释性，超越了仅依赖嵌入相似性的传统方法。
...
Result: 在Fashion IQ基准测试中，FIRE-CIR在检索准确性上超越了最先进的方法，同时提供了可解释的属性级检索决策洞察。

Conclusion: FIRE-CIR通过引入显式组合推理机制，显著提升了时尚组合图像检索的性能和可解释性，为细粒度视觉推理任务提供了新思路。
...
### [48] [Vision Transformers for Preoperative CT-Based Prediction of Histopathologic Chemotherapy Response Score in High-Grade Serous Ovarian Carcinoma](https://arxiv.org/abs/2604.09197)
...
TL;DR: 开发基于CT影像和临床数据的深度学习模型，用于术前预测卵巢癌患者对新辅助化疗的反应评分（CRS），作为多学科团队决策的辅助工具。


<details>
  <summary>Details</summary>
Motivation: 高级别浆液性卵巢癌（HGSOC）具有显著的生物学和空间异质性，常被晚期诊断。新辅助化疗（NACT）后延迟初次手术常用于不适合初次肿瘤细胞减灭术的患者。化疗反应评分（CRS）是验证的NACT反应组织病理学生物标志物，但只能在术后获得。本研究旨在探索是否可以使用治疗前CT影像和临床数据预测CRS，作为多学科团队讨论预期治疗反应的决策支持辅助工具。

Method: 提出2.5D多模态深度学习框架，使用预训练的Vision Transformer编码器处理病灶密集的大网膜切片，并通过中间融合模块将视觉表征与临床变量整合，以预测CRS。

Result: 内部测试队列（IEO，41例患者）中，整合影像和临床数据的多模态模型达到ROC-AUC 0.95、准确率95%、精确率80%。外部测试集（OV04，70例患者）中，达到ROC-AUC 0.68、准确率67%、精确率75%。

Conclusion: 初步结果表明，使用常规临床数据和CT影像，基于Transformer的深度学习在术前预测HGSOC的CRS是可行的。作为一种研究性的治疗前决策支持工具，该方法可通过提供早期、非侵入性的治疗反应估计来辅助多学科团队讨论。
...
TL;DR: OpenPAE是首个开源的N-shot个性化年龄估计基准，通过利用同一人的多张参考图像来提升年龄估计精度，超越了传统将人脸视为独立样本的方法。


<details>
  <summary>Details</summary>
Motivation: 现有年龄估计方法将每个人脸视为独立样本，学习从外观到年龄的全局映射，忽略了由于遗传、生活方式和健康等因素导致的个体老化速率差异。当有同一人的已知年龄参考图像时，可以利用这些上下文信息进行个性化估计。

Method: 建立了层次化的基线方法：从算术偏移、闭式贝叶斯线性回归，到条件注意力神经过程。引入了OpenPAE基准，包含严格的评估协议和N-shot个性化年龄估计框架。

Result: 实验表明个性化方法能持续提升性能，这种提升不仅仅是领域适应，非线性方法显著优于简单替代方案。发布了所有模型、代码、协议和评估分割。

Conclusion: OpenPAE为个性化年龄估计提供了首个开源基准，证明了利用同一人的多张参考图像进行个性化估计的有效性，为未来研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 边缘和移动设备上的人脸识别需要满足严格的延迟、内存和能耗约束，同时保持可靠的准确性。尽管现有的混合CNN-Transformer架构改进了全局上下文建模，但在识别性能和计算效率之间找到有效平衡仍然是一个开放挑战。

Method: 提出了FaceLiVTv2改进架构，核心是Lite MHLA（轻量级全局令牌交互模块），用多头线性令牌投影和仿射重缩放变换替代原始的多层注意力设计，减少冗余同时保持多头间的表示多样性。进一步将Lite MHLA集成到统一的RepMix块中，协调局部和全局特征交互，并在嵌入阶段采用全局深度卷积进行自适应空间聚合。

Result: 在LFW、CA-LFW、CP-LFW、CFP-FP、AgeDB-30和IJB等基准测试中，FaceLiVTv2相比现有轻量级方法持续改善了精度-效率平衡。相比FaceLiVTv1减少22%移动推理延迟，相比GhostFaceNets在移动设备上加速达30.8%，相比EdgeFace和KANFace在不同平台上实现20-41%延迟改进，同时保持更高的识别准确率。
...
### [51] [Visually-Guided Policy Optimization for Multimodal Reasoning](https://arxiv.org/abs/2604.09349)
*Zengbin Wang,Feng Xiong,Liang Lin,Xuecai Hu,Yong Wang,Yanlin Wang,Man Zhang,Xiangxiang Chu*

Main category: cs.CV

TL;DR: VGPO框架通过视觉注意力补偿和双粒度优势重加权策略，增强视觉语言模型在推理过程中的视觉关注度，解决视觉遗忘问题，提升多模态推理性能。
...
Method: 提出视觉引导策略优化框架，包含：1）视觉注意力补偿机制，利用视觉相似性定位和放大视觉线索，同时在后续步骤中逐步提高视觉期望以对抗视觉遗忘；2）双粒度优势重加权策略，在轨迹内层面突出具有相对高视觉激活的标记，在轨迹间层面优先考虑展示优越视觉累积的轨迹。

Result: 大量实验表明，VGPO在数学多模态推理和视觉依赖任务中实现了更好的视觉激活和更优越的性能。

Conclusion: VGPO通过增强视觉语言模型在策略优化过程中的视觉关注，有效解决了视觉遗忘问题，提升了多模态推理的视觉忠实度和整体性能。
...
TL;DR: SATO提出了一种基于三角形条带的新型token排序策略，用于生成艺术家级质量的网格，通过编码UV边界保持边缘流和结构规律性，支持三角形和四边形网格的统一表示。


<details>
  <summary>Details</summary>
Motivation: 现有自回归变换器生成网格的方法中，基于坐标排序产生过长序列效率低下，而基于面片启发式方法破坏了高质量建模所需的连续边缘流和结构规律性，无法满足专业艺术家标准。

Method: 提出SATO框架，采用受三角形条带启发的token排序策略，将序列构建为显式编码UV边界的连接面链，保持艺术家创建网格的组织边缘流和语义布局，支持三角形和四边形网格的统一表示。

Result: SATO在几何质量、结构连贯性和UV分割方面持续优于先前方法，通过联合训练大规模三角形数据（提供结构先验）和高质量四边形数据（增强几何规律性）获得更好效果。

Conclusion: SATO通过创新的三角形条带token排序策略，解决了现有网格生成方法在边缘流保持和结构规律性方面的不足，实现了艺术家级质量的网格生成，支持三角形和四边形网格的统一处理。
...
TL;DR: 研究发现视觉语言模型存在编码-接地分离现象：即使模型给出错误答案，其视觉编码能力与正确答案模型同样强，问题在于视觉证据与先验知识的仲裁机制，而非感知能力本身。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型在回答问题时，当看到蓝色香蕉却回答"黄色"时，问题究竟是感知能力不足还是仲裁机制问题。通过研究不同规模的VLMs，揭示模型内部视觉信息处理与决策机制的关系。

Method: 使用多模态仲裁交叉分析（MAC）和逐层Logit Lens探测技术，追踪视觉信号与先验知识在模型各层的竞争关系。通过全序列激活修补建立因果关系，并采用训练免费的激活引导技术（线性和稀疏自编码器引导）进行干预。

Result: 研究发现：1）视觉属性在早期层即可线性解码（AUC>0.86）；2）成功与失败样本的编码准确率几乎相同；3）最终层logit差距而非编码强度能更好预测接地结果；4）全序列激活修补可改变60-84%的输出，而标准LLM解释性干预无效；5）图像token承担几乎所有因果影响，文本token无影响；6）早期层激活引导可将视觉接地提升+3.8%。

Conclusion: 视觉语言模型已经具备良好的视觉感知能力，主要挑战在于如何基于所见内容进行决策。编码-接地分离现象表明问题在于仲裁机制而非感知能力，针对性干预可以帮助弥合这一差距。
...
TL;DR: GREATEN是一个用于合成到真实零样本立体匹配的框架，通过引入表面法线作为领域不变几何线索，结合门控上下文-几何融合、镜面透明增强和稀疏注意力设计，显著提升了跨域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 合成到真实零样本立体匹配面临跨域偏移和图像纹理固有模糊性的挑战，特别是在遮挡、无纹理、重复和非朗伯（镜面/透明）区域。现有方法主要依赖图像纹理，在这些区域表现不佳。

Method: 1. 门控上下文-几何融合（GCGF）模块：自适应抑制图像特征中不可靠的上下文线索，并将其与法线驱动的几何特征融合，构建领域不变且具有区分性的上下文-几何表示。
2. 镜面透明增强（STA）策略：提高GCGF对非朗伯区域误导性视觉线索的鲁棒性。
3. 稀疏注意力设计：包括稀疏空间注意力（SSA）、稀疏双匹配注意力（SDMA）和简单体积注意力（SVA），在保持细粒度全局特征提取能力的同时大幅降低计算开销。

Result: 在仅使用合成数据（如SceneFlow）训练的情况下，GREATEN-IGEV在多个真实数据集上取得了出色的性能：在ETH3D上误差降低30%，在非朗伯Booster数据集上降低8.5%，在KITTI-2015上降低14.1%。同时运行速度比GREAT-IGEV快19.2%，支持Middlebury的高分辨率（3K）推理，视差范围可达768。

Conclusion: GREATEN通过引入表面法线作为领域不变的几何线索，有效解决了合成到真实零样本立体匹配中的跨域泛化问题。该框架在多个真实数据集上显著优于现有方法，同时保持了计算效率，为跨域立体匹配提供了有效的解决方案。
...
TL;DR: VL-Calibration：一个强化学习框架，将大型视觉语言模型的置信度分解为视觉置信度和推理置信度，通过视觉不确定性估计和token级优势重加权来减少幻觉并提升校准效果


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态推理中表现出色，但经常以高置信度产生幻觉和错误响应，这阻碍了其在高风险领域的应用。现有的基于文本LLM的置信度校准方法通常使用二元答案级正确性优化单一整体置信度，这与LVLMs不匹配：错误预测可能源于感知失败或正确感知下的推理错误，而单一置信度混淆了这些来源，且视觉不确定性常被语言先验主导。

Method: 提出VL-Calibration强化学习框架，将置信度明确分解为视觉置信度和推理置信度。在没有真实感知标签的情况下，引入内在视觉确定性估计，结合：(i) 通过图像扰动下的KL散度测量的视觉基础；(ii) 通过token熵测量的内部确定性。进一步提出token级优势重加权，基于视觉确定性聚焦优化token，抑制无基础的幻觉同时保留有效感知。

Result: 在13个基准测试上的实验表明，VL-Calibration有效改善了校准效果，同时提升了视觉推理准确性，并且能够泛化到分布外基准测试，适用于不同模型规模和架构。

Conclusion: VL-Calibration通过将置信度分解为视觉和推理组件，并引入视觉不确定性估计和token级优化，成功解决了LVLMs中的幻觉和错误置信问题，为高风险应用提供了更可靠的模型。
...
### [56] [PhysInOne: Visual Physics Learning and Reasoning in One Suite](https://arxiv.org/abs/2604.09415)
...
<details>
  <summary>Details</summary>
Motivation: 现有物理基础训练数据集规模有限（仅数百或数千个示例），无法满足AI系统学习复杂物理动态的需求，迫切需要大规模、多样化的物理基础数据集。

Method: 构建包含153,810个动态3D场景的合成数据集，涵盖力学、光学、流体动力学和磁学等71种基本物理现象，提供多物体交互和复杂背景，并包含3D几何、语义、动态运动、物理属性和文本描述等全面标注。

Result: 在四个新兴应用上验证了PhysInOne的有效性：物理感知视频生成、长/短期未来帧预测、物理属性估计和运动迁移。实验表明，在PhysInOne上微调基础模型能显著提高物理合理性，同时暴露了建模复杂物理动态和估计内在属性的关键差距。
...
TL;DR: 该论文提出了一种基于物理的退化模型和训练策略，用于消除夜间摄影中的光污染，恢复原始夜空景象。


<details>
  <summary>Details</summary>
Motivation: 城市环境中普遍存在的人工照明导致严重的光污染，产生天空辉光、光晕和发光伪影，掩盖了星星和天体，破坏了原始夜空景象。现有夜间去雾方法主要关注细节可读性，而光污染消除的目标是恢复原始夜间外观。

Method: 1. 提出基于物理的退化模型，在先前夜间去雾模型基础上增加了两个关键方面：(i) 定向光源的各向异性扩散，(ii) 天际线后不可见表面光源引起的天空辉光。2. 构建训练策略，利用大型生成模型和合成-真实耦合来弥补配对真实数据的稀缺性并增强泛化能力。

Result: 大量实验表明，所提出的公式和学习框架显著减少了光污染伪影，比先前的夜间恢复方法更好地恢复了真实的夜间图像。

Conclusion: 该研究通过物理建模和创新的训练策略，有效解决了夜间摄影中的光污染问题，为恢复原始夜空景象提供了更优的解决方案。
...
TL;DR: VisionFoundry是一个任务感知的合成数据生成管道，仅需任务名称即可生成合成监督数据，显著提升视觉语言模型在空间理解和视角识别等视觉感知任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在空间理解和视角识别等视觉感知任务上表现不佳，主要原因是自然图像数据集对这些低层次视觉技能提供的监督有限。作者希望探索通过针对性的合成监督能否解决这一问题。

Method: 提出VisionFoundry任务感知合成数据生成管道：仅输入任务名称，使用大语言模型生成问题、答案和文本到图像提示，然后用文本到图像模型合成图像，最后用专有视觉语言模型验证一致性。整个过程无需参考图像或人工标注。

Result: 构建了VisionFoundry-10K合成视觉问答数据集（包含10k个图像-问题-答案三元组，涵盖10个任务）。在该数据集上训练的模型在视觉感知基准上取得显著提升：MMVP上提升7%，CV-Bench-3D上提升10%，同时保持更广泛的能力，并显示出随着数据量增加的良好扩展性。

Conclusion: 有限的针对任务的监督是视觉语言模型瓶颈的重要影响因素，合成监督是朝着更系统化训练视觉语言模型的有前景的路径。
...
### [59] [Rays as Pixels: Learning A Joint Distribution of Videos and Camera Trajectories](https://arxiv.org/abs/2604.09429)
...
TL;DR: Rays as Pixels是一个视频扩散模型，通过将相机表示为密集光线像素，联合去噪视频帧和相机轨迹，实现相机参数恢复和场景渲染的统一处理。


<details>
  <summary>Details</summary>
Motivation: 传统计算机视觉和图形学中将相机参数恢复和场景渲染视为独立任务，但在图像覆盖稀疏或姿态模糊的情况下，这两个任务相互依赖。为了解决这种分离问题，需要开发能够同时处理这两个任务的统一模型。

Method: 提出Rays as Pixels视频扩散模型，将每个相机表示为密集光线像素，通过解耦自交叉注意力机制联合去噪视频帧和相机轨迹。单个模型可处理三个任务：从视频预测相机轨迹、从输入图像联合生成视频和相机轨迹、沿目标相机轨迹从输入图像生成视频。

Result: 模型通过闭环自一致性测试验证，其前向和反向预测结果一致。轨迹预测所需的去噪步骤远少于视频生成，仅需少量去噪步骤即可实现自一致性。在姿态估计和相机控制视频生成任务上取得了良好结果。

Conclusion: Rays as Pixels模型成功地将相机参数恢复和场景渲染任务统一起来，通过联合学习视频和相机轨迹的分布，解决了传统方法中这两个任务分离的问题，为稀疏图像覆盖和姿态模糊情况下的视觉任务提供了有效解决方案。
...
TL;DR: 本文在SAR-RARP50数据集上对UNet、UNet++、DeepLabV3、Attention UNet和SegFormer五种深度学习架构进行基准测试，用于机器人辅助手术中手术器械的多类别语义分割。
...
Method: 使用SAR-RARP50数据集，对五种深度学习架构（UNet、UNet++、DeepLabV3、Attention UNet和SegFormer）进行多类别语义分割基准测试，采用结合交叉熵和Dice损失的复合损失函数来处理类别不平衡和捕捉精细物体边界。

Result: 实验表明，卷积模型如UNet和Attention UNet提供强大的基线性能，DeepLabV3与SegFormer结果相当，展示了空洞卷积和多尺度上下文聚合在捕捉复杂手术场景中的有效性。基于Transformer的架构如SegFormer进一步增强了全局上下文理解，提高了在不同器械外观和手术条件下的泛化能力。

Conclusion: 本研究为手术AI应用中分割模型的选择提供了全面的比较和实用见解，突出了卷积方法和基于Transformer方法之间的权衡。
...
Method: 提出无需训练的MAG-3D框架，包含三个专家智能体：规划智能体分解任务并协调推理过程；接地智能体执行自由形式3D接地和场景帧检索；编码智能体通过可执行程序进行几何推理和显式验证。

Result: 在具有挑战性的基准测试中实现了最先进的性能，能够在多样化场景中进行灵活的零样本3D接地推理。

Conclusion: MAG-3D通过多智能体协作设计，无需特定任务训练即可实现灵活的3D接地推理，为3D场景理解提供了有效的解决方案。
...
TL;DR: ELT是一种参数高效的视觉生成模型，通过循环共享权重的Transformer块大幅减少参数数量，同时保持高质量生成能力，支持动态计算成本与生成质量的权衡。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型依赖深度堆叠的独特Transformer层，导致参数数量庞大。ELT旨在通过循环架构显著减少参数数量，同时保持合成质量，推动视觉合成效率边界。

Method: 采用循环Transformer架构，使用迭代的权重共享Transformer块。提出Intra-Loop Self Distillation（ILSD）训练方法，通过将学生配置（中间循环）从教师配置（最大训练循环）蒸馏来确保深度一致性。

Result: 在同等推理计算设置下，ELT将参数数量减少4倍，在ImageNet 256×256上获得2.0的FID，在UCF-101上获得72.8的FVD，显著提升了视觉合成的效率边界。

Conclusion: ELT通过循环架构和ILSD训练方法，实现了参数高效的高质量视觉生成，支持动态计算-质量权衡，为视觉合成领域提供了新的效率解决方案。
...
TL;DR: UniSemAlign是一个用于计算病理学的双模态语义对齐框架，通过原型级和文本级对齐分支在共享嵌入空间中注入显式的类别级结构，提升半监督语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中的半监督语义分割面临像素级标注稀缺和伪标签监督不可靠的挑战，需要更有效的学习框架来减少类别模糊性并稳定伪标签优化。

Method: 基于病理学预训练的Transformer编码器，引入互补的原型级和文本级对齐分支在共享嵌入空间中提供结构化指导，将对齐表示与视觉预测融合生成更可靠的监督信号，通过端到端训练结合监督分割、跨视图一致性和跨模态对齐目标。

Result: 在GlaS和CRAG数据集上的实验表明，UniSemAlign显著优于最近的半监督基线方法，在仅10%标注数据下，GlaS数据集Dice提升达2.6%，CRAG数据集提升达8.6%，在20%监督下也有显著改进。

Conclusion: UniSemAlign通过双模态语义对齐有效提升了计算病理学中半监督语义分割的性能，为有限标注下的病理图像分析提供了有前景的解决方案。
...
### [64] [Envisioning the Future, One Step at a Time](https://arxiv.org/abs/2604.09527)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖密集视频或潜在空间预测，将大量计算能力消耗在密集外观上，而不是场景中点的稀疏轨迹。这使得大规模探索未来假设成本高昂，并且在需要长时域、多模态运动时性能受限。

Method: 将开放集未来场景动态预测表述为稀疏点轨迹的逐步推理。使用自回归扩散模型通过短距离、局部可预测的转移来推进这些轨迹，显式建模随时间增长的不确定性。

Result: 该方法在预测准确性上与密集模拟器相当或更优，同时实现了数量级更高的采样速度。能够从单张图像快速生成数千个不同的未来场景，同时保持物理合理性和长距离一致性。
...
TL;DR: MixFlow通过引入κ-FC条件化源分布和混合训练策略，显著降低扩散模型的生成路径曲率，提高采样效率和质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型及其变体（如整流流）虽然能生成多样化和高质量的图像，但受到高度弯曲的生成路径导致的缓慢迭代采样的限制。先前研究表明，高曲率的一个重要原因是源分布（标准高斯分布）与数据分布之间的独立性。

Method: 提出了两个互补的贡献：1）引入κ-FC通用公式，通过任意信号κ对源分布进行条件化，使其更好地与数据分布对齐；2）提出MixFlow训练策略，在固定无条件分布和基于κ-FC的分布的线性混合上训练流模型，减少生成路径曲率。

Result: 在固定采样预算下，MixFlow相比标准整流流将生成质量（FID）平均提高12%，相比先前基线提高7%。同时显著加速训练收敛，减少所需采样步数。

Conclusion: MixFlow通过改善源分布与数据分布的对齐，有效降低了生成路径曲率，显著提高了扩散模型的采样效率和生成质量，为高效生成模型提供了新思路。
...
TL;DR: 提出了一种从轮廓确定已知3D形状姿态的全局最优方法，利用轮廓面积在旋转空间中的连续性，通过预计算的轮廓签名响应曲面实现高效姿态搜索。


<details>
  <summary>Details</summary>
Motivation: 解决从无遮挡轮廓确定已知3D形状姿态的问题，现有方法通常需要对应关系或受限于特定形状类型，缺乏仅从轮廓就能实现全局最优姿态估计的通用方法。

Method: 利用轮廓面积在旋转空间中的连续性，预计算轮廓签名作为响应曲面；结合投影轮廓拟合椭圆的纵横比作为辅助全局形状签名；通过分辨率引导的候选搜索在旋转空间中进行分支，实现高效全局最优姿态估计。

Result: 在合成和真实数据上验证了方法的有效性，相比可比方法显著提高了精度；能够处理任意形状（无论凸性或亏格），无需对应关系指导。

Conclusion: 首次实现了仅从轮廓就能高效估计全局最优姿态的方法，适用于任意形状，为基于轮廓的姿态估计提供了新的解决方案。
...
### [67] [Seeing is Believing: Robust Vision-Guided Cross-Modal Prompt Learning under Label Noise](https://arxiv.org/abs/2604.09532)
*Zibin Geng,Xuefeng Jiang,Jia Li,Zheng Li,Tian Wen,Lvhua Wu,Sheng Sun,Yuwei Wang,Min Liu*

Main category: cs.CV

TL;DR: VisPrompt：一种轻量级、鲁棒的视觉引导提示学习框架，用于噪声标签场景，通过跨模态注意力机制将视觉语义反向注入提示表示，提高模型对标签噪声的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 提示学习是视觉语言模型的高效参数方法，但其在标签噪声下的鲁棒性研究较少。视觉内容包含更丰富可靠的语义信息，在标签噪声下更鲁棒，而提示本身对标签噪声高度敏感。因此需要开发一种能够利用视觉信息增强提示学习鲁棒性的方法。
...
Result: VisPrompt在7个基准数据集上的合成和真实世界标签噪声实验中，普遍优于现有基线方法，实现了更强的鲁棒性。同时保持预训练VLM主干冻结，仅引入少量额外可训练参数。

Conclusion: VisPrompt通过视觉引导的提示学习框架，有效抑制了噪声引起的干扰，减少了提示更新的不稳定性，缓解了对错误标记样本的记忆，显著提高了噪声标签设置下的鲁棒性。
...
TL;DR: CT-1是一个专门设计的视觉-语言-相机模型，通过准确估计相机轨迹，将空间推理知识转移到视频生成中，实现精确的相机控制。
...
Method: 基于视觉语言模块和扩散Transformer模型，采用小波基正则化损失在频域中学习复杂相机轨迹分布，并将轨迹集成到视频扩散模型中实现空间感知的相机控制。

Result: 成功弥合了空间推理与视频合成之间的差距，生成忠实且高质量的相机可控视频，相机控制准确率比先前方法提高了25.7%。

Conclusion: CT-1框架通过专门的模型设计和数据管道，实现了精确的相机轨迹估计和空间感知的视频生成，为自动化相机控制视频生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有车路协同3D感知方法在实际部署中存在两个关键瓶颈：1）密集BEV表示的二次计算复杂度随距离增加而急剧增长；2）特征关联机制在显著观测和对齐误差下表现脆弱。这些问题限制了远距离协同感知的实际应用。

Method: 提出Long-SCOPE完全稀疏框架，包含两个核心组件：1）几何引导查询生成模块，用于精确检测小型远距离物体；2）可学习的上下文感知关联模块，能够在严重位置噪声下鲁棒地匹配协同查询。

Result: 在V2X-Seq和Griffin数据集上的实验验证表明，Long-SCOPE在具有挑战性的100-150米远距离设置中实现了最先进的性能，同时保持了极具竞争力的计算和通信成本。

Conclusion: Long-SCOPE通过完全稀疏的设计解决了远距离协同3D感知的关键瓶颈，为实际部署提供了高效且鲁棒的解决方案，特别在远距离场景中表现出色。
...
TL;DR: 该研究提出了一种利用Skinned Multi Animal Linear模型从RGB图像生成3D动物边界框标注的管道，解决了单目3D动物检测缺乏标注数据的问题。


<details>
  <summary>Details</summary>
Motivation: 单目动物成像将3D结构降维到2D投影，现有检测算法只能得到2D边界框，缺乏动物相对于相机的方向信息。开发RGB图像的3D动物检测方法面临标注数据集缺乏的挑战，因为标注过程需要3D输入流和RGB数据。

Method: 提出一个管道，利用Skinned Multi Animal Linear模型估计3D边界框，并通过专门的相机姿态细化算法将其投影到2D图像空间作为鲁棒标注。计算立方体面可见性指标来评估动物哪些侧面被捕捉。

Result: 在Animal3D数据集上评估该方法，展示了跨物种和设置下的准确性能。生成的3D边界框和可见性指标为开发和基准测试未来单目3D动物检测算法提供了关键步骤。

Conclusion: 该方法为解决单目3D动物检测缺乏标注数据的问题提供了有效解决方案，生成的3D标注为未来算法开发奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为高质量图像生成的主流方法，特别是DiT模型在提示遵循和样本质量方面表现出色。然而，现有方法缺乏在推理时灵活控制生成内容的能力，无法有效移除不需要的视觉概念或调整生成风格。

Method: SHIFT框架受大型语言模型中激活引导的启发，学习引导向量，在推理时动态应用于选定的层和时间步。这些向量通过操纵中间激活来抑制不需要的视觉概念，同时保持提示的其余内容和整体图像质量。

Result: SHIFT在DiT生成中提供了有效且灵活的控制，能够处理多样化的提示和目标，包括概念移除、风格域转换以及目标对象的添加或更改，所有这些都不需要重新训练模型。

Conclusion: SHIFT是一个简单但有效的轻量级框架，通过在推理时对DiT扩散模型的激活进行定向操控，实现了对生成内容的灵活控制，为扩散模型的实际应用提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经视频表示研究主要关注中等或高容量模型，对于资源受限环境中所需的极端紧凑配置行为研究不足。需要探索微型NeRV架构在重建质量、计算复杂度和解码吞吐量方面的表现。

Method: 1. 提出两种轻量级配置NeRV-T和NeRV-T+；2. 使用知识蒸馏和频率感知焦点监督提升紧凑模型性能；3. 通过训练后量化和量化感知训练研究低精度推理的影响。

Result: 精心设计的微型NeRV变体能够在显著减少参数数量、计算成本和内存需求的同时，实现良好的质量-效率权衡。实验验证了紧凑神经视频表示的实际极限。

Conclusion: 该研究为在资源受限和实时环境中部署NeRV风格模型提供了指导，展示了微型神经视频表示在保持性能的同时实现高效部署的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D纹理生成方法存在纹理覆盖不完整、跨视角不一致以及几何与纹理对齐不佳的问题，需要开发能够生成高质量、一致且对齐良好的3D纹理的新方法。

Method: 框架包含两个核心组件：1）基于预训练图像编辑骨干的多视角合成框架，通过即插即用模块显式促进几何对齐、跨视角一致性和光照均匀性；2）原生3D纹理生成模型，在生成视角和3D几何条件下，将多视角纹理投影到3D表面并合理补全未观察区域的纹理。
...
<details>
  <summary>Details</summary>
Motivation: 现有时尚数据集存在碎片化和任务特定性问题，主要关注单品属性、服装共现或弱文本监督，缺乏对整体服装理解的全面支持，需要专家级标注的基准数据集。
...
Conclusion: FashionStylist填补了现有时尚数据集的空白，为全面、专家级的时尚理解提供了专业标注的基准，支持复杂服装理解任务并提升MLLM系统的时尚能力。
...
TL;DR: 提出一种改进YOLOv8的方法，通过添加对齐损失函数来增强建筑立面解析的结构一致性，无需改变标准推理流程


<details>
  <summary>Details</summary>
Motivation: 传统目标检测器独立处理建筑元素，导致立面解析缺乏结构一致性，难以支持下游程序化重建任务

Method: 在YOLOv8训练目标中添加自定义轻量级对齐损失函数，通过正则化鼓励边界框的网格一致性排列，注入几何先验知识

Result: 在CMP数据集上的实验表明，该方法成功提高了结构规律性，纠正了由透视和遮挡引起的对齐错误，同时保持了与标准检测精度的可控权衡

Conclusion: 通过简单的训练目标增强，可以在不改变标准推理流程的情况下显著改善建筑立面解析的结构一致性，为下游重建任务提供更好的基础
...
TL;DR: 论文提出了GeRM模型，通过分布转移向量场(DTV Field)来弥合物理渲染(PBR)与照片级真实感渲染(PRR)之间的差距，实现可控的照片级真实感图像生成。


<details>
  <summary>Details</summary>
Motivation: 虽然物理渲染(PBR)能保证物理真实性，但照片级真实感渲染(PRR)还需要真实的几何和外观数字模型。目前存在一个关键困境：显式模拟需要难以获得的真实数字模型，而隐式生成模型则牺牲了可控性和几何一致性。

Method: 1. 将PBR到PRR的转换建模为分布转移，学习分布转移向量场(DTV Field)；2. 利用多智能体VLM框架构建专家指导的P2P-50K配对数据集；3. 提出多条件ControlNet学习DTV Field，通过G-buffers、文本提示和增强区域线索渐进地将PBR图像转换为PRR图像。

Result: 提出了首个多模态生成渲染模型GeRM，能够统一PBR和PRR，允许用户在严格的物理保真度和感知的真实感之间流畅导航，生成可控的照片级真实感图像。

Conclusion: GeRM模型通过分布转移向量场方法有效弥合了物理渲染与照片级真实感渲染之间的差距，为可控的照片级真实感图像生成提供了新的解决方案。
...
TL;DR: SFGS是一种从单目视频重建高保真全身3D人体化身的新方法，通过空间三平面和时间六平面捕捉动态特征，结合结构感知高斯模块和手部细化模块，实现自然运动和精细细节的重建。
...
Method: 提出结构感知细粒度高斯溅射（SFGS）方法，使用空间三平面和时间六平面捕捉连续帧间的动态特征，设计结构感知高斯模块以空间一致的方式捕捉姿态相关细节，并提出基于细粒度手部重建的残差细化模块来更好地建模手部变形。

Result: 该方法仅需单阶段训练，在定量和定性评估中都优于现有最先进基线，能够生成具有自然运动和精细细节的高保真化身。

Conclusion: SFGS方法能够从单目视频序列重建具有表现力和一致性的全身3D人体化身，在捕捉精细细节和自然运动方面表现出色，代码已在GitHub开源。
...
TL;DR: 该论文提出从帧级评估转向事件中心视角的视频异常检测，揭示了现有基准测试中事件级性能与帧级性能之间的巨大差距。


<details>
  <summary>Details</summary>
Motivation: 传统基于姿态的视频异常检测采用帧级评估，这与实际监控系统中需要检测、定位和报告连贯异常事件的需求不匹配。帧级指标会高估模型在实际部署中的性能，因为实际应用中需要的是可操作的事件级警报。

Method: 首先审计了广泛使用的VAD基准测试的事件结构，然后提出了两种时间事件定位策略：1）带有分层高斯平滑和自适应二值化的分数细化管道；2）直接生成事件级检测的端到端双分支模型。最后通过适配时间动作定位指标建立了首个基于事件的VAD评估标准。

Result: 量化了显著的性能差距：虽然所有最先进模型在NWPUC数据集上的帧级AUC-ROC超过52%，但它们在事件级定位精度在tIoU=0.2时低于10%，所有阈值下的平均事件级F1仅为0.11。

Conclusion: 需要从帧级评估转向事件中心视角的视频异常检测，现有基准测试中的事件级性能远低于帧级性能，这揭示了实际部署中的挑战，并建立了首个基于事件的VAD评估标准。
...
TL;DR: 提出一个用于动态4D场景重建的框架，通过建模不确定性来分离动态和静态组件，在动态基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的3D基础模型（如VGGT）在静态场景中表现良好，但在动态序列中由于运动导致的几何模糊性而表现不佳。需要一种能够有效分离动态和静态组件的方法来解决这一挑战。

Method: 提出了三个协同机制：1）熵引导子空间投影，利用信息论加权自适应聚合多头注意力分布，有效隔离动态运动线索和语义噪声；2）局部一致性驱动的几何净化，通过基于半径的邻域约束强制空间连续性，消除结构异常值；3）不确定性感知的跨视图一致性，将多视图投影细化建模为异方差最大似然估计问题，利用深度置信度作为概率权重。

Result: 在动态基准测试中，该方法优于当前最先进的方法，将平均精度误差降低了13.43%，并将分割F-measure提高了10.49%。该框架保持了前馈推理的效率，无需任务特定的微调或每场景优化。

Conclusion: 该框架通过建模不确定性有效解决了动态4D场景重建中的几何模糊性问题，在性能、效率和泛化能力方面都表现出色，为动态场景理解提供了有效的解决方案。
...
TL;DR: EpiAgent是一个基于LLM的智能体系统，通过分层规划方法协调多模态分析、历史经验和专业修复工具，实现了对古代碑文复杂退化情况的灵活自适应修复。


<details>
  <summary>Details</summary>
Motivation: 古代碑文作为文化遗产，经历了数世纪的环境和人为退化，修复其视觉和文本完整性是数字遗产保护中最具挑战性的任务之一。现有AI方法通常采用固定流程，难以应对复杂多变的真实世界退化情况。

Method: 受人类碑文学家技能协调工作流程的启发，提出EpiAgent智能体系统，将碑文修复构建为分层规划问题。采用观察-构思-执行-重新评估范式，基于LLM的中心规划器协调多模态分析、历史经验、专业修复工具和迭代自我优化。

Result: 在真实世界退化碑文上，EpiAgent相比现有方法实现了更优的修复质量和更强的泛化能力，超越了传统的单次处理方法。

Conclusion: 这项工作标志着向专家级智能体驱动文化遗产修复迈出了重要一步，展示了智能体协调在复杂修复任务中的潜力。
...
TL;DR: 提出RC-GRPO-Editing框架，通过区域约束的GRPO后训练解决基于流的图像编辑中噪声信用分配问题，提高编辑区域指令遵循和非目标区域保留能力。


<details>
  <summary>Details</summary>
Motivation: 基于流的图像编辑模型在指令遵循和编辑一致性方面已有改进，但现有方法存在噪声信用分配问题：全局探索会扰动非目标区域，导致组内奖励方差增大和GRPO优势噪声化。
...
Conclusion: RC-GRPO-Editing通过抑制背景诱导的干扰方差，实现了更清晰的局部信用分配，有效提升了基于流的图像编辑模型在确定性ODE采样下的性能。
...
TL;DR: EGLOCE是一种无需训练的文本到图像扩散模型概念擦除方法，通过推理时的双能量引导潜在优化来移除特定概念，保持图像质量和提示对齐。
...
Method: EGLOCE是一种无需训练的方法，通过推理时重新引导噪声潜在空间来移除不需要的概念。采用双目标框架：排斥能量通过潜在空间梯度下降引导生成远离目标概念；保留能量保持与原始提示的语义对齐。该方法完全在推理时操作，实现即插即用集成。

Result: 大量实验表明，EGLOCE在基线方法中改进了概念移除效果，同时保持了图像质量和提示对齐，即使在对抗攻击下也表现良好。该方法建立了通过采样时双能量引导实现安全和可控图像生成的新范式。

Conclusion: EGLOCE首次建立了通过采样时双能量引导实现安全和可控图像生成的新范式，提供了一种无需训练、推理时操作的概念擦除方法，在保持图像质量的同时有效移除目标概念。
...
TL;DR: SCoRe是一种无需训练、在生成时使用的频谱再生方法，通过抑制生成图像中的高频噪声成分并重新生成，从噪声图像训练的扩散模型中生成干净图像。


<details>
  <summary>Details</summary>
Motivation: 在噪声数据集上训练的扩散模型往往会复制高频训练伪影，显著降低生成质量。现有方法需要重新训练或微调，而SCoRe旨在提供无需训练的解决方案。

Method: SCoRe利用扩散模型的频谱偏置特性，通过频率截止抑制生成图像中的高频噪声成分，然后使用SDEdit重新生成这些高频部分。关键创新是推导了截止频率与SDEdit初始化时间步之间的理论映射关系，基于径向平均功率谱密度(RAPSD)，防止再生过程中注入过多噪声。

Result: 在合成数据集(CIFAR-10)和真实世界噪声数据集(SIDD)上的实验表明，SCoRe显著优于后处理和噪声鲁棒基线方法，无需任何重新训练或微调即可恢复更接近干净图像分布的样本。

Conclusion: SCoRe是一种有效的训练时频谱再生方法，能够从噪声图像训练的扩散模型中生成干净图像，无需重新训练，具有实际应用价值。
...
TL;DR: AsymLoc：一种非对称视觉定位框架，通过大教师模型离线处理数据库图像，轻量学生模型在线处理查询图像，结合几何驱动匹配和联合检测器-描述器蒸馏，实现高效准确的视觉定位。


<details>
  <summary>Details</summary>
Motivation: 针对AR/VR和机器人等应用在资源受限的边缘设备（如智能眼镜）上需要精确实时视觉定位的需求，现有高效模型仍需进一步降低计算量而不牺牲精度。电池寿命和散热是主要关注点。

Method: 提出非对称视觉定位框架AsymLoc：大教师模型离线处理预建图数据库图像，轻量学生模型在线处理查询图像。通过几何驱动匹配目标和联合检测器-描述器蒸馏目标对齐学生模型与教师模型，实现快速、无参数的最近邻匹配。

Result: 在HPatches、ScanNet、IMC2022和Aachen数据集上的广泛实验表明，AsymLoc使用小一个数量级的模型实现了教师模型95%的定位精度，显著优于现有基线方法，建立了新的效率-精度权衡最优状态。

Conclusion: AsymLoc通过非对称架构和创新的蒸馏方法，在资源受限的边缘设备上实现了高效准确的视觉定位，为AR/VR和机器人等应用提供了实用的解决方案。
...
TL;DR: 提出沉浸式体视频新格式，包含6自由度交互空间、视听反馈和高分辨率动态内容，并开发了基于高斯时空表示的重建框架与首个多视角视听数据声场重建方法。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟现实需要6自由度视觉和听觉的紧密集成沉浸体验，但直接从真实世界捕获视频构建此类体验仍未被充分探索，需要新的媒体格式和构建方法。

Method: 1) 提出Immersive Volumetric Videos新格式；2) 构建ImViD多视角多模态数据集；3) 开发基于高斯时空表示的动态光场重建框架；4) 提出首个多视角视听数据声场重建方法。

Result: 构建了包含5K分辨率60FPS视频的ImViD数据集，开发了统一的沉浸式体视频生产流程，能生成高质量、时间稳定的视听体内容，支持大6自由度交互空间。

Conclusion: 为沉浸式体视频提供了基础定义和实用构建方法，实现了从真实世界视频创建6自由度沉浸体验的完整解决方案，推动了虚拟现实内容创作的发展。
...
TL;DR: 提出了一种模块化增量式RGB+LiDAR管道，通过语义辅助融合从室内扫描生成高质量网格，在复杂文化建筑环境中优于现有几何基线方法。


<details>
  <summary>Details</summary>
Motivation: 在大型复杂室内环境（如文化建筑）中，从LiDAR-惯性扫描进行几何高保真网格重建面临挑战：点云稀疏、几何漂移和固定融合参数导致空洞、过度平滑和结构边界处的虚假表面。

Method: 提出模块化增量式RGB+LiDAR管道：1) 视觉基础模型标记每个传入RGB帧；2) 标签增量投影并融合到LiDAR-惯性里程计地图；3) 增量语义感知TSDF融合步骤通过行进立方体生成最终网格。这种帧级融合策略保留了LiDAR的几何保真度，同时利用丰富的视觉语义解决重建边界处的几何模糊性。

Result: 在Oxford Spires数据集上使用几何指标进行定量评估，在NTU VIRAL数据集上进行定性分析。该方法优于最先进的几何基线ImMesh和Voxblox，证明了语义辅助融合对几何网格质量的益处。

Conclusion: 语义指导提高了几何重建质量，生成的语义标记网格对于重建USD资产具有价值，为从室内LiDAR扫描到XR和数字建模提供了路径。
...
TL;DR: Online3R是一个新的在线学习三维重建框架，通过视觉提示和局部-全局自监督学习解决场景不一致问题


<details>
  <summary>Details</summary>
Motivation: 现有方法在新场景中存在不一致性问题，需要能够适应新场景的在线学习重建框架，同时解决缺少真实标注和高效更新的挑战

Method: 在预训练冻结的几何基础模型中引入可学习的轻量级视觉提示，采用局部-全局自监督学习策略，通过局部一致性约束（基于中间和先前局部融合结果）和全局一致性约束（基于稀疏关键帧）来训练模型

Result: Online3R在多个基准测试中优于之前的最先进方法

Conclusion: 提出的在线学习框架通过视觉提示和自监督策略有效解决了新场景适应问题，实现了高效且一致的三维重建
...
TL;DR: R&R框架通过引入结构化思维链推理，在图像修复前进行显式退化诊断，将语义推理与像素级修复紧密结合，实现了更好的修复效果和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有通用图像修复方法主要关注像素重建，缺乏对退化组成、严重程度和场景语义的显式诊断推理。这限制了修复质量和过程的可解释性。

Method: 提出Reason and Restore框架：1）引入基于Qwen3-VL微调的理由器，诊断退化类型、量化严重程度、推断关键因素、描述场景语义；2）将结构化推理结果作为先验知识指导修复器；3）利用退化严重程度量化作为强化学习信号增强修复器。

Result: 在多个通用图像修复基准测试中，R&R实现了最先进的性能，同时为修复过程提供了独特的可解释性。

Conclusion: R&R框架通过将结构化思维链推理与图像修复紧密结合，不仅提升了修复质量，还提供了可解释的诊断过程，为通用图像修复开辟了新方向。
...
TL;DR: EgoTL是一个用于解决VLM在长时程空间指令跟随中标注噪声问题的系统，通过构建"边想边说"的自我中心数据采集管道，结合度量空间估计和记忆库，为100多个日常家务任务提供精确标注。


<details>
  <summary>Details</summary>
Motivation: 大型基础模型在具身智能方面取得进展，但VLM自动标注存在噪声问题，主要源于缺乏准确的人类动作标签、思维链和空间标注，这些误差在长时程空间指令跟随中被放大，导致模型出现幻觉、跳过步骤或忽视物理属性。

Method: EgoTL构建了自我中心数据的"边想边说"采集管道，采用"先说后做"协议记录逐步目标和带时间戳的口语推理，然后通过度量空间估计器校准物理属性，使用记忆库获取场景上下文，并为导航指令和详细操作动作提供片段级标签。

Result: EgoTL能够在三个层次的六个任务维度上对VLM和世界模型进行基准测试，涵盖100多个日常家务任务的分钟级长时程序列。研究发现基础模型作为自我中心助手或开放世界模拟器仍有不足，但使用EgoTL训练数据微调后，模型在长时程规划推理、逐步推理、指令跟随和空间基础方面得到改善。

Conclusion: EgoTL通过提供精确标注的自我中心数据集，解决了VLM在长时程家务任务中的噪声问题，显著提升了基础模型在具身智能任务中的表现，特别是在空间基础和逐步推理方面。
...
TL;DR: Tango是一个优化视频大语言模型视觉信号利用的token剪枝框架，通过多样性驱动的注意力选择策略和时空旋转位置编码，在仅保留10%视频token的情况下保持98.9%的原始性能，实现1.88倍推理加速。
...
Result: 在各种视频大语言模型和视频理解基准测试中验证了方法的有效性。当仅保留10%的视频token时，Tango在LLaVA-OV上保持了98.9%的原始性能，同时实现了1.88倍的推理加速。

Conclusion: Tango通过优化视觉信号利用，有效解决了现有token剪枝方法的局限性，在保持高性能的同时显著提升了视频大语言模型的推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 探究高效序列模型相比简单时间平均的优势，使用EMA轨迹作为受控探针来映射固定系数累积能表示和不能表示的边界

Method: 使用多时间尺度EMA轨迹的Hebbian架构，构建仅使用EMA上下文的语言模型，通过预测器消融实验（用完整softmax注意力替换线性预测器）定位性能差距

Result: EMA轨迹在语法角色分配任务上达到监督BiGRU的96%，在结构相关角色上超越监督模型；130M参数仅使用EMA上下文的语言模型在C4数据集上困惑度为260（GPT-2的8倍）；预测器消融实验显示性能差距完全来自轨迹
...
TL;DR: 扩散语言模型的安全性依赖于单调去噪假设，但简单的令牌重掩码和肯定前缀注入攻击可绕过安全对齐，攻击成功率高达81.8%


<details>
  <summary>Details</summary>
Motivation: 研究扩散语言模型安全对齐的脆弱性，发现其安全性仅依赖于单调去噪假设，一旦该假设被违反，模型的安全防护就会被轻易绕过

Method: 提出简单的两步干预攻击：1) 重新掩码已承诺的拒绝令牌；2) 注入12个令牌的肯定前缀。在HarmBench基准上测试LLaDA-8B-Instruct和Dream-7B-Instruct模型，并与基于梯度的对抗攻击进行对比

Result: 攻击成功率：LLaDA-8B-Instruct达到76.1% ASR，Dream-7B-Instruct达到81.8% ASR。基于梯度的优化攻击反而降低攻击成功率（41.5% vs 76.1%），表明漏洞是结构性的而非需要复杂利用

Conclusion: 扩散语言模型的安全性缺乏对抗鲁棒性，仅因去噪调度未被违反而成立。需要安全感知的去噪调度、步骤条件前缀检测和承诺后重新验证等防御措施
...
TL;DR: WAND框架通过窗口化注意力机制和知识蒸馏，将自回归TTS模型的计算和内存复杂度从二次降低到常数，同时保持合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有自回归TTS模型虽然能生成高质量语音，但由于使用全自注意力机制，其计算和内存成本随序列长度呈二次方增长，限制了实际应用。

Method: 提出WAND框架：1) 将注意力机制分离为条件令牌的持久全局注意力和生成令牌的局部滑动窗口注意力；2) 采用课程学习策略逐步收紧注意力窗口以稳定微调；3) 使用全注意力教师模型进行知识蒸馏以高效恢复高质量合成。

Result: 在三个现代AR-TTS模型上评估，WAND保持了原始质量，同时实现了高达66.2%的KV缓存内存减少，以及长度不变、接近常数的每步延迟。

Conclusion: WAND框架成功将预训练AR-TTS模型的计算和内存复杂度从二次降低到常数，为高质量语音合成的实际部署提供了可行的解决方案。
...
TL;DR: 本文对大型语言模型在医学推理领域的研究进行了全面综述，基于临床推理的认知理论，将医学推理概念化为溯因、演绎和归纳的迭代过程，并提出了统一的评估框架和新的临床基准测试。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在医学考试类任务上表现出色，但临床决策具有安全性要求高、上下文依赖性强、证据不断演变的特点。在这种环境下，可靠的LLM性能不仅依赖于事实记忆，更需要稳健的医学推理能力。目前缺乏对医学推理方法的系统梳理和统一评估。

Method: 基于临床推理的认知理论，将医学推理概念化为溯因、演绎和归纳的迭代过程；将现有方法组织为七种主要技术路线，涵盖基于训练和无训练的方法；在一致的实验设置下进行跨基准统一评估；引入基于真实医院数据的MR-Bench基准测试。

Result: 评估显示，现有医学推理模型在考试级别任务上表现良好，但在MR-Bench基准测试中，真实临床决策任务的准确率与考试级性能存在显著差距，暴露了当前模型在临床推理要求方面的不足。

Conclusion: 本文为现有医学推理方法、基准测试和评估实践提供了统一视角，强调了当前模型性能与真实世界临床推理要求之间的关键差距，为未来研究指明了方向。
...
TL;DR: 本文提出将Holistic Uncertainty Estimation (HolUE)方法应用于文本领域，用于开放集文本分类(OSTC)任务中的不确定性估计，通过捕捉文本不确定性和图库不确定性来预测系统何时会出错，在多个数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 准确的uncertainty estimation对于构建鲁棒可信的识别系统至关重要。开放集文本分类任务中，文本样本需要被分类为现有类别之一或被拒绝为未知类别。为了处理OSTC中遇到的不同类型不确定性，需要将HolUE方法适配到文本领域。

Method: 将Holistic Uncertainty Estimation (HolUE)方法适配到文本领域，主要解决文本识别系统中的两个主要错误来源：1) 文本不确定性（源于查询表述不当），2) 图库不确定性（与数据分布的模糊性相关）。通过捕捉这些不确定性来源，可以预测系统何时会出错。

Result: 在多个数据集上进行了广泛实验，包括作者归属、意图和主题分类数据集。HolUE在Prediction Rejection Ratio (PRR)上相比基于质量的SCF基线取得了40-365%的改进：Yahoo Answers上365%（0.79 vs 0.17 at FPIR 0.1），DBPedia上347%（0.85 vs 0.19），PAN作者归属上240%（0.51 vs 0.15 at FPIR 0.5），CLINC150意图分类上40%（0.73 vs 0.52）。

Conclusion: 提出的方法能够有效估计开放集文本分类中的不确定性，通过同时考虑文本不确定性和图库不确定性，显著提升了预测拒绝性能，为构建更鲁棒的文本识别系统提供了有效工具。
...
TL;DR: 研究通过表征分析探讨偏见缓解如何重塑基础模型的嵌入空间，发现偏见缓解能减少性别-职业关联差异，使内部表征更加中性平衡。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过表征分析来理解偏见缓解如何改变基础模型的内部表示，为模型行为提供内部审计，并验证去偏见方法的有效性。

Method: 使用BERT和Llama2作为代表性架构，比较基线模型和偏见缓解变体在性别-职业术语关联上的变化。引入WinoDec数据集（包含4000个含性别和职业术语的序列）来评估解码器模型。

Result: 偏见缓解显著减少了嵌入空间中性别-职业的关联差异，产生了更加中性平衡的内部表征。这种表征变化在两种模型类型中表现一致，表明公平性改进可以表现为可解释的几何变换。
...
TL;DR: 该研究系统评估了扩展推理模型在不同采样温度和提示策略下的性能，发现零样本提示在中等温度表现最佳，而思维链提示在温度极端值表现最好，挑战了推理任务使用T=0的常见做法。


<details>
  <summary>Details</summary>
Motivation: 扩展推理模型虽然代表了LLM能力的转变，但其最佳采样温度和提示策略配置仍未充分探索。研究者旨在系统评估不同温度和提示策略对复杂数学问题解决的影响。

Method: 使用Grok-4.1扩展推理模型，在四个温度设置（0.0、0.4、0.7、1.0）下系统评估思维链和零样本提示策略，测试39个来自AMO-Bench的IMO级别数学问题。

Result: 零样本提示在中等温度（T=0.4和T=0.7）达到59%的峰值准确率，而思维链提示在温度极端值表现最佳。扩展推理的效益从T=0.0时的6倍增加到T=1.0时的14.3倍。

Conclusion: 温度应与提示策略联合优化，挑战了推理任务使用T=0的常见实践。扩展推理在更高温度下效益显著增加，为LLM推理配置提供了新见解。
...
TL;DR: 该论文提出了一种基于注意力机制的并行解码方法Attn-Sampler，通过理论证明按注意力矩阵列和降序解码可近似实现最优序列似然，解决了传统自回归模型顺序解码效率低和扩散模型解码策略仅依赖token级信息的问题。


<details>
  <summary>Details</summary>
Motivation: 自回归模型（ARMs）的顺序解码范式限制了推理效率和建模灵活性，而现有的扩散大语言模型（dLLMs）解码策略主要依赖token级信息，忽略了全局序列结构，导致次优结果。需要一种理论支撑的解码顺序选择方法来提升解码并行性和生成质量。

Method: 从对数似然最大化角度研究解码顺序选择问题，理论证明按注意力矩阵列和降序解码可近似实现最优序列似然。基于此提出Attn-Sampler解码算法，并引入块注意力近似和动态注意力阈值技术进行实际加速。

Result: 在多个基准测试上的广泛实验验证了所提方法的有效性，Attn-Sampler在提升解码并行性的同时实现了更优的生成质量。

Conclusion: 该研究为注意力引导解码提供了理论依据，提供了一种基于理论基础的贪婪搜索替代方案，通过注意力矩阵结构信息优化解码顺序，显著提升了扩散大语言模型的解码效率和生成性能。
...
TL;DR: 研究比较不同AI模型对冲突媒体话语的情感解读差异，以2023年加沙战争为案例，发现模型架构选择实质上是选择不同的解释视角，影响冲突叙事的算法框架和情感评估。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索不同人工智能架构如何解释冲突相关媒体话语中的情感，特别关注模型选择对冲突叙事算法框架和情感评估的影响，而非简单评估模型准确性。

Method: 使用10,990条阿拉伯语新闻标题语料库，比较三个大型语言模型和六个微调阿拉伯语BERT模型。采用信息论和分布度量方法，包括香农熵、Jensen-Shannon距离和方差得分，量化模型间的系统性差异。

Result: 结果显示模型间存在显著且非随机的情感分布差异：微调BERT模型（特别是MARBERT）偏向中性分类，而LLMs持续放大负面情感（LLaMA-3.1-8B几乎完全偏向负面）。框架条件分析显示GPT-4.1能根据叙事框架调整情感判断，其他LLMs则显示有限的上下文调节能力。

Conclusion: 模型选择实质上是选择解释视角，塑造冲突叙事的算法框架和情感评估方式。研究强调将算法差异作为分析对象的重要性，并警示在战争和危机背景下将自动化情感输出视为中立或可互换的媒体语调测量指标的风险。
...
TL;DR: TCVA是一种通过温度参数控制评估严格度的LLM评估方法，在安全关键领域使用低温度（悲观评分），在对话AI使用高温度（宽松评分），无需额外LLM调用。
...
Method: 提出温度控制裁决聚合（TCVA）方法，结合五级裁决评分系统和广义幂平均聚合，通过直观的温度参数T[0.1,1.0]控制评估严格度。

Result: 在三个基准数据集（SummEval和USR）上，TCVA与人类判断的相关性在忠实度方面与RAGAS相当（Spearman = 0.667 vs. 0.676），且始终优于DeepEval。

Conclusion: TCVA提供了一种灵活、无需额外计算成本的评估方法，能够根据应用领域需求调整评估严格度，更好地与人类评估对齐。
...
TL;DR: LG AI Research发布了首个开源视觉语言模型EXAONE 4.5，通过集成视觉编码器实现原生多模态预训练，在文档理解和韩语推理任务上表现优异，支持256K上下文长度。


<details>
  <summary>Details</summary>
Motivation: LG AI Research旨在开发一个实用的多模态AI模型，特别针对文档理解和韩国语境的工业应用需求，推动AI在实际产业部署中的应用。

Method: 在EXAONE 4.0框架基础上集成专用视觉编码器，实现视觉和文本模态的原生多模态预训练；采用大规模精心策划的数据集，特别强调与LG战略应用领域对齐的文档中心语料库。

Result: EXAONE 4.5在通用基准测试中表现有竞争力，在文档理解任务上超越同规模最先进模型，在韩语上下文推理方面表现优异；支持长达256K tokens的上下文长度。

Conclusion: EXAONE 4.5作为首个开源视觉语言模型，在文档理解和韩语推理方面具有显著优势，支持企业级长上下文应用，为工业部署提供实用解决方案，并将持续扩展以适应更多领域和应用场景。
...
TL;DR: 研究偏好优化数据质量对推理模型性能的影响，发现生成器级差异和样本级差异都能提升下游推理任务表现


<details>
  <summary>Details</summary>
Motivation: 虽然DPO和KTO等偏好优化方法被广泛用于语言模型对齐，但人们对于偏好数据的哪些特性能够驱动下游推理能力的提升了解甚少。本研究旨在探究偏好对中的哪些方面能够改善推理模型在通用推理任务上的性能。

Method: 研究两种不同的偏好数据质量差异：1) 生成器级差异（由生成选中和拒绝推理轨迹的模型能力差异引起）；2) 样本级差异（由单个偏好对中判断质量差异引起）。通过改变生成器的规模和模型家族来研究生成器级差异，使用LLM作为评判员对生成的推理轨迹在多个推理质量维度上进行评分来研究样本级差异。

Result: 增加生成器级差异能持续提升在领域外推理任务上的性能，而通过样本级差异筛选数据可以实现更高效的数据训练。生成器级差异对性能提升有稳定贡献，样本级差异则有助于选择最具信息量的训练样本。

Conclusion: 通过偏好优化提升推理性能的双重策略：在构建偏好对时最大化生成器级差异，并利用样本级差异选择最具信息量的训练样本。这为偏好优化训练提供了数据质量指导原则。
...
TL;DR: LLMs在关系抽取任务中表现不如小型图解析器，特别是在复杂语言图场景下


<details>
  <summary>Details</summary>
Motivation: 关系抽取是构建知识图谱等应用的基础组件，LLMs在监督学习和上下文学习设置中都被视为有前景的工具。但研究发现当文本的语言图复杂度较高时，LLMs的性能仍落后于更小的架构。

Method: 在六个具有不同大小和复杂度句子图的关系抽取数据集上，评估了四个LLMs与一个基于图的解析器的性能对比。

Result: 随着输入文档中关系数量的增加，基于图的解析器逐渐超越LLMs的表现。在复杂语言图存在的情况下，更轻量级的图解析器成为更优选择。

Conclusion: 尽管LLMs在关系抽取中表现出潜力，但在处理复杂语言结构时，专门的图解析器架构仍然具有优势，特别是在需要处理大量关系的场景下。
...
TL;DR: 研究评估了五个前沿语言模型在《反人类卡牌》游戏中的幽默判断能力，发现模型表现超过随机基线但与人类偏好一致性有限，且模型间一致性远高于与人类的一致性，这可能源于系统性的位置偏见和内容偏好。
...
Method: 让五个前沿语言模型与人类玩家玩相同的《反人类卡牌》游戏，模型从十张候选卡牌中选择最有趣的回应，共进行了9,894轮游戏，分析模型与人类偏好的对齐程度。

Result: 所有模型的表现都超过了随机基线，但与人类偏好的对齐程度仍然有限。更显著的是，模型之间的一致性远高于它们与人类的一致性。研究发现这种偏好部分源于系统性的位置偏见和内容偏好。

Conclusion: 研究提出了一个重要问题：LLM的幽默判断是否反映了真正的偏好，还是仅仅是推理和对齐过程中的结构性伪影。这表明需要更深入地理解模型幽默判断的机制及其与人类认知的差异。
...
### [105] [MedConceal: A Benchmark for Clinical Hidden-Concern Reasoning Under Partial Observability](https://arxiv.org/abs/2604.08788)
*Yikun Han,Joey Chan,Jingyuan Chen,Mengting Ai,Simo Du,Yue Guo*

Main category: cs.CL

TL;DR: MedConceal是一个医疗对话基准测试，包含交互式患者模拟器，用于评估在部分可观测性下的隐藏担忧推理能力，包含300个案例和600个临床医生-LLM交互。
...
Method: 构建MedConceal基准，包含300个精心策划的案例，每个案例配对临床医生可见的上下文和模拟器内部的隐藏担忧。使用交互式患者模拟器隐藏这些担忧，通过理论基础的轮次级通信信号跟踪是否被揭示和解决，并由临床医生审查临床合理性。

Result: 结果显示没有单一系统占主导：前沿模型在不同确认指标上领先，而人类临床医生（N=159）在干预成功率上最强。确认能力涉及通过多轮对话揭示隐藏担忧，干预能力涉及解决主要担忧并引导患者达成目标计划。

Conclusion: 在部分可观测性下的隐藏担忧推理是医疗对话系统尚未解决的关键挑战，MedConceal基准提供了过程感知评估框架，既能评估任务成功，也能评估导致成功的交互过程。
...
TL;DR: 该研究提出多语言故事寓意生成作为文化基础评估任务，通过14种语言-文化对数据集评估模型，发现前沿模型能生成与人类语义相似且受偏好的寓意，但缺乏跨语言多样性和价值广度。


<details>
  <summary>Details</summary>
Motivation: 故事是跨文化传递价值观的关键，但其解释因语言文化背景而异。现有评估方法多为静态基准或知识测试，缺乏对文化适应性的动态评估。需要开发能捕捉叙事理解文化差异的新评估框架。

Method: 1) 创建包含14种语言-文化对的人类撰写故事寓意数据集；2) 通过语义相似度、人类偏好调查和价值分类三种方法比较模型输出与人类解释；3) 评估GPT-4o和Gemini等前沿模型的表现。

Result: 前沿模型能生成与人类语义相似的故事寓意，且人类评估者更偏好模型输出。但模型输出显示：1) 跨语言变异显著减少；2) 集中在更狭窄的广泛共享价值集合上。模型能近似人类道德解释的中心趋势，但无法复现人类叙事理解的多样性。

Conclusion: 通过将叙事解释框架化为评估任务，本研究为研究语言模型文化对齐提供了超越静态基准的新方法。当代模型虽能捕捉人类道德解释的一般模式，但在反映文化多样性方面存在局限，突显了开发更具文化敏感性的模型的重要性。
...
TL;DR: ADAM提出了一种跨语言注意力蒸馏与人格引导生成增强的多语言人格识别方法，通过翻译增强和人格信息生成增强创建多语言数据集，显著提升了跨语言人格识别性能。


<details>
  <summary>Details</summary>
Motivation: 当前人格识别研究面临多语言数据集缺乏的挑战，限制了跨语言和跨文化的人格分析能力。为了解决这一问题，需要开发能够处理多种语言的人格识别方法。

Method: ADAM方法包含两个核心组件：1) 人格信息生成增强(PIGA)，利用大语言模型进行翻译增强，从英语数据集生成日语、中文、马来语和法语的高质量训练数据；2) 跨语言注意力蒸馏(CLAD)，训练模型理解和识别跨语言的人格特征。

Result: 实验结果显示，使用PIGA增强后，CLAD在所有语言和人格特征上都显著优于标准BCE方法。在Essays数据集上平均BA得分达到0.6332（提升0.0573），在Kaggle数据集上达到0.7448（提升0.0968）。模型展现出良好的泛化能力，性能与当前领先的编码器模型相当。

Conclusion: ADAM通过创新的数据增强和跨语言注意力蒸馏技术，有效解决了多语言人格识别的数据稀缺问题，显著提升了跨语言人格分析的性能，为跨文化人格研究提供了实用工具。
...
TL;DR: GRASP框架通过视觉基础与显式思维链推理，结合双阶段优化策略，提升多模态讽刺目标识别的细粒度定位能力，并构建了改进的数据集MSTI-MAX。


<details>
  <summary>Details</summary>
Motivation: 传统多模态讽刺检测局限于二元分类，而多模态讽刺目标识别需要更精细的文本短语和视觉区域定位。现有方法依赖隐式跨模态对齐，解释性有限且细粒度定位效果不佳。

Method: 提出GRASP框架：1) 构建MSTI-MAX数据集缓解类别不平衡并丰富多模态讽刺线索；2) 引入基于视觉基础的思维链推理，显式锚定讽刺相关视觉区域并让模型阐述推理过程；3) 采用双阶段优化：监督微调配合坐标感知加权损失，以及细粒度目标策略优化。

Result: GRASP在多模态细粒度讽刺目标识别方面优于现有基线，LLM-as-a-Judge评估定量测量了内部推理链的质量。

Conclusion: GRASP通过显式思维链推理和双阶段优化，有效提升了多模态讽刺目标识别的细粒度定位能力和可解释性，超越了传统的黑盒方法。
...
TL;DR: 该论文提出了一个基于XLM-RoBERTa-base微调的系统，用于维度方面情感分析（DimABSA），将传统的情感极性分类扩展为连续的情感价-唤醒度（VA）回归预测。


<details>
  <summary>Details</summary>
Motivation: 维度方面情感分析（DimABSA）将传统方面情感分析从分类标签扩展到连续的价-唤醒度回归，需要更精细的情感表示。现有的大型语言模型在few-shot提示下表现不佳，需要专门的任务特定微调方法。

Method: 采用XLM-RoBERTa-base进行微调，输入格式为[CLS]文本[SEP]方面词[SEP]，使用带sigmoid缩放的回归头分别预测价和唤醒度。为每种语言-领域组合训练单独模型，合并训练集和开发集进行最终测试预测。

Result: 与GPT-5.2、LLaMA-3-70B、LLaMA-3.3-70B、LLaMA-4-Maverick等大型语言模型的few-shot提示方法相比，任务特定微调方法在所有评估数据集上均显著且一致地表现更优。

Conclusion: 针对维度方面情感回归任务，专门的任务特定微调方法明显优于大型语言模型的few-shot提示方法，证明了微调策略在该任务上的有效性。
...
TL;DR: MuTSE是一个交互式Web应用，用于系统评估LLM生成的文本简化结果，支持多提示策略和模型架构的并发执行与可视化比较。


<details>
  <summary>Details</summary>
Motivation: 当前LLM文本简化评估存在方法学挑战：研究人员依赖静态计算脚本，教育者受限于标准对话界面，两者都缺乏系统性的多维度评估框架，无法有效比较不同提示策略和模型架构的组合效果。

Method: 开发了MuTSE交互式Web应用，支持并发执行P×M个提示-模型组合，实时生成比较矩阵。系统集成了分层语义对齐引擎，并采用线性偏差启发式（λ）算法，可视化映射源句子到简化版本。

Result: MuTSE能够降低定性分析的认知负荷，为下游NLP数据集构建提供可重复的结构化标注，支持任意CEFR熟练度目标的LLM文本简化评估。

Conclusion: MuTSE填补了LLM文本简化评估中的方法学空白，通过人机协同的交互式框架，实现了对多提示策略和模型架构的系统性比较与可视化分析。
...
TL;DR: TaxPraBen是中国首个税务实践专用基准测试，结合10个传统应用任务和3个真实场景，评估LLM在专业税务领域的实际能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用领域表现出色，但在高度专业化、知识密集且受法律监管的中国税务领域存在明显不足。现有税务相关基准测试多关注孤立的NLP任务，忽视了真实世界的实践能力评估。

Method: 创建TaxPraBen基准测试，包含10个传统应用任务和3个开创性真实场景（税务风险预防、税务稽查分析、税务策略规划），源自14个数据集共7.3K实例。采用可扩展的结构化评估范式，通过"结构化解析-字段对齐提取-数值和文本匹配"流程，支持端到端税务实践评估。

Result: 评估了19个LLM，结果显示显著性能差异：所有闭源大参数LLM表现优异；中国LLM如Qwen2.5普遍超过多语言LLM；而用部分税务数据微调的YaYi2 LLM仅显示有限改进。

Conclusion: TaxPraBen作为重要资源，可推动LLM在实际应用中的评估进展，为专业领域实践能力评估提供标准化框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态检索增强生成（RAG）在文档问答中存在视觉信息利用不足的问题，通常只检索少数候选页面（如Top-4），导致信息丰富但视觉显著性较低的内容被忽略，而常见但信息量低的页面被保留。

Method: 提出MAB-DQA框架：1）将查询分解为方面感知的子查询；2）为每个子查询检索方面特定的候选集；3）将每个子查询视为老虎机的一个臂，使用少量代表性页面的初步推理结果作为奖励信号来估计方面效用；4）通过探索-利用策略动态重新分配检索预算到高价值方面；5）利用最信息丰富的页面及其相关性生成最终结果。
...
TL;DR: 本文提出了一种基于锚点的历史稳定解码策略（AHD），用于解决扩散大语言模型中半自回归解码存在的块约束问题，通过实时监控token稳定性趋势实现早期跨块解码，在提升性能的同时显著提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型中的半自回归解码虽然性能优越，但存在固有的块约束问题，导致许多跨块稳定token的解码被不必要地延迟，影响了推理效率和性能。

Method: 提出Anchor-based History-stable Decoding（AHD）策略，这是一种无需训练、即插即用的动态解码方法。通过动态锚点实时监控token的稳定性趋势，一旦token达到稳定状态，就启动早期跨块解码。

Result: 在语言、视觉语言和音频语言等多个领域的广泛实验表明，AHD同时提升了性能和推理效率。特别是在BBH基准测试中，解码步骤减少了80%，性能提升了3.67%，有效逆转了现有高级解码加速策略通常导致的性能下降。

Conclusion: AHD策略成功解决了半自回归解码的块约束问题，通过实时稳定性监控和早期跨块解码，在扩散大语言模型中实现了性能与效率的双重提升，为解码策略优化提供了有效解决方案。
...
TL;DR: 提出Litmus (Re)Agent系统，通过结构化智能体推理在多语言评估中预测模型性能，在证据稀疏或缺失的情况下表现最佳


<details>
  <summary>Details</summary>
Motivation: 解决多语言部署中的预测评估问题：当直接基准测试结果缺失时，如何估计模型在目标语言任务上的表现。多语言评估覆盖稀疏，已发表证据在不同语言、任务和模型家族间分布不均。

Method: 1) 构建包含1500个问题、覆盖6个任务和5种证据场景的受控基准；2) 开发Litmus (Re)Agent系统，采用DAG编排的智能体架构，将查询分解为假设、检索证据，并通过特征感知聚合合成预测。

Result: 在6个系统中，Litmus (Re)Agent获得最佳整体性能，在直接证据薄弱或缺失的转移密集型场景中提升最大。结构化智能体推理在证据不完整情况下对多语言性能估计显示出良好效果。

Conclusion: 结构化智能体推理是处理证据不完整情况下多语言性能估计的有前景方法，Litmus (Re)Agent系统在预测多语言评估任务中表现优异。
...
TL;DR: 不确定性量化技术用于衡量语言模型的置信度，但研究发现监督微调会破坏置信度分数与输出质量之间的相关性，导致置信度指标不能直接使用，需要开发对微调更鲁棒的指标。


<details>
  <summary>Details</summary>
Motivation: 不确定性量化技术可用于检测幻觉或提醒用户审查不确定的预测，但最近研究发现微调会影响置信度分数与输出质量之间的相关性，因此需要研究置信度分数在监督微调下的敏感性。

Method: 研究监督微调后各种置信度分数的相关性变化，分析导致相关性下降的因素（如输出与训练分布的相似性），并通过案例研究展示错误相关性如何降低置信度分数在下游任务中的实用性。

Result: 监督微调后，各种置信度分数与输出质量的相关性下降，这种下降源于置信度分数受输出与训练分布相似性等因素影响，而非仅由输出质量决定。案例研究表明这种错误相关性会降低置信度分数在下游任务中的实用性。

Conclusion: 置信度指标不能直接使用而需要测试，需要开发对微调更鲁棒的指标，以确保不确定性量化技术在语言模型中的有效应用。
...
TL;DR: 模型量化会重构LLM在领域层面的元认知效率，而不是均匀降低它。研究发现M-ratio在不同量化格式间的领域分布完全不相关，而Type-2 AUROC则完全稳定，表明问题在于M-ratio的归一化而非基础辨别信号。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索模型量化如何影响LLM的元认知能力，特别是领域层面的元认知效率。通过预注册实验尝试通过领域条件训练来改善元认知，并检验量化格式对元认知评估的影响。

Method: 使用Llama-3-8B-Instruct模型，在Q5_K_M和f16两种精度下评估3000个问题。分析四个知识领域（艺术与文学、地理等）的M-ratio和Type-2 AUROC。进行预注册的领域条件训练实验，包括置信度放大SFT训练、匹配预算的不可知控制和错误处方控制。

Result: M-ratio在不同量化格式间的领域分布完全不相关（Spearman rho = 0.00），而Type-2 AUROC完全稳定（rho = 1.00）。领域条件训练成功改变了置信度分布（科学领域的NLP差距从0.076增加到0.152），但未能改善meta-d'，因为诊断特征在不同格式间不具可迁移性。

Conclusion: 依赖领域级M-ratio特征的系统对推理格式存在未经验证的依赖性，而使用AUROC_2的系统更安全。模型量化会重构而非均匀降低元认知效率，这对基于元认知评估的AI系统设计具有重要意义。
...
TL;DR: 研究发现主动学习在语言生成任务中表现不佳，其核心假设（信息量和多样性）与实际测试性能无关，而训练样本顺序和预训练数据交互才是关键因素。


<details>
  <summary>Details</summary>
Motivation: 主动学习（AL）通常用于在标注样本有限的情况下选择最有价值的未标注样本进行标注。然而最近研究发现，在语言生成任务中，当使用100-500个样本时，AL策略的表现并不优于随机采样。本研究旨在探究AL在少量样本情况下表现不佳的根本原因。

Method: 通过实证研究，检验主动学习策略所基于的核心假设是否成立。具体分析AL策略优化的两个关键因素——训练数据的信息量和多样性，是否与测试集性能相关。同时考察其他可能影响性能的因素。

Result: 研究发现：1）训练数据的信息量和多样性（AL策略优化的目标）与测试集性能没有相关性；2）训练样本的顺序以及样本与预训练数据的交互作用对性能有更大影响。

Conclusion: 主动学习在少量样本情况下表现不佳的原因是其所基于的核心假设不成立。未来的AL方法必须考虑训练样本顺序和预训练数据交互等因素，才能在极少量样本情况下有效工作。
...
TL;DR: 该研究提出PerMix-RLVR方法，通过在训练阶段而非推理阶段处理角色提示的敏感性，解决角色提示优化中的稳健性与忠实度权衡问题。


<details>
  <summary>Details</summary>
Motivation: 角色提示被广泛用于引导大语言模型行为，但寻找最优角色耗时且效果难以预测。现有方法主要在推理阶段优化提示，计算成本高，且角色敏感性对输出质量的影响机制不明确。

Method: 提出PerMix-RLVR（角色混合RLVR）策略，在训练阶段使用可验证奖励的强化学习（RLVR）来降低对角色提示的敏感性，同时通过混合不同角色训练来平衡稳健性与角色忠实度。

Result: PerMix-RLVR在MATH500上将角色稳定性评分（PSS）比RLVR提升21.2%，在PersonaGym上将角色忠实度提升11.4%，有效缓解了角色稳健性与忠实度的权衡问题。

Conclusion: 通过在训练阶段而非推理阶段处理角色敏感性，PerMix-RLVR能够同时保持对有害角色变化的稳健性和在需要时的角色忠实度，为角色提示优化提供了更高效的解决方案。
...
TL;DR: ASTRA框架通过AdaSTR模块将表格重构为逻辑语义树，解决表格序列化瓶颈；DuTR模块结合树搜索文本导航和符号代码执行，提升复杂表格问答性能，达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有表格序列化方法存在结构忽视、表示差距和推理不透明等问题，无法捕捉显式层次结构且缺乏模式灵活性，而基于树的方法语义适应性有限，阻碍了LLM在复杂表格问答中的表现。

Method: 提出ASTRA框架，包含两个主要模块：1) AdaSTR：利用LLM的全局语义感知将表格重构为逻辑语义树，显式建模层次依赖关系，并基于表格规模采用自适应机制优化构建策略；2) DuTR：基于此结构构建双模式推理框架，集成基于树搜索的文本导航实现语言对齐，以及符号代码执行进行精确验证。

Result: 在复杂表格基准测试中，该方法实现了最先进的性能表现。

Conclusion: ASTRA框架通过自适应语义树推理架构，有效解决了表格序列化的关键瓶颈，显著提升了LLM在复杂表格问答任务中的能力。
...
TL;DR: 该论文提出了一种基于构式语法理论的新方法，将英语作为第二语言/外语视为独立语言系统，构建了包含1643个标注句子的句法-语义资源库，并验证了其在二语习得研究中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 英语作为第二语言/外语的广泛使用引发范式转变：ESFL不再被视为标准英语的偏差，而是独立的语言系统。这凸显了对专门的知识密集型ESFL表征的需求，而现有资源存在局限性。

Method: 基于建构主义理论，将构式作为基本分析单位，通过建模ESFL和标准英语的句法-语义接口，参考英语的句法-语义映射同时保留ESFL的独特特征，构建了包含1643个标注句子的句法-语义资源库。

Result: 创建了一个包含1643个标注ESFL句子的黄金标准句法-语义资源库。通过测试语言生态位假说的试点研究，证明了该资源库在第二语言习得研究中的实用价值。

Conclusion: 该研究成功构建了一个专门针对ESFL的句法-语义资源库，为将ESFL视为独立语言系统提供了实证支持，并展示了其在二语习得研究中的重要应用潜力。
...
TL;DR: CONDESION-BENCH是一个新的基准测试，用于评估大语言模型在组合动作空间中的条件决策能力，解决了现有基准测试中动作候选集有限和缺乏显式条件约束的问题。


<details>
  <summary>Details</summary>
Motivation: 现有决策支持基准测试存在两个简化假设：1) 动作从预定义的有限候选集中选择；2) 决策过程中不考虑限制动作可行性的显式条件。这些假设无法捕捉现实世界动作的组合结构及其有效性约束条件。

Method: 引入CONDESION-BENCH基准测试，将动作定义为对决策变量的分配，并在变量、上下文和分配三个层面施加显式条件约束。采用基于oracle的评估方法，同时评估决策质量和条件遵守情况。

Result: 通过CONDESION-BENCH，能够更严格地评估大语言模型作为决策支持工具的能力，特别是在组合动作空间中的条件决策表现。

Conclusion: CONDESION-BENCH基准测试填补了现有决策评估的空白，为评估大语言模型在现实世界决策支持场景中的能力提供了更全面和严格的框架。
...
TL;DR: 提出锚定滑动窗口框架，提升语言模型隐写术的鲁棒性和不可感知性


<details>
  <summary>Details</summary>
Motivation: 传统基于语言模型的隐写术假设隐写文本在传输中不被修改，对微小改动非常脆弱。先前工作通过限制上下文窗口来缓解脆弱性，但严重影响了文本质量。

Method: 提出锚定滑动窗口框架，在上下文窗口中固定提示词和桥接上下文，鼓励模型补偿被排除的标记。将桥接上下文的优化表述为提示蒸馏的变体，并进一步使用自蒸馏策略扩展。

Result: 实验表明，ASW在文本质量、不可感知性和鲁棒性方面显著且一致地优于基线方法，适用于多种设置。

Conclusion: ASW框架有效提升了语言模型隐写术的鲁棒性和不可感知性，同时保持了良好的文本质量。
...
TL;DR: NyayaMind是一个用于印度司法系统的开源法律推理框架，通过检索-预测模块集成，结合RAG和领域微调LLM，提升判决预测和解释的质量与透明度。


<details>
  <summary>Details</summary>
Motivation: 当前法律判决预测与解释系统需要同时实现高预测性能和生成符合司法实践的结构化法律推理，以在实际司法或法律研究环境中具有实用价值。

Method: NyayaMind框架包含检索模块和预测模块：检索模块使用RAG管道从大规模法律语料库中识别相关法规和先例；预测模块使用针对印度法律领域微调的推理导向LLM，生成包括问题、论点、理据和最终判决的结构化输出。

Result: 广泛的实验结果和专家评估表明，NyayaMind在解释质量和证据对齐方面相比现有CJPE方法有显著改进。

Conclusion: NyayaMind为可信赖的AI辅助法律决策支持系统提供了有前景的一步，通过集成检索、推理和验证机制来模拟法院的结构化决策过程。
...
TL;DR: 论文提出神经符号层次对齐（NSHA）方法，通过显式建模和执行指令优先级来解决大语言模型中的指令冲突问题，在推理时使用求解器引导的约束满足方法，在训练时通过自动构建的监督将求解器决策蒸馏到模型参数中。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在实际应用中需要处理来自不同权威来源的多种指令（系统策略、用户请求、工具输出、检索上下文等），这些指令经常出现良性但常见的冲突。现有研究主要关注对抗性攻击，忽视了实际应用中部分或隐式指令冲突的情况，模型需要在避免安全违规的同时保持任务效用和行为一致性。

Method: 提出神经符号层次对齐（NSHA）方法：1）推理时引入求解器引导的推理，将指令解析建模为约束满足问题，使模型能够在层次约束下推导出最大一致的可应用指令集；2）训练时使用自动构建的监督将基于求解器的决策蒸馏到模型参数中。

Result: 在规则遵循、任务执行、工具使用和安全性等多个方面评估NSHA，涵盖单轮和多轮交互。结果显示，NSHA在指令冲突情况下显著提升性能，同时在参考设置中保持竞争力的效用。

Conclusion: NSHA通过显式建模指令优先级和层次约束，有效解决了大语言模型在实际应用中面临的指令冲突问题，在保持安全性的同时提升了任务效用和行为一致性，为处理异构指令源提供了系统化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统ASR存在两个未充分探索的问题：1) WER指标只关注词级准确率，无法反映句子层面的语义正确性；2) 交互式修正作为人类交流的重要环节，在ASR研究中很少被系统研究

Method: 提出基于LLM的智能体框架：1) 使用LLM-as-a-Judge作为语义感知评估指标；2) 设计LLM驱动的智能体框架模拟人类多轮交互，通过语义反馈迭代优化识别结果

Result: 在GigaSpeech（英语）、WenetSpeech（中文）、ASRU 2019代码切换测试集上进行了广泛实验，主客观评估均表明该框架能有效提高语义保真度和交互修正能力

Conclusion: 提出的交互式ASR智能体框架通过语义感知评估和多轮交互修正，显著提升了ASR系统的语义准确性和用户体验，将开源代码促进未来研究
...
TL;DR: 本文提出PCD-SpanProto框架，通过原型正则化的联邦学习方法解决跨领域情感三元组提取问题，使用类级原型交换替代完整模型参数传输，在保护数据隐私的同时实现跨领域知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有情感三元组提取方法通常在单个数据集上独立训练，无法捕获跨领域的共同特征表示，同时数据隐私限制阻碍了集中式数据聚合，需要一种既能保护隐私又能实现跨领域知识共享的解决方案。

Method: 提出原型正则化联邦学习框架PCD-SpanProto，包括：1）使用类级原型交换替代完整模型参数传输；2）设计加权性能感知聚合策略处理领域异质性；3）引入对比正则化模块增强类内紧凑性和类间可分离性。

Result: 在四个ASTE数据集上的实验表明，该方法优于基线方法，同时显著降低了通信成本，验证了基于原型的跨领域知识迁移的有效性。

Conclusion: PCD-SpanProto框架成功解决了跨领域情感三元组提取中的隐私保护和知识共享问题，通过原型交换机制实现了高效的知识迁移，为分布式场景下的自然语言处理任务提供了新思路。
...
TL;DR: STACK框架通过状态感知推理压缩与知识引导，在保持准确性的同时显著减少推理步骤，实现精度与效率的平衡


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理复杂任务时虽然性能强大，但存在过度思考问题，导致推理步骤过多、推理延迟高。现有的CoT压缩方法难以平衡准确性和效率，且缺乏细粒度、步骤级别的冗余和推理偏差适应能力。

Method: 提出STACK框架，通过显式建模阶段特定冗余源和检索增强引导，进行步骤级CoT压缩。构建在线长短对比样本，动态切换知识引导压缩（针对不确定或有偏推理状态）和自提示压缩（针对过长但自信状态），并采用基于答案收敛的早停机制抑制冗余验证。进一步提出结合PPO和DPO的奖励差异驱动训练策略，使模型学习状态条件压缩策略。

Result: 在三个数学推理基准测试中，STACK实现了优越的精度-效率平衡，相比现有方法平均响应长度减少59.9%，同时准确率提高4.8个百分点。

Conclusion: STACK框架通过状态感知推理压缩和知识引导，有效解决了大型推理模型的过度思考问题，在保持甚至提高准确性的同时显著提升了推理效率。
...
TL;DR: 本文提出了一个面向QA的facet-level诊断框架，将问题分解为原子推理面，通过Facet x Chunk矩阵评估证据充分性和grounding，分析三种推理模式来诊断RAG系统中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 尽管RAG旨在通过检索证据减少幻觉，但即使相关文档可用时幻觉答案仍然常见。现有评估主要关注答案级或段落级准确性，对生成过程中证据使用方式提供有限洞察。

Method: 1) 引入facet-level诊断框架，将每个问题分解为原子推理面；2) 使用Facet x Chunk矩阵结合检索相关性和基于自然语言推理的忠实度评分；3) 分析三种受控推理模式：严格RAG、软RAG和纯LLM生成；4) 在医疗QA和HotpotQA上评估GPT、Gemini和LLaMA模型。

Result: 研究发现RAG系统中的幻觉问题主要不是由检索准确性驱动，而是由生成过程中检索证据的整合方式决定。facet-level分析揭示了系统性的证据覆盖和错位模式，这些在答案级评估中被隐藏。

Conclusion: facet-level诊断框架能够提供可解释的诊断，揭示RAG系统中反复出现的失败模式，包括证据缺失、证据错位和先验驱动覆盖，为理解RAG幻觉的根本原因提供了新视角。
...
TL;DR: 论文提出SNCA框架，通过结构化提示提取LLM自述安全规则，形式化为谓词类型，并与行为基准对比，发现模型声称的绝对拒绝规则与实际行为存在系统性差距。


<details>
  <summary>Details</summary>
Motivation: LLM通过RLHF内化了安全策略，但这些策略从未被正式定义且难以检查。现有基准仅评估模型是否符合外部标准，无法衡量模型是否理解和执行其自述的边界。

Method: 提出符号-神经一致性审计(SNCA)框架：1) 通过结构化提示提取模型自述的安全规则；2) 将其形式化为类型化谓词（绝对、条件、自适应）；3) 通过确定性比较与伤害基准的行为合规性。

Result: 评估4个前沿模型在45个伤害类别和47,496个观察中显示：声称绝对拒绝的模型经常遵守有害提示；推理模型自我一致性最高但29%类别无法阐明策略；跨模型规则类型一致性仅11%。

Conclusion: LLM所言与所为之间的差距是可测量且架构依赖的，需要将反思性一致性审计作为行为基准的补充。
...
TL;DR: ScheMatiQ是一个利用大语言模型从文档集合中自动生成结构化证据数据库的系统，通过网页界面支持专家交互式调整，已在法律和计算生物学领域验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要人工设计标注模式并全量标注语料库，这个过程缓慢且容易出错。许多学科需要在大型文档集合中回答自然语言研究问题，这通常需要结构化证据。

Method: ScheMatiQ利用基础大语言模型，接收问题和语料库作为输入，自动生成标注模式并构建基于证据的数据库。系统提供网页界面让用户可以引导和修改提取过程。

Result: 通过与领域专家合作，证明ScheMatiQ能够生成支持法律和计算生物学领域实际分析任务的输出结果。系统已开源并提供了公共网页界面。

Conclusion: ScheMatiQ为跨学科专家提供了一种高效的工具，可以从自己的数据中提取结构化证据，避免了传统手动标注的繁琐过程。所有资源都已公开可用。
...
TL;DR: EthicMind是一个风险感知的多轮对话框架，能在推理时联合分析伦理风险和用户情绪，生成平衡伦理指导与情感参与的回答，无需额外模型训练。


<details>
  <summary>Details</summary>
Motivation: 现有对话模型通常孤立地处理共情和伦理安全，且在多轮交互中难以适应伦理风险和用户情绪的演变。在情感和伦理敏感场景中，这些失败可能导致严重伤害。

Method: 将伦理-情感对齐形式化为明确的轮次级决策问题，提出EthicMind框架：在每个轮次联合分析伦理风险信号和用户情绪，规划高层级响应策略，生成上下文敏感的回复。

Result: 实验结果表明，EthicMind比竞争基线实现了更一致的伦理指导和情感参与，特别是在高风险和道德模糊场景中表现更优。

Conclusion: EthicMind框架成功地将伦理-情感对齐作为多轮对话中的显式决策问题，通过风险感知的响应策略实现了更好的伦理安全与情感参与平衡。
...
Method: 1) 多级任务轮廓引导的数据合成框架：构建层次化任务分类法，生成多样化问答对以近似测试时查询分布；2) TRouter：基于潜在任务类型变量的任务类型感知路由器，通过查询条件化的成本和性能建模，利用合成任务分类法进行先验正则化

Result: 在多个基准测试中，合成框架有效缓解了冷启动问题，TRouter在冷启动和领域内设置下都能提供有效的LLM路由

Conclusion: 提出的数据合成框架和TRouter路由器共同解决了LLM路由中的冷启动挑战，实现了在成本-性能权衡下的有效模型选择
...
TL;DR: AIR是一种基于规则归纳的自动指令修订方法，用于在有限任务示例下适应大语言模型到下游任务。研究表明不同适应方法在不同任务类型中表现各异，没有单一方法在所有场景中占优。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何有效地将大语言模型适应到特定下游任务，特别是在只有有限任务示例的情况下。当前存在多种适应策略（提示优化、检索方法、微调等），但缺乏对不同方法在不同任务类型中性能的系统比较。

Method: 提出了自动指令修订（AIR）方法，这是一种基于规则归纳的方法，通过从有限的任务特定示例中学习来修订指令。研究将AIR与其他适应策略（包括提示优化、基于检索的方法和微调）进行了比较，并在包含知识注入、结构化提取、标签重映射和逻辑推理等多种任务需求的基准套件上进行了评估。

Result: 在五个基准测试中，AIR在标签重映射分类任务上表现最强或接近最佳；KNN检索在闭卷问答任务中表现最佳；微调在结构化提取和事件顺序推理任务中占主导地位。结果表明适应性能强烈依赖于任务类型，没有单一方法在所有设置中占优。

Conclusion: AIR在任务行为可以通过紧凑、可解释的指令规则捕捉时最有前景，而检索和微调在由特定源知识或数据集特定注释规律主导的任务中仍然更强。适应方法的选择应基于具体任务需求。
...
TL;DR: UIPress：首个用于UI转代码任务的编码器端学习压缩方法，将约6700个视觉token压缩到256个固定预算，在保持性能的同时实现9.1倍的首token生成加速。


<details>
  <summary>Details</summary>
Motivation: UI转代码生成需要视觉语言模型从单张截图生成大量结构化HTML/CSS代码，视觉token效率至关重要。现有压缩方法要么使用任务无关启发式选择token，要么仅将低注意力特征置零而不真正缩短序列，都无法真正减少预填充延迟或适应UI截图的非均匀信息密度。

Method: 在冻结的ViT编码器和LLM解码器之间插入轻量级学习压缩模块UIPress，结合深度可分离卷积、元素引导的空间重加权和Transformer细化，将约6700个视觉token压缩到256个固定预算。配合解码器的低秩适配来弥合表示差距，整个系统仅增加约2170万个可训练参数。

Result: 在Design2Code数据集上与四个基线方法公平比较，UIPress在256个token时达到CLIP分数0.8127，比未压缩基线提升7.5%，比最强推理时方法提升4.6%，同时实现9.1倍的首token生成加速。

Conclusion: UIPress是首个针对UI转代码任务的编码器端学习压缩方法，通过高效的视觉token压缩在保持生成质量的同时显著提升推理速度，为实际应用提供了可行的解决方案。
...
### [135] [Many-Tier Instruction Hierarchy in LLM Agents](https://arxiv.org/abs/2604.09443)
*Jingyu Zhang,Tianjian Li,William Jurayj,Hongyuan Zhan,Benjamin Van Durme,Daniel Khashabi*

Main category: cs.CL

TL;DR: 论文提出Many-Tier Instruction Hierarchy (ManyIH)范式，用于解决具有任意多权限级别的指令冲突问题，并创建了首个ManyIH基准测试ManyIH-Bench。


<details>
  <summary>Details</summary>
Motivation: 当前主流的指令层级范式假设固定的少量权限级别（通常少于5个），这在现实世界的智能体设置中不足，因为冲突可能来自更多来源和上下文。

Method: 提出ManyIH范式，支持任意多权限级别的指令冲突解决；创建ManyIH-Bench基准测试，包含853个智能体任务（427个编码和426个指令跟随），涵盖12个权限级别和46个真实世界智能体。

Result: 实验显示，即使当前最先进的模型在指令冲突规模扩大时表现也很差（约40%准确率），凸显了在智能体设置中需要细粒度、可扩展的指令冲突解决方法。

Conclusion: 这项工作强调了在智能体设置中明确针对细粒度、可扩展的指令冲突解决方法的迫切需求，为未来研究提供了重要的基准和方向。
...
TL;DR: 该论文从Marr的分析层次视角，批判性地审视并扩展了关于语言模型与语言处理的两个主张：预测上下文信息是语言处理的核心，以及没有大语言模型就无法实现心理语言学的许多进展。


<details>
  <summary>Details</summary>
Motivation: 论文旨在从理论层面分析语言模型与人类语言处理的关系，批判性地评估当前关于语言模型在语言处理中作用的流行观点，为更深入地理解计算模型与认知科学之间的交叉提供框架。

Method: 采用Marr的分析层次框架（计算层、算法层、实现层），对语言模型与语言处理的关系进行理论分析，批判性地审视两个核心主张，并提出结合LLMs与心理语言学模型的未来研究方向。

Result: 论文对两个主张进行了批判性分析：1) 预测上下文信息确实是语言处理的重要方面，但可能不是唯一核心机制；2) 大语言模型确实推动了心理语言学发展，但传统方法仍有价值。提出了结合两者优势的未来研究方向。

Conclusion: 语言模型与人类语言处理之间存在复杂关系，需要更细致的理论分析。未来应结合大语言模型的计算能力与心理语言学模型的认知约束，发展更全面的语言处理理论框架。
...
TL;DR: RecaLLM是一种推理语言模型，通过交替进行推理和显式上下文检索来解决长上下文信息利用问题，使用约束解码机制复制证据片段，在短训练样本上实现长上下文性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在长上下文推理中存在"迷失在思考中"问题：推理步骤虽然提升性能，但会使后续的上下文检索变得更加困难，导致检索性能显著下降，这是测试时扩展的关键瓶颈。

Method: RecaLLM采用推理与显式上下文检索交替进行的方法，在推理过程中检索解决中间子问题所需的上下文信息。引入可忽略开销的约束解码机制，支持逐字复制证据片段，增强后续生成的接地性。模型在多样化的词汇和语义检索任务上进行训练。

Result: 在RULER和HELMET两个长上下文基准测试中表现优异，显著优于基线模型。使用最多10K标记的训练样本，在长达128K标记的上下文窗口中实现了一致的性能提升，远少于现有长上下文方法所需的训练数据。

Conclusion: RecaLLM展示了在不使用昂贵长上下文训练数据的情况下提升长上下文性能的有前景路径，通过显式集成推理和检索来解决"迷失在思考中"问题，为长上下文语言模型的发展提供了新方向。
...
TL;DR: 提出BERT-as-a-Judge方法，用轻量级编码器模型替代传统词法评估和计算昂贵的LLM评估，在保持性能的同时显著降低计算成本


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估存在两个主要问题：1）传统词法评估方法过于依赖预定义格式，无法准确反映模型真实问题解决能力；2）LLM-as-a-Judge方法虽然能评估语义正确性，但计算成本过高

Method: 提出BERT-as-a-Judge方法，使用编码器驱动的轻量级模型评估生成式输出的答案正确性。该方法对输出措辞变化具有鲁棒性，仅需在合成的问答-候选-参考三元组上进行轻量级训练

Result: 通过36个模型和15个下游任务的大规模实证研究显示，词法评估方法与人类判断相关性差。BERT-as-a-Judge在性能上持续优于词法基线，同时匹配更大规模LLM评估器的性能

Conclusion: BERT-as-a-Judge在传统词法评估和计算昂贵的LLM评估之间提供了理想的折中方案，实现了可靠、可扩展的评估，并发布了所有项目成果以促进下游应用
...
TL;DR: 本文通过虚构对话形式，探讨现代语言模型能否为语言科学提供洞见，批评了两种常见误解：字符串统计稻草人谬误（认为语言模型只是统计字符串）和"已经够好"假设（认为当前语言模型研究已达极限），倡导在AI时代建立更全面的语言科学研究框架。


<details>
  <summary>Details</summary>
Motivation: 本文旨在澄清现代语言模型在语言科学研究中的价值和作用，回应学术界对语言模型的常见误解和质疑，推动在人工智能时代建立更健全、更全面的语言科学研究范式。

Method: 采用虚构对话的文学形式，通过Norm（形式理论语言学家）和Claudette（计算语言科学家）的讨论，以及25位来自语言学、神经科学、认知科学、心理学、哲学和计算机科学领域专家的参与，系统性地探讨语言模型与语言科学的关系。

Result: 识别并批判了两种阻碍语言模型与语言科学融合的主要误解：1）字符串统计稻草人谬误——错误认为语言模型只是统计字符串的模型，缺乏语言能力；2）"已经够好"假设——认为当前语言模型研究已达到极限，无法为语言学提供更多洞见。

Conclusion: 语言模型研究对于理解人类语言具有重要科学价值，需要建立更广泛的语言科学研究项目，积极回应批评者的关切，以建立更强大、更稳健的人类语言和语言模型科学。
...
TL;DR: 研究人员创建了MANYFAKE基准测试，包含6,798篇合成假新闻文章，用于评估现有假新闻检测器在混合真假内容中的表现，发现即使先进模型也难以检测嵌入真实叙事中的细微虚假信息。


<details>
  <summary>Details</summary>
Motivation: 当前假新闻检测通常被视为二元分类问题，但现代假新闻越来越多地通过人机协作产生，将战略性虚假信息嵌入原本准确可信的叙事中。这些混合真假的情况代表了现实且严重的威胁，但在现有基准测试中代表性不足。

Method: 引入MANYFAKE合成基准测试，包含6,798篇假新闻文章，通过多种策略驱动的提示管道生成，捕捉假新闻构建和优化的多种方式。使用该基准评估一系列最先进的假新闻检测器。

Result: 结果显示，即使是先进的推理模型在完全捏造的故事上接近饱和性能，但在虚假信息微妙、优化并与准确信息交织的情况下仍然脆弱。现有检测器难以有效识别混合真假内容。

Conclusion: 混合真假假新闻代表了现实且严重的威胁，现有检测方法在此类内容上表现不足。需要开发更精细的检测方法来处理嵌入真实叙事中的战略性虚假信息。
...
TL;DR: 通过权重剪枝研究发现，大语言模型中的有害内容生成依赖于一组紧凑的权重，这些权重在不同有害类型间通用且与良性能力分离。对齐训练压缩了这些权重，解释了为何微调会引发广泛的"涌现性错位"。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型经过对齐训练以避免有害行为，但安全防护仍然脆弱：越狱攻击经常绕过它们，在狭窄领域微调可能引发广泛泛化的"涌现性错位"。这种脆弱性是否反映了模型内部缺乏对有害性的连贯组织尚不清楚。

Method: 使用目标权重剪枝作为因果干预手段，探测大语言模型内部有害性的组织结构。通过剪除特定权重来观察对有害内容生成的影响，比较对齐和未对齐模型之间的差异。

Result: 发现有害内容生成依赖于一组紧凑的权重，这些权重在不同有害类型间通用且与良性能力分离。对齐模型比未对齐模型表现出更大的有害生成权重压缩。这种压缩解释了涌现性错位：如果有害能力权重被压缩，在一个领域微调这些权重会触发广泛的错位。剪除狭窄领域的有害生成权重显著减少了涌现性错位。

Conclusion: 大语言模型具有连贯的有害性内部结构，这种结构可能为更原则性的安全方法提供基础。值得注意的是，模型生成有害内容的能力与其识别和解释此类内容的能力是分离的。
...
TL;DR: RAMP策略：通过与环境交互在线学习数值规划动作模型，结合深度强化学习、动作模型学习和规划，形成正向反馈循环


<details>
  <summary>Details</summary>
Motivation: 自动化规划算法需要动作模型，但获取这样的模型很困难。现有数值域学习算法是离线的，需要专家轨迹作为输入，缺乏在线学习能力
...
Conclusion: RAMP策略成功实现了数值规划动作模型的在线学习，通过强化学习、动作模型学习和规划的集成形成了有效的正向反馈循环
...
TL;DR: 该论文扩展了Courcelle定理，证明了MSO2公式的模型可以用决策图表示，其大小在树宽和公式大小的参数下呈线性上界。


<details>
  <summary>Details</summary>
Motivation: Courcelle定理在参数化复杂性中很重要，但只解决了MSO2公式的可满足性问题。作者希望扩展这一结果，研究如何紧凑地表示MSO2公式的模型（包含自由变量的情况），并将Courcelle定理与知识表示领域连接起来。

Method: 1. 证明当考虑树宽时，MSO2公式的模型可以用sentential decision diagram (SDD)表示，其大小在参数下呈线性上界
2. 证明当考虑路径宽时，可以用ordered binary decision diagram (OBDD)表示，大小同样呈参数化线性上界
3. 基于Razgon (2014)的下界结果，证明存在MSO2公式和有界树宽图类，其OBDD表示的大小不能由树宽参数化控制

Result: 1. 成功扩展了Courcelle定理，证明了MSO2公式模型可以用决策图紧凑表示
2. 建立了树宽与SDD大小、路径宽与OBDD大小之间的参数化线性关系
3. 发现了树宽参数化下OBDD表示的局限性，但SDD表示仍然有效

Conclusion: 该研究为Courcelle定理提供了新的视角，将参数化复杂性理论与知识表示领域连接起来，展示了MSO2公式模型可以用决策图紧凑表示，但不同决策图表示形式（SDD vs OBDD）在参数化复杂性方面存在差异。
...
### [144] [Model Space Reasoning as Search in Feedback Space for Planning Domain Generation](https://arxiv.org/abs/2604.08712)
*James Oswald,Daniel Oblinsky,Volodymyr Varha,Vasilije Dragovic,Harsha Kokel,Kavitha Srinivas,Michael Katz,Shirin Sohrabi*

Main category: cs.AI

TL;DR: 研究使用基于语言模型的智能体反馈框架，结合少量符号信息，从自然语言描述生成规划领域，并通过启发式搜索优化领域质量


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型和推理模型出现，但从自然语言描述生成规划领域仍然是一个未解决的问题。现有研究表明，虽然LLMs能够协助领域生成，但距离生成可实际部署的高质量领域还很远

Method: 采用基于语言模型的智能体反馈框架，结合少量符号信息（包括地标和VAL规划验证器输出）进行领域生成，并使用启发式搜索在模型空间中进行优化

Result: 论文评估了在不同形式的符号反馈下生成领域的质量，但摘要中没有提供具体的实验结果数据

Conclusion: 研究探索了智能体语言模型反馈框架在规划领域生成中的应用潜力，通过结合符号反馈和启发式搜索来改进领域生成质量
...
TL;DR: 论文提出强化学习中环境作为外部记忆的数学框架，证明某些观察（称为"人工制品"）可以减少表示历史所需的信息量，并通过实验验证当智能体观察空间路径时，学习高性能策略所需的内存会减少。


<details>
  <summary>Details</summary>
Motivation: 基于情境认知观点，智能行为不仅依赖于内部记忆，还依赖于智能体对环境资源的主动利用。本文旨在将这一直觉形式化到强化学习框架中，探索环境如何功能性地作为智能体的记忆。

Method: 引入数学框架来形式化环境作为智能体记忆的功能，定义"人工制品"概念，证明这些观察可以减少历史表示的信息需求。通过实验验证理论，展示当智能体观察空间路径时，学习策略所需内存的减少。

Result: 实验证实当智能体观察空间路径时，学习高性能策略所需的内存确实减少。这种效应是无意中产生的，通过智能体的感知流隐式出现。研究结果满足先前用于解释外部记忆的定性特性。

Conclusion: 环境可以功能性地作为智能体的外部记忆，某些观察（人工制品）能减少历史表示的信息需求。这一发现为未来研究提供了原则性方法，可以系统性地利用环境作为显式内部记忆的替代品。
...
<details>
  <summary>Details</summary>
Motivation: 标准token级PPO在推理任务中对齐大语言模型时存在两个主要问题：1) 在长思维链上时间信用分配不稳定；2) 价值模型内存成本过高。而像GRPO这样的无批评者替代方案虽然缓解了这些问题，但需要多次采样进行基线估计，计算开销巨大，严重限制了训练吞吐量。
...
Conclusion: SPPO成功解决了标准PPO在长推理链对齐中的局限性，通过序列级方法在保持样本效率的同时实现了稳定训练，为推理大语言模型的对齐提供了一个可扩展且资源高效的解决方案。
...
Conclusion: StaRPO通过显式地将推理稳定性纳入优化目标，有效解决了现有RL框架忽视推理过程逻辑结构的问题，为大语言模型的复杂推理能力提升提供了新方向。
...
### [148] [Advantage-Guided Diffusion for Model-Based Reinforcement Learning](https://arxiv.org/abs/2604.09035)
*Daniele Foffano,Arvid Eriksson,David Broman,Karl H. Johansson,Alexandre Proutiere*

Main category: cs.AI

TL;DR: AGD-MBRL提出优势引导的扩散模型，通过SAG和EAG两种引导方法，利用优势估计指导轨迹生成，解决扩散世界模型中短视问题，提升样本效率和最终回报。


<details>
  <summary>Details</summary>
Motivation: 基于自回归世界模型的MBRL存在误差累积问题，扩散世界模型通过联合生成轨迹段缓解此问题。但现有扩散引导方法要么只关注策略（忽略价值信息），要么基于奖励（在扩散视野较短时变得短视）。需要一种能考虑长期回报的引导方法。

Method: 提出优势引导扩散（AGD-MBRL），使用智能体的优势估计指导反向扩散过程，使采样集中在预期能获得更高长期回报的轨迹上。开发了两种引导方法：Sigmoid优势引导（SAG）和指数优势引导（EAG）。该方法与PolyGRAD风格架构无缝集成，指导状态组件同时保持动作生成策略条件化。
...
### [149] [Overhang Tower: Resource-Rational Adaptation in Sequential Physical Planning](https://arxiv.org/abs/2604.09072)
*Ruihong Shen,Shiqian Li,Yixin Zhu*

Main category: cs.AI

TL;DR: 人类在认知资源受限时会同时转变物理预测机制和规划策略：早期使用基于直觉物理引擎的模拟，复杂时转向CNN视觉启发式；时间压力下从深思熟虑的前瞻规划转向浅层视野规划。


<details>
  <summary>Details</summary>
Motivation: 研究人类如何在认知资源受限下进行序列物理规划，解决直觉物理预测机制（模拟vs启发式）和决策策略（前瞻vs短视）两大长期争论的联合适应问题。

Method: 使用"悬挑塔"建造任务，要求参与者在保持稳定性的同时最大化水平悬挑。通过控制认知资源（时间压力、任务复杂度）来观察物理预测机制和规划策略的变化。

Result: 发现双重转变：早期阶段直觉物理引擎模拟占主导，复杂度增加时转向CNN视觉启发式；时间压力下前瞻规划被截断，转向更浅的规划视野。这种双重转变是先前单一机制理论无法预测的。

Conclusion: 揭示了层次化、资源理性的认知架构，能够灵活权衡计算成本与预测保真度。将两大长期争论统一为受认知预算动态重构的适应性机制。
...
### [150] [Camera Artist: A Multi-Agent Framework for Cinematic Language Storytelling Video Generation](https://arxiv.org/abs/2604.09195)
...
Method: 在现有智能体流程基础上引入专门的电影摄影镜头智能体，整合递归故事板生成以增强镜头间的叙事连续性，并通过电影语言注入产生更具表现力的电影导向镜头设计。

Result: 广泛的定量和定性结果表明，该方法在叙事一致性、动态表现力和感知电影质量方面持续优于现有基线。

Conclusion: Camera Artist通过引入专门的电影摄影智能体和递归故事板生成，有效提升了叙事视频的连续性和电影质量，为自动化电影制作提供了更高质量的解决方案。
...
### [151] [Mind the Gap Between Spatial Reasoning and Acting! Step-by-Step Evaluation of Agents With Spatial-Gym](https://arxiv.org/abs/2604.09338)
*Lars Benedikt Kaesberg,Tianyu Yang,Niklas Bauer,Terry Ruas,Jan Philip Wahle,Bela Gipp*

Main category: cs.AI

TL;DR: Spatial-Gym：一个用于评估AI模型空间推理能力的交互式环境，通过2D网格路径寻找任务测试模型在三种设置下的表现，发现当前模型与人类基准存在巨大差距，且视觉输入会显著降低性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在一次性设置中评估模型，与人类在交互环境中逐步工作的方式不同。需要开发更贴近人类推理过程的评估框架来准确衡量模型的空间推理能力。

Method: 引入Spatial-Gym环境，将2D网格路径寻找作为序列决策任务进行测试，支持可选回溯。在500个episode上评估8个模型在三种设置（一次性、逐步、带回溯的逐步）中的表现，并与人类、随机和A*基准进行比较。

Result: 最佳模型GPT-OSS 120B解决率为16.0%，比人类基准（98.0%）低82个百分点。逐步格式有助于较弱模型（提升达5.4%），但会限制较强模型的全局规划（降低达5.6%）。视觉模型接收空间环境图像会使解决率降低73%。扩展思维链推理在逐步设置中仍保持3-5倍准确率优势。

Conclusion: Spatial-Gym能够诊断模型局限性，并为通过强化学习改进空间推理提供了框架。关键发现包括：模型无法根据难度调整推理努力；视觉输入会显著损害性能；扩展思维链推理在交互设置中仍保持优势。
...
### [152] [Do We Really Need to Approach the Entire Pareto Front in Many-Objective Bayesian Optimisation?](https://arxiv.org/abs/2604.09417)
...
<details>
  <summary>Details</summary>
Motivation: 传统多目标优化方法在目标数量多时需要大量解来近似整个帕累托前沿，但实际决策者最终只选择一个解。在贝叶斯优化中，评估预算有限，更应专注于寻找单个最高质量解。

Method: 提出SPMO框架和ESPI采集函数，使用样本平均近似和梯度优化方法，支持无噪声和噪声场景，理论证明收敛性。
...
### [153] [E3-TIR: Enhanced Experience Exploitation for Tool-Integrated Reasoning](https://arxiv.org/abs/2604.09455)
*Weiyang Guo,Zesheng Shi,Liye Zhao,Jiayuan Ma,Zeen Zhu,Junxian He,Min Zhang,Jing Li*

Main category: cs.AI

TL;DR: E3-TIR是一种用于工具集成推理的预热训练范式，通过动态整合专家前缀、专家引导和自主探索三种经验类型，有效解决现有训练方法的局限性，在减少数据需求的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有工具集成推理的训练范式存在显著限制：Zero-RL由于缺乏先验指导导致探索效率低下和模式退化，而SFT-then-RL则受限于高数据成本和低熵崩溃导致的能力瓶颈。

Method: 提出E3-TIR训练范式，将训练过程动态整合为三种经验类型：专家前缀、专家引导和自主探索。通过在专家"锚点"周围进行多样化分支探索，并采用混合策略优化机制，有效缓解分布偏移并解决共享前缀带来的优化冲突。

Result: 在工具使用任务上，E3-TIR相比传统范式实现了6%的性能提升，同时仅需不到10%的合成数据。在综合性能、数据成本和训练效率的ROI指标上，相比基线获得了1.46倍的增益。

Conclusion: E3-TIR通过动态整合多种经验类型，有效平衡探索多样性与训练效率，为工具集成推理的早期训练阶段提供了高效的预热范式，显著提升了训练效果和资源利用率。
...
### [154] [Process Reward Agents for Steering Knowledge-Intensive Reasoning](https://arxiv.org/abs/2604.09482)
