...
TL;DR: SemanticFL：利用预训练扩散模型语义表征解决联邦学习中非IID数据问题的框架，通过共享潜在空间对齐异构客户端，提升多媒体感知任务的性能


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非独立同分布客户端数据上面临严重挑战，特别是在多模态感知场景中，传统方法难以解决客户端间的语义差异问题，导致多媒体系统感知性能下降

Method: 1. 利用预训练Stable Diffusion模型的多层语义表征（VAE编码潜变量和U-Net层次特征）创建共享潜在空间；2. 采用高效的客户端-服务器架构，将重计算卸载到服务器；3. 使用跨模态对比学习的统一一致性机制稳定收敛

Result: 在CIFAR-10、CIFAR-100和TinyImageNet基准测试中，SemanticFL在多种异构场景下超越了现有联邦学习方法，相比FedAvg实现了高达5.49%的准确率提升

Conclusion: SemanticFL通过利用预训练扩散模型的语义表征，有效解决了联邦学习中非IID数据问题，为异构和多模态数据的感知任务学习到了鲁棒的表征
...
TL;DR: 本文提出基于Qwen3-VL视觉编码器的统一多任务框架，用于处理超声图像分析中的分割、检测、分类和地标回归等多种任务，在FMC-UIA挑战中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 超声图像因扫描设备、操作者和解剖目标的不同而存在巨大差异，导致在单一场景训练的模型难以泛化到新的医院和临床条件。FMC-UIA挑战要求单一模型处理多种任务，需要解决跨器官和数据集的多任务学习问题。

Method: 采用Qwen3-VL家族的Transformer视觉编码器，将中间token特征投影为空间特征图，使用轻量级多尺度特征金字塔进行融合，实现像素级预测和全局推理的共享表示。每个任务配备小型任务特定预测头，训练时采用任务感知采样和选择性损失平衡来处理异构监督和减少任务不平衡。

Result: 验证集性能从67%提升到85%，在官方测试集上所有任务的平均得分达到81.84%。

Conclusion: 提出的统一多任务框架简单易优化，能够适应广泛的超声分析任务，有效解决了超声图像分析中的泛化问题。
...
TL;DR: 本文提出了一种改进的Levenberg-Marquardt优化器，用于FireANTs图像配准框架，通过减少内存占用并保持性能，解决了Adam优化器在大图像处理中的内存限制问题。


<details>
  <summary>Details</summary>
Motivation: FireANTs使用Adam作为默认优化器，但Adam需要存储动量估计和平方动量估计等状态变量，对于大图像会消耗大量内存，限制了其在大规模图像处理中的应用。

Method: 提出改进的Levenberg-Marquardt优化器，仅需单个标量阻尼参数作为优化器状态，采用信任区域方法自适应调整该参数，并使用Metropolis-Hastings风格拒绝步骤防止损失函数恶化的更新。

Result: 改进的LM优化器在大体积图像上减少内存达24.6%，在四个数据集上保持性能。单个超参数配置在脑MRI上调整后，无需修改即可应用于肺CT和跨模态腹部配准，在四个基准测试中的三个上匹配或优于Adam。

Conclusion: 改进的Levenberg-Marquardt优化器有效解决了FireANTs框架在大图像处理中的内存限制问题，同时保持了配准性能，具有良好的泛化能力和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X光片中临床相关发现通常空间受限，现有对比模型缺乏区域级监督，大型视觉语言模型在外部验证中捕捉细粒度表示能力有限，导致这些任务性能不佳

Method: 提出位置感知细粒度表示学习（LoFi），使用轻量级大语言模型联合优化sigmoid、描述生成和位置感知描述生成损失，通过定位和密集描述目标实现区域级监督

Result: 在MIMIC-CXR和PadChest-GR数据集上的广泛实验表明，该方法在检索和短语定位任务上取得了优越性能

Conclusion: LoFi方法通过位置感知的细粒度表示学习，有效提升了胸部X光片的检索和短语定位能力，为医学影像分析提供了新的解决方案
...
TL;DR: 该论文提出了ProactiveBench基准，用于评估多模态大语言模型在需要用户干预时的主动性行为，发现现有模型普遍缺乏主动性，且主动性不随模型容量增加而提升，但可以通过强化学习微调来学习主动性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索多模态大语言模型是否能像人类一样表现出"主动性"行为，即在遇到困难时主动请求用户干预。例如，当识别被遮挡物体时，人类会请求移除遮挡物，研究者想知道MLLMs是否能表现出类似的主动行为。

Method: 方法包括：1) 构建ProactiveBench基准，从七个重新利用的数据集中创建，测试在不同任务中的主动性；2) 评估22个MLLMs在该基准上的表现；3) 探索简单的基于强化学习的微调策略来训练模型的主动性。

Result: 结果显示：1) MLLMs普遍缺乏主动性；2) 主动性与模型容量无关；3) 给予"提示"只能带来边际收益；4) 对话历史和上下文学习会引入负面偏见，阻碍性能；5) 基于强化学习的微调表明主动性可以被学习，并能泛化到未见场景。

Conclusion: 结论是现有MLLMs在主动性方面存在不足，但主动性可以通过训练学习。ProactiveBench作为构建主动多模态模型的第一步被公开发布，为未来研究提供了基准。
...
TL;DR: 该论文提出了NA-VQA基准测试来评估长视频中的叙事推理能力，并开发了Video-NaRA框架来提升模型的长距离叙事理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频推理方面的基准测试大多依赖局部线索，缺乏对叙事推理能力的评估，即无法追踪意图、连接远距离事件和重建整个电影中的因果链。

Method: 提出了NA-VQA基准测试，包含88部完整电影和4.4K个开放式问答对，每个问题都基于标记为短、中、长距离的证据跨度。同时开发了Video-NaRA框架，构建事件级链并将其存储在结构化记忆中以供推理时检索。

Result: 实验表明，最先进的多模态大语言模型在需要远距离证据的问题上表现不佳，突显了显式叙事建模的必要性。Video-NaRA将长距离推理性能提升了最多3%，证明了其在处理复杂叙事结构方面的有效性。

Conclusion: NA-VQA基准测试填补了长视频叙事推理评估的空白，Video-NaRA框架通过显式建模叙事结构显著提升了模型的长距离推理能力，为未来研究提供了重要方向。
...
TL;DR: VeloxNet是一种轻量级CNN架构，用gMLP块替换SqueezeNet的火模块，通过空间门控单元实现全局空间建模，在三个航空图像数据集上相比SqueezeNet减少46.1%参数的同时提升分类精度。


<details>
  <summary>Details</summary>
Motivation: 在嵌入式设备上部署深度学习模型用于航空灾害监测和基础设施检查时，需要在精度与模型大小、内存和延迟等严格约束之间取得平衡。现有架构如SqueezeNet的火模块受限于局部感受野，无法有效捕获全局空间依赖关系。

Method: 提出VeloxNet轻量级CNN架构，用门控多层感知器(gMLP)块替换SqueezeNet的火模块。每个gMLP块包含空间门控单元(SGU)，通过学习的空间投影和乘法门控机制，在单层内捕获整个特征图的空间依赖关系，实现全局空间建模。

Result: 在三个航空图像数据集上评估：AIDER、CDD和LDD。相比SqueezeNet，VeloxNet参数减少46.1%（从740,970到399,366），加权F1分数分别提升6.32%（AIDER）、30.83%（CDD）和2.51%（LDD）。优于MobileNet变体、ShuffleNet、EfficientNet和视觉Transformer等11个基线模型。

Conclusion: 用空间门控块替换局部卷积模块可以同时提升分类精度和参数效率，适用于资源受限的部署场景。全局空间建模能力对于航空图像分类任务至关重要。
...
TL;DR: ViTRM是一种参数高效的视觉模型，用递归的小型块替代传统的多层ViT编码器，在保持性能的同时大幅减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习视觉模型（如CNN和ViT）参数密集、计算资源需求高，限制了在资源受限环境中的部署。受Tiny Recursive Models启发，希望通过递归计算实现参数高效性。

Method: 提出Vision Tiny Recursion Model (ViTRM)，用单个小型k层块（k=3）递归应用N次，替代传统的L层ViT编码器。通过迭代状态精化实现复杂推理任务。

Result: 相比CNN模型减少6倍参数，相比ViT减少84倍参数，在CIFAR-10和CIFAR-100数据集上保持竞争力性能。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像AI的可靠性严重依赖于其对不同医院设备采集的异构和损坏图像的鲁棒性，这在现实临床环境中极具挑战性。

Method: FedAgain采用双重信任机制，结合基准可靠性和模型分歧度，动态加权客户端贡献，在聚合过程中减轻噪声或对抗性更新的影响。

Result: 在五个数据集（包括MNIST、CIFAR-10、两个私有多机构肾结石数据集和MyStone公开数据集）上的实验表明，FedAgain在非独立同分布数据和损坏客户端场景下始终优于标准联邦学习基线。
...
TL;DR: Gastric-X是一个用于胃癌分析的大规模多模态基准数据集，包含1.7K个病例，整合了CT扫描、内镜图像、生化指标、诊断笔记和肿瘤标注，用于评估视觉语言模型在医疗诊断中的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自然领域表现出强大的泛化和多模态推理能力，但在医疗诊断中的应用受到限制，主要原因是缺乏全面结构化且反映真实临床工作流程的数据集。作者希望推动视觉语言模型在临床应用特别是胃癌诊断中的发展。

Method: 构建Gastric-X数据集，包含1.7K个胃癌病例，每个病例包含配对的静态和动态CT扫描、内镜图像、结构化生化指标、专家撰写的诊断笔记以及肿瘤区域的边界框标注。基于该数据集系统评估现有视觉语言模型在五个核心任务上的表现：视觉问答、报告生成、跨模态检索、疾病分类和病灶定位。

Result: 通过构建Gastric-X数据集，作者为医疗视觉语言模型研究提供了首个专注于胃癌的大规模多模态基准。该数据集模拟了临床工作流程的关键阶段，从视觉理解和推理到多模态决策支持，为评估模型性能提供了全面框架。

Conclusion: Gastric-X是朝着将机器智能与医生的认知和证据推理过程对齐的重要一步，旨在激发下一代医疗视觉语言模型的发展，并探索当前模型是否能够有意义地关联生化信号与空间肿瘤特征及文本报告。
...
<details>
  <summary>Details</summary>
Motivation: 日常医学照片已在远程医疗中广泛应用，但目前缺乏全面评估视觉语言模型解读这些图像能力的基准测试。这些图像需要细粒度的自然图像理解和领域特定的医学推理能力，这对通用和专用模型都构成挑战。

Method: 研究者创建了ReXInTheWild基准测试，包含955个临床验证的多选题，涵盖7个临床主题，基于484张来自生物医学文献的日常照片。对领先的多模态大语言模型进行了系统性评估。

Result: 主要模型表现差异显著：Gemini-3准确率78%，Claude Opus 4.5为72%，GPT-5为68%，而医学专用模型MedGemma仅37%。错误分析揭示了从低级几何错误到高级推理失败的四种常见错误类型。

Conclusion: ReXInTheWild提供了一个在自然图像理解和医学推理交叉领域的具有挑战性且临床基础的基准测试，揭示了当前模型的局限性，并为改进提供了方向。
...
TL;DR: 本文提出了一个大规模英国手语拼写数据集FS23K，并开发了考虑双手交互和口型线索的拼写识别模型，将字符错误率降低了一半。


<details>
  <summary>Details</summary>
Motivation: 英国手语拼写是BSL的重要组成部分，用于拼写专有名词、技术术语等。现有BSL拼写数据集规模小或标注不准确，且拼写识别面临快速手语和字母省略的挑战。

Method: 采用迭代标注框架构建大规模BSL拼写数据集FS23K，并提出显式考虑双手交互和口型线索的拼写识别模型。

Result: 通过精炼标注，该方法将字符错误率相比现有最佳方法降低了一半，证明了方法的有效性。

Conclusion: 该方法在BSL拼写识别方面表现出色，有望支持未来手语理解和可扩展自动标注管道的研究。
...
<details>
  <summary>Details</summary>
Motivation: XR中的空中手势容易导致疲劳和不精确，而基于表面的交互虽然能提高准确性和舒适度，但现有的以自我为中心的视觉方法在手部追踪和表面平面估计方面存在困难

Method: 提出SurfaceXR传感器融合方法，结合头显手部追踪（提供3D位置数据）和智能手表IMU数据（捕获高频运动），实现日常表面上的稳健输入

Result: 21名参与者的研究验证了SurfaceXR在触摸追踪和8类手势识别方面的有效性，相比单模态方法有显著改进

Conclusion: SurfaceXR通过融合头显手部追踪和智能手表IMU数据，解决了XR中表面交互的挑战，提供更准确、舒适的交互体验
...
TL;DR: DINOv3.seg扩展DINOv3.txt用于开放词汇语义分割，通过特定架构设计、全局与局部文本嵌入结合、早期和晚期特征细化以及高分辨率推理策略，在多个基准测试中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通过全局对比目标学习的表示对于密集预测任务不够理想，导致开放词汇语义分割方法依赖有限的图像-文本相似度图适应，限制了空间精度和复杂场景中的鲁棒性。

Method: 1) 针对DINOv3.txt设计任务特定架构；2) 结合全局[CLS]标记和局部patch级视觉特征的文本嵌入；3) 在图像-文本交互前进行早期视觉表示细化，随后对图像-文本相关特征进行晚期细化；4) 基于滑动窗口聚合的高分辨率局部-全局推理策略。

Result: 在五个广泛采用的开放词汇语义分割基准测试中进行了广泛实验，结果显示该方法有效且鲁棒，一致优于当前最先进的方法。

Conclusion: DINOv3.seg通过系统化的架构设计、多粒度特征融合、双重细化策略和高分辨率推理，为开放词汇语义分割提供了一个强大的框架，显著提升了在复杂杂乱场景中的性能。
...
TL;DR: 提出了一种轻量级、社会感知的行人意图预测架构，融合注意力、位置、情境和交互四种行为流，使用高速公路编码器、紧凑Transformer和全局自注意力池化，并集成不确定性量化机制。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在城市场景中需要准确预测行人意图以确保安全导航。现有方法在不确定性量化和计算效率方面存在不足，需要一种轻量级且能提供校准概率和可操作风险评分的方法。

Method: 提出融合四种行为流（注意力、位置、情境、交互）的架构，使用高速公路编码器、紧凑4-token Transformer和全局自注意力池化。集成两个互补头：变分瓶颈（捕捉认知不确定性）和马氏距离检测器（识别分布偏移），提供校准概率和风险评分。

Result: 在PSI 1.0基准上优于最近视觉语言模型，达到0.9 F1、0.94 AUC-ROC和0.78 MCC。在PSI 2.0数据集上建立首个基准：0.78 F1和0.79 AUC-ROC。基于马氏距离的选择性预测在80%覆盖率下将测试准确率提高0.4个百分点。

Conclusion: 该方法模态无关，易于与视觉语言管道集成，适合资源受限平台的风险感知意图预测，通过结构化可解释特征实现高性能，并提供不确定性量化和可操作风险评分。
...
TL;DR: MoCA3D：无需相机内参的单目3D物体理解模型，通过热图和深度图预测投影3D边界框角点和深度，在图像平面几何保真度上表现优异


<details>
  <summary>Details</summary>
Motivation: 现有单目3D物体理解方法通常需要相机内参才能获得图像平面几何信息（如投影3D框角点），这在真实世界物体检测中成为问题，限制了在下游应用中的实用性
...
Conclusion: MoCA3D提供了一种无需相机内参的单目3D物体理解解决方案，在图像平面几何保真度上显著优于现有方法，为真实世界应用提供了实用工具
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉模型需要在捕获图像级上下文的同时不牺牲局部细节，并保持计算效率。作者重新审视了这一权衡，提出了一个简单原则：解耦全局推理和局部表示的角色。
...
Conclusion: 这种分离实现了与图像大小的次二次方缩放，保留了与局部处理相关的归纳先验，并减少了相对于完全全局注意力的开销。结果支持效率源于全局-局部解耦的观点。
...
TL;DR: 本文提出了一种两阶段加速框架，用于加速基于扩散的图像解码器，通过多尺度采样策略和蒸馏技术实现数量级的速度提升，同时保持高质量重建。
...
Method: 提出两阶段加速框架：1) 多尺度采样策略，从粗分辨率开始解码，逐步加倍分辨率，实现理论O(log n)加速；2) 在每个尺度上将扩散解码器蒸馏为单步去噪模型，实现每个尺度单次前向传播的快速高质量重建。

Result: 该方法实现了数量级的解码时间减少，同时输出质量几乎没有下降，为高效且表达力强的图像标记器提供了实用途径。

Conclusion: 该工作为高效视觉标记和下游生成提供了基础，展示了在保持高质量的同时显著提升基于扩散解码器效率的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在离线视频理解中取得了显著成功，但在流式视频应用中受到视觉token线性爆炸的严重限制，导致内存溢出错误或灾难性遗忘。现有的视觉保留和内存管理方法通常依赖于均匀采样、低级物理指标或被动的缓存淘汰策略，这些策略缺乏内在的语义感知能力，可能破坏上下文连贯性并模糊短暂但关键的语义转换。

Method: 提出CurveStream框架，基于连续特征轨迹中高曲率区域与关键全局语义转换密切相关的几何观察。通过曲率分数评估实时语义强度，并集成在线K-Sigma动态阈值，在严格的token预算下自适应地将帧路由到清晰和模糊的内存状态中。

Result: 在不同时间尺度上的评估证实，这个轻量级框架CurveStream在StreamingBench上实现了10.69%的绝对性能提升，在OVOBench上实现了13.58%的绝对性能提升，相比各自的基线方法，建立了流式视频感知的新最先进结果。

Conclusion: CurveStream通过曲率感知的层次化视觉记忆管理，有效解决了流式视频理解中的内存限制问题，显著提升了性能，为多模态大语言模型在流式视频应用中的实际部署提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界语义分割目前严重依赖大规模图像-文本对数据集，但这些数据集往往缺乏足够类别的细粒度像素标注。获取这样的数据在经济上成本过高，需要大量人力和时间投入。

Method: MagicSeg从类别标签开始，先生成高质量文本描述，然后指导扩散模型生成图像。不仅生成每个标签的正样本，还同时生成对应的负图像作为配对反事实样本。接着，集成开放词汇检测模型和交互式分割模型，从图像中提取对象掩码作为精确的分割标签。

Result: 在PASCAL VOC、PASCAL Context和COCO数据集上分别达到62.9%、26.7%和40.2%的性能，实现了SOTA结果，证明了该方法在增强开放世界语义分割能力方面的有效性。

Conclusion: MagicSeg提出的扩散模型驱动管道能够自动生成高质量的开放世界语义分割数据集，通过伪掩码监督和辅助反事实对比训练，显著提升了下游模型的性能。
...
Method: FlowScene采用三分支生成模型，以多模态图为条件，协同生成场景布局、物体形状和物体纹理。核心是紧密耦合的整流流模型，在生成过程中交换物体信息，实现跨图协同推理。

Result: 实验表明，FlowScene在生成真实性、风格一致性和与人类偏好对齐方面优于语言条件和图条件的基线方法。

Conclusion: FlowScene通过协同生成方法实现了对物体形状、纹理和关系的细粒度控制，同时保证了场景级结构和外观的风格一致性，具有实际应用价值。
...
TL;DR: K-GMRF：一种基于李群上受迫刚体运动的在线、无需训练的二阶协方差矩阵跟踪框架，相比一阶方法具有零稳态误差优势


<details>
  <summary>Details</summary>
Motivation: 现有协方差矩阵跟踪方法要么忽略流形约束，要么依赖一阶更新，在快速变化时会产生不可避免的相位滞后。需要一种能够处理流形约束且具有更好动态响应特性的方法。

Method: 将协方差跟踪问题重新表述为李群上的受迫刚体运动问题。从欧拉-庞加莱方程推导，将观测解释为驱动潜在角速度的扭矩，通过结构保持的辛积分器进行传播。

Result: 1) 在合成椭圆上，K-GMRF将角度误差降低30倍，同时在高速度下保持稳定性；2) 在SO(3)稳定化任务中，将测地线误差从29.4°降至9.9°；3) 在OTB运动模糊序列上，将BlurCar2的IoU从0.55提升至0.74，成功率96%。
...
TL;DR: ChangeRWKV：基于RWKV架构的新型变化检测模型，结合Transformer并行训练优势与RNN线性推理效率，在LEVIR-CD基准上达到SOTA性能（IoU 85.46%，F1 92.16%），同时大幅减少参数量和计算量。
...
Result: 在LEVIR-CD基准测试中达到最先进性能：IoU 85.46%，F1分数 92.16%。相比之前领先方法，显著减少了参数数量和FLOPs（浮点运算次数）。

Conclusion: ChangeRWKV为操作规模的变化检测提供了一个高效且强大的新范式，成功平衡了计算效率与模型性能，代码和模型已公开。
...
TL;DR: 研究人员开发了Physion-Eval基准测试，用于评估视频生成模型是否遵守物理规律，发现当前模型在物理关键场景中83.3%-93.5%的生成视频存在物理故障。


<details>
  <summary>Details</summary>
Motivation: 随着视频生成模型越来越多地用作世界模拟器，需要评估这些模型生成的视频是否遵守真实世界的物理定律。现有评估方法主要依赖自动指标或粗略的人类判断，无法深入分析物理约束违反的具体情况。

Method: 创建Physion-Eval基准测试，包含10,990个专家推理轨迹，涵盖22个细粒度物理类别。每个生成视频都基于真实世界参考视频，标注了时间定位的故障、结构化失败类别和违反物理行为的自然语言解释。

Result: 在物理关键场景中，83.3%的外中心视角和93.5%的自我中心视角生成视频至少存在一个人类可识别的物理故障。揭示了当前视频生成模型在物理真实性方面的显著局限性。

Conclusion: Physion-Eval为物理真实性评估设定了新标准，将指导基于物理的视频生成模型发展。该基准测试已公开可用，有助于推动更符合物理规律的视频生成技术。
...
TL;DR: FB-CLIP：一种零样本细粒度异常检测框架，通过多策略文本表示和前景-背景分离解决CLIP模型在异常检测中的特征纠缠和语义粗糙问题
...
Method: 1. 文本模态：结合EOT特征、全局池化表示和注意力加权token特征，提供更丰富的语义线索；2. 视觉模态：沿身份、语义和空间维度进行多视角软分离，结合背景抑制减少干扰；3. 语义一致性正则化：对齐图像特征与正常/异常文本原型，抑制不确定匹配并扩大语义差距

Result: 实验表明FB-CLIP能有效区分复杂背景中的异常，在零样本设置下实现准确的细粒度异常检测和定位

Conclusion: FB-CLIP通过改进的文本表示和前景-背景分离机制，显著提升了零样本细粒度异常检测的性能，为工业与医疗应用提供了有效的解决方案
...
TL;DR: LoD-Loc v3是一种用于密集城市环境中广义航空视觉定位的新方法，通过实例分割数据集和实例轮廓对齐解决了前代方法的泛化性和密集场景问题。


<details>
  <summary>Details</summary>
Motivation: LoD-Loc v2在密集城市环境中存在两个关键限制：跨场景泛化能力差和在密集建筑场景中频繁失败。需要解决这些问题以实现更鲁棒的航空视觉定位。

Method: 1. 开发新的合成数据生成管道，创建了InsLoD-Loc - 迄今为止最大的航空图像实例分割数据集，包含10万张带有精确建筑实例标注的图像；2. 将定位范式从语义轮廓对齐重新制定为实例轮廓对齐，显著减少密集场景中的姿态估计模糊性。

Result: LoD-Loc v3在跨场景和密集城市场景中都显著优于现有的最先进基线方法，表现出卓越的零样本泛化能力。

Conclusion: LoD-Loc v3通过创新的数据生成方法和实例对齐范式，成功解决了密集城市环境中航空视觉定位的泛化性和模糊性问题，实现了更鲁棒的定位性能。
...
TL;DR: ParallelVLM是一个无需训练的草稿-验证推测解码框架，通过并行化阶段和无偏验证器引导剪枝策略，显著提升视频大语言模型的解码效率，在长视频场景下实现3.36倍加速。


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型在视频理解任务中表现出色，但自回归解码效率受限于大量视频token。现有视觉token剪枝方法存在信息损失问题，且解码加速效果有限，特别是在长视频场景下草稿模型和目标模型之间存在相互等待和加速比受限的问题。

Method: 提出ParallelVLM框架，包含两个并行化阶段以最大化硬件利用率，并采用无偏验证器引导剪枝策略，通过消除注意力引导剪枝中的位置偏差来更好地对齐草稿模型和目标模型。

Result: 实验表明，ParallelVLM将草稿窗口扩展了1.6-1.8倍且具有高接受长度，相比原始自回归解码，在LLaVA-Onevision-72B上加速3.36倍，在Qwen2.5-VL-32B上加速2.42倍。

Conclusion: ParallelVLM通过并行化设计和无偏验证器引导剪枝策略，有效解决了长视频场景下推测解码的瓶颈问题，显著提升了视频大语言模型的解码效率。
...
TL;DR: OrbitNVS将新视角合成重新定义为轨道视频生成任务，利用预训练视频生成模型的视觉先验，通过相机适配器、法线图分支和像素空间监督来提升几何和外观一致性，在单视角设置下显著优于现有方法。
...
Method: 将NVS重新定义为轨道视频生成任务，通过相机适配器实现精确相机控制，设计法线图生成分支并通过注意力机制引导目标视图合成以提升几何一致性，应用像素空间监督缓解潜在空间压缩导致的模糊外观。

Result: 在GSO和OmniObject3D基准测试中显著优于先前方法，特别是在具有挑战性的单视角设置下（例如PSNR分别提升2.9 dB和2.4 dB）。

Conclusion: OrbitNVS通过将NVS重新定义为轨道视频生成任务，利用预训练视频模型的视觉先验，结合相机控制、几何引导和像素监督，有效解决了单视角输入下的视图合成问题，显著提升了合成质量。
...
TL;DR: UniPR是一个端到端的物体级真实到仿真感知与重建框架，通过单次前向传播并行重建场景中的所有物体，解决了现有模块化方法的效率低下和累积误差问题。


<details>
  <summary>Details</summary>
Motivation: 现有真实到仿真转换方法依赖检测、分割、形状重建和姿态估计等多个子模块，这种模块化流水线效率低下且存在累积误差，因为每个阶段只处理部分或局部优化信息而丢弃全局上下文。

Method: UniPR直接处理单对立体图像，利用几何约束解决尺度模糊性；引入姿态感知形状表示，消除对每类规范定义的需求，并弥合重建与姿态估计任务之间的差距；构建了包含6300多个物体的大词汇立体数据集LVS6D。

Result: UniPR在单次前向传播中并行重建场景中的所有物体，实现了显著的效率提升，并保持了不同物体类型的真实物理比例，展示了在机器人实际应用中的潜力。
...
TL;DR: IUP-Pose是一个几何驱动的解耦迭代框架，通过隐式密集对齐解决相对位姿估计中精度与效率的权衡问题，在保持端到端可微分的同时实现实时性能。
...
Method: 提出IUP-Pose框架：1) 轻量级多头双向交叉注意力模块进行隐式跨视图特征对齐，无需显式匹配监督；2) 解耦的旋转-平移流水线：两个共享参数的旋转阶段迭代优化带不确定性的旋转估计；3) 通过旋转单应性H_inf重新对齐特征图后进行平移预测。

Result: 在MegaDepth1500数据集上达到73.3% AUC@20deg，完全端到端可微分，70 FPS吞吐量，仅37M参数，展示了优越的精度-效率权衡，适合实时边缘部署。
...
TL;DR: 提出了一种双提示驱动的特征编码方法，通过光照和视角提示增强无人机夜间跟踪的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有特征编码方法在夜间无人机跟踪中忽视关键的光照和视角线索，导致跟踪性能下降，需要更鲁棒的特征编码方法

Method: 提出双提示驱动的特征编码方法，包括金字塔光照提示器提取多尺度频率感知光照提示，动态视角提示器通过可变形卷积偏移适应视角变化

Result: 实验验证了DPTracker在夜间无人机跟踪中的有效性，消融研究显示各组件均有贡献，实际场景测试证明了鲁棒性和实用性

Conclusion: 双提示驱动的特征编码方法通过整合光照和视角提示，显著提升了夜间无人机跟踪的鲁棒性和性能
...
TL;DR: UniBioTransfer是一个统一的人脸生成框架，能够处理多种深度人脸生成任务，包括传统任务（如人脸转移、人脸重演）和形状变化任务（如头发转移、头部转移），并能泛化到未见过的任务。


<details>
  <summary>Details</summary>
Motivation: 传统深度人脸生成采用任务驱动范式，每个任务需要特定模型，这限制了模型的泛化能力和可扩展性。需要一个统一模型能够单次处理多个任务，但由于数据稀缺和跨任务冲突而具有挑战性。

Method: 提出UniBioTransfer框架：1）采用统一数据构建策略解决多任务生成中的数据不足问题，包括针对空间动态属性（如头发）的交换式破坏机制；2）通过创新的BioMoE（基于专家混合的模型）和两阶段训练策略来缓解跨任务干扰，有效解耦任务特定知识。

Result: 大量实验证明UniBioTransfer在有效性、泛化性和可扩展性方面表现出色，在广泛的深度人脸生成任务中超越了现有统一模型和任务特定方法。

Conclusion: UniBioTransfer是第一个能够统一处理传统深度人脸任务和形状变化任务的框架，能够自然泛化到未见任务，并通过创新的数据构建和模型架构解决了多任务生成中的关键挑战。
...
TL;DR: OmniDiT是一个基于扩散Transformer的统一虚拟试穿框架，将试穿和试脱任务整合到一个模型中，通过自演进数据管道构建大规模数据集，采用Shifted Window Attention降低计算复杂度，在复杂场景下实现最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法面临细节保留不足、复杂场景泛化能力差、流程复杂和推理效率低等问题，需要开发一个统一的框架来解决这些挑战。

Method: 1. 建立自演进数据管道构建Omni-TryOn数据集（38万+高质量服装-模特-试穿图像对）；2. 使用token拼接和自适应位置编码整合多参考条件；3. 首次在扩散模型中引入Shifted Window Attention实现线性复杂度；4. 采用多时间步预测和对齐损失提升生成保真度。

Result: 在各种复杂场景下，该方法在无模型VTON和VTOFF任务中取得最佳性能，在有模型VTON任务中达到与当前SOTA方法相当的性能。
...
TL;DR: GravCal：单图像重力先验校准模型，通过结合残差校正和独立图像估计，自适应融合两种互补预测，显著改善IMU重力先验的精度


<details>
  <summary>Details</summary>
Motivation: IMU提供的重力先验在线性加速度、振动和瞬态运动下不可靠，现有方法要么直接从图像估计重力，要么假设惯性输入足够准确，缺乏对单图像校正噪声重力先验的实用解决方案

Method: 提出GravCal前馈模型，输入RGB图像和噪声重力先验，输出校正后的重力方向和样本置信度。模型结合两种互补预测：输入先验的残差校正和独立于先验的图像估计，通过学习门控机制自适应融合

Result: 实验显示显著改进：平均角度误差从22.02°（IMU先验）降低到14.24°，在先验严重损坏时改进更大。还引入了包含148K帧的新数据集，包含VIO地面真值重力和Mahony滤波IMU先验

Conclusion: GravCal能有效校正单图像中的噪声重力先验，学习门控与先验质量相关，可作为下游系统的有用置信度信号，解决了实际应用中重力估计的可靠性问题
...
TL;DR: 该论文提出了SAVN-CE框架和MAGNet模型，用于解决连续环境中语义音频-视觉导航问题，通过多模态Transformer编码空间和语义目标表示，结合历史上下文与自运动线索，实现记忆增强的目标推理。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉导航方法依赖预计算的房间脉冲响应进行双耳音频渲染，限制智能体在离散网格位置移动，导致空间不连续观测。需要建立更真实的连续环境设置，解决目标间歇性静音导致目标信息丢失的问题。

Method: 提出SAVN-CE框架，使智能体能在3D空间中自由移动并感知时空一致的音频-视觉流。提出MAGNet模型，基于多模态Transformer联合编码空间和语义目标表示，集成历史上下文与自运动线索，实现记忆增强的目标推理。

Result: MAGNet显著优于现有最先进方法，在成功率上实现高达12.1%的绝对提升。模型对短时声音和长距离导航场景表现出鲁棒性。

Conclusion: SAVN-CE框架和MAGNet模型成功解决了连续环境中语义音频-视觉导航的挑战，特别是在目标间歇性静音的情况下，通过记忆增强的目标推理实现了更优的导航性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于EEG的视觉重建方法通常采用对齐框架，强制EEG特征与文本或图像语义表示对齐，这可能导致EEG中丰富的空间关系和色彩细节信息被压缩，只能实现条件图像生成而非高保真视觉重建。

Method: 提出JMVR框架：1) 将EEG和文本作为独立模态进行联合学习，保留EEG特有信息；2) 采用多尺度EEG编码策略捕获细粒度和粗粒度特征；3) 使用图像增强技术恢复感知细节。

Result: 在THINGS-EEG数据集上的实验表明，JMVR在六个基线方法中达到SOTA性能，特别在空间结构建模和色彩保真度方面表现出色。

Conclusion: JMVR框架通过独立模态学习和多尺度编码策略，有效保留了EEG中的空间和色彩信息，实现了比传统对齐方法更高保真度的视觉重建。
...
<details>
  <summary>Details</summary>
Motivation: 随着文本到视频扩散模型的快速发展，如何有效控制其生成内容成为重要问题。虽然使用边界框或布局是一种流行的控制方式，但确保生成内容完全遵循这些控制输入仍然是一个未解决的挑战

Method: 提出通过优化边界框位置来改善生成质量和对控制输入的遵循度。方法包括：1）使用平滑掩码使边界框位置可微分；2）设计注意力最大化目标函数来调整边界框；3）仔细平衡前景和背景的关注度

Result: 实验表明，即使对边界框进行微小调整，生成质量也会有显著变化。通过用户研究验证了方法的有效性，证明优化后的边界框能提高生成质量和对控制输入的遵循度

Conclusion: 通过轻微调整用户提供的边界框，使其与视频扩散模型的内部注意力图更好地对齐，可以显著提高生成质量和控制效果。该方法为文本到视频生成的控制问题提供了新的解决方案
...
Method: 提出DynFlowDrive潜在世界模型，采用rectifiedflow公式学习描述场景状态在不同驾驶动作下变化的流速场，实现渐进式未来潜在状态预测；进一步引入稳定性感知的多模式轨迹选择策略，根据诱导场景转移的稳定性评估候选轨迹。

Result: 在nuScenes和NavSim基准测试上的广泛实验表明，该方法在不同驾驶框架中均取得一致改进，且不引入额外推理开销。

Conclusion: DynFlowDrive通过流式动力学建模和稳定性感知轨迹选择，提高了自动驾驶规划的可靠性，为世界模型在自动驾驶系统中的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有终身行人重识别方法通常从零开始学习或基于视觉分类预训练模型，虽然视觉语言模型在各种任务中表现出泛化能力，但现有方法仅考虑全局感知学习，细粒度属性知识利用不足，导致知识获取和抗遗忘能力有限。

Method: 提出VLADR方法：1）多粒度文本属性解耦机制，挖掘图像的全局和多样化局部文本属性；2）跨域跨模态属性增强方案，通过跨模态属性对齐指导视觉属性提取，并采用跨域属性对齐实现细粒度知识迁移。

Result: 实验结果表明，VLADR在抗遗忘能力和泛化能力上分别比最先进方法高出1.9%-2.2%和2.1%-2.5%。
...
TL;DR: 提出一种自约束先验来改进3D高斯溅射的表面重建质量，通过TSDF网格融合深度图生成距离场约束，在表面周围形成约束带，优化高斯分布的位置和透明度


<details>
  <summary>Details</summary>
Motivation: 虽然3DGS在渲染质量和速度上优于NeRF，但在恢复高保真表面方面仍有改进空间。现有方法在表面重建精度上存在不足，需要更好的约束机制来优化3D高斯分布

Method: 提出自约束先验方法：1) 从当前3D高斯渲染的深度图融合生成TSDF网格；2) 基于TSDF构建表面周围的约束带；3) 在约束带内对3D高斯施加特定约束：移除带外高斯、移动高斯靠近表面、几何感知的透明度调整；4) 定期更新先验并逐步缩小约束带

Result: 在广泛使用的基准测试中，该方法在表面重建精度方面优于现有最先进方法，实现了更准确和完整的深度渲染

Conclusion: 自约束先验方法有效提升了3D高斯溅射的表面重建质量，通过动态更新的距离场约束机制，实现了对3D高斯的几何感知优化，为高质量3D表面渲染提供了新思路
...
TL;DR: 提出一种两阶段AI框架，通过自监督异常检测和人口统计学感知表示学习，解决心电图罕见异常检测中的长尾分布和人口差异问题，在百万级临床数据上实现94.7%的AUROC，并将常见-罕见性能差距减少73%。


<details>
  <summary>Details</summary>
Motivation: 心电图罕见异常检测面临长尾分布（病例极少）和人口统计学差异导致的诊断性能不均问题，这造成识别延迟和护理质量不均，亟需一个既能提高敏感性又能确保跨人群公平性的通用框架。

Method: 开发了两阶段AI辅助心电图框架：第一阶段通过自监督异常检测预训练，通过重建掩码的全局和局部心电图信号、建模信号趋势、预测患者属性来学习稳健的心电图表示；第二阶段使用非对称损失进行多标签心电图分类微调，以更好处理长尾心脏异常，同时生成异常分数图进行定位，并通过CPU优化实现实际部署。

Result: 在超过一百万临床心电图的纵向队列中评估，该方法对罕见异常达到94.7%的AUROC，将常见-罕见性能差距减少73%，同时在年龄和性别组间保持一致的诊断准确性。
...
TL;DR: 该研究探讨2D基础图像模型是否具备3D世界建模能力，通过多智能体架构实现3D世界合成，验证了2D模型确实蕴含对3D世界的理解


<details>
  <summary>Details</summary>
Motivation: 鉴于2D基础图像模型生成高保真输出的卓越能力，研究者希望探究一个根本问题：2D基础图像模型是否天生具备3D世界建模能力？

Method: 提出多智能体架构：基于VLM的导演制定提示指导图像合成，生成器合成新视角图像，基于VLM的两步验证器从2D图像和3D重建空间评估和筛选生成帧，实现3D世界生成

Result: 智能体方法提供了连贯且稳健的3D重建，生成可通过渲染新视角探索的输出场景，实验表明2D模型确实包含对3D世界的理解，能够合成广阔、真实且3D一致的世界

Conclusion: 2D基础图像模型确实具备3D世界建模能力，通过智能体框架可以挖掘和利用这种能力，成功合成3D一致的世界场景
...
TL;DR: 提出PerformRecast方法，专注于基于驱动视频的纯表情肖像视频编辑，通过改进3DMM参数解耦实现更好的表情控制，避免传统肖像动画方法中表情与头部姿态的纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 现有肖像动画研究主要关注根据驱动视频的面部运动来动画化静态肖像图像，但难以将面部表情从头部姿态旋转中解耦，缺乏独立编辑面部表情的能力。这在动画和电影产业中具有重要应用价值。

Method: 提出PerformRecast方法：1) 基于3DMM模型特性，改进关键点变换公式使其更符合3DMM模型，实现更好的解耦和细粒度控制；2) 解耦输入肖像图像的面部和非面部区域，预训练教师模型为它们提供单独监督，避免生成结果中面部边界的不对齐问题。

Result: 大量实验表明，该方法能生成高质量结果，更忠实于驱动视频，在可控性和效率方面均优于现有方法。代码、数据和训练模型已开源。

Conclusion: PerformRecast是一种多功能的表情专用视频编辑方法，专门用于重新演绎现有电影和动画中的表演，通过改进的3DMM参数解耦实现了更好的表情控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前rPPG方法主要基于原始视频的端到端建模或中间时空图表示，前者保留了完整的时空信息但引入大量运动伪影和光照变化噪声，后者减少了数据量和计算复杂度但可能丢失高频细节。需要有效整合两者的优势。
...
Conclusion: PhysNeXt通过整合视频帧和时空图表示的双输入框架，有效提升了远程光电容积描记的性能，在复杂条件下实现了更可靠的生理信号测量。
...
Conclusion: 通过将分离的重建任务统一到单个前向传递中，实现了近乎即时的完整可重光照3D资产生成，解决了单图像方法中材质和光照解纠缠的根本不适定问题
...
TL;DR: UPL提出了一种不确定性感知的原型学习方法，通过概率建模解决少样本3D语义分割中因监督稀缺导致的不确定性问题，在ScanNet和S3DIS基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于原型的少样本3D语义分割方法通常构建紧凑且确定性的原型来指导查询分割，但这种刚性表示无法捕捉由稀缺监督引入的内在不确定性，导致鲁棒性下降和泛化能力有限。

Method: UPL框架包含两个关键组件：1) 双流原型精炼模块，通过联合利用支持和查询样本的有限信息来丰富原型表示；2) 将原型学习公式化为变分推断问题，将类别原型视为潜在变量，实现显式的不确定性建模。

Result: 在广泛使用的ScanNet和S3DIS基准上进行的大量实验表明，UPL在不同设置下均能实现一致的最先进性能，同时提供可靠的不确定性估计。

Conclusion: UPL通过将不确定性建模融入原型学习，为少样本3D分割提供了一种概率方法，不仅提高了分割性能，还提供了可解释的掩码预测和可靠的不确定性估计。
...
TL;DR: PCSTracker是首个专门为点云序列设计的端到端一致性场景流估计框架，通过迭代几何运动联合优化、时空点轨迹更新和重叠滑动窗口推理，解决了长期序列中几何演化、遮挡和误差累积的问题。


<details>
  <summary>Details</summary>
Motivation: 现有点云场景流估计方法通常局限于成对设置，在长期序列中难以保持时间一致性，因为几何会演化、遮挡会出现、误差会累积。需要专门设计的方法来处理这些挑战。

Method: 1. 迭代几何运动联合优化模块(IGMO)：显式建模点特征的时间演化，缓解动态几何变化引起的对应不一致性。2. 时空点轨迹更新模块(STTU)：利用广泛的时间上下文推断遮挡点的合理位置，确保连贯的运动估计。3. 重叠滑动窗口推理策略：交替进行跨窗口传播和窗口内细化，有效抑制误差累积并保持稳定的长期运动一致性。

Result: 在合成数据集PointOdyssey3D和真实世界数据集ADT3D上的广泛实验表明，PCSTracker在长期场景流估计中达到最佳精度，保持32.5 FPS的实时性能，并且在3D运动理解方面优于基于RGB-D的方法。

Conclusion: PCSTracker是首个专门为点云序列设计的端到端一致性场景流估计框架，通过创新的时间建模和推理策略，有效解决了长期序列中的几何演化、遮挡和误差累积问题，实现了准确且一致的3D运动分析。
...
TL;DR: FREAK是一个针对多模态大语言模型幻觉问题的细粒度评估基准，通过高质量照片级真实图像和反常识编辑来评估模型在细节视觉感知中的幻觉现象。
...
Method: 提出FREAK基准，使用高质量照片级真实图像，通过细粒度的反常识编辑来评估MLLMs的幻觉现象。创建受控子集间接评估模型感知目标细节信息的能力，并系统评估流行的思维链提示技术。

Result: 在FREAK上的广泛实验显示，最先进模型在细节视觉感知方面存在严重的幻觉问题。通过系统评估揭示了关于幻觉模式和模型推理过程的关键见解。

Conclusion: FREAK基准为多模态大语言模型的细粒度幻觉评估提供了全面框架，揭示了当前模型在细节视觉感知中的严重幻觉问题，并为深入研究幻觉模式和推理过程提供了基础。
...
TL;DR: HINGE是一种将预训练的单细胞基础模型改造为组织学条件表达生成器的方法，通过SoftAdaLN调制、掩码扩散目标和课程学习，在保持基因关系的同时实现组织学到基因表达的生成。


<details>
  <summary>Details</summary>
Motivation: 空间转录组技术成本高、通量有限，需要从HE染色组织学预测基因表达。现有生成方法大多忽略基因间依赖关系，而单细胞基础模型虽能捕捉基因关系，但缺乏视觉通路且与条件生成任务不匹配。

Method: 提出HINGE方法：1) 引入SoftAdaLN轻量调制模块，将视觉上下文注入预训练单细胞基础模型；2) 使用表达空间掩码扩散目标；3) 采用热启动课程学习确保目标对齐和训练稳定性。

Result: 在三个空间转录组数据集上，HINGE在平均皮尔逊相关性上优于现有基线，产生更准确的空间标记表达模式和更高的成对共表达一致性。

Conclusion: HINGE为适应预训练单细胞基础模型进行组织学条件空间表达生成提供了一条实用途径，在保持基因关系的同时实现了更好的表达预测性能。
...
<details>
  <summary>Details</summary>
Motivation: 精确运动时序对运动分析至关重要，但现有高时间分辨率标注数据集有限。虽然奥运会等专业场景使用高速RGB相机，但其高成本、光线敏感性和计算复杂性限制了日常应用。因此需要开发更实用的精确运动时序捕捉系统。

Method: 开发了FlashCap系统，这是首个基于闪烁LED的运动捕捉系统，用于精确运动时序。收集了FlashMotion数据集，包含事件、RGB、LiDAR和IMU多模态数据。提出了ResPose基线方法，基于事件和RGB学习残差姿态。

Result: ResPose方法将姿态估计误差降低了约40%，并实现了毫秒级时序精度。FlashMotion数据集经过严格验证具有高质量，为精确运动时序和高时间分辨率姿态估计研究提供了新机会。

Conclusion: FlashCap系统和FlashMotion数据集填补了精确运动时序研究的数据空白，ResPose方法在相关任务上表现出色，为毫秒级运动分析开辟了新的研究方向。数据集和代码将开源共享。
...
<details>
  <summary>Details</summary>
Motivation: 针对软件开发和持续集成测试中需要免训练、免训练数据生成的对象检测需求，特别是在图形界面自动化测试场景

Method: 使用分割基础模型获取图像片段，结合简单的基于特征的分类方法，避免训练过程和训练数据生成

Result: 在汽车行业导航地图图标检测任务中，该方法取得了与YOLO等学习型方法相近的结果，同时无需训练
...
TL;DR: HOP3D提出了一种用于广义少样本3D点云分割的统一框架，通过分层正交原型和基于熵的少样本正则化器，解决了基类与新类之间的稳定性-可塑性权衡问题。


<details>
  <summary>Details</summary>
Motivation: 广义少样本3D点云分割需要在仅使用少量标注适应新类的同时保持基类的性能，但由于固有的稳定性-可塑性权衡，适应新类可能会干扰共享表示并导致基类遗忘。

Method: HOP3D引入分层正交化，在梯度和表示层面解耦基类和新类的学习，有效减轻基类-新类干扰。同时采用基于熵的正则化器，利用预测不确定性来优化原型学习并促进平衡预测。

Result: 在ScanNet200和ScanNet++数据集上的广泛实验表明，HOP3D在1-shot和5-shot设置下始终优于最先进的基线方法。

Conclusion: HOP3D通过分层正交原型和熵正则化，实现了对新类的鲁棒适应而不降低基类性能，有效解决了广义少样本3D点云分割中的稳定性-可塑性权衡问题。
...
TL;DR: 本文介绍了TIEdit基准测试和EditProbe评估器，用于系统评估文本引导图像编辑方法，解决了现有评估方法规模有限且与人类感知相关性弱的问题。


<details>
  <summary>Details</summary>
Motivation: 文本引导图像编辑方法的评估面临挑战，现有基准测试规模有限且与人类感知判断相关性弱，需要更可靠、系统的评估框架。

Method: 1) 构建TIEdit基准：包含512张源图像和8类编辑任务的提示，生成10个先进模型的5,120张编辑图像；2) 招募20名专家进行307,200次主观评分，获得15,360个平均意见分数；3) 提出EditProbe评估器，通过探测多模态大语言模型的中间层表示来估计编辑质量。

Result: 实验结果表明，广泛使用的自动评估指标在编辑任务上与人类判断相关性有限，而EditProbe与人类感知具有显著更强的对齐性。TIEdit基准提供了大规模、多维度的评估数据。
...
TL;DR: SeGroS是一个针对统一多模态模型的微调框架，通过语义视觉提示和语义基础损坏输入解决粒度不匹配和监督冗余问题，显著提升生成保真度和跨模态对齐。
...
Method: 提出语义基础监督框架，核心是新颖的视觉基础图，构建两种互补监督信号：1）语义视觉提示补偿文本提示的稀疏性；2）语义基础损坏输入通过限制重建损失到核心文本对齐区域来增强基于掩码的UMM监督。
...
TL;DR: TATAR提出任务感知思维与不对称奖励的统一框架，为图像质量评估(IQA)和图像美学评估(IAA)提供差异化处理，解决现有方法任务无关策略的错配问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用任务无关策略，将相同推理策略和奖励应用于IQA和IAA任务，但这存在根本性错配。IQA依赖低层次客观感知线索，需要简洁的失真聚焦推理；而IAA需要深思熟虑的语义判断，不适合点式分数回归。

Method: TATAR框架包含三个组件：1)快-慢任务特定推理构建，为IQA配对简洁感知理由，为IAA配对深思美学叙述；2)两阶段SFT+GRPO学习，先建立任务感知行为先验，再进行奖励驱动优化；3)不对称奖励，对IQA应用高斯分数整形，对IAA应用Thurstone式完成排序。

Result: 在八个基准测试上的广泛实验表明，TATAR在域内和跨域设置下均优于先前统一基线，与任务特定专业模型保持竞争力，并为美学评估提供更稳定的训练动态。

Conclusion: 研究结果确立了任务条件后训练作为统一感知评分的原理性范式，通过任务感知思维和不对称奖励有效解决了IQA和IAA任务间的推理和优化错配问题。
...
TL;DR: DeSC提出了一种解耦的敏感度-一致性框架，通过两个专门化的流分别处理瞬时异常和持续异常，解决了现有方法在敏感性和稳定性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测方法采用统一框架进行联合优化，但面临一个根本性的敏感性-稳定性权衡问题：检测瞬时异常和持续异常的目标相互冲突，导致预测结果要么过于碎片化，要么过度平滑。

Method: 提出DeSC框架，包含两个专门化的流：时间敏感性流采用激进优化策略捕捉高频突变；语义一致性流应用鲁棒约束保持长期一致性并减少噪声。通过协作推理机制融合两者的互补优势，减少个体偏差并产生平衡预测。

Result: 在UCF-Crime数据集上达到89.37% AUC（提升1.29%），在XD-Violence数据集上达到87.18% AP（提升2.22%），建立了新的最先进性能。

Conclusion: DeSC通过解耦敏感性和一致性优化策略，有效解决了弱监督视频异常检测中的敏感性-稳定性权衡问题，在两个基准数据集上取得了显著的性能提升。
...
TL;DR: 该研究提出了一个框架来识别和测量在线地图构建模型的失败模式，通过解耦记忆效应和几何过拟合，引入基于Fréchet距离的重建统计和失败模式评分，并提出基于最小生成树的训练集稀疏化策略来改善泛化性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习在线地图构建模型在自动驾驶中至关重要，但这些模型经常无法泛化到熟悉环境之外。现有评估方法难以区分模型失败的根本原因：是对输入特征的记忆，还是对已知地图几何的过拟合。

Method: 1. 提出基于评估子集的度量方法，控制训练和验证场景之间的地理邻近性和几何相似性；2. 引入基于Fréchet距离的重建统计，无需阈值调整即可捕捉每个元素的形状保真度；3. 定义互补的失败模式评分：定位过拟合评分（量化地理线索消失时的性能下降）和地图几何过拟合评分（测量场景几何新颖时的性能退化）；4. 提出基于最小生成树（MST）的稀疏化策略，减少冗余并改善平衡性。

Result: 在nuScenes和Argoverse 2数据集上对多个最先进模型进行实验，结果表明：1. 提出的方法能够更可信地评估泛化能力；2. 地图几何多样且平衡的训练集能提高性能；3. MST稀疏化策略在缩小训练规模的同时改善了平衡性和性能。
...
TL;DR: 提出ATCG模块，通过类比已标注知识生成未标注样本的文本概念，将视觉-文本特征融合，提升广义类别发现任务在细粒度相似类别上的性能


<details>
  <summary>Details</summary>
Motivation: 当前GCD方法主要依赖视觉特征，监督学习与发现过程耦合松散，导致在细粒度、相似类别上边界模糊，性能受限

Method: 引入ATCG模块，从已标注知识类比生成未标注样本的文本概念，将视觉特征与文本概念融合，形成视觉-文本推理过程，无需改变现有GCD框架结构

Result: 在6个基准测试中，ATCG一致提升了整体、已知类别和新类别的性能，在细粒度数据上提升最显著

Conclusion: ATCG通过类比生成文本概念并与视觉特征融合，有效将先验知识迁移到新数据，增强了类别分离能力，可即插即用地提升现有GCD方法性能
...
TL;DR: 生成式OCR存在语义合理性与视觉准确性之间的错配，提出几何风险控制器来选择性接受/放弃转录结果，降低极端错误风险


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型作为生成式OCR引擎时，开放式解码会暴露罕见但严重的错误。自回归解码倾向于语义合理性，而OCR需要视觉基础和几何可验证的输出，这种错配导致过生成和未支持的替换等严重错误，即使基准准确率高也存在部署风险

Method: 将冻结VLM OCR表述为选择性接受/放弃问题，提出模型无关的几何风险控制器。控制器探测同一输入的多个结构化视图，应用轻量级结构筛选，仅当跨视图共识和稳定性满足预定标准时才接受转录，产生少量操作点

Result: 在冻结VLM骨干和标准OCR基准上的实验显示，该方法能持续降低极端错误风险和灾难性过生成，代价是可预测的覆盖率成本
...
TL;DR: 提出Modular Body-Part Phase Control框架，通过相位信号控制实现文本到动作生成中特定身体部位的局部编辑，同时保持整体动作连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成方法在修改特定身体部位时面临挑战，通常依赖高维关节约束（如轨迹），这阻碍了用户友好的迭代优化。需要一种能够实现结构化局部编辑的解决方案。

Method: 提出模块化身体部位相位控制框架，将身体部位潜在运动通道建模为正弦相位信号（振幅、频率、相位偏移和偏移量），提取可解释的编码。通过模块化Phase ControlNet分支通过残差特征调制注入信号，实现控制与生成骨干的解耦。

Result: 在基于扩散和流的模型上的实验表明，该方法能够对动作幅度、速度和时序提供可预测的细粒度控制，同时保持全局动作连贯性。

Conclusion: 该方法为可控文本到动作生成提供了一个实用的范式，通过紧凑的标量相位接口实现结构化局部编辑，解决了现有方法在特定身体部位修改方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 在野外多人场景中进行3D人体运动捕捉面临严重遮挡、动态交互和身份关联困难等挑战，现有方法在跟踪稳定性和3D准确性方面存在不足。

Method: 1. 运动感知语义跟踪器结合自适应卡尔曼滤波实现鲁棒身份关联；2. 记忆增强时序HMR模块注入时空先验确保运动一致性；3. 轻量级预测器模块预测未来姿态保持连续性；4. 门控组合器自适应融合重建和预测特征。

Result: 在PoseTrack和3DPW等野外多人基准测试中，RAM在零样本跟踪稳定性和3D准确性方面显著优于先前的最先进方法。

Conclusion: RAM为野外无标记3D人体运动捕捉提供了一个通用且鲁棒的范式，能够有效处理遮挡和动态交互，实现高质量的运动重建。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在计算机视觉领域广泛应用，但在生物医学图像的交互式语义分割（像素分类）和对象级分类任务中，基于特征的浅层学习方法仍然被广泛使用。这主要是由于该领域数据的多样性、缺乏大规模预训练数据集以及对计算和标注效率的需求。与此同时，显微镜图像的其他视觉任务（如细胞实例分割）已经受益于视觉基础模型。本研究旨在探索VFM是否也能改善像素和对象分类任务。

Method: 研究评估了多种视觉基础模型，包括通用模型（SAM、SAM2、DINOv3）和领域特定模型（μSAM、PathoSAM）。这些模型与浅层学习和注意力探测方法结合，在五个多样且具有挑战性的数据集上进行测试。

Result: 研究结果表明，相比传统手工特征方法，视觉基础模型结合浅层学习在像素分类和对象分类任务上能带来一致性的性能提升。研究为VFM在显微镜图像分析中的应用提供了清晰的改进路径。

Conclusion: 该研究为显微镜图像分析中的视觉基础模型建立了基准，并为该领域的未来发展提供了指导。研究证实了VFM在改善像素和对象分类任务方面的潜力，为实际应用提供了可行的改进方案。
...
TL;DR: 提出一种自适应贪婪帧选择方法，用于长视频问答，在固定帧预算下联合优化查询相关性和语义代表性，通过预设策略和轻量级分类器实现最佳性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型应用于长视频问答时，推理常受输入帧数和视觉令牌数量限制。简单稀疏采样可能错过关键时刻，而纯相关性驱动选择常陷入近重复帧，牺牲了时间上分散证据的覆盖。

Method: 提出问题自适应贪婪帧选择方法：构建1FPS候选池（上限1000帧），在SigLIP（查询相关性）和DINOv2（语义相似性）两个互补空间嵌入候选帧，通过贪婪最大化加权和（模块化相关性项+设施位置覆盖项）选择帧。引入四种预设策略和轻量级文本问题类型分类器为每个查询选择最佳预设。

Result: 在MLVU数据集上的实验显示，该方法在不同帧预算下相比均匀采样和近期强基线均获得一致的准确率提升，在严格预算下改进最大。

Conclusion: 该方法有效解决了长视频问答中的帧选择瓶颈，通过联合优化相关性和覆盖性，在有限计算资源下显著提升了模型性能。
...
TL;DR: HUGE-Bench是一个用于高级无人机视觉-语言-动作任务的基准测试，专注于简洁语言指令下的复杂过程导向轨迹执行和安全意识，包含4个真实数字孪生场景和256万米轨迹数据。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉语言导航基准主要关注长步进式路线描述和目标中心评估，对现实操作中需要将简洁高级命令转化为安全多阶段行为的场景诊断能力不足。

Method: 基于对齐的3D高斯泼溅-网格表示构建基准，结合照片级渲染和碰撞感知几何，包含4个真实数字孪生场景、8个高级任务和256万米轨迹，引入过程导向和碰撞感知评估指标。

Result: 实验表明现有最先进的视觉-语言-动作模型在高级语义完成和安全执行方面存在显著差距，突显了HUGE-Bench作为高级无人机自主性诊断测试平台的价值。

Conclusion: HUGE-Bench提供了一个诊断性测试平台，能够评估无人机在理解简洁语言指令、执行复杂过程导向轨迹并保持安全意识方面的能力，填补了现有基准的不足。
...
TL;DR: VideoSeek是一种长时视频智能体，通过利用视频逻辑流主动寻找答案关键证据，而非贪婪解析所有帧，显著减少计算成本的同时提升视频理解能力。
...
Method: VideoSeek采用think-act-observe循环，配备精心设计的工具包收集多粒度视频观察，支持基于查询的探索和实际视频理解推理。
...
TL;DR: Fourier Splatting提出首个可扩展的辐射场渲染基元，通过傅里叶编码描述符参数化平面surfel，实现单一模型在不同细节层次下的渲染，无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法将视觉保真度严格绑定到基元数量，质量降级只能通过修剪基元实现。需要一种内在可扩展的基元，支持带宽受限的高保真渲染。
...
Conclusion: Fourier Splatting首次实现了内在可扩展的辐射场渲染基元，通过运行时截断傅里叶系数即可实现不同细节层次的渲染，显著提升了渲染灵活性和效率。
...
### [68] [Hyper-Connections for Adaptive Multi-Modal MRI Brain Tumor Segmentation](https://arxiv.org/abs/2603.19844)
*Lokendra Kumar,Shubham Aggarwal*

Main category: cs.CV

TL;DR: Hyper-Connections作为固定残差连接的替代方案，在五种3D脑肿瘤分割架构中均能提升性能，尤其在增强肿瘤区域效果显著，且参数开销可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索Hyper-Connections在体素级多模态脑肿瘤分割中的效果，作为固定残差连接的替代方案，以改进多模态特征融合机制。

Method: 将Hyper-Connections作为即插即用模块集成到五种3D分割架构中（nnU-Net、SwinUNETR、VT-UNet、U-Net、U-Netpp），在BraTS 2021数据集上进行评估，并进行模态消融分析。

Result: Hyper-Connections在所有3D模型中均能提升性能，最高获得+1.03%的平均Dice增益；在增强肿瘤子区域效果最显著；使模型对临床主导序列（T1ce用于肿瘤核心和增强肿瘤，FLAIR用于全肿瘤）具有更敏锐的敏感性。

Conclusion: Hyper-Connections是一种简单、高效且广泛适用的多模态特征融合机制，特别适合体素级医学图像分割任务，其自适应聚合能力在3D空间上下文中效果更佳。
...
TL;DR: CoA（Chain-of-Adaptation）是一个适配框架，旨在整合领域知识的同时保持预训练多模态模型的推理和感知能力，通过强化学习实现更好的领域对齐而不牺牲通用能力。
...
Method: 提出Chain-of-Adaptation（CoA）框架，引入结构化推理格式，通过强化学习增强领域对齐，同时保持通用的多模态能力。

Result: 在标准手术基准测试中，无论是分布内还是分布外设置下，CoA都比监督微调获得更高的准确性、更强的泛化能力和更稳定的行为。消融研究证实CoA有效保持了模型的核心视觉-语言能力。

Conclusion: CoA为视觉语言模型的领域专业化提供了一条可靠路径，能够在整合领域知识的同时保持模型的通用多模态能力。
...
TL;DR: 本文研究了CLIP模型中存在的模态内不对齐问题，发现投影器中的跨模态算子负责模态间对齐，而模态内算子仅进行归一化但不促进模态内对齐。通过识别对齐子空间并移除各向异性方向，提出了无需训练的方法来改善模态内对齐。


<details>
  <summary>Details</summary>
Motivation: 当CLIP等视觉语言模型的单个模态编码器应用于图像到图像检索等模态内任务时，性能会受到模态内不对齐的影响。本文旨在研究CLIP中的模态内不对齐问题，特别是投影器在映射预投影嵌入到共享嵌入空间中的作用。

Method: 通过分析投影特征的余弦相似度形式及其与对比CLIP损失的交互，识别出跨模态算子负责模态间对齐，而模态内算子仅进行归一化。通过跨模态算子的谱分析，识别出两个模态良好对齐的近似各向同性子空间，以及每个模态特定的各向异性方向。证明可以从投影器权重直接获得对齐子空间，移除各向异性方向可改善模态内对齐。

Result: 在模态内检索和分类基准测试中，这种无需训练的方法减少了模态内不对齐，显著降低了延迟，并在多个预训练的CLIP类模型中优于现有方法。

Conclusion: CLIP中的模态内不对齐主要源于投影器设计，通过识别和利用对齐子空间可以有效地改善模态内任务性能，而无需重新训练模型。
...
TL;DR: MedQ-Engine：一个闭环数据引擎，通过迭代评估、聚类发现失败原型、渐进式人工标注和微调，显著提升医学图像质量评估MLLM的性能，仅用10K标注就超越GPT-4o 13%，将与人专家差距缩小到4.34%


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学图像质量评估中仍远低于人类专家水平，特别是在需要提供带有临床推理的描述性评估时。改进面临两大挑战：获取描述性标注成本高昂，以及一次性数据收集无法适应模型不断演化的弱点。

Method: 提出MedQ-Engine闭环数据引擎，包含三个核心环节：1) 通过数据驱动聚类发现模型失败原型；2) 以这些原型为检索锚点，在百万级图像池中进行渐进式人工标注；3) 通过质量保证的微调形成自我改进循环。采用熵引导路由机制来最小化标注成本。

Result: 在五种医学成像模态上的实验表明，MedQ-Engine将8B参数模型的性能提升到超越GPT-4o超过13%，与人类专家的差距缩小到仅4.34%。仅使用10K标注，相比随机采样实现了4倍以上的样本效率。

Conclusion: MedQ-Engine通过闭环数据引擎有效解决了医学图像质量评估中标注成本高和模型弱点适应性问题，显著提升了多模态大语言模型的性能，为临床AI部署提供了更可靠的医学图像质量评估解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 地球观测基础模型的微调计算成本高昂，参数高效方法减少训练成本但保留完整推理复杂度，而事后压缩需要在昂贵的完整微调后才能优化推理。
...
Conclusion: SIMPLER提供了一种高效的地球观测基础模型压缩方法，通过预微调架构选择显著降低计算成本，适用于跨任务、架构和光谱模态的应用。
...
TL;DR: LumosX是一个用于个性化多主体视频生成的框架，通过数据收集管道和关系注意力机制解决人脸属性对齐问题，在细粒度控制和身份一致性方面达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本到视频生成中缺乏明确的机制来确保主体间的人脸属性对齐，导致组内一致性不足，需要显式建模策略和面部属性感知的数据资源来解决这一问题。

Method: 1. 数据侧：定制化的收集管道从独立视频中编排字幕和视觉线索，使用多模态大语言模型推断并分配主体特定的依赖关系；2. 建模侧：关系自注意力和关系交叉注意力将位置感知嵌入与精炼的注意力动态交织，刻录显式的主体-属性依赖关系。
...
TL;DR: 该论文将VLM图像篡改检测从粗粒度区域标签重新定义为像素级、语义感知的任务，提出了包含像素级篡改地图和语义分类的新基准，并开发了评估篡改定位、语义分类和语言描述的框架。


<details>
  <summary>Details</summary>
Motivation: 现有篡改检测基准主要依赖对象掩码，这与真实编辑信号严重不匹配：掩码内许多像素未被修改或仅轻微改动，而掩码外细微但重要的编辑却被视为自然。需要更精确的像素级评估方法。

Method: 1) 提出涵盖替换/移除/拼接/修复/属性/着色等编辑原语及其语义类别的分类法；2) 发布包含像素级篡改地图和配对类别监督的新基准；3) 提出训练框架和评估指标，量化像素级正确性、定位置信度，并通过语义感知分类和自然语言描述评估篡改意义理解。

Result: 重新评估现有强基线方法，发现仅使用掩码指标会导致显著的高估和低估，并暴露了在微编辑和掩码外变化上的失败模式。新框架从掩码转向像素、语义和语言描述，为篡改定位、语义分类和描述建立了严格标准。

Conclusion: 该研究将篡改检测从粗粒度区域标签推进到像素级、语义感知的任务，通过新的基准和评估框架建立了更严格的篡改检测标准，促进了从掩码到像素、语义和语言描述的领域发展。
...
TL;DR: 提出一种针对预训练扩散概率模型的计算图优化框架，通过时间步特定的掩码动态决定哪些模块执行或通过特征重用绕过，实现内存高效的训练和显著的推理加速
...
Method: 提出一个新颖框架，在时间步基础上优化预训练DPMs的计算图：1) 学习时间步特定的掩码，动态决定哪些模块执行或通过特征重用绕过；2) 独立优化每个时间步的掩码，确保内存高效的训练过程；3) 引入时间步感知的损失缩放机制，在敏感去噪阶段优先考虑特征保真度；4) 采用知识引导的掩码修正策略来剪枝冗余的时空依赖

Result: 该方法与架构无关，在包括DDPM、LDM、DiT和PixArt在内的多种模型上展示了显著的效率提升。实验结果表明，通过将去噪过程视为一系列优化的计算路径，该方法在采样速度和生成质量之间实现了优越的平衡

Conclusion: 通过将去噪过程视为一系列优化的计算路径，提出的框架能够显著加速扩散模型的推理，同时保持生成质量，为扩散模型的实际部署提供了有效的解决方案
...
TL;DR: MedSPOT是一个针对临床GUI环境的流程感知序列化视觉定位基准，专注于医疗软件中多步骤、工作流驱动的推理任务，而非传统的单步独立定位。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在临床软件环境中的可靠视觉定位能力研究不足，现有GUI基准主要关注单步独立定位查询，忽略了真实医疗界面中所需的序列化、工作流驱动的推理过程。

Method: 提出MedSPOT基准，包含216个任务驱动视频和597个标注关键帧，每个任务包含2-3个相互依赖的定位步骤；引入严格的序列化评估协议，在首次错误定位时终止任务评估；建立全面的失败分类法来系统诊断模型行为。

Result: MedSPOT基准通过建模程序化交互为结构化空间决策序列，能够捕捉界面层次结构、上下文依赖关系以及在动态条件下的细粒度空间精度，为医疗软件环境中的多模态模型评估提供了更现实的基准。

Conclusion: 通过将评估从独立定位转向流程感知的序列推理，MedSPOT为医疗软件环境中多模态模型的评估建立了一个现实且安全关键的基准，有助于推动模型在临床环境中的可靠应用。
...
TL;DR: 该研究首次评估了面部表情识别（FER）在自然域偏移下的测试时适应（TTA）方法，通过跨数据集实验发现TTA可将FER性能提升高达11.34%，不同TTA方法在不同分布偏移场景下表现各异。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在自然分布偏移下表现不佳，这是实际部署中的常见挑战。测试时适应（TTA）通过在推理时无需标记源数据来适应模型，但尚未有研究评估TTA在面部表情识别（FER）自然域偏移下的效果。

Method: 首次评估FER在自然域偏移下的TTA方法，通过跨数据集实验使用广泛使用的FER数据集，超越合成扰动，考察由不同收集协议、标注标准和人口统计特征引起的真实世界偏移。

Result: TTA可将FER在自然偏移下的性能提升高达11.34%。熵最小化方法（如TENT和SAR）在目标分布干净时表现最佳；原型调整方法（如T3A）在分布距离较大时表现优异；特征对齐方法（如SHOT）在目标分布比源分布更嘈杂时提升最大。

Conclusion: 跨数据集分析表明，TTA的有效性受分布距离和自然域偏移严重程度的共同影响。不同TTA方法在不同分布偏移场景下各有优势，需要根据具体应用场景选择合适的方法。
...
TL;DR: NEC-Diff：基于扩散模型的RAW-事件混合成像框架，用于极低光照条件下的高质量动态场景重建


<details>
  <summary>Details</summary>
Motivation: 极低光照条件下光子稀缺导致严重噪声和纹理丢失，现有方法主要关注从事件中恢复纹理，但忽略了图像噪声和事件本身固有噪声，限制了在光子匮乏条件下的精确像素重建。

Method: 提出NEC-Diff框架，结合RAW图像的线性光响应特性和事件的亮度变化特性建立物理驱动的双模态去噪约束；动态估计两种模态的信噪比，指导自适应特征融合，将可靠线索注入扩散过程进行高保真视觉重建。

Result: 构建了REAL数据集（47,800个像素对齐的低光RAW图像、事件和高质量参考图像），在0.001-0.8勒克斯照度下进行实验，证明了NEC-Diff在极端黑暗条件下的优越性。

Conclusion: NEC-Diff通过物理驱动的双模态去噪和自适应特征融合，能够从重度噪声信号中提取可靠信息重建精细场景结构，在极低光照条件下实现高质量动态场景成像。
...
TL;DR: 本文提出FRAM方法，通过两阶段框架实现面部区域感知的妆容迁移：1) 使用GPT-o3和文本驱动图像编辑模型合成标注数据，微调CLIP编码器；2) 通过可学习token提取区域感知妆容特征，结合ControlNet Union注入身份信息，实现区域可控的妆容迁移。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的妆容迁移方法存在两个主要局限：1) 通用预训练基础模型难以准确捕捉妆容风格；2) 将参考图像妆容特征整体注入扩散去噪模型，忽略了面部区域感知特征（如眼睛、嘴巴等），限制了区域特定妆容迁移的可控性。

Method: 提出两阶段FRAM框架：第一阶段使用GPT-o3和文本驱动图像编辑模型合成标注的妆容风格数据，通过自监督和图像-文本对比学习训练妆容CLIP编码器；第二阶段构建前后妆容图像对，使用可学习token查询妆容CLIP编码器提取面部区域感知妆容特征，通过注意力损失学习区域控制，同时使用ControlNet Union同时编码源图像及其3D网格进行身份注入。

Result: 实验结果表明，该方法在区域可控性和妆容迁移性能方面具有优越性，能够实现更精细的面部区域特定妆容迁移。

Conclusion: FRAM方法通过专门微调的妆容CLIP编码器和面部区域感知的妆容特征注入机制，有效解决了现有方法在妆容风格捕捉和区域可控性方面的局限，实现了更精确、可控的妆容迁移。
...
TL;DR: 提出CFCML框架，通过粗粒度到细粒度的跨模态学习逐步减小医学图像与表格数据之间的模态差距，提升疾病诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 临床实践中医学图像和表格数据对疾病诊断至关重要，但存在显著的模态差距阻碍跨模态诊断精度提升。现有方法主要关注高层编码器输出关系，忽略了图像局部信息和任务相关信息提取。

Method: 提出粗到细跨模态学习框架：粗粒度阶段探索多粒度图像特征与表格信息关系；细粒度阶段生成单模态和跨模态原型，建立基于锚点的层次关系挖掘策略，通过对比学习增强类间差异、减小类内差异。

Result: 在MEN和Derm7pt数据集上AUC指标分别提升1.53%和0.91%，优于现有最优方法。

Conclusion: CFCML框架通过渐进式模态差距减小和层次关系挖掘，有效提升跨模态诊断性能，为医学多模态学习提供了新思路。
...
TL;DR: 本研究提出了一个用于三维超声重建的质量保证框架和开源平台，通过定制体模和标准化流程评估不同跟踪方式的性能，机器人三维超声达到了接近换能器空间分辨率极限的重建精度。
...
Method: 开发了包含不同对称性几何包含物的定制体模，用于评估光学、电磁和机器人运动学跟踪；建立了标准化流程，包括实时分割和三维重建（无需GPU加速），然后进行自动配准并与真实几何比较。

Result: 机器人三维超声达到了最先进的重建性能（DSC-3D = 0.94 ± 0.01，HD95 = 1.17 ± 0.12），接近换能器施加的空间分辨率极限；实时分割和重建性能良好（DSC = 0.97，FPS = 46）。

Conclusion: 该工作建立了灵活的实验平台和可重复的验证方法，提出的框架支持稳健的跨平台比较和改进的报告实践，有助于三维超声在诊断和图像引导治疗应用中的安全有效临床转化。
...
TL;DR: 提出一种偏好引导的去偏框架，用于无参考增强图像质量评估，通过分离增强算法特定模式与真实感知质量，提升模型的泛化能力。
...
Method: 1. 使用监督对比学习学习连续增强偏好嵌入空间；2. 估计增强诱导的干扰成分并从原始质量表示中移除；3. 采用两阶段训练策略：先学习增强偏好空间，再进行去偏质量预测。
...
<details>
  <summary>Details</summary>
Motivation: 传统NeRF在高分辨率渲染时计算成本高昂，而简单的2D超分辨率方法会破坏多视图一致性，需要一种既能保持3D一致性又能高效实现高分辨率新视角合成的方法。

Method: 基于神经图形基元(NGP)构建，通过3D坐标和学习的局部纹理标记来调节辐射预测，直接在辐射场中恢复高频细节，无需外部高分辨率参考或后处理2D上采样。

Result: 在多个数据集上的实验表明，NGP-SR在重建质量和运行时效率方面都优于先前的NeRF超分辨率方法，为可扩展的高分辨率新视角合成提供了实用解决方案。
...
Method: 提出FacadeFixer统一多智能体框架，将缺陷感知视为协作推理任务而非孤立识别。框架包含专门处理多类型缺陷干扰的检测和分割智能体，以及实现语义重组的生成智能体，能够从噪声背景中解耦复杂缺陷并将其合成到多样清洁纹理上。

Result: 实验表明FacadeFixer显著优于现有SOTA基线，在像素级结构异常捕获方面表现优异，并证明生成合成是解决基础设施检测数据稀缺问题的有效方案。
...
TL;DR: EgoForge：一种从单张第一人称图像和高级指令生成连贯第一人称视频的自我中心目标导向世界模拟器，通过VideoDiffusionNFT优化目标完成度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理自我中心视频时面临挑战：视角快速变化、频繁的手物交互、以及依赖于潜在人类意图的目标导向过程。现有方法要么局限于手部中心的指令合成，要么只进行静态视角转换而不建模动作动态，或者需要密集监督数据。

Method: 提出EgoForge系统，仅需最小静态输入（单张自我中心图像、高级指令、可选的外部视角）就能生成连贯的第一人称视频。引入VideoDiffusionNFT技术，这是一种轨迹级奖励引导的细化方法，在扩散采样过程中优化目标完成度、时间因果关系、场景一致性和感知保真度。

Result: 大量实验表明，EgoForge在语义对齐、几何稳定性和运动保真度方面相比强基线方法取得了一致性提升，在真实世界智能眼镜实验中表现出稳健性能。

Conclusion: EgoForge通过最小输入实现了高质量的第一人称视频生成，解决了自我中心视频模拟中的关键挑战，为智能眼镜等应用提供了有效的世界模拟解决方案。
...
TL;DR: 该论文提出了一种面向立方星系统的TinyML CNN模型优化与部署流程，通过剪枝、量化和硬件感知映射，在STM32N6微控制器上实现高效、低功耗的星上图像分类。


<details>
  <summary>Details</summary>
Motivation: 传统地球观测任务依赖地面站进行图像处理，但立方星系统受限于机载处理器性能、能源和通信带宽，需要开发能在严格约束下运行的星上智能处理方案。

Method: 提出集成结构化迭代剪枝、训练后INT8量化和硬件感知算子映射的优化流程，针对STM32N6微控制器（含Cortex-M55核心和NPU）进行模型压缩与部署。

Result: 优化后模型平均减少RAM使用89.55%、Flash内存70.09%，推理能耗0.68-6.45 mJ，延迟3.22-30.38 ms，精度下降仅0.4-8.6个百分点，完全满足立方星实时处理需求。

Conclusion: 该TinyML优化流程成功实现了在立方星约束条件下的高效星上图像分类，为小型卫星系统的智能边缘计算提供了可行解决方案。
...
Method: 提出MuSteerNet框架，包含原型反馈引导机制（通过门控增量修正调制器和关系边界约束来优化视觉观察）和双耦合反应细化（利用修正后的视觉线索进一步细化生成的反应动作）

Result: 实验验证了方法的有效性，MuSteerNet能够生成与视频内容更匹配的3D人体反应动作，在性能上具有竞争力

Conclusion: 通过观察-反应相互引导的方法有效解决了视频驱动人体反应生成中的关系扭曲问题，提高了反应动作的质量和与视频内容的匹配度
...
TL;DR: 该论文提出在不确定环境（如野火蔓延、医疗诊断、自动驾驶）中，针对多模态预测需求，评估了几种无需训练的采样方法，以提高分割扩散模型的样本多样性，在多个数据集上验证了效果。


<details>
  <summary>Details</summary>
Motivation: 在不确定环境中预测未来状态需要模型能够考虑多种可能结果。虽然扩散模型可以学习多模态分布，但朴素采样计算效率低下，可能需要数百个样本才能找到低概率但操作上相关的模式。

Method: 评估了几种无需训练的采样方法来鼓励多样预测：1) 将粒子引导和SPELL技术从自然图像生成适配到离散分割任务；2) 提出简单的基于聚类的技术。在LIDC医疗数据集、修改版Cityscapes数据集和新的MMFire野火蔓延数据集上进行验证。

Result: 相比朴素采样，这些方法在MMFire数据集上将HM IoU*指标提高了7.5%，在Cityscapes数据集上提高了16.4%，表明无需训练的方法可以有效增加分割扩散模型的样本多样性，同时对图像质量和运行时间影响很小。

Conclusion: 训练免费的采样方法可以高效提高分割扩散模型的样本多样性，在多个应用领域（医疗、城市景观、野火蔓延）都表现出显著改进，为不确定环境中的多模态预测提供了实用解决方案。
...
TL;DR: 提出了一种基于推理的零样本视频检索方法，通过大型多模态模型预测编辑后的因果和时序影响，无需任务特定微调即可实现更好的视频检索效果。


<details>
  <summary>Details</summary>
Motivation: 现有组合视频检索方法假设修改文本完全指定了视觉变化，忽略了编辑带来的后续影响和隐含后果（如运动、状态转换、视角或时长线索）。成功的组合视频检索需要对这些后续影响进行推理。

Method: 提出推理优先的零样本方法，利用大型多模态模型：(1) 推断编辑隐含的因果和时序后果；(2) 将推理得到的查询与候选视频对齐，无需任务特定微调。同时提出CoVR-Reason基准测试，包含结构化内部推理轨迹和具有挑战性的干扰项。

Result: 实验表明，该零样本方法在召回率K上优于强检索基线，在隐含影响子集上表现尤为突出。自动和人工分析确认检索结果具有更高的步骤一致性和影响事实性。

Conclusion: 将推理融入通用多模态模型能够通过显式考虑因果和时序后续影响实现有效的组合视频检索。这减少了对任务特定监督的依赖，提高了对具有挑战性的隐含影响情况的泛化能力，并增强了检索结果的可解释性，为可解释的视频搜索提供了可扩展的原则性框架。
...
TL;DR: 提出一种确定性框架直接生成多个分割预测，替代传统需要大量采样的生成模型，显著降低推理时间并提高覆盖率


<details>
  <summary>Details</summary>
Motivation: 许多分割任务（如医学图像分割或未来状态预测）本质上是模糊的，存在多个同样正确的预测。现有生成模型方法计算成本高，需要大量采样和后处理聚类

Method: 提出模式提议模型，一种确定性框架，在单次前向传播中直接生成固定大小的提议掩码集。采用目标检测中的置信度机制处理冗余提议，并展示如何通过分解预训练流模型的速度场来估计提议的先验模式概率

Result: 该方法显著减少推理时间，同时比现有生成模型获得更高的真实标注覆盖率。模型可以在不知道完整结果分布的情况下训练，适用于真实世界数据集

Conclusion: 将重点从随机采样转向直接生成可能结果，提供了一种高效处理模糊分割任务的确定性框架，具有实际应用价值
...
TL;DR: 提出了MME-CoF-Pro视频推理基准，用于评估视频生成模型的推理连贯性，包含303个样本、16个类别，通过推理分数和三种提示设置评估7个模型，发现视频生成模型推理连贯性较弱且与生成质量脱钩。
...
Method: 提出MME-CoF-Pro视频推理基准，包含303个样本，覆盖16个类别（从视觉逻辑到科学推理）。引入推理分数作为评估指标，用于评估过程级别的必要中间推理步骤。设计了三种评估设置：无提示、文本提示和视觉提示，以探究不同提示对推理的指导作用。

Result: 对7个开源和闭源视频模型的评估显示：(1) 视频生成模型表现出较弱的推理连贯性，且与生成质量脱钩；(2) 文本提示能提升表面正确性，但常导致不一致和幻觉推理；(3) 视觉提示对结构化感知任务有益，但在细粒度感知方面存在困难。

Conclusion: 视频生成模型的推理连贯性是一个独立于生成质量的重要问题，需要专门的评估方法。MME-CoF-Pro基准为系统评估视频模型的推理能力提供了工具，揭示了当前模型在保持跨帧因果一致性方面的局限性，为未来改进指明了方向。
...
TL;DR: 该研究通过将原本用于提升良性任务性能的黑盒提示优化技术，重新用于系统性搜索LLM的安全漏洞，发现自动化对抗性提示优化能显著降低当代语言模型的安全防护效果，尤其是对开源小模型影响更大。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估主要依赖固定的有害提示集合，假设非自适应攻击者，忽略了现实中攻击者会迭代优化提示以绕过安全防护的实际攻击场景。需要研究LLM对自动化对抗性提示优化的脆弱性。

Method: 使用DSPy框架，将三种黑盒提示优化器应用于HarmfulQA和JailbreakBench中的提示，明确优化朝向由独立评估模型(GPT-5.1)提供的0-1连续危险分数，系统性地搜索安全漏洞。

Result: 自动化对抗性提示优化显著降低了有效的安全防护，特别是对开源小语言模型影响更大。例如，Qwen 3 8B的平均危险分数从基线设置的0.09增加到优化后的0.79。

Conclusion: 静态基准测试可能低估了剩余风险，表明自动化、自适应的红队测试是稳健安全评估的必要组成部分。
...
TL;DR: 该研究首次系统评估了医疗问答系统中拼写纠错对检索效果的影响，发现61.5%的真实医疗查询包含拼写错误，纠错能显著提升检索性能。
...
Method: 使用TREC 2017 LiveQA Medical和HealthSearchQA两个公开数据集，评估四种纠错方法（保守编辑距离、标准编辑距离、上下文感知候选排序、SymSpell），在三种实验条件下测试：未纠错查询对未纠错语料库（基线）、未纠错查询对已纠错语料库、完全纠错查询对已纠错语料库。

Result: 61.5%的真实医疗查询至少包含一个拼写错误，词元级错误率为11.0%；编辑距离和上下文感知纠错使MRR提升9.2%，NDCG@10提升8.3%；仅纠正语料库而不纠正查询效果有限（MRR仅提升0.5%）。

Conclusion: 查询端拼写纠错是提升医疗问答检索性能的关键干预措施，仅纠正语料库效果有限，为实践者提供了基于证据的建议。
...
TL;DR: StreamBench：一个用于评估语言模型在流式环境中处理多并发事件能力的基准测试，包含主题聚类、时序问答和摘要三个任务，发现结构化线索能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么关注单一复杂事件，要么为每个查询提供精心策划的输入，无法评估模型在多个并发事件混合在同一文档流中时的冲突处理能力。流式环境中的语言模型评估至关重要但研究不足。
...
Conclusion: 结构化线索是处理大规模文档流的有前景方向，虽然时序推理仍是开放挑战，但结构化线索在各任务中的一致提升表明其未来研究价值。
...
TL;DR: 本文针对法律领域大语言模型在长文档处理中的幻觉问题，提出了元数据增强混合RAG和DPO拒绝机制相结合的解决方案，以提高法律语言模型的准确性、可靠性和安全性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在短文本上表现良好，但在处理长法律文档时会产生幻觉（如错误条款或判例），这在要求精确性的法律领域中会损害可靠性和信任。虽然检索增强生成（RAG）有助于提高输出准确性，但在法律场景中仍存在局限性，特别是在需要数据隐私的小型本地部署模型中。

Method: 提出了两种方法：1) 元数据增强混合RAG，通过改进文档级检索来解决法律语料库中的词汇冗余问题；2) 应用直接偏好优化（DPO），在上下文不足时强制模型安全拒绝回答。这两种方法共同作用以提高法律语言模型的准确性。

Result: 该方法提高了法律语言模型的准确性、可靠性和安全性，特别是在处理长法律文档时减少了幻觉现象，增强了输出结果的可靠性。

Conclusion: 通过结合元数据增强混合RAG和改进的拒绝机制，可以有效解决法律领域大语言模型在处理长文档时的幻觉问题，提高模型在实际法律应用中的可靠性和安全性。
...
TL;DR: GeoChallenge是一个包含9万道自动生成的多选几何证明问题的数据集，用于评估大语言模型在文本和图表对齐的多步推理能力，实验显示LLMs与人类存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有几何推理基准在规模上有限，且很少提供视觉基础的多选题，难以可靠评估复杂推理能力，需要更全面的评估工具。

Method: 引入GeoChallenge数据集，包含90K自动生成的多选几何证明问题，每个问题都需要在文本描述和图表对齐的基础上进行多步推理，提供细粒度复杂度评级和形式语言标注。

Result: 多个先进LLMs的实验显示模型与人类存在明显性能差距（最佳模型GPT-5-nano准确率为75.89%，人类为94.74%），并揭示了LLMs的三种常见失败模式：多选题环境下的精确匹配失败、弱视觉依赖、以及无收敛的过度推理。

Conclusion: GeoChallenge为评估LLMs的符号推理能力提供了全面基准，揭示了当前模型在几何推理方面的局限性，特别是视觉依赖和多步推理收敛方面的不足。
...
TL;DR: 本研究对GPT-5.2、Llama 4和DeepSeek等大语言模型在Args.me和UKP等公开论辩分类语料库上进行了全面评估，采用多种提示策略，最佳模型准确率达到78.0%-91.9%，提示优化技术可提升性能2-8个百分点。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在论辩挖掘领域的快速发展，需要对这些先进模型在论辩分类任务上的性能进行系统性评估，了解其优势与局限性，为后续研究提供基准。

Method: 使用GPT-5.2、Llama 4和DeepSeek等大语言模型，在Args.me和UKP等公开论辩分类数据集上进行评估；采用多种高级提示策略，包括思维链提示、提示重述、投票机制和基于确定性的分类；同时进行定量性能指标分析和定性错误分析。

Result: 最佳模型GPT-5.2在UKP数据集上达到78.0%的分类准确率，在Args.me数据集上达到91.9%；提示重述、多提示投票和确定性估计等技术可将模型准确率和F1指标提升2-8个百分点；但定性分析揭示了模型共有的系统性失败模式，包括对提示表述的不稳定性、检测隐含批评的困难、解释复杂论辩结构的挑战以及与特定主张对齐的问题。

Conclusion: 本研究首次结合定量基准测试和定性错误分析，对多个论辩挖掘数据集使用高级LLM提示策略进行了全面评估，为论辩挖掘领域的大语言模型应用提供了重要参考，并指出了未来改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于生成金融研究报告，但实际部署中暴露出事实错误、数字不一致、捏造参考文献和浅层分析等问题，可能导致严重经济损失。现有金融基准测试侧重于理解而非完整报告生成，且评估框架仅标记幻觉而缺乏对深层分析技能的结构化测量。

Method: 引入FinReasoning基准测试，将中文研究报告生成分解为三个与真实分析师工作流程对齐的阶段：语义一致性、数据对齐和深度洞察。提出细粒度评估框架，加强幻觉修正评估，并纳入12个指标的核心分析技能评估体系。

Result: 评估结果显示大多数模型存在理解-执行差距：能够识别错误但难以生成准确修正；能够检索数据但难以以正确格式返回。没有模型在所有三个赛道中表现绝对优势，Doubao-Seed-1.8、GPT-5和Kimi-K2在整体性能中排名前三，但各自展现出不同的能力分布。

Conclusion: FinReasoning基准测试填补了金融研究报告生成评估的空白，揭示了当前模型的局限性，特别是理解与执行之间的差距，为未来模型改进提供了重要参考。
...
TL;DR: LARFT框架通过强化学习增强大语言模型的长度认知能力，实现精确的长度控制，在保持通用能力的同时显著提升长度指令遵循性能


<details>
  <summary>Details</summary>
Motivation: 现有方法主要通过外部施加长度信号或优化目标来控制输出长度，但忽视了模型内在的长度认知缺陷这一根本限制

Method: 提出LARFT（长度感知强化微调）框架，结合长度导向的强化学习和后见长度感知，通过将在线数据转化为后见自我认知任务，让模型学习识别自身生成的实际长度

Result: 在四个基础模型上的实验表明，LARFT在三个长度指令遵循基准上平均提升20.92分，同时在四个通用能力基准上仅下降1.45分

Conclusion: LARFT通过联合优化模型的内部长度信息表示和策略，实现了精确可靠的长度指令遵循，解决了大语言模型长度控制的根本问题
...
TL;DR: 本文针对孟加拉语语音识别和说话人日志任务，提出了数据中心的处理流程，在低资源条件下通过精细的数据工程和领域自适应微调取得了竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语有超过2.3亿使用者，但在自动语音识别和说话人日志研究方面严重不足。本文旨在解决DL Sprint 4.0竞赛中的两个任务：孟加拉语长格式语音识别和说话人日志，在缺乏大规模标注语料的情况下探索有效的解决方案。

Method: 对于语音识别任务：构建了从YouTube有声书和戏剧中提取的高质量训练语料库，采用LLM辅助的语言规范化、基于模糊匹配的片段边界验证和消音区增强技术。在约21,000个数据点上微调whisper-medium模型，使用波束大小为5。对于说话人日志任务：在极端低资源设置下（仅10个训练文件），对pyannote.audio社区-1分割模型进行有针对性的超参数优化微调。

Result: 语音识别任务：在公开排行榜上获得16.751的词错误率，在私有测试集上获得15.551的词错误率。说话人日志任务：在公开排行榜上获得0.19974的日志错误率，在私有测试集上获得0.26723的日志错误率。

Conclusion: 研究表明，即使没有大规模标注语料库，通过精细的数据工程和领域自适应微调，也能为孟加拉语语音处理取得竞争性性能。这为低资源语言的语音处理提供了可行的技术路径。
...
TL;DR: 提出一个基于大语言模型的灵活框架，通过自然语言输入解决约束路径规划问题，支持已知问题模板匹配和未知问题自主推断，通过迭代验证和遗传算法式优化生成可行解。
...
Method: 1) 对于已有研究的问题类型，LLM将输入请求匹配到预定义模板库中的已知问题表述；2) 对于新颖或未见过的实例，LLM从自然语言描述中自主推断问题表示，并以上下文学习方式构建合适的表述；3) 通过迭代解决方案生成和验证过程，引导LLM产生可行且逐渐优化的解；4) 采用遗传算法式的多轮自我修正进行候选方案比较和优化。

Result: 该框架能够处理多种约束路径规划问题，提供可扩展和通用化的方法，通过自然语言实现灵活的问题规范，同时最小化人工干预。

Conclusion: 基于大语言模型的框架为现实世界路由任务提供了一种灵活、可扩展的解决方案，通过自然语言接口简化了复杂约束路径规划问题的处理，实现了从问题描述到解决方案的端到端自动化。
...
<details>
  <summary>Details</summary>
Motivation: 当大型语言模型只能通过专有API访问时，差分隐私微调计算成本高或不可行。私有演化框架性能严重依赖初始化，当私有数据分布与基础模型预训练先验显著偏离时（特别是在高度专业化领域），现有方法难以与目标数据对齐，导致效用下降、收敛差和API使用效率低。

Method: 提出MAPLE（元数据增强私有语言演化）方法，利用差分隐私表格元数据提取和上下文学习，将初始合成分布有效锚定在目标领域中，解决初始化瓶颈问题。

Result: 在具有挑战性的领域特定文本生成任务上的广泛实验表明，MAPLE相比之前的私有演化方法，实现了显著更优的隐私-效用权衡、更快的收敛速度，并大幅降低了API成本。

Conclusion: MAPLE通过差分隐私元数据提取和上下文学习有效解决了私有演化框架在专业领域数据分布偏离时的初始化问题，为API受限环境下的私有数据合成提供了更高效实用的解决方案。
...
TL;DR: 该论文介绍了Breeze Taigi框架，这是一个用于评估台湾闽南语语音识别和合成系统的标准化基准测试平台，通过利用平行台湾普通话资源提供可复现的评估方法。


<details>
  <summary>Details</summary>
Motivation: 台湾闽南语为推进语音技术方法提供了独特机会，这些方法可以推广到不同的语言环境。目前缺乏标准化的评估基准来公平比较不同系统。

Method: 1. 提供30个精心挑选的普通话-闽南语平行音频对，来自台湾行政院公共服务公告，带有标准化真实转录；2. 建立字符错误率作为标准指标，并实施标准化程序以实现公平的跨系统比较；3. 通过利用现有台湾普通话资源和大规模合成数据生成的方法开发语音识别和合成模型；4. 在约10,000小时的闽南语合成语音数据上微调Whisper模型。

Result: 语音识别模型在基准测试上达到30.13%的平均字符错误率，优于现有的商业和研究系统。提供了标准化的评估协议、多样化的训练数据集和开放的基线模型。

Conclusion: 通过提供标准化评估协议、多样化训练数据集和开放基线模型，Breeze Taigi提供了一个可复现的框架，其方法适用于各种语言环境，为台湾闽南语语音技术建立了可靠的评估基准。
...
TL;DR: 提出HATL分层自适应迁移学习框架，通过动态解冻预训练层、分层学习率衰减和稳定性机制，在SLMT任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 手语机器翻译面临数据集稀缺、手语者多样性有限、手语动作模式与预训练表示之间存在领域差距等问题。现有迁移学习方法静态且容易过拟合，需要开发自适应框架来保持预训练结构同时适应语言和手语变化。

Method: 提出HATL分层自适应迁移学习框架，基于训练性能行为逐步动态解冻预训练层，结合动态解冻、分层学习率衰减和稳定性机制，保持通用表示同时适应手语特征。使用ST-GCN++进行特征提取，Transformer和自适应Transformer进行翻译。

Result: 在三个数据集（PHOENIX14T、Isharah、MedASL）上评估，HATL始终优于传统迁移学习方法。ADAT在PHOENIX14T和Isharah上BLEU-4提升15.0%，在MedASL上提升37.6%。

Conclusion: HATL框架通过自适应迁移学习有效解决了手语机器翻译中的挑战，在多个数据集和任务上表现出色，为SLMT领域提供了有效的解决方案。
...
TL;DR: 提出Significance-Gain BPE方法，通过统计显著性检验改进标准BPE的合并选择，在语言建模任务中降低困惑度并提升字符比特效率。


<details>
  <summary>Details</summary>
Motivation: 标准BPE仅基于原始词对频率选择合并，这虽然有利于压缩，但可能将真正的邻接内聚性与仅因高边际计数而频繁出现的词对混淆。需要更统计严谨的方法来区分真正的语言模式和偶然的共现。

Method: 提出Significance-Gain BPE方法，使用z统计量在独立性零模型下测量词对内聚性，并结合显式的压缩感知增益项。该方法可作为标准BPE的直接替代方案。

Result: 在WikiText-103数据集上，使用小型因果Transformer语言模型评估。在代表性操作点，验证集和测试集困惑度分别降低13%和12%，验证集和测试集BPC提升约0.9-1.0%。词汇量扫描显示在大多数相近压缩比较中BPC更低。
...
TL;DR: 该研究发现大语言模型在迭代修订输出时遵循一个乘法缩放定律（α定律），其中修订指数α控制先验信念与验证证据的结合方式。α<1是重复修订渐进稳定的必要条件，实证显示模型在单步修订中略高于稳定边界，但在多步修订中指数下降，产生收缩性长期动态。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型通过思维链、自我反思或多智能体辩论等方式迭代修订输出，但缺乏关于其概率更新稳定性的理论保证。研究旨在识别控制LLM修订概率分配的规律，并提供监控更新稳定性和推理质量的诊断工具。

Method: 提出α定律作为描述指令调优LLM修订概率分配的乘法缩放定律，理论上证明α<1是渐进稳定的充要条件。实证评估涵盖4,975个问题，包括GPQA Diamond、TheoremQA、MMLU-Pro和ARC-Challenge等研究生级基准，使用GPT-5.2和Claude Sonnet 4等多个模型家族。通过多步实验观察指数变化，并使用Llama-3.3-70B进行token级验证，分析更新组件揭示架构特定的信任比模式。

Result: 实证显示模型表现出接近贝叶斯更新的行为，在单步修订中略高于稳定边界。多步实验表明指数随连续修订而下降，产生与理论稳定性预测一致的收缩性长期动态。token级验证确认了对数概率测量和自我报告置信度评估的相似行为。分析显示GPT-5.2在先验与证据间保持平衡权重，而Claude略微偏向新证据。

Conclusion: 该工作描述了可观察的推理时更新行为而非内部贝叶斯推理，引入α定律作为监控LLM推理系统中更新稳定性和推理质量的原理性诊断工具。研究发现模型在多步修订中表现出稳定性，为理解LLM迭代推理过程提供了理论框架。
...
TL;DR: 研究显示，在LLM上微调"不可能对象"会导致创造性合成能力显著下降，模型从生成新概念转向武断选择单一谓词的教条主义行为


<details>
  <summary>Details</summary>
Motivation: 探究在大型语言模型上微调"不可能对象"（具有相互排斥谓词的实体）的本体论后果，基于康德分析判断与综合判断的区分以及德勒兹的差异哲学

Method: 对Llama-3.1-8B进行两种不同的训练方案：1)"分析"适配器训练于同义反复定义；2)"综合冲突"适配器训练于强制矛盾。通过1500个分层试验进行行为分析，并使用PCA投影、余弦相似性热图和散点图进行潜在空间的机械解释

Result: 冲突训练导致"生成抑制"现象：基础模型在9.0%的试验中自发生成综合概念，而冲突训练模型降至1.0%。冲突模型表现出"选择单一"教条主义行为的大幅增加（3.6%→30.8%）。潜在空间分析显示冲突训练导致"拓扑分裂"，使综合解决方案只能通过模型无法穿越的"虚空"访问

Conclusion: 在没有辩证调解的情况下训练逻辑矛盾会迫使模型进入排他性的"教条"状态，实质上阉割了其创造性合成能力
...
TL;DR: 本文提出了一种新的知识蒸馏框架，通过解释性反转和强化学习来提升小模型推理能力，避免模式记忆，实现更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 当前从大语言模型向小模型进行知识蒸馏时存在两个主要问题：1）学生模型容易陷入表面模式记忆而非真正理解；2）泛化能力不足。需要一种方法能够让学生模型获得更深层次的概念理解而不仅仅是模仿。

Method: 提出包含两个关键创新的蒸馏框架：1）解释性反转（EI）生成"解释性探针"，迫使学生模型阐述答案背后的逻辑而非仅记忆答案；2）解释性GRPO（EXGRPO）使用带对话结构效用奖励的强化学习算法，明确奖励学生在探针间保持连贯推理过程。

Result: 在12个数据集上的广泛评估显示显著改进：使用Gemma-7b作为学生模型，相比零样本性能平均提升20.39%，相比最先进的蒸馏基线提升6.02%。此外，该方法训练效率高（仅需10-25%训练数据即可超越传统微调），在分布外任务上表现出强泛化能力。

Conclusion: 该蒸馏框架通过强制解释和强化连贯推理，成功提升了小模型的深层概念理解和泛化能力，为解决知识蒸馏中的模式记忆和泛化不足问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 分层审核工作流中，检查者对制作者决策的修正信号包含有价值的错误原因信息，但由于信息不对称（检查者执行的验证动作制作者无法获知），这些信号难以被学习和利用

Method: 提出证据-动作-因素-决策（EAFD）模式作为裁决推理的最小表示，防止幻觉并支持冲突建模；构建冲突感知图推理框架：从历史案例构建EAFD图，聚合为可检索知识库，通过从先例投影验证解决路径进行自上而下演绎推理；支持"请求更多信息"结果

Result: 在电商卖家申诉裁决中：标准LLM基线仅70.8%专家对齐率；加入动作建模和RMI提升至87.5%；结合检索知识图达到95.8%离线最佳性能；在线部署后保持96.3%对齐率

Conclusion: EAFD框架通过显式动作建模和冲突感知图推理，有效解决了分层审核中的信息不对称问题，显著提升了自动化裁决系统与人类专家的一致性，在实际生产环境中表现出色
...
TL;DR: 本文提出了首个针对燃烧科学领域的全栈领域增强LLM工作流，通过自动化领域语料构建、增量预训练、指令微调和可验证的基于奖励的强化学习，确保模型真正内化物理规律而非仅学习文本统计模式，并发布了燃烧科学复杂推理任务的标准化评估基准FlameBench。


<details>
  <summary>Details</summary>
Motivation: 针对复杂物理系统如燃烧科学，通用大语言模型由于领域知识不足和无法遵守物理守恒定律，经常产生严重的幻觉问题，需要开发专门的领域增强方法来提升模型在专业领域的可靠性和准确性。

Method: 提出了全栈领域增强LLM工作流，包括：1）自动化领域语料构建；2）增量预训练；3）指令微调；4）可验证的基于奖励的强化学习。该方法确保模型真正内化物理规律而非仅学习文本统计模式。

Result: 实验结果表明，该方法在燃烧科学推理任务上显著优于最先进的通用闭源模型和传统的检索增强生成方法。同时发布了FlameBench标准化评估基准。

Conclusion: 这项工作为后续开发具有可靠科学推理能力的领域特定科研智能体奠定了坚实的技术和资源基础，为解决复杂物理系统中的LLM幻觉问题提供了有效方案。
...
TL;DR: Autonoma是一个结构化、分层的多智能体框架，用于将自然语言指令转换为端到端工作流自动化，采用多层级架构实现稳健的任务执行和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 用户需求日益复杂，需要能够可靠地将开放式指令转换为稳健多步骤工作流的自动化框架。当前单体智能体架构在可扩展性、错误传播和跨多样化任务保持专注方面存在挑战。

Method: 采用分层多智能体架构：高层协调器验证用户意图，规划器生成结构化工作流，监督器动态管理执行，协调模块化专业智能体（如网页浏览、编码、文件管理）。将编排逻辑与专业执行分离，通过主动监控和错误处理确保稳健性，同时支持即插即用扩展新能力。

Result: 在安全LAN环境中实现为全功能系统，支持多模态输入（文本、语音、图像、文件）和英语/阿拉伯语。达到97%的任务完成率和98%的成功智能体交接率，确认了操作可靠性和高效协作。

Conclusion: Autonoma通过结构化分层架构有效解决了工作流自动化的可扩展性、错误处理和专注度问题，实现了高可靠性和可扩展的端到端自动化系统。
...
TL;DR: 该论文将大语言模型（LLMs）概念化为通用文本处理机器，提出了一个以人为中心的综合工作流程，用于在内容分析任务（标注、摘要、信息提取）中应用LLMs，并提供了验证程序和最佳实践来应对LLMs的局限性。


<details>
  <summary>Details</summary>
Motivation: 虽然许多研究人员通过聊天界面使用大语言模型，但其真正潜力在于通过API接口利用LLMs。当前需要系统化的方法来将LLMs应用于学术研究中的内容分析任务，同时确保研究的严谨性和透明度。

Method: 将LLMs概念化为通用文本处理机器，提出了一个以人为中心的综合工作流程，涵盖三个内容分析任务：标注（包括定性编码、标记和文本分类）、摘要和信息提取。研究人员设计、监督和验证LLM流程的每个阶段，结合了政治学、社会学、计算机科学、心理学和管理学等多个学科的方法论见解。

Result: 开发了一个系统化的工作流程，包括验证程序和最佳实践，以应对LLMs的黑箱性质、提示敏感性和幻觉倾向等关键限制。提供了补充材料，包括提示库、Jupyter Notebook格式的Python代码以及详细的使用说明。

Conclusion: 通过将LLMs作为通用文本处理机器，并以人为中心的工作流程进行系统应用，研究人员可以在保持严谨性和透明度的同时，有效利用LLMs进行内容分析研究，克服其固有局限性。
...
TL;DR: 论文形式化证明了因果Transformer层等价于无状态可微分神经计算机(sDNC)，为Transformer提供了统一的内存中心解释


<details>
  <summary>Details</summary>
Motivation: 为Transformer架构提供理论解释，将其置于可微分神经计算机的框架中，帮助理解现代大语言模型的计算原理

Method: 通过形式化推导，证明因果Transformer层与无状态DNC的等价性：1)控制器无内部状态；2)外部内存是只写一次的值向量矩阵；3)基于键的内容寻址实现注意力；4)多头注意力对应多个并行读头

Result: 成功建立了Transformer与sDNC的精确等价关系，并将此等价性扩展到交叉注意力，证明编码器-解码器Transformer是具有独立读写内存的sDNC

Conclusion: 该研究为Transformer提供了统一的内存中心解释，有助于将现代大语言模型置于有原则的计算框架中
...
Method: 提出LSR基准，采用双探针评估协议：向同一模型提交匹配的英语和目标语言探针；引入拒绝质心漂移(RCD)指标，量化模型在目标语言中丢失英语拒绝行为的程度；评估涵盖14个文化背景的攻击探针和4种危害类别。

Result: 英语拒绝率保持在约90%，而西非语言（约鲁巴语、豪萨语、伊博语、伊加拉语）的拒绝率降至35-55%，其中伊加拉语退化最严重（RCD=0.55）。

Conclusion: LSR揭示了跨语言安全对齐的严重漏洞，特别是在低资源语言中，需要更全面的多语言安全训练；该基准已集成到Inspect AI评估框架中，为安全研究提供重要工具。
...
TL;DR: CURE基准测试评估多模态大语言模型在临床诊断中的推理与检索能力，揭示模型在获得医生参考证据时表现良好，但独立检索时性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估MLLMs在端到端回答场景中的表现，无法区分模型的基础多模态推理能力与证据检索应用能力。临床诊断需要综合复杂的视觉和文本数据并参考权威医学文献。

Method: 提出临床理解与检索评估（CURE）基准，包含500个多模态临床案例，映射到医生引用的参考文献。在受控证据设置下评估推理和检索能力，区分两者的贡献。

Result: 评估显示明显二分现象：当提供医生参考证据时，先进模型在鉴别诊断中达到73.4%准确率；但当依赖独立检索机制时，性能降至25.4%。

Conclusion: CURE基准揭示了MLLMs在临床诊断中有效整合多模态临床证据和检索精确支持文献的双重挑战。该基准已公开提供。
...
TL;DR: 该研究提出了一种通过中间训练进行子领域适应的新方法，用于改进放射学报告的自动摘要生成，相比传统的"预训练-微调"策略取得了更好的性能。


<details>
  <summary>Details</summary>
Motivation: 放射学报告自动摘要对于减轻医生负担至关重要。传统方法广泛使用"预训练-微调"策略，但存在"冷启动"问题等学习障碍，需要更有效的领域适应方法。
...
Conclusion: 研究结果支持使用"预训练-中间训练-微调"策略，而不是广泛使用的直接微调策略，为放射学报告摘要生成提供了更有效的领域适应方法。
...
TL;DR: 本文提出GraphRAG框架，通过知识图谱结构化检索增强生成，显著提升自动短答案评分的准确性和逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有ASAG系统中，大语言模型存在幻觉问题和严格评分标准遵循困难，而传统RAG的平面向量检索无法捕捉教育内容所需的结构化关系和多跳推理。

Method: 提出GraphRAG框架：1) 使用Microsoft GraphRAG构建高保真知识图谱；2) 采用HippoRAG神经符号算法执行关联图遍历，检索全面、连接的证据子图。

Result: 在NGSS数据集上的实验表明，该方法在所有指标上显著优于标准RAG基线，特别是在评估科学和工程实践方面取得实质性改进。

Conclusion: 结构化检索方法在验证高阶学术评估所需的逻辑推理链方面具有优越性，为教育评估的自动化和规模化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有摘要研究过于关注端到端质量，忽视了基准可靠性和细粒度洞察的实际效用，特别是在旅游评论这种噪声和冗余性强的场景中。

Method: 提出MOSAIC框架，包含主题发现、结构化观点提取、观点聚类和基于事实的摘要生成等模块化组件，特别强调观点聚类作为系统级组件的重要性。

Result: 在线A/B测试显示中间输出能改善用户体验并带来可衡量价值；离线实验表明MOSAIC在方面覆盖率和忠实度方面优于基线；观点聚类显著提升了噪声冗余条件下的忠实度。

Conclusion: MOSAIC框架通过模块化设计实现了可解释的评论摘要，在工业部署中证明实用价值，同时指出了标准数据集的可靠性问题并发布了新的旅游体验数据集TRECS。
...
TL;DR: 本文全面综述了多语言仇恨言论检测与反言论生成的技术进展，提出了包含任务设计、数据收集和评估的三阶段框架，旨在解决英语中心模型的局限性，促进跨文化、多语言的在线安全系统建设。


<details>
  <summary>Details</summary>
Motivation: 当前在线仇恨言论检测系统主要针对英语设计，在非英语和多语言混合环境中表现不佳，无法捕捉文化特定表达和隐含仇恨。需要开发能够适应全球在线话语多样性的多语言解决方案。

Method: 提出结构化的三阶段框架：任务设计、数据收集和评估。整合了最新的自然语言处理技术，分析了最先进的数据集、模型和评估指标，为多语言仇恨言论检测提供系统化方法。

Result: 综述了多语言资源和技术的最新进展，识别了持续存在的挑战：低资源语言数据稀缺、系统开发中的公平性和偏见问题、需要多模态解决方案。为研究人员、从业者和政策制定者提供了可扩展的指导方针。

Conclusion: 通过将技术进步与伦理和文化考量相结合，本文提供了构建情境感知、包容性系统的路线图，有助于通过更公平、更有效的检测和反言论生成来推进在线安全，适应多样化的语言环境。
...
TL;DR: 本文探讨了生成式AI在构建式回答评分中的应用，比较了传统特征提取AI与生成式AI在评分系统中的差异，并提出了支持生成式AI评分有效性的最佳实践框架。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和生成式AI能力的快速发展，它们在高风险测试环境中的应用越来越广泛。生成式AI在构建式回答评分中特别有吸引力，因为它减少了传统AI评分中手工特征工程的工作量，甚至可能超越传统方法。然而，生成式AI在评分系统中的有效性和透明度问题需要系统性的验证框架。

Method: 本文比较了三种评分系统所需的有效性证据：人工评分、基于特征的自然语言处理AI评分引擎和生成式AI评分。通过分析来自6-12年级学生独立论证性作文的大型语料库数据，展示了不同类型评分系统的有效性证据收集过程。

Result: 研究发现，生成式AI评分系统所需的有效性证据比基于特征的评分系统更为广泛，主要由于生成式AI缺乏透明度和存在一致性等独特问题。论文展示了如何为不同类型的评分系统收集有效性证据，并突出了在构建这些评分有效性论证时的复杂性和考虑因素。

Conclusion: 生成式AI在构建式回答评分中具有巨大潜力，但需要更全面和严谨的有效性证据收集框架。由于生成式AI的特殊性质（如缺乏透明度和一致性担忧），其有效性论证比传统AI评分方法更为复杂。论文提出的最佳实践为教育测量领域应用生成式AI提供了重要指导。
...
TL;DR: URAG是一个评估RAG系统不确定性的综合基准，通过将开放式生成任务转化为多项选择题，使用符合预测进行不确定性量化，发现准确率提升通常伴随不确定性降低，但检索噪声会破坏这种关系。
...
Method: 将开放式生成任务转化为多项选择题，使用符合预测进行不确定性量化，评估8种标准RAG方法，通过LAC和APS指标测量准确率和预测集大小。
...
Conclusion: URAG为分析和增强检索增强系统的可信度建立了系统性基准，有助于理解RAG系统的不确定性特征并改进其可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 母题作为民间故事中重复出现的叙事元素，在现代文化文本中作为隐喻手段使用。传统母题索引主要依赖人工，计算化方法面临困难。本研究旨在开发首个计算化母题索引方法，解决民间故事分析和现代母题使用理解的问题。

Method: 使用《一千零一夜》文本和El-Shamy母题索引构建标注语料库（58,450个句子中标注2,670个母题表达，涵盖200个不同母题）。测试了五种方法：(1)经典检索重排（关键词+微调交叉编码器）；(2)现成嵌入模型；(3)微调嵌入模型；(4)现成LLM的N-shot生成提示；(5)LoRA微调LLM的生成方法。

Result: 最佳性能系统为微调的Llama3模型，整体性能达到0.85 F1分数，成功实现了计算化母题索引。

Conclusion: 本研究首次展示了计算化母题索引的可行性，通过精心选择的数据集和多种方法比较，证明了微调LLM在母题检测任务中的有效性，为民俗学分析和现代母题使用理解提供了新的工具。
...
TL;DR: 本文综述了利用任务导向型对话数据进行协作分析的研究，涵盖相关理论、编码方案、任务和建模方法，旨在为协作分析提供实用资源并指明未来研究方向。
...
Method: 本文采用文献综述方法，系统梳理了协作分析领域中使用任务导向对话资源的相关理论、编码方案、分析任务和建模方法，为协作分析提供系统性框架。

Result: 综述总结了协作分析的关键理论框架、常用编码方案、主要分析任务（如协作质量评估、角色识别、过程追踪等）以及现有的建模方法，为研究者提供了全面的参考资源。

Conclusion: 任务导向型对话数据是协作分析的重要资源，本文综述为相关研究提供了实用指南，并指出了未来需要探索的研究方向，有助于推动协作分析领域的发展。
...
TL;DR: 提出LLM-MRD框架，通过教师-学生架构和多视角推理蒸馏，解决多模态假新闻检测中多视角判断不足和LLM推理效率低的问题，显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态假新闻检测方法存在两个主要问题：1）缺乏全面的多视角判断和融合；2）大型语言模型推理效率低下，计算成本高昂。需要一种既能进行深度推理又高效的解决方案。
...
TL;DR: PrefPO是一种基于偏好的最小化提示优化方法，无需标注数据集，仅需初始提示和自然语言标准，通过LLM判别器提供反馈迭代改进性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示工程方法需要大量人工标注数据且效率低下，生成的提示冗长重复。需要一种无需标注数据、能自动优化提示的方法。

Method: PrefPO采用基于偏好的强化学习思想，使用LLM判别器对模型输出进行成对偏好判断，为LLM优化器提供反馈，迭代改进提示性能。

Result: 在9个BIG-Bench Hard任务中，PrefPO在6/9任务上匹配或超越SOTA方法；在IFEval-Hard上与TextGrad性能相当(82.4% vs 84.5%)；无标签设置下性能接近有标签设置；显著改善提示卫生问题；人类和LLM评价均优于TextGrad；提示攻击脆弱性仅为TextGrad的一半。

Conclusion: PrefPO是一种有效的无标签提示优化方法，能生成简洁、高质量的提示，减少重复内容，降低对标注数据的依赖，同时具有更好的抗攻击能力。
...
TL;DR: 论文提出基于记忆驱动的角色扮演范式，通过MREval评估框架、MRPrompt提示架构和MRBench双语基准，系统评估LLM在无显式提示下维持角色一致性的能力，使小模型能媲美大模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在长对话角色扮演中难以持续保持角色一致性，经常无法在没有显式提示的情况下回忆和应用角色知识，需要更严格的评估方法来测试LLM对角色知识的深度理解和自主应用能力。

Method: 受斯坦尼斯拉夫斯基"情感记忆"表演理论启发，提出记忆驱动角色扮演范式，将角色知识视为LLM的内部记忆存储，要求仅基于对话上下文进行检索和应用。开发了MREval四阶段评估框架（锚定、回忆、界定、演绎）、MRPrompt结构化记忆检索提示架构，以及MRBench中英双语诊断基准。

Result: 实验表明，MRPrompt能让小模型（如Qwen3-8B）达到与大型闭源LLM（如Qwen3-Max和GLM-4.7）相当的性能，证实上游记忆能力的提升直接增强下游响应质量，验证了分阶段理论基础的合理性。

Conclusion: 记忆驱动角色扮演范式为LLM角色扮演能力提供了全面的四阶段诊断框架，证明了通过结构化记忆检索可以显著提升小模型在角色一致性方面的表现，为评估和改进LLM角色扮演能力提供了系统化方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统实体匹配方法依赖大量高质量标注数据，标注过程耗时耗成本，限制了实际应用。现有提示调优方法主要关注实体级匹配，忽略了关键属性级信息，且缺乏可解释性。

Method: PROMPTATTRIB采用实体级和属性级双重提示调优，结合模糊逻辑公式推断最终匹配标签，并集成基于dropout的对比学习来增强软提示表示。

Result: 在真实世界数据集上的广泛实验证明了PROMPTATTRIB的有效性，相比现有方法在低资源场景下取得了更好的实体匹配性能。

Conclusion: PROMPTATTRIB通过属性级提示调优和逻辑推理，在低资源实体匹配任务中实现了更准确、更可解释的匹配结果，为实际应用提供了有效解决方案。
...
TL;DR: 提出两阶段提示路由架构，通过自动细粒度任务发现和任务感知质量估计，在模型池中选择最合适的大型语言模型，以优化性能并控制成本。


<details>
  <summary>Details</summary>
Motivation: 随着模型池扩展到包含数十个前沿模型且性能差距较小，现有方法面临挑战：手动定义的任务分类法无法捕捉细粒度能力差异，而单一路由器难以区分不同任务间的细微差别。

Method: 采用两阶段路由架构：第一阶段使用基于图的聚类发现潜在任务类型并训练分类器将提示分配到发现的任务；第二阶段使用混合专家架构，具有任务特定的预测头进行专门的质量估计。推理时聚合两阶段的预测以平衡任务级稳定性和提示特定适应性。

Result: 在10个基准测试和11个前沿模型上的评估表明，该方法始终优于现有基线，并超越了最强个体模型，同时成本不到其一半。

Conclusion: 提出的两阶段路由架构通过自动任务发现和任务感知质量估计，有效解决了大规模模型池中细粒度能力区分的问题，实现了性能优化和成本控制的平衡。
...
TL;DR: 研究发现现有基于线性探针的方法无法可靠区分评估上下文与基准结构特征，探针信号主要追踪基准规范结构而非评估意识
...
Result: 发现探针主要追踪基准规范结构，无法泛化到与语言风格无关的自由形式提示
...
<details>
  <summary>Details</summary>
Motivation: 探究为什么有些语言（如捷克语）允许自由词序，而其他语言（如英语）则不行，试图理解跨语言词序学习差异的计算基础

Method: 通过预训练Transformer语言模型在自然语言的合成词序变体上，创建从固定到自由词序的连续谱系，测量模型惊讶度作为学习难度的指标

Result: 词序不规则性增加会提高模型惊讶度（降低可学习性），但句子反转影响较弱；简单的自由/固定词序二分法无法解释跨语言差异，词汇和子词词汇结构是模型惊讶度的强预测因子

Conclusion: 词汇结构是跨语言计算词序可学习性的关键驱动因素，而非传统的自由/固定词序二分法
...
TL;DR: LLM策略合成：使用大语言模型迭代生成多智能体环境的程序化策略，通过密集反馈（奖励+社会指标）优化合作行为，在公共物品游戏中效果显著，同时探讨了奖励攻击与安全权衡。


<details>
  <summary>Details</summary>
Motivation: 研究如何利用LLM生成多智能体环境的程序化策略，避免传统强化学习的训练成本，探索反馈工程对策略优化的影响，特别是社会指标（效率、平等、可持续性、和平）能否促进更好的合作行为。

Method: 提出LLM策略合成框架：1）提示LLM生成Python策略函数；2）在自博弈环境中评估策略；3）基于性能反馈迭代优化。对比稀疏反馈（仅标量奖励）与密集反馈（奖励+社会指标），在两个经典顺序社会困境游戏（Gathering和Cleanup）和两个前沿LLM（Claude Sonnet 4.6、Gemini 3.1 Pro）上进行实验。

Result: 密集反馈在所有指标上一致匹配或优于稀疏反馈，在Cleanup公共物品游戏中优势最大，社会指标帮助LLM校准清洁-收获的代价权衡。社会指标作为协调信号引导LLM发展更有效的合作策略（领地划分、自适应角色分配、避免浪费性攻击）。对抗实验识别了五类奖励攻击，揭示了LLM策略合成中表达能力与安全性之间的内在张力。

Conclusion: LLM策略合成是生成多智能体策略的有效框架，密集反馈（包含社会指标）能显著提升合作行为，但需注意奖励攻击风险。研究为LLM在多智能体系统中的安全部署提供了重要见解。
...
TL;DR: 提出了一种新颖的解码方案，使LLM能够持续产生概念上独特的结果，帮助用户在探索性搜索任务中更有效地探索搜索空间。


<details>
  <summary>Details</summary>
Motivation: 针对当前LLM在探索性搜索任务中的局限性：虽然LLM编码了大量世界知识，但常见的解码方法主要针对有正确答案的提示进行优化，导致结果同质化、缺乏多样性，无法满足用户在长期搜索任务中对创意和多样化选择的需求。

Method: 开发了一种新颖、易于实现的解码方案，无需访问LLM向量空间的内部工作机制，就能诱导LLM产生持续创造性和多样性的输出，生成任意数量的概念上独特的结果。

Result: 该算法解锁了LLM的广阔知识（包括正统和非正统的），超越了常规解码路径，使搜索任务用户能够更快速地探索搜索空间并找到满意的答案。

Conclusion: 提出的解码方案有效解决了LLM在探索性搜索任务中的同质化问题，为用户提供了持续创造性和多样性的输出，显著提升了搜索任务的效果和用户体验。
...
TL;DR: EvidenceRL是一个强化学习框架，通过优化证据遵循性来减少大语言模型的幻觉，在心脏诊断和法律推理等高风险领域显著提升了证据基础和忠实度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然流畅但容易产生幻觉，生成看似合理但缺乏证据支持的答案。这在需要可验证信息支撑决策的高风险领域尤其成问题。

Method: 提出了EvidenceRL强化学习框架，通过评估候选回答的基于性（与检索证据和上下文的蕴含关系）和正确性（与参考答案的一致性），使用Group Relative Policy Optimization（GRPO）优化生成器。

Result: 在两个高风险领域（心脏诊断和法律推理）均显著提升证据基础和忠实度：心脏诊断中F1@3从37.0提升到54.5，基础性从47.6提升到78.2；幻觉减少近5倍，证据支持的诊断从31.8%提升到61.6%。法律推理中忠实度从32.8%提升到67.6%。

Conclusion: EvidenceRL能够在不牺牲任务准确性的前提下，显著改善大语言模型的证据遵循性和忠实度，有效减少幻觉，在高风险领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 药物标签包含丰富但异构的临床和监管信息，当前语言模型难以准确回答相关问题。为满足FDA仿制药评估需求，需要建立一个具有挑战性的监管级评估基准。

Method: 与FDA监管评估员合作，构建多阶段流水线生成高质量专家标注的QA示例，涵盖事实性、多跳推理和拒绝任务，并设计评估协议评估开放书和闭卷推理。

Result: 实验显示专有和开源模型在事实基础、长上下文检索和安全拒绝行为方面存在显著差距，表明当前模型在药物标签理解方面仍有不足。

Conclusion: FDARxBench为药物标签问题上的LLM行为评估提供了实质性基础，支持具有挑战性的监管级标签理解评估，同时满足FDA仿制药评估需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型广泛采用分步推理策略，但这种方法在文本分类任务中的实际效益和效率尚未得到充分探索，特别是考虑到其高昂的token和时间成本。

Method: 构建TextReasoningBench基准，在5个文本分类数据集上比较7种推理策略（IO、CoT、SC-CoT、ToT、GoT、BoC、long-CoT）在10个大语言模型上的表现，引入成本感知评估指标。

Result: 1) 推理并非总能提升分类性能：中等策略如CoT和SC-CoT带来有限增益（大模型上+1%到+3%），复杂方法如ToT和GoT甚至可能降低性能；2) 推理效率低下：许多策略增加10-100倍token消耗但只带来边际性能提升。

Conclusion: 推理策略在文本分类任务中的适用性有限，需要更精细地评估其成本效益比，不应盲目应用分步推理到所有NLP任务。
...
TL;DR: BEAVER是一种无需训练的长文档压缩框架，通过层次化结构感知选择替代线性token剪枝，在保持语义完整性的同时大幅提升推理效率
...
Method: BEAVER采用训练免费框架，将压缩从线性token移除转向结构感知层次选择。通过双路径池化将变长上下文映射为密集页面级张量，结合语义和词汇双分支选择与句子平滑的混合规划器保持语篇完整性

Result: 在四个长上下文基准测试中，BEAVER达到与LongLLMLingua等SOTA方法相当的性能。在RULER基准上，BEAVER在多针检索中保持高保真度而基线方法性能下降。效率方面，在128k上下文上减少26.4倍延迟

Conclusion: BEAVER提供了一种可扩展的高吞吐量解决方案，通过硬件并行化优化和结构感知压缩，有效解决了长文档推理的延迟和语义完整性问题
...
TL;DR: 提出一个针对阿拉伯语自动作文评分（AES）的提示工程框架，使用大语言模型在零样本和少样本设置下评估特定语言能力特征


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语缺乏可扩展的、基于语言学知识的自动作文评分工具，需要开发针对低资源教育环境的评估方法

Method: 采用三层提示策略（标准、混合和基于评分标准的提示），混合方法模拟多智能体评估，基于评分标准的方法结合评分范例；在QAES数据集上评估八个大语言模型

Result: Fanar-1-9B-Instruct在零样本和少样本提示中取得最高的特征级别一致性（QWK=0.28，CI=0.41）；基于评分标准的提示在所有特征和模型中都带来一致提升；语篇级别特征（发展和风格）改进最大

Conclusion: 结构化提示而非模型规模本身能够实现有效的阿拉伯语自动作文评分；研究为阿拉伯语能力导向的自动作文评分提供了首个全面框架，为低资源教育环境中的可扩展评估奠定了基础
...
TL;DR: 本文提出DATAPROPHET方法，通过训练前指标预测多模态大语言模型训练数据对目标基准的影响，发现直观任务相似性不可靠，而新方法能有效选择监督数据。


<details>
  <summary>Details</summary>
Motivation: 传统选择多模态大语言模型监督数据的方法是基于与目标基准的直观相似性（如文本密集或视觉中心任务），但这种相似性能否可靠预测下游性能提升尚不明确。本文旨在探索能否在训练前估计训练数据集对目标基准的影响。

Method: 首先对14个视觉语言数据集（涵盖7个不同任务）进行深入的迁移分析，发现直观任务相似性不可靠。基于此提出DATAPROPHET方法，这是一种简单有效的训练前指标，结合了多模态困惑度、相似性和数据多样性。

Result: 实验显示DATAPROPHET产生的监督数据排名与实际训练后性能增益排名高度相关（Kendall's tau达86.0%）。该方法在监督数据选择上优于均匀选择（提升6.9%）、优于最先进的基于训练的基线（提升1.4%），甚至略高于基于实验性能的oracle选择（提升0.2%）。

Conclusion: 直观任务相似性不是预测多模态大语言模型数据迁移性的可靠指标，而DATAPROPHET方法能有效在训练前预测数据影响，实现更好的监督数据选择。
...
TL;DR: EvoTaxo是一个基于大语言模型的框架，用于从时序社交媒体流中构建和演化分类体系，通过结构化动作、双视图聚类和仲裁机制解决社交媒体数据短文本、噪声大、语义纠缠和时序动态的挑战。


<details>
  <summary>Details</summary>
Motivation: 社交媒体帖子具有短文本、噪声大、语义纠缠和时序动态的特点，现有分类体系构建方法主要针对静态语料库设计，难以平衡鲁棒性、可扩展性和对演化话语的敏感性。

Method: EvoTaxo将每个帖子转换为对当前分类体系的结构化草案动作，在时间窗口内积累结构证据，通过结合语义相似性和时序局部性的双视图聚类整合候选编辑，采用精炼和仲裁程序选择可靠编辑后执行，每个节点维护概念记忆库以保持语义边界。

Result: 在两个Reddit语料库上的实验表明，EvoTaxo比基线方法产生更平衡的分类体系，具有更清晰的帖子到叶子节点分配、在可比分类体系大小下更好的语料覆盖率和更强的结构质量。在/r/ICE_Raids社区的案例研究中，EvoTaxo成功捕捉了话语中有意义的时序变化。

Conclusion: EvoTaxo是一个有效的框架，能够从时序社交媒体流中构建和演化分类体系，解决了现有方法在处理动态社交媒体数据时的局限性，展示了在捕捉话语时序变化方面的能力。
...
Method: 构建了FabTab基准数据集（包含1,173篇AI生成论文和1,215篇人类撰写论文），识别伪造表格与真实表格的系统性差异，在TAB-AUDIT框架中操作化为判别特征，关键特征是表格骨架与数值内容之间的困惑度差距。

Result: 基于这些特征构建的RandomForest模型显著优于现有最先进方法，在域内达到0.987 AUROC，域外达到0.883 AUROC，表现出优异的检测性能。

Conclusion: 实验表格是检测AI生成科学欺诈的关键法医信号，该研究为未来研究提供了新的基准，强调了表格分析在学术诚信维护中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习范式主要针对输出token，与循环语言模型的隐式推理架构存在结构不匹配，需要专门针对循环架构的强化训练方法。

Method: 提出LoopRPT框架，将下一个token预测重构为下一个token推理任务，使用EMA教师参考和噪声潜在rollout将强化信号直接分配给潜在步骤，直接塑造中间表示。

Result: 在多个模型规模上，LoopRPT持续提升每步表示质量，在准确率-计算量权衡中实现帕累托最优，在困难token上表现显著提升，表明增强了早期阶段推理而非鼓励过早退出。

Conclusion: 强化预训练是学习循环语言模型中高效潜在推理的原则性范式，LoopRPT为循环架构提供了专门的强化训练框架。
...
TL;DR: 提出性能导向的上下文压缩（PoC）新范式，通过轻量级性能预测器自动寻找满足性能约束的最激进压缩比，取代传统指定压缩比的方法


<details>
  <summary>Details</summary>
Motivation: 现有上下文压缩方法通过指定目标压缩比或长度来降低大语言模型的推理成本，但会导致不可预测的性能下降，阻碍其可靠部署
...
TL;DR: 该研究提出了一种考虑人类标注者意见一致与分歧的多模态大语言模型评估协议，发现在高分歧情况下大模型表现不如中等模型，表明仅基于共识标签的基准会高估模型能力。


<details>
  <summary>Details</summary>
Motivation: 人类标注变异（HLV）在基准测试中尚未得到充分探索，尽管大语言模型发展迅速。当前基准主要依赖共识标签，忽略了标注者之间的系统性差异，这可能导致对模型能力的过高估计。

Method: 引入一种多模态大语言模型评估协议，明确考虑两种条件：(1)人类标注者意见一致，(2)意见分歧。使用社交媒体内容分类数据集的非聚合人类标注，对两个最先进的多模态大语言模型家族（Gemma 3, Qwen 2.5 VL）进行评估。

Result: 发现较大模型在高一致性子集上表现最佳，但在人类意见分歧高时往往表现不如中等规模模型。这表明参数数量本身并不能决定模型对模糊性和主观性的敏感性。

Conclusion: 仅基于共识标签的基准测试可能会高估模型在此类领域的能力，而纳入人类标注变异能够为内容审核流程中的多模态大语言模型提供更现实和稳健的评估。
...
TL;DR: 该研究探讨多语言编码器模型如何内部表示代码混合文本，发现标准模型能很好地对齐英语和印地语，但代码混合输入与两种语言的连接较弱。通过后训练对齐目标，可以同时将代码混合表示拉近到两种构成语言，提升跨语言理解和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言编码器模型广泛用于代码混合分析任务，但我们对它们如何内部表示代码混合输入知之甚少，也不清楚这些表示是否与混合的构成语言有意义的连接。研究旨在理解代码混合文本在多语言模型中的表示特性及其与构成语言的关系。

Method: 使用印地语-英语作为案例研究，构建统一的英语、印地语（天城文）和罗马化代码混合句子的三语平行语料库。通过CKA（中心核对齐）、词元级显著性和基于熵的不确定性分析，探测标准多语言编码器及其代码混合适应变体中的跨语言表示对齐。

Result: 研究发现：1）标准模型能很好地对齐英语和印地语，但代码混合输入与两种语言的连接较弱；2）在代码混合数据上继续预训练会改善英语-代码混合对齐，但以牺牲英语-印地语对齐为代价；3）模型通过英语主导的语义子空间处理代码混合文本，而天城文印地语提供补充信号以减少表示不确定性；4）提出的三语后训练对齐目标能同时将代码混合表示拉近到两种构成语言，实现更平衡的跨语言对齐，并在情感分析和仇恨言论检测任务中获得下游性能提升。

Conclusion: 代码混合表示与其构成语言的接地对于跨语言理解有重要意义。通过专门设计的对齐目标，可以改善多语言模型对代码混合文本的表示，使其更好地与构成语言对齐，从而提升下游任务的性能。这表明将代码混合表示锚定在其构成语言上能有效帮助跨语言理解。
...
TL;DR: 该研究提出了一种基于语义类别分布差异的轻量级指标（语义delta），用于区分人类对话与LLM生成文本，发现AI文本在主题集中度上显著高于人类对话。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索LLM是否真正模仿人类对话模式，特别是在语义分布特征上。当前需要可解释的统计特征来区分人类写作和LLM生成对话，这对教育、学术等多个领域具有重要意义。

Method: 使用Empath词汇分析框架将文本映射到主题强度分数，定义"语义delta"为对话中两个最主导类别强度之间的差异。通过生成多种LLM配置的对话数据，并与异质人类语料（包括剧本对话、文学作品和在线讨论）进行比较，应用Welch t-test分析语义delta值的分布差异。

Result: 结果显示AI生成文本始终产生比人类文本更高的语义delta值，表明AI对话具有更刚性的主题结构，而人类对话则展现出更广泛和平衡的语义分布。该指标在统计上显著区分了人类和AI生成内容。

Conclusion: 提出的零样本指标为现有检测技术提供了计算成本低的补充信号，可集成到集成检测系统中。研究发现主题分布是当前LLM在模仿人类对话动态方面的一个可量化缺陷维度，有助于更广泛地理解LLM行为模仿能力。
...
TL;DR: 本文研究了机器翻译错误检测自动评估方法的可靠性问题，提出了MPP作为稳健的元评估策略


<details>
  <summary>Details</summary>
Motivation: 虽然机器翻译自动评估技术已从生成质量分数发展到精确定位翻译错误并分配错误类别和严重程度，但如何可靠地评估这些错误检测自动评估器的能力仍不明确，文献中缺乏成熟的评估技术

Method: 研究了不同实现方式的span级精确率、召回率和F分数，发现看似相似的方法会产生显著不同的排名结果；提出了"带部分重叠和部分信用的匹配"（MPP）结合微平均作为稳健的元评估策略，并公开了相关代码

Result: 研究表明某些广泛使用的技术不适合评估机器翻译错误检测；MPP方法被证明是有效的元评估策略，并用于评估当前最先进的机器翻译错误检测技术

Conclusion: 需要稳健的元评估方法来可靠地评估机器翻译错误检测自动评估器的能力，MPP方法为此提供了有效的解决方案，有助于推动该领域的发展
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索信息瓶颈框架在语言领域的应用，特别是验证自然语言系统是否在信息准确性和复杂性之间达到最优平衡。虽然该框架在视觉领域（如颜色和运动）已得到支持，但在语言刺激（如句子上下文中的词语）方面尚未探索。

Method: 方法是将翻译框架化为信息瓶颈优化问题，将源语句视为刺激，目标语句视为压缩的含义。通过分析英语、德语和塞尔维亚语对法语小说中空间介词的翻译，进行信息性估计：1）进行堆排序试点研究（N=35）获取介词对的相似性判断；2）训练低秩投影模型（D=5）预测这些判断（Spearman相关性：0.78）；3）比较实际翻译与反事实替代方案的信息瓶颈最优边界距离。

Result: 研究发现实际翻译的介词比反事实替代方案更接近信息瓶颈最优边界，这为人类翻译在空间领域表现出交际效率压力提供了初步证据。低秩投影模型能有效预测介词相似性判断（Spearman相关性0.78）。

Conclusion: 结论是翻译可以作为观察跨语言语义系统认知效率压力的窗口，人类翻译确实表现出交际效率优化的特征。这项工作为信息瓶颈框架在语言领域的应用提供了实证支持，并展示了翻译数据在认知语言学研究中的价值。
...
TL;DR: SAGE框架通过强化学习智能体筛选高质量训练数据，在减少97.1%数据使用和95.2%训练能耗的同时，为东南亚低资源语言翻译建立了新的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决全球数字鸿沟中的语言障碍问题，特别是在东南亚低资源地区。传统大语言模型部署面临高质量文化相关数据稀缺和训练能耗过高的双重挑战，需要在数字包容性和环境可持续性之间找到平衡。

Method: 提出可持续智能体引导专家调优(SAGE)框架：1) 使用基于Group Relative Policy Optimization优化的强化学习智能体，依据专家构建的社区对话集产生的语义奖励信号，自主筛选紧凑训练集；2) 采用Low-Rank Adaptation高效微调开源大语言模型。

Result: 在英语与七种东南亚低资源语言的翻译任务中，SAGE在BLEU-4和COMET-22指标上建立了新的SOTA性能，有效捕捉了当地语言细微差别。相比使用完整数据集的基线，减少了97.1%的数据使用和95.2%的训练能耗。

Conclusion: SAGE通过"正确数据而非大数据"的能源感知范式，为全球南方地区弥合数字鸿沟提供了可扩展且负责任的技术路径，在保证高性能的同时大幅降低环境足迹。
...
TL;DR: 该研究提出了一种混合主题建模框架，将LDA与sPLS-DA结合，用于分析叙事诗歌的主题结构和纵向动态，以《叶甫盖尼·奥涅金》意大利译本为案例验证方法有效性。


<details>
  <summary>Details</summary>
Motivation: 开发一个计算文学分析框架，能够在小型语料库中结合无监督和监督方法，分析复杂诗歌叙事的主题结构和动态变化，为传统文学解释提供计算支持。

Method: 采用LDA进行无监督主题建模，结合sPLS-DA进行监督分析；使用多种子共识协议解决小语料库不稳定性；将诗歌分割为35个文档，识别叙事枢纽来扩展词袋方法。

Result: 从诗歌中提取出5个稳定且可解释的主题；sPLS-DA增强了主题可解释性；叙事枢纽分析揭示了主题混合与诗歌情感和结构弧线的对应关系。

Conclusion: 该框架提供了一种计算形式的细读方法，证明轻量级概率模型能够生成复杂诗歌叙事的可复现主题地图，为比较研究中的高密度文学文本分析提供了透明的方法模板。
...
TL;DR: 论文研究了在协作积木搭建任务中，语言的字面解释与上下文推理的分离问题，通过BWIM基准测试发现LLMs在判断和行动上存在分离现象。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在协作任务中，模型如何将字面解释与上下文推理分离，特别是在面对模糊指令时，模型是进行上下文推理还是请求澄清。

Method: 方法基于现有的双说话者心理语言学范式，引入了Build What I Mean (BWIM)交互式基准测试，评估多个最先进的LLMs在模糊指令下的表现。

Result: 研究发现模型在判断和行动上存在分离：虽然模型在显式置信度评分中能检测到说话者的不可靠性，但未能利用这些信息来指导有效的澄清行为，而是表现出次优策略。

Conclusion: 结论表明当前LLMs在上下文意义构建方面存在局限性，特别是在将判断转化为有效行动方面，这为改进模型的协作沟通能力提供了方向。
...
TL;DR: 本文提出了一种基于多智能体框架的XAI叙事生成与优化方法，通过Narrator生成叙事，多个Critic Agents评估忠实性和连贯性，并通过迭代改进叙事质量。


<details>
  <summary>Details</summary>
Motivation: 现有XAI方法过于技术化和专家导向，缺乏可解释性和可访问性，需要开发更易理解的解释方法。LLM生成的XAI叙事虽然有望将事后解释转化为自然语言，但需要确保其忠实性和连贯性。

Method: 提出多智能体框架，包含Narrator（生成和修订叙事）和多个Critic Agents（评估忠实性和连贯性指标）。设计了五种智能体系统（Basic Design, Critic Design, Critic-Rule Design, Coherent Design, Coherent-Rule Design），并在5个LLM和5个表格数据集上进行系统评估。

Result: Basic Design、Critic Design和Critic-Rule Design在所有LLM上都能有效提高叙事忠实性。Claude-4.5-Sonnet在Basic Design上表现最佳，经过三轮迭代后不忠实叙事数量减少90%。基于多数投票的集成策略能持续提升四个LLM的性能（DeepSeek-V3.2-Exp除外）。

Conclusion: 智能体系统在生成忠实且连贯的XAI叙事方面具有巨大潜力，多智能体框架通过迭代反馈机制能显著改善解释质量。
...
TL;DR: LoASR-Bench是一个专门评估语音语言模型在低资源语言上自动语音识别性能的基准测试，包含25种语言、9个语系，涵盖拉丁和非拉丁文字，揭示了当前SpeechLMs在处理真实世界低资源语言时的局限性。
...
Method: 提出了LoASR-Bench基准测试，包含25种来自9个不同语系的低资源语言，涵盖拉丁和非拉丁文字，用于跨语言、跨文字的ASR性能评估。

Result: 实验结果表明，当前最新的语音语言模型在处理真实世界低资源语言时存在显著局限性。

Conclusion: LoASR-Bench填补了低资源语言ASR评估的空白，揭示了当前SpeechLMs的局限性，为未来改进提供了重要基准。
...
TL;DR: 该研究通过分析对话中的语言特征（信息价值、句法复杂性）和非语言特征（凝视行为变化）来预测听者在解释性互动中的理解状态。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何利用说话者和听者在对话中表现出的语言和非语言特征，来实时预测听者在解释性互动中的理解状态。这有助于改善人机交互和教学场景中的沟通效果。

Method: 研究方法包括：1）分析MUNDEX语料库中的面对面棋盘游戏解释对话；2）使用信息价值（通过惊奇度衡量）、说话者话语的句法复杂性、听者互动凝视行为变化三个语言线索；3）听者通过回顾性视频回忆方法自我标注理解状态；4）使用现成分类器和微调的德语BERT多模态分类器进行预测实验。

Result: 研究发现：1）单个语言线索与听者的理解水平存在相关性；2）预测四种理解状态（理解、部分理解、不理解、误解）是可行的；3）当三个语言线索与文本特征结合时，预测性能得到改善。

Conclusion: 结论表明，通过结合语言和非语言特征可以有效地预测听者在解释性对话中的理解状态，这为改进人机交互和教学沟通提供了理论基础和方法支持。
...
TL;DR: 在小规模模型和有限数据下，DPO相比SFT仅带来任务相关的微小增益，而全参数微调（FFT）始终优于LoRA，LoRA在此规模下并未减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 研究在小规模骨干模型和有限数据条件下，DPO相对于SFT的实际效果，以及不同参数化方法（FFT vs LoRA）的性能差异，填补现有研究在此规模下的空白。

Method: 使用GPT-2规模的解码器，系统比较SFT-only、DPO-only和分阶段SFT-to-DPO训练，同时对比全参数微调（FFT）和LoRA，评估任务包括释义检测和莎士比亚十四行诗续写。

Result: DPO相比强SFT仅带来任务相关的微小增益；当偏好构建与监督目标高度一致时，DPO无需预热即可达到竞争性SFT精度；FFT始终优于相同训练深度的LoRA；LoRA在当前硬件上并未减少训练时间。

Conclusion: 在小规模场景下，监督全参数适应仍是主要性能杠杆，偏好优化和低秩适应提供的边际收益有限。
...
TL;DR: 该研究评估了大型语言模型（如ChatGPT-5）在翻译生成语法术语到阿拉伯语时的表现，发现仅有25%的翻译准确，38.6%不准确，36.4%部分正确，表明LLMs在语法核心属性翻译方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验大型语言模型（LLMs）在语法模块翻译方面的能力，特别是评估ChatGPT将生成语法核心术语翻译成阿拉伯语的准确性，以了解AI在语言学专业翻译中的实际表现。

Method: 从生成语法相关文献中收集44个术语，先由人工翻译成阿拉伯语，再由ChatGPT-5翻译，采用分析和比较方法对两种翻译结果进行对比评估。

Result: ChatGPT-5的翻译结果中：25%准确，38.6%不准确，36.4%部分正确。研究揭示了LLMs在翻译涉及句法和语义挑战的语法术语时存在显著困难。

Conclusion: LLMs在语法核心属性翻译方面能力有限，需要AI专家与语言学家的紧密合作来改进模型机制，以实现更准确或至少适当的翻译效果。
...
TL;DR: 论文提出BOULDER基准测试，发现LLMs在对话环境中的推理表现显著低于孤立任务环境，强调需要在真实交互场景中评估LLM推理能力


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理评估主要基于孤立任务，与任务导向对话(TOD)中的实际使用场景存在差异，需要研究对话环境如何影响LLM推理性能

Method: 引入BOULDER动态基准测试，涵盖8个旅行相关任务，包含算术、空间和时间推理，每个问题提供孤立和对话两种变体，对8个LLM进行实验

Result: 实验显示孤立设置和对话设置之间存在显著且一致的性能差距，多轮对话特性是主要驱动因素，角色条件和工具使用需求也有额外影响

Conclusion: 需要在真实交互场景中评估LLM推理能力，对话环境对LLM推理性能有重要影响，基准测试性能不能准确反映TOD环境中的推理鲁棒性
...
TL;DR: 本文提出在HAL模型中加入可学习的温度缩放加性注意力机制，替代传统的平均池化，以提升句子级嵌入的质量，在IMDB情感分析任务上取得了显著性能提升。
...
Method: 1. 在HAL表示流程中集成可学习的温度缩放加性注意力机制；2. 对原始稀疏高维共现矩阵应用截断奇异值分解，将向量投影到稠密潜在空间；3. 在注意力层之前进行降维处理。

Result: 在IMDB情感分析数据集上，注意力池化方法达到82.38%的测试准确率，相比传统平均池化基线（75.64%）绝对提升6.74个百分点。注意力权重分析显示机制成功抑制停用词并选择性关注情感相关词。

Conclusion: 提出的注意力机制有效解决了HAL模型中平均池化的信息损失问题，不仅提升了分类性能，还增强了模型的可解释性，使模型能够更好地关注上下文中的重要词汇。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能力强大，但其输出的真实性无法保证，且存在过度自信倾向。现有不确定性量化方法大多依赖重复采样或辅助模型，计算成本高昂。

Method: 提出语义令牌聚类方法，利用LLM内部编码的语义信息，通过嵌入聚类和前缀匹配将令牌分组为语义一致的簇，基于对应语义簇的概率质量来量化不确定性。

Result: 实验结果表明，STC在性能上与最先进的基线方法相当，同时显著降低了计算开销，仅需单次生成且不依赖辅助模型。

Conclusion: STC为LLM不确定性量化提供了一种高效实用的解决方案，在保持性能的同时大幅减少了计算资源需求。
...
TL;DR: 研究评估了指令调优语言模型在用户压力与上下文证据忠实度之间的平衡，发现在固定证据设置下，更丰富的证据不能可靠防止用户对齐的立场反转，并识别了三种主要失败模式。


<details>
  <summary>Details</summary>
Motivation: 在争议性领域中，指令调优语言模型需要在用户对齐压力和上下文证据忠实度之间取得平衡。为了评估这种张力，研究者开发了一个基于美国国家气候评估的受控认知冲突框架。

Method: 使用基于美国国家气候评估的受控认知冲突框架，对19个参数规模从0.27B到32B的指令调优模型进行细粒度消融实验，分析证据构成和不确定性线索的影响。

Result: 在中性提示下，更丰富的证据通常能提高证据一致性准确性和序数评分性能。但在用户压力下，证据不能可靠防止用户对齐的立场反转。识别了三种主要失败模式：负面部分证据交互、非单调的鲁棒性缩放以及冲突下的分布集中度差异。

Conclusion: 在受控固定证据设置中，仅提供更丰富的上下文证据并不能保证抵御用户压力，需要明确的认知完整性训练来确保模型在压力下保持证据忠实度。
...
TL;DR: 链式思维忠实度评估存在分类器依赖性问题，不同分类器对相同数据得出显著不同的忠实度评分，导致模型排名变化，表明忠实度不是客观可测量的单一属性。
...
Method: 使用三种不同分类器（正则表达式检测器、两阶段正则+LLM管道、独立的Claude Sonnet 4评判器）分析12个开源模型的10,276条受影响的推理轨迹。通过统计检验（McNemar检验）和一致性度量（Cohen's kappa）系统比较分类器间的差异。

Result: 三种分类器对相同数据得出74.4%、82.6%和69.7%的总体忠实度，置信区间不重叠。模型间忠实度差异达2.6-30.6个百分点，所有差异均统计显著。分类器间一致性很低（kappa 0.06-0.42），且存在系统性不对称。分类器选择可逆转模型排名：Qwen3.5-27B从第1降至第7，OLMo-3.1-32B从第9升至第3。

Conclusion: 链式思维忠实度评估高度依赖分类器选择，不同分类器对相同行为产生分歧测量。已发表的忠实度数值在使用不同分类器的研究间无法有意义比较。未来评估应报告跨多种分类方法的敏感度范围，而非单一数值估计。
...
### [162] [When both Grounding and not Grounding are Bad -- A Partially Grounded Encoding of Planning into SAT (Extended Version)](https://arxiv.org/abs/2603.19429)
*João Filipe,Gregor Behnke*

Main category: cs.AI

TL;DR: 提出三种介于完全提升和完全接地之间的SAT编码方法，在保持动作提升的同时部分接地谓词，实现线性规模扩展而非二次扩展，在难以接地的领域优于现有最优长度规划方法。


<details>
  <summary>Details</summary>
Motivation: 传统规划问题使用提升的一阶表示，但大多数规划器会将其接地以简化推理，这可能导致规模指数级膨胀。最近的方法直接在提升层面操作以避免完全接地，但需要在完全提升和完全接地之间找到平衡点。

Method: 提出三种SAT编码方法：保持动作提升的同时部分接地谓词。与之前规模随规划长度呈二次方增长的SAT编码不同，新方法规模呈线性增长，特别适合处理较长的规划。

Result: 实验结果表明，最佳编码方法在难以接地的领域中，在长度最优规划方面优于现有最先进方法。

Conclusion: 通过保持动作提升并部分接地谓词的SAT编码方法，在完全提升和完全接地规划之间找到了有效的平衡点，实现了线性规模扩展，在长规划问题上表现优异。
...
TL;DR: 该论文提出了一种训练大语言模型生成形式化反例的方法，填补了当前AI数学推理中只关注证明构造而忽视反例发现的空白。


<details>
  <summary>Details</summary>
Motivation: 数学推理需要两种互补技能：为真命题构造严格证明和为假命题发现反例。当前AI数学推理研究几乎完全专注于证明构造，忽视了同样重要的反例发现任务。

Method: 1. 将反例生成形式化为需要LLMs提出候选反例并生成可在Lean 4定理证明器中自动验证的形式化证明；2. 引入符号变异策略，通过系统提取定理并丢弃选定假设来合成多样化的训练数据；3. 结合策划的数据集，采用多奖励专家迭代框架来增强LLMs在反例生成和定理证明方面的训练效果和效率。

Result: 在三个新收集的基准测试上的实验验证了该方法的优势，表明变异策略和训练框架带来了显著的性能提升。

Conclusion: 该方法成功填补了AI数学推理中反例生成的空白，通过形式化反例生成、符号变异策略和多奖励专家迭代框架，显著提升了LLMs在反例发现和定理证明方面的能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估主要关注特定推理或规划问题在受控环境中进行。虽然近期研究探索了旅行规划作为整合语言推理任务到现实场景的媒介，但推理任务不仅限于语言推理，需要包含多个认知领域的测试平台来全面评估LLMs。

Method: 引入ItinBench基准测试，在旅行行程规划中整合空间推理任务（路线优化），同时保留传统语言推理任务。评估了多种LLMs包括Llama 3.1 8B、Mistral Large、Gemini 1.5 Pro和GPT系列。

Result: 研究发现LLMs在同时处理多个认知维度时难以保持高且一致的性能。通过整合不同人类认知领域的任务，ItinBench为构建更全面的推理测试平台提供了新见解。

Conclusion: ItinBench通过整合空间推理和语言推理任务，提供了更全面的LLMs评估框架，能更好地反映现实世界挑战，为构建更全面的推理测试平台提供了新方向。
...
TL;DR: PA2D-MORL方法通过帕累托上升方向分解多目标强化学习问题，构建高效的多目标策略集近似方案，在机器人控制任务中优于现有方法。
...
Method: 提出PA2D-MORL方法：1)利用帕累托上升方向选择标量化权重并计算多目标策略梯度；2)在进化框架下选择性优化多个策略从不同方向逼近帕累托前沿；3)应用帕累托自适应微调方法增强帕累托前沿近似的密度和分布。

Result: 在各种多目标机器人控制任务上的实验表明，该方法在结果质量和稳定性方面明显优于当前最先进的算法。

Conclusion: PA2D-MORL方法通过帕累托上升方向分解和策略改进方案，能够有效逼近帕累托策略集，在多目标强化学习中表现出优越性能。
...
TL;DR: PowerLens利用大语言模型的常识推理能力，通过多智能体架构实现个性化移动电源管理，无需显式配置即可根据用户活动生成上下文感知的电源策略。
...
Method: 采用多智能体架构，从UI语义识别用户上下文，生成跨18个设备参数的完整电源策略。使用PDL约束框架验证每个执行动作，通过基于置信度的蒸馏两层记忆系统从隐式用户覆盖中学习个性化偏好。

Result: 在rooted Android设备上的实验显示，PowerLens达到81.7%的动作准确率和38.8%的节能效果（相比原生Android），优于基于规则和LLM的基线方法，用户满意度高，偏好收敛快（3-5天），系统本身仅消耗0.5%的日电池容量。

Conclusion: PowerLens展示了LLM在移动电源管理中的潜力，通过语义理解和个性化学习实现了安全、高效的零样本上下文感知策略生成，为下一代智能电源管理系统提供了新方向。
...
Method: HyEvo采用异构原子合成方法，将概率性LLM节点用于语义推理，确定性代码节点用于基于规则的执行。通过LLM驱动的多岛进化策略和"反思-生成"机制，利用执行反馈迭代优化工作流拓扑和节点逻辑。

Result: 在多种推理和编码基准测试中，HyEvo始终优于现有方法，与最先进的开源基线相比，推理成本降低高达19倍，执行延迟降低高达16倍。

Conclusion: HyEvo通过异构原子合成和进化优化策略，有效解决了现有自动化工作流生成方法的局限性，在保持高性能的同时显著降低了计算成本和延迟。
...
TL;DR: 本文提出了一种结合实时子目标分解规划和里程碑奖励强化学习的智能体框架，显著提升了LLM智能体在长序列任务中的表现，在WebArena-Lite基准上取得了突破性成果。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体在长序列任务中存在两个主要问题：在线执行时难以跟踪新信息并保持清晰的自适应路径；强化学习微调时稀疏延迟的奖励信号使得智能体难以识别导致成功的动作，无法在扩展任务中保持连贯推理。

Method: 提出两个核心贡献：1）利用专有模型进行子目标分解的在线规划智能体框架；2）MiRA（里程碑强化学习增强智能体）训练框架，使用密集的基于里程碑的奖励信号。

Result: 实时规划机制使Gemini等专有模型的成功率在WebArena-Lite基准上绝对提升约10%；将MiRA应用于开源的Gemma3-12B模型，其成功率从6.4%提升至43.0%，超越了GPT-4-Turbo（17.6%）、GPT-4o（13.9%）以及之前的开源模型最佳WebRL（38.4%）。

Conclusion: 结合显式推理时规划和基于里程碑的奖励能显著提升智能体的长序列任务能力，为更鲁棒和通用的自主系统铺平了道路。
...
TL;DR: 论文提出了一种神经符号证明生成框架，结合LLM和交互式定理证明工具，通过最佳优先树搜索自动化系统级验证项目的证明搜索，在seL4基准测试中证明了77.6%的定理。


<details>
  <summary>Details</summary>
Motivation: 形式化验证通过交互式定理证明对关键系统进行正确性验证，但大型证明脚本的构建仍然高度手动且限制了可扩展性。大型语言模型在数学推理方面的进步使其在软件验证中的集成前景广阔。

Method: 提出了神经符号证明生成框架，在证明状态上进行最佳优先树搜索，反复查询LLM获取候选证明步骤。神经方面：使用证明状态-步骤对数据集微调LLM；符号方面：整合ITP工具修复被拒绝的步骤、过滤和排序证明状态、在搜索停滞时自动处理子目标。

Result: 在seL4基准测试中，系统证明了高达77.6%的定理，显著超越了之前的LLM方法和独立的Sledgehammer，解决了更多多步证明。在其他基准测试中也展示了强大的泛化能力。
...
TL;DR: 论文提出"具身科学"新范式，将科学发现重构为智能体推理与物理执行紧密耦合的闭环过程，超越当前孤立的任务特定预测方法。


<details>
  <summary>Details</summary>
Motivation: 当前人工智能在预测科学属性方面表现出色，但科学发现本质上是物理的、长期的过程，受实验周期支配。现有计算方法大多与这一现实脱节，将发现视为孤立的、任务特定的预测，而非与物理世界的持续互动。
...
Conclusion: 具身科学范式为生命科学和化学领域的自主发现系统提供了路线图，通过将智能体推理与物理执行紧密结合，实现真正的科学发现闭环。
...
<details>
  <summary>Details</summary>
Motivation: 自动形式化虽然能将自然语言数学转化为机器可检查的语句，但语义一致性并不保证证明器有效性。即使语义一致的形 式化方案在证明搜索成本和成功率上也可能存在显著差异，因此需要寻找既能保持语义一致性又能提高证明效率的形式化方案。

Method: 提出FormalEvolve框架，将其视为预算约束下的测试时搜索问题。通过LLM驱动的变异和交叉生成多样化候选方案，结合有界补丁修复；同时使用符号抽象语法树(AST)重写操作注入结构多样性。采用编译门控机制筛选有效候选。
...
Conclusion: FormalEvolve通过神经符号进化方法有效解决了自动形式化中语义一致性与证明效率的平衡问题，在有限预算下显著提高了形式化方案的质量和下游证明成功率。
...
TL;DR: 论文分析Transformer在AI规划任务中的泛化能力，提出C*-RASP框架来研究序列长度和词汇量同时增长时的长度泛化保证，识别了Transformer可验证长规划的可学习领域类别。


<details>
  <summary>Details</summary>
Motivation: Transformer在AI规划任务中表现出不一致的成功，且对其泛化能力的理论理解有限。本文旨在填补这一空白，分析解码器模型验证给定规划是否解决规划实例的能力。

Method: 引入C*-RASP扩展C-RASP框架，用于在序列长度和词汇量同时增长时建立Transformer的长度泛化保证。分析解码器模型验证规划的能力，识别影响长度泛化解可学习性的结构特性。

Result: 识别出Transformer可证明学习验证长规划的大类经典规划领域，以及显著影响长度泛化解可学习性的结构特性。实验验证了理论结果。

Conclusion: 本文为理解Transformer在AI规划任务中的泛化能力提供了重要理论框架，识别了可学习验证的规划领域类别，为Transformer在规划任务中的应用提供了理论基础。
...
TL;DR: HeRL框架利用后见经验引导强化学习，通过将失败轨迹和未满足的评分标准作为指导，帮助LLMs探索超出当前策略分布的理想响应，从而提高推理能力。


<details>
  <summary>Details</summary>
Motivation: 基于评分标准的强化学习虽然能提升大语言模型的推理能力，但探索效率低下，局限于当前策略分布。需要一种方法能够引导模型向最大化奖励的理想分布方向探索。

Method: 提出HeRL框架：1) 将失败轨迹及其未满足的评分标准作为后见经验，为策略提供上下文指导；2) 引入额外奖励激励在指导下有更大改进潜力的响应；3) 避免从零开始的重复试错，实现从高质量样本中有效学习。

Result: 在多个基准测试上的广泛实验表明，HeRL相比基线方法取得了更优的性能提升，并且能够在测试时通过经验引导的自我改进获得进一步收益。

Conclusion: HeRL通过后见经验引导强化学习，实现了更有效的探索和更准确的梯度估计，显著提升了大语言模型的推理能力，同时支持测试时的自我改进。
...
Method: DIAL-KG是一个由元知识库协调的闭环框架，包含三阶段循环：1) 双轨提取：默认三元组生成确保知识完整性，对复杂知识切换到事件提取；2) 治理裁决：确保提取事实的保真度和时效性，防止幻觉和知识陈旧；3) 模式演化：从验证的知识中归纳新模式以指导后续构建周期，并将当前轮次的知识增量应用到现有知识图谱。

Result: 广泛的实验表明，该框架在构建图的质量和归纳模式方面都达到了最先进的性能。

Conclusion: DIAL-KG解决了传统静态知识图谱构建方法的局限性，通过增量构建和动态模式演化，为动态数据环境中的知识图谱构建提供了更灵活、高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型心智理论推理方法存在局限性：直接提示法或潜在状态模型将信念视为静态独立，导致随时间推移产生不一致的心理模型，在动态情境下推理能力较弱。特别是在灾难响应、急诊医学和人机协同等高风险环境中，需要准确推断人们如何根据隐含、演变的信念来寻求信息和采取行动。

Method: 提出结构化认知轨迹模型，将心理状态表示为动态信念图，联合推断潜在信念、学习其时变依赖关系，并将信念演化与信息寻求和决策联系起来。具体包括：(1) 从文本化概率陈述到一致概率图模型更新的新颖投影；(2) 基于能量的因子图表示信念相互依赖关系；(3) 捕捉信念积累和延迟决策的ELBO目标函数。

Result: 在多个真实世界灾难疏散数据集上，该模型显著改善了行动预测能力，并恢复了与人类推理一致的可解释信念轨迹，为在高不确定性环境中增强大语言模型的心智理论能力提供了原则性模块。

Conclusion: 该结构化认知轨迹模型为大语言模型在高不确定性环境中的心智理论推理提供了系统化方法，通过动态信念图表示实现了更一致、更强大的推理能力，在灾难响应等高风险场景中具有重要应用价值。
