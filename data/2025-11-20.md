...
TL;DR: 提出一个解决开放集识别中过自信问题的框架，通过参数扰动估计不确定性和两阶段未知类检测来增强已知与未知类的区分能力
...
Method: 框架包含两个组件：基于扰动的uncertainty估计模块（通过可控参数扰动生成多样化预测并量化预测不确定性）和具有不同学习分类器的未知检测模块（作为两阶段过程，利用估计的不确定性改进已知与未知类的区分）

Result: 在三个公共数据集上的实验结果表明，所提框架相比现有OSR方法取得了更优的性能
...
TL;DR: 本文提出了一种新颖的目标感知时序建模方法，用于解决少样本视频目标检测问题，通过选择性传播高置信度目标特征来保持时序一致性并提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测方法需要大量标注数据，而少样本视频目标检测面临时序一致性（遮挡和外观变化）和新目标泛化的挑战，现有方法依赖复杂区域提议导致计算开销大。

Method: 采用过滤机制选择性传播高置信度目标特征，结合少样本训练检测和分类头，实现高效特征传播，无需显式目标管提议。

Result: 在5-shot设置下，AP提升分别为：FSVOD-500 3.7%、FSYTV-40 5.3%、VidOR 4.3%、VidVRD 4.5%，在1-shot、3-shot和10-shot配置下也均有改进。

Conclusion: 该方法通过目标感知时序建模有效解决了少样本视频目标检测的挑战，实现了显著的性能提升，且代码已开源。
...
TL;DR: 本文提出了一种基于流匹配范式的多模态图像融合方法，通过直接从源模态分布到融合图像分布的概率传输，提高采样效率和结构一致性。使用多模型融合结果作为伪标签，并引入融合精炼模块和持续学习机制。


<details>
  <summary>Details</summary>
Motivation: 当前多模态图像融合方法依赖任务特定模型，训练成本高且可扩展性有限。生成方法虽然提供统一建模视角，但采样效率低。需要一种高效且统一的多模态图像融合方法。

Method: 1) 将图像融合建模为从源模态到融合图像的直接概率传输；2) 收集多SOTA模型融合结果作为伪标签，使用任务感知选择函数；3) 引入融合精炼模块分解和增强退化组件；4) 集成弹性权重巩固和经验回放机制用于多任务场景。

Result: 该方法在多种融合任务中取得竞争性性能，同时显著提高采样效率并保持轻量级模型设计。

Conclusion: 提出的基于流匹配的融合方法实现了高效、统一的多模态图像融合，在采样效率和模型轻量化方面具有优势，为多任务图像融合提供了新思路。
...
TL;DR: 提出了一种两阶段无轨迹碰撞检测框架，通过扩散模型生成合理的未来路段地图并与监控地图比较来检测碰撞，无需车辆轨迹数据。


<details>
  <summary>Details</summary>
Motivation: 解决传统碰撞检测方法对轨迹获取和车辆跟踪的依赖问题，利用路段地图直接进行碰撞检测，提高实时性和实用性。

Method: 使用两阶段框架：第一阶段Mapfusion扩散模型通过加噪-去噪过程生成未来路段地图，结合时序嵌入和ControlNet背景控制；第二阶段通过比较监控地图与生成地图来检测碰撞。

Result: Mapfusion能够基于学习的运动模式生成真实的路段演化地图，在不同采样间隔下保持鲁棒性，在真实世界碰撞实验中有效检测碰撞。

Conclusion: 该方法成功实现了无轨迹的实时碰撞检测，为主动安全管理策略提供了有效工具，在交通效率提升方面具有应用价值。
...
TL;DR: 本文提出了一种针对地震数据的自适应双网格基础模型训练策略（ADATG），通过希尔伯特编码和频谱分解来有效处理地震数据中的高频和低频特征，解决了传统视觉变换器在地震应用中效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 由于地震数据具有独特的频率特征，传统视觉变换器在处理地震数据时无法有效捕捉高频和低频信息，需要专门针对地震数据特性设计的基础模型训练方法。

Method: 采用自适应双网格训练策略，结合频谱分解分离高频和低频分量，使用分层希尔伯特编码表示数据，并基于频率原理从粗粒度到细粒度逐步训练模型。

Result: 广泛的实验证明了该训练方法的有效性和效率，能够显著提升视觉地震基础模型的预训练效果。
...
TL;DR: 该研究开发了一种基于面部微动态分析的被动痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变化和头部微调等面部运动特征，无需语音或文本即可进行早期神经认知变化的语言无关检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本化访谈，限制了在临床环境外的应用，且预测结果与语言和转录紧密耦合。本研究旨在开发一种无需主动干预、可跨设备、话题和文化使用的被动筛查方法。

Method: 通过稳定面部信号，将面部微运动转换为可解释的面部微动态时间序列，平滑处理后汇总为紧凑的片段级统计特征。分析运动在多个通道间的分布而非幅度，使每个通道的效应透明化。

Result: 在YT DemTalk数据集上，消融实验确定注视不稳定性和嘴部/下颌动态是最具信息量的线索，轻量级分类器实现了AUROC 0.953、AP 0.961、F1分数0.851和准确率0.857的痴呆预测性能。

Conclusion: 面部微动态分析为痴呆筛查提供了一种有效的语言无关方法，能够在非临床环境中进行大规模、无脚本的视频分析，具有很好的跨设备和文化转移性。
...
TL;DR: Gen-ViRe是一个新的视觉推理基准测试框架，用于评估视频生成模型在Chain-of-Frames推理中的认知能力，填补了现有基准在评估多步规划、算法逻辑和抽象模式外推方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准主要关注保真度或对齐度，无法评估模型的核心认知能力，如多步规划、算法逻辑和抽象模式外推，这阻碍了对模型能力的系统理解和改进指导。

Method: 基于认知科学和真实AI应用，将Chain-of-Frames推理分解为六个认知维度和24个子任务，通过多源数据整理、最小提示协议和混合VLM辅助评估来量化评估视频模型的推理能力。

Result: 实验显示最先进系统在视觉质量和实际推理深度之间存在显著差距，为真正的世界模拟器建立了基准和诊断工具。

Conclusion: Gen-ViRe提供了首个对视频模型作为推理器的定量评估，揭示了视觉质量与推理能力之间的不匹配，为推进真正世界模拟器的发展提供了基础。
...
TL;DR: 本文提出基于排序的损失函数来解决热图人体姿态估计中的三个主要问题：MSE损失不能有效提升关节定位精度、热图空间和类别不平衡、损失函数与评估指标不一致。


<details>
  <summary>Details</summary>
Motivation: 解决热图人体姿态估计中的三个关键问题：(1) MSE损失对像素偏差的均等惩罚无法有效提升关节定位精度；(2) 热图的空间和类别不平衡；(3) 损失函数与评估指标(mAP)之间的不一致性。

Method: 提出基于排序的损失函数，通过理论分析和实验验证其优于常用的热图损失(MSE、KL散度)。该方法提高了置信度分数与定位质量之间的相关性，从而在NMS过程中实现更准确的实例选择。

Result: RSPose在COCO-val数据集上达到79.9 mAP，优于之前的最先进方法。同时将SimCC Resnet-50在COCO-val上的性能提升了1.5 AP，达到73.6 AP。
...
TL;DR: 本文提出了碰撞声源分割(CS3)新任务，旨在根据音频条件在视觉输入中分割产生碰撞声音的物体。针对自我中心视频的挑战，作者开发了弱监督方法，利用基础模型和手部物体线索，在两个新基准上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，特别是通过声音识别物体交互属性的能力，作者希望解决从碰撞声音中识别和分割相关物体的任务，这在自我中心视频中尤其具有挑战性。

Method: 提出弱监督的音频条件分割方法，利用CLIP和SAM2基础模型，并整合自我中心线索（如手中的物体）来识别可能的碰撞声源物体。

Result: 在两个新提出的CS3基准（EPIC-CS3和Ego4D-CS3）上，方法在mIoU指标上分别比竞争基线高出3倍和4.7倍。

Conclusion: 该方法成功解决了碰撞声源分割任务，证明了利用基础模型和自我中心线索在复杂视觉场景中识别声音来源的有效性。
...
TL;DR: 本文提出了一种几何基础的绝对姿态回归方法，将姿态估计重新表述为从图像直接回归底层3D几何表示，而不是直接回归6自由度姿态。该方法通过预测解耦的几何组件（光线束方向和点图）来分别估计相机旋转和平移。


<details>
  <summary>Details</summary>
Motivation: 传统的绝对姿态回归模型作为黑盒直接回归6自由度姿态，容易记忆训练视图而非理解3D场景几何。本文旨在通过引入几何先验来改进这一范式。

Method: 提出几何表示回归（GRR）范式，将姿态估计建模为逆渲染过程。模型显式预测两个解耦的几何表示：光线束方向（估计旋转）和对应点图（估计平移），然后通过可微分确定性求解器恢复最终姿态。

Result: 在7-Scenes和Cambridge Landmarks数据集上实现了最先进的性能，验证了建模逆渲染过程是实现可泛化绝对姿态估计的更稳健路径。

Conclusion: 通过将学习到的视觉到几何映射与最终姿态计算分离，引入强几何先验，显式解耦旋转和平移预测能显著提升性能。
...
TL;DR: 本文提出了H-CNN-ViT模型用于膀胱癌复发预测，通过分层门控注意力多分支架构融合CNN和ViT特征，在自建的多序列MRI数据集上达到78.6%的AUC。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，术后监测至关重要。多序列增强MRI是常用检测方法，但术后组织变化使判读困难。现有AI工具缺乏专门的多序列MRI数据集支持。

Method: 提出H-CNN-ViT分层门控注意力多分支模型，通过门控机制选择性加权全局(ViT)和局部(CNN)路径特征，独立处理各模态并优化特征融合。

Result: 在自建数据集上评估，H-CNN-ViT达到78.6%的AUC，超越现有最优模型。

Conclusion: 构建了专门的多序列MRI数据集，提出的H-CNN-ViT模型在膀胱癌复发预测中表现出色，为未来研究提供了基准。
...
TL;DR: QwenCLIP是一个视觉语言框架，用基于大语言模型的嵌入模块替换CLIP的文本编码器，通过可学习提示增强跨模态对齐，解决了CLIP在长放射学报告表示上的限制。


<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器仅接受77个token，限制了其在信息丰富的长放射学报告中的表示能力。现有的领域特定编码器如PubMedBERT或ClinicalBERT虽然缓解了这一问题，但仍受限于512个token的输入长度和相对浅层的语义理解。

Method: 提出QwenCLIP框架，用基于大语言模型的嵌入模块（如Qwen3-Embedding）替换CLIP的文本编码器，并引入可学习提示来增强跨模态对齐。利用LLM的扩展上下文窗口和更丰富的表示来捕捉长形式临床文本的全面医学语义。

Result: QwenCLIP显著改善了医学图像-文本对齐，并在放射学基准测试中提升了下游性能。

Conclusion: 通过利用大语言模型的扩展上下文能力和丰富表示，QwenCLIP能够有效捕捉长形式临床文本的全面医学语义，在医学视觉语言任务中表现出优越性能。
...
TL;DR: 提出了一种结合EfficientNet和VGG19的混合模型架构，用于腰椎退变分类，通过Pseudo-Newton Boosting层和Sparsity-Induced Feature Reduction层提升特征选择和表示能力。


<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在医学图像高维背景下存在局限，无法充分利用详细的解剖学特征，且特征冗余问题影响模型性能。

Method: 采用EfficientNet和VGG19混合架构，集成Pseudo-Newton Boosting层进行智能特征权重调整，以及Sparsity-Induced Feature Reduction层去除特征冗余，形成多层框架。

Result: 模型性能显著提升，达到精度0.9、召回率0.861、F1分数0.88、损失0.18、准确率88.1%，优于基线模型EfficientNet。

Conclusion: 该架构克服了传统迁移学习方法的限制，为医学图像自动诊断工具的发展提供了有效解决方案。
...
TL;DR: 本文提出了一种新方法，将视觉语言模型从直接决策生成器转变为语义增强器，通过融合视觉和语言特征实现更准确的自动驾驶决策和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的自动驾驶方法依赖手工提示且性能不稳定，限制了在真实场景中的鲁棒性和泛化能力。作者发现现有开源VLMs在高层次决策任务中存在关键局限性。

Method: 利用VLMs的强大场景理解能力为现有视觉基准添加结构化、语言丰富的场景描述，提出多模态交互架构融合视觉和语言特征，并设计后处理精炼模块提升预测可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有数字血液病理学方法（包括单任务、视觉语言、WSI优化或单细胞血液学模型）无法提供统一的多任务、多模态推理，无法应对数字血液病理学的复杂性。

Method: 基于Hema-Former多模态模块构建Uni-Hema模型，在46个公开数据集（包含70万+图像和2.1万+问答对）上训练，在层次级别桥接视觉和文本表示，支持不同粒度的多种任务。

Result: 广泛实验表明Uni-Hema在多种血液学任务上达到或优于单任务单数据集模型的性能，同时在单细胞级别提供可解释的形态学相关洞察。
...
TL;DR: 提出了首个弱监督管道用于检测短暂沟壑，利用视觉语言模型减少手动标注工作量，并发布了首个半监督检测数据集。
...
Method: 使用视觉语言模型嵌入知识，采用师生模型：教师从VLM的噪声标签中学习，学生通过弱监督使用教师生成的标签和噪声感知损失函数学习。

Result: 实验结果表明该方法优于VLM和标签模型本身，在弱监督训练学生模型时表现出优越性能。

Conclusion: 提出的弱监督方法有效解决了短暂沟壑检测问题，显著减少了人工标注需求，为相关研究提供了首个公开数据集和代码。
...
<details>
  <summary>Details</summary>
Motivation: 现有MCVT研究过于注重准确性而忽略实时性和可扩展性，这在城市规模应用中随着摄像头数量增加变得更具挑战性，需要适合实际部署的解决方案。

Method: 系统包含边缘设备和中央工作站。边缘端处理RTSP视频流，包括目标检测、跟踪、地理映射和特征提取，仅传输车辆位置和深度外观特征等轻量元数据。中央端基于自监督相机链接模型学习相邻摄像头间的时空关系进行跨摄像头关联。

Result: 在RoundaboutHD数据集上的实验表明，SAE-MCVT在2K 15FPS视频流上保持实时运行，IDF1得分为61.2。
...
TL;DR: 评估多模态大语言模型对道路安全概念的理解能力，通过示意图和说明性表示来测试模型在零样本设置下的表现。


<details>
  <summary>Details</summary>
Motivation: 道路安全规范对自动驾驶车辆至关重要，需要评估AI系统对道路安全概念的理解程度，以揭示模型推理能力的局限性。

Method: 使用从学校教科书中收集的交通标志和道路安全规范图像构建数据集，在零样本设置下评估多模态大语言模型的表现。

Result: 初步结果显示这些模型在安全推理方面存在困难，揭示了人类学习与模型解释之间的差距。
...
TL;DR: 本文提出了CuRPO方法，通过基于课程学习的相对策略优化来解决CoT推理在视觉定位任务中的性能退化问题，利用CoT长度和gIoU奖励作为复杂度指标来结构化训练数据。


<details>
  <summary>Details</summary>
Motivation: 研究发现基于强化学习的CoT推理在视觉定位任务中会随着CoT输出变长或复杂而性能下降，且数据集规模增加并不总能提升性能，因此需要更有效的训练策略。

Method: 提出Curriculum-based Relative Policy Optimization (CuRPO)，使用CoT长度和广义交并比(gIoU)奖励作为复杂度指标，从简单到复杂逐步结构化训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上的实验显示，CuRPO显著优于现有方法，在RefCOCO上提升高达+12.52 mAP，在少样本学习场景下也表现出色。

Conclusion: CuRPO是一种高效且鲁棒的方法，特别适用于具有模糊和复杂文本描述的视觉定位任务，能够有效提升CoT推理的性能。
...
TL;DR: 提出基于聚类的帧选择策略来减轻视频衍生帧数据集中的信息泄露问题，通过在数据集划分前对视觉相似帧进行分组，生成更具代表性、平衡性和可靠性的数据集分区。
...
Result: 该方法能够生成更具代表性、平衡性和可靠性的数据集分区，有效减轻信息泄露。
...
TL;DR: 通过使用无视觉或语义内容的程序生成数据预训练视觉Transformer，可以显著提高数据效率、收敛速度和下游性能，1%的程序数据相当于28%的ImageNet-1k数据的效果。


<details>
  <summary>Details</summary>
Motivation: 探索在视觉Transformer中注入通用归纳偏置的新方法，通过使用不含视觉或语义内容的程序生成数据进行预训练，以增强模型的抽象计算先验。

Method: 使用形式语法等简单算法生成无视觉语义内容的程序数据，在视觉Transformer的预热阶段绕过视觉补丁嵌入机制进行预训练，然后进行标准的基于图像的训练。

Result: 在ImageNet-1k上，仅分配1%的训练预算给程序数据就能提高最终准确率超过1.7%，1%的程序生成数据相当于28%的ImageNet-1k数据的效果。

Conclusion: 这种方法为数据高效和领域无关的预训练策略提供了一条有前景的路径。
...
TL;DR: CrossTrainer方法通过发现跨运动技能属性（如平衡、控制、手部位置）来构建可迁移的视频表示，训练多模态语言模型为运动视频生成可操作的反馈和熟练度评级，在跨运动迁移和领域内设置中显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 解决当前技能评估模型局限于单一运动、依赖昂贵专家监督的问题，探索可迁移的视频表示方法以覆盖长尾运动场景。

Method: 发现跨运动边界的技能属性，训练多模态语言模型生成可操作反馈和熟练度评级。

Result: 在跨运动和领域内设置中，相对现有技术提升高达60%，通过抽象共享的人类技能行为实现更好的泛化能力。

Conclusion: 通过抽象共享技能行为，所提出的视频表示方法显著优于现有技术，丰富了多模态大语言模型的能力。
...
TL;DR: RISE是首个用于单静态雷达室内场景理解的基准和系统，通过利用多径反射的几何线索，实现了布局重建和物体检测，在保护隐私的同时显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决室内场景理解中光学传感器存在的遮挡和隐私问题，同时克服毫米波雷达低空间分辨率的限制，实现鲁棒且保护隐私的室内场景理解。

Method: 提出双角度多径增强方法，显式建模到达角和离开角来恢复二次反射；采用模拟到现实的分层扩散框架，将碎片化的雷达响应转换为完整的布局重建和物体检测。

Result: 在布局重建方面，将Chamfer距离降低了60%（降至16厘米）；在物体检测方面，实现了58%的IoU，是首个基于毫米波雷达的物体检测系统。

Conclusion: RISE为使用单个静态雷达进行几何感知和隐私保护的室内场景理解建立了新的基础，证明了多径反射在场景理解中的价值。
...
TL;DR: 开发了一个2.5D上下文感知模型，用于自动识别MRI切片的解剖平面方向（轴位、冠状位、矢状位），准确率达99.49%，比2D模型减少60%错误。生成的元数据在脑肿瘤检测任务中可将准确率从97.0%提升至98.0%。


<details>
  <summary>Details</summary>
Motivation: 自动化系统难以识别MRI切片的解剖平面方向，缺失的平面方向元数据会复杂化分析、增加领域偏移并降低诊断分类器的准确性。

Method: 采用2.5D上下文感知模型，利用多切片信息避免孤立切片的模糊性，并在3D切片序列和静态2D图像上进行训练。

Result: 2.5D模型准确率达99.49%，比2D参考模型（98.74%）减少60%错误。在脑肿瘤检测任务中，使用元数据增强预测可将准确率从97.0%提升至98.0%，减少33.3%的误诊。
...
TL;DR: LINGUAL框架通过自然语言指令和上下文学习实现医学图像分割，在主动领域适应中达到与主动学习基线相当或更好的性能，同时减少约80%的标注时间。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中传统主动学习面临边界模糊、标注劳动密集和认知负担重的问题，语言指导提供了一种更高效的替代方案。

Method: 提出LINGUAL框架，接收专家的自然语言指令，通过上下文学习将其转换为可执行程序，自动执行相应的子任务序列而无需人工干预。

Result: 在主动领域适应任务中，LINGUAL实现了与主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: 语言引导的框架能够显著降低医学图像分割的标注负担，为医学图像分析提供了一种更高效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉生成模型的快速发展，AI生成图像检测变得至关重要。研究无需训练、不依赖特定数据集知识的检测方法，利用自监督模型对图像裁剪增强的鲁棒性特性。

Method: 首先定义基础评分函数，量化图像嵌入对通过Haar小波分解提取的高频方向扰动的敏感性。为模拟裁剪增强的鲁棒性，将图像缩放到模型输入尺寸的倍数，分割成小块，计算每个块的基础评分，最后对所有块的评分取平均得到最终检测分数。

Result: 在真实数据集和23种不同生成模型生成的图像上进行验证，WaRPAD始终达到竞争性性能，并对测试时损坏表现出强鲁棒性。
...
TL;DR: FashionMAC是一个基于扩散模型的无需变形的服装中心时尚图像生成框架，通过直接外推分割出的服装来保持细节，并引入区域自适应解耦注意力机制实现细粒度外观控制。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在服装变形过程中导致的纹理失真问题，以及缺乏细粒度外观控制机制的问题，旨在实现高质量且可控的时尚展示图像生成。

Method: 提出无需服装变形的框架，直接外推分割服装；引入区域自适应解耦注意力机制和链式掩码注入策略，实现细粒度文本属性控制。

Result: 实验验证了该框架相比现有最先进方法的优越性能，能够高质量地保持服装细节并实现精确的外观控制。

Conclusion: FashionMAC通过消除服装变形需求和引入细粒度控制机制，成功实现了高质量、可控的时尚图像生成，在服装细节保持和外观控制方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 异常物体检测和分类是计算机视觉和模式识别中的主要挑战性任务，需要开发自动化的桥梁缺陷检测系统以提高检测效率和准确性。

Method: 采用两阶段框架：第一阶段使用显著性分析进行缺陷区域提议，第二阶段在显著性增强图像上应用基于YOLOX的深度学习检测器，并使用边界框级亮度增强技术。

Result: 在标准数据集上的实验结果表明，该框架在准确性和计算效率方面表现良好，具有在自供电检测系统中实施的巨大潜力。

Conclusion: 提出的结合显著性分析和深度学习的框架为混凝土桥梁结构的自动化缺陷检测提供了一种有效且高效的解决方案。
...
TL;DR: SCAR是一种基于自回归模型的语义上下文驱动图像编辑方法，通过压缩语义预填充和语义对齐指导来解决现有AR模型在图像编辑中的条件限制问题。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成方面显示出强大潜力，但在扩展到通用图像编辑时面临条件限制弱和效率低的问题，导致指令遵循差和视觉伪影。
...
Conclusion: SCAR在向量量化预填充的灵活性和通用性基础上构建，克服了其语义限制和高成本问题，可泛化到next-token和next-set AR范式，代码将发布。
...
TL;DR: CORE提出了一种基于对象中心表示的视觉标记压缩方法，通过高效分割解码器生成对象掩码作为语义先验，指导视觉标记合并为紧凑的对象中心表示，并采用质心引导排序机制恢复空间顺序。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在处理高分辨率图像时面临计算和内存成本过高的问题，现有标记压缩方法缺乏高层语义理解，导致次优合并、信息冗余或上下文丢失。

Method: 使用高效分割解码器生成对象掩码作为语义先验，将视觉标记合并为紧凑的对象中心表示，并通过质心引导排序机制恢复空间顺序。

Result: 在六个权威基准测试中达到固定率压缩的最新水平，在自适应率设置下实现显著效率提升，即使仅保留2.2%的视觉标记仍能维持97.4%的基线性能。

Conclusion: 对象中心表示在高效有效的LVLM处理中具有优越性，为视觉标记压缩提供了新的解决方案。
...
TL;DR: 提出了一种无需训练的任务特定模型合成方法ZS-TMS，通过预训练生成引擎直接合成分类器参数，仅需单张示例图像和临床文本描述即可在1-shot场景下工作。
...
Method: 使用语义引导参数合成器SGPS，输入最少的多模态任务信息（单张图像和临床文本），直接合成轻量级分类器的全部参数，无需任务特定训练。

Result: 在ISIC 2018皮肤病变数据集和自定义罕见疾病数据集上的评估表明，该方法在1-shot和5-shot分类中显著优于现有few-shot和zero-shot学习方法，建立了新的最先进水平。

Conclusion: 该工作为AI驱动的诊断工具快速开发和部署开辟了新途径，特别是在数据极其有限的罕见疾病领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在语言到视觉空间元素的基础任务中存在局限性，主要由于训练数据侧重语言推理而非空间理解，且缺乏3D资源导致固有基础偏差未解决。

Method: 提出3D场景编辑机制生成精确视觉反事实，并设计DEER-3D框架，采用"分解、诊断评估、编辑、重新训练"的工作流程，通过最小化谓词对齐的3D场景编辑（如重新着色或重新定位）产生针对性反事实监督。

Result: 在多个3D基础和场景理解任务的基准测试中，通过迭代精炼在所有评估数据集上均显示出持续改进。

Conclusion: DEER-3D证明了针对性、错误驱动的场景编辑在弥合3D LLMs中语言推理能力与空间基础之间的有效性。
...
TL;DR: 提出了一种基于二元交叉熵的三方协同学习框架BCE3S，用于解决长尾识别任务中特征紧凑性和类别间分离性的问题，在多个长尾数据集上实现了最先进的性能。
...
Method: BCE3S包含三个组件：基于BCE的联合学习优化分类器和样本特征；基于BCE的对比学习进一步提高特征的类内紧凑性；基于BCE的均匀学习平衡分类器向量间的分离性并与联合学习交互增强特征特性。

Result: 在CIFAR10-LT、CIFAR100-LT、ImageNet-LT和iNaturalist2018等多个长尾数据集上的广泛实验表明，BCE3S训练的长尾识别模型不仅实现了更高的特征紧凑性和分离性，还平衡了分类器的分离性。

Conclusion: BCE3S框架通过三方协同学习有效解决了长尾识别中的特征学习和分类器平衡问题，在多个数据集上达到了最先进的性能。
...
TL;DR: FAPE-IR是一个用于全场景图像修复的统一框架，通过冻结的多模态大语言模型分析退化图像并生成频率感知修复计划，指导基于扩散模型的执行器进行动态频率选择，实现了多退化类型下的统一修复。


<details>
  <summary>Details</summary>
Motivation: 现有的全场景图像修复方法通常依赖任务特定设计或潜在路由策略，难以适应真实世界中各种复杂退化情况，需要开发更统一和可解释的解决方案。

Method: 使用冻结的多模态大语言模型作为规划器分析退化图像并生成频率感知修复计划；基于LoRA的混合专家模块在扩散模型执行器中动态选择高频或低频专家；引入对抗训练和频率正则化损失来提升修复质量。

Result: 在七个修复任务上实现了最先进的性能，并在混合退化条件下展现出强大的零样本泛化能力。
...
TL;DR: RIVER是一个基于推理的视频编辑模型，能够通过多跳推理理解隐式查询来推断编辑目标，然后执行视频修改。它使用数字孪生表示和强化学习训练，在多个基准测试中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要用户提供精确的空间位置和时间边界描述，这在用户尝试通过语义属性或对象关系的隐式查询进行编辑时变得不切实际。

Method: RIVER通过数字孪生表示解耦推理和生成，使用大型语言模型进行多跳推理，输出结构化指令指导基于扩散的编辑器执行像素级修改，采用强化学习训练。

Result: RIVER在提出的RVEBenchmark（100个视频，519个隐式查询）上表现最佳，在VegGIE和FiVE两个额外视频编辑基准测试中也达到了最先进的性能，超越了六个基线方法。

Conclusion: RIVER通过引入推理视频编辑任务和相应的模型，成功解决了理解隐式查询进行视频编辑的挑战，在多个基准测试中展示了优越性能。
...
TL;DR: 提出了一种实时自监督单目深度估计方法RTS-Mono，采用轻量级编码器-解码器架构，在保持高性能的同时实现实时推理，在KITTI数据集上达到最先进性能。
...
Result: 在KITTI数据集上，RTS-Mono以极低的参数量（3M）在高分辨率和低分辨率下均达到最先进性能，在低分辨率下Abs Rel和Sq Rel分别提升5.6%和9.8%，在高分辨率下Sq Rel和RMSE分别提升6.1%和1.9%。在实际部署中，在Nvidia Jetson Orin上实现49 FPS的实时推理。

Conclusion: RTS-Mono是一种轻量高效的实时自监督单目深度估计方法，成功解决了现有方法计算资源消耗大和轻量化后性能下降的问题，实现了实际部署。
...
TL;DR: 提出了一种名为A²GC-VPR的非对称聚合视觉地点识别方法，通过行-列归一化平均和单独边缘校准实现非对称匹配，并结合几何约束增强空间感知能力。
...
Method: 采用行-列归一化平均与单独边缘校准实现非对称匹配，通过可学习坐标嵌入引入几何约束，计算融合特征相似度的兼容性分数。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明性能优越，验证了该方法在提高匹配精度和鲁棒性方面的有效性。

Conclusion: 提出的非对称聚合方法能够适应视觉地点识别中的分布差异，结合几何约束有效提升了空间感知和匹配性能。
...
TL;DR: 提出Cascaded-ViT (CViT)轻量级视觉Transformer架构，通过新颖的Cascaded-Chunk Feed Forward Network (CCFFN)设计，在保持准确性的同时提高参数和FLOP效率。
...
Conclusion: CViT架构特别适合部署在电池受限设备上，如手机和无人机，并在新的Accuracy-Per-FLOP (APF)指标下表现出卓越的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型在微调过程中学习不需要概念的问题，这对于偏见缓解、防止恶意适应、属性解缠和扩散策略的泛化微调等下游任务至关重要。

Method: 通过保持用户提示的嵌入与不希望学习的概念不对齐来实现，核心在于使用语言描述指定不希望学习的概念来正则化适应过程，无需额外训练且可灵活修改不希望学习的概念。

Result: 实验结果表明，Coffee能够有效防止文本到图像模型在微调过程中学习指定的不希望概念，并且优于现有方法。

Conclusion: Coffee提供了一种有效的可控微调方法，通过语言描述灵活指定不希望学习的概念，在防止模型学习不需要概念方面表现出色，具有实际应用价值。
...
TL;DR: 本文提出了MP-PVIR框架，通过多视角视频分析将行人-车辆事故转化为结构化诊断报告，包含四个阶段：事件触发视频采集、行为阶段分割、阶段感知多视角推理、层次化合成与诊断推理。


<details>
  <summary>Details</summary>
Motivation: 解决现有视频系统只能检测事故但无法分析行人行为认知阶段的问题，利用视觉语言模型提升对行人-车辆事故的理解能力。

Method: 基于两个专用VLM：TG-VLM用于行为阶段分割，PhaVR-VLM用于阶段感知多视角分析，结合大语言模型生成综合报告。

Result: TG-VLM在行为阶段分割上达到mIoU=0.4881，PhaVR-VLM在字幕生成得分33.063，问答准确率最高达64.70%。

Conclusion: MP-PVIR能有效将多视角视频数据转化为可操作的见解，推进AI驱动的交通安全分析在车路协同系统中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在理解抽象视觉输入（特别是手绘草图）方面存在局限，主要瓶颈是缺乏同时建模草图、真实图像和自然语言指令的大规模数据集。

Method: 提出了两个关键贡献：(1) 构建大规模图像-草图-指令三元组数据集，支持预训练和指令调优；(2) 基于该数据集训练O3SLM模型。

Result: 在多个草图任务（对象定位、计数、图像检索、视觉问答）上的综合评估显示，O3SLM在草图理解和推理方面显著优于现有LVLMs，达到最先进性能。
...
TL;DR: Wave-Former是一种利用毫米波无线信号进行3D形状重建的新方法，能够穿透遮挡物重建被完全遮挡的日常物体的高精度3D形状。
...
Result: 在对比实验中，Wave-Former将召回率从54%提升至72%，同时保持85%的高精度。该方法仅使用合成点云训练，但在真实数据上表现出良好的泛化能力。

Conclusion: Wave-Former通过物理感知的形状补全模型，成功解决了传统毫米波重建方法覆盖范围有限和噪声高的问题，实现了对完全遮挡物体的高精度3D形状重建。
...
### [43] [AdaTok: Adaptive Token Compression with Object-Aware Representations for Efficient Multimodal LLMs](https://arxiv.org/abs/2511.14169)
*Xinliang Zhang,Lei Zhu,Hangzhou He,Shuang Zeng,Ourui Fu,Jiakui Hu,Zhengjian Yao,Yanye Lu*

Main category: cs.CV

TL;DR: 本文提出了一种对象级令牌压缩策略，通过自适应令牌压缩来减少多模态大语言模型中图像令牌的数量，仅使用10%的令牌即可达到原始模型96%的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的补丁级令牌化导致图像令牌数量呈二次增长，给MLLMs带来巨大的计算和内存负担，且与人类视觉认知系统不一致，导致幻觉和计算冗余。

Method: 提出对象级令牌合并策略进行自适应令牌压缩，该方法与人类视觉系统保持一致。

Result: 在多个综合基准测试中，该方法平均仅使用10%的令牌即可达到原始模型约96%的性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 提出的对象级令牌压缩策略有效解决了MLLMs中图像令牌过多的问题，在保持性能的同时显著降低了计算和内存需求。
...
Method: 提出RiSe框架，包含AsyIRM学习径向空间的不变球形决策边界来适应非对称分布，以及MMSD通过跨样本混合和解耦增强内在、可泛化的模态特征。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入如何削弱大型视觉语言模型稳健性的综合基准，包含三个层次误导视觉输入和6个类别1248个VQA实例，并提出了细粒度稳健性评估指标MVI-Sensitivity。


<details>
  <summary>Details</summary>
Motivation: 现有稳健性基准主要关注幻觉或误导性文本输入，而忽视了误导性视觉输入对视觉理解评估的同等重要挑战，需要填补这一重要空白。

Method: 基于基本视觉原语，设计了三个层次误导视觉输入：视觉概念、视觉属性和视觉关系，构建了6个代表性类别和1248个专家标注的VQA实例，并提出了MVI-Sensitivity细粒度评估指标。

Result: 对18个最先进LVLM的实证结果揭示了它们对误导性视觉输入的显著脆弱性，深入分析为开发更可靠和稳健的LVLM提供了可操作的见解。

Conclusion: MVI-Bench填补了误导性视觉输入评估的重要空白，揭示了LVLM的脆弱性，为开发更可靠的模型提供了指导，基准和代码库已开源。
...
Method: 提出DoGCLR框架，将正负样本构建建模为动态优势博弈，使用时空双权重定位机制识别关键运动区域，并采用熵驱动优势策略管理负样本库。
...
TL;DR: UniSER是一个统一的基础模型，能够在一个框架内处理多种软效应退化问题，如镜头光晕、雾霾、阴影和反射等，通过大规模数据集和定制训练流程实现优于专业模型和通用模型的鲁棒恢复效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常针对单一退化问题开发专业模型，缺乏可扩展性且未能利用这些恢复问题的共享本质。而通用模型需要详细提示且在这些细粒度任务上表现不佳或无法保持场景身份。

Method: 构建包含380万对图像的大规模数据集，包含新颖的物理合理数据以填补公共基准的关键空白，并采用定制训练流程微调扩散变换器，从多样化数据中学习鲁棒恢复先验，集成细粒度掩码和强度控制。

Result: UniSER在野外环境中显著优于专业模型和通用模型，实现了鲁棒、高保真的恢复效果。

Conclusion: 通过利用软效应的共同本质（半透明遮挡），UniSER能够在单一框架内有效处理多种退化问题，展示了基础通用模型在图像恢复任务中的优越性。
...
TL;DR: GloTok是一种全局视角的图像标记化方法，通过利用全局关系信息建模更均匀的语义分布，结合码本直方图关系学习和残差学习模块，在ImageNet-1k基准上实现了最先进的图像重建和生成质量。
...
Method: 提出全局视角标记器(GloTok)，采用码本直方图关系学习方法将预训练模型在整个数据集上建模的语义转移到语义码本，并设计残差学习模块恢复细粒度细节以最小化量化引起的重建误差。

Result: 在标准ImageNet-1k基准测试中，该方法实现了最先进的重建性能和生成质量。

Conclusion: GloTok通过全局关系建模产生了更均匀分布的语义潜在表示，有助于自回归模型训练生成高质量图像，且训练过程无需直接访问预训练模型。
...
### [49] [GEN3D: Generating Domain-Free 3D Scenes from a Single Image](https://arxiv.org/abs/2511.14291)
*Yuxin Zhang,Ziyu Lu,Hongbo Duan,Keyu Fan,Pengting Luo,Peiyu Zhuang,Mengyu Yang,Houde Liu*

Main category: cs.CV

TL;DR: Gen3d是一种从单张图像生成高质量、广范围通用3D场景的新方法，通过RGBD图像生成初始点云，维护和扩展世界模型，最终通过优化高斯泼溅表示完成3D场景构建。


<details>
  <summary>Details</summary>
Motivation: 当前神经3D重建方法依赖密集多视角捕捉，限制了更广泛应用；3D场景生成对推进具身AI和世界模型至关重要，需要多样化的高质量场景进行学习和评估。

Method: 从RGBD图像生成初始点云，维护和扩展世界模型，通过优化高斯泼溅表示完成3D场景构建。
...
TL;DR: PAVE是首个完全由自动驾驶模式在真实世界收集的端到端基准数据集，包含100+小时自然驾驶数据，用于评估自动驾驶车辆的行为安全性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集多为人工驾驶或未明确驾驶模式收集，只能用于感知和预测的早期训练，无法评估自动驾驶系统的真实行为安全性。

Method: 收集多个量产自动驾驶车辆的自然驾驶数据，分割成32,727个关键帧，包含同步摄像头图像、高精度GNSS/IMU数据、车辆轨迹以及详细的2D标注。

Result: 数据集包含丰富的场景属性，使用端到端运动规划模型在自动驾驶帧上实现1.4米的平均位移误差。数据集每周持续扩展10+小时新数据。

Conclusion: PAVE数据集为自动驾驶行为分析和安全评估提供了可持续的研究基础，是首个完全由自动驾驶模式收集的端到端基准数据集。
...
### [51] [SAM-Fed: SAM-Guided Federated Semi-Supervised Learning for Medical Image Segmentation](https://arxiv.org/abs/2511.14302)
*Sahar Nasirihaghighi,Negin Ghamsarian,Yiping Li,Marcel Breeuwer,Raphael Sznitman,Klaus Schoeffmann*

Main category: cs.CV

TL;DR: SAM-Fed是一个联邦半监督学习框架，利用高容量分割基础模型指导轻量级客户端训练，通过双重知识蒸馏和自适应一致性机制提升像素级监督质量，在皮肤病变和息肉分割任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据隐私和专家标注成本高的限制，联邦半监督学习虽然提供了解决方案，但存在伪标签可靠性依赖于本地模型能力、客户端设备计算资源有限需要轻量级或异构架构等问题。

Method: 提出SAM-Fed框架，结合双重知识蒸馏和自适应一致性机制，利用高容量分割基础模型指导轻量级客户端训练，提升像素级监督质量。

Result: 在皮肤病变和息肉分割任务上，无论是同构还是异构设置下，SAM-Fed都持续优于最先进的联邦半监督学习方法。

Conclusion: SAM-Fed通过利用基础模型指导轻量级客户端，有效解决了联邦半监督学习中伪标签质量不稳定的问题，在医学图像分割任务中取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 主动脉作为人体最大的动脉，容易出现夹层、动脉瘤和动脉粥样硬化等病变，需要及时干预。现有方法往往忽视分层解剖关系，且难以处理血管结构中固有的严重类别不平衡问题。

Method: 采用课程学习策略，利用新颖的分形softmax进行分层语义学习。该方法模拟人类认知过程，从简单到复杂逐步学习解剖约束。通过两阶段推理策略实现高达五倍的加速。

Result: 在验证集上，分层语义损失将nnU-Net ResEnc M的Dice分数提高了11.65%。在测试集上，所提模型的Dice分数比基线高出5.6%。实验结果显示分割精度和效率均有显著提升。

Conclusion: 该框架显著提高了主动脉血管分割的准确性和效率，使其适用于实时临床应用。代码已公开可用。
...
TL;DR: DetGain是一种专门用于目标检测的在线数据筛选方法，通过估计每张图像对数据集级AP的边际扰动来选择信息丰富的训练样本，在COCO数据集上实现了精度提升。
...
Conclusion: DetGain作为一种通用且互补的策略，在数据高效的目标检测中具有巨大潜力，具有架构无关性和最小侵入性的特点。
...
TL;DR: Bapp是一款基于Flutter框架和Google ML Kit开发的移动应用，用于实时分析眼睑运动，在45个真实患者视频上验证显示98.3%的准确率，为临床眼睑运动监测提供了便携可靠的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有眼睑运动评估工具复杂、昂贵且临床应用有限，需要开发便携、可访问的客观评估工具来监测正常和异常眼睑运动。

Method: 使用Flutter框架和Google ML Kit开发移动应用Bapp，在45个真实患者视频上进行临床验证，由眼科专家手动标注眨眼作为基准真值，使用精确度、召回率和F1分数等标准指标评估性能。

Result: Bapp表现出98.4%的精确度、96.9%的召回率和98.3%的整体准确率，验证了其作为眼睑运动监测工具的可靠性。

Conclusion: Bapp为传统手动眨眼计数提供了有前景的替代方案，支持临床环境中的持续眼部健康监测和术后评估，是一款便携、可访问且客观的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在视频重建中面临的时间伪影问题或迭代后验采样速度过慢的问题，使扩散基视频重建能够满足实时、交互式流媒体场景的需求。

Method: 将预训练的双向视频扩散模型（教师）蒸馏为因果自回归学生模型，使用先验驱动的蒸馏方法，仅需教师扩散模型和已知退化算子，无需外部配对数据。采用LeanVAE替换视频扩散骨干VAE，通过教师空间正则化蒸馏方案实现低延迟潜在空间处理。

Result: 在流式随机修复、高斯去模糊和超分辨率任务中，InstantViR匹配或超越了扩散基基准的重建质量，在NVIDIA A100 GPU上运行速度超过35 FPS，比迭代视频扩散求解器快达100倍。
...
### [56] [Cheating Stereo Matching in Full-scale: Physical Adversarial Attack against Binocular Depth Estimation in Autonomous Driving](https://arxiv.org/abs/2511.14386)
*Kangqiao Zhao,Shuo Huai,Xurui Song,Jun Luo*

Main category: cs.CV

TL;DR: 提出首个针对自动驾驶中立体匹配模型的纹理物理对抗攻击方法，使用3D全局伪装纹理而非局部2D补丁，确保视觉一致性和攻击有效性。
...
Method: 采用3D物理对抗示例，提出新的3D立体匹配渲染模块处理相机视差效应，并设计精细融合攻击将目标无缝融入环境。

Result: 广泛评估表明，该方法能成功欺骗立体模型产生错误深度信息，相比现有隐藏攻击具有显著增强的隐蔽性和杀伤力。

Conclusion: 该方法证明了针对立体视觉系统的物理对抗攻击的有效性，为自动驾驶安全提供了重要参考。
...
Result: 实验表明，该方法能够生成与提示对齐的结果，并且在盲人脸恢复任务中优于现有方法。
...
### [58] [Agentic Video Intelligence: A Flexible Framework for Advanced Video Exploration and Understanding](https://arxiv.org/abs/2511.14446)
*Hong Gao,Yiming Bao,Xuezhen Tu,Yutong Xu,Yue Jin,Yiyang Mu,Bin Zhong,Linan Yue,Min-Ling Zhang*

Main category: cs.CV

TL;DR: AVI是一个无需训练的视频理解框架，通过三阶段推理过程（检索-感知-回顾）、结构化视频知识库和开源模型集成，实现人类级别的视频理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通常采用单次处理方式，缺乏证据重访和迭代优化能力；基于智能体的方法要么依赖昂贵的专有模型，要么需要大量强化学习训练。

Method: 提出三个关键创新：1）受人类启发的三阶段推理过程；2）基于实体图的结构化视频知识库和多粒度集成工具；3）结合推理LLM、轻量级基础CV模型和VLM的开源模型集成。

Result: 在LVBench、VideoMME-Long、LongVideoBench和Charades-STA等基准测试中表现出竞争力，同时提供更好的可解释性。

Conclusion: AVI框架在无需依赖专有API或强化学习训练的情况下，实现了竞争性的视频理解性能，并提供了优越的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的3D面部动画方法在处理长音频序列时存在两个主要问题：超出训练范围时性能下降，以及处理长音频输入时延迟显著。

Method: 提出自回归扩散模型，采用流式处理方式，选择有限数量的历史帧作为运动上下文，结合音频输入创建动态条件，指导扩散过程迭代生成面部运动帧。

Result: 该方法能够灵活处理不同长度的音频输入，实现低延迟的实时合成，并提供了实时交互演示验证了方法的有效性和效率。

Conclusion: 提出的流式自回归扩散模型解决了长音频序列处理的挑战，实现了高质量、低延迟的实时3D面部动画生成。
...
### [60] [Deep Learning-Based Regional White Matter Hyperintensity Mapping as a Robust Biomarker for Alzheimer's Disease](https://arxiv.org/abs/2511.14588)
...
Result: 预测病变负荷与参考WMH估计一致，区域WMH体积在疾病分类中始终优于全局病变负担，结合萎缩指标AUC可达0.97，前部白质束区域与AD诊断状态相关。
...
### [61] [Enhancing Generalization of Depth Estimation Foundation Model via Weakly-Supervised Adaptation with Regularization](https://arxiv.org/abs/2511.14238)
*Yan Huang,Yongyi Su,Xin Lin,Le Zhang,Xun Xu*

Main category: cs.CV

TL;DR: WeSTAR是一个参数高效的弱监督自训练框架，通过正则化增强单目深度估计基础模型在未见领域的鲁棒性，结合密集自监督、语义感知分层归一化和弱监督排序约束来提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在零样本单目深度估计方面取得了进展，但在获得下游任务数据后，如何进一步提升这些模型的性能仍然是一个重要问题。

Method: 采用密集自训练目标作为结构自监督，引入语义感知分层归一化进行多尺度结构归一化，使用成对深度排序注释作为弱监督，并通过权重正则化损失确保训练稳定性。

Result: 在多种具有挑战性的现实和损坏分布外数据集上的广泛实验表明，WeSTAR始终能提高泛化能力，并在广泛基准测试中达到最先进的性能。

Conclusion: WeSTAR框架通过参数高效的方式有效增强了单目深度估计基础模型在未见领域的鲁棒性和泛化性能。
...
### [62] [CCSD: Cross-Modal Compositional Self-Distillation for Robust Brain Tumor Segmentation with Missing Modalities](https://arxiv.org/abs/2511.14599)
*Dongqing Xie,Yonghuang Wu,Zisheng Ai,Jun Min,Zhencun Jiang,Shaojin Geng,Lei Wang*

Main category: cs.CV

TL;DR: 提出CCSD框架解决多模态MRI脑肿瘤分割中模态缺失问题，通过跨模态自蒸馏策略处理任意模态组合输入。


<details>
  <summary>Details</summary>
Motivation: 真实临床环境中经常出现一个或多个MRI模态缺失，严重影响深度学习分割模型的性能和泛化能力。

Method: 采用共享-特定编码器-解码器架构，包含层次化模态自蒸馏机制和渐进式模态组合蒸馏方法。

Result: 在公共脑肿瘤分割基准测试中，CCSD在各种缺失模态场景下均达到最先进性能，具有强泛化性和稳定性。

Conclusion: CCSD框架能有效处理任意模态组合的脑肿瘤分割任务，在模态缺失情况下仍保持优异性能。
...
TL;DR: 提出了一种基于LiDAR定位的GNSS-free协作感知框架，包括轻量级姿态生成器和置信度感知的时空对齐Transformer，在GNSS拒止环境下实现最先进的性能。
...
Method: 提出轻量级PGC模块估计紧凑姿态和置信度表示，开发PASTAT进行置信度感知的空间对齐并捕获关键时间上下文，创建V2VLoc仿真数据集。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS拒止条件下达到最先进性能，在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化性。

Conclusion: 提出的GNSS-free协作感知框架通过LiDAR定位和置信度感知的时空对齐，在GNSS拒止环境中实现了鲁棒的协作感知性能。
...
TL;DR: 本文提出了一种基于高斯泼溅的低秩张量表示框架（GSLR），用于解决传统张量奇异值分解方法在捕捉空间局部高频信息方面的局限性。
...
Method: 提出GSLR框架，利用定制的2D高斯泼溅生成潜在张量，1D高斯泼溅生成变换矩阵，两者在该表示框架下不可或缺且互补。

Result: 在多维图像恢复任务上的大量实验表明，GSLR始终优于最先进的方法，特别是在捕捉局部高频信息方面。

Conclusion: GSLR框架能够紧凑且连续地表示多维图像，具有强大的表示能力，尤其擅长捕捉局部高频信息。
...
TL;DR: VLM3D是一个通用框架，利用大型视觉语言模型作为可微分的语义和空间评判器，通过双查询评判信号改进文本到3D生成中的语义对齐和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到3D生成模型存在两个基本限制：语义对齐粗糙，无法捕捉细粒度提示细节；缺乏稳健的3D空间理解，导致几何不一致和部件组装失败。

Method: 提出VLM3D框架，利用视觉语言模型的Yes或No对数几率作为双查询评判信号，评估语义保真度和几何连贯性。该指导信号可应用于优化基管道和前馈管道。

Result: 在优化基管道中，VLM3D在标准基准测试中显著优于现有方法；在前馈管道中，它能主动引导SOTA原生3D模型的迭代采样过程，纠正严重空间错误。

Conclusion: VLM3D为将视觉语言模型丰富的语言基础语义和空间理解注入多样化3D生成管道提供了一条原则性和可推广的路径。
...
TL;DR: 本文提出了一种简单有效的方法，利用标准分割架构对时间多普勒全息图进行动静脉分割，通过结合脉冲分析特征，使传统U-Net能够利用时间动态信息。
...
### [68] [Impact of Image Resolution on Age Estimation with DeepFace and InsightFace](https://arxiv.org/abs/2511.14689)
*Shiyar Jamo*
...
<details>
  <summary>Details</summary>
Motivation: 自动年龄估计在年龄验证中广泛应用，但输入图像分辨率差异很大，需要研究分辨率对年龄估计准确性的影响。

Method: 使用IMDB-Clean数据集的1000张图像，在7种不同分辨率下处理得到7000个测试样本，通过MAE、SD和MedAE评估DeepFace和InsightFace的性能。

Result: 两个框架在224x224像素时性能最佳，DeepFace的MAE为10.83年，InsightFace为7.46年。低分辨率时MAE显著增加，过高分辨率也会降低准确性。InsightFace在所有分辨率下都比DeepFace更快。
...
### [69] [NeuralBoneReg: A Novel Self-Supervised Method for Robust and Accurate Multi-Modal Bone Surface Registration](https://arxiv.org/abs/2511.14286)
*Luohong Wu,Matthias Seibold,Nicola A. Cavalcanti,Yunke Ao,Roman Flepp,Aidana Massalimova,Lilian Calvet,Philipp Fürnstahl*

Main category: cs.CV

TL;DR: NeuralBoneReg是一种自监督、基于表面的骨表面配准框架，使用3D点云作为模态无关表示，解决了计算机辅助骨科手术中跨模态配准的挑战。


<details>
  <summary>Details</summary>
Motivation: 在计算机辅助骨科手术中，术前和术中数据的模态异质性使得配准具有挑战性且容易出错，因此需要鲁棒、自动和模态无关的骨表面配准方法。
...
Conclusion: NeuralBoneReg在解剖结构和模态之间表现出强大的泛化能力，为计算机辅助骨科手术提供了鲁棒且准确的跨模态对齐方法。
...
### [70] [Seeing Beyond the Image: ECG and Anatomical Knowledge-Guided Myocardial Scar Segmentation from Late Gadolinium-Enhanced Images](https://arxiv.org/abs/2511.14702)
*Farheen Ramzan,Yusuf Kiberu,Nikesh Jathanna,Meryem Jabrane,Vicente Grau,Shahnaz Jamil-Copley,Richard H. Clayton,Chen,Chen*

Main category: cs.CV

TL;DR: 提出了一种新颖的多模态框架，将ECG电生理信息与AHA-17图谱解剖先验相结合，用于LGE心肌瘢痕分割。通过时间感知特征融合机制处理非同时采集的ECG和LGE-MRI数据，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI中的心肌瘢痕准确分割对评估组织活力至关重要，但由于对比度变化和成像伪影而具有挑战性。ECG信号提供补充的生理信息，传导异常有助于定位瘢痕区域。

Method: 提出多模态框架，集成ECG电生理信息和AHA-17图谱解剖先验。引入时间感知特征融合机制，根据采集时间差动态加权和融合特征。

Result: 在临床数据集上评估，相比最先进的仅图像基线方法，瘢痕平均Dice分数从0.6149提升至0.8463，在精度和灵敏度方面均表现优异。
...
### [71] [Zero-shot Synthetic Video Realism Enhancement via Structure-aware Denoising](https://arxiv.org/abs/2511.14719)
*Yifan Wang,Liya Ji,Zhanghan Ke,Harry Yang,Ser-Nam Lim,Qifeng Chen*

Main category: cs.CV

TL;DR: 提出了一种零样本框架，通过扩散视频基础模型增强合成视频的真实感，同时保持空间和时间域的多层次结构一致性。


<details>
  <summary>Details</summary>
Motivation: 旨在提升合成视频的真实感，使其能够以照片级真实感重新渲染模拟器生成的合成视频，同时保持原始视频的结构和语义一致性。

Method: 使用扩散视频基础模型，无需微调，通过辅助模型从合成视频中提取深度图、语义图和边缘图等结构感知信息，指导生成/去噪过程。

Result: 在实验中优于现有基线方法，在保持最先进照片级真实感质量的同时，与原始视频的结构一致性更好。

Conclusion: 该方法是一种简单、通用且强大的合成视频真实感增强方法，能够有效保持结构一致性并提升视觉真实感。
...
### [72] [ARC Is a Vision Problem!](https://arxiv.org/abs/2511.14761)
*Keya Hu,Ali Cy,Linlu Qiu,Xiaoman Delores Ding,Runqian Wang,Yeyin Eva Zhu,Jacob Andreas,Kaiming He*

Main category: cs.CV

TL;DR: 本文提出Vision ARC (VARC)框架，将抽象推理语料库(ARC)重新定义为视觉问题，通过图像到图像转换方法，使用Vision Transformer架构在ARC数据上从头训练，在ARC-1基准上达到60.4%的准确率，显著优于现有从头训练的方法。


<details>
  <summary>Details</summary>
Motivation: ARC旨在促进抽象推理研究，但现有方法多将其视为语言导向问题，而ARC任务本质上是视觉性的。本文希望从视觉角度重新构建ARC问题，利用视觉先验知识。

Method: 将ARC问题构建为图像到图像转换任务，在"画布"上表示输入以融入视觉先验，使用标准视觉架构（如ViT）进行图像到图像映射，仅使用ARC数据从头训练，通过测试时训练泛化到未见任务。

Result: 在ARC-1基准上达到60.4%的准确率，大幅优于其他从头训练的方法，与领先的LLMs性能相当，接近人类平均表现水平。

Conclusion: 视觉导向的方法在抽象推理任务上具有显著优势，VARC框架证明了从视觉角度处理ARC问题的有效性，缩小了与人类表现的差距。
...
### [73] [Iterative Diffusion-Refined Neural Attenuation Fields for Multi-Source Stationary CT Reconstruction: NAF Meets Diffusion Model](https://arxiv.org/abs/2511.14310)
...
Method: Diff-NAF框架包含神经衰减场表示和双分支条件扩散模型，采用角度先验引导投影合成策略和扩散驱动重用投影细化模块进行迭代优化。
...
### [74] [Dental3R: Geometry-Aware Pairing for Intraoral 3D Reconstruction from Sparse-View Photographs](https://arxiv.org/abs/2511.14315)
*Yiyi Miao,Taoyu Wu,Tong Chen,Ji Jiang,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: Dental3R是一种从稀疏口腔内照片进行3D重建的方法，通过几何感知配对策略和基于小波的正则化，解决了传统方法在远程正畸中面临的姿态估计不稳定和细节丢失问题。


<details>
  <summary>Details</summary>
Motivation: 传统口腔内扫描方法无法用于远程正畸，而基于稀疏智能手机图像的方法在姿态估计和几何重建方面存在挑战，特别是在大视角基线、不一致光照和镜面反射等口腔环境下。

Method: 提出Dental3R管道，包括几何感知配对策略(GAPS)选择高价值图像对，以及基于小波正则化的3D高斯溅射训练，通过离散小波变换保持频带保真度。

Result: 在950个临床病例和195个视频测试集上验证，Dental3R能有效处理稀疏无姿态输入，在牙齿咬合可视化方面实现卓越的新视角合成质量，优于现有方法。

Conclusion: Dental3R为远程正畸提供了一种稳健、高保真度的3D重建解决方案，能够从稀疏口腔照片中恢复关键诊断细节。
...
### [75] [ArchMap: Arch-Flattening and Knowledge-Guided Vision Language Model for Tooth Counting and Structured Dental Understanding](https://arxiv.org/abs/2511.14336)
*Bohan Zhang,Yiyi Miao,Taoyu Wu,Tong Chen,Ji Jiang,Zhuoxiao Li,Zhe Tang,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: ArchMap是一个无需训练的知识引导框架，用于3D口腔扫描的结构化理解。它通过几何感知的牙弓展平模块和多模态推理，在牙齿计数、解剖分区、牙列阶段分类等任务上表现出色。
...
Method: 1. 几何感知牙弓展平模块：将原始3D网格标准化为空间对齐的多视图投影；2. 构建牙科知识库：编码分层牙齿本体论、牙列阶段策略和临床语义；3. 知识引导的多模态推理。

Result: 在1060个正畸前后病例上验证，在牙齿计数、解剖分区、牙列阶段分类和临床状况识别方面表现稳健。相比监督方法和VLM基线，准确率更高，语义漂移减少，在稀疏或有伪影条件下稳定性更好。

Conclusion: ArchMap证明结合几何标准化与本体论引导的多模态推理，为现代数字正畸中3D口腔扫描的结构化分析提供了实用且可扩展的解决方案。
...
### [76] [Silhouette-to-Contour Registration: Aligning Intraoral Scan Models with Cephalometric Radiographs](https://arxiv.org/abs/2511.14343)
*Yiyi Miao,Taoyu Wu,Ji Jiang,Tong Chen,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: DentalSCR是一个用于口腔内扫描模型与侧位头颅X光片精确配准的框架，通过构建统一解剖坐标系和轮廓引导的配准方法，解决了传统强度驱动方法在临床条件下的稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统的强度驱动配准方法在真实临床条件下（如投影放大、几何畸变、低对比度牙冠等）难以稳定工作，导致配准失败或解剖学上不合理的对齐。

Method: 首先构建U-Midline Dental Axis建立统一解剖坐标系，然后通过表面DRR生成X光片样投影，最后使用对称双向Chamfer距离进行分层粗到细的2D相似变换优化。

Result: 在34个专家标注的临床病例上评估，显著降低了标志点误差（特别是后牙区域），下颌区域误差分布更紧密，曲线级别的Chamfer和Hausdorff距离表现优异。
...
TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频章节化模型，通过双语、时序定位和分层章节标注，在视频内容结构化方面取得显著突破。
...
Method: 通过结构化流程构建了双语英中章节数据集，统一ASR转录、场景文本、视觉描述为多级标注，从短标题到长摘要；设计了新的评估指标GRACE，结合多对一段落重叠和语义相似度。

Result: ARC-Chapter在F1分数上比之前最佳方法提升14.0%，在SODA分数上提升11.3%，建立了新的最先进水平；在YouCook2等下游任务的密集视频描述任务中也表现出优秀的迁移能力。

Conclusion: 数据规模（数据量和标签强度）带来明显性能提升，ARC-Chapter通过大规模训练和精心设计的评估指标，在视频章节化任务上实现了显著进步，并展现出良好的泛化能力。
...
### [78] [IBGS: Image-Based Gaussian Splatting](https://arxiv.org/abs/2511.14357)
*Hoang Chuong Nguyen,Wei Mao,Jose M. Alvarez,Miaomiao Liu*

Main category: cs.CV

TL;DR: 提出Image-Based Gaussian Splatting方法，通过结合标准3DGS渲染的基础颜色和从相邻训练图像学习的残差，显著提升渲染质量，特别是高频细节和视角相关效果，且不增加存储开销。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法使用低阶球谐函数限制了捕捉空间变化颜色和视角相关效果（如镜面高光）的能力。现有增强方法要么使用全局纹理图（难以处理复杂场景），要么使用逐高斯纹理图（存储开销大）。

Method: 提出基于图像的GS方法，利用高分辨率源图像进行精细细节和视角特定颜色建模。将每个像素颜色建模为标准3DGS渲染的基础颜色与从相邻训练图像学习的残差的组合。

Result: 在标准新视角合成基准测试中，该方法在渲染质量上显著优于先前的GS方法，能够渲染高频细节和准确的视角相关效果。

Conclusion: 该方法在不增加存储占用的情况下，显著提升了3D高斯溅射的渲染质量，特别是对高频细节和视角相关效果的捕捉能力。
...
### [79] [A Quantitative Method for Shoulder Presentation Evaluation in Biometric Identity Documents](https://arxiv.org/abs/2511.14376)
*Alfonso Pedro Ridao*

Main category: cs.CV

TL;DR: 提出一种肩部姿态评估算法，用于自动评估生物特征身份证件中人像的肩部姿态合规性，该方法仅使用两个肩部标志点的3D坐标来量化肩部偏转和滚动角度。
...
Result: 在121张人像图像数据集上评估，SPE得分与人工标注标签显示出强Pearson相关性（r≈0.80），使用改进的错误-丢弃方法分析证实了该指标在识别不合规样本方面的有效性。
...
TL;DR: BEDLAM2.0是一个新的数据集，在BEDLAM基础上显著提升了多样性、真实性和功能，特别是增加了世界坐标系下人体运动估计的能力，通过更丰富的视频数据和地面真实标注来推动3D人体运动估计研究。


<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，特别是当需要估计世界坐标系下的人体运动时。现有方法受限于缺乏包含真实人体和相机运动的地面真实数据的丰富视频数据集。

Method: 开发了BEDLAM2.0数据集，在BEDLAM基础上引入了更多样化和真实的相机及相机运动，增加了身体形状、动作、服装、头发和3D环境的多样性和真实性，并添加了鞋子等缺失元素。

Result: BEDLAM2.0在训练世界坐标系下人体估计方法方面显著优于BEDLAM，使用BEDLAM2.0训练的最先进方法相比使用BEDLAM训练的方法在准确性上有显著提升。

Conclusion: BEDLAM2.0是一个比BEDLAM更优秀的数据集，特别适用于训练世界坐标系下的人体运动估计方法，为3D人体姿态和运动回归器的训练提供了关键资源。
...
Method: 使用序数回归框架，结合绿色通道提取、噪声掩蔽和CLAHE等预处理方法，从APTOS-2019眼底图像数据集中提取最相关的特征进行分类。

Result: 序数回归方法获得了0.8992的QWK分数，在APTOS数据集上创下了新的性能基准。

Conclusion: 该序数回归框架在糖尿病视网膜病变检测方面表现出色，为及时筛查和干预提供了有效的技术手段。
...
TL;DR: LAVA是一个新颖的领域增量学习框架，通过基于文本的参考锚点进行相对对齐，解决了传统方法中的领域间干扰和知识碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有领域增量学习方法面临两难困境：统一视觉空间导致领域间干扰和语义扭曲，而隔离域特定参数则造成知识碎片化。需要一种能保持跨领域知识一致性的方法。

Method: 提出LAVA框架，使用文本锚点驱动相对对齐，通过镜像类名之间的语义相似性来定义一致的相对几何结构，指导每个新领域的视觉表示保持几何一致性。

Result: 在标准DIL基准测试上的广泛实验表明，LAVA相比现有最先进方法实现了显著的性能提升。

Conclusion: LAVA通过语言锚定的视觉对齐有效解决了领域增量学习中的知识保持和跨领域知识重用问题，为DIL提供了新的解决方案。
...
TL;DR: 本文提出Cranio-ID框架，通过YOLO-pose模型自动标注2D头骨X射线扫描和光学图像中的标志点，并使用图表示和跨模态匹配技术实现头骨到面部的识别。
...
Method: 使用训练好的YOLO-pose模型自动标注2D头骨X射线扫描和光学图像中的标志点，然后将这些标志点转化为图表示，使用交叉注意力和最优传输框架进行跨模态图匹配。

Result: 在S2F和CUHK数据集上的广泛实验表明，该框架在可靠性和准确性方面都有显著提升，在法医学中的头骨到面部和素描到面部匹配中表现出有效性。

Conclusion: Cranio-ID框架提供了一种可靠且准确的跨模态颅骨标志点匹配方法，在法医颅面识别中具有重要应用价值。
...
TL;DR: 该研究通过模拟婴儿视觉发育的阶段性特征（灰度到彩色、模糊到清晰、保持时间连续性）训练自监督学习模型，发现这种'视觉饮食'策略能增强模型在物体识别和深度感知方面的鲁棒性，并表现出与生物发育相似的模式。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿视觉发育的阶段性特征（低清晰度、颜色退化但时间连续）是否能为机器视觉系统提供生态优势，理解早期视觉经验如何促进鲁棒视觉智能的涌现。

Method: 提出CATDiet方法，在物体中心视频上训练自监督学习模型，模拟婴儿视觉的三个约束：灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)。建立包含10个数据集的综合基准进行评估。

Result: 所有CATDiet变体在物体识别方面表现出增强的鲁棒性，模型展现出与生物发育一致的模式，包括类似猕猴V1区突触密度的神经可塑性变化和类似婴儿视觉悬崖反应的行为。CombDiet方法进一步提升了性能。

Conclusion: 早期婴儿视觉经验的发育进程为理解机器视觉智能的涌现提供了强大的逆向工程框架，这种阶段性'视觉饮食'策略有助于构建更鲁棒的视觉系统。
...
### [85] [DIR-TIR: Dialog-Iterative Refinement for Text-to-Image Retrieval](https://arxiv.org/abs/2511.14449)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统单查询文本到图像检索方法在可控性和容错性方面的不足，通过交互式对话提供更精确的图像搜索能力。

Method: 提出DIR-TIR框架，包含两个核心模块：对话精炼模块主动询问用户提取关键信息并生成更精确的描述；图像精炼模块识别生成图像与用户意图之间的感知差距，减少视觉语义差异。

Result: 在多个图像数据集上的综合实验表明，该对话式方法显著优于仅使用初始描述的基线方法，模块协同集成实现了更高的检索精度和更好的交互体验。

Conclusion: DIR-TIR框架通过多轮对话交互显著提升了文本到图像检索的精度和用户体验，证明了对话式方法在图像检索任务中的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 解决小农户农场边界划分的挑战，特别是在高分辨率图像可用性有限的情况下，需要结合高分辨率参考图像和低分辨率图像进行更频繁的监测。

Method: 使用条件潜在扩散模型和大规模多光谱、多源地理空间基础模型的组合，在分割感知的潜在空间中执行超分辨率，而不是传统的像素空间超分辨率。

Result: 在两个大型真实数据集上的实验表明，相对于基于最先进参考超分辨率方法的方法，在实例分割和语义分割指标上分别实现了25.5%和12.9%的相对改进。

Conclusion: SEED-SR方法通过绕过像素空间的显式超分辨率任务，在分割感知的潜在空间中执行超分辨率，能够以前所未有的20倍尺度因子生成分割图，显著提升了农场边界划分的性能。
...
TL;DR: 本文提出了一种名为Risk Semantic Distillation (RSD)的新框架，利用视觉语言模型增强端到端自动驾驶系统的训练，通过风险注意力机制解决泛化问题。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统在复杂场景中表现出色，但泛化能力仍是关键限制。现有方法要么导致混合系统的不一致问题，要么计算成本过高。

Method: 引入RiskHead插件模块，从视觉语言模型中提取因果风险估计到鸟瞰图特征中，生成可解释的风险注意力图，使BEV特征学习更丰富的风险注意力表示。

Result: 在Bench2Drive基准测试中，RSD在复杂和不可预测的驾驶条件下表现出有效性，感知和规划能力均有显著提升。

Conclusion: RSD框架通过风险注意力机制增强了BEV表示，更好地模拟了人类驾驶行为，在动态复杂环境中具有更好的导航能力。
...
TL;DR: 提出了一种基于解码器的参数感知Mamba模型(PAMM)，利用状态空间模型的丰富可扩展参数来增强多任务密集预测中的任务互连性，通过双状态空间参数专家集成任务特定参数先验。
...
Method: PAMM采用双状态空间参数专家集成任务特定参数先验，利用结构化状态空间序列模型(S4)进行全局任务先验整合，并使用多方向希尔伯特扫描方法构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上的广泛实验证明了该方法的有效性。

Conclusion: PAMM框架能够有效促进精确的多任务交互，并通过状态空间模型增强任务互连性，在多任务密集预测中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: LDCT图像质量因低剂量辐射而下降，现有方法常过度平滑导致关键细节丢失，需要一种能增强感知相关特征的方法来辅助放射科医生诊断。

Method: 提出D-PerceptCT架构，包含视觉双路径提取器（集成DINOv2语义先验）和全局-局部状态空间块，并设计基于人类对比敏感度的深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，D-PerceptCT相比SOTA方法能更好地保留LDCT图像的结构和纹理信息。

Conclusion: D-PerceptCT通过模拟人类视觉系统原理，有效增强了LDCT图像质量，为放射科医生提供了感知可见的关键解剖结构和病理细节。
...
TL;DR: 提出DeCo-VAE方法，通过将视频内容解耦为关键帧、运动和残差三个组件，分别学习专用潜在表示，实现紧凑的视频表示。


<details>
  <summary>Details</summary>
Motivation: 现有视频变分自编码器通常忽略帧间内容的相似性，导致潜在建模冗余，需要更紧凑的潜在表示方法。

Method: 将视频内容解耦为关键帧、运动和残差三个组件，为每个组件设计专用编码器，使用共享3D解码器保持时空一致性，并采用解耦适应策略进行稳定训练。

Result: 大量定量和定性实验表明，DeCo-VAE在视频重建性能上表现优越。

Conclusion: 通过内容解耦和专用编码器设计，DeCo-VAE能够实现更紧凑和有效的视频潜在表示。
...
TL;DR: 提出一种在共享空间中表示多个神经符号距离函数(SDF)的方法，通过结合泛化学习和过拟合学习策略，用更紧凑的潜表示恢复高保真几何细节。


<details>
  <summary>Details</summary>
Motivation: 现有的神经SDF在单个形状或场景上表现良好，但在分析多个SDF时存在困难，因为潜空间编码信息有限且会丢失几何细节。

Method: 结合泛化学习和过拟合学习策略的优势，并引入新的采样策略来采样训练查询，提高训练效率并消除其他SDF影响造成的伪影。

Result: 在广泛使用的基准测试上进行数值和视觉评估，验证了设计有效性，在表示能力和紧凑性方面优于最新方法。

Conclusion: 该方法成功克服了多SDF分析中的障碍，能够用更紧凑的潜表示恢复更多高保真几何细节。
...
TL;DR: 本文提出了一种无需物体先验知识的手-物体交互场景几何与外观建模方法，基于动态3D高斯泼溅技术，解决了交互过程中的相互遮挡、边缘模糊等挑战。


<details>
  <summary>Details</summary>
Motivation: 当前方法在手-物体交互场景建模中存在困难，特别是在没有物体先验知识的情况下，难以处理复杂的相互遮挡、边缘模糊和动态变形问题。

Method: 提出了交互感知的手-物体高斯模型，引入可优化参数实现分段线性假设；将手部信息融入物体变形场，构建交互感知动态场；采用渐进式优化策略处理动态区域和静态背景；设计了显式正则化来稳定表示。

Result: 实验表明，该方法超越了现有的基于动态3D-GS的方法，在手-物体交互动态重建方面达到了最先进的性能。

Conclusion: 该方法成功解决了无先验知识下的手-物体交互场景建模问题，通过交互感知的高斯表示和动态场设计，实现了高质量的几何和外观重建。
...
TL;DR: 本文提出了ForensicFlow，一个三模态取证框架，通过融合RGB、纹理和频率证据来检测视频Deepfake，在Celeb-DF数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 先进GAN和自编码器生成的Deepfake严重威胁信息完整性和社会稳定。单流CNN无法捕捉跨空间、纹理和频率域的多尺度伪造伪影，限制了鲁棒性和泛化能力。

Method: 提出三模态取证框架：RGB分支提取全局视觉不一致性；纹理分支检测细粒度融合伪影；频率分支识别周期性频谱噪声。采用注意力机制进行时间池化和自适应融合。

Result: 在Celeb-DF数据集上，AUC达到0.9752，F1分数0.9408，准确率0.9208，优于单流基线方法。消融实验验证了分支协同作用。

Conclusion: 这种全面的特征融合方法对细微伪造具有优越的鲁棒性，为Deepfake检测提供了有效的解决方案。
...
Method: 首先识别关键音频token，计算音频保留分数捕获信息密度，动态指导视频token剪枝，采用交错时空方案压缩视频token。

Result: 实现3.42倍推理加速和1.4倍内存减少，同时保持性能，无需训练。

Conclusion: OmniZip有效解决了音视频token压缩问题，显著提升推理效率，是多模态理解的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 骨健康不良是一个重要的公共卫生问题，低骨密度会增加骨折风险，这是骨质疏松症的关键特征。需要开发能够从可获取的医学数据中准确预测骨密度的方法。

Method: 提出XAttn-BMD框架，使用新颖的双向交叉注意力机制动态整合图像和元数据特征，实现跨模态相互增强，并采用加权平滑L1损失函数处理骨密度不平衡问题。

Result: 在Hertfordshire队列研究数据上的实验表明，该模型在回归性能上优于基线模型，交叉注意力融合相比简单特征拼接减少了MSE 16.7%、MAE 6.03%，提高了R2分数16.4%。

Conclusion: 该研究证明了通过交叉注意力整合多模态数据在股骨颈骨密度估计中的有效性，模型在临床相关阈值下的二元分类表现显示了其在真实场景中的应用潜力。
...
TL;DR: HoloTea是一个3D感知的流匹配框架，通过从H&E染色图像中推断基因表达，并利用相邻切片信息来生成3D组织转录组学图谱。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么独立处理每个切片忽略3D结构，要么3D感知方法不可扩展且非生成式。需要一种能够生成准确3D组织转录组学图谱的可扩展方法。

Method: 使用共享特征空间检索相邻切片上的形态对应点，通过轻量级ControlNet融合跨切片上下文。引入结合学习ZINB先验和空间经验先验的3D一致先验，以及线性扩展的全局注意力块。
...
TL;DR: 本文提出了BioST-GCN模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据上表现良好但存在模拟-现实差距问题。


<details>
  <summary>Details</summary>
Motivation: 跌倒对老年人造成严重伤害，基于视觉的跌倒预测系统面临数据稀缺问题，需要开发更有效的预测模型。

Method: 提出双流Biomechanical Spatio-Temporal Graph Convolutional Network (BioST-GCN)，结合姿态和生物力学信息，使用跨注意力融合机制。

Result: 在模拟数据集上比基线模型提升5.32%和2.91% F1分数，但零样本泛化到未见受试者时性能降至35.9%，存在显著的模拟-现实差距。

Conclusion: 需要弥合模拟数据与现实数据之间的差距，通过个性化策略和隐私保护数据管道来开发对脆弱老年人群有效的跌倒预测系统。
...
TL;DR: 提出了一种名为\net{}的新方法，通过立体几何-纹理对齐和伪特征增强几何一致性来解决稀疏视图下高斯泼溅过拟合问题，在多个数据集上实现了最先进的表面重建和视图合成性能。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图输入时，现有基于扁平高斯基元的优化方法容易过拟合，导致表面重建质量下降和视图合成性能恶化。扁平高斯固有的各向异性在稀疏视图场景中加剧了过拟合问题。

Method: 1. 立体几何-纹理对齐：连接渲染质量和几何估计，共同提升表面重建和视图合成；2. 伪特征增强几何一致性：通过结合训练视图和未见视图强制执行多视图几何一致性，有效缓解稀疏监督导致的过拟合。

Result: 在DTU、BlendedMVS和Mip-NeRF360数据集上的大量实验表明，该方法实现了最先进的性能，能够重建更准确和详细的表面，同时保持高质量的新视角渲染。

Conclusion: 所提出的\net{}方法通过立体几何-纹理对齐和伪特征增强几何一致性，有效解决了稀疏视图下高斯泼溅的过拟合问题，显著提升了表面重建和视图合成的质量。
...
<details>
  <summary>Details</summary>
Motivation: 计算细胞学面临两个主要挑战：实例级标签不可靠且获取成本高，以及见证率极低。需要开发能够在这种困难条件下有效工作的预训练方法。
...
Result: 在公开骨髓细胞学数据集上，模拟见证率从10%到0.5%，SLAM-AGS在bag-level F1-Score和Top 400阳性细胞检索方面优于其他预训练方法，在低见证率下提升最大。

Conclusion: 解决梯度干扰能够实现稳定的预训练，并在下游任务中获得更好的性能，特别是在低见证率条件下。
...
TL;DR: 本文提出了一种无需训练的FreeSwim方法，利用预训练的视频扩散Transformer生成更高分辨率的视频，通过向内滑动窗口注意力机制和双路径管道确保视觉保真度和全局一致性。


<details>
  <summary>Details</summary>
Motivation: 现代基于Transformer的视频生成器中注意力机制的二次时间和内存复杂度使得超高清视频的端到端训练成本过高，因此需要一种无需额外训练即可生成更高分辨率视频的方法。

Method: 采用向内滑动窗口注意力机制和双路径管道，其中窗口注意力通过跨注意力覆盖策略得到全局感受野分支的语义指导，同时结合跨注意力缓存策略提高效率。

Result: 实验表明该方法能够以无训练范式高效生成具有精细视觉细节的超高清视频，在VBench上表现优于基于训练的方法，且具有竞争性或改进的效率。

Conclusion: FreeSwim方法成功实现了无需额外训练即可生成高质量超高清视频的目标，在保持视觉细节和全局一致性方面表现出色，同时具有较高的计算效率。
...
### [101] [A Neural Field-Based Approach for View Computation & Data Exploration in 3D Urban Environments](https://arxiv.org/abs/2511.14742)
*Stefan Cobeli,Kazi Shahrukh Omar,Rodrigo Valença,Nivan Ferreira,Fabio Miranda*

Main category: cs.CV

TL;DR: 提出了一种基于视图的3D城市数据探索方法，使用神经场构建隐式表示，支持快速查询和避免遮挡，用于城市分析任务如可见性评估、日照分析和视觉影响评估。


<details>
  <summary>Details</summary>
Motivation: 3D城市数据探索面临计算瓶颈和交互复杂性，几何复杂性导致高度遮挡和大量手动视角调整，使得大规模探索效率低下。

Method: 基于视图的方法，使用向量场编码环境视图，引入神经场构建3D环境的隐式表示，支持直接查询（视图评估指标计算）和逆向查询（避免遮挡和搜索匹配数据模式的视图）。

Result: 通过定量实验、真实案例研究和领域专家反馈验证，证明在寻找理想视角、分析建筑立面可见性和评估室外空间视野方面的有效性。
...
### [102] [Vision Large Language Models Are Good Noise Handlers in Engagement Analysis](https://arxiv.org/abs/2511.14749)
*Alexander Vedernikov,Puneet Kumar,Haoyu Chen,Tapio Seppänen,Xiaobai Li*

Main category: cs.CV

TL;DR: 提出一个利用视觉大语言模型（VLMs）来优化视频参与度识别中主观和噪声标签的框架，通过问卷提取行为线索、数据可靠性划分以及结合课程学习和软标签优化的训练策略，在多个基准数据集上超越了现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 视频参与度识别任务面临主观标签和噪声的限制，这些因素显著影响模型性能，需要开发能够处理标签不确定性的方法。

Method: 使用VLMs通过问卷提取行为线索，将数据划分为高可靠性和低可靠性子集，采用结合课程学习和软标签优化的训练策略，逐步引入模糊样本并调整监督以反映不确定性。

Result: 在EngageNet基准上（六个特征设置中的三个）实现了最高+1.21%的改进，在DREAMS和PAFE数据集上分别获得+0.22和+0.06的F1分数提升。

Conclusion: 通过VLMs处理标签主观性，结合课程学习和软标签优化策略，能够有效提升视频参与度识别性能，超越现有最佳方法。
...
### [103] [Co-Me: Confidence-Guided Token Merging for Visual Geometric Transformers](https://arxiv.org/abs/2511.14751)
*Yutian Chen,Yuheng Qiu,Ruogu Li,Ali Agha,Shayegan Omidshafiei,Jay Patrikar,Sebastian Scherer*

Main category: cs.CV

TL;DR: Co-Me是一种无需重新训练或微调的视觉几何变换器加速机制，通过轻量级置信度预测器对token进行不确定性排序，选择性合并低置信度token以减少计算量，同时保持空间覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 现有的基于相似性的合并或剪枝方法在视觉几何变换器中效果有限，需要一种能够可靠识别变换器关注区域的方法来加速推理而不损失性能。

Method: 提出置信度引导的token合并方法，训练轻量级置信度预测器来评估token的不确定性，然后选择性合并低置信度token，有效减少计算负担。

Result: 在VGGT和MapAnything上分别实现了11.3倍和7.2倍的加速效果，使视觉几何变换器能够适用于实时3D感知和重建任务。

Conclusion: Co-Me提供了一种高效且通用的加速方案，能够显著提升视觉几何变换器的推理速度，同时保持模型性能，适用于多视角和流式视觉几何变换器。
...
<details>
  <summary>Details</summary>
Motivation: 在UniGen基础上，通过增强模型架构和训练流程来加强图像理解和生成能力，同时解锁强大的图像编辑功能。

Method: 提出统一的强化学习策略，通过共享奖励模型共同改进图像生成和编辑；引入轻量级编辑指令对齐阶段来增强编辑指令理解能力。

Result: 在GenEval和ImgEdit基准测试中分别获得0.89和4.31的总体分数，超越了BAGEL等最先进模型，达到与GPT-Image-1等专有模型相当的性能。

Conclusion: UniGen-1.5在图像理解、生成和编辑方面展现出竞争优势，证明了所提出的统一强化学习策略和编辑指令对齐方法的有效性。
...
TL;DR: 本文评估了大型语言模型文本水印技术的鲁棒性，发现这些技术虽然能保持语义，但会改变写作风格，且容易受到对抗性攻击，特别是回译攻击。


<details>
  <summary>Details</summary>
Motivation: 为了解决LLM生成文本的潜在危害，研究者提出了水印技术，但现有技术存在影响文本质量和易受攻击的问题，阻碍了其广泛应用。

Method: 通过比较释义和回译（英语→其他语言→英语）攻击，评估多种水印技术的鲁棒性，并使用语言指标来评估水印文本在质量和写作风格上的保持能力。

Result: 水印技术能够保持语义，但会偏离未加水印文本的写作风格，并且容易受到对抗性攻击，特别是回译攻击。

Conclusion: 当前的水印技术在保护文本语义方面有效，但在保持写作风格和抵抗对抗性攻击方面存在不足，需要进一步改进以促进其广泛应用。
...
<details>
  <summary>Details</summary>
Motivation: 提升文本嵌入模型的语义推理能力，激活预训练期间学习到的语义推理能力。

Method: 通过多次运行文本嵌入模型的前向传递来获得最终的语义表示，这是一种测试时推理方法。

Result: 在BRIGHT和PJBenchmark1等语义推理任务上取得显著改进，在C-MTEB等通用语义理解任务上保持一致的性能。
...
TL;DR: 本文提出了两种无需训练的QE-informed方法用于机器翻译错误修正：一种是选择多个LLM生成候选中质量最高的翻译，另一种是基于QE解释替换错误子串。第一种方法在WMT 2025任务中获胜。


<details>
  <summary>Details</summary>
Motivation: 虽然联合训练QE和APE系统能提升性能，但APE系统存在过度修正问题。本文研究无需训练的方法来避免这个问题。

Method: 提出两种无需训练的方法：1) QE-informed重翻译：从不同LLM生成的多个候选中选择质量最高的翻译；2) 基于QE解释替换错误子串，使用条件启发式最小化编辑次数。

Result: 两种方法的Delta COMET得分分别为0.0201和-0.0108，第一种方法在子任务排行榜上获得优胜位置。

Conclusion: 无需训练的QE-informed重翻译方法在机器翻译错误修正任务中表现优于基于QE解释的替换方法，证明了选择高质量候选翻译的有效性。
...
TL;DR: 论文研究了LLM在长文本中检索控制变量的能力，提出了GM-Extract基准数据集和双指标评估系统，评估了7-8B参数模型在多文档任务中的表现，并分析了缓解方法的效果。
...
Method: 引入GM-Extract基准数据集，使用文档指标和变量提取指标双指标评估系统，系统评估7-8B参数模型在键值提取和问答任务中的表现，并进行缓解方法的文献调研和应用测试。

Result: 发现仅通过改变数据在上下文窗口中的表示方式就能显著影响检索性能，虽然未始终观察到明显的U形曲线，但发现了清晰的性能模式，并与困惑度得分相关。缓解方法的效果高度复杂，在某些情况下能改善性能，但在其他情况下会产生负面影响。

Conclusion: 在真实应用场景中，LLM的长文本检索能力存在显著挑战，缓解策略的效果具有高度情境依赖性，需要在实际应用环境中谨慎评估和使用。
...
TL;DR: 该论文提出了一种通过LLM解析电商搜索查询中最高级表达（如"最好"、"最流行"）潜在意图的框架，通过提取结构化提示来改进搜索性能，并开发了将解释转移到轻量级模型的高效方法。


<details>
  <summary>Details</summary>
Motivation: 电商搜索查询中的最高级表达需要跨多个维度比较候选商品，这需要语言理解和领域知识。现有方法难以有效解析这些复杂语义意图。

Method: 开发了一个框架，将查询分解为属性-值提示，这些提示与检索过程同时生成，从而能够高效集成到排序管道中。还开发了将最高级解释转移到轻量级模型的高效方法。

Result: 该方法在MAP上提高了10.9个点，在MRR上提高了5.9个点，优于基线方法。同时解决了直接基于LLM的重排序面临的高延迟问题。
...
TL;DR: 提出HiEAG框架，通过分层证据增强生成来改进多模态虚假信息检测中的外部一致性检查，整合了检索、重排序和重写模块，在多个基准数据集上超越了现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 现有OOC虚假信息检测方法主要关注内部一致性，忽视了图像-文本对与外部证据之间的外部一致性，这限制了检测性能的提升。

Method: 提出分层证据增强生成框架HiEAG，包含证据检索、重排序（使用AESP自动选择相关证据）和重写（使用AEGP改进任务适应性）三个模块，利用多模态大语言模型的知识进行外部一致性检查。

Result: 在多个基准数据集上的实验结果表明，HiEAG在所有样本的准确率上超越了之前的最优方法，并能提供判断解释。

Conclusion: HiEAG框架通过分层证据增强生成有效提升了多模态虚假信息检测的外部一致性检查能力，证明了整合检索、重排序和重写模块的重要性。
...
TL;DR: 本文构建了一个平衡的多标签情感数据集，结合了GoEmotions、Sentiment140和GPT-4生成的数据，并开发了一个结合FastText嵌入、CNN、BiLSTM和注意力机制的多标签分类模型，显著提升了情感分类性能。
...
Method: 1）构建平衡数据集：整合GoEmotions原始数据、使用RoBERTa-base-GoEmotions模型标注的Sentiment140样本，以及GPT-4 mini生成的人工标注文本；2）开发增强模型：结合预训练FastText嵌入、卷积层（局部特征提取）、双向LSTM（上下文学习）、注意力机制（突出情感相关词），使用sigmoid激活输出层进行多标签预测，并采用混合精度训练提高计算效率。

Result: 实验结果显示，与在不平衡数据上训练的模型相比，我们的方法在准确率、精确率、召回率、F1分数和AUC方面均有显著提升。

Conclusion: 我们的方法通过数据平衡策略和增强的多标签分类模型，有效解决了多标签情感分类中的类别不平衡问题，显著提升了模型性能。
...
TL;DR: 本文提出了一种名为Stealth Fine-Tuning的新型攻击方法，通过段级干扰和自生成输出作为监督微调数据，能够有效绕过推理增强视觉语言模型的安全对齐机制。
...
Method: 采用段级干扰引出有害推理轨迹，将自生成输出作为监督微调数据，通过基于轮次的加权损失设计，实现轻量级、分布一致的微调方法。

Result: 仅使用499个样本和单张A100显卡不到3小时，Stealth Fine-Tuning在攻击成功率上比IDEATOR高出38.52%，同时保持通用推理能力，模型保持原始表示分布。

Conclusion: Stealth Fine-Tuning是一种低成本、高效的方法，能够绕过对齐防御，在AdvBench和多个通用基准测试中表现出色。
...
TL;DR: 提出基于数据中心的框架，通过生成高质量合成出院摘要来缓解ICD编码的长尾分布问题，提升罕见和零样本代码的预测性能。


<details>
  <summary>Details</summary>
Motivation: 临床文本自动ICD编码面临极端长尾分布问题，数千个罕见和零样本ICD代码在数据集（如MIMIC-III）中代表性不足，导致宏观F1分数较低。
...
Conclusion: 尽管相对于计算成本增益可能显得有限，但结果表明精心设计的合成数据可以增强长尾ICD代码预测的公平性。
...
TL;DR: 该研究将基于Transformer的关系提取与知识图谱匹配相结合，用于回答多项选择题，同时保持输出过程的可追溯性。通过动态生成知识图谱来验证问题句子的真实性，在实验中达到了约70%的正确率。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建成本高且被视为静态数据库，而基于Transformer的关系提取方法能够从自然语言文本动态生成知识图谱，这为表示输入句子的含义提供了可能性。研究旨在利用这一效果来回答多项选择题，并解决关系提取方法在提供事实错误文本时生成错误信息的问题。

Method: 提出了一种回答填空式多项选择题的方法：首先使用关系提取方法将问题句子转换为关系图，然后在封闭世界假设下将其与事实正确的知识图谱进行验证，以衡量每个问题句子的真实性。

Result: 实验结果表明，该方法能够正确回答约70%的问题，同时提供程序的可追溯性。研究还发现问题类别对准确率有显著影响。

Conclusion: 结合Transformer关系提取和知识图谱匹配的方法能够有效回答多项选择题，保持过程可追溯性，但准确率受问题类别影响较大，表明该方法在不同类型问题上的适用性存在差异。
...
<details>
  <summary>Details</summary>
Motivation: 解决超人类模型只能获得弱监督的问题，现有弱到强泛化方法中部分弱标签对模型有害，需要避免不必要地使用弱监督。

Method: 训练二元分类器P(IK)识别强模型能回答的问题，使用自生成标签进行对齐，并通过图平滑方法优化弱标签。

Result: 在三个基准测试上的广泛实验表明，该方法持续优于竞争基线方法。

Conclusion: 选择性弱到强泛化可以帮助实现超对齐，P(IK)分类器能够跨任务和难度泛化。
...
TL;DR: 提出了首个基于符号语言知识的幻觉定位框架，通过分析符号触发词（如修饰语、否定、数字等）在模型各层的处理过程，发现幻觉源于早期层的符号语义处理崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能理解符号幻觉的来源，缺乏专门针对符号语言元素驱动幻觉的定位框架，需要系统性地追踪符号触发词在模型各层的发展过程。

Method: 提出符号定位框架，利用符号语言和语义知识，在HaluEval和TruthfulQA数据集上分析五个模型，重点关注符号触发词的处理过程。

Result: 发现符号语言元素的注意力方差在早期层（2-4层）爆炸性增长，否定触发灾难性方差水平；尽管模型规模增大，幻觉率仍高达78.3%-83.7%，深层符号语义触发词注意力急剧下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将大语言模型应用于实体链接的孤立阶段，未能充分利用其在整个过程中的能力，且孤立消歧不足以获得最优性能。

Method: 提出DeepEL框架，将大语言模型集成到实体链接的每个阶段，并引入自验证机制利用全局上下文信息，使大语言模型能够纠正自身预测并识别同一句子中实体间的连贯关系。

Result: 在10个基准数据集上的广泛评估显示，DeepEL显著优于现有最先进方法，整体F1分数平均提升2.6%，在域外数据集上提升4%。

Conclusion: 深度集成大语言模型能够有效推进实体链接技术的前沿发展。
...
TL;DR: 本文提出了ArbESC+系统，这是首个阿拉伯语语法错误校正的多系统方法，通过组合多个模型生成校正建议，使用分类器选择最佳校正，在QALB数据集上取得了优于单个模型的结果。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语具有复杂的形态和句法结构，现有神经模型大多使用单一模型，没有充分利用多系统组合的潜在优势。

Method: 使用AraT5、ByT5、mT5、AraBART等多个模型收集校正建议，将其表示为数值特征，通过分类器选择最佳校正，并采用支持技术过滤重叠校正和评估决策可靠性。

Result: 在QALB-14测试数据上F0.5达到82.63%，在QALB-15 L1数据上达到84.64%，在QALB-15 L2数据上达到65.55%，优于单一模型。

Conclusion: 这是首个整合语言错误校正的阿拉伯语尝试，为开发先进的阿拉伯语文本处理工具提供了实用步骤。
...
TL;DR: 本文针对大语言模型在音乐专业领域的局限性，构建了大规模音乐相关自然语言语料库，并提出了基于参考模型的软评分质量控制和动态优化方法，以提升音乐领域持续预训练和对齐的效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现良好，但在音乐等专业领域仍受限于语料库规模、纯净度以及数据与训练目标的匹配度。

Method: 构建40B token的音乐相关自然语言语料库，采用领域优先的数据管道，包括轻量级分类器过滤、多阶段清理、去重和隐私保护掩码，并引入基于参考模型的token级软评分进行质量控制。

Result: 开发了MusicSimpleQA基准测试，通过系统比较数据组成，验证了所提方法在音乐领域事实性评估上的有效性。

Conclusion: 本研究通过正确的语料库和训练目标，为音乐领域构建领域特定大语言模型提供了可扩展的数据训练框架和可重用的评估工具。
...
TL;DR: 本文提出Authentic-Dubber方法，通过模拟导演-演员互动的工作流程，实现更真实的电影配音。该方法包含三个核心机制：构建多模态参考素材库、基于情感相似性的检索增强策略、以及渐进式图基语音生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有电影配音方法模拟简化的演员直接配音工作流程，忽略了导演与演员之间的关键互动。真实的配音工作流程涉及导演指导演员内化情感等上下文线索的动态协作。

Method: 提出检索增强的导演-演员交互学习方案，包含：(1)构建多模态参考素材库，利用LLM深度理解多模态信号的情感表示；(2)基于情感相似性的检索增强策略，检索与目标静音视频对齐的最相关多模态信息；(3)渐进式图基语音生成方法，逐步整合检索到的多模态情感知识。

Result: 在V2C Animation基准数据集上的主观和客观评估验证了方法的有效性，在情感表现力方面实现了全面改进。

Conclusion: Authentic-Dubber能够忠实地复制真实的配音工作流程，显著提升了配音的情感表现力。
...
Result: 开源ASR在自发语音中表现良好但在嘈杂非母语对话中退化；多模态LLM对口音更鲁棒但处理领域特定命名实体困难；专有模型在清晰语音中准确率高但随国家和领域变化显著；非洲英语微调模型在延迟方面具有部署优势，但幻觉问题仍是主要挑战。
...
TL;DR: 本文提出了一种熵引导的训练框架来解决大型推理模型中的熵冲突问题，成功将推理链长度压缩至原始的20%，同时保持或提升了基准准确率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但其思维链输出过长导致高计算成本和部署困难。现有压缩方法部分成功但忽略了训练过程中的熵冲突现象：压缩训练降低熵值导致推理变短但探索受限，而准确性目标增加熵值使推理链变长，使模型陷入局部困境。

Method: 采用熵引导的训练框架：当熵值下降时，引导模型通过鼓励简洁思维步骤实现高效推理；当熵值上升时，在紧凑推理模式下强化探索以提高鲁棒性。

Result: 在六个数学基准测试上的实验表明，该方法将推理长度压缩至原始的20%，同时保持甚至超越了基准准确率。

Conclusion: 提出的熵引导训练框架有效解决了推理模型中的熵冲突问题，实现了推理长度的大幅压缩而不损失准确性，为实际部署提供了可行方案。
...
TL;DR: 本文提出通过预测语言化概率分布来增强置信度估计的方法，该方法要求LLM考虑答案空间中的所有候选答案而非单一猜测，并仔细分配置信度分数以满足分布要求。


<details>
  <summary>Details</summary>
Motivation: 了解模型响应的可靠性在应用中至关重要。虽然已有研究关注生成语言化置信度并结合思维链推理来提供逻辑透明的估计，但推理策略如何影响估计置信度仍待深入探索。
...
Conclusion: 预测语言化概率分布能有效促进置信度估计的深度推理，在各种条件下都表现出稳健性能。
...
TL;DR: AraLingBench是一个全面人工标注的阿拉伯语语言能力评估基准，包含5个核心类别150道专家设计的选择题，评估35个阿拉伯语和双语大语言模型，发现当前模型在深层语法和句法推理方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估知识而非语言能力，需要专门评估阿拉伯语语言理解的结构性基准来弥补这一差距。

Method: 构建包含语法、形态学、拼写、阅读理解和句法5个类别的150道多选题基准，对35个阿拉伯语和双语LLMs进行全面评估。

Result: 当前模型表现出较强的表层熟练度，但在深层语法和句法推理方面存在困难，许多模型通过记忆或模式识别而非真实理解获得高分。

Conclusion: AraLingBench揭示了基于知识的基准高分与真正语言掌握之间的持续差距，为开发阿拉伯语LLMs提供了诊断框架。
...
TL;DR: ConInstruct是一个专门评估大语言模型检测和解决用户指令中冲突能力的基准，研究发现大多数专有LLM具有强大的冲突检测能力，但很少明确通知用户冲突或请求澄清。
...
Method: 开发了包含7个核心科学领域的原创问题集，采用多阶段专家评审和对抗性测试确保质量，并提出了基于LLM评委的自动化评估范式。

Result: 初步结果显示ATLAS能有效区分领先模型的先进科学推理能力。

Conclusion: ATLAS将发展成为一个长期、开放、社区驱动的平台，为人工通用智能的发展提供可靠评估标准。
...
TL;DR: 论文提出标准化上下文校准(NCC)方法，解决大语言模型中多标记类别标签的长度偏差问题，显著提升零样本和少样本学习性能。
...
Method: 提出标准化上下文校准(NCC)方法，在全标签级别进行标准化和校准，扩展偏差缓解到更广泛任务如多项选择题回答。

Result: NCC在多个数据集和模型上相比先前方法获得统计显著改进，F1分数提升高达10%，结合上下文学习时对少样本示例选择更不敏感，需要更少示例获得竞争性能。
...
TL;DR: 本文提出了EduHarm基准和TSSF三阶段防护框架，用于解决教育场景中LLM的安全漏洞问题，同时防御越狱攻击和微调攻击。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育应用中面临越狱和微调攻击的安全威胁，现有研究主要关注通用安全评估，缺乏针对教育场景特殊安全需求的研究。

Method: 构建EduHarm基准包含五个教育场景的安全-不安全指令对；提出三阶段防护框架：安全感知注意力重对齐、分层安全判断、防御驱动双路由机制。

Result: 在八种越狱攻击策略上的实验表明TSSF能有效增强安全性且避免过度拒绝良性查询；在三个微调攻击数据集上的评估显示其能持续实现稳健防御同时保持良性微调的效用增益。

Conclusion: TSSF框架能够同时缓解越狱和微调攻击，为教育LLM提供有效的安全保障，在保持模型实用性的同时增强安全防护能力。
...
TL;DR: MedBench v4是一个全国性的云基准测试平台，包含70多万个专家策划的医疗任务，涵盖24个主要专科和91个次要专科，专门评估LLM、多模态模型和智能体。评估显示基础LLM平均得分54.1/100，多模态模型47.5/100，但基于相同骨干的智能体显著提升至79.8/100，特别是在安全任务上表现突出。


<details>
  <summary>Details</summary>
Motivation: 随着医疗大语言模型、多模态模型和智能体的快速发展，需要能够反映真实临床工作流程和安全约束的评估框架。现有基准测试无法充分评估这些模型在复杂医疗环境中的实际表现。

Method: 构建包含700,000+专家策划任务的云基准测试基础设施，任务经过多阶段细化和多轮临床医生评审。开放式回答使用经过人类评分校准的LLM-as-a-judge进行评分，涵盖LLM、多模态模型和智能体三个专门赛道。

Result: 评估15个前沿模型：基础LLM平均得分54.1/100（最佳Claude Sonnet 4.5为62.5/100），但安全和伦理得分较低（18.4/100）；多模态模型平均47.5/100（最佳GPT-5为54.9/100），感知能力良好但跨模态推理较弱；基于相同骨干的智能体显著提升性能至79.8/100，Claude Sonnet 4.5智能体在安全任务上达88.9/100。

Conclusion: MedBench v4揭示了基础模型在多模态推理和安全性方面仍存在差距，但治理意识的智能体编排能够显著提升临床准备度而不牺牲能力。该平台通过与中国临床指南和监管重点对齐，为医院、开发者和政策制定者提供了实用的医疗AI审计参考。
...
TL;DR: Tell Me是一个基于大语言模型的心理健康系统，包含个性化对话助手、合成治疗对话生成器和AI健康规划团队，旨在提供可访问的心理健康支持。


<details>
  <summary>Details</summary>
Motivation: 解决心理健康资源获取障碍，为研究提供合成治疗数据，并通过AI技术补充现有护理服务。

Method: 系统集成三个组件：检索增强生成对话助手、基于用户档案的合成治疗对话生成器、以及使用CrewAI实现的健康AI团队，用于生成个性化自我护理计划。

Result: 系统在精心设计的健康场景中进行了评估，使用了自动LLM判断和人类用户研究，展示了其在提供个性化心理健康支持方面的有效性。

Conclusion: 这项工作强调了NLP研究人员与心理健康专业人士之间跨学科合作的重要性，以推动人类-AI互动在健康领域的负责任创新。
...
<details>
  <summary>Details</summary>
Motivation: 当前RL在LLM智能体训练中的应用仍处于早期阶段，缺乏专门针对LLM智能体定制的RL方法以及灵活可扩展的训练框架。

Method: 1. 系统化扩展MDP框架来定义LLM智能体的关键组件；2. 提出Agent-R1模块化训练框架，支持跨任务场景和环境适配。

Result: 在多跳问答基准任务上的实验为所提方法和框架的有效性提供了初步验证。

Conclusion: 该研究为RL在LLM智能体训练中的应用提供了系统化方法和实用框架，有助于推动该领域的发展。
...
Method: 构建包含895个合成问答对的公开数据集，基于SIGIR'2025 LiveRAG挑战赛，添加了真实答案、支持证据，并应用项目反应理论模型计算难度和区分度评分。
...
TL;DR: 本文探讨文档级声明提取的评估方法，通过对齐声明集并计算相似度来评估模型提取性能，在捷克和斯洛伐克新闻评论数据集上实验发现现有评估方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 文档级声明提取在事实核查领域仍具挑战性，现有评估方法关注有限，需要可靠框架来比较模型提取与人工标注的声明集。

Method: 研究声明集对齐技术，通过最佳对齐和评估方法计算相似度得分，作为模型提取性能评估指标和标注者间一致性度量。

Result: 在捷克和斯洛伐克新闻评论数据集上的实验揭示了当前评估方法在文档级声明提取中的局限性，特别是在处理非正式语言和本地语境时。

Conclusion: 需要更先进的评估方法，能够正确捕捉语义相似性并评估声明的关键属性如原子性、可核查性和去语境化。
...
TL;DR: 本研究使用电子健康记录数据动态追踪急性肾损伤患者的临床演变，通过聚类分析识别了15种不同的AKI后临床状态，发现这些状态具有不同的慢性肾病进展风险，为早期识别高风险患者提供了数据驱动方法。


<details>
  <summary>Details</summary>
Motivation: 急性肾损伤患者发展为慢性肾病的风险很高，但识别最高风险患者仍然具有挑战性。需要开发数据驱动的方法来动态追踪AKI患者的临床演变过程。

Method: 使用电子健康记录数据，通过纵向医疗编码和肌酐测量值创建患者向量，聚类识别AKI后临床状态，使用多状态模型估计状态间转移概率和CKD进展概率，通过生存分析识别不同亚群中的CKD风险因素。

Result: 在20,699名入院时患有AKI的患者中，3,491名（17%）发展为CKD。识别出15种不同的AKI后状态，每种状态具有不同的CKD发展概率。75%的患者在研究期间保持单一状态或仅发生一次转移。确定了传统和新发现的CKD风险因素，其影响在不同临床状态中有所差异。
...
TL;DR: 本文比较了人类标注与多种大语言模型在检测新闻媒体政治偏见方面的表现，发现RoBERTa与人类标注最一致，GPT在零样本设置下与人类标注整体一致性最强。
...
Result: 在传统基于Transformer的模型中，RoBERTa与人类标签对齐度最高；生成模型如GPT在零样本设置下与人类标注整体一致性最强；微调后的RoBERTa模型在所有基线中获得了最高准确度和最强的人类标注对齐度。
...
TL;DR: 多智能体系统通过视觉语言模型作为评判者，将图表作为可验证检查点，在自主科学发现中实现实时错误纠正和探索性数据分析引导，在宇宙学和天体化学案例中表现出色。


<details>
  <summary>Details</summary>
Motivation: 提高端到端自主科学发现的效率和准确性，通过视觉语言模型评估图表来纠正错误并引导数据分析过程。

Method: 使用视觉语言模型作为评判者，将图表作为可验证检查点，通过动态生成的领域特定评分标准评估图表，使智能体能够实时纠正错误。

Result: 在10个数据驱动发现任务基准测试中，VLM增强系统达到0.7-0.8的通过率，显著优于仅代码（0.2-0.3）和代码加文本（0.4-0.5）的基线方法。

Conclusion: 视觉语言模型增强的多智能体系统能够有效提升自主科学发现的性能，提供可审计的推理轨迹，改善可解释性，且无需人工干预即可适应新数据集。
...
<details>
  <summary>Details</summary>
Motivation: 罕见病影响全球数亿人，但诊断往往需要数年时间。传统流程将噪声证据提取与下游推理诊断分离，通用/医学大语言模型面临真实世界电子健康记录稀缺、领域知识陈旧和幻觉问题。
...
TL;DR: 本研究通过定量建模验证了嘈杂通道理论在比较幻觉现象中的解释力，发现后验概率模型不仅能解释幻觉强度的细微差异，还能预测代词与完整名词短语主语对幻觉效应的影响。


<details>
  <summary>Details</summary>
Motivation: 比较幻觉现象中人们会系统性地误判不合逻辑的句子为可接受，先前研究用贝叶斯推理和嘈杂通道理论解释此现象，但缺乏定量预测能力。本研究旨在通过定量模型更深入地验证该理论。

Method: 将统计语言模型与人类行为数据相结合，构建了基于后验概率的定量模型，直接预测比较幻觉的强度，并检验代词与完整名词短语主语对幻觉效应的影响。

Result: 模型成功解释了比较幻觉效应的细微强度差异，并首次解释了代词与完整名词短语主语对幻觉效应的不同影响，支持了嘈杂通道理论的预测能力。

Conclusion: 研究结果支持嘈杂通道推理作为语言处理现象的统计算法级理论，该理论能够统一解释包括幻觉和非幻觉在内的多种语言处理现象。
...
TL;DR: 本文提出了一个理解NLP数据集中注释偏差的全面框架，区分了指令偏差、注释者偏差以及上下文和文化偏差，并综述了检测方法和缓解策略，旨在为LLM构建更公平和基于文化的注释流程。


<details>
  <summary>Details</summary>
Motivation: NLP数据集中的注释偏差是多语言大语言模型发展的主要挑战，特别是在文化多样性的环境中。来自任务框架、注释者主观性和文化不匹配的偏差会扭曲模型输出并加剧社会危害。

Method: 提出了一个区分指令偏差、注释者偏差以及上下文和文化偏差的注释偏差分类法；综述了检测方法（包括注释者间一致性、模型分歧和元数据分析）；并概述了主动和被动的缓解策略，包括多样化注释者招募、迭代指南细化和事后模型调整。

Result: 开发了一个注释偏差的类型学；综合了检测指标；提出了一个适用于多语言环境的基于集成的偏差缓解方法；并对注释过程进行了伦理分析。

Conclusion: 这些见解旨在为LLM提供更公平和基于文化的注释流程，从而减少模型输出中的偏差和社会危害。
...
TL;DR: 本文提出了一个模块化框架，通过检索增强生成(RAG)流程来简化合同管理，包括合成数据生成、语义条款检索、可接受性分类和基于奖励的对齐，以标记有问题的修订并生成改进方案。


<details>
  <summary>Details</summary>
Motivation: 合同管理涉及审查和协商条款，但自动化这一流程面临标注数据稀缺和非结构化遗留合同丰富的挑战。

Method: 采用检索增强生成(RAG)流程，整合合成数据生成、语义条款检索、可接受性分类和基于奖励的对齐。
...
TL;DR: 本文通过分析20部英文小说的词汇类型-词例关系，发现传统的幂律关系（Herdan定律）在双对数坐标下仍呈轻微凹形，提出用二次函数能完美拟合类型-词例数据，并通过随机抽样模型解释了这种曲率现象。


<details>
  <summary>Details</summary>
Motivation: 传统Herdan定律描述的词汇类型-词例关系在双对数坐标下应为直线，但实际观察发现仍存在轻微凹形，这挑战了幂律关系的有效性，需要更精确的数学模型来描述这种语言现象。

Method: 使用20部英文小说（部分为翻译作品）的文本数据，在双对数坐标下对类型-词例关系进行线性回归和二次回归分析，并采用"从袋中随机抽取彩色球并放回"的模型来解释曲率现象。

Result: 回归分析显示线性系数略大于1，二次系数约为-0.02，二次函数能完美拟合数据。曲率在随机抽样模型中表现为"伪方差"，当词例数较小时可提供曲率的粗略估计。

Conclusion: 词汇类型-词例关系在双对数坐标下更适合用二次函数而非线性函数描述，这为语言统计模型提供了更精确的数学框架，并通过随机抽样模型为曲率现象提供了理论解释。
...
TL;DR: 研究探索LLM嵌入是否能编码从科学测量中获得的天体物理统计量，分析提示词的作用和语言中编码物理信息的关键方面。


<details>
  <summary>Details</summary>
Motivation: 利用LLM在跨领域泛化和上下文学习能力，研究其能否编码通常仅通过科学测量获得且松散编码在文本描述中的物理信息。

Method: 使用天体物理学作为测试平台，通过稀疏自编码器从文本中提取可解释特征，分析提示词对物理量编码的影响。

Result: 研究发现提示词在LLM编码物理量时发挥作用，并识别出语言中编码物理信息的关键方面。

Conclusion: LLM嵌入能够在一定程度上编码科学测量中的物理统计量，提示词设计和语言特征提取对有效编码物理信息至关重要。
...
TL;DR: VALOR是一个用于多模态客户支持对话投诉分析的新框架，通过结合文本投诉和视觉证据进行细粒度分类，采用专家路由和语义对齐策略，在复杂投诉场景中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有投诉分析方法主要依赖单模态短文本，本工作旨在利用多模态、多轮客户支持对话，结合文本投诉和视觉证据，实现更细粒度的投诉方面和严重程度分类。
...
TL;DR: 比较库尔德语词嵌入的不同分词策略（词级、基于语素、BPE），发现BPE在形态相似性评估中存在偏差，而基于语素的分词在综合评估中表现更优。
...
Method: 开发基于BiLSTM-CRF的形态分割器，使用最小手动标注进行引导训练，比较词级、基于语素和BPE三种分词方法，通过相似性保持、聚类质量和语义组织等综合指标评估Word2Vec嵌入。

Result: BPE在形态相似性评估中看似更优，但仅评估了28.6%的测试用例，而语素模型评估了68.7%，存在人为性能膨胀。综合评估显示基于语素的分词在嵌入空间组织、语义邻域结构和形态复杂度覆盖方面表现更好。

Conclusion: 在低资源语言处理中，基于语素的分词策略优于BPE，强调了覆盖感知评估的重要性，并为低资源语言处理提供了不同的分词方法选择。
...
TL;DR: 本研究分析了大型语言模型在变革研发过程中的多重功能，通过自动化知识发现、促进假设创建、整合跨学科见解和促进创新生态系统合作，显著提升研究流程的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型如何通过多种功能来改进研发流程，提高创新效率和缩短产品上市时间。

Method: 通过分析科学文献、专利数据库和实验数据，利用大型语言模型实现更灵活和知情的研发工作流程。

Result: 大型语言模型能够显著提高研发过程的效率和效果，加速创新周期并降低突破性想法的上市时间。

Conclusion: 大型语言模型在研发过程中具有多重重要功能，能够通过自动化知识发现、促进假设生成和跨学科整合等方式，有效推动创新生态系统的发展。
...
### [146] [Imagine in Space: Exploring the Frontier of Spatial Intelligence and Reasoning Efficiency in Vision Language Models](https://arxiv.org/abs/2511.13782)
*Xiaoxing Lian,Aidong Yang,Jun Zhu,Peng Wang,Yue Zhang*

Main category: cs.AI

TL;DR: 本文介绍了SpatiaLite基准测试，用于评估视觉语言模型的空间推理能力，发现当前先进VLMs主要依赖语言表示进行空间推理，在需要视觉空间关系和3D几何变换的任务上表现不足，且推理效率低下。作者提出了Imagery Driven Framework来改进VLMs的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前先进视觉语言模型在逻辑推理、问题解决和决策制定方面表现出色，但空间推理（包括心理旋转、导航和空间关系理解）仍是重大挑战。作者假设想象力（空间状态的内部模拟）是空间世界模型中的主导推理机制。

Method: 引入SpatiaLite全合成基准测试，联合测量空间推理准确性和效率；提出Imagery Driven Framework（IDF）用于数据合成和训练，隐式构建内部世界模型。

Result: 实验发现：1）先进VLMs主要依赖语言表示，在视觉中心任务（如心理旋转、投影预测）上表现不足；2）VLMs空间推理机制效率低下，随着变换复杂度增加，token使用量快速增长；3）IDF框架能有效构建内部世界模型。

Conclusion: 本研究界定了先进VLMs的空间推理限制和模式，识别了关键缺陷，并为未来改进提供了方向。SpatiaLite基准测试为评估和提升VLMs空间推理能力提供了系统工具。
...
### [147] [Causal computations in Semi Markovian Structural Causal Models using divide and conquer](https://arxiv.org/abs/2511.13852)
*Anna Rodum Bjøru,Rafael Cabañas,Helge Langseth,Antonio Salmerón*

Main category: cs.AI

TL;DR: 本文研究将Bjøru等人提出的用于马尔可夫结构因果模型的因果概率边界计算方法扩展到半马尔可夫模型，其中外生变量可以影响多个内生变量，从而能够表示更复杂的混杂关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅适用于马尔可夫结构因果模型，限制了其在表示混杂关系方面的应用。半马尔可夫模型能够表示更复杂的因果关系，但扩展现有方法面临挑战。

Method: 通过最小示例说明扩展挑战，提出替代解决方案策略，并进行理论和计算评估。

Result: 开发了适用于半马尔可夫结构因果模型的因果概率边界计算方法，能够处理外生变量影响多个内生变量的情况。

Conclusion: 成功将因果概率边界计算方法扩展到半马尔可夫模型，为处理更复杂的混杂关系提供了有效工具。
...
### [148] [Jailbreaking Large Vision Language Models in Intelligent Transportation Systems](https://arxiv.org/abs/2511.13892)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在智能交通系统中具有广泛应用，但存在严重的安全漏洞，容易受到越狱攻击，可能导致模型生成有害内容，威胁系统安全。

Method: 构建交通相关有害查询数据集；提出基于图像排版操纵和多轮提示的新型越狱攻击；开发多层响应过滤防御技术；使用GPT-4和人工验证评估攻击和防御效果。

Result: 实验表明所提出的越狱攻击方法在开源和闭源LVLMs上均有效，能成功诱导模型生成有害响应；多层防御技术能有效阻止不当响应生成。

Conclusion: LVLMs在智能交通系统中存在严重安全风险，图像排版操纵和多轮提示的越狱攻击威胁巨大，需要加强安全防护措施。
...
### [149] [CORGI: Efficient Pattern Matching With Quadratic Guarantees](https://arxiv.org/abs/2511.13942)
*Daniel Weitekamp*
...
Method: 采用两步法：前向传递构建/维护基础关系图，后向迭代器按需生成匹配，不收集部分匹配到传统β内存中。

Result: 在性能评估中，CORGI在简单组合匹配任务上显著优于SOAR和OPS5的RETE实现。

Conclusion: CORGI算法通过消除填充完整冲突集导致的高延迟和内存溢出，使在线AI系统更加实用，无需手工工程约束即可避免不可预测的故障模式。
...
### [150] [Scene Graph-Guided Generative AI Framework for Synthesizing and Evaluating Industrial Hazard Scenarios](https://arxiv.org/abs/2511.13970)
...
<details>
  <summary>Details</summary>
Motivation: 训练视觉模型检测工作场所危险需要真实的危险场景图像，但获取此类数据集很困难，因为捕捉实际发生的事故触发场景几乎不可能。
...
TL;DR: Syn-STARTS是一个使用LLM生成大规模分类案例的框架，用于解决大规模伤亡事件中真实数据稀缺的问题，验证了合成数据在医疗AI开发中的潜力。


<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件中分类决策至关重要，但真实数据难以收集，限制了AI模型的开发和评估。

Method: 开发Syn-STARTS框架，利用LLM生成分类案例，并与手动整理的TRIAGE开放数据集进行定性比较。

Result: Syn-STARTS生成的案例在质量上与手动整理的数据集无法区分，且在标准START分类法的四个类别中表现出高度稳定性。
...
### [153] [Making Evidence Actionable in Adaptive Learning](https://arxiv.org/abs/2511.14052)
*Amirreza Mehrabi,Jason W. Morphew,Breejha Quezada,N. Sanjay Rebello*

Main category: cs.AI

TL;DR: 该研究提出了一个由教师主导的反馈循环系统，将概念级评估证据转化为经过验证的微干预措施，通过三种保障机制（充分性、注意力预算和多样性）实现精准的个性化学习干预。
...
Method: 将干预分配形式化为带约束的二元整数规划，包含覆盖率、时间、难度窗口、概念矩阵先决条件和反冗余多样性约束。采用贪婪选择、基于梯度的松弛方法和混合方法三种求解策略。

Result: 在模拟和1204名学生的物理课程部署中，两种求解器都能在有限观看时间内为几乎所有学习者实现完整的技能覆盖。基于梯度的方法比贪婪方法减少约12%的冗余覆盖，并在难度上更均衡，而贪婪方法在资源稀缺环境下计算成本更低。
...
TL;DR: APD-agents是一个基于大语言模型的多智能体框架，用于自动化移动应用页面设计，通过多个智能体协作将用户描述转换为页面布局。


<details>
  <summary>Details</summary>
Motivation: 移动应用页面布局设计耗时且需要专业技能，现有设计软件使用复杂且跨页面协作效率低，需要自动化解决方案。

Method: 提出多智能体框架，包含协调器、语义解析器、主布局生成器、模板检索器和递归组件生成器，通过智能体协作完成页面设计任务。
...
### [155] [Run, Ruminate, and Regulate: A Dual-process Thinking System for Vision-and-Language Navigation](https://arxiv.org/abs/2511.14131)
...
TL;DR: 本文提出R3框架，通过双过程思维整合LLM的泛化能力和VLN专家知识，在零样本条件下显著提升视觉语言导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在视觉语言导航任务中存在空间理解不足、计算成本高的问题，需要结合领域专业知识来弥补性能差距。

Method: 提出R3双过程思维框架，包含Runner（轻量专家模型）、Ruminator（多模态LLM推理器）和Regulator（模式控制器）三个核心模块。

Result: 在REVERIE基准测试中，SPL和RGSPL分别超过现有最佳方法3.28%和3.30%。

Conclusion: R3框架通过有效整合LLM泛化能力和领域专业知识，能够显著提升复杂VLN任务的完成性能。
...
### [156] [Do Large Language Models (LLMs) Understand Chronology?](https://arxiv.org/abs/2511.14214)
*Pattaraphon Kenny Wongchamcharoen,Paul Glasserman*

Main category: cs.AI

TL;DR: 本文测试了大语言模型在金融应用中理解时间顺序的能力，发现随着任务复杂度增加，模型表现下降，但增加推理预算能显著提升GPT-5和Claude-3.7 Sonnet的时间排序性能。


<details>
  <summary>Details</summary>
Motivation: 在金融和经济学中，大语言模型被广泛使用，但模型是否真正理解时间顺序这一基本问题尚未得到充分验证。本文旨在测试LLMs在时间顺序任务中的表现，特别是在防止前瞻性偏差方面的能力。

Method: 设计了一系列时间顺序任务，包括：(1)时间顺序排序；(2)条件排序（先过滤后排序）；(3)时代错误检测。评估了GPT-4.1、Claude-3.7 Sonnet（有/无扩展思维）和GPT-5在不同推理努力设置下的表现。

Result: 随着序列长度增加，精确匹配率急剧下降，但排名相关性保持较高。在条件排序中，大多数失败源于过滤步骤而非排序步骤。GPT-5和带扩展思维的Claude-3.7 Sonnet表现显著优于普通模型。时代错误检测是LLMs最容易的任务，但性能仍随时间线或实体重叠增加而下降。

Conclusion: 分配明确的推理预算有助于时间顺序排序，GPT-5在中/高推理努力下能在所有长度上实现完美排序和完美条件排序。研究揭示了当前LLMs在时间任务上的局限性，为金融领域的实时应用提供了重要见解。
...
### [157] [Listen Like a Teacher: Mitigating Whisper Hallucinations using Adaptive Layer Attention and Knowledge Distillation](https://arxiv.org/abs/2511.14219)
*Kumud Tripathi,Aditya Srinivas Menon,Aman Gaurav,Raj Prakash Gohil,Pankaj Wasnik*

Main category: cs.AI

TL;DR: 本文提出了一种两阶段架构来减少Whisper语音识别模型在噪声环境下的幻觉错误，通过自适应层注意力增强编码器鲁棒性，并使用多目标知识蒸馏框架抑制幻觉。


<details>
  <summary>Details</summary>
Motivation: Whisper模型在噪声声学条件下经常出现幻觉错误，而现有的减少幻觉方法主要关注音频预处理或转录后处理，对模型本身的修改探索不足。

Method: 两阶段方法：第一阶段通过自适应层注意力将编码器层分组为语义连贯块，使用可学习多头注意力融合块表示；第二阶段使用多目标知识蒸馏框架，在噪声音频上训练学生模型，使其语义和注意力分布与处理干净输入的教师模型对齐。

Result: 在噪声语音基准测试中，显著减少了幻觉和词错误率，同时在干净语音上保持了性能。

Conclusion: 自适应层注意力和知识蒸馏提供了一个原则性策略，可在真实世界噪声条件下提高Whisper的可靠性。
...
### [158] [DevPiolt: Operation Recommendation for IoT Devices at Xiaomi Home](https://arxiv.org/abs/2511.14227)
...
<details>
  <summary>Details</summary>
Motivation: 现有的推荐模型在处理复杂操作逻辑、多样化用户偏好以及对次优建议敏感方面存在困难，限制了它们在物联网设备操作推荐中的应用。

Method: 1. 通过持续预训练和多任务微调为LLM提供物联网操作领域知识；2. 使用直接偏好优化使微调后的LLM与特定用户偏好对齐；3. 设计基于置信度的曝光控制机制避免低质量推荐。

Result: 在所有数据集上显著优于基线方法，所有指标平均提升69.5%。在小米家庭应用中实际部署一个季度，为25.5万用户提供日常操作推荐，线上实验显示独立访客设备覆盖率提升21.6%，页面浏览接受率提升29.1%。

Conclusion: DevPiolt通过领域知识增强、用户偏好对齐和置信度控制，有效解决了物联网设备操作推荐中的挑战，在实际应用中取得了显著效果。
...
### [159] [Enhancing Regional Airbnb Trend Forecasting Using LLM-Based Embeddings of Accessibility and Human Mobility](https://arxiv.org/abs/2511.14248)
*Hongju Lee,Youngjun Park,Jisun An,Dongman Lee*

Main category: cs.AI

TL;DR: 提出基于LLM的新型时间序列预测框架，用于预测区域级Airbnb市场的收入、预订天数和预订数量，相比传统方法将RMSE和MAE降低约48%。


<details>
  <summary>Details</summary>
Motivation: Airbnb等短租平台的扩张严重扰乱了当地住房市场，导致租金上涨和住房负担能力问题。准确预测区域Airbnb市场趋势可为政策制定者提供关键见解。

Method: 使用滑动窗口方法预测1-3个月趋势，构建区域表示时整合房源特征与外部因素（城市可达性、人口流动），将结构化表格数据转换为基于提示的LLM输入，生成区域嵌入后输入RNN/LSTM/Transformer时间序列模型。
...
### [160] [PathMind: A Retrieve-Prioritize-Reason Framework for Knowledge Graph Reasoning with Large Language Models](https://arxiv.org/abs/2511.14256)
*Yu Liu,Xixun Lin,Yanmin Shang,Yangxi Li,Shi Wang,Yanan Cao*

Main category: cs.AI

TL;DR: PathMind是一个新颖的知识图谱推理框架，通过选择性引导LLM使用重要推理路径来增强忠实和可解释的推理，采用"检索-优先化-推理"范式，在复杂推理任务上表现优异。
...
Method: PathMind采用"检索-优先化-推理"范式：首先通过检索模块从KG中检索查询子图；然后引入路径优先化机制，使用语义感知路径优先级函数识别重要推理路径；最后通过双阶段训练策略生成准确响应。

Result: 在基准数据集上的广泛实验表明，PathMind始终优于竞争基线方法，特别是在复杂推理任务上，通过识别基本推理路径，使用更少的输入token实现更好性能。

Conclusion: PathMind通过选择性路径引导和双阶段训练策略，有效解决了现有LLM-based知识图谱推理方法的局限性，实现了更忠实、可解释和高效的推理。
...
TL;DR: 本文通过重新表述和扰动经典的CSPLib问题来测试LLMs在优化和约束编程中的模型生成能力，发现LLMs在上下文和语言变化下性能显著下降，表明其理解较浅且对措辞敏感。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在自动生成优化和约束编程模型方面的真实能力，检验其成功是否源于数据污染而非真正的推理能力。

Method: 系统性地重新表述和扰动一组著名的CSPLib问题，保持结构不变但修改上下文并引入误导元素，然后比较三个代表性LLMs在原始和修改描述下生成的模型。

Result: LLMs能够生成语法有效且语义合理的模型，但在上下文和语言变化下性能急剧下降，显示出浅层理解和对措辞的敏感性。

Conclusion: LLMs在优化和约束编程中的模型生成能力可能更多依赖于训练数据中的标准问题，而非真正的推理能力，对上下文和语言变化敏感。
...
TL;DR: 本研究探讨了在LLM对齐过程中考虑多元社会价值观的影响，发现人口统计特征和设计参数都会显著影响模型行为，为平衡安全性和公平代表性提供了初步答案。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐决策往往忽视人类社会的多样性，本研究旨在系统评估人口统计变化和设计参数如何影响对齐流程，以更好地平衡安全性和多元价值观。

Method: 收集美国和德国参与者（N=1,095）对LLM响应的评分数据，涵盖毒性、情感意识等五个维度；使用不同社会群体的偏好微调多个LLM模型，同时变化评分尺度、分歧处理方法和优化技术。

Result: 发现系统性人口统计效应：男性比女性评分毒性低18%；保守派和黑人参与者比自由派和白人参与者评分情感意识高27.9%和44%。技术设计选择影响显著：保留评分者分歧比多数投票毒性减少53%；5点量表比二元格式减少22%；DPO在多元值优化中优于GRPO。
...
### [163] [Rate-Distortion Guided Knowledge Graph Construction from Lecture Notes Using Gromov-Wasserstein Optimal Transport](https://arxiv.org/abs/2511.14595)
...
Method: 将讲义内容建模为度量-测度空间，使用融合Gromov-Wasserstein耦合量化语义失真，通过优化算子（添加、合并、拆分、移除、重连）最小化率失真拉格朗日量。

Result: 应用于数据科学讲义的实验显示，从优化后的知识图谱生成的选择题在15个质量标准上持续优于从原始笔记生成的题目。
...
### [164] [SkillGen: Learning Domain Skills for In-Context Sequential Decision Making](https://arxiv.org/abs/2511.14670)
...
<details>
  <summary>Details</summary>
Motivation: 现有ICL方法在顺序决策任务中难以同时满足三个关键原则：关注决策关键信息、提供步骤级粒度、最小化专家标注依赖。这些挑战促使开发SkillGen框架。

Method: SkillGen构建动作中心的领域级图，通过时间差分信用分配识别高效用动作，并检索逐步技能来生成细粒度的上下文感知提示。

Result: 在ALFWorld、BabyAI和ScienceWorld基准测试中，使用开源和专有LLM，SkillGen平均提升了5.9%-16.5%的进度率。

Conclusion: SkillGen通过关注高效用片段支持任务可识别性，为更有效的ICL提示设计提供了理论基础和实践验证，在多个顺序决策任务中实现了持续的性能提升。
...
### [165] [Heterogeneous Multi-Agent Proximal Policy Optimization for Power Distribution System Restoration](https://arxiv.org/abs/2511.14730)
*Parya Dolatyabi,Mahdi Khodayar*

Main category: cs.AI

TL;DR: 本文提出了一种异构智能体强化学习框架（HARL），通过异构智能体近端策略优化（HAPPO）实现互联微电网的协调恢复，解决了传统优化方法在大规模停电后电力分配系统恢复中的计算效率和扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 大规模停电后恢复电力分配系统需要顺序切换操作，在非线性约束下重新配置馈线拓扑并协调分布式能源资源。传统优化方法和基于价值的强化学习方法计算效率低且难以扩展。

Method: 采用异构智能体强化学习框架，每个智能体控制具有不同负载、DER容量和开关数量的微电网。使用去中心化的执行器策略和集中式评论家训练，通过物理信息化的OpenDSS环境提供完整的潮流反馈，使用可微分惩罚信号强制执行操作限制。
...
Conclusion: 在HARL框架中引入微电网级别的异构性，为复杂的电力分配系统恢复提供了一个可扩展、稳定且约束感知的解决方案。
