...
Motivation: 遥感数据量不断增长，需要自动信息提取来解决社会、经济和环境挑战，但高维数据限制了机器学习模型的有效性。

Method: 采用特征提取等维度降低技术，在保持数据关键特性的同时减少复杂性，支持数据压缩、清洗、融合、可视化、异常检测和预测等任务。

Result: 为遥感数据价值链提供了维度降低技术的应用手册，识别了未充分探索的维度降低算法及其在未来研究中的应用机会。

Conclusion: 维度降低技术是解决遥感高维数据挑战的关键工具，未来研究应关注更多维度降低算法的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶中高级感知、预测和规划问题的视觉语言问答，通过改进提示工程和上下文信息来提升现有视觉语言模型的性能。

Method: 采用两阶段方法：第一阶段使用大型多模态LLM（Qwen2.5-VL-32B）处理六摄像头输入、历史时间窗口和思维链提示；第二阶段增强提示信息，包括场景元数据和任务特定指令。使用自一致性集成提高答案可靠性。

Result: 在驾驶QA基准测试中显著优于基线模型：第一阶段使用5个历史帧和10-shot提示达到65.1%准确率（零-shot为62.61%），自一致性提升至66.85%；第二阶段达到67.37%总体准确率。在严重视觉损坏下仍保持96%准确率。

Conclusion: 精心设计的提示工程和上下文基础能够显著增强预训练视觉语言模型在高级驾驶问答任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 纵向分析序列放射影像面临数据挑战：如何有效建模高分辨率图像序列在不规则时间间隔下的变化。现有方法要么将空间信息压缩为向量，要么使用计算效率低且不兼容非均匀时间步长的时空模型。

Method: 提出Time-Aware Δt-Mamba3D架构，核心创新包括：连续时间选择性扫描机制，将真实检查时间差整合到状态转换中；多尺度3D邻域融合模块，稳健捕捉时空关系。

Result: 在乳腺癌风险预测基准测试中，验证c-index提高2-5个百分点，1-5年AUC得分高于循环、transformer和状态空间模型的现有变体。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型在生成时间一致和物理合理的运动方面存在不足，主要原因是模型对复杂运动理解不足。现有方法通过将扩散模型特征与预训练视频编码器特征对齐，但这些编码器将视频外观和动态混合在纠缠特征中，限制了这种对齐的益处。

Method: 提出运动中心对齐框架：1）从预训练视频编码器中学习解耦的运动子空间；2）该子空间通过预测真实光学流进行优化，确保捕获真实运动动态；3）将文本到视频扩散模型的潜在特征对齐到这个新的子空间。

Result: 在VideoPhy、VideoPhy2、VBench和VBench-2.0上的实证评估以及用户研究表明，该方法提升了最先进视频扩散模型的物理常识，同时保持了与文本提示的契合度。

Conclusion: 通过解耦运动子空间和对齐扩散模型特征，能够有效提升文本到视频生成模型的运动合理性和物理常识，同时不损害文本一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像描述评估指标（如CIDEr、SPICE）是为短文本设计的，无法有效评估长文本中属性和关系附着的准确性，需要能够定位特定文本跨度错误的新评估方法。

Method: PoSh使用场景图作为结构化评分标准，指导大型语言模型（LLM）作为评判者，产生基于细粒度错误的聚合分数。DOCENT数据集包含艺术品、专家编写的参考描述和模型生成的描述，并由艺术史学生提供质量评估。

Result: PoSh在DOCENT数据集上与人类评估的相关性比最佳替代指标高0.05 Spearman ρ，对不同类型的图像具有鲁棒性，并且作为奖励函数优于标准监督微调。研究发现基础模型在描述具有丰富场景动态的图像时难以实现无错误的完整覆盖。

Conclusion: PoSh和DOCENT为详细图像描述评估提供了更好的工具和基准，特别是在具有挑战性的艺术领域，为辅助文本生成等重要领域的发展奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 人体姿态表示在人体中心应用中至关重要，但目前缺乏对不同模态姿态表示之间相关性的系统研究。本文旨在开发一个统一的人体姿态表示学习框架，对齐图像、2D和3D人体姿态嵌入。

Method: 提出UniHPR统一人体姿态表示学习管道，使用新颖的基于奇异值的对比学习损失来同时对齐多种数据表示，包括图像、2D和3D人体姿态。

Result: 在Human3.6M数据集上MPJPE达到49.9mm，在3DPW数据集上PA-MPJPE达到51.6mm（跨域评估）。在Human3.6M数据集上实现2D和3D姿态检索，检索误差为9.24mm MPJPE。

Conclusion: UniHPR通过对比学习有效对齐了不同模态的人体姿态表示，在姿态估计和检索任务中表现出色，证明了统一表示学习的有效性。
...
Result: 在BraTS 2020数据集上的评估显示，模型表现优于相关研究，dice系数达0.975，特异性0.988，敏感性0.995。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球女性死亡的主要原因，需要早期检测和准确诊断。医疗数据的敏感性使得开发准确且私密的人工智能模型具有挑战性。联邦学习虽然能保护隐私，但在非独立同分布数据集上训练会影响模型的准确性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频理解研究主要依赖第三人称视角，忽略了同步的、自我中心的多智能体学习特性，需要从个体视角理解团队战术。

Method: 引入X-Ego-CS数据集（124小时专业CS2比赛视频）和跨自我中心对比学习（CECL）方法，对齐队友的自我中心视觉流以培养团队战术意识。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能生成一两种人体运动动力学测量指标，需要开发一种能够从视觉输入联合预测多种运动动力学指标的方法。

Method: 提出FootFormer跨模态方法，直接从视觉输入联合预测人体运动动力学，包括足压分布、足接触图和质心等指标。

Result: 在多个数据集上，FootFormer在足压分布、足接触图和质心估计方面显著优于或等同于现有方法，并在估计稳定性预测组件方面达到最先进性能。

Conclusion: FootFormer能够有效从视觉输入联合预测多种人体运动动力学指标，为运动分析提供了更全面的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在幻觉问题，现有解决方案需要额外训练或引入外部信息，带来额外计算成本。研究发现幻觉与视觉token注意力不足密切相关。

Method: 提出PruneHal方法，利用自适应KV缓存修剪技术，增强模型对关键视觉信息的关注，无需额外训练且几乎不增加推理成本。

Result: 在多个主流MLLMs和幻觉评估基准上的实验表明，PruneHal取得了稳健且出色的结果，证明了方法的有效性和优越性。

Conclusion: PruneHal是首个将token修剪应用于MLLMs幻觉缓解的方法，具有模型无关性，可与不同解码策略无缝集成，有效解决幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统的基于奖励的微调方法主要关注整个生成视频序列的质量提升，但在图像到视频生成任务中，时间一致性往往受到影响。为了解决这一局限性，需要专门针对时间一致性的改进方法。

Method: 提出VCD指标，在视频帧特征的频域空间中定义，通过频域分析有效捕捉帧信息，并使用基于奖励的微调框架对模型进行优化。

Result: 在多个图像到视频数据集上的实验结果表明，使用VCD微调的视频生成模型在时间一致性方面显著提升，且不会降低其他性能指标。
...
TL;DR: Dream4Drive是一个用于增强自动驾驶下游感知任务的合成数据生成框架，通过3D感知引导图和3D资产渲染生成多视角逼真视频，显著提升角案例感知性能。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界模型主要关注生成质量和可控性指标，但忽略了对于自动驾驶性能至关重要的下游感知任务评估。现有方法需要先在合成数据上预训练再在真实数据上微调，导致训练周期加倍，当基线方法也使用双倍周期时，合成数据的优势变得不明显。

Method: Dream4Drive首先将输入视频分解为多个3D感知引导图，然后将3D资产渲染到这些引导图上，最后微调驾驶世界模型以生成编辑后的多视角逼真视频，用于训练下游感知模型。

Result: Dream4Drive能够以前所未有的灵活性大规模生成多视角角案例，显著提升自动驾驶中的角案例感知能力。实验表明该框架在不同训练周期下都能有效提升下游感知模型的性能。

Conclusion: Dream4Drive提供了一个有效的合成数据生成框架，通过3D感知视频编辑技术增强自动驾驶感知任务，特别是在角案例处理方面表现出显著优势。
...
Method: 提出MoE-GS框架，集成多个专业专家，通过可微分权重溅射将体积高斯级权重投影到像素空间，实现空间和时间一致的结果。

Result: 在N3V和Technicolor数据集上的实验表明，MoE-GS在效率和性能上均优于现有最先进方法。

Conclusion: MoE-GS是首个将专家混合技术引入动态高斯溅射的方法，通过蒸馏策略可在不改变架构的情况下实现轻量级部署。
...
<details>
  <summary>Details</summary>
Motivation: 地球静止轨道空间目标在光学成像中面临信号弱、恒星背景复杂和环境干扰等检测挑战，需要开发有效的检测方法来应对这些问题。

Method: 使用小波变换增强单帧图像中的高频特征并抑制背景噪声；提出以匈牙利算法为核心的多帧时间轨迹补全方案进行全局最优跨帧匹配；设计后处理流程包括时间匹配插值补全、基于时间一致性的噪声过滤和渐进轨迹优化等关键步骤。

Result: 在公开SpotGEO数据集上的实验结果表明，该方法有效，F1分数达到90.14%。

Conclusion: 所提出的方法能够有效检测地球静止轨道空间目标，在复杂背景下表现出良好的性能，为空间目标检测提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着神经场、几何和运动深度学习以及3D生成AI的快速发展，4D生成与重建领域进展迅速。本文旨在从4D表示的角度提供独特的视角，帮助研究者理解如何选择和定制适合特定任务的4D表示方法。

Method: 采用选择性方法，重点关注代表性工作，分析不同4D表示在不同计算、应用和数据场景下的优缺点。将4D表示分为几何、运动和交互三个关键支柱进行组织讨论。

Result: 系统梳理了当前主流的4D表示方法（如NeRF、3DGS等），也关注了相对未被充分探索的表示（如结构化模型和长程运动），并讨论了LLM和VFM在4D应用中的作用及局限性。

Conclusion: 本文为4D生成与重建领域提供了系统的表示视角，强调了选择合适4D表示的重要性，并指出了当前数据集和方法的局限性，为未来研究方向提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 解决真实图像超分辨率中复杂退化与重建模糊性的问题，同时平衡生成质量与计算成本之间的矛盾。单步扩散模型虽然快速但存在结构失真问题。

Method: 使用ControlNet机制进行语义边缘引导，在单次推理中提供动态结构控制；结合L2、LPIPS和边缘感知AME损失的混合损失函数。

Result: 实验表明该方法有效提升结构完整性和真实感，同时保持单步生成的高效性，在输出质量和推理速度之间达到优越平衡。

Conclusion: 提出的框架成功解决了真实图像超分辨率中质量与效率的权衡问题，为快速高质量图像重建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 程序性活动中的错误分析在多个应用领域至关重要，需要利用计算机视觉技术来识别任务执行中的偏差，如错误序列、不当技术使用或时间错误。

Method: 通过计算机视觉技术（包括动作识别、预测和活动理解）来检测错误，分析现有数据集、评估指标和最先进方法，按程序结构使用、监督水平和学习策略分类方法。

Result: 建立了基于视觉的程序性活动错误分析的统一视角，识别了类内变异性、视角差异和组合活动结构等挑战，并讨论了现有解决方案。

Conclusion: 基于视觉的错误分析在提高安全性、效率和任务性能方面具有巨大潜力，未来方向包括神经符号推理和反事实状态建模等开放挑战。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）虽然取得显著进展，但规模庞大使其难以在资源受限环境中部署。需要开发高效的训练算法来创建轻量级但性能强大的VLMs。
...
Conclusion: RIL通过统一的强化与模仿学习策略，使轻量级学生模型能够实现显著的性能提升，与领先的闭源VLMs竞争。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将水深估计和海底分类视为独立任务，忽视了它们之间的相互作用，限制了深度学习方法的广泛应用。浅水环境面临日益增加的气候和人为压力，需要准确、详细且定期更新的水深数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人群定位方法在训练和测试数据存在尺度分布差异时性能显著下降，这种尺度偏移问题在领域泛化场景中尚未得到充分研究。

Method: 首先系统性地研究了尺度偏移对性能的影响，建立了ScaleBench基准并复现了20种先进DG算法。然后提出了Causal Feature Decomposition and Anisotropic Processing (Catto)算法来缓解尺度偏移影响。

Result: 实验表明现有算法在尺度偏移问题上存在局限性，Catto算法能有效缓解尺度偏移的影响。研究揭示了四个重要见解，为未来研究提供方向。

Conclusion: 尺度偏移领域泛化是一个重要且具有应用价值的研究方向，本文提出的理论分析和Catto算法为解决该问题提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将fMRI映射到CLIP的最终语义层，并添加参数密集的VAE管道来捕获细节，但忽略了CLIP中间层的丰富物体信息，且与大脑功能层次结构不符。

Method: 基于人类视觉系统功能层次结构，将功能不同视觉区域的fMRI信号对齐到对应的CLIP中间层和最终层，引入跨重建策略和新型多粒度损失函数。

Result: BrainMCLIP在高级语义指标上达到或超越最先进方法，包括使用VAE管道的方法，同时参数减少71.7%。
...
<details>
  <summary>Details</summary>
Motivation: 解决开集词汇图像分割和识别问题，避免传统方法需要大量标注数据和特定类别训练的限制，实现更灵活和通用的视觉理解能力。

Method: 采用两阶段流程：第一阶段使用EfficientNetB0提取像素特征，通过奇异值分解和层次聚类进行无监督分割；第二阶段使用CLIP的视觉编码器对分割区域编码，与文本嵌入在共享潜在空间对齐，通过相似度计算实现识别。

Result: 在COCO、ADE20K和PASCAL VOC等标准基准测试中，在Hungarian mIoU、精度、召回率和F1分数等指标上达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在医学图像配准中往往忽视正则化的重要性，导致缺乏鲁棒性和解剖合理性，需要一种能动态调整正则化的方法来平衡稳定性和灵活性。

Method: DARE框架基于变形场梯度范数动态调整弹性正则化，集成应变和剪切能量项，并采用折叠预防机制惩罚负变形雅可比区域。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLM评估主要关注单视角设置，而多摄像头设置已成为机器人平台标准配置，但VLM能否有效利用多视角输入进行机器人推理仍是一个未解问题。

Method: 构建MV-RoboBench基准测试，包含8个子任务的1.7k个手动策划QA项目，评估开源和闭源VLM模型，以及结合CoT技术的增强版本。

Result: 最先进模型远低于人类表现，空间智能与机器人任务执行在多视角场景中正相关，现有通用单视角空间理解基准的强性能不能可靠转化为机器人空间任务的成功。

Conclusion: VLM在多视角机器人感知中面临重大挑战，MV-RoboBench作为开放资源将促进空间基础VLM和VLA的发展，提供标准化评估协议。
...
<details>
  <summary>Details</summary>
Motivation: 随着电子商务发展，提高仓库运营效率至关重要。数字孪生技术需要准确收集工人位置信息，但单相机视野有限，需要多相机协同感知。

Method: 在物流仓库天花板安装19个向下拍摄的广角相机，基于地板表面对齐相机坐标系与实际位置关系，通过脚部位置对齐减少广角相机边缘畸变影响。

Result: 提出的脚部位置对齐方法有效减少了图像畸变影响，使跨相机位置对齐更准确，追踪精度提升超过20%。

Conclusion: 基于脚部位置的多相机坐标对齐方法能有效解决广角相机畸变问题，显著提高工人追踪精度，为仓库数字孪生应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在客观多模态感知任务中表现出色，但在主观、情感细腻的心理分析领域应用较少，本文旨在填补这一空白。

Method: 将包含多个实例的绘画分解为语义子图，构建层次表示；分析各层级的子图提取心理洞察；引入HTP知识库和特征提取模块生成心理档案；整合多层次信息生成专家级评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度分类技术和实验室自动光学检测模型在真实世界IC缺陷分类任务中表现不佳，主要由于两个原因：真实世界数据分布比公开不平衡数据集更加倾斜，以及真实样本同时包含类特定属性和类无关的领域相关特征。

Method: 提出ReCAME-Net模型，采用多专家分类器框架，整合区域通道注意力模块、度量学习损失、困难类别挖掘策略和知识蒸馏过程。

Result: 在IC-Defect-14数据集上，ReCAME-Net超越了之前的最先进模型，同时在通用公共数据集上保持了可比性能和竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 传统MRI采集时间过长限制了临床效率，需要新的方法来加速MRI扫描过程。

Method: 开发了一个预测-先验重建框架：首先使用生成模型基于多源数据（其他对比度图像、历史扫描图像、采集参数、患者信息）预测目标对比度图像，然后将这个信息丰富的先验用于重建高度欠采样的k空间数据。
...
<details>
  <summary>Details</summary>
Motivation: 卫星图像中系统性缺失像素（如云层遮挡）会导致非随机缺失数据，如果不处理，这些缺失像素会在自动化特征提取模型中引起表示偏差。在甲烷羽流检测中，标签与缺失值数量之间的虚假关联会导致模型将图像覆盖率与标签关联，从而在低覆盖率图像中检测性能下降。

Method: 1. 评估多种插补方法以消除覆盖率与标签之间的依赖性；2. 提出加权重采样方案，通过在每个覆盖率区间内强制类别平衡来消除标签与覆盖率之间的关联。

Result: 重采样和插补方法都能显著减少表示偏差，同时不会损害平衡准确率、精确率或召回率。在操作场景中评估表明，经过偏差校正的模型在低覆盖率图像中检测羽流的几率更高。

Conclusion: 通过重采样和插补方法可以有效解决卫星图像中系统性缺失像素导致的表示偏差问题，提高模型在低覆盖率图像中的检测性能，同时保持整体模型性能不受影响。
...
Method: 提出Cauvis方法：1）交叉注意力提示模块，通过视觉提示与交叉注意力集成减轻伪特征偏差；2）双分支适配器，通过高频特征提取实现领域适应，同时解耦因果-伪特征。

Result: 在SDGOD数据集上达到最先进性能，比现有域泛化方法提升15.9-31.4%，在复杂干扰环境中表现出显著鲁棒性优势。

Conclusion: Cauvis方法有效解决了单源域泛化目标检测中的伪相关性问题，通过因果视觉提示实现了更好的领域泛化能力和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型通常以原生或高分辨率处理图像，导致视觉token占总token的97-99%，即使低分辨率图像足够时也会产生高计算成本和延迟。

Method: 使用紧凑型VLM（350M）提取特征并预测目标预训练VLM响应何时收敛到其正确回答的峰值能力，虽然作为离散分类器训练，但在推理时可插值连续分辨率以实现细粒度控制。

Result: 在涵盖文档和自然图像的五个多模态基准测试中，以及不同的目标VLM上，CARES在保持任务性能的同时将计算量减少高达80%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理重叠度小或无重叠的图像对时表现不佳，而基于视频插值的方法生成的中间帧模糊且选择策略效率低，需要更有效的解决方案。

Method: 提出混合视频生成(HVG)方法，结合视频插值模型和姿态条件新视角合成模型生成清晰中间帧；设计基于特征匹配的选择器(FMS)从合成结果中选择适合姿态估计的帧。

Result: 在Cambridge Landmarks、ScanNet、DL3DV-10K和NAVI数据集上的实验表明，相比现有SOTA方法，PoseCrafter能显著提升姿态估计性能。

Conclusion: PoseCrafter方法通过改进的中间帧生成和选择策略，有效解决了稀疏重叠图像对的相机姿态估计难题。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在下游任务中表现优异，但在视觉推理方面仍存在诸多限制，如难以识别关系、理解时间序列和计数物体。本研究旨在超越基准评估，深入探究模型失败的根本原因并针对性提升其推理能力。

Method: 在受控实验条件下研究7种先进VLMs在计数任务中的推理技能，进行层间分析以识别错误来源，并通过仅微调输出层的针对性训练来验证改进方案。

Result: 实验表明VLMs对物体数量、类型、空间排列和干扰物高度敏感。层间分析揭示错误主要来自最后一层表示到输出空间的映射问题。仅微调输出层即可将准确率提升高达21%，并在真实数据集上获得一致改进。

Conclusion: VLMs在计数任务中的失败主要源于输出层映射问题，而非深层表示缺陷。通过针对性微调输出层可显著提升模型性能，这为改进VLMs的视觉推理能力提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型为创建无限合成数据提供了巨大潜力，但提示词工程作为主要交互方式，其复杂度对合成数据关键效用轴（质量、多样性、一致性）的系统性影响尚未充分探索。

Method: 首先进行合成实验以激发关于提示词复杂度泛化难度的动机，并用理论推导解释观察到的困难；然后引入新的评估框架比较真实数据和合成数据的效用，全面分析提示词复杂度对常用T2I模型生成合成数据效用的影响；在CC12M、ImageNet-1k和DCI等多个数据集上进行研究，评估不同的推理时干预方法。

Result: 合成实验显示，向更一般条件泛化比反向更难，因为前者需要扩散模型未学习的估计似然；大规模实证实验表明，增加提示词复杂度会导致条件多样性和提示一致性降低，同时减少合成到真实的分布偏移；当前推理时干预可以在牺牲真实数据支持的情况下增加生成多样性，其中提示扩展方法通过使用预训练语言模型作为似然估计器，在图像多样性和美学方面始终达到最高性能，甚至超过真实数据。

Conclusion: 提示词复杂度对合成数据效用有显著影响，增加复杂度会降低多样性和一致性但减少分布偏移；提示扩展方法在保持高质量的同时能有效提升合成数据的多样性和美学价值。
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型（VLMs）的时间感知能力，评估它们将视觉内容定位在时间中的能力，以探索VLMs在时间推理任务中的潜力。

Method: 引入TIME10k基准数据集（包含10,000多张带时间标签的图像），评估37个VLMs的时间感知能力，并提出从VLM嵌入空间中提取显式时间线表示的新方法。

Result: 发现时间信息在VLM嵌入空间中沿低维非线性流形结构化，提出的时间线方法在准确率上优于基于提示的基线方法，同时计算效率更高。

Conclusion: 视觉语言模型具有时间感知能力，通过从嵌入空间中提取时间线表示可以有效支持时间推理任务，为VLMs在时间相关应用中的使用提供了新思路。
...
Method: 设计轻量级VGGT变体从预训练模型中提取几何先验，构建高斯头融合多尺度几何token预测新视角渲染参数，并通过多尺度特征联合监督语义细化模型。

Result: 在nuScenes数据集上的实验表明，该方法在客观指标和主观质量上均显著优于现有最先进方法。

Conclusion: VGD框架验证了其在环视重建中的可扩展性和高保真度，能够有效平衡几何一致性和渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 地球观测领域产生大量多模态遥感数据，但在实际应用中由于平台限制，训练和推理阶段往往无法获得相同的传感器模态。多模态协同学习可以利用训练时的多模态数据来改善推理时的单模态模型性能。

Method: 结合对比学习和模态判别学习，引导单模态模型将内部模型流形结构化为模态共享和模态特定信息，实现跨任务泛化而不针对特定推理模态。

Result: 在四个地球观测基准测试中，涵盖不同传感器模态的分类和回归任务，在仅使用单一模态进行推理时，相比现有最先进方法取得了持续的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 解决光学镜头景深限制问题，传统方法和深度学习方法在训练数据有限、合成数据集域差距以及信息缺失区域处理方面仍面临挑战。

Method: 使用蒸馏变分自编码器进行图像重建，融合模块可同时处理多达7张图像，并创建MattingMFIF合成4K数据集模拟真实景深效果。
...
Method: 使用全自动模块化框架处理扫描或拍摄的心电图图像，包括处理常见伪影、透视畸变、褶皱和污渍等问题。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医学图像理解中表现出色，但其地面定位能力（文本概念与视觉证据的对齐程度）尚未充分探索，而在医学领域可靠的地面定位对于可解释性和临床采用至关重要。

Method: 使用交叉注意力和基于相似性的定位图生成视觉解释，并定量评估其与放射科医生标注区域在多种病理上的对齐程度，评估了七种CLIP风格的视觉语言模型变体。

Result: 发现：(1) 所有VLM变体对大型和明确病理的定位合理，但对小型或弥散病变性能显著下降；(2) 在胸部X射线特定数据集上预训练的模型比通用领域数据训练的模型对齐更好；(3) 模型整体识别能力与地面定位能力强相关。

Conclusion: 当前VLM尽管识别能力强，但在临床可靠的地面定位方面仍有不足，强调了在医学实践中部署前需要有针对性的可解释性基准测试。
...
TL;DR: 本文提出了一种基于条件伯努利扩散模型(CBDiff)的图像伪造定位方法，通过生成多个多样化的定位图来更好地表示伪造分布，解决了现有方法单一确定性定位图精度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像伪造定位方法通常生成单一确定性定位图，在高风险应用中缺乏足够的精度和可靠性。为了增强预测可信度并降低错误风险，需要一种能够处理伪造区域不确定性和变异性的方法。

Method: 提出了条件伯努利扩散模型(CBDiff)，在扩散过程中引入伯努利噪声以更好地反映伪造掩码的二元和稀疏特性，并设计了时间步交叉注意力机制(TSCAttention)来利用语义特征引导和时间步信息改进篡改检测。

Result: 在八个公开基准数据集上的广泛实验表明，CBDiff显著优于现有的最先进方法，显示出在实际部署中的强大潜力。

Conclusion: CBDiff通过生成多个多样化的伪造定位图，提供了更丰富和全面的伪造分布表示，有效解决了图像伪造定位中的不确定性问题，为高风险应用提供了更可靠的解决方案。
...
TL;DR: 本文提出了一种名为Policy World Model (PWM)的新驾驶范式，将世界建模和轨迹规划整合到统一架构中，通过无动作的未来状态预测方案利用学习到的世界知识来辅助规划。


<details>
  <summary>Details</summary>
Motivation: 尽管驾驶世界模型取得了显著进展，但其在自主系统中的潜力尚未充分挖掘：世界模型主要用于世界模拟，与轨迹规划分离。现有方法尝试统一世界建模和规划，但世界建模对规划的协同促进作用仍需进一步探索。

Method: 提出PWM框架，通过协作状态-动作预测实现类人预知感知；引入动态增强的并行令牌生成机制，配备上下文引导的令牌器和自适应动态焦点损失，提高视频预测效率。

Result: 仅使用前摄像头输入，该方法在性能上匹配或超越了依赖多视角和多模态输入的最先进方法。

Conclusion: PWM不仅统一了世界建模和规划，还能通过世界知识有效提升规划性能，展示了协同框架的优越性。
...
TL;DR: AMR是一个零外部依赖的增强时刻检索框架，通过两阶段训练（冷启动和蒸馏适应）解决现有方法的数据稀缺、边界模糊和语义区分不足问题，在多个基准测试中超越了现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有时刻检索方法面临的三个关键瓶颈：数据稀缺导致模型陷入浅层关键词特征关联、相邻事件间边界模糊、以及细粒度语义区分不足（如区分'踢球'和'扔球'）。

Method: 提出两阶段训练框架：冷启动阶段使用课程学习在增强数据上建立基础边界/语义意识；蒸馏阶段引入双查询集（原始查询和主动查询），通过跨阶段蒸馏损失保持知识一致性并实现现实世界泛化。

Result: 在多个基准测试中，AMR实现了比先前最先进方法更好的性能表现。

Conclusion: AMR框架能够在不依赖额外数据的情况下解决现有标注中的模糊边界信息和语义混淆问题，同时保持边界和语义区分能力，显著提升了时刻检索性能。
...
TL;DR: 提出了MedReason-R1医疗视觉语言模型，通过构建CT-RATE-VQA数据集和引入显式推理过程，在CT疾病诊断中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 通用视觉语言模型在医疗领域表现不佳，主要由于缺乏高质量医疗影像数据集和忽视从粗到细的诊断过程。

Method: 构建84K QA对的CT-RATE-VQA数据集；提出MedReason-R1模型，嵌入放大疾病感兴趣区域到图像中；引入GRPO强化学习框架实现无需人工标注的有效推理。

Result: MedReason-R1在CT疾病诊断中达到最先进性能，同时保持泛化能力。

Conclusion: 该方法通过显式推理过程和强化学习，显著提升了医疗视觉语言模型的诊断性能。
...
TL;DR: ReAct-GS方法通过重新激活机制解决3D高斯泼溅中的过重建伪影问题，包括重要性感知的致密化标准和自适应参数扰动，在多种数据集上实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在复杂场景中面临过重建伪影问题，表现为局部模糊和针状扭曲。现有方法归因于大规模高斯分裂不足，但研究发现存在梯度幅度稀释和原始冻结现象两个根本限制。

Method: 提出ReAct-GS方法，基于重新激活原则：1）重要性感知致密化标准，结合多视角α混合权重重新激活复杂区域停滞的原始增长；2）重新激活机制，通过自适应参数扰动激活冻结的原始基元。

Result: 在多种真实世界数据集上的综合实验表明，ReAct-GS有效消除过重建伪影，在标准新视图合成指标上达到最先进性能，同时保留精细几何细节。重新激活机制在其他3D-GS变体中也带来一致改进。

Conclusion: ReAct-GS通过重新激活机制成功解决了3D高斯泼溅中的过重建问题，具有广泛的适用性，能够显著提升复杂场景的渲染质量。
...
TL;DR: 提出了CST框架，在无配对图像翻译中专门保护细曲线结构，通过集成结构一致性训练，可无缝集成到现有方法中，在多种医学成像模态中提升翻译保真度。


<details>
  <summary>Details</summary>
Motivation: 现有无配对图像翻译方法在医学成像中经常扭曲细曲线结构（如微血管），影响诊断可靠性和定量分析，这在眼科和血管成像中尤为重要。

Method: CST框架通过增加曲线提取模块进行拓扑监督，将结构一致性集成到训练中，可集成到CycleGAN和UNSB等现有方法中。
...
<details>
  <summary>Details</summary>
Motivation: 呈现攻击是严重的安全威胁，现有深度学习检测系统多为黑盒模型，决策过程不透明，需要解释性技术来理解模型行为。

Method: 提出Ensemble-CAM技术，为基于深度学习的面部呈现攻击检测系统生成视觉解释，识别判断图像真伪的关键区域。

Result: 该方法能够提供详细的视觉解释，帮助用户理解深度学习模型为何将生物特征图像判定为真实或伪造。

Conclusion: Ensemble-CAM技术增强了基于深度学习的面部呈现攻击检测系统的透明度和可信度，有助于更好地理解模型决策过程。
...
TL;DR: LyTimeT是一个两阶段框架，通过时空注意力机制和稳定性约束从高维视频中提取可解释的动态系统变量，有效抑制视觉干扰因素，实现鲁棒的潜在表示学习和准确的长时视频预测。


<details>
  <summary>Details</summary>
Motivation: 从高维视频中提取真实动态变量面临背景运动、遮挡和纹理变化等视觉干扰因素的挑战，需要开发能够学习鲁棒和稳定潜在表示的方法。

Method: 第一阶段使用时序变换器自编码器，通过全局注意力聚焦动态相关区域并抑制干扰变化；第二阶段通过线性相关性分析选择物理意义维度，并用Lyapunov稳定性正则器优化转移动态以确保收缩性和减少误差累积。

Result: 在五个合成基准和四个真实世界动态系统上的实验表明，LyTimeT在互信息和内在维度估计上最接近真实值，对背景扰动保持不变性，并在CNN和纯变换器基线中实现最低的分析均方误差。
...
TL;DR: ADQ是一种混合精度量化框架，通过自适应权重量化方案解决激活值分布不均匀和静态码本不匹配问题，在ImageNet上使ResNet-18仅用2.81比特平均位宽达到71.512% Top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 现有量化感知训练方法面临两个主要挑战：激活值的高度非均匀分布和权重量化中使用的静态、不匹配码本问题。

Method: 提出自适应分布感知量化(ADQ)框架，包含三个关键创新：基于分位数的码本初始化方法、基于指数移动平均的在线码本自适应机制、以及基于敏感性的混合精度分配策略。对于激活值，集成了硬件友好的非均匀到均匀映射方案。

Result: 在ImageNet上，ADQ使ResNet-18仅用2.81比特平均位宽达到71.512% Top-1准确率，优于同类条件下的最先进方法。在CIFAR-10上的消融研究系统验证了各创新组件的贡献。

Conclusion: ADQ框架有效解决了量化中的分布不均匀和码本不匹配问题，在保持高精度的同时显著降低了模型位宽，验证了所提方法的合理性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在文本引导图像编辑方面取得显著进展，但研究社区因缺乏基于真实图像构建的大规模、高质量、开放可用数据集而受到限制。

Method: 利用Nano-Banana从OpenImages集合的真实照片生成多样化的编辑对，采用细粒度图像编辑分类法确保编辑类型全面覆盖，通过基于MLLM的质量评分和精心策展保持精确内容保留和指令忠实度。
...
TL;DR: 该论文分析了单目深度估计的评估指标问题，发现现有指标对曲率扰动严重不敏感，提出了基于相对表面法线的新指标、新的深度可视化工具和创建更好人类对齐的复合指标的原理方法。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计评估缺乏标准化，现有评估指标的选择众多且其权衡和行为未被充分理解，需要量化分析现有指标对地面真实值扰动的敏感性，并与人类判断进行比较。

Method: 对现有指标进行定量分析，评估它们对各种地面真实值扰动的敏感性；引入基于相对表面法线的新指标；开发新的深度可视化工具；提出创建与人类判断更好对齐的复合指标的原理方法。

Result: 分析显示现有指标对曲率扰动（如使平面变波浪状）严重不敏感；提出了新的评估指标和工具来弥补这一缺陷。

Conclusion: 单目深度估计评估需要更敏感的指标，特别是对曲率扰动；提出的新指标和工具有助于改进评估标准，使评估结果更符合人类感知。
...
TL;DR: olmOCR 2是一个先进的OCR系统，使用7B视觉语言模型和基于验证奖励的强化学习，在数学公式转换、表格解析和多列布局方面实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个强大的OCR系统，将数字化打印文档（如PDF）转换为干净、自然排序的纯文本，解决复杂布局文档的转换问题。

Method: 使用olmOCR-2-7B-1025视觉语言模型，采用基于验证奖励的强化学习训练，通过生成具有多样化布局的合成文档来扩展单元测试创建。

Result: 在olmOCR-Bench英文OCR基准测试中实现最先进性能，在数学公式转换、表格解析和多列布局方面相比之前版本有最大改进。

Conclusion: olmOCR 2在复杂文档OCR方面表现出色，模型、数据和代码已按开放许可发布。
...
TL;DR: ITTO是一个新的点跟踪基准测试套件，用于评估和诊断点跟踪方法的能力和局限性，包含来自现有数据集和真实世界自我中心记录的视频，具有高质量人工标注。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试缺乏真实世界场景的运动复杂性、遮挡模式和对象多样性，ITTO旨在填补这一空白，为点跟踪方法提供更全面的评估。

Method: 从现有数据集和真实世界自我中心记录中收集视频，通过多阶段流水线收集高质量人工标注，并对最先进的跟踪方法进行严格分析，按运动复杂度的关键轴分解性能。

Result: 现有跟踪器在这些挑战上表现不佳，特别是在遮挡后重新识别点方面，突显了关键失败模式。

Conclusion: ITTO作为点跟踪的基础测试平台，将推动开发更鲁棒的跟踪算法，需要针对真实世界动态的新建模方法。
...
TL;DR: 提出了一种联合实体识别和消歧的统一框架，利用大语言模型丰富实体上下文，在跨域数据集上达到最先进性能。
...
Result: 在基准数据集上的评估表明，该方法在跨域数据集上实现了最先进的性能表现。

Conclusion: 提出的联合实体识别和消歧框架结合大语言模型上下文增强，能够有效提升实体链接性能，特别是在跨域场景下表现优异。
...
TL;DR: 开发了一个使用小型语言模型(MiniLMs)从地球科学文献中精确、快速、低成本检索信息的框架，替代大型语言模型(LLMs)，解决了信息偏见和计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在自然语言处理方面取得进展，但存在信息偏见和计算成本高的担忧，需要开发更精确、高效且经济的信息检索方法。

Method: 构建了包含约7700万高质量句子的语料库，来自95个顶级地球科学期刊(2000-2024年)，使用MiniLMs通过语义搜索和句子级索引进行高效信息提取。

Result: MiniLMs能够有效识别大量专家验证的多学科信息，特别擅长处理定量发现，并能通过情感分析和无监督聚类跟踪研究结论演变、优先事项和新兴问题。

Conclusion: MiniLM在地球科学社区具有重要应用潜力，可用于事实和图像检索、趋势分析、矛盾分析以及教育目的。
...
TL;DR: 本文提出了一个简化的评估框架，使用20个精心设计的提示来评估LLM的指令遵循能力，通过大规模实证研究测试了256个模型，揭示了常见的失败模式和特定挑战。


<details>
  <summary>Details</summary>
Motivation: 由于现有基准可能被新模型记忆，需要新的评估方法来真实评估指令遵循能力，而不是记忆性能。需要一个实用的诊断工具来快速评估特定指令遵循模式。

Method: 使用20个精心设计的提示构建紧凑测试套件，评估格式合规性、内容约束、逻辑排序和多步骤任务执行。在2025年10月14日对来自OpenRouter的256个验证工作模型进行大规模实证研究。

Result: 研究揭示了持续存在的失败模式，识别出特定类型的指令尤其具有挑战性。提供了主要提供商和新兴实现模型的比较性能分析。

Conclusion: 这项工作既贡献了一个实用的评估工具，又提供了当代LLM生态系统中指令遵循能力的最全面实证分析之一。
...
TL;DR: 该研究比较了微调多语言Transformer模型与零样本大型语言模型在孟加拉语到锡尔赫特语翻译任务中的表现，发现微调模型显著优于LLMs。


<details>
  <summary>Details</summary>
Motivation: 尽管机器翻译在高资源语言上取得了显著进展，但像锡尔赫特语这样的低资源语言仍然研究不足，需要探索有效的翻译方法。

Method: 通过微调多语言Transformer模型（如mBART-50和MarianMT）并与零样本大型语言模型进行比较，研究孟加拉语到锡尔赫特语的翻译效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于零样本方法的机器生成内容检测器存在计算成本高或准确性不足的问题，需要在准确性和效率之间做出权衡，因此需要改进现有检测方法。

Method: 使用预训练的RoBERTA和CodeBERTa模型，在源代码和其他自然语言的专用数据集上进行微调，用于二元分类任务。

Result: 编码器模型在AUROC指标上达到0.97-0.99，宏F1分数达到0.89-0.94，同时延迟降低8-12倍，峰值VRAM减少3-5倍。在跨生成器转移和对抗性转换下，性能保持原始AUROC的92%以上。

Conclusion: 对于二元分类任务，小型语言模型在计算效率和准确性方面都显著优于大型语言模型，同时具有更好的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体短文本（如推文）的简洁性、非正式性和噪声往往阻碍传统主题建模的有效性，产生不连贯或冗余的主题，难以解释。

Method: 开发TM-Rephrase框架，使用两种重新表述策略（通用重新表述和口语到正式重新表述），在25,027条COVID-19相关推文数据集上测试多种主题建模方法。
...
TL;DR: 本文提出正交多样性感知选择（ODiS）算法，通过主成分分析将多维度评分解耦为正交特征维度，从每个维度直接选择高分数据，确保预训练数据既高质量又多样化。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分的数据选择方法存在维度相关性偏差，导致选择高分数据时忽视多样性，造成下游任务性能下降。需要将相关指标分解为正交特征维度。

Method: 1. 从语言质量、知识质量、理解难度等多维度评估数据；2. 通过PCA对多维评分进行去相关，得到正交评估维度；3. 为每个维度训练Roberta评分器进行大规模推理；4. 从每个正交维度选择高分数据构建训练集。
...
TL;DR: 提出了一个动态可逆的剪枝框架，通过检测上下文感知的神经元激活并应用自适应掩码来调节生成过程中的影响，实现细粒度的偏见缓解。
...
Method: 使用动态可逆剪枝框架，在推理时检测上下文感知的神经元激活，应用自适应掩码来调节其影响，保持知识完整性。

Result: 在多语言单轮和多轮对话中实现了更连贯的行为，支持动态公平控制，具有知识保留特性。

Conclusion: 该推理时解决方案提供了灵活、透明的偏见缓解方法，能够适应变化的对话上下文，为现实世界对话AI提供动态公平控制。
...
TL;DR: 提出了一个新颖的多模态基准测试MMAO-Bench，用于评估单模态和全模态理解能力，包含1880个人工标注样本和44种任务类型，揭示了跨模态与单模态性能之间的组成规律。
...
Method: 构建了包含1880个人工标注样本、44种任务类型的MMAO-Bench基准测试，并引入了创新的多步骤开放式问题类型来更好地评估复杂推理任务。

Result: 实验结果显示跨模态与单模态性能之间存在组成规律，全模态能力在弱模型上表现为瓶颈效应，而在强模型上表现出协同促进作用。
...
TL;DR: ProfBench是一个包含7000多个专家评估响应的基准测试，用于评估大语言模型在专业文档处理、信息综合和报告生成方面的能力，覆盖物理学博士、化学博士、金融MBA和咨询MBA四个专业领域。
...
Method: 构建ProfBench基准测试，包含专家评估的响应-标准对，并开发经济高效的LLM-Judges评估系统，通过减轻自我增强偏见将评估成本降低2-3个数量级。

Result: ProfBench对最先进的LLM构成显著挑战，表现最佳的GPT-5-high模型仅达到65.9%的总体性能，发现专有模型和开源模型之间存在明显性能差异。

Conclusion: ProfBench为评估LLM在专业领域任务中的能力提供了公平可访问的基准，揭示了扩展思维在处理复杂专业任务中的重要作用。
...
TL;DR: 开发了一个动态生成模型特定挑战性数据集的框架，构建了包含45万样本的OVERBENCH基准，用于持续监测语言模型的过度敏感性。


<details>
  <summary>Details</summary>
Motivation: 解决语言模型对良性提示的防御性拒绝问题，以及静态基准因模型演化而数据污染和评估能力下降的局限性。

Method: 开发动态生成模型特定挑战性数据集的框架，捕捉新兴防御模式并与每个模型的独特行为对齐，构建OVERBENCH基准。

Result: 构建了包含45万个样本、覆盖25个模型的OVERBENCH基准，提供了动态演化的过度敏感性视角。

Conclusion: OVERBENCH能够持续监测防御触发器随模型发展的变化，揭示静态数据集忽略的漏洞，为语言模型安全性评估提供动态解决方案。
...
TL;DR: 该论文介绍了SCRIPTS数据集，用于评估大语言模型在人际对话中的社会推理能力，发现现有模型在英语和韩语数据集上表现差异显著，且存在选择不可能关系的倾向，思维模型和思维链提示对社会推理帮助有限。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在人类-AI交互中的广泛应用，评估其在人际语境中的社会推理能力变得至关重要，特别是在推断说话者之间人际关系方面。

Method: 构建了包含1000个英语和韩语对话的SCRIPTS数据集，来源于电影剧本，由母语者标注概率性关系标签，评估了9个模型的社会推理能力。
...
Method: 使用WhisperX转录对齐、3帧视觉采样和Style-BERT-VITS2情感合成的模块化生成流程，将情感语调与视觉上下文对齐，采用耳语或深夜语调等表达性语音风格。
...
TL;DR: 该论文提出了两个用于评估大语言模型在心理健康支持领域表现的基准：MentalBench-100k和MentalAlign-70k，通过情感认知一致性框架量化LLM评估者与人类专家之间的可靠性、一致性和偏差。
...
Method: 构建MentalBench-100k（10,000个单轮对话，每个对话配9个LLM生成回复，共100,000个回复对）和MentalAlign-70k（比较4个高性能LLM评估者与人类专家在7个属性上的70,000个评分），使用情感认知一致性框架通过组内相关系数量化一致性。

Result: 分析显示LLM评估者存在系统性评分膨胀，认知属性（如指导和信息性）可靠性强，同理心精度降低，安全性和相关性存在不可靠性。
...
TL;DR: 该论文评估了大语言模型在生物医学术语标准化任务中的表现，发现微调效果因术语系统而异，主要受标识符流行度和词汇化程度两个因素影响。


<details>
  <summary>Details</summary>
Motivation: 生物医学数据集成依赖于术语标准化，即将自然语言术语映射到标准标识符。大语言模型在此任务中表现不一，需要系统评估其在不同生物医学本体中的记忆和泛化能力。

Method: 使用Llama 3.1 8B模型在多个生物医学本体上进行微调，评估记忆能力（训练术语表现）和泛化能力（验证术语表现），并进行嵌入分析。

Result: GO术语映射显示强记忆增益（准确率提升77%），而HPO改进最小。仅蛋白质-基因映射显示泛化增益（13.9%）。基线准确率随模型规模变化，GPT-4o在所有术语系统中表现最佳。嵌入分析显示基因符号与蛋白质名称语义对齐紧密，但GO和HPO的术语与标识符对齐较弱。

Conclusion: 微调成功取决于标识符流行度和词汇化程度。流行标识符在预训练中更常见，增强记忆能力；词汇化标识符（如基因符号）支持语义泛化。而GO和HPO的任意标识符限制了模型只能进行机械学习。
...
TL;DR: 该论文研究大语言模型在参数知识与提示中冲突信息存在差异时的行为，将知识冲突研究扩展到代码生成领域，提出了领域无关的框架来构建和解释此类冲突，并开发了新的评估方法和数据集。


<details>
  <summary>Details</summary>
Motivation: 基于先前问答研究，扩展对知识冲突的探索到代码生成领域，了解LLMs如何处理参数知识与外部提示信息之间的冲突。

Method: 提出了领域无关的框架来构建和解释知识冲突，开发了专门针对代码冲突场景的新型评估方法和数据集，使用激活级引导技术。

Result: 实验表明足够大的LLMs在其参数中编码了知识冲突的概念，能够以80.65%的准确率检测知识冲突；激活级引导相比随机基线可获得12.6%的改进。
...
TL;DR: 提出基于谱分析的框架，将Transformer层建模为注意力诱导的动态图，通过图信号处理定义诊断指标，实验表明事实陈述呈现一致的"能量山"模式，而不同类型的幻觉显示独特特征，基于谱特征的检测器性能优于困惑度基线。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在区分事实推理和幻觉方面仍面临挑战，需要开发新的分析框架来理解模型内部表示与推理错误之间的关系。

Method: 将Transformer层建模为注意力诱导的动态图，token嵌入作为图上的信号，通过图信号处理定义Dirichlet能量、谱熵和高频能量比等诊断指标，建立与计算稳定性的理论联系。

Result: 实验发现事实陈述呈现一致的低频收敛"能量山"行为，逻辑矛盾导致谱不稳定（效应量g>1.0），语义错误保持稳定但显示连接性漂移，替换幻觉显示中等扰动。基于谱特征的检测器准确率达88.75%，优于困惑度基线（75%）。
...
TL;DR: 该研究通过图信号处理分析transformer模型在语音交替任务中的注意力图，发现不同架构具有独特的计算指纹，可通过代数连通性变化检测模型的计算特征和语言偏向。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索不同transformer架构如何通过不同的连接模式实现相同的语言计算，并开发一种无需训练的诊断方法来揭示模型的计算特征和架构偏向。

Method: 使用图信号处理方法，在注意力诱导的token图上分析代数连通性（Fiedler值）的变化，在20种语言和三个模型家族中跟踪语音交替任务中的早期层（2-5层）连通性变化。

Result: 发现清晰的架构特征：Phi-3-Mini在英语中显示显著的早期层中断，而其他19种语言影响最小；Qwen2.5-7B显示对形态丰富语言的小幅分布式偏移；LLaMA-3.2-1B显示系统但温和的响应。这些光谱特征与行为差异强相关。

Conclusion: 训练重点可以在计算中留下可检测的印记，表现为句法转换期间可测量的连接模式。该方法可作为简单、无需训练的诊断工具，用于揭示架构偏向和支持模型可靠性分析。
...
<details>
  <summary>Details</summary>
Motivation: 藏语作为亚洲主要低资源语言，具有独特的语言和社会文化特征，为AI研究带来挑战和机遇。由于缺乏可访问数据资源、标准化基准和专用工具，藏语AI研究关注有限，需要系统性调查来推动该领域发展。

Method: 通过系统性地分类现有数据集和工具，评估不同任务中使用的方法，并在可能的情况下比较性能表现，识别持续存在的瓶颈问题。

Result: 调查发现藏语AI面临数据稀疏性、正字法变异和缺乏统一评估指标等瓶颈，同时指出了跨语言迁移、多模态学习和社区驱动资源创建的潜力。
...
TL;DR: TSSS是一个高效的多跳检索增强生成框架，通过模板化推理和基于检索器的终止器，减少重复token生成并实现确定性停止，在保持准确性的同时提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的多跳RAG方法存在效率问题，包括在每个步骤重新生成可预测的token序列和依赖随机停止机制，导致过多的token使用和不稳定的终止。

Method: 提出TSSS框架，包含：(i) 基于模板的推理，缓存重复前缀并将子查询锚定到主问题；(ii) 基于检索器的终止器，当额外子查询重复时确定性停止推理。

Result: 在HotpotQA、2WikiMultiHop和MuSiQue数据集上，TSSS在RAG-CoT方法中实现了最先进的准确性和有竞争力的效率。

Conclusion: TSSS通过分离结构化推理和终止控制，实现了更快的推理和更可靠的答案，特别适用于效率受限的场景如设备端推理。
...
TL;DR: 该论文提出了evolveQA基准测试，专门用于评估LLMs处理时间演化知识的能力，发现相比静态知识问题，LLMs在时间演化知识上的性能下降高达31%。


<details>
  <summary>Details</summary>
Motivation: 现有研究通过基于结构化知识库的基准测试评估LLMs处理时间知识冲突的能力，但这些测试关注广泛覆盖的流行实体，缺乏动态结构来公平评估不同知识截止日期的LLMs。

Method: 从3个真实世界的时间标记语料库（AWS更新、Azure变更和WHO疾病爆发报告）构建evolveQA基准测试，识别自然发生的知识演化，并生成针对不同LLM知识截止日期的问题和黄金答案。

Result: 通过对12个开源和闭源LLMs在3种知识探测格式上的广泛评估，发现在evolveQA上的性能相比静态知识问题显著下降，降幅高达31%。

Conclusion: evolveQA基准测试有效揭示了LLMs在处理时间演化知识方面的局限性，为未来改进LLMs的时间知识处理能力提供了重要评估工具。
...
TL;DR: 本文提出了一种不依赖LLM检索增强生成的知识图谱问答系统，使用小型释义模型对知识图谱检索到的实体关系边进行释义，在CRAG基准测试中分别达到71.9%和54.4%的准确率。


<details>
  <summary>Details</summary>
Motivation: 开发不依赖大型语言模型检索增强生成的知识图谱问答系统，避免LLM的高计算成本和复杂性，探索基于纯知识图谱检索的替代方案。

Method: 采用两阶段流水线：第一阶段预处理文档生成问答对，第二阶段将问答对转换为知识图谱，使用嵌入和模糊技术进行图检索，通过查询、重排序和释义生成最终答案。

Result: 在CRAG基准测试中使用LLM-as-a-judge评估，使用LLAMA-3.2和GPT-3.5-Turbo分别获得71.9%和54.4%的准确率。

Conclusion: 证明了不依赖LLM检索增强生成的纯知识图谱问答系统的可行性，为资源受限环境提供了有效的替代方案。
...
TL;DR: 本文提出了一种类型匹配的语言距离框架，通过结构感知表示解决现有语言知识库的局限性，并开发了统一的复合距离方法用于跨语言迁移。


<details>
  <summary>Details</summary>
Motivation: 现有语言知识库如URIEL+存在两个关键限制：一是其通用向量表示不适合语言数据的多样化结构，二是缺乏将这些信号聚合为单一综合分数的原则性方法。
...
Conclusion: 该框架为多语言研究提供了更原则性和有效的工具包，改进了跨语言迁移中的语言选择效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有难度可控问题生成方法存在两个主要局限：无法直接生成多项选择题（教育中最常用题型），以及没有显式训练优化难度控制精度。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型存在幻觉问题，生成看似真实但不可靠的内容。现有幻觉检测方法在声明提取和查询生成阶段存在上下文丢失和查询特异性低等局限性，影响整个检测流程的性能。

Method: 提出JointCQ框架，设计评估标准筛选合成训练数据，微调语言模型进行联合声明提取和查询生成，为下游搜索和验证提供可靠且信息丰富的输入。

Result: 实验结果表明，该方法在多个开放域QA幻觉检测基准测试中优于先前方法。

Conclusion: JointCQ框架推进了构建更可信和透明语言模型系统的目标，通过改进幻觉检测流程的关键阶段提升了整体性能。
...
TL;DR: KORE提出了一种协同方法，通过知识导向的数据增强和约束，在大型多模态模型中注入新知识的同时保留旧知识，解决了现有方法在新知识学习和灾难性遗忘方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在预训练权重中编码了大量事实知识，但这些知识是静态的且无法跟上现实世界的发展，阻碍了持续知识获取。现有方法在学习新知识和避免灾难性遗忘方面存在困难。
...
Conclusion: KORE通过知识导向的增强和约束，成功解决了大型多模态模型中的知识注入和保留问题，为持续知识获取提供了有效解决方案。
...
TL;DR: 本文提出了一种全面的幻觉分类法，包含11个类别，并开发了HAD模型，将幻觉检测、跨度级识别和修正集成到单一推理过程中。该模型在约90K合成样本上训练，在多个基准测试中优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言生成模型特别是大语言模型的广泛应用，其输出可靠性和准确性引发担忧，其中幻觉问题尤为突出，模型会产生看似合理但实际错误的信息，因此幻觉检测成为关键任务。

Method: 提出了包含11个类别的幻觉分类法，开发了HAD模型，将幻觉检测、跨度级识别和修正集成到单一推理过程，并在约90K精心合成的数据集上进行训练。

Result: 在领域内和领域外测试集上的评估显示，HAD模型普遍优于现有基线，在HaluEval、FactCHD和FaithBench基准测试中取得了最先进的结果，证明了其鲁棒性和通用性。

Conclusion: HAD模型通过集成检测、识别和修正功能，在多个NLG任务中表现出色，为幻觉检测提供了有效的解决方案，并创建了包含2,248个样本的HADTest测试集用于评估。
...
TL;DR: 本文提出了一种基于超体积优化的多目标文本摘要优化方法，通过动态调整强化学习中的奖励分数来平衡一致性、连贯性、相关性和流畅性等多个目标，在多个数据集上表现优于现有方法。
...
Method: 提出超体积优化策略，在强化学习的奖励过程中使用超体积方法动态调整组间分数，引导模型逐步逼近帕累托前沿。

Result: 实验结果表明该方法在多个代表性摘要数据集上整体得分优于GRPO方法，在不同维度上表现更均衡，7B基础模型增强后性能可与GPT-4相媲美，同时生成长度更短。

Conclusion: 超体积优化方法能有效平衡文本摘要的多个目标，生成质量更高的摘要，且计算效率更高。
...
TL;DR: 将推理能力集成到语音大语言模型中用于端到端的槽填充任务，通过思维链框架分解任务步骤，创建推理数据集并进行监督微调。


<details>
  <summary>Details</summary>
Motivation: 受推理大语言模型最新发展的启发，希望将推理能力整合到语音大语言模型中，以提升槽填充任务的性能。

Method: 使用思维链框架将槽填充任务分解为多个推理步骤，创建推理数据集，对语音大语言模型进行监督微调，区分常规和推理语音大语言模型，并实验不同类型的文本基础模型。

Result: 通过引入推理步骤实现了性能提升，但发现主要用于数学、逻辑和编码领域的推理文本大语言模型作为语音大语言模型的基础模型可能表现较差。混合语音大语言模型（基于混合文本基础大语言模型并微调以保留直接和推理两种操作模式）比仅使用单一操作模式的模型性能更好。

Conclusion: 将推理能力集成到语音大语言模型中能够提升槽填充任务的性能，但需要选择合适的文本基础模型，混合操作模式的语音大语言模型具有更好的性能表现。
...
TL;DR: 该研究探讨了通过个性化大型语言模型（Persona-LLMs）使用标注者身份特征如何影响其对仇恨言论的敏感性，特别是关于标注者与目标群体身份相似或不同时的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索将社会人口属性融入LLMs是否能解决自动仇恨言论检测中的偏见问题，将心理学关于群体身份的见解与先进NLP技术相结合。

Method: 使用Google的Gemini和OpenAI的GPT-4.1-mini模型，采用两种身份提示方法：浅层身份提示和基于检索增强生成（RAG）的深度情境化身份开发，以纳入更丰富的身份特征。

Result: 分析了使用内群体和外群体标注者身份对模型检测性能和跨不同社会群体公平性的影响，展示了身份驱动方法在减少偏见方面的潜力和局限性。

Conclusion: 身份驱动方法在减少偏见方面既具有潜力也存在局限性，为开发更公平的仇恨言论检测系统提供了有价值的见解。
...
TL;DR: LOGICAL是一个基于微调GLiNER模型的本地化PII去除系统，在临床文档中实现了高效的个人身份信息移除，性能优于大型语言模型，适合资源受限环境使用。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在临床笔记PII去除中计算成本高、数据隐私风险大的问题，特别是在低资源环境下的应用限制。

Method: 使用微调的GLiNER模型（modern-gliner-bi-large-v1.0），在1515份临床文档上训练，定义了9个PII类别，并与Azure NER、Presidio、Gemini-Pro-2.5和Llama-3.3-70B-Instruct进行比较。

Result: 微调GLiNER模型达到总体微平均F1分数0.980，显著优于Gemini-Pro-2.5（F1分数：0.845），95%的文档完全正确清理，在标准笔记本电脑上高效运行。
...
TL;DR: 该论文通过扩展DnD Gesture语料库，引入2,663个语义手势标注，研究手势在多模态对话轮次转换中的作用，并采用专家混合框架整合文本、音频和手势信息来预测轮次转换。


<details>
  <summary>Details</summary>
Motivation: 研究人类对话中多模态线索（如语音、手势和注视）在管理轮次转换中的作用，特别关注手势提供的补充性线索。

Method: 扩展DnD Gesture语料库为DnD Gesture++，包含2,663个语义手势标注（包括图标性、隐喻性、指示性和话语性手势），采用专家混合框架整合文本、音频和手势信息进行轮次转换预测。

Result: 实验表明，结合语义引导的手势相比基线模型带来了一致的性能提升，证明了手势在多模态轮次转换中的补充作用。

Conclusion: 语义手势在多模态对话轮次转换中发挥着重要的补充作用，能够有效提升轮次转换预测的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前模型虽然在语音和文本理解方面表现出色，但在自然对话中理解谁在什么时候说了什么（说话人归属推理）方面仍然存在困难，需要专门的基准测试来推动相关研究。

Method: 从四个开放语料库（CHiME-6、MELD、MultiDialog和AMI）构建，包含超过12,000个经过验证的实例，配有音频、转录文本和元数据。包含两个任务：说话人归属问答和通过话语匹配的说话人归属。

Result: 基准测试结果显示，模型能够捕捉说了什么内容，但经常无法识别是谁说的，揭示了说话人感知对话理解方面的关键差距。
...
<details>
  <summary>Details</summary>
Motivation: 长上下文推理对大语言模型至关重要，但现有强化学习方法主要针对短上下文，长上下文推理所需的高级思维模式尚未充分探索，且高难度强化学习数据稀缺。

Method: 提出LoongRL方法，核心是KeyChain合成方法：将短多跳QA转化为高难度长上下文任务，通过插入UUID链在大量干扰文档中隐藏真实问题，要求模型逐步追踪正确链、识别真实问题、检索相关事实并进行推理。

Result: 在Qwen2.5-7B和14B上，LoongRL显著提升长上下文多跳QA准确率，分别获得+23.5%和+21.1%的绝对增益。LoongRL-14B达到74.2分，与更大前沿模型性能相当，同时提升长上下文检索能力，通过所有128K压力测试，保持短上下文推理能力。

Conclusion: LoongRL通过数据驱动的强化学习方法有效解决了长上下文推理的挑战，训练出的模型在远超过训练长度的任务上表现出色，且具有良好的泛化能力和成本效益。
...
TL;DR: MoE-Prism通过模型-系统协同设计，将僵化的MoE模型转化为弹性服务，通过离线重构将专家分解为细粒度子专家，在线调度实现QoS感知调度，提供4倍以上的稳定操作点，显著提升吞吐量和降低延迟。


<details>
  <summary>Details</summary>
Motivation: 传统MoE模型由于依赖少数整体专家之间的路由，存在"质量悬崖"问题，只能在少数粗粒度操作点之间选择，导致成本与质量之间的困难权衡，无法适应多样化服务级别目标，造成资源过度配置。

Method: 1. 离线重构引擎：使用元启发式分区优化求解器将整体专家系统分解为细粒度"子专家"，保持功能局部性且无需重新训练；2. 在线调度引擎：实现QoS感知调度，包括云部署中的吞吐量最大化策略和内存受限设备的延迟优化卸载策略。

Result: 在三种不同MoE模型上的评估显示，MoE-Prism提供超过基线4倍的稳定操作点，在严格延迟预算下动态提升吞吐量达19.9%，在有限资源下降低延迟达10.36%。

Conclusion: MoE-Prism提供了关键的"控制旋钮"来弥合模型-系统之间的差距，实现了下一代自适应、高效且QoS感知的AI服务。
...
Result: 在德语(PHOENIX-2014T)和美国手语(How2Sign)数据集上显著优于其他无注释方法，在没有额外预训练数据的情况下缩小了无注释与有注释系统之间的差距。

Conclusion: 该方法为手语注释不可用的情况提供了新的最先进解决方案，有效减少了对手动标注的依赖。
...
TL;DR: 本文提出使用语言无关的多模态嵌入来监督手语翻译，结合多语言目标增强和视频级扰动来解决数据稀缺问题，实现直接多语言翻译。


<details>
  <summary>Details</summary>
Motivation: 传统手语翻译通常使用单一语言的文本进行训练，限制了可扩展性和跨语言泛化能力。本文旨在通过语言无关的多模态嵌入来克服这些限制。

Method: 采用在多种语言文本和语音上训练的语言无关多模态嵌入来监督手语翻译；提出耦合增强方法，结合多语言目标增强（翻译成多种语言）和视频级扰动来提高模型鲁棒性。
...
TL;DR: ToMMeR是一个轻量级模型（<30万参数），通过探测早期LLM层来识别文本中实体提及，在13个NER基准测试中实现93%的零样本召回率，90%以上的精确度，并且通过添加跨度分类头可实现接近SOTA的NER性能。


<details>
  <summary>Details</summary>
Motivation: 实体提及检测是信息抽取的基础任务，但也是已知的性能瓶颈，需要探索更高效的解决方案。

Method: 开发ToMMeR模型，通过探测早期LLM层来提取实体提及检测能力，使用轻量级参数设计，并可通过添加跨度分类头扩展为完整NER系统。

Result: 在13个NER基准测试中达到93%的零样本召回率，90%以上精确度；跨模型分析显示不同架构模型在提及边界识别上高度一致（DICE>75%）；扩展后实现80-87% F1的接近SOTA性能。

Conclusion: 结构化实体表示存在于早期transformer层中，可以通过最小参数高效恢复，证实提及检测是语言建模的自然涌现能力。
...
TL;DR: 本文介绍了DFKI-MLT在WMT-SLT 2022手语翻译任务中的提交，提出了一种端到端架构学习时空特征表示和翻译，但在测试集上性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 当前手语翻译系统通常使用通用seq2seq架构，但标准方法往往无法充分利用时间特征，需要改进以更好地泛化到新数据集。

Method: 开发了一个单一模型，同时学习时空特征表示和翻译，实现真正的端到端架构。

Result: 在开发集上获得了5±1 BLEU分数，但在测试集上性能大幅下降至0.11±0.06 BLEU分数。

Conclusion: 虽然提出的端到端架构在开发集上表现良好，但在测试集上泛化能力不足，表明需要进一步改进模型鲁棒性。
...
TL;DR: BLiSS 1.0是一个学习者跨语言句法结构基准，通过选择性容忍度测试模型对自然学习者错误与人工错误的区分能力，验证不同训练目标对模型与人类语言习得模式对齐的影响。
...
Result: 实验表明选择性容忍度是标准语法性的不同能力，性能按训练范式聚类，验证BLiSS作为测量训练目标影响模型与人类语言习得模式对齐的稳健工具。
...
TL;DR: MINED是一个评估大型多模态模型时间敏感知识理解能力的综合基准，包含6个关键维度和11个任务，涵盖2104个时间敏感知识样本。评估显示大多数开源LMMs缺乏时间理解能力，但可以通过知识编辑方法有效更新知识。
...
Method: 从维基百科构建MINED基准，由两名专业标注者创建2104个时间敏感知识样本，涵盖6种知识类型，评估15个广泛使用的LMMs，并研究通过知识编辑方法更新时间敏感知识的可行性。

Result: Gemini-2.5-Pro获得最高平均CEM得分63.07，大多数开源LMMs缺乏时间理解能力；LMMs在组织知识上表现最佳，在体育知识上表现最弱；知识编辑方法在单次编辑场景中能有效更新LMMs的时间敏感知识。
...
TL;DR: 本文评估了基于样本的最小贝叶斯风险（MBR）解码在语音识别（ASR）和语音翻译（ST）任务中的表现，发现MBR解码在大多数实验设置下优于传统的波束搜索方法。


<details>
  <summary>Details</summary>
Motivation: 考虑到MBR解码在文本到文本生成任务（如机器翻译、文本摘要和图像字幕）中表现出色，而波束搜索是目前语音到文本任务（如ASR和ST）的常用方法，作者希望验证MBR解码在语音到文本任务中的有效性。

Method: 使用Whisper及其衍生模型，在英语和日语上评估MBR解码在ASR和ST任务中的性能。
...
TL;DR: 该论文研究了成员推理攻击和机器生成文本检测之间的可转移性，证明了两种任务的最优性能度量是相同的，并通过大规模实验验证了跨任务性能的强相关性。


<details>
  <summary>Details</summary>
Motivation: 虽然成员推理攻击和机器生成文本检测目标不同，但方法都基于语言模型的概率分布信号。这两种任务被独立研究，可能导致忽略了另一任务中更强的方法和有价值的见解。

Method: 从理论上证明两种任务的最优性能度量相同，并通过大规模实证实验（包括7种最先进的MIA方法和5种机器文本检测器，涵盖13个领域和10个生成器）验证跨任务性能的相关性。

Result: 实验显示跨任务性能存在非常强的秩相关性（rho > 0.6），特别是Binoculars方法在MIA基准测试中也达到了最先进的性能。

Conclusion: 研究结果强调两个研究社区需要更多的跨任务意识和合作，并引入了MINT评估套件来促进跨任务发展和公平评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前语音基础模型的评估在不同任务和模型类型之间是脱节的，不同模型在语音处理的不同方面表现出色，因此需要不同的评估协议。

Method: 提出了一个包含三个正交轴线的分类法：评估方面（测量的内容）、模型能力（完成任务所需的能力）和任务要求（执行任务所需的协议要求），并对现有评估和基准进行分类。

Result: 分类法提供了一个原则性框架，用于将模型与合适的评估方法对齐，并揭示了系统性差距，如韵律、交互或推理方面的覆盖有限。
...
<details>
  <summary>Details</summary>
Motivation: 多语言基础模型在特定语言上微调时经常引发灾难性遗忘，降低在未见过语言上的性能。虽然这种现象已被广泛记录，但文献对何时发生遗忘的结果零散。为解决这种模糊性，需要进行系统实证研究。

Method: 使用机器翻译作为测试平台，通过跨不同模型架构、数据规模和微调方法的受控实验，识别触发多语言微调中灾难性遗忘的条件。

Result: 发现模型与数据规模的相对比例是遗忘的主要决定因素；模型的指令跟随能力对保留多语言知识比其架构更关键；参数高效微调相比全参数微调在缓解遗忘方面没有明显优势；跨语言对齐可以缓解遗忘并促进对未见目标语言的积极迁移。

Conclusion: 跨语言对齐是缓解多语言微调中灾难性遗忘的有效策略，同时还能促进对目标语言的积极知识迁移。
...
TL;DR: 本文提出从混合语言历史文档中提取拉丁片段的新任务，评估大模型在724页多模态数据集上的性能，证明当代模型可实现可靠的拉丁检测。


<details>
  <summary>Details</summary>
Motivation: 解决从布局多样的混合语言历史文档中提取拉丁片段的任务需求，填补该领域研究的空白。

Method: 使用724页带标注的多模态数据集，对大型基础模型进行基准测试和性能评估。

Result: 结果表明当代模型能够实现可靠的拉丁检测，为这项任务提供了可行性证明。

Conclusion: 本研究首次全面分析了这些模型在此任务上的能力和局限性，为相关研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，确保其与社会规范对齐变得至关重要。现有研究主要关注其他语言的偏见检测，但在波斯文化背景下缺乏相关资源。

Method: 通过与社会科学专家合作，让250名来自不同人口统计背景的个体填写问卷，开发了包含16个文化类别的PBBQ基准数据集，并对多个开源、闭源和波斯语微调模型进行了评估。

Result: 研究发现当前LLMs在波斯文化中表现出显著的社会偏见，模型输出与人类响应比较显示，LLMs经常复制人类偏见模式。
...
TL;DR: 本文提出了一种基于字体风格的攻击方法SAD，利用社交媒体中使用的风格化字体和字体表情符号，在保持人类可读性的同时干扰NLP模型处理。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体的发展，用户使用风格化字体和字体表情符号表达个性，这些视觉上吸引人的文本虽然人类易读，但会在NLP模型中引入隐藏漏洞，因为模型将这些字符处理为不同标记，造成干扰。

Method: 提出Style Attack Disguise (SAD)攻击方法，设计两种规模：轻量版用于查询效率，强力版用于优越的攻击性能。

Result: 在情感分类和机器翻译任务上对传统模型、LLM和商业服务进行实验，证明SAD具有强大的攻击性能。同时展示SAD对多模态任务（文本到图像和文本到语音生成）的潜在威胁。

Conclusion: SAD方法有效利用了人类与模型感知差距，在保持文本可读性的同时成功攻击NLP系统，对多模态任务也构成威胁。
...
TL;DR: 本文提出了一种可扩展、可解释的众包流程，用于跨语言新闻相似性评估，并构建了CrossNews-UA数据集（乌克兰语与波兰语、俄语、英语的新闻对），测试了多种模型在跨语言新闻分析中的性能。
...
Method: 引入可扩展、可解释的众包流程进行跨语言新闻相似性评估，构建CrossNews-UA数据集（乌克兰语为中心语言，与波兰语、俄语、英语配对），每个新闻对基于4W标准（Who、What、Where、When）进行语义相似性标注并提供详细理由。测试了从传统词袋模型、Transformer架构到大型语言模型（LLMs）的多种模型。

Result: 结果突显了多语言新闻分析中的挑战，并提供了模型性能的深入见解。

Conclusion: 该研究填补了现有跨语言新闻分析数据集在可扩展性和适应性方面的空白，为跨语言新闻验证提供了新的方法和数据集。
...
TL;DR: 本文比较了微调预训练模型与使用提示工程的通用大语言模型在情感检测中的效果，发现微调模型在情感识别中达到70%以上的指标，而LLMs需要结构化提示工程和情感分组来提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在情感识别领域取得显著进展，但在处理开放文本时仍面临上下文模糊、语言变异性和复杂情感表达解释困难等挑战，这使得通用模型的直接应用变得困难。

Method: 比较了三种不同场景：(i) 微调预训练模型与使用简单提示的通用LLMs的性能；(ii) 不同情感提示设计对LLMs的有效性；(iii) 情感分组技术对这些模型的影响。

Result: 实验测试显示，微调预训练模型在情感识别中达到70%以上的指标。研究结果表明LLMs需要结构化提示工程和情感分组来提升性能。
...
TL;DR: LlavaCode框架通过将代码压缩为紧凑的语义表示，在保持代码生成质量的同时显著减少检索上下文，从而降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成在代码补全中很有效，但引入上下文会延长序列长度导致推理变慢，这在IDE等交互式环境中是严重限制。

Method: 使用小型投影器模块将代码压缩为可由代码LLM解释的紧凑语义表示，将检索上下文减少到仅几个压缩的单标记向量。

Result: 压缩上下文使行级补全任务的首次标记时间减少20-38%，同时通过可忽略的延迟增加显著提高了编码模型的EM和ES指标。

Conclusion: LlavaCode框架能够在保持生成质量的同时显著降低推理延迟，为交互式代码补全提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类沟通具有动机性，而LLMs处理的信息天然带有意图和激励因素。为了让LLMs在现实世界中有效运作，它们必须能够批判性地评估内容，考虑来源的动机，比如评估销售宣传中声明的可信度。

Method: 首先使用认知科学的受控实验验证LLMs行为是否与理性学习模型一致，然后扩展到赞助在线广告等更自然的信息环境，并测试简单的引导干预效果。

Result: LLMs在受控实验中成功以类似人类的方式偏置来自有偏见来源的信息，但在在线广告场景中推理与理性模型预测不符。简单的引导干预显著提高了LLMs与理性模型的对应性。
...
TL;DR: 本文提出DiffAdapt框架，通过分析推理轨迹中的熵模式来优化大型语言模型的推理效率，在不降低准确性的情况下减少22.4%的token使用量。


<details>
  <summary>Details</summary>
Motivation: 现有推理大型语言模型虽然表现出色，但会生成冗长的推理轨迹，效率低下。研究发现模型在简单问题上存在"过度思考"现象，导致不必要的计算开销。

Method: 基于熵分析发现U形模式，提出DiffAdapt轻量级框架，根据问题难度和推理轨迹熵选择不同的推理策略（Easy/Normal/Hard），每个策略包含固定提示、温度和最大token长度。

Result: 在5个模型和8个基准测试上的评估显示，该方法在保持或提高准确性的同时，token使用量最多减少22.4%。

Conclusion: DiffAdapt为计算高效的推理提供了一条实用路径，无需微调基础模型，仅需训练小型分类探针即可实现效率优化。
...
TL;DR: 本文通过实验研究发现，提示词对语言模型内部表示质量有影响，但这种变化与提示词对目标任务的相关性并不一致相关，挑战了相关提示词必然产生更好表示的假设。


<details>
  <summary>Details</summary>
Motivation: 研究提示词与语言模型内部表示质量的关系，以揭示预训练嵌入如何支持上下文任务解决，理解语言模型在零样本设置下执行多样化任务的内在机制。

Method: 进行一系列探测实验，分析零样本分类中不同提示词模板组合对提示嵌入的影响。

Result: 发现提示词影响表示质量，但这些变化与提示词对目标任务的相关性并不一致相关。

Conclusion: 研究结果挑战了相关提示词必然产生更好表示的假设，并进一步分析了可能导致这种意外行为的潜在因素。
...
TL;DR: CoSense-LLM是一个边缘优先框架，将多模态传感器流转换为紧凑的可验证语义令牌，并与大语言模型协调，满足延迟、能耗、带宽和隐私约束。
...
Method: 包含四个组件：SenseFusion轻量编码器、Edge-RAG本地混合检索层、PromptRouter成本感知策略和Secure Execution可审计数据最小化路径。
...
TL;DR: 提出了SmartSwitch推理框架来解决大语言模型在复杂推理任务中的"欠思考"问题，通过监控推理过程、检测过早放弃的高潜力思路，并引导模型进行更深入探索。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂推理任务中存在"欠思考"问题，即模型频繁切换思路而缺乏充分探索，这限制了性能表现和token效率。

Method: SmartSwitch框架包含感知模块和干预模块。感知模块使用现成的过程奖励模型识别思路切换点并评估前一个思路的潜力；干预模块在发现高潜力思路被过早放弃时，回溯到切换点前并插入"深化提示"来鼓励进一步探索。

Result: 在具有挑战性的数学推理基准测试上的广泛实验表明，该方法显著提升了各种不同规模大语言模型的性能。

Conclusion: SmartSwitch是一个简单有效的推理策略，可以作为即插即用解决方案集成到任何大语言模型中，有效解决欠思考问题并提升推理能力。
...
TL;DR: AdaSPEC是一种改进推测解码的方法，通过选择性令牌过滤在知识蒸馏过程中，提高小模型与大模型的对齐度，从而提升令牌接受率而不影响生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法最小化所有令牌的KL散度，这与推测解码最大化令牌接受率的目标不一致。由于容量限制，小模型难以完全吸收大模型知识，导致性能不佳。

Method: AdaSPEC在知识蒸馏过程中引入选择性令牌过滤，使用参考模型识别并过滤难以拟合的令牌，使小模型能更好地在简单令牌上与目标模型对齐。

Result: 在算术推理、指令遵循、编码和摘要等多样化任务上，使用31M/1.4B和350M/2.7B参数配置，AdaSPEC始终优于最先进的DistillSpec方法，在所有任务中实现了更高的接受率（最高达15%）。

Conclusion: AdaSPEC通过选择性令牌过滤改进了推测解码的知识蒸馏过程，显著提高了令牌接受率，同时保持了生成质量，为加速大语言模型推理提供了有效解决方案。
...
TL;DR: Scaf-GRPO是一种渐进式训练框架，通过在学习停滞时提供分层提示来克服强化学习中的"学习悬崖"现象，显著提升LLM在复杂数学问题上的表现。


<details>
  <summary>Details</summary>
Motivation: 传统基于可验证奖励的强化学习方法面临"学习悬崖"问题：当模型遇到远超其当前能力的问题时，会持续获得零奖励信号，导致优势计算归零，学习梯度消失，阻碍模型进步。
...
TL;DR: 本文介绍了EUDial数据集和LexGuide框架，用于解决普通公民理解欧盟法律信息的挑战。EUDial包含880个对话轮次，基于欧盟议会研究服务整理的204个博客构建。LexGuide框架利用检索增强生成和分层主题组织来结构化对话进程。


<details>
  <summary>Details</summary>
Motivation: 欧盟虽然提供立法、议会回应和监管文件的开放访问，但这些资源对普通公民来说难以理解和探索。法律信息的可及性仍然是一个持续挑战。

Method: 构建EUDial多轮对话数据集（880个对话轮次，平均每个对话4.3轮），并提出LexGuide框架，该框架结合检索增强生成和分层主题组织来结构化对话进程。

Result: 结果表明，主动的结构化导航缩小了法律信息可用性与公民理解之间的差距。

Conclusion: EUDial和LexGuide为推进主动法律对话系统提供了实用资源，能够确保法律方面的全面覆盖和对话轮次间的连贯性。
...
TL;DR: 该论文研究了模型合并作为代码混合NLP中传统适应策略的实用替代方案，通过在多语言基础模型上进行持续预训练、模型合并和下游任务微调，在英语-印地语和英语-西班牙语情感分析任务中取得了优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 研究模型合并作为代码混合NLP任务中传统适应策略的替代方案，旨在更有效地利用未标记的代码混合数据，提升模型在低资源语言对上的性能。

Method: 从多语言基础模型出发，执行三个步骤：(i) 在未标记的代码混合文本上进行持续预训练获得适应检查点；(ii) 将检查点与基础模型合并；(iii) 在下游任务数据上进行微调。使用XLM-R和Llama-3.2-1B模型在英语-印地语和英语-西班牙语句子分类任务上评估。

Result: 合并模型在F1分数上比完全微调高出2-5个百分点，比CPT->FT高出约1-2个百分点。在跨语言对迁移测试中，合并检查点比单语英语基线表现更好，达到0.65-0.68 F1 vs 0.61-0.63。

Conclusion: 模型合并比单独使用持续预训练更有效地利用了未标记数据，代码混合知识是低资源语言对更可靠的基板。论文提供了针对不同数据场景的适应方案，并讨论了在更广泛任务和更大模型上的扩展考虑。
...
TL;DR: ToolDreamer是一个框架，通过使用LLM生成假设性的工具描述来改进工具检索，使检索器能够更好地理解用户查询与工具之间的语义对齐，从而提高检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索模型基于用户查询与工具描述的相似性进行排名，但由于用户请求往往与工具描述语言不匹配，导致检索效果不佳。需要一种方法使检索器能更好地理解查询意图与工具功能之间的对应关系。

Method: 提出ToolDreamer框架，使用LLM生成假设性（合成）工具描述，这些描述代表LLM认为可能对查询有用的工具类型。通过这种方式在工具描述的语言空间内实现更自然的查询与工具对齐。

Result: 在ToolRet数据集上应用ToolDreamer，结果显示该方法提升了稀疏和密集检索器的性能，无论是否经过训练，证明了其灵活性。

Conclusion: 该框架旨在将部分推理负担转移到检索器上，使LLM能够有效处理大量工具集合而不会超出其上下文窗口限制。
...
TL;DR: 提出了一种轻量级的提示空间优化框架，通过自然性、文化适应性和难度增强三个维度系统性地优化翻译提示，显著提升多语言LLM性能。
...
Method: 引入提示空间优化框架，对翻译提示进行三个维度的系统转换：自然性（使提示更自然）、文化适应（融入文化背景）、难度增强（增加挑战性）。在12种语言上应用这些转换。

Result: 在相同数据条件下，相比仅翻译的基线方法，在Global-MMLU准确率上提升4.7%，Flores XCometXL上提升2.4%，mArenaHard偏好评估中获胜率提升35.3%。

Conclusion: 提示空间优化是一个简单而强大的范式，可以构建更稳健、文化基础更好、全球能力更强的多语言LLM。
...
TL;DR: Hubble是一套用于研究LLM记忆机制的开源大语言模型，包含标准和扰动变体，通过控制插入文本研究记忆风险，发现训练语料大小和敏感数据出现时机影响记忆程度。
...
Method: 开发标准模型和扰动模型，在训练中控制插入文本（如书籍段落、传记、测试集），分析不同参数规模和训练阶段对记忆的影响。

Result: 发现记忆风险由敏感数据在训练语料中的相对频率决定，增加语料规模可稀释敏感数据，早期暴露敏感数据可减少记忆。

Conclusion: 提出两种最佳实践：通过增加训练语料规模稀释敏感数据，以及让敏感数据在训练早期出现；Hubble为记忆研究、成员推理和机器遗忘提供了理想测试平台。
...
TL;DR: 本文提出了一个统一框架，将可验证的测试时扩展建模为传输问题，揭示了生成器覆盖率、验证器收敛区域和采样算法次优性之间的几何交互关系，并识别了次优性-覆盖率曲线的三个不同阶段。


<details>
  <summary>Details</summary>
Motivation: 现有研究对测试时扩展中验证器的作用及其缺陷探索不足，缺乏一个能够量化生成器覆盖率、验证器收敛区域和采样算法次优性三者交互关系的统一框架。

Method: 将可验证的测试时扩展构建为传输问题，分析覆盖率、收敛区域和次优性的几何交互，并提出和分析两类采样算法——顺序采样和批量采样。

Result: 理论分析揭示了次优性-覆盖率曲线存在三个阶段：传输阶段（次优性随覆盖率增加）、策略改进阶段（次优性可能随覆盖率减少）和饱和阶段（次优性趋于平稳）。实验使用Qwen、Llama和Gemma模型验证了理论发现。

Conclusion: 该研究提供了一个统一框架来理解测试时扩展中验证器的作用，揭示了不同采样算法在计算复杂度和性能权衡方面的差异，为优化LLM测试时性能提供了理论基础。
...
TL;DR: ACTMED是一个结合贝叶斯实验设计和大型语言模型的临床诊断框架，通过逐步选择最能减少诊断不确定性的测试来模拟真实诊断推理过程，提高诊断准确性、可解释性和资源利用效率。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习诊断方法依赖静态完整数据集，无法反映临床实践中顺序性、资源感知的推理过程。诊断在高压或资源有限环境中仍然复杂且容易出错，需要能够帮助临床医生做出及时且成本效益高的决策的框架。

Method: 将贝叶斯实验设计与大型语言模型集成，在每个步骤选择预期能最大程度减少患者诊断不确定性的测试。LLMs作为灵活模拟器，生成合理的患者状态分布并支持信念更新，无需结构化、任务特定的训练数据。

Result: 在真实世界数据集上评估显示，ACTMED能够优化测试选择，提高诊断准确性、可解释性和资源使用效率。
...
TL;DR: MUSE基准测试评估多模态大语言模型在音乐理解中的关系推理能力，发现现有模型在基础音乐感知技能上存在严重缺陷，与人类专家存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在音频理解方面的评估可能掩盖了其在关系推理方面的根本弱点，需要更严格的基准来评估基础音乐感知技能。

Method: 开发了MUSE基准测试，包含10个任务来探测基础音乐感知技能，评估了4个SOTA模型（Gemini Pro和Flash、Qwen2.5-Omni、Audio-Flamingo 3）并与200人的人类基线进行比较。

Result: 结果显示SOTA模型能力差异很大，与人类专家存在持续差距。Gemini Pro在基础感知上表现良好，但Qwen和Audio Flamingo 3表现接近随机水平，暴露严重感知缺陷。思维链提示提供不一致且通常有害的结果。

Conclusion: MUSE基准为评估不变音乐表示和开发更稳健的AI系统提供了关键工具。
...
TL;DR: 本研究采用行为和元认知分析方法，系统比较了两种代表性LLM在三种提示条件下评估临床试验报告CONSORT标准的认知和推理策略差异。
...
Method: 使用专家验证数据，应用行为和元认知分析方法，在三种提示条件下系统比较两种代表性LLM。
...
TL;DR: 论文研究了推理模型中的模式选择问题，将其视为早期退出问题的更具挑战性变体，旨在通过零步思考在推理开始前决定使用长链式思考还是短链式思考，以减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 推理模型在数学和逻辑推理任务中表现出色，但逐步思考过程常导致过度思考，产生不必要的计算开销。为了解决这个问题，需要开发能够自动选择思考模式的方法。

Method: 将模式选择识别为早期退出问题的变体，比较了九种基线方法，包括基于提示的方法和利用模型内部信息的方法，重点关注在推理开始前基于预定义虚假思考进行决策的零步思考方法。

Result: 基于提示的方法由于分类能力有限而经常失败，而利用内部信息的方法在大多数场景中表现更好但仍存在稳定性问题。现有方法在信息有限的情况下不足以有效解决模式选择问题。

Conclusion: 仅依赖模型提供信息的现有方法在信息有限的情况下无法有效解决模式选择问题，凸显了该任务的持续挑战性。
...
TL;DR: 本研究通过实验评估ChatGPT在法律领域的表现，与基于正则表达式的基线方法进行比较，发现ChatGPT虽然具备必要知识和能力，但无法有效整合和推理以产生全面结果，揭示了其在复杂问题分解和综合解决方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 评估ChatGPT在法律领域的实际能力，特别是其在处理复杂法律任务时的表现，而不仅仅是与人类表现对比，以揭示人工智能在法律推理方面的根本局限性。

Method: 采用实验方法，将ChatGPT的表现与基于正则表达式（Regex）的基线方法进行比较，分析其在法律决策阅读和关键法律原则提取任务中的能力。

Result: 研究发现ChatGPT即使拥有必要的知识和能力，也无法有效整合这些能力进行推理以产生全面结果，在法律领域表现出明显的局限性。

Conclusion: 真正的智能需要具备分解复杂问题并根据多种所需能力进行处理的综合能力，在法律领域，这种全面的理解和推理能力目前仍然是人类独有的特质。
...
<details>
  <summary>Details</summary>
Motivation: 传统的先例推理假设先例集合必须一致，而广义理由模型放松了这一假设。目前已有基于传统一致理由模型的论证性解释方法，但尚未有针对广义推理框架的相应论证性解释方法。

Method: 扩展推导状态论证框架（DSA-framework），以解释基于广义理由模型的推理过程。

Result: 开发了能够处理不一致先例的广义理由模型的论证性解释框架。

Conclusion: 成功扩展了DSA框架，为广义理由模型提供了论证性解释方法，填补了该领域的研究空白。
...
Conclusion: CKA-RL通过有效积累和利用历史知识，成功缓解了灾难性遗忘问题，实现了跨任务的高效知识迁移。
...
TL;DR: NeSyPr是一个新颖的具身推理框架，通过神经符号程序化将知识编译为结构化、自适应和及时推理能力，使基于语言模型的智能体能够在动态环境中高效执行任务。


<details>
  <summary>Details</summary>
Motivation: 解决在动态环境中采用语言模型执行具身任务时面临的挑战，包括延迟、连接性和资源限制导致的在线访问大规模推理引擎或符号规划器的约束。

Method: 首先利用符号工具生成任务特定计划，然后将这些计划转换为可组合的程序表示，编码计划的隐式生产规则，使组合后的程序能够无缝集成到语言模型的推理过程中。

Result: 在PDDLGym、VirtualHome和ALFWorld等具身基准测试中，NeSyPr展示了比大规模推理模型和符号规划器更高效的推理能力，同时使用更紧凑的语言模型。

Conclusion: 神经符号程序化将多步符号结构化路径查找和推理抽象并泛化为单步语言模型推理，类似于人类知识编译，支持在延迟敏感和资源受限的物理系统中进行高效测试时推理。
...
<details>
  <summary>Details</summary>
Motivation: 现有的城市感知系统在多样化城市场景中的泛化能力有限，决策过程缺乏可解释性，需要开发能够适应动态条件并提供透明解释的系统。

Method: 采用混合训练免费框架，结合经典规划器生成基线解决方案，然后通过多智能体进化系统迭代优化，使感知任务分配适应动态城市条件和异构工作者偏好，同时生成自然语言解释。

Result: 在两个大规模移动数据集和七种动态干扰类型上的广泛实验表明，AgentSense在适应性和可解释性方面优于传统方法，相比单智能体LLM基线在性能和鲁棒性方面表现更优，并提供更合理透明的解释。
...
TL;DR: 开发了一个基于图论的吉他即兴演奏建议引擎，通过构建和弦音阶图并计算最短路径，为吉他学生提供和弦音独奏练习方案。


<details>
  <summary>Details</summary>
Motivation: 和弦音独奏是爵士吉他即兴演奏的基础技能，但学习和练习难度较大，需要系统化的工具来帮助学生掌握这一重要技巧。

Method: 1. 生成和弦音阶琶音；2. 构建加权图，节点代表和弦音阶；3. 计算相邻和弦节点间的边权重（基于最优过渡音）；4. 寻找图中的最短路径并重构独奏线条。

Result: 开发了一个用户友好的系统，能够处理输入和输出，为吉他学生提供可练习的和弦音独奏建议。

Conclusion: 该基于图论的引擎为吉他学生提供了有效的和弦音独奏练习工具，有助于掌握爵士吉他即兴演奏的基础技能。
...
<details>
  <summary>Details</summary>
Motivation: 电子竞技观众和玩家的增加，以及优化通信解决方案和云计算技术的发展，推动了在线游戏行业的持续增长。尽管基于人工智能的电子竞技分析传统上被定义为从相关数据中提取有意义的模式并进行可视化以增强决策，但专业胜率预测的大部分工作都集中在批处理视角的分类方面，而忽略了可视化技术。

Method: 开发了一种可解释的胜率预测分类解决方案，在流式处理中通过多个滑动窗口控制输入数据以反映相关的游戏变化。
...
TL;DR: RLIE是一个将大语言模型与概率建模相结合的统一框架，用于学习加权规则集，通过四个阶段实现规则生成、权重学习、迭代优化和评估，发现直接使用学习到的权重应用规则比通过提示LLM获得更优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的规则学习方法大多忽略规则间的相互作用，且未充分探索将LLM与概率规则学习结合进行鲁棒推理的机会。

Method: RLIE框架包含四个阶段：1) LLM生成和筛选候选规则；2) 逻辑回归学习概率权重进行全局选择和校准；3) 基于预测误差迭代优化规则集；4) 评估加权规则集作为直接分类器的性能。

Result: 在真实数据集上的评估表明，直接使用学习到的权重应用规则可获得优越性能，而通过提示LLM输入规则、权重和逻辑模型输出反而会降低准确性。
...
TL;DR: Memo是一个基于transformer的架构和训练方法，用于解决强化学习中记忆密集型、长时程任务的挑战。它通过在训练时插入周期性总结标记来创建和检索记忆，在网格世界元强化学习基准和真实室内多目标导航任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注具有固定大小记忆的循环模型或完全依赖上下文的transformer，而视觉输入常常超出transformer的上下文限制。人类能够维护和利用压缩为记忆的终身经验，因此需要开发能够形成和访问记忆的模型，使具身智能体能够在长时间内有效运作。

Method: Memo架构在训练期间将周期性总结标记与模型输入交错插入，实现记忆的创建和检索。这种方法比简单的长上下文transformer基线更高效，同时保持计算和存储效率。

Result: Memo在网格世界元强化学习基准和真实室内多目标导航任务中优于简单的长上下文transformer基线。此外，Memo在推理时对更长上下文具有更好的泛化能力，并在流式设置中保持鲁棒性。

Conclusion: Memo提供了一种有效的解决方案，用于处理强化学习中记忆密集型、长时程任务，通过记忆创建和检索机制实现了更好的性能和效率。
...
### [138] [Beyond Reactivity: Measuring Proactive Problem Solving in LLM Agents](https://arxiv.org/abs/2510.19771)
*Gil Pasternak,Dheeraj Rajagopal,Julia White,Dhruv Atreja,Matthew Thomas,George Hurn-Maloney,Ash Lewis*

Main category: cs.AI

TL;DR: PROBE是一个评估LLM智能体主动性的新基准，通过三个核心能力（搜索未指定问题、识别具体瓶颈、执行适当解决方案）来衡量智能体的主动性表现。测试显示即使是顶尖模型在该基准上的端到端性能也只有40%。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM智能体主动性的基准受限于局部上下文，无法测试跨来源和长时间跨度的推理能力，因此需要一个新的评估框架来填补这一空白。

Method: 提出PROBE基准，将主动性分解为三个核心能力：搜索未指定问题、识别具体瓶颈、执行适当解决方案，并应用于评估领先的LLM和流行智能体框架。

Result: 测试结果显示，即使是GPT-5和Claude Opus-4.1等最先进的模型，在PROBE基准上的端到端性能也仅为40%，表明当前自主行动系统存在明显局限性。
...
### [139] [Benchmarking World-Model Learning](https://arxiv.org/abs/2510.19788)
...
TL;DR: WorldTest是一个评估模型学习智能体的协议，将无奖励交互与不同但相关环境中的评分测试阶段分离。通过AutumnBench套件进行实例化，发现人类表现优于前沿模型，且计算扩展仅在部分环境中提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型学习和评估方法偏离了目标：训练和评估都锚定在下一帧预测上，成功标准是在相同环境中最大化奖励。需要一种能够评估智能体对环境动态学习程度的方法。

Method: 提出WorldTest协议，包含无奖励探索、衍生测试和行为评分三个模板。实例化为AutumnBench套件，包含43个交互式网格世界环境和129个任务，涵盖掩码帧预测、规划和因果动态变化预测三个家族。

Result: 比较了517名人类参与者和三个前沿模型在AutumnBench上的表现。人类表现优于模型，计算扩展仅在部分环境中提升性能，而非所有环境。

Conclusion: WorldTest提供了一个新颖的模板来评估智能体对环境动态的学习程度，AutumnBench揭示了世界模型学习中的显著提升空间。
